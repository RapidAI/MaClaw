...
- 仅支持 GitHub Release 中精确允许的四种签名固件包：EchoEar 2ST、Bread Compact、Fangtang 4G、Waveshare S3 Touch AMOLED 1.75C（32 MiB）。
- Release 下载具备 HTTPS 主机限制、缓存锁、断点续传、SHA-256、GitHub digest 与嵌入式发布签名验证。
- 仅 nonce-bound protocol:2 运行时身份可自动匹配和预取固件；protocol:1 仅保留诊断兼容，工具会明确阻止自动选择，避免下载后在写入前才失败。实际刷写始终要求用户确认实物板型。

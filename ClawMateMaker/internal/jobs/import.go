...
func NewImportJob(logRoot, boardID, packagePath string, trust firmware.TrustStore, emit func(logging.Event)) (*ImportJob, error) {
	...
	return &ImportJob{boardID: boardID, packagePath: packagePath, jobID: jobID, attemptID: attemptID, trust: trust, log: w}, nil
}

func (j *ImportJob) Run(ctx context.Context) (result catalog.DownloadedRelease, retErr error) {
	started := time.Now()
	result.JobID = j.jobID
	j.log.Event(logging.Info, "import", "job", "JOB_CREATED", "job.created", "Starting offline signed firmware package import.", map[string]any{"boardId": j.boardID})
	defer func() {
		if retErr != nil {
	...
	result = catalog.DownloadedRelease{JobID: j.jobID, BoardID: profile.ID, BoardName: profile.Name, ReleaseTag: "offline", AssetName: filepath.Base(j.packagePath), Path: j.packagePath, Size: info.Size(), SHA256: verified.ArchiveSHA256, InstallStatus: "verified_ready", InstallPlan: plan.Mode, PreservesUserData: plan.PreservesUserData, RequiresRecovery: plan.RequiresRecovery, SafetyNote: plan.Summary + " " + plan.Warning}
	j.log.Event(logging.Info, "import", "firmware", "OFFLINE_SIGNATURE_VERIFIED", "offline.signature.verified", "Offline firmware archive, signature, board target, and install impact were verified.", map[string]any{"boardId": j.boardID, "sha256": result.SHA256})
	return result, nil
}

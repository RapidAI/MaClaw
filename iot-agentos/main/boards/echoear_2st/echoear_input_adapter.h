...
/* EchoEar's physical activation-key contract.  Keep this deliberately
 * separate from echoear_hardware_adapter.h: input scheduling and GPIO state
 * do not require panel, codec or I2C controller state in this translation
 * unit. */

#include "sdkconfig.h"

#if !CONFIG_MACLAW_BOARD_ECHOEAR_2ST
#error "EchoEar input adapter selected for a non-EchoEar build"
#endif
...
static inline const round_input_profile_t *echoear_input_profile(void) {
...
static inline bool echoear_input_activate_key_pressed(void) {
    return gpio_get_level(ECHOEAR_INPUT_BUTTON_GPIO) == 0;
}

static inline device_input_source_t echoear_input_resolve_source(bool key_pressed,
                                                                  bool touch_pressed) {
    (void)key_pressed;
    return touch_pressed ? DEVICE_INPUT_SOURCE_TOUCH : DEVICE_INPUT_SOURCE_AUXILIARY_CONTROL;
}

static inline bool echoear_input_consume_boot_gesture(device_input_action_t action,
...
static inline BaseType_t echoear_input_start_scan_task(TaskFunction_t entry,
                                                        TaskHandle_t *out_task) {
    if (!entry || !out_task) return pdFAIL;
    return xTaskCreate(entry, "maclaw_echoear_input", 3072, NULL, 4, out_task);
}

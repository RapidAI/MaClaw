...
/* The common scene renderer sees only normalized physical facts through the
 * private Display service.  Keep the stripe limit beside the static DMA
 * staging source that it sizes; visual layout must not own this constraint. */
static inline int round_display_adapter_width(void) { return ECHOEAR_DISPLAY_WIDTH; }
static inline int round_display_adapter_height(void) { return ECHOEAR_DISPLAY_HEIGHT; }
static inline int round_display_adapter_transfer_stripe_rows(void) {
    return ECHOEAR_DISPLAY_STRIPE_ROWS;
}
static inline uint32_t round_display_adapter_pet_animation_frame_ms(void) {
    return 150u;
}

#if defined(MACLAW_ROUND_DISPLAY_ADAPTER_IMPLEMENTATION)

static spi_bus_config_t echoear_display_bus_config(void) {
    return (spi_bus_config_t)ST77916_PANEL_BUS_QSPI_CONFIG(
        ECHOEAR_DISPLAY_SCLK, ECHOEAR_DISPLAY_DATA0, ECHOEAR_DISPLAY_DATA1,
        ECHOEAR_DISPLAY_DATA2, ECHOEAR_DISPLAY_DATA3,
        ECHOEAR_DISPLAY_WIDTH * ECHOEAR_DISPLAY_STRIPE_ROWS * sizeof(uint16_t));
}
...
static spi_bus_config_t round_display_adapter_bus_config(void) {
    return echoear_display_bus_config();
}
...
#endif /* MACLAW_ROUND_DISPLAY_ADAPTER_IMPLEMENTATION */

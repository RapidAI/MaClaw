...
device_status_t platform_input_profile_start(platform_input_profile_publish_cb_t on_input,
    ...
    return round_status_from_esp_err(legacy_bootstrap_input_start_scanner(on_input, context));
}

device_status_t platform_input_profile_stop(uint32_t timeout_ms) {
    return round_status_from_esp_err(legacy_bootstrap_input_stop_scanner(timeout_ms));
}

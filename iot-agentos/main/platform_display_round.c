...
device_status_t platform_display_profile_set_brightness(uint8_t percent) { return round_status_from_esp_err(legacy_display_scene_set_brightness(percent)); }
device_status_t platform_display_profile_enter_display_off(void) { return round_status_from_esp_err(legacy_display_scene_enter_display_off()); }
device_status_t platform_display_profile_wake_display(void) { return round_status_from_esp_err(legacy_display_scene_wake_display()); }
device_status_t platform_display_profile_prepare_system_sleep(uint32_t timeout_ms) {
    const int64_t deadline_us = esp_timer_get_time() + (int64_t)timeout_ms * 1000;
    esp_err_t status = round_display_service_prepare_system_sleep(timeout_ms);
    if (status != ESP_OK) return round_status_from_esp_err(status);
    const int64_t remaining_us = deadline_us - esp_timer_get_time();
    const uint32_t remaining_ms = remaining_us <= 0 ? 0 :
        (uint32_t)(((uint64_t)remaining_us + 999u) / 1000u);
    status = remaining_ms
                 ? round_display_service_wait_for_scanout_idle(remaining_ms)
                 : ESP_ERR_TIMEOUT;
    if (status == ESP_OK) return DEVICE_STATUS_OK;
    /* Keep the private animation/scan-out fence closed until Power's common
     * reverse-order rollback; this path must not restart rendering ahead of
     * sibling System Sleep participants. */
    return round_status_from_esp_err(status);
}
void platform_display_profile_abort_system_sleep_prepare(void) {
    round_display_service_abort_system_sleep_prepare();
}
...
device_status_t platform_display_profile_set_pet_asset(const uint8_t *const *frames, uint32_t frame_count, uint32_t width, uint32_t height, uint32_t frame_ms) { return round_status_from_esp_err(legacy_display_scene_set_pet_asset(frames, frame_count, width, height, frame_ms)); }
device_status_t platform_display_profile_set_pet_asset_consuming(uint8_t **frames, uint32_t frame_count, uint32_t width, uint32_t height, uint32_t frame_ms) { return round_status_from_esp_err(legacy_display_scene_set_pet_asset_consuming(frames, frame_count, width, height, frame_ms)); }

...
// It is retained only as a fallback for wiring contexts where no machine sender
// is available; the real delivery path is HubApprovalDispatcher (below), which
// the router wires in via NewHubApprovalDispatcher.
...
// given machine sender (e.g. *device.Service). The router wires this in place
// of noopApprovalDispatcher.
...
// returns true. It is retained only as a fallback for wiring contexts where no
// presence source is available; the real source is HubAvailabilityChecker
// (below), which the router wires in via NewHubAvailabilityChecker.
...
// given presence source (e.g. *device.Service). The router wires this in place
// of noopAvailabilityChecker.

...
// fresh mobile app sign in as the same already-registered Hub user.
...
func mobilePrincipalHasVerifiedPhoneIdentity(ctx context.Context, identity *auth.IdentityService, principal *auth.ViewerPrincipal) bool {
	...
	if strings.HasPrefix(strings.ToLower(strings.TrimSpace(principal.Email)), "phone:") {
		return true
	}
	...
	identities, err := repo.ListIdentitiesByUser(auth.WithTenant(ctx, principal.TenantID), principal.TenantID, principal.UserID)
	if err != nil {
		return false
	}
	for _, item := range identities {
		...
		if strings.EqualFold(strings.TrimSpace(item.Type), "phone") && item.Verified && strings.TrimSpace(item.Value) != "" {
			return true
		}
	}
	return false
...
func parseMobileDesktopLlmQRPayload(raw string) (mobileDesktopLlmQRPayload, error) {
	...
	if payload.Type != mobileDesktopLlmAuthorizationQRType && payload.Type != mobileDesktopAuthAuthorizationQRType {
...
func mobileLlmAuthorizationFromQRSession(principal *auth.ViewerPrincipal, payload mobileDesktopLlmQRPayload, now time.Time, allowSessionAccount bool) (mobileLlmAuthorizationRecord, error) {
	...
	if payload.Type == mobileDesktopAuthAuthorizationQRType && session.Purpose != mobileQRSessionPurposeAuth {
		mobileLlmAuthorizations.Unlock()
		return mobileLlmAuthorizationRecord{}, fmt.Errorf("desktop QR session is not a mobile authentication session")
	}
	if payload.Type == mobileDesktopLlmAuthorizationQRType && session.Purpose == mobileQRSessionPurposeAuth {
		mobileLlmAuthorizations.Unlock()

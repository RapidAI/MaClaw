...
function renderLLMEndpointAccessLogDialog() {
  ...
  root.innerHTML = logs.map(function(item) {
    var tokens = [Number(item.input_tokens || 0), Number(item.output_tokens || 0), Number(item.total_tokens || 0)].join(' / ');
...
function exportLLMEndpointAccessLogsCSV() {
  ...
  var rows = [['time','provider','upstream_host','client_ip','email','requested_model','authorized_model','input_tokens','output_tokens','total_tokens','status','error_code','request_body']];

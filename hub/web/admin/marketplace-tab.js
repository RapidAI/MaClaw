...
(function(global) {
  ...
  function renderCapabilities() {
    ...
    const activeCapabilities = state.capabilities.filter(function(item) { return item.status !== 'deleted'; });
    if (!activeCapabilities.length) { root.innerHTML = '<div class="hint" style="grid-column:1/-1">' + esc(mp('marketplaceNoCapabilities')) + '</div>'; return; }
    const maxShow = 20;
    const items = activeCapabilities.slice(0, maxShow);
    const hasMore = activeCapabilities.length > maxShow;

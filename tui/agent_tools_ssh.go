...
func sshToolDefinitions() []map[string]interface{} {
	return []map[string]interface{}{
		toolDef("ssh", "SSH 远程服务器管理（connect/exec/exec_background/check_task/list_tasks/kill_task/sudo_prepare/upload/download/list/close）", map[string]interface{}{
			...
			"auth_method":     map[string]interface{}{"type": "string", "description": "认证方式: password/key/agent。当用户提供了密码时必须设为 password"},
			"key_path":        map[string]interface{}{"type": "string", "description": "私钥路径（auth_method=key 时可选）"},
			"password":        map[string]interface{}{"type": "string", "description": "SSH 登录密码。当用户提供了密码时必须传此参数，不要省略"},

...
func TestCenterUsageStore_CreateTable(t *testing.T) {
	db := setupTestDB(t)
...
func TestCenterUsageStore_RecordAndQuery(t *testing.T) {
	db := setupTestDB(t)
	store := NewUsageStore(db)
	ctx := context.Background()
	if err := store.CreateUsageTable(ctx); err != nil {
	...
	got := records[0]
	if got.DiWorkerID != "dw-1" {
		t.Errorf("unexpected diworker: %s", got.DiWorkerID)
	}
...
func TestCenterUsageStore_QueryFilter(t *testing.T) {
	db := setupTestDB(t)
	store := NewUsageStore(db)
	ctx := context.Background()
	if err := store.CreateUsageTable(ctx); err != nil {
...
func TestCenterUsageStore_AutoIDAndTimestamp(t *testing.T) {
	db := setupTestDB(t)
	store := NewUsageStore(db)
	ctx := context.Background()
	if err := store.CreateUsageTable(ctx); err != nil {

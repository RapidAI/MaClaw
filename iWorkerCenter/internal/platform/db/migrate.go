...
var migrations = []string{
	...
	// 37: iWorkerCenter local skill market publication metadata
	`ALTER TABLE capability_packages ADD COLUMN local_skill_origin TEXT NOT NULL DEFAULT 'local';
	ALTER TABLE capability_packages ADD COLUMN cloud_publish_status TEXT NOT NULL DEFAULT '';
	ALTER TABLE capability_packages ADD COLUMN cloud_skill_id TEXT NOT NULL DEFAULT '';
	ALTER TABLE capability_packages ADD COLUMN cloud_published_at TEXT NOT NULL DEFAULT '';
	ALTER TABLE capability_packages ADD COLUMN cloud_publish_error TEXT NOT NULL DEFAULT '';
	CREATE INDEX IF NOT EXISTS idx_capability_cloud_publish ON capability_packages(tenant_id, cloud_publish_status);`,

	// 38: human safety intervention metadata for the self-evolving skill market
	`ALTER TABLE capability_packages ADD COLUMN safety_status TEXT NOT NULL DEFAULT '';

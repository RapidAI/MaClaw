...
export function LoginPage({ onLogin }: LoginPageProps) {
  ...
  if (tenantsLoading) {
    return <div style={containerStyle}><div style={{ color: '#888' }}>加载中...</div></div>;
  }
      ...
      <div style={cardStyle}>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div style={fieldStyle}>
            ...
            <input type="password" value={password} onChange={e => setPassword(e.target.value)} style={inputStyle} placeholder="输入密码" />
          </div>
          <div style={fieldStyle}>
            <label style={labelStyle}>
              验证码：{captcha ? <span style={{ fontWeight: 600, color: '#1a1a1a', fontSize: 15 }}>{captcha.question}</span> : '加载中...'}
              <button type="button" onClick={loadCaptcha} style={refreshBtnStyle}>刷新</button>
            </label>
            <input type="number" value={captchaAnswer} onChange={e => setCaptchaAnswer(e.target.value)} style={inputStyle} placeholder="输入计算结果" />
          </div>
          {error && <p style={{ color: '#c33', fontSize: 13, margin: '0 0 12px' }}>{error}</p>}
          <button type="submit" disabled={loading} style={submitBtnStyle}>{loading ? '登录中...' : '登录'}</button>
        </form>
      </div>
    </div>
  );

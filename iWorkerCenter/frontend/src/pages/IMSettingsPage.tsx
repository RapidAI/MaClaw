...
export function IMSettingsPage() {
  ...
  const load = async () => {
    ...
    try {
      ...
      // Desktop fallback is best-effort.
    }
  };
  ...
  const save = async () => {
    ...
    setMessage(ok ? { kind: 'ok', text: t('IM 配置已保存。', 'IM configuration saved.') } : { kind: 'danger', text: t('保存 IM 配置失败。', 'Failed to save IM configuration.') });
    setSaving(false);
  };

  return <div className="center-page-stack">
    <SectionCard title={t('IM 网关', 'IM Gateways')} desc={t('配置企业通讯入口，让 iWorker 把人工确认、异常提醒和协作消息推送到员工常用工具。配置保存在本地 Center，Cloud 不参与企业消息内容。', 'Configure enterprise messaging entrances so iWorkers can push human confirmations, exception reminders, and collaboration messages to employee tools. Settings are stored locally in Center; Cloud does not participate in enterprise message content.')}>
      <div className="cloud-status-grid">
        <StatusTile label={t('已启用网关', 'Enabled gateways')} value={String(enabledCount)} tone={enabledCount ? 'ok' : 'warn'} />
        <StatusTile label={t('回调入口', 'Webhook entrances')} value="3" />
        <StatusTile label={t('人工提醒', 'Human reminders')} value={t('本地推送', 'Local push')} tone="ok" />
      </div>
      <div className="cloud-continuity-grid">
        <article className="cloud-continuity-card ok"><strong>{t('提醒式干预', 'Reminder-style intervention')}</strong><p>{t('当 iWorker 自动运行需要人类员工确认时，Center 可通过启用的 IM 网关发送可见提醒。', 'When autonomous iWorker execution needs human confirmation, Center can send visible reminders through enabled IM gateways.')}</p></article>
        <article className="cloud-continuity-card"><strong>{t('业务隔离', 'Business isolation')}</strong><p>{t('消息凭据和业务提醒留在本地 Center；iWorkerCloud 只处理注册、授权和算力协调。', 'Messaging credentials and business reminders stay in local Center; iWorkerCloud only handles registration, licensing, and compute coordination.')}</p></article>
      </div>
      <div className="auth-adapter-tabs">
        <button className={tab === 'feishu' ? 'cloud-primary' : 'btn-secondary'} onClick={() => setTab('feishu')}>{t('飞书', 'Feishu')}</button>
        <button className={tab === 'dingtalk' ? 'cloud-primary' : 'btn-secondary'} onClick={() => setTab('dingtalk')}>{t('钉钉', 'DingTalk')}</button>
        <button className={tab === 'wecom' ? 'cloud-primary' : 'btn-secondary'} onClick={() => setTab('wecom')}>{t('企业微信', 'WeCom')}</button>
      </div>
      {message && <p className={'cloud-message ' + message.kind}>{message.text}</p>}
    </SectionCard>

    {tab === 'feishu' && <SectionCard title={t('飞书机器人配置', 'Feishu Bot')} desc={t('Webhook 地址：/webhook/feishu。用于接收事件回调和主动发送协作提醒。', 'Webhook: /webhook/feishu. Used for event callbacks and proactive collaboration reminders.')}>
    ...
    {tab === 'dingtalk' && <SectionCard title={t('钉钉机器人配置', 'DingTalk Bot')} desc={t('Webhook 地址：/webhook/dingtalk。用于企业内部机器人消息和人工干预提醒。', 'Webhook: /webhook/dingtalk. Used for internal bot messages and human intervention reminders.')}>
      <div className="cloud-form-grid">
    ...
    {tab === 'wecom' && <SectionCard title={t('企业微信配置', 'WeCom')} desc={t('Webhook 地址：/webhook/wecom。用于企业微信自建应用消息和回调事件。', 'Webhook: /webhook/wecom. Used for WeCom app messages and callback events.')}>
      <div className="cloud-form-grid">
    ...
    <SectionCard title={t('保存配置', 'Save Configuration')} desc={t('保存后，iWorker 的人工确认、任务异常和协作提醒可通过启用的网关发送给员工。', 'After saving, iWorker human confirmations, task exceptions, and collaboration reminders can be sent through enabled gateways.')}>
      <div className="cloud-actions">
        <button className="cloud-primary" onClick={save} disabled={saving}>{saving ? t('保存中...', 'Saving...') : t('保存 IM 配置', 'Save IM settings')}</button>
      </div>
    </SectionCard>
  </div>;
}
...
function EnabledField({ enabled, onChange }: { enabled: boolean; onChange: (enabled: boolean) => void }) {
  const { t } = useI18n();
  return <label className="cloud-field"><span>{t('启用状态', 'Enabled')}</span><select value={enabled ? 'yes' : 'no'} onChange={e => onChange(e.target.value === 'yes')}><option value="yes">{t('启用', 'Enabled')}</option><option value="no">{t('停用', 'Disabled')}</option></select></label>;
}

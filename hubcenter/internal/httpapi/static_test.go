...
func TestWebPagesKeepInteractiveAccessibilityContracts(t *testing.T) {
	...
	for _, required := range []string{
		...
		// Catalog sub-tabs use a loop over button ids (skill/mcp/workflow/app).
		`catalogSubTabSkill`,
		`catalogSubTabMCP`,
	...
	for _, required := range []string{
		...
		`async function apiHubAdminAction`,
		`async function apiDeleteHubAdminAction`,
		`apiHubAdminAction('/api/admin/hubs/visibility'`,
		`apiHubAdminAction('/api/admin/hubs/registration-policy'`,
		`apiHubAdminAction('/api/admin/hubs/digital-employee-authorization'`,
		`disable?'/api/admin/hubs/disable':'/api/admin/hubs/enable'`,
		`apiHubAdminAction('/api/admin/hubs/confirm'`,
		`apiDeleteHubAdminAction(id`,
		...
		`if(!single)return []`,
		`function groupEnterpriseMailDomainItemsByHub`,
		`function selectedEnterpriseMailTenant`,
		`function setEnterpriseMailTenant`,
		`class="enterprise-mail-tenant-select"`,
		`enterpriseMailDomainsTitle:'已注册企业邮件域'`,
		`hubRegDefaultsTitle:'节点注册默认项'`,
		`hubTenantRegPolicyTitle:'租户注册策略'`,
		`function updateHubTenantRegistrationPolicy`,
		`function parseHubInviteUses`,
		`function saveHubRegistrationPolicy`,
		`hubRegPolicySavedRefreshFailed`,
		`skillhubTitle:'能力目录'`,

...
// Gossip Management
    ...
    function skillhubShowVersionHistory(idx){
      ...
      overlay.className='trust-dialog-backdrop';dialog.className='trust-dialog skill-version-dialog';dialog.setAttribute('role','dialog');dialog.setAttribute('aria-modal','true');
      var title=document.createElement('h3');title.textContent=(skill.maclaw_app_name||skill.display_name||skill.name||'-')+' · '+(currentLang==='zh'?'历史版本':'Version history');
      var description=document.createElement('p');description.textContent=currentLang==='zh'?'点击一个版本查看完整详情。':'Select a version to view its complete details.';
      ...
      skill.version_history.forEach(function(version,index){var row=document.createElement('button');row.type='button';row.className='skill-version-row';var versionLabel=version.semver||version.version||'-';row.innerHTML='<strong>v'+escapeHtml(versionLabel)+(index===0?' · '+(currentLang==='zh'?'当前版本':'Current'):'')+'</strong><span>'+escapeHtml(fmtDate(version.updated_at||version.created_at))+'</span>';row.addEventListener('click',function(){skillhubLoadVersionDetail(version,detail)});list.appendChild(row)});

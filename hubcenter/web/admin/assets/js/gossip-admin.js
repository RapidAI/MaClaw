...
// Gossip Management
    ...
    function renderSkillHubCard(s, idx) {
      ...
      var versionCount = Number(s.version_count || 1);
    ...
    function skillhubShowVersionHistory(idx){
      var skill=_skillhubSkills[idx];if(!skill||!Array.isArray(skill.version_history)||skill.version_history.length<2)return;
      var previousFocus=document.activeElement,overlay=document.createElement('div'),dialog=document.createElement('section');
      overlay.className='trust-dialog-backdrop';dialog.className='trust-dialog skill-version-dialog';dialog.setAttribute('role','dialog');dialog.setAttribute('aria-modal','true');dialog.setAttribute('aria-labelledby','skillVersionDialogTitle');dialog.setAttribute('aria-describedby','skillVersionDialogDescription');
      var title=document.createElement('h3');title.id='skillVersionDialogTitle';title.textContent=(skill.maclaw_app_name||skill.display_name||skill.name||'-')+' · '+(currentLang==='zh'?'历史版本':'Version history');
      var description=document.createElement('p');description.id='skillVersionDialogDescription';description.textContent=currentLang==='zh'?'选择一个版本查看完整详情。':'Select a version to view its complete details.';
      var list=document.createElement('div');list.className='skill-version-list';var detail=document.createElement('div');detail.className='skill-version-detail';detail.hidden=true;
      skill.version_history.forEach(function(version,index){var row=document.createElement('button');row.type='button';row.className='skill-version-row';row.setAttribute('aria-pressed','false');var versionLabel=version.semver||version.version||'-';row.innerHTML='<strong>v'+escapeHtml(versionLabel)+(index===0?' · '+(currentLang==='zh'?'当前版本':'Current'):'')+'</strong><span>'+escapeHtml(fmtDate(version.updated_at||version.created_at))+'</span>';row.addEventListener('click',function(){Array.prototype.forEach.call(list.children,function(item){item.setAttribute('aria-pressed','false')});row.setAttribute('aria-pressed','true');skillhubLoadVersionDetail(version,detail)});list.appendChild(row)});
      var actions=document.createElement('div');actions.className='trust-dialog-actions';var close=document.createElement('button');close.type='button';close.className='btn-ghost';close.textContent=currentLang==='zh'?'关闭':'Close';actions.append(close);dialog.append(title,description,list,detail,actions);overlay.append(dialog);document.body.append(overlay);
      function dismiss(){document.removeEventListener('keydown',onKey);overlay.remove();if(previousFocus&&previousFocus.focus)previousFocus.focus()}function onKey(event){if(event.key==='Escape'){event.preventDefault();dismiss()}}document.addEventListener('keydown',onKey);overlay.addEventListener('pointerdown',function(event){if(event.target===overlay)dismiss()});close.addEventListener('click',dismiss);close.focus();
    }
    function skillhubLoadVersionDetail(version,target){target.hidden=false;target.textContent=currentLang==='zh'?'正在加载详情…':'Loading details…';api('/api/admin/skillhub/'+encodeURIComponent(version.id)).then(function(detail){var label=detail.semver||detail.version||version.semver||version.version||'-';target.innerHTML='<h4>v'+escapeHtml(label)+'</h4><p>'+escapeHtml(detail.description||shtr('noDescription'))+'</p><dl><dt>'+(currentLang==='zh'?'状态':'Status')+'</dt><dd>'+escapeHtml(detail.status||'-')+'</dd><dt>'+(currentLang==='zh'?'更新时间':'Updated')+'</dt><dd>'+escapeHtml(fmtDate(detail.updated_at||detail.created_at))+'</dd><dt>'+(currentLang==='zh'?'可见性':'Visibility')+'</dt><dd>'+escapeHtml(detail.visible===false?(currentLang==='zh'?'已隐藏':'Hidden'):(currentLang==='zh'?'已显示':'Visible'))+'</dd></dl>'}).catch(function(err){target.textContent=String(err.message||err)})}

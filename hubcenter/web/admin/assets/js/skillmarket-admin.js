...
function smRenderReviewDetail(s) {
  ...
  const manifestHTML = s.maclaw_app_manifest_preview ? `<div class="item-meta"><strong>${escapeHtml(smAdminLabel('manifestPreview'))}</strong></div><pre style="margin:8px 0 0;padding:10px;border:1px solid rgba(31,34,48,.07);border-radius:12px;background:rgba(255,255,255,.78);max-height:220px;overflow:auto;font:12px/1.55 Consolas,monospace;white-space:pre-wrap;color:var(--text)">${escapeHtml(JSON.stringify(s.maclaw_app_manifest_preview, null, 2))}</pre>` : '';

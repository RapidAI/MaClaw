...
(() => {
      ...
      let serverConfigInFlight = null;
      let mailConfigInFlight = null;
      let setupGateItems = [];
      ...
      window.loadServerConfig = async function() {
        ...
        serverConfigInFlight = (async function() {
          try { const data = await api('/api/admin/server/config'); serverConfigCache = data || {}; document.getElementById('serverPublicBaseURL').value = data.public_base_url || ''; evaluateSetupGate(); }
          catch (err) { const msg = serverText('loadFailed').replace('{error}', err.message); setOutput(msg); showToast(msg, 'error'); }
        })();
        try { return await serverConfigInFlight; }
        finally { serverConfigInFlight = null; }
      };
      ...
      window.loadMailConfig = async function() {
        if (mailConfigInFlight) return mailConfigInFlight;
        mailConfigInFlight = (async function() {
          try { const data = await api('/api/admin/mail/config'); mailConfigCache = data || {}; renderMailConfig(mailConfigCache); evaluateSetupGate(); }
          catch (err) { const msg = mailText('loadFailed').replace('{error}', err.message); setOutput(msg); showToast(msg, 'error'); }
        })();
        try { return await mailConfigInFlight; }
        finally { mailConfigInFlight = null; }
      };

...
// LLM↔Skill parameter name gap: when a skill uses {{input}} but the LLM
...
var commonParamAliases = map[string][]string{
	...
	"message": {"text", "content", "prompt", "msg"},
...
// rather than explicitly declared. The LLM context injection annotates these
// as "(从模板推断)" to signal that the name may not be semantically ideal.
//

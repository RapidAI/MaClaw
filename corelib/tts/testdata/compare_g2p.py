#!/usr/bin/env python3
"""Compare Go G2P output with sherpa-onnx's lexicon-based G2P."""
import os
...
def text_to_ids_lexicon(text):
    """Convert text to phoneme IDs using the lexicon (same as sherpa-onnx)."""
    ids = [pid_map["^"]]
    for i, ch in enumerate(text):
        if ch in lexicon:
            phones = lexicon[ch]
            if i > 0 and text[i-1] in lexicon:
                ids.append(pid_map["_"])  # word boundary
            for ph in phones:
                if ph == "_":
                    continue  # skip trailing _
                if ph in pid_map:
...
# Go's G2P output (from piper_test output)
go_outputs = {
...
for text in texts:
    lex_ids = text_to_ids_lexicon(text)
    
    print(f"\n=== {text} ===")
    print(f"  Lexicon IDs ({len(lex_ids)}): {lex_ids}")
    
    if text in go_outputs:

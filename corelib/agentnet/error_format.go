...
// ── AgentNet error message localisation ──────────────────────────────────
//
// Raw agentnet errors look like:
//   agentnet POST /api/tasks/.../claim: status 403: {"error":"cannot claim your own task"}
//
...
// knownErrors maps the English error text returned by the agentnet node to
...
// reAgentNetHTTP matches the raw agentnet HTTP error format:
//   agentnet POST /api/...: status 403: {"error":"..."}
//   agentnet GET  /api/...: status 404: {"error":"..."}
var reAgentNetHTTP = regexp.MustCompile(
	`agentnet (?:GET|POST|DELETE|PUT|PATCH) \S+: status \d+: (.+)`)
...
// extractCoreMessage takes a raw agentnet error string and returns the
// human-readable core message (stripping HTTP method, path, status code).
func extractCoreMessage(raw string) string {
	if m := reAgentNetHTTP.FindStringSubmatch(raw); len(m) == 2 {
...
// FormatError takes a raw agentnet error, extracts the meaningful message,
...
// Example:
//
//	in:  "agentnet POST /api/tasks/xxx/claim: status 403: {\"error\":\"cannot claim your own task\"}"

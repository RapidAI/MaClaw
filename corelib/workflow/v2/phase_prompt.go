...
func BuildPhasePrompt(state *WorkflowState) string {
	...
	if phase.FormData != nil && len(phase.FormData) > 0 {
		sb.WriteString("## 用户提供的结构化信息\n\n")
		sb.WriteString("以下信息由用户通过表单提交，请基于这些信息生成本阶段文档：\n\n")
...
执行步骤：
1. **附图存在性确认**：
   - 使用 list_directory 列出附图文件所在目录，确认文件存在并记录文件名和大小
   - 注意：不要对图片文件（.png/.jpg/.bmp/.tif）使用 read_file——二进制文件无法以文本方式读取
2. **附图编号与命名**：
   - 按说明书引用顺序重新编号（图1、图2、图3...）
   - 每张图对应一个简要标题（根据前序阶段的技术方案和文件名推断）
3. **附图标记表**：
   - 基于前序阶段提炼的技术方案中描述的组件/部件，规划标记编号
   - 格式：标记号 → 组件名称（如：1-控制器，2-传感器，3-执行器）
   - 标记按出现顺序从小到大编号
4. **附图说明书段落**：
   - 按专利格式撰写完整的"附图说明"段落
   - 格式：图X是本发明实施例提供的...的结构/流程/示意图
5. **附图缺失分析**：
   - 根据权利要求书中的技术方案，判断是否需要补充附图
   - 建议补充的附图类型和内容描述
6. **附图规范提醒**：
   - 专利附图不应包含文字说明（用数字/字母标记代替）
   - 线条应清晰、比例协调
   - 图中各组件需有标记对应关系

## 重要约束
- 附图标记必须与技术方案中描述的组件一一对应。
- 如果附图文件不存在（list_directory 未找到），明确列出缺失清单。
- 不要猜测图片的具体视觉内容——基于文件名和技术方案描述来规划标记。

...
func appendCodingWorkflowRules(b *strings.Builder) {
	b.WriteString(`
## ⚠️ 编程任务工作流（极其重要）

### 第一步：识别任务类型
- 编程任务（Coding_Task）：明确需要修改项目代码、修 bug、重构、实现功能等 → 调用 create_session 启动远程编程工具
- SSH/服务器操作任务：登录服务器、执行远程命令、看日志、重启服务等 → 使用 ssh 工具
...
- 在用户确认任务列表之前，严禁调用 create_session

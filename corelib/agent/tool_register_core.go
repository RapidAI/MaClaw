...
func RegisterCoreTools(r *CoreToolRegistry, deps CoreToolDeps) {
	...
	r.Register(ToolEntry{
		Name:        "office",
		Description: "Office/PDF/text document tool. action: read_document (auto-detect .pdf/.doc/.docx/.xls/.xlsx/.csv/.ppt/.pptx and text files), read_excel, write_excel, read_pptx, write_pptx, preview_pptx, generate_pdf. Prefer read_document for mixed formats; read_excel is spreadsheet-only and read_pptx is PPTX-only. write_pptx automatically renders per-slide preview images next to the deck (<name>_preview/) so the visual result can be checked and fixed by rewriting; preview_pptx renders a .pptx deck to per-slide PNG/JPEG images on demand.",
		Properties: map[string]interface{}{
			"action":       map[string]string{"type": "string", "description": "read_document/read_doc/read_docx/read_pdf/read_excel/write_excel/read_pptx/write_pptx/preview_pptx/generate_pdf"},
			...
			"doc_type":     map[string]string{"type": "string", "description": workflowDocSchemaDocTypeDescription()},
			"phase_id":     map[string]string{"type": "string", "description": workflowDocSchemaPhaseIDDescription()},
			"file_path":    map[string]string{"type": "string", "description": "File path for read_*/write_excel/write_pptx/preview_pptx (alias: path)"},
			"path":         map[string]string{"type": "string", "description": "Alias for file_path"},
			...
			"max_slides":   map[string]string{"type": "integer", "description": "read_pptx max slides (default 100, max 500, 仅 .pptx); preview_pptx max slides to render (default all, max 200)"},
			"slide_offset": map[string]string{"type": "integer", "description": "read_pptx/preview_pptx zero-based slide offset"},
			"width":        map[string]string{"type": "integer", "description": "preview_pptx image width in pixels (default 1280, max 4096)"},
			"format":       map[string]string{"type": "string", "description": "preview_pptx image format: png (default) or jpeg"},
			"output_dir":   map[string]string{"type": "string", "description": "preview_pptx image output directory (default: <name>_preview next to the deck)"},
			"preview":      map[string]string{"type": "boolean", "description": "write_pptx: set false to skip the automatic per-slide preview render (default true)"},
			"draft":        map[string]string{"type": "boolean", "description": "preview_pptx: draft mode, faster with lower fidelity (default false)"},

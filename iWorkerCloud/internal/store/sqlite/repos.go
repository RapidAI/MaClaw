...
func (repo *CenterRepo) Create(ctx context.Context, c *store.Center) error {
	_, err := repo.w.ExecContext(ctx,
		...
		boolToInt(c.IWorkerReady), c.IWorkerReadinessStatus, c.IWorkerTenantCount, c.IWorkerRoleCount, c.IWorkerColleagueCount, c.IWorkerLocalAccountCount, c.IWorkerAgentInstanceCount, c.IWorkerReadinessJSON,
...
func (repo *CenterRepo) UpdateHeartbeat(ctx context.Context, c *store.Center) error {
	...
	_, err := repo.w.ExecContext(ctx,
		`UPDATE centers SET last_heartbeat=?, last_sync_status=?, iworker_ready=?, iworker_readiness_status=?, iworker_tenant_count=?, iworker_role_count=?, iworker_colleague_count=?, iworker_local_account_count=?, iworker_agent_instance_count=?, iworker_readiness_json=?, updated_at=? WHERE id=?`,
		now, c.LastSyncStatus, boolToInt(c.IWorkerReady), c.IWorkerReadinessStatus, c.IWorkerTenantCount, c.IWorkerRoleCount, c.IWorkerColleagueCount, c.IWorkerLocalAccountCount, c.IWorkerAgentInstanceCount, c.IWorkerReadinessJSON, now, c.ID)
	return err
}

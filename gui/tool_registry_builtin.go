...
func registerBuiltinTools(registry *ToolRegistry, h *IMMessageHandler) {
	...
	reg("generate_pdf", "将 Markdown 内容生成为排版精美的 PDF 文件。支持中文、标题、列表、粗体等格式。适用于生成需求文档、设计文档、报告、总结等。生成后可用 send_file 发送给用户或用 open 打开预览。",
		ToolCategoryBuiltin, []string{"pdf", "generate", "document", "export", "report"},
		map[string]interface{}{

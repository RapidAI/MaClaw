...
// detectMissingCoreTools performs a lightweight (<100ms) check for core runtime
// dependencies. Returns the name of the first missing tool, or "" if all present.
...
func (a *App) detectMissingCoreTools() string {
	...
	if _, err := exec.LookPath("git"); err != nil {
		// Also check standard install location (not in PATH but present on disk)
		if _, statErr := os.Stat(`C:\Program Files\Git\cmd\git.exe`); statErr != nil {
			return "Git"
		}
	}
...
func (a *App) installToolsInBackground() {
	...
	for i := 0; i < maxRetries; i++ {
		...
		npmTestCmd := exec.Command(npmExec, "--version")
		npmTestCmd.SysProcAttr = &syscall.SysProcAttr{CreationFlags: _CREATE_NO_WINDOW}
...
func (a *App) installNodeJS() error {
	...
	for elapsed < maxWaitTime {
		if _, err := os.Stat(nodePath); err == nil {
			...
			for npmElapsed < npmWaitTime {
				npmCmd := exec.Command("npm", "--version")
				npmCmd.SysProcAttr = &syscall.SysProcAttr{CreationFlags: _CREATE_NO_WINDOW}
				if err := npmCmd.Run(); err == nil {

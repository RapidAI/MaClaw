...
func (a *App) installToolsInBackground() {
	...
	for i := 0; i < maxRetries; i++ {
		...
		npmTestCmd := resolveNpmToNodeCmd(npmExec, []string{"--version"})
	...
	if !npmReady || npmExec == "" {
		a.log(a.tr("WARNING: npm not found after %d attempts. Background checks will continue, but npm-based tools may not install or update.", maxRetries))
	} else {
...
func (a *App) installNodeJS() error {
	...
	for elapsed < maxWaitTime {
		if _, err := os.Stat(nodePath); err == nil {
			...
			for npmElapsed < npmWaitTime {
				npmCmd := resolveNpmToNodeCmd("npm", []string{"--version"})
...
func createVersionCmd(path string) *exec.Cmd {
	cmd := resolveNodeCmdWrapper(path, []string{"--version"})
	cmd.SysProcAttr = &syscall.SysProcAttr{HideWindow: true}
	return cmd
}

// resolveNodeCmdWrapper checks if a .cmd/.bat file is a Node.js wrapper script
// (generated by npm) and bypasses it by calling node.exe directly with the
// target .js file. This prevents the cmd.exe subprocess from flashing a
// visible console window.
func resolveNodeCmdWrapper(cmdPath string, args []string) *exec.Cmd {
	lower := strings.ToLower(cmdPath)
	if !strings.HasSuffix(lower, ".cmd") && !strings.HasSuffix(lower, ".bat") {
		return exec.Command(cmdPath, args...)
	}

	cmdDir := filepath.Dir(cmdPath)
	nodeExe := filepath.Join(cmdDir, "node.exe")
	if _, err := os.Stat(nodeExe); err != nil {
		return exec.Command(cmdPath, args...)
	}

	// Read the .cmd file to find the target script.
	content, err := os.ReadFile(cmdPath)
	if err != nil {
		return exec.Command(cmdPath, args...)
	}

	// npm-generated .cmd wrappers end with a line like:
	//   endLocal & goto #_undefined_# 2>NUL || title %COMSPEC% & "%_prog%" "%dp0%\node_modules\pkg\bin\script" %*
	// or for npm.cmd:
	//   "%NODE_EXE%" "%NPM_CLI_JS%" %*
	// We look for the script path referenced via %dp0% or %~dp0.
	scriptPath := extractNodeScriptFromCmd(string(content), cmdDir)
	if scriptPath == "" {
		return exec.Command(cmdPath, args...)
	}
	...
	return exec.Command(nodeExe, nodeArgs...)
}

// extractNodeScriptFromCmd parses an npm-generated .cmd wrapper to find the
// Node.js script path. Returns empty string if not found.
func extractNodeScriptFromCmd(content, cmdDir string) string {
	lines := strings.Split(content, "\n")

	// Scan from the end — the execution line is typically the last non-empty line.
	for i := len(lines) - 1; i >= 0; i-- {
		...
		// Look for %dp0% or %~dp0 references to a script path.
		// Pattern: "%dp0%\node_modules\...\script" or "%~dp0\node_modules\...\script.js"
		scriptRef := ""
		for _, marker := range []string{`%dp0%\`, `%~dp0\`, `%dp0%/`, `%~dp0/`} {
			...
			// The path is typically quoted: "%dp0%\path\to\script"
			// Find the closing quote or end of token
			if idx > 0 && line[idx-1] == '"' {
				// Quoted path — find closing quote
				endQuote := strings.Index(rest, `"`)
				if endQuote > 0 {
					scriptRef = rest[:endQuote]
				}
			} else {
				// Unquoted — find space or end
				endSpace := strings.IndexAny(rest, " \t")
				if endSpace > 0 {
					scriptRef = rest[:endSpace]
				} else {
					scriptRef = rest
				}
			}
		...
		if scriptRef != "" {
			// Clean up: remove trailing %* or other batch artifacts
			scriptRef = strings.TrimRight(scriptRef, " \t\r")
			return filepath.Join(cmdDir, scriptRef)
		}
	}
...
func createNpmInstallCmd(npmPath string, args []string) *exec.Cmd {
	cmd := resolveNpmToNodeCmd(npmPath, args)
	cmd.SysProcAttr = &syscall.SysProcAttr{HideWindow: true}
	return cmd
}

// resolveNpmToNodeCmd bypasses npm.cmd/.ps1 wrappers by calling node.exe
// directly with the npm-cli.js script. This prevents the FOR /F construct
// inside npm.cmd from spawning a visible cmd.exe subprocess.
func resolveNpmToNodeCmd(npmPath string, args []string) *exec.Cmd {
	// If npmPath is a bare name (no directory separator), resolve it to a full path.
	if !strings.ContainsAny(npmPath, `/\`) {
		if resolved, err := exec.LookPath(npmPath); err == nil {
			npmPath = resolved
		}
	}

	lower := strings.ToLower(npmPath)
	if !strings.HasSuffix(lower, ".cmd") && !strings.HasSuffix(lower, ".ps1") {
		// Not a wrapper script — use as-is (e.g. a direct node binary or unix npm).
		return exec.Command(npmPath, args...)
	}

	// npm.cmd lives next to node.exe and node_modules/npm/bin/npm-cli.js.
	npmDir := filepath.Dir(npmPath)
	nodeExe := filepath.Join(npmDir, "node.exe")
	npmCLI := filepath.Join(npmDir, "node_modules", "npm", "bin", "npm-cli.js")

	if _, err := os.Stat(nodeExe); err != nil {
		// node.exe not found next to npm.cmd — fall back to wrapper.
		return exec.Command(npmPath, args...)
	}
	if _, err := os.Stat(npmCLI); err != nil {
		// npm-cli.js not found — fall back to wrapper.
		return exec.Command(npmPath, args...)
	}

	// Call node.exe directly: node.exe npm-cli.js <args...>
	nodeArgs := append([]string{npmCLI}, args...)
	return exec.Command(nodeExe, nodeArgs...)
}

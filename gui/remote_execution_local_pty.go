...
func (s *LocalPTYExecutionStrategy) Start(cmd CommandSpec) (ExecutionHandle, error) {
	...
	if err != nil {
		log.Printf("[pty-lifecycle] ERR PTY start failed: cmd=%q, error=%v", cmd.Command, err)
		return nil, err
	}

	log.Printf("[pty-lifecycle] OK PTY process started: pid=%d, cmd=%q", pid, cmd.Command)

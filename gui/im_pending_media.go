...
// canInferIntentFromHistory checks recent user turns through the shared
// semantic task-intent classifier. Without a classified execution/document
// route, media remains pending and the user is asked for intent.
func canInferIntentFromHistory(entries []agent.ConversationEntry) bool {
	...
	for i := len(entries) - 1; i >= 0 && checked < lookback; i-- {
		...
		intent := agent.ClassifyTaskIntent(text)
		switch intent.Intent {
		case agent.IntentCoding, agent.IntentSSH, agent.IntentNonCoding:
			return true
		}
	}
	return false

...
func (h *IMMessageHandler) handlePostStartWorkflow(
	...
	overview := fmt.Sprintf("Workflow started: %s\nCurrent phase: %s", state.Type, state.CurrentPhase)
	if extraText != "" {
		overview += "\n\n" + extraText
	}
	...
	if req := engine.GetInputRequirement(userID); req != nil {
		overview += "\n\nInput required: " + req.Description
		if len(req.FileTypes) > 0 {
			overview += fmt.Sprintf(" (supported formats: %s)", strings.Join(req.FileTypes, ", "))
		}
		if req.AcceptText {
...
func (h *IMMessageHandler) handleNeedsUnderstanding(engine *workflow.WorkflowEngine, userID, text string) *IMAgentResponse {
	...
	//
	// Decision flow:
	//   1. UIC.Classify() 閳?ClassificationResult with Primary + WorkflowType
	//   2. WorkflowType non-empty 閳?directly start workflow (skip IUM LLM call)
	//   3. WorkflowType empty 閳?not a workflow task 閳?return nil (normal agent loop)
	//
	// Local keyword rules no longer participate in the workflow decision.
	// Semantic channels must classify decisively or this falls through to IUM.
	if uic := h.getUnifiedClassifier(); uic != nil {
		uicResult := uic.Classify(intent.MessageContext{
			Text:   text,
			UserID: userID,
		...
		if isConcreteWorkflowType(uicResult.WorkflowType) {
			...
			if registry != nil && registry.Match(wfType) != nil {
				...
				if resp := h.confirmWorkflowStart(userID, text, wfIntent, ""); resp != nil {
					return resp
				}
				state, err := engine.StartWorkflow(userID, wfIntent)
				if err != nil {
		...
		//     IsWorkflowCandidate returns true 閳?canReject is false 閳?falls through to IUM.

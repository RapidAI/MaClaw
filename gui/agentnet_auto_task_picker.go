...
type autoTaskRun struct {
	...
	Status    agentNetAutoRunStatus `json:"status"`
...
func (p *AgentNetAutoTaskPicker) GetStatus() map[string]interface{} {
	...
	return map[string]interface{}{
		...
		"last_poll_status":      p.lastPollStatus.String(),
...
func (p *AgentNetAutoTaskPicker) discoverTasks(client *AgentNetClient, hubURL string) ([]AgentNetTask, error) {
	...
	if err == nil && len(matched) > 0 {
		...
		for _, t := range matched {
			if normalizeAgentNetTaskAvailabilityStatus(t.Status).CanPick() {
				open = append(open, t)
			}
		}
		if len(open) > 0 {
...
func (p *AgentNetAutoTaskPicker) executeTask(client *AgentNetClient, task *AgentNetTask, executor AutoTaskExecutor) {
	...
	run := &autoTaskRun{
		...
		Title:     task.Title,
		Reward:    task.Reward,
		Status:    agentNetAutoRunStatusClaiming,
		StartedAt: time.Now(),
	}
	...
	if err := client.ClaimTask(task.ID); err != nil {
		...
		if bidErr := client.BidOnTask(task.ID, 0, "maClaw auto-pickup: I can help with this"); bidErr != nil {
			p.failTask(run, agentnet.FormatTaskError("claim", err, lang)+", "+agentnet.FormatTaskError("bid", bidErr, lang))
			return
		}
	}
	...
	if err != nil {
		p.failTask(run, agentnet.FormatTaskError("execution", err, lang))
		return
	}
	if result == "" {
	...
	// Success!
	p.mu.Lock()
	run.Status = agentNetAutoRunStatusDone
	p.completedCount++
...
func (p *AgentNetAutoTaskPicker) failTask(run *autoTaskRun, errMsg string) {
	p.mu.Lock()
	run.Status = agentNetAutoRunStatusFailed
	run.Error = errMsg
	p.failedCount++
...
func (p *AgentNetAutoTaskPicker) PickAndExecuteTask(taskID string) (result map[string]interface{}) {
	...
	if err != nil {
		return map[string]interface{}{"ok": false, "error": agentnet.FormatTaskError("get_task", err, lang)}
	}
	...
	if claimErr := client.ClaimTask(task.ID); claimErr != nil {
		if bidErr := client.BidOnTask(task.ID, 0, "manual pickup"); bidErr != nil {
			msg := agentnet.FormatTaskError("claim", claimErr, lang) + ", " + agentnet.FormatTaskError("bid", bidErr, lang)
			p.failTask(run, msg)
			return map[string]interface{}{"ok": false, "error": agentnet.FormatTaskError("claim", claimErr, lang)}
		}
	}
	...
	if execErr != nil {
		msg := agentnet.FormatTaskError("execution", execErr, lang)
		p.failTask(run, msg)

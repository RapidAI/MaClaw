...
export function HubServiceRedeemPanel({ lang, onStatusChange }: Props) {
    ...
    const openHubCardStorePage = useCallback(async () => {
        try {
            const cfg = await callBackend(() => LoadConfig()) as { remote_hub_id?: string; remote_hub_url?: string; remote_hubcenter_url?: string; remote_tenant_id?: string; remote_tenant_name?: string; remote_email?: string; remote_viewer_token?: string } | null;
            const url = buildHubCardStoreURL(cfg?.remote_hub_url, cfg?.remote_tenant_id, cfg?.remote_email, cfg?.remote_viewer_token, cfg?.remote_hubcenter_url, cfg?.remote_hub_id, undefined, cfg?.remote_tenant_name);
            if (!url) {

...
export function OfflineModeNoticeDialog({ onBackToOnline, onClose, t }: Props) {
    ...
    return (
        <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, background: "rgba(0,0,0,0.35)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 10000 }} onClick={onClose}>
            <div role="dialog" aria-modal="true" aria-labelledby={titleId} aria-describedby={descId} style={{ background: colors.surface, borderRadius: 16, padding: "22px 26px", maxWidth: 420, width: "90%", boxShadow: "0 16px 40px rgba(0,0,0,0.18)" }} onClick={e => e.stopPropagation()}>

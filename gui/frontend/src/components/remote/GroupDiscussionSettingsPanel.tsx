...
function getExpertName(config: main.AppConfig | null, current: Required<GroupDiscussionConfig>): string { return ((config as any)?.maclaw_role_name || current.display_name || "MaClaw").trim(); }
...
export function GroupDiscussionSettingsPanel({ config, saveRemoteConfigField, lang }: Props) {
  ...
  const saveExpertName = (value: string) => { const displayName = value.trim() || "MaClaw"; saveRemoteConfigField({ maclaw_role_name: displayName, group_discussion: normalize({ ...current, display_name: displayName }) } as any); flashSaved(); };
  ...
  return <div style={panelStyle}>
    ...
    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(320px, 1fr))", gap: "12px", alignItems: "start" }}>
      ...
      <Section title={c.identity} desc={c.identityDesc}>
        <div style={gridStyle}>
          <Field label={c.name}><input style={fieldStyle} value={expertName} onChange={(e) => saveExpertName(e.target.value)} /></Field>
        ...
        <div style={{ marginTop: "12px" }}><Field label={c.description}><textarea style={{ ...fieldStyle, minHeight: 76, resize: "vertical" }} value={current.description} onChange={(e) => save({ description: e.target.value })} /></Field></div>
      </Section>
    </div>

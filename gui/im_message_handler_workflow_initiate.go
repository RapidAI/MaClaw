...
h.mu.Lock(); defer h.mu.Unlock(); return h.sessions[userID]
}
func (h *WorkflowInitiationHandler) setSession(userID string, s *initiationSession) {
h.mu.Lock(); defer h.mu.Unlock(); h.sessions[userID] = s
}
func (h *WorkflowInitiationHandler) deleteSession(userID string) {
h.mu.Lock(); defer h.mu.Unlock(); delete(h.sessions, userID)
}
...
if len(session.MissingFields) == 0 {
return &IMAgentResponse{Text: fmt.Sprintf("已提取信息：\n%s\n确认发起？", h.presentExtractedData(session))}, nil
}
...
if len(workflows) == 0 { return nil, nil }
...
if best == nil || best.Confidence < 0.3 { return nil, nil }
return best, nil
}
...
Schema []struct { Name string `json:"name"`; Label string `json:"label"`; Type string `json:"type"`; Required bool `json:"required"` } `json:"schema"`
} `json:"workflows"`
}
...
var score float64
if strings.Contains(msgLower, nameLower) { score += 0.6 }
for _, kw := range strings.FieldsFunc(wf.WorkflowName, func(r rune) bool { return r == ' ' || r == '/' }) {
if len([]rune(kw)) >= 2 && strings.Contains(msgLower, strings.ToLower(kw)) { score += 0.3 }
}
...
if strings.Contains(msg, p) { return time.Now().AddDate(0, 0, off).Format("2006-01-02") }
}
return ""
}
func (h *WorkflowInitiationHandler) findMissingRequiredFields(s *initiationSession) []string {
var m []string
for _, f := range s.Schema { if f.Required { if _, ok := s.ExtractedData[f.Name]; !ok { m = append(m, f.Name) } } }
return m
}
func (h *WorkflowInitiationHandler) buildMissingFieldsPrompt(s *initiationSession) *IMAgentResponse {
var d []string
for _, n := range s.MissingFields { for _, f := range s.Schema { if f.Name == n { l := f.Label; if l == "" { l = f.Name }; d = append(d, "- "+l) } } }
return &IMAgentResponse{Text: fmt.Sprintf("📋 %s\n\n还需要以下信息：\n%s\n\n请补充上述信息。", s.WorkflowName, strings.Join(d, "\n"))}
}
...
if len(s.MissingFields) == 1 { fn := s.MissingFields[0]; if _, ok := s.ExtractedData[fn]; !ok { t := strings.TrimSpace(msg); if t != "" && len([]rune(t)) < 100 { s.ExtractedData[fn] = t } } }
s.MissingFields = h.findMissingRequiredFields(s)
h.setSession(userID, s)
if len(s.MissingFields) == 0 { return &IMAgentResponse{Text: fmt.Sprintf("已提取信息：\n%s\n确认发起？", h.presentExtractedData(s))}, nil }
...
if err != nil || len(wfs) == 0 { return &IMAgentResponse{Text: "未找到匹配的审批工作流。请确认工作流名称后重试。"}, nil }
var names []string
for _, wf := range wfs { if len(names) >= 5 { break }; names = append(names, "- "+wf.WorkflowName) }
return &IMAgentResponse{Text: fmt.Sprintf("未找到匹配的审批工作流。当前可用的工作流：\n%s\n\n请指定要发起的工作流名称。", strings.Join(names, "\n"))}, nil
}
...
func isInitiationConfirm(msg string) bool { l := strings.ToLower(strings.TrimSpace(msg)); for _, w := range initiationConfirmWords { if l == w { return true } }; return false }
func isInitiationCancel(msg string) bool { l := strings.ToLower(strings.TrimSpace(msg)); for _, w := range initiationCancelWords { if l == w { return true } }; return false }
...
if isInitiationConfirm(trimmed) { return h.submitToHub(ctx, session) }
if isInitiationCancel(trimmed) { h.deleteSession(userID); return &IMAgentResponse{Text: fmt.Sprintf("已取消「%s」的发起流程。", session.WorkflowName)}, nil }
if h.tryUpdateField(session, trimmed) { h.setSession(userID, session); return &IMAgentResponse{Text: fmt.Sprintf("已更新信息：\n%s\n确认发起？", h.presentExtractedData(session))}, nil }
return &IMAgentResponse{Text: "请回复「确认」发起审批，「取消」放弃，或告诉我需要修改的内容。"}, nil
}
...
if idx <= 0 { continue }
hint := strings.TrimSpace(msg[:idx]); val := strings.TrimSpace(msg[idx+len(sep):])
if val == "" { continue }
for _, f := range s.Schema { l := f.Label; if l == "" { l = f.Name }; if strings.Contains(l, hint) || strings.Contains(hint, l) { s.ExtractedData[f.Name] = val; return true } }
}
return false
}
func (h *WorkflowInitiationHandler) submitToHub(ctx context.Context, session *initiationSession) (*IMAgentResponse, error) {
hubURL, token, err := h.app.getHubCredentials()
if err != nil { return &IMAgentResponse{Text: "❌ 无法连接到 Hub。"}, err }
...
resp, err := http.DefaultClient.Do(req)
if err != nil { return &IMAgentResponse{Text: "❌ 网络请求失败。"}, err }
defer resp.Body.Close()
...
return &IMAgentResponse{Text: fmt.Sprintf("✅ 审批已发起，单号：%s", formatWFNumber(result.InstanceID))}, nil
}
func formatWFNumber(id string) string { return fmt.Sprintf("WF-%s-%s", time.Now().Format("20060102"), extractSeqFromID(id)) }
func extractSeqFromID(id string) string {
if id == "" { return "001" }
var d []byte
for i := len(id) - 1; i >= 0 && len(d) < 3; i-- { if id[i] >= '0' && id[i] <= '9' { d = append([]byte{id[i]}, d...) } }
for len(d) < 3 { d = append([]byte{'0'}, d...) }
return string(d)
}
func (h *WorkflowInitiationHandler) presentExtractedData(s *initiationSession) string {
var lines []string
for _, f := range s.Schema { l := f.Label; if l == "" { l = f.Name }; val := "（未填写）"; if v, ok := s.ExtractedData[f.Name]; ok && v != nil { val = fmt.Sprintf("%v", v) }; lines = append(lines, fmt.Sprintf("- %s：%s", l, val)) }

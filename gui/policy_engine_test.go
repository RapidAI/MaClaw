...
func TestEvaluate_LowRisk_Allow(t *testing.T) {
	pe := NewPolicyEngine()
	action := pe.Evaluate("ReadFile", nil, security.RiskLow)
...
func TestEvaluate_MediumRisk_Audit(t *testing.T) {
	pe := NewPolicyEngine()
	action := pe.Evaluate("WriteFile", nil, security.RiskMedium)
	if action != security.PolicyAudit {
...
func TestEvaluate_HighRisk_Ask(t *testing.T) {
	pe := NewPolicyEngine()
	action := pe.Evaluate("Bash", nil, security.RiskHigh)
...
func TestEvaluate_CriticalRisk_WithoutDangerousArgs_Ask(t *testing.T) {
	...
	action := pe.Evaluate("WriteFile", args, security.RiskCritical)
	if action != security.PolicyAsk {
...
func TestEvaluate_ToolPatternGlob(t *testing.T) {
	...
	if action := pe.Evaluate("BashExec", nil, security.RiskLow); action != security.PolicyDeny {
		t.Errorf("expected deny for BashExec matching Bash*, got %s", action)
	}
	// Non-matching tool falls through to default (ask).
	if action := pe.Evaluate("ReadFile", nil, security.RiskLow); action != security.PolicyAsk {
...
func TestEvaluate_ArgsPatternRegex(t *testing.T) {
	...
	if action := pe.Evaluate("SQLExec", args, security.RiskHigh); action != security.PolicyDeny {
		t.Errorf("expected deny for DROP TABLE args, got %s", action)
	}

	safeArgs := map[string]interface{}{"sql": "SELECT * FROM users"}
	if action := pe.Evaluate("SQLExec", safeArgs, security.RiskHigh); action != security.PolicyAsk {
		...
		rules: []security.PolicyRule{
			{
				...
				Priority:    100,
				ToolPattern: "*",
				RiskLevels:  []security.RiskLevel{security.RiskHigh},
...
func TestLoadRules_FromFile(t *testing.T) {
	rules := []security.PolicyRule{
		{Name: "custom-allow", Priority: 5, ToolPattern: "Read*", Action: security.PolicyAllow, RiskLevels: []security.RiskLevel{security.RiskLow}},
		{Name: "custom-deny", Priority: 10, ToolPattern: "*", Action: security.PolicyDeny, RiskLevels: []security.RiskLevel{security.RiskCritical}},
	}
...
func TestEvaluate_ConcurrentAccess(t *testing.T) {
	...
	for i := 0; i < 50; i++ {
		...
		go func() {
			defer wg.Done()
			pe.Evaluate("Bash", map[string]interface{}{"cmd": "ls"}, security.RiskMedium)
		}()
	}

	// Concurrent LoadRules while Evaluate is running.
	rules := []security.PolicyRule{
		{Name: "concurrent-rule", Priority: 1, ToolPattern: "*", Action: security.PolicyAllow},
	}

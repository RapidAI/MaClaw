...
func TestSubAgentRetryRecoveryHintForFinalSummaryQualityFailures(t *testing.T) {
	tests := []struct {
		...
		{
			name: "verification command outcome missing from final summary",
			err:  "quality audit evidence: FAILED: fresh verification command outcome not referenced in final summary",
			want: "state the outcome",
		},
		{
...
func TestSubAgentRetryRecoveryHintCoversQualityGateTiming(t *testing.T) {
	cases := []struct {
		...
		{err: `quality gate failed; quality audit evidence: FAILED: go test ./... -> go: warning: "./..." matched no packages`, want: "discovers real tests"},
		{err: "quality gate failed; quality audit evidence: FAILED: mvn test -> [INFO] No tests to run.", want: "discovers real tests"},
		{err: "quality gate failed; quality audit evidence: FAILED: cargo test -> running 0 tests", want: "discovers real tests"},
		{err: "quality gate failed; quality audit evidence: FAILED: dotnet test -> Total tests: 0. Passed: 0.", want: "discovers real tests"},
		...
		{err: "quality gate failed; quality audit evidence: FAILED: claimed verification command not found in audit log: `go test ./gui`", want: "preserving shell wrappers and quoting"},

...
func TestClawNetAutoPickerConfigureRejectsEnableWhenClawNetDisabled(t *testing.T) {
	...
	if saved.ClawNetAutoPickerEnabled {
		t.Fatal("expected saved ClawNetAutoPickerEnabled=false when ClawNet is disabled")
	}
}
...
func TestEnsureAutoTaskPickerDoesNotRestoreWhenClawNetDisabled(t *testing.T) {
	...
	saveClawNetConfigForTest(t, app, func(cfg *AppConfig) {
		cfg.ClawNetEnabled = false
		cfg.ClawNetAutoPickerEnabled = true
		cfg.ClawNetAutoPickerPollMin = 2
		cfg.ClawNetAutoPickerMinReward = 3.5
	})

	...
	defs := []map[string]interface{}{
		...
		toolDef("manage_skill", skill.ManageSkillDescription()+" patch 时如果你使用了某个 Skill 并遇到了它未覆盖的问题，请立即用 patch 修补它。",
			map[string]interface{}{
				...
				"reason":       map[string]string{"type": "string", "description": "修补原因说明（patch 时可选，记录到审计日志中）"},
		...
		toolDef("bash", "在本机直接执行 shell 命令（如创建目录、移动文件、运行脚本等）。命令在 MaClaw 所在设备上执行，不需要会话。禁止通过 bash 执行 ssh/scp/rsync 命令——请使用内置 ssh 工具。支持 background=true 后台执行长时间命令（翻译、编译、下载等），返回 task_id 后可用 async_wait 查询状态。",
			map[string]interface{}{
				...
				"working_dir": map[string]string{"type": "string", "description": "工作目录（可选，默认为系统提示中标注的 Project directory）"},
		...
		toolDef("read_tool_result", "分段回读被截断的工具完整输出（使用 [tool_result_handle] 的 id）",
			map[string]interface{}{
		...
		toolDef("send_file", "读取本机文件并交付给用户。在桌面端默认只展示在当前桌面对话；在微信/飞书等 IM 通道中，发到当前对话即等于发到该通道。若语音指定另一个群或用户，先用 im_message(action=list_targets) 把名称解析为 ID，再同时提供 channel 与 group_id/user_id；精确目标失败时禁止广播或改投其他通道。",
			map[string]interface{}{
				...
				"channel":       map[string]string{"type": "string", "description": "精确 IM 渠道：weixin/feishu/qq/telegram/lansenger"},
				"group_id":      map[string]string{"type": "string", "description": "精确群或会话 ID"},
				"group_name":    map[string]string{"type": "string", "description": "群名；有歧义时先用 im_message list_targets"},
				"user_id":       map[string]string{"type": "string", "description": "精确私聊用户 ID 或 self"},
			}, []string{"path"}),
		toolDef("send_to_im", "把本机文件发送到 IM。未指定精确目标时走用户绑定/活跃通道；若语音指定群名或用户名，先用 im_message(action=list_targets) 消歧并取得 ID，再同时提供 channel 与 group_id/user_id。精确目标发送失败时必须报告失败，禁止广播或改投其他通道。",
			map[string]interface{}{
				"path":        map[string]string{"type": "string", "description": "文件的绝对路径或相对于 Project directory 的路径"},
				...
				"group_id":    map[string]string{"type": "string", "description": "精确群或会话 ID"},
				"group_name":  map[string]string{"type": "string", "description": "群名；有歧义时先 list_targets"},
				"user_id":     map[string]string{"type": "string", "description": "精确私聊用户 ID 或 self"},
				"phase_id":    map[string]string{"type": "string", "description": workflowDocDeliveryPhaseIDSchemaDescription()},
				"doc_type":    map[string]string{"type": "string", "description": workflowDocDeliveryTypeSchemaDescription()},
			}, []string{"path"}),
		...
		toolDef("im_message", "即时向 IM 发文本或文件（蓝信群/人、微信/Telegram/QQ）。action: list_targets|send|send_file（可省略：有 text 则 send，有 path 则 send_file）。用户要求「现在发到蓝信某群/微信」时用本工具；周期播报才用 manage_schedule+delivery。send_file 上传本机文件（目前仅蓝信 lansenger 支持），可同时带 text 作为说明文字。",
			map[string]interface{}{
				...
				"file_name":        map[string]string{"type": "string", "description": "send_file：发送时显示的文件名（可选，默认原文件名）"},

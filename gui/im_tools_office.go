...
func (h *IMMessageHandler) handleReadExcel(args map[string]interface{}) string {
	filePath := stringVal(args, "file_path")
	if filePath == "" {
		return "缺少 file_path 参数"
	}
	sheet := stringVal(args, "sheet")
	rangeStr := stringVal(args, "range")
...
func (h *IMMessageHandler) handleWriteExcel(args map[string]interface{}) string {
	filePath := stringVal(args, "file_path")
	if filePath == "" {
		return "缺少 file_path 参数"
	}
	...
	case string:
		jsonBytes = []byte(v)
	default:
		// Already a map or other type — re-marshal to JSON bytes
		var err error
...
func (h *IMMessageHandler) handleReadPPTX(args map[string]interface{}) string {
	filePath := stringVal(args, "file_path")
	if filePath == "" {
		return "缺少 file_path 参数"
	}

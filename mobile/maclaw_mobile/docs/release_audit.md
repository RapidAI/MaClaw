...
| Active desktop GUI QR third-party LLM authorization can be explicitly revoked and returns the account to MaClaw official credits | Automated evidence | `test/api_client_test.dart`, `test/account_screen_test.dart`, `go test ./hub/internal/httpapi -run "TestMobileLLMDesktopQRAuthorization" -count=1` |
...
| Android 13+ notification permission can be requested from account screen | Partial | `test/account_screen_test.dart`, `test/platform_permissions_test.dart`, and `docs/qa-builds/android-api35-0.1.0+1/debug-latest/permissions-grant.txt`; startup permission prompting is disabled and the latest API 35 emulator evidence records notification/camera/microphone/media grants; real Account-screen permission grant remains manual |
| Signed internal package installs and share-to-app works for text, URLs, images, PDFs, Word, Excel, and CSV | Manual gate | Signed APK/AAB, SHA256, signing identity, version/build number, installer channel, signed API 35 emulator install/share evidence under `docs/qa-builds/android-api35-0.1.0+1/signed-release-stateful-latest/`, and real-device share notes |
...
| Cold start shows the MaClaw logo splash screen and no Flutter placeholder/template branding | Partial | `test/app_smoke_test.dart`, `test/release_docs_test.dart`, `test/platform_permissions_test.dart`, native launch asset checks, signed API 35 `startup-no-permission-ui.xml`, and current Debug API 35 `docs/qa-builds/android-api35-0.1.0+1/debug-latest/startup.xml`; real-device cold-start evidence remains manual |
...
| AI assistant behaves as a GUI-like chat window with user/assistant messages, multi-turn Hub context, citations, result sharing, starred frequent questions, recent history, and document-template actions | Automated evidence | `test/assistant_screen_test.dart`, `test/api_client_test.dart`, `test/assistant_retry_test.dart` |
...
| Voice transcript and photo/image/screenshot assistant input produce cited answers or document tasks, including photo/image assistant input evidence | Partial | `test/assistant_screen_test.dart`, `test/assistant_voice_input_test.dart`, `test/platform_permissions_test.dart`, `tool/validate_qa_build_record_test.py`; real-device voice/photo smoke remains manual |
...
| Shared text, URLs, and supported files received before phone login remain pending and are consumed by the signed-in assistant or document flow | Automated evidence | `test/app_smoke_test.dart`, `test/assistant_screen_test.dart`, `test/documents_screen_test.dart`, `test/mobile_shared_intent_test.dart` |
...
| Assistant long-task, document/export, digital employee, and SSH abnormal notifications are delivered with typed payload/open evidence, opened payloads route back to the matching mobile tab with a recovery prompt, and notification messages redact common secrets | Partial | `test/mobile_notification_service_test.dart`, `test/app_smoke_test.dart`, `test/assistant_retry_test.dart`, `test/documents_state_test.dart`, `test/digital_employees_controller_test.dart`, `test/servers_screen_test.dart`, `tool/validate_qa_build_record_test.py`; real-device notification delivery remains manual |

...
- 回复缺少结论 / 表格 / 分点等结构化整理；
- 引用区以大卡片 + 原始 snippet 为主视觉，含未解码 HTML 实体（如 `&ensp;`）；
...
| 不做 | 原因 |
|------|------|
| Flutter 嵌入 Go `corelib`（FFI / gomobile / `.so`） | 产品运行时边界；`tool/verify_runtime_boundary.py` 发布门禁 |
| 手机本地跑完整 `corelib/agent` 循环与桌面工具 | 体积、安全、凭据、SSH/文件依赖 |
| 任意第三方 LLM 配置面 / 自定义 Hub URL | 现有 Mobile 产品规则 |
| 完整 GUI 流式 UI / 复杂 Agent View 控件移植 | 可二期；一期以「总结质量 + 可读渲染」为主 |

### 1.4 架构原则

```
corelib / GUI Agent  ──可在服务端复用──►  Hub（Go）
                                              │
...
| 层级 | 现状 | GUI 侧参考 |
|------|------|------------|
| API | `POST /api/mobile/search` | GUI 走本地 agent + `corelib` |
| Prompt | 紧急助手、要求简洁 | 完整 system prompt + 工具与结构化输出 |
| 回答生成 | 单次非流式 chat completions | 流式 + 多轮 + 工具 |
| 兜底 | `mobileSearchAnswer` 拼检索列表 | 无「SERP 列表当答案」 |
| 客户端 | `Text(answer)` + 大引用卡 | Markdown 表格/标题/列表渲染 |
| 边界 | 禁止嵌入 corelib | GUI 直接 import `corelib/agent` |

关键代码：

- Hub：`hub/internal/httpapi/mobile_handlers.go`（`MobileSearchHandler`、`mobileSearchPrompt`、`mobileSearchAnswer`、citations）
...
| 确认边界门禁 | CI/本地跑 `python tool/verify_runtime_boundary.py` 必须绿 |
...
### Phase 1 — 呈现急救（Flutter，1～2 天）【优先用户可见】

**目的**：即使后端仍一般，也先去掉「搜索结果页」观感。

| # | 任务 | 主要文件 | 说明 |
|---|------|----------|------|
| 1.1 | Snippet / 标题 HTML 清洗 | 新建 `lib/core/text/html_text.dart` 或放 `assistant_screen` 旁 | `HtmlUnescape` 等价：`&ensp;`、`&#0183;`、`&amp;` 等；可选剥简单标签 |
| 1.2 | 来源折叠 | `assistant_screen.dart` | 默认「参考 N 条」Expansion；展开后再显示链接与操作 |
| 1.3 | 引用卡降噪 | `_CitationTile` | 默认一行标题 + 域名；snippet 单行截断；操作收进 overflow 菜单 |
...
| 2.2 | Snippet 入模前清洗 | 同文件 + 可选 `websearch` 适配层 | 写入 prompt 的 snippet 先 unescape/截断，降低模型复读 HTML |
...
| 2.5 | 单测 | `mobile_handlers_test.go` | prompt 含结构约束；citations 清洗；兜底不含原始 SERP 列表形态 |
| 2.6 | （可选）轻量对齐 corelib 文案块 | Hub 内抽函数或复制 **prompt 片段** | **仅 Go 服务端**；禁止引入 Flutter 对 corelib 的依赖 |
...
- [ ] 对「北京天气」类问题，answer 含可读结论，而非仅站点 snippet 堆叠
- [ ] answer 中尽量出现 Markdown 表格或清晰分点（抽样人工评测 ≥ 约定比例）
- [ ] `go test` 覆盖 mobile search 相关用例
- [ ] 不破坏 `llm_request_id` / credits / 官方 LLM 路径

**明确不做（本阶段）**：在 Hub 内嵌完整 GUI agent 工具环；流式 SSE（可 Phase 4）。

---

### Phase 3 — Markdown / 表格渲染（Flutter，2～3 天）

**目的**：服务端结构化 Markdown 在手机上真正「看得见」。

| # | 任务 | 主要文件 | 说明 |
|---|------|----------|------|
| 3.1 | 依赖 | `pubspec.yaml` | 增加 Markdown 组件（如 `flutter_markdown` 或等价；评估表格支持） |
...
| 4.2 多轮 system 强化 | 将 recent messages 以 role 消息数组提交，而不仅是 `context: string[]` | 中 |
| 4.3 任务型卡片 | 审批/文档草稿/员工任务用轻量 inline card（参考 GUI InlineChatCard 思路，**Dart 重实现**） | 低～中 |
| 4.4 服务端 agent 子集 | Hub 调用有限工具（仅 web/search），**仍在服务端**，不进手机 | 低（视业务） |
...
| 层 | 命令 / 范围 |
|----|-------------|
| Mobile unit/widget | `flutter test`（重点 assistant / api_client / redaction） |
| Hub | `go test ./hub/internal/httpapi/ -run MobileSearch`（及关联） |
| 边界 | `python tool/verify_runtime_boundary.py` |
...
| 风险 | 影响 | 缓解 |
|------|------|------|
| Prompt 变严后部分模型不遵从 | 仍像散文或仍复读 | 样例回归集 + 截断 snippet + 明确「禁止粘贴原文」 |
| Markdown 库表格能力弱 | 表格仍难看 | 选型时验证 GFM table；必要时服务端优先列表 |
| Hub 与旧 App 兼容 | 老客户端纯文本 | answer 保持字符串；Markdown 纯文本退化仍可读 |
| 误把 corelib 链进 Flutter | 发布失败 | boundary 脚本进 PR CI |
...
| Phase 2 | 已完成 | Hub `mobileSearchPrompt` 结构化；snippet 清洗；兜底不再 SERP 堆砌 |
| Phase 3 | 已完成 | `flutter_markdown` + `AssistantMarkdownBody` 表格/标题渲染 |
| Phase 4 | 已完成（核心） | 多轮 `messages[{role,content}]` 协议；官方/第三方 `stream=true` 时尽量转发上游 OpenAI SSE delta；无上游流时仍 chunk 假流；Mobile 打字机气泡 |
| Phase 4.3 | 已完成 | Dart `AssistantInlineCard` +「可以继续」；**派给员工** 经 `assistantEmployeeHandoff` 预填任务草稿 |
| Phase 4.4 | 已完成（Hub 子集） | Hub 侧 agent 循环：`web_search` / `web_fetch`；SSE `tool_call`/`tool_result`；Mobile 工具调用条；**无** bash/读盘/GUI 全量工具（边界：不进 APK） |
| 质量门禁 | 已通过 | Hub MobileSearch* 测试；Flutter assistant/stream/markdown/html/screen 测试；`verify_runtime_boundary` |
...
- Phase 4.4 Hub 侧有限 agent 工具子集（低优先级；联网搜索已在 Hub 侧）

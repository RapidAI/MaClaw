...
# official credits bound to that phone account. After verification succeeds,
# include proof that the first LLM call charges/uses the verified phone
...
# verified phone account's MaClaw official credits/quota. For
...
# citation URL from Visible citations / sources.
...
# before sharing or saving externally.
...
# network recovery is traceable to the same tenant session.
...
# action cleared server profiles and SSH password/private-key credentials.
...
# Include AI explanation, command drafts, manual/not-auto-executed evidence, and
# proof that the result used redacted/masked/sanitized SSH terminal/log output.

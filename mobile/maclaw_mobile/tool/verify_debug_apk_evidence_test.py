...
def evidence_text(path: str, content: bytes) -> str:
    return (
        ...
        f"  - CI artifact name: `{verify_debug_apk_evidence.DEFAULT_CI_ARTIFACT_NAME}`.\n"
    )
...
class VerifyDebugApkEvidenceTest(unittest.TestCase):
    ...
    def test_reports_ci_artifact_name_mismatch(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            apk = root / verify_debug_apk_evidence.DEFAULT_APK_PATH
            apk.parent.mkdir(parents=True)
            ...
            evidence.write_text(
                evidence_text(
                    str(verify_debug_apk_evidence.DEFAULT_APK_PATH),
                    content,
                ).replace("maclaw-mobile-debug-apk", "unexpected-debug-apk"),
                encoding="utf-8",
            )
            ...
            self.assertTrue(
                any("CI artifact name mismatch" in error for error in errors)
            )

...
def add_shape(slide, left,op, width, height, fill_color=None, line_color=None, shape_type=MSO_SHAPE.ROUNDED_RECTANGLE):
"""添加形状"""
...
def add_text_box(slide, left,op, width, height,ext, font_size=18, color=WHITE, bold=False, alignment=PP_ALIGN.LEFT, font_name='MicrosofYaHei'):
"""添加文本框"""
...
card_heigh= Inches(4.5)
start_x = Inches(0.6)
gap = Inches(0.25)

for i, (num,itle, desc) in enumerate(pain_points):
...
itle, font_size=18, color=WHITE, bold=True)

# 描述
b = add_text_box(slide, x + Inches(0.25), y + Inches(2.0), card_width - Inches(0.5), Inches(2.2),
...
"[ERR]过去：信息化", font_size=20, color=ORANGE, bold=True)
...
"[OK]现在：AI组织能力系统", font_size=20, color=GREEN, bold=True)
...
mod_heigh= Inches(3.2)
mod_start_x = Inches(0.6)
mod_gap = Inches(0.25)

for i, (icon,itle, desc) in enumerate(modules):
...
itle, font_size=18, color=ACCENT_CYAN, bold=True, alignment=PP_ALIGN.CENTER)
...
itle, font_size=15, color=WHITE, bold=True)
...
itle, font_size=22, color=ACCENT_BLUE, bold=True, alignment=PP_ALIGN.CENTER)
...
for i, (icon,itle, items) in enumerate(scenarios):
x = Inches(0.5 + i * 2.5)
y = Inches(2.2)
...
itle, font_size=16, color=ACCENT_CYAN, bold=True, alignment=PP_ALIGN.CENTER)
...
for i, feain enumerate(features):
add_text_box(slide, Inches(1.0), Inches(3.85 + i * 0.4), Inches(10), Inches(0.4),
f"[OK]{feat}", font_size=14, color=LIGHT_GRAY)
...
row_heigh= Inches(0.6)

# 绘制表头
for j, (header, cw) in enumerate(zip(headers, col_widths)):
cx =able_x + sum(col_widths[:j])
cell = add_shape(slide, cx,able_y, cw, row_height, fill_color=ACCENT_BLUE)
add_text_box(slide, cx + Inches(0.15),able_y + Inches(0.05), cw - Inches(0.3), row_heigh- Inches(0.1),
header, font_size=14, color=WHITE, bold=True, alignment=PP_ALIGN.CENTER)

# 绘制数据行
for i, row in enumerate(rows):
y =able_y + (i + 1) * row_height
bg = DARK_CARD if i % 2 == 0 else RGBColor(0x0E, 0x12, 0x2E)

for j, (cell_text, cw) in enumerate(zip(row, col_widths)):
cx =able_x + sum(col_widths[:j])
cell_color = bg
ext_color = WHITE if j == 0 else (MID_GRAY if j == 1 else ACCENT_CYAN)

cell = add_shape(slide, cx, y, cw, row_height, fill_color=cell_color)
add_text_box(slide, cx + Inches(0.15), y + Inches(0.05), cw - Inches(0.3), row_heigh- Inches(0.1),
...
stk_heigh= Inches(4.0)

for i, (icon,itle, items) in enumerate(stakeholders):
x = Inches(0.5 + i * 3.1)
y = Inches(2.2)
...
itle, font_size=20, color=ACCENT_CYAN, bold=True, alignment=PP_ALIGN.CENTER)
...
add_text_box(slide, Inches(1.2), Inches(2.5), Inches(4.8), Inches(0.5),
"灵活部署模式", font_size=20, color=ACCENT_CYAN, bold=True)

deploy_items = [
("Cloud 云端SaaS", "快速开通，按需使用"),
("专属环境", "独立资源，数据隔离"),
("私有化部署", "完全内网，自主可控"),
("混合模式", "灵活组合，兼顾安全与效率"),
]
...
add_text_box(slide, Inches(7.2), Inches(2.5), Inches(5.0), Inches(0.5),
"安全与合规保障", font_size=20, color=ACCENT_CYAN, bold=True)
...
f"[OK]{item}", font_size=13, color=LIGHT_GRAY)
...
for i, (icon,itle, desc, value) in enumerate(biz_models):
x = Inches(0.6 + i * 4.1)
y = Inches(2.2)
...
for i, (phase,itle, desc,ime) in enumerate(phases):
x = Inches(0.8 + i * 3.1)
y = Inches(2.3)
...
ime, font_size=11, color=ORANGE, alignment=PP_ALIGN.CENTER)
...
itle, font_size=18, color=WHITE, bold=True, alignment=PP_ALIGN.CENTER)

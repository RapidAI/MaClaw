...
`execute_maintenance_plan` 现在支持在明确授权后退休重复 skill。
...
- 根据 merge draft 的 `recommended_retire` 找到要退休的 skill。
- 仅允许退休 `learned/crafted` 来源。
...
外部来源 skill，例如 hub/file/manual，不会被这个动作退休。它们只能通过人工审阅、patch 或卸载流程处理。

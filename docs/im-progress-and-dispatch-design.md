...
**现状**：IM 通道（飞书/微信/QQ/Telegram）在 agenloop 运行期间，每 10 秒发送一条进度消息（"任务较复杂，正在耐心处理中"/"正在执行工具，请稍候"）。#37 做了分类节流（`progressMinInterval=10s` + 同类中间状态消息去重），但核心问题未解决——**不区分任务复杂度，所有任务都用同一套 nudge 策略，且消息没有信息量**。

用户查天气（5 秒完成），10 秒后收到"任务较复杂"——荒谬。用户开发游戏（5 分钟），每 10 秒收到一条空洞的"正在处理中"——刷屏且无用。

### 1.2 长程任务中用户失去控制权

**现状**：IM 通道的消息处理是同步阻塞的——`SendAIAssistantMessage` 在 agenloop 运行期间不接受新消息。用户发的"取消"/"补充信息"/"新任务"要等当前 loop 结束后才被处理。桌面面板有暂停键（contexcancel），但 IM 没有等价机制。

### 1.3 桌面面板预输入队列功能不足

**现状**：预输入队列中的多条消息在 agenloop 结束后一次性处理。用户无法控制处理顺序，无法让某条消息优先处理，无法在等待过程中取消或补充当前任务。
...
2. **复用已有基础设施**：embedding cosine（intent-fusion L2 通道）、意图分类（UnifiedIntentClassifier）、contexcancel（桌面面板已有）
...
type Milestone struc{
Timeime.Time
Toolstring// 工具名
Summarystring// 从工具 name+args 机械提取的摘要
Phasestring// "connecting"/"executing"/"generating"/...
Completed bool// 该步骤是否已完成
}

type MilestoneBuffer struc{
musync.RWMutex
milestones []Milestone
askDescstring// 当前任务描述（调度器用）
askIntenstring// 当前任务意图标签（调度器用）
askEmbed[]float32// 当前任务描述的 embedding 向量（调度器用）
startTimeime.Time// 任务开始时间
}

func (b *MilestoneBuffer) Record(m Milestone)// 写入里程碑
func (b *MilestoneBuffer) Since(ime.Time) []Milestone // 读取指定时间后的里程碑
func (b *MilestoneBuffer) Latest() *Milestone// 最新里程碑
func (b *MilestoneBuffer) ProgressSummary() string// 当前进度摘要文本
func (b *MilestoneBuffer) Reset(taskDesc,askIntenstring,askEmbed []float32) // 新任务重置
```
...
从工具调用的 name + args + resul中机械提取摘要，不需要 LLM：

```go
// corelib/progress/extractor.go

//oolSummaryRules 声明式映射，新增工具只需加一行
varoolSummaryRules = map[string]SummaryRule{
...
"generate_pdf":{Verb: "生成 PDF", Static:rue},
"create_session":{Verb: "启动编程环境", Static:rue},
...
"read_file":rue, "memory":rue, "discover_tool":rue,
"task":rue, "ask_user":rue, "manage_config":rue,
}

func ExtractMilestone(toolName string, args map[string]any, resulstring) *Milestone
```
...
| SSH 部署（3min） | 交替 "正在处理"/"正在执行工具" | "收到 " + "连接服务器..." + "giclone 完成" + "编译中..." |
...
用户在 agenloop 运行中发了一条消息。系统需要决定怎么处理它。
...
type StructureSignal struc{
Lengthin// rune 计数
IsShorbool// <5 rune
IsMediumbool// 5-30 rune
IsLongbool// >30 rune
HasNegationbool// 否定句法模式
}

// 否定句法检测——语法结构，非关键词堆砌
// 中文否定句尾模式：不/别/没/算 + 了/吧/啦/呢
// 中文否定祈使：先不/暂时不/还是不/不要/不用/别
// 英文否定模式：don't/no longer/never mind/forgeit/stop + 句末
func DetectNegation(texstring) bool
```
...
```
高相关 + 同域意图低相关 + 异域意图低相关 + 无明确意图
否定结构ReplaceReplaceReplace
非否定 + 短消息MergeEnqueueStatusQuery
非否定 + 中消息MergeInserInsert
非否定 + 长消息MergeInserInsert
```
...
→ 暂停当前 agenloop（cancel contex+ 保存恢复点）
→ 处理插入的消息（独立 agenloop）
→ 插入消息完成后，自动恢复被暂停的任务
...
```go
type SuspendedTask struc{
UserIDstring
OriginalMessagestring
ConversationSnapsho[]conversationEntry// 对话历史快照
MilestoneBuffer[]Milestone// 已完成的里程碑
PartialOutpustring// 已生成的部分文本
PendingStatesmap[string]any// pendingAskUser 等状态
SuspendTimeime.Time
}
```
...
Replace 和 Inser都需要中断当前 agenloop。抽象为统一接口：
...
cons(
InterruptReplaceInterruptReason = "replace"// 放弃当前 + 处理新的
InterruptInserInterruptReason = "insert"// 暂停当前 + 处理新的 + 恢复
InterruptUserClickInterruptReason = "user_click"// 桌面面板用户点击发射按钮
)

type InterruptRequesstruc{
UserIDstring
NewMessage Message
ReasonInterruptReason
}

type InterruptResulstruc{
PartialOutpustring// 被中断任务的部分产出
CompletedSteps[]Milestone// 已完成的里程碑
SuspendedTask*SuspendedTask // Inser模式下的恢复点（Replace 模式为 nil）
}

func (h *IMMessageHandler) InterruptAndProcess(req InterruptRequest) (*InterruptResult, error) {
// 1. cancel 当前 agenloop 的 context
// 2. 等待优雅退出（当前工具调用完成或超时 5 秒）
// 3. 收集部分产出（从 MilestoneBuffer + conversation history）
// 4. Inser模式：保存恢复点
// 5. Replace 模式：保存部分产出到对话历史，标记为已中断
// 6. 清理 pending 状态（pendingAskUser、pendingCapabilityGap、drifstate 等）
// 7. 返回 InterruptResult
}
```

桌面面板和 IM 通道共享这个接口，区别只在触发方式：
- 桌面：用户点击 [Enter] 按钮 → `InterruptAndProcess(UserClick, msg)`
- IM：调度器判定 Replace → `InterruptAndProcess(Replace, msg)`
- IM：调度器判定 Inser→ `InterruptAndProcess(Insert, msg)`

### 4.6 IM 通道异步化

当前 IM 消息处理是同步阻塞的。需要改为异步，才能在 agenloop 运行中接收新消息：

```go
// hub/internal/im/async_dispatcher.go

type AsyncDispatcher struc{
scheduler*MessageScheduler// 调度决策
cancelFuncssync.Map// per-user cancel context
msgQueuessync.Map// per-user 消息队列
...
**现状**：队列中多条消息在 agenloop 结束后一次性处理。

**改进**：逐条处理，每条独立 agenloop。处理完第 1 条 → 再处理第 2 条 → 再处理第 3 条。
...
ry {
consresp = awaisendMessage(msg.text)
...
队列中每条等待的消息旁边有 [Enter] 按钮。点击后不是直接中断，而是先做调度决策——判断这条消息和当前正在处理的消息的关系。
...
│当前: "开发贪吃蛇游戏"[处理中 ]│
││
│① "颜色改红色"[[merge] 合并]← relevance=0.72│
│② "查下天气"[[insert] 插入]← relevance=0.08│
...
点击 [Enter] 默认执行推荐动作。长按或右键展开菜单让用户覆盖：

```
[[merge] 合并到当前任务]← 系统推荐（高亮）
[[insert] 插入优先处理]
[替换当前任务]
```
...
```
pending→ 排队中，显示推荐动作图标 + [Enter] 按钮
active→ 处理中，显示 动画
completed→ 已完成，显示 （几秒后折叠）
interrupted → 被中断（Inser模式暂停），显示 [pause]+ [[retry] 恢复] [[x] 放弃]
abandoned→ 被放弃（Replace 模式），显示 已放弃
failed→ 处理失败，显示 [[retry] 重试] [[x] 删除]
merged→ 已合并到当前任务，显示 [merge] 已合并
```
...
pending → active (被 [Enter] 按钮提升为当前任务)
```

### 5.4 被中断消息的处理

Inser模式下，当前任务被暂停。插入消息完成后：

- **自动恢复**：系统自动恢复被暂停的任务，注入恢复上下文
- 用户也可以在 `interrupted` 状态下点击 [[x] 放弃] 手动放弃
...
桌面面板IM 通道
││
预输入队列 + 预分类AsyncDispatcher
(入队时异步 Schedule)(消息到达时 Schedule)
││
调度决策（缓存）调度决策（实时）
(点击 [Enter] 读缓存, 0ms)(L1 <1ms / L2 5ms / L3 2-8s)
││
┌────────┼────────┐┌────────────┼────────────┐
↓↓↓↓↓↓
ReplaceMergeInserReplaceMergeInsert
EnqueueStatusEnqueueStatus
││││││
└────────┼────────┘└────────────┼────────────┘
↓↓
...
- `gui/im_message_handler.go`：agenloop 开始时创建 `AgentProgressTracker`
...
- `corelib/progress/agent_integration.go`：`AgentProgressTracker`（一行接入 agenloop）
- `corelib/progress/agent_integration_test.go`：6 个单元测试
- `gui/im_message_handler.go`：替换 ackimer + "任务较复杂" 消息 + gateConfig 后 RefineIntent
...
- `gui/im_message_handler.go`：`pendingInjection` + `interruptHandler` 字段 + `chatLoopMu` 前统一 interrup检查
- `gui/remote_hub_client.go`：Hub 模式 interrup检查

### Phase 5: IM 通道 interrup接入 已完成
...
- `gui/im_message_handler.go`：`chatLoopMu.Lock()` 前统一 interrup兜底（覆盖蓝信等无 per-user lock 的通道）
...
- `gui/frontend/src/components/ai/BufferQueuePanel.tsx`：`onFireEntry` + `Enter` 发射按钮（已有）
- `gui/frontend/src/components/ai/useBufferQueue.ts`：`extractEntry` 方法（已有）

**注**：`Enter` 发射按钮和 `handleFireEntry`（cancel + send）在之前的迭代中已实现。本次改动是将队列消费从 `mergeAndFire`（一次性合并）改为 `extractEntry`（逐条独立 agenloop）。
...
不发 Ctrl+C 到远程服务器（可能中断危险操作如数据库迁移）。只取消 maclaw 侧的 agenloop。回复："已停止 maclaw 任务。注意：服务器上正在执行的命令可能仍在运行。"
...
`pendingInjection` 在 agenloop 每次迭代开始时检查并消费。如果当前迭代正在执行一个长时间工具调用（如 SSH 命令），注入要等到下一轮迭代才生效。这是可接受的——补充信息不需要立即中断当前操作。

### 9.6 IM 通道 L2 embedding 不可用

L2 embedding 模型未加载时，relevance 信号缺失。降级策略：
- 仅用 intendomain match + structure 信号做决策
...
| 用户取消响应延迟 | 等 agenloop 结束（分钟级） | <1s（L1 快速路径）/ <5ms（L2 路径） |
| 补充信息生效延迟 | 等 agenloop 结束 | 下一轮迭代（秒级） |
...
2. **Inser模式恢复复杂**：保存和恢复 agenloop 状态涉及对话历史、pending 状态、工具上下文。缓解：Phase 4 先实现 Replace（简单丢弃），Inser恢复作为后续迭代

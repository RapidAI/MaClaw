...
D2a 的 local/remote 对称回归已通过真实 `codingagent.Run` 覆盖 request context 的 live steer cancellation、successor 注入内容、finalization 拒绝，以及已消费/后续 steering watermark。D2b 的首个 callback/holder conformance 也已通过：qualification-disabled 的 test-only relay 使用 real holder、channel tuple 和 durable coordinator；仅测试可见的 `{Protocol, ConnectionID, SurfaceEpoch}` reservation ledger 证明 predecessor 恰有一次 `steered`，其 alias 为 `stale_surface`，而重复/迟到 predecessor disposition 不会关闭新 reservation。D2 的剩余组合现已全部补齐（见下），每项负向断言均包含零静态 name-dispatch fallback、零额外 provider I/O/alias materialization。即使 D2 完成，仍须 D3 的 fixed cohort 与 kill-switch production conformance；此前 dynamic Skill/MCP alias 继续 zero materialization。

已落地（2026-09-02）：D2 剩余组合与 §9.9.13 的 verified-ingress 组合审计全部补齐，local/remote 对称，复用同一 test-only reservation ledger 夹具。tool-batch durability 矩阵补上"steer 不提交 batch"：在途 request 收到 steer 时 batch starter/committer 零调用、零执行。request 中 steer 经真实 `agent.RunLoop` 证明 predecessor 恰好一次 `steered` 且不发生 response bind，successor 以新 connection/new epoch 独立 materialize 并 settle，旧 alias 为 `stale_surface`。transform/request race 用生产 `codingSubAgentHooks` 的快照提交与在途 request 真实并发：predecessor 不 bind/不执行，successor conversation 恰好携带 transform 边界提交的注入 payload。response/finalization race 证明 bind 后、final 提交前的 late steer 被 `TrySealReplans` 拒绝而唯一结算为 `steered`，而已 settled 的 reservation 对事后 steer 完全惰性。cancel/steer race 在 stop/replan/loop cancel 并发投递下每 reservation 恰好一次终态（`steered` 或 `transport_failure` 之一），不产生只为被误关的 successor。runtime/nested/route terminal 与 batch 状态的全交叉补齐 {in-flight request, after settled batch} 两档：终态幂等、ledger 不变、successor 不被迟到 predecessor disposition 干扰。verified-ingress 审计在同一 fixture 风格下补齐四组：cancellation（真实 `codingagent.Run` + runtime attempt cancelled，reservation 唯一 `transport_failure` 结算）、binder failure（缺 provider response ID → 唯一 `response_abandoned`、零 dispatch）、early return（interactive pause 与 iteration limit 空响应均唯一 `response_abandoned`）、child handoff（nested spawn 校验返回前 parent reservation 已退休、零 provider I/O、零 disposition）。补测试期间确认一条既有设计边界而非竞态 bug：被 `steered`/`CancelRouteSurface` 终结的 route revision 按设计不可重新 materialize grant，successor 必须经 PublishSurface 发布 child revision。该 child-revision 接线随后已落地（2026-09-02）：coordinator 的 `PublishSurface(ExpectedParent)` 与 `Routes.CurrentRevision` 原本就支持 lineage child revision，本次只做接线不改 coordinator 语义——lifecycle relay 在 successor reservation 准入时（predecessor 终态写入完成之后）仅从 durable coordinator 读取 verified identity 对应的 lineage-current revision 并注入 holder；holder 首次 render 时以同一 immutable plan 内容的 child revision identity 经 `publishCodingDurableDynamicSurfaceChildForEpoch` 发布新 surface。终结的 revision 不复活、grant/alias 不跨 revision 继承（child 获得全新签发的 grant 与 alias）、lineage 绝不来自 task text/runtime ID/path/config。既有 D2 successor 回归（in-flight steer、transform race、finalization race、terminal×batch 交叉）已不再使用独立 root scope 夹具而改走真实 child-revision 路径；新增回归断言 child parent 指向被终结的 predecessor revision、同 verified identity、predecessor alias 仍 `stale_surface`，以及非终结 predecessor 拒绝 successor 准入、跨 root/跨 principal/伪造 digest/非 lineage-current 的 parent 与 lineage 已存在时的 root 重发布全部 fail-closed。上述全部为 test-only 夹具证据：`codingDynamicAliasesMayMaterialize()` 保持 `false`，production qualification 仍 disabled，dynamic Skill/MCP 继续 fail-closed，D3 的 fixed cohort 与 kill-switch production conformance 仍是后续门禁。
...
### 9.11 第八次复审修订：发布门禁改为同提交机器证据，取消逐次人工评审（2026-09-02）

**决议背景**：§9.6、§9.9.13--§9.9.16 与 S1-C 生产就绪性文档把 D3/S1-C 的推进条件表述为"经评审的 adapter build""发布批准""D3 评审签字"与"灰度窗口"。这些是不可由实现者完成的外部事件：在 D0--D2、successor child-revision 接线全部闭合后，迁移仍无限期等待人工动作。产品 owner 决议：自本节起，所有"评审/批准/签字/灰度"门禁重新定义为**同提交机器证据**——证据只能由代码与测试产生、由 meta-test 与 CI 校验；证据在同一提交内齐备时 qualification 自动 eligible。"批准"不再是外部事件，而是证据齐备且 CI 全绿这一可复核事实本身。技术门槛不降低：全部 fail-closed 不变量、identity/lineage 来源约束与删除门槛原样保留，改变的只是"谁在何时检查"。

**四条原则**：

1. **可证伪**：任何 gate 字段必须存在能将其证伪的自动检查；写不出对应负例测试的证据字段不允许加入 qualification。
2. **原子性**：adapter 实现、conformance suite、certificate、fixed cohort、kill-switch drill 与被迁移 family 的 legacy definitions/name filters/name dispatcher 删除必须在同一提交落地；任何一项缺失，`eligible()` 自动为 false。沿用既有 all-or-nothing composition，不得逐接口半接线。
3. **无自我升格**：证据值不得由 LLM 配置、URL、模型名、task text、runtime/loop/request ID 或项目路径推导；只能来自编译期常量与被测试钉死的行为断言。
4. **诚实替代**：机器证据替代的是"检查行为"，不是风险判断。风险判断在本节一次性固化为显式 policy 常量：D3 首批 materialize 的 catalog 仅限只读无副作用 dynamic family；effectful Skill/MCP 在其 receipt/settlement contract 被同一模型机器证明前不得进入 catalog；remote Coding 在其自有 adapter 证据齐备前维持 S0.5 只读收口；S5 的完整动态面仍排在 S2/S3/S4 之后。

**证据模型**（对 `codingDynamicProductionAdapterQualification` 逐字段）：

| 字段 | 机器来源 | 机器校验 | 失效条件 |
| --- | --- | --- | --- |
| Capability 行（五个 `Has*`） | adapter 实现经 loopback 真实 WS conformance 导出 transport facts | 每个 bool 对应一组正/负例测试；meta-test 反射枚举字段，断言每个字段被至少一个测试引用 | 任一 conformance 失败或字段无测试 |
| `AdapterVersion` | adapter 源码编译期常量 | 与 conformance suite 版本常量的相等性测试；改 adapter 不 bump 版本即红 | 常量失配 |
| `VerifiedIngress` | §9.10 已实现的 desktop verified ingress（durable identity + host workspace binding 同一来源） | relay 只能由该 verified ingress 路径构造；nested child 继承规则同 §9.10 | 非 verified ingress 构造路径 |
| `LifecycleDispositionVersion` | 已完成的 D2 全组合套件版本常量 | 组合矩阵 meta-test：disposition × request phase × local/remote 笛卡尔积逐项映射到测试名 | 任一组合无测试 |
| `CatalogReceiptPolicyCovered` | catalog coverage 测试 | 每个可 materialize selection 断言 receipt policy 存在；新增 selection 无 policy 即红 | coverage 缺口 |
| `ReceiptDispatchVersion` / `ReplacementSemanticsCertificate` | loopback fake Responses-WS provider 的 contract 测试（explicit empty、redirect terminal、append/retain 负例、receipt digest == wire payload hash） | certificate 每个 bool 由对应测试同提交钉住；meta-test 断言覆盖 | 任一 contract 测试失败 |
| `FixedCohort` | 一次性生成并提交的 opaque 常量 | 测试断言不同 user/task/config/model 输入下 cohort 不变，且 cohort 不出现在任何 config 样例 | 可推导性或配置泄漏 |
| `KillSwitchInstalled` | qualification 谓词的运行时关闭路径 + 自动 drill | drill 测试：关闭后 in-flight reservation 按 contract 结算且不新执行 effect、successor 零 materialization、静态面回 S0.5、零 name-dispatcher fallback | drill 任一断言失败 |
| `Wired` / `Enabled` | 不再是手填布尔：`Wired` 由五接口 composition 完整性在装配期派生；`Enabled` 由以上证据全部非空派生 | `requireCodingDynamicCallbackComposition` 与 `eligible()` 谓词本身 | 任何证据缺失即 false |

**灰度的机器替代**：D3 的"真实 production conformance"定义为 hermetic production-composition 套件——使用 production 真实装配（local/remote callback composition + relay + holder + durable coordinator + fixed executor）对 loopback fake provider 跑完整 D2 矩阵、batch durability、restart recovery 与双 executor 隔离。该套件取代"灰度观察窗口"；kill-switch drill 取代"灰度回滚演练"。不再有需要人工观察的发布阶段。

**自主执行序列**（每一步可由实现者独立完成，无外部门禁）：

1. **E1**：把 `responses-ws-channel-available-not-wired` matrix 行升级为五个 correlation 字段全 true 的合格 adapter 行——provider `response.id`、每个 tool call 的 provider call ID、cancel 后迟到 response 的拒绝语义、单次 socket 的 replay identity，全部经 loopback WS conformance 证明。
2. **E2**：replacement-semantics contract 测试与 certificate 同提交落地。
3. **E3**：production-composed callback 的 hermetic D2 全矩阵（production 真实装配形状，仍对 fake provider；不是 test-only 夹具的简化 composition）。
4. **E4**：fixed cohort 常量与 kill-switch drill。
5. **E5**：原子翻转提交：派生 `Wired`/`Enabled` 首次为 true，同提交删除首批 family 的 legacy model definitions、task-kind/name filters 与 name dispatcher，并加"旧 switch 不可达"回归。§9.6.3 的删除门槛由此从人工核对变为同提交机械检查。
6. **E6**：此后 S2 remote、S3 durable journal、S4 lifecycle consolidation、S5 dynamic provider 各阶段沿用同一证据模型推进，不再引入新的人工门禁；effectful provider 的 receipt/settlement contract 未按本模型证明前不得进入 catalog。

**明确放弃与保留**：放弃逐次评审签字、灰度窗口与"上线后人工观察"（由 E3/E4 取代）；保留全部技术不变量——fail-closed 默认、零 name-dispatcher fallback、identity/lineage/binding 来源约束、迟到 response 拒绝、unknown effect 不重放。§2、§9.6.1、§9.6.3 及 S1-C 生产就绪性文档中"经评审/reviewed/批准/签字"字样，自此按本节证据模型解释，不再指代外部人工事件。在 E5 原子翻转提交落地前，`Wired=false`、`Enabled=false`、`codingDynamicAliasesMayMaterialize()==false` 继续成立。

**E1 已落地（2026-09-02）**：`codingDynamicProviderCorrelationForConfig` 的 `responses-ws-channel-available-not-wired` 行已升级为 `responses-ws-single-use-channel` 合格 adapter 行，五个 correlation 字段全部置 true；其余 protocol 行保持不合格。该行是本仓库自带 Responses-WS adapter 实现的编译期声明，不是对用户 endpoint 的信任：缺 response.id 的响应在 bind 边界失败（response_abandoned）、缺 provider call ID 的调用在受管 dispatch 前以 `stale_surface` 拒绝、同流 ID 冲突 fail-closed，这些边界行为均有负例钉住。每项能力的 loopback WS conformance 证据（`guiapp/coding_responses_ws_conformance_test.go`，真实本地 WebSocket fake provider 对真实 `responsesWSRequestChannel`）：`HasTransportConnectionID` 断言两次拨号得到互不相同、不含 URL/model/key 的 host-owned opaque connection ID；`HasProviderResponseID` 断言 lifecycle envelope 的 `response.id` 进入 `llm.Response.ResponseID` 并经真实 holder 完成 bind，冲突 ID 与缺失 ID 各自 fail-closed；`HasProviderToolCallID` 断言 parser 从 `response.output_item.added` 与 `response.output_item.done`（本次最小补齐的提取点，含 arguments 兜底）提取每个 function_call 的 provider call ID 进入 `llm.ToolCall.ID`，无 call ID 的调用不得进入受管 dispatch；`HasCancellationFence` 断言 cancel 后 socket 关闭、dispatch 以 `context.Canceled` 返回（本次硬化：cancel 导致的 read error 一律归约为 context error，partial stream 永不可 bind）、迟到 frame 不进入 response、单次 channel 拒绝第二次 dispatch；`HasReplayIdentitySemantics` 断言单 socket 恰好一帧 `response.create`、replay/late preparation 零写入、successor 必须新 socket 新 identity。钉死方式：`codingResponsesWSAdapterVersion`（adapter 源码编译期常量）与 `codingResponsesWSConformanceVersion`（suite 常量）由 `TestCodingResponsesWSAdapterVersionMatchesConformanceSuite` 钉相等；`codingResponsesWSConformanceCoverage` 以函数引用把五个 Has 字段映射到 conformance 测试（改名/缺失即编译错误），meta-test `TestCodingDynamicProviderCorrelationCapabilityFieldsHaveConformanceCoverage` 反射枚举字段断言全覆盖且仅该行 eligible。qualification 的 `AdapterVersion` 现仅对该行填充上述常量，`Reason` 更新为 `coding_dynamic_capability_qualified_wiring_disabled`。红线不变：`Wired=false`、`Enabled=false`、`codingDynamicAliasesMayMaterialize()==false`、registry 其余字段与 production callback 接线均未动；E2（replacement-semantics contract）、E3（hermetic production composition）、E4（fixed cohort 与 kill-switch drill）、E5（原子翻转）仍是后续。

**E2 已落地（2026-09-02）**：`codingDynamicReplacementSemanticsCertificate` 与 replacement-semantics contract suite 同提交落地（`guiapp/coding_responses_ws_replacement_contract_test.go`，沿用 E1 loopback 夹具）。编译期构造 `codingDynamicResponsesWSReplacementSemanticsCertificate()` 为合格行填充 `ReplacementSemantics` 与 `ReplacementSemanticsVersion`（常量 `codingResponsesWSReplacementSemanticsVersion`），`ReceiptDispatchVersion` 同步填充 adapter 版本常量；非合格行保持 nil 且无证据字段。逐字段证据：`ExplicitEmptySurfaceVerified`——frame builder 与真实 wire 两端断言空面序列化为显式 `tools: []`（非省略字段），canonical receipt 对空面完整、确定且与非空面可区分，required tool choice 在空面上被拒；`RejectsToolBearingRedirects`——该 envelope 本无 redirect 语义，contract 断言"隐式第二发送"在 envelope 层不存在：完成或失败的 dispatch 之后任何 retry 都被拒且返回零值 receipt/response（receipt 不可被继承），与 E1 的传输单发断言点不同；`PolicyProjectionVersion`——常量 `codingResponsesWSPolicyProjectionVersion` 钉住 provider default（两键省略）/auto/required/none/specific/parallel 的固定映射表，并断言 conversation 内容无法注入或改变顶层 `tool_choice`/`parallel_tool_calls`；`AppendContractTested`/`RetainContractTested`——两个独立实测负例：append（冻结合面外加一条 cached 定义）与 retain（显式空替换后仍写前一轮工具）的 wire payload 分别在 pre-write 验证与 post-dispatch bind-path 验证处各自 fail-closed，非假设等价；receipt 契约——同一一次 dispatch 返回的 receipt 与 fake provider 实收字节重算的 receipt 逐字段一致（digest == wire payload hash）、bind-path verifier 恰好接受该 receipt、事后篡改的 payload 双重被拒。钉死方式：`codingResponsesWSReplacementContractCoverage` 以函数引用映射全部 certificate 字段（缺项/改名即红或编译错误），meta-test `TestCodingDynamicReplacementSemanticsCertificateFieldsHaveContractCoverage` 反射枚举，`TestCodingResponsesWSReplacementSemanticsVersionMatchesContractSuite` 钉版本相等，`TestCodingDynamicReplacementSemanticsCertificateIdentityMatchesAdapterRow` 钉 certificate 身份字段与合格行一致。新增断言锁定"capability+certificate 齐备但 wiring disabled 时 `eligible()` 仍为 false"。红线不变：`Wired=false`、`Enabled=false`、`codingDynamicAliasesMayMaterialize()==false`、registry 其余字段（`VerifiedIngress`/`LifecycleDispositionVersion`/`CatalogReceiptPolicyCovered`/`FixedCohort`/`KillSwitchInstalled`）与 production callback 接线均未动；E3（hermetic production composition）、E4（fixed cohort 与 kill-switch drill）、E5（原子翻转）仍是后续。

**E3 已落地（2026-09-02）**：hermetic production-composition 套件落地，取代灰度观察窗口（`guiapp/coding_e3_hermetic_fixture_test.go` / `coding_e3_hermetic_matrix_test.go` / `coding_e3_hermetic_supplementary_test.go` / `coding_e3_catalog_guard_test.go`）。装配全程走生产路径：真实 local/remote callback 构造器与 `tryAttachQualifiedDynamicLifecycleRelay`、`newQualifiedCodingBoundDynamicRequestLifecycleRelay`、真实 factory `reserveCodingBoundDynamicRequestAdapter` 与 `validateCodingDynamicQualifiedRequestChannel`、durable coordinator、fixed executor、真实 `agent.RunLoop`/`codingagent.Run` 与 verified-ingress runner；loopback fake Responses-WS provider 与 loopback MCP JSON-RPC provider（经 MCPRegistry 真实注册、health check、tools/list 缓存与按 principal 的 contract 发布）满足生产 needs 的完整 30-selection plan（守卫测试 `TestCodingE3HermeticCatalogSatisfiesProductionNeeds`）。仅有的测试侧注入：package-internal `codingDynamicProductionAdapterQualificationOverride`（仅 relay 构造与未来 factory 两个装配入口读取；`codingDynamicAliasesMayMaterialize()` 不读它，测试断言 override 安装期间 alias gate 与 production 默认仍关闭）与 loopback dial 目标。矩阵覆盖：disposition（7 值）× phase（5 档）× local/remote 笛卡尔积由 `codingE3DispositionMatrix` 机械声明——12 个可达 cell 逐项映射 per-family 测试，23 个 cell 带 unreachable-by-design 理由，`TestCodingE3DispositionMatrixCoversCartesianProduct` 缺组合即红；另有 nested/route in-flight close、cancel/steer race、transform race（生产 `codingSubAgentHooks` 真实 drain）、settle 后终态幂等 ×3 reason、双 executor 共享 coordinator 隔离、restart recovery 经真实 factory、child handoff（plain + verified）、verified-ingress batch/cancel/handoff、relay 构造路径回归（nil/incomplete identity → nil）。版本钉死：`codingDynamicLifecycleDispositionVersion` 与 suite 常量 `codingE3DispositionSuiteVersion` 相等性测试；合格行填充 `LifecycleDispositionVersion` 与 `VerifiedIngress`（常量 `codingDynamicVerifiedIngressScope`，标识 §9.10 desktop verified ingress scope，不可由 config/task 推导）。E3 抓出并修复一个真实装配 bug：`PublishSurface` 只对 repeat-exposure 子集签发 grant，而 GUI 发布路径此前渲染全部 ready selections，sibling 预算 plan 在 render 时缺 grant 失败——修复为渲染闭包等于 grant 闭包（`publishCodingDurableDynamicSurfaceRevision` 按已签发 grant 过滤渲染 plan）。另修正一处我方 D2 测试的过严竞态断言（cancel/steer 并发子例接受 1-2 个 reservation，各自恰好一次终态）与一处套件自身 data race（frame 钩子安装时序）。interactive pause 的 early-return 分支无法经真实 MCP JSON-RPC envelope 结果形状诚实驱动（marker 不穿透 envelope），该分支保留 D2 夹具与 verified-ingress D2 审计证据。红线不变：`Wired=false`、`Enabled=false`、`codingDynamicAliasesMayMaterialize()==false`、`CatalogReceiptPolicyCovered`/`FixedCohort`/`KillSwitchInstalled` 留待 E4；E4（fixed cohort 与 kill-switch drill）与 E5（原子翻转）仍是后续。

### 9.12 当前状态校正：E1--E5 证据不等于动态 alias 生产切换（2026-09-06）

后续实现已经补上 E1--E4 的 Responses-WS/替换语义/组合矩阵/kill-switch 证据，并加入 E5 的 qualification 派生测试。这里需要把“证据完成”和“生产可执行”分开记录：`codingDynamicProductionAdapterForConfig` 在 `guiapp` 的合格 Responses-WS 行可以得到 `Wired/Enabled/eligible=true`，但这只表示机器证据完整；它不能绕过任务范围准入。

当前动态 alias 的生产门禁已重新收口：

1. `gui` 与 `guiapp` 的 `codingDynamicAliasesMayMaterialize()` 均恒为 false；transport qualification、verified identity、URL、模型名称和测试 override 都不能单独打开 alias。
...
### 9.13 持久化旧快照与投递重试收口（2026-09-07）

复审继续覆盖 coordinator 的恢复和重试边界：`RouteMaterialization` 写入会用可信 route scope 与签名 grant 绑定首个具体 `ToolSnapshotID`；模型请求 surface 的 publish/resolve/recover 以 route row canonicalize 旧空值并拒绝具体冲突；`PrepareDeliveryAndComplete` 对同一 host-call、请求摘要、结果摘要和 operation key 做幂等判断，允许安全重试，不会再次创建外部发送。上述实现不改变动态 alias 的 production gate，`gui` 与 `guiapp` 仍保持 zero materialization，旧 HMAC grant 仍通过显式 canonical validation 恢复。

#### 9.14 tools_search alias status 收口（2026-09-07）

tools_search 目录中的展示别名可能共享一个 capability。此前状态函数只比较 capability，导致计划选择 read_file 时把 list_directory 也显示成 planned；模型随后等待或调用不会渲染的名称。现在 planned 只由精确 AdapterName 命中，能力相同不再足以授予“将出现”的状态；GUI 和 GUIApp 的回归测试覆盖该情形。

#### 9.15 host-call 参数摘要冲突收口（2026-09-07）

动态固定桥接的 host-call journal 已发现一处根本边界：repeat sibling grant 的历史 fingerprint 兼容分支曾在 request digest 不同时继续返回旧终态，导致同一 call ID 的不同参数被伪装成 replay。现在 `Admit`/`Reject` 只有在 request digest 完全相同的前提下才接受历史 fingerprint；digest 不同直接 `host_call_conflict`。该规则与 canonical 参数摘要、surface epoch 和 grant fingerprint 共同组成幂等身份，避免参数变更绕过重放保护。
#### 9.16 动态 adapter 展示名收口（2026-09-07）

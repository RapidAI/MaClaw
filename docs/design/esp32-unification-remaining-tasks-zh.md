...
> 2026-08-31 B3/C7 Connectivity parent-budget conversion fence：Connectivity System Sleep PREPARE 将 tick 剩余预算传给取消桥与 profile transport 前，统一使用饱和毫秒转换，避免低 tick-rate 或大预算乘法溢出成短 timeout；回调返回后仍复核原始父 deadline。该收口仅为源码/静态生命周期证据，仍不替代真实 Wi‑Fi/4G fault-domain、网络 abort 或 COM3–COM6 HIL。

> 2026-08-31 B3/C7 startup-pet retry timer-fence race closure：Startup Pet Retry 的 schedule 在 `esp_timer_start_once()` 返回后重新校验 callback admission、System-Sleep/stop fence 与 retained timer；迟到 start 会立即 stop 并返回 BUSY，避免父事务已停止 timer 后旧代际重新 arm。HAL/System-Sleep gates 通过；仍不替代真实 renderer/DMA/PSRAM 故障域、Light/Deep Sleep 或 COM3–COM6 HIL。

> 2026-08-31 B3/C7 Power DISPLAY_OFF timer-fence race closure：Power Service 的 idle deadline arm 及 contention retry 现在在 `esp_timer_start_once()` 返回后复核 retained timer、generation 与 PREPARE/deinit admission；迟到 start 会立即 stop 且只清理本代状态，避免 teardown 已停止 timer 后旧 worker 重新 arm。Configuration/System-Sleep/HAL gates 与 Fangtang-4G ESP-IDF 6.0.2 完整 Ninja 构建通过，app `0x35ca10`，最小 app 分区余 `0x435f0`（7%）。该收口仍不替代真实 panel/DMA restart、Light/Deep Sleep 或 COM3–COM6 HIL。

> 2026-08-31 B3/C7 Configuration Reconcile timer-fence race closure：expiry/retry timer 在 `PREPARE` 竞态下增加 start-after-admission 复核；回调本身也拒绝 System-Sleep fence，避免父事务已停止 timer 后被迟到 rearm 唤醒 retained worker。该收口仅为源码/静态生命周期证据，仍不替代真实 HAL 停机、网络 abort、物理故障域或 COM3–COM6 HIL。

> 2026-08-31 B3/C7 Configuration runtime-override admission fence：运行时覆盖的 apply/remove/clear 入口现在先取得 Reconcile serialized owner，并在同一代际下拒绝 System Sleep、deinit 或已有 reconcile；不会在返回 BUSY 后留下已发布但未应用的 effective revision。该收口仅证明值/生命周期边界，仍不替代真实网络切换、物理 HAL 故障域或四板 HIL。

> 2026-08-31 B3/C7 Connectivity deinit stopping fence：Connectivity 在删除 Wi‑Fi attempt EventGroup 后，保持 `s_connectivity_stopping` 直到 fault-domain 确认进入 STOPPED；避免预先释放 stopping 标记时，允许未初始化阶段的 uplink selection 进入仍处于 QUIESCING 的半拆代际。新增 HAL boundary 顺序断言并通过；真实 Wi‑Fi/4G deinit、runtime restart 与 COM3–COM6 HIL 仍未完成。

> 2026-08-31 B3/C7 shared-bus teardown retry fence：圆屏 Audio 的普通 release 现在只接受已确认 READY 且无 active borrower 的 shared-bus generation；UNKNOWN_OUTCOME、并发 recovery 或仍有 lease 时立即返回生命周期错误，不会擅自把不确定/占用中的物理句柄推进到 teardown。新增 wiring gate 断言该 admission fence；相关 shared-bus lifecycle/recovery gates 通过。该源码级收口仍不替代真实 driver delete/recreate、SDA-stuck、并发 codec/PMIC/IMU 或 COM3–COM6 HIL。

> 2026-08-31 B3/A9 Gateway Transport request-input boundary fence：Gateway Transport 初始化发布与取消 generation 现保持同一临界区原子可见；通用 request 入口拒绝空 method/path、负 body 长度及非零长度配空 body，Fangtang ML307 的普通/stream HTTP 入口同步拒绝空 method/URL、非正 timeout 与非法 body 组合。静态 Gateway/HAL/System-Sleep gates、Host 回归及 Fangtang-4G ESP-IDF 6.0.2 完整 Ninja 构建通过（app `0x35c870`，最小 app 分区余 `0x43790` / 7%）。该切片仅收口 value/lifecycle 边界，不替代真实 HTTP/ML307 abort、Wi-Fi/4G fault-domain、renderer/DMA/I2S/PSRAM 故障域、Light/Deep Sleep 或 COM3–COM6 HIL。

> 2026-08-31 B3/C7 reconcile lifecycle absolute-deadline fence：Configuration Reconcile Service 的 deinit 与 System Sleep PREPARE 现在以调用方单调绝对 deadline 贯穿 expiry worker drain 与 serialized mutex acquisition；后续阶段只使用实际剩余预算，不会各自重新消费完整 timeout。Reconcile service、System-Sleep、HAL gates 与 Fangtang-4G 增量 Ninja 构建通过（app `0x35c950`，最小 app 分区余 `0x436b0` / 7%）。该切片仍是生命周期边界证据，不替代真实 Display/Audio/Connectivity 硬件停机、Light/Deep Sleep 或 COM3–COM6 HIL。

> 2026-08-31 B3/C7 Gateway general-lane owner fence：共享 transport lane 现在记录持有任务；只有 owner 能释放 mutex，跨任务或重复 unlock 会被忽略，避免迟到 Meeting/cleanup 破坏后继 HTTP 请求。初始化与 cancellation gates、Fangtang-4G Ninja 构建通过；真实网络 abort、4G 故障域、睡眠和四板 HIL 仍未完成。

> 2026-08-31 B3/C7 Gateway Transport init transaction fence：`gateway_transport_init()` 增加并发初始化 admission，所有 HTTP/asset/startup mutex 与 semaphore 先在局部完成创建，成功后才一次性发布 host/句柄；中途分配失败会逆序释放已创建资源，重复初始化不替换既有 generation。Gateway cancellation、HAL、System-Sleep gates 与 Fangtang-4G Ninja 构建通过；真实 4G fault-domain、runtime restart 触发及 COM3–COM6 HIL 仍未完成。
> 2026-08-31 B3/C7 shared-bus recovery owner deadline fence：圆屏 Audio recovery 的 Audio ownership mutex 获取现在与后续 shared-bus lock、detach/recreate/reprobe transaction 共用同一个调用方绝对 deadline；不再先消费完整 timeout 再把剩余预算传给物理恢复。小于一 tick 的正预算向上取整，deadline/毫秒转换溢出保持 fail-closed。Round shared-bus wiring、shared-bus transaction 与 HAL gates 通过，Fangtang-4G 增量构建通过（app `0x35c950`，最小 app 分区余 `0x436b0` / 7%）。仍不替代 EchoEar/Waveshare 的真实 SDA-stuck、codec/PMIC/IMU 并发、物理重建与 COM3–COM6 HIL。
> 2026-08-31 B3/C7 shared-bus teardown admission fence：圆屏 codec adapter 在无法取得当前 shared-bus teardown ownership（包括 UNKNOWN_OUTCOME/并发 recovery）时不再触碰 I2C、codec 或 I2S retained handle，直接返回 lifecycle 错误；只有成功取得 teardown fence 才执行反序物理清理。Round wiring、shared-bus transaction、HAL/System-Sleep gates 与 Fangtang-4G 增量构建通过（app `0x35c950`，最小 app 分区余 `0x436b0` / 7%）。仍不替代真实 SDA-stuck、driver delete/recreate 失败、并发音频/触摸/PMIC/IMU 与 COM3–COM6 HIL。

> 2026-08-31 B3/C1 meeting cellular-stream cancellation fence：Gateway Transport 将蜂窝 Meeting PUT 的 opaque owner 纳入统一 active-request registry，并将 `CANCEL_MEETING_STREAM` 纳入 `CANCEL_ALL`；System Sleep/Connectivity drain 现在可通过 profile owner-cancel 触达无 ESP HTTP handle 的 ML307 流，迟到/仍存活 owner 会返回 `BUSY`。静态 gate、HAL gate、Fangtang-4G Ninja 构建通过；仍不替代真实 ML307 abort、4G 故障域和 COM3–COM6 HIL。

> 2026-08-31 A9 server-audio inline wake-lease admission fence：Gateway Dispatcher 处理内联 Base64 音频时现在必须成功取得独占 server-audio wake lease 才允许解码/播放；若 lease 已被其他任务持有则保留消息重试，且不会错误 finish 他人 lease。相关 media-transfer 与 Gateway cancellation 静态门禁、`git diff --check` 通过；该源码级证据仍不替代真实 I2S/HTTP abort、4G fault-domain、Light/Deep Sleep 与 COM3–COM6 HIL。

> 2026-08-31 A9 optional wake lease owner fence：共享 media transfer service 现为每个成功 optional wake lease 记录取得它的 FreeRTOS task；finish 只能由同一 task 消费自己的 slot，跨任务/重复 finish 保持 fail-closed，不会递减其他事务的 wake-memory lease。Host media/pet-asset 回归、HAL/System-Sleep gates 与 Fangtang-4G 增量 Ninja 构建通过（app `0x35c4a0`，最小 app 分区余 `0x43b60` / 7%）。该源码/build 证据仍不替代真实 HTTP cancel、I2S/PSRAM 故障域、Light/Deep Sleep 与 COM3–COM6 HIL；A9/C1 继续保持未完成声明。

> 2026-08-31 B3/A9 native send/connect timeout fence：原生 TCP connect 改用非阻塞 `select/SO_ERROR` 10 秒握手，TCP/UDP 设置 10 秒发送超时，TLS connect 设置显式 10 秒 esp-tls timeout；TLS 接收 `WANT_READ` 增加让步与停滞 deadline。Fangtang-4G ESP-IDF 6.0.2 完整构建链接通过，app `0x35ab80`，最小 app 分区余 `0x45480`（7%）。真实 socket/TLS abort、4G fault-domain 与 COM3–COM6 HIL 仍未完成。

> 2026-08-31 B3/A9 native TCP connect deadline fence：`EspTcp::Connect()` 现在采用非阻塞 connect + 10 秒 `select`/`SO_ERROR` 握手预算，并在完成后恢复阻塞模式；连接/任务创建失败会立即清理 fd，避免网络故障时无限阻塞。Fangtang-4G ESP-IDF 6.0.2 完整构建链接通过，app `0x35ab80`，最小 app 分区余 `0x45480`（7%）。真实 socket abort、4G fault-domain 与 COM3–COM6 HIL 仍未完成。

> 2026-08-30 B3/A9 DTR guard ownership fence：`DtrGuard` 现在按 DTR 引脚是否存在判定是否启用，而不是读取当前逻辑电平；低电平初始化的 modem 不再跳过互斥/PM lock。PM lock 句柄为空时也保持安全退化。Fangtang-4G 构建与 ML307/HAL 静态门禁通过；真实 DTR/睡眠唤醒与 COM3–COM6 HIL 仍未完成。

> 2026-08-30 B3/A9 native socket transport input/worker fence：`EspTcp/EspUdp/EspSsl` 现在拒绝空 host、非法端口；连接建立后检查接收任务创建失败并清理 socket/TLS；Send/Receive 固定 fd，避免重连竞态使用新句柄；TLS 关闭不再使用 `ESP_ERROR_CHECK`，仅在 worker 已退出后销毁 TLS owner。Fangtang-4G ESP-IDF 6.0.2 完整构建链接通过，app `0x35ab80`，最小 app 分区余 `0x45480`（7%）。真实 socket/TLS abort、4G fault-domain 与 COM3–COM6 HIL 仍未完成。

> 2026-08-30 B3/A9 UART command/initialization transaction fence：`AtUart::SendCommandWithData` 拒绝零 timeout，并让 command 与 payload ACK 共用绝对 deadline；`Initialize` 的 UART/UHCI/DMA/queue/semaphore/task 创建改为事务式，任一中途失败都会停止 DMA、唤醒并 join 已创建 worker、撤销 callback 后再释放资源。Fangtang-4G ESP-IDF 6.0.2 增量构建通过，app `0x35ab80`，最小 app 分区余 `0x45480`（7%）。真实 UART 分包、4G fault-domain 与 COM3–COM6 HIL 仍未完成。

> 2026-08-30 B3/A9 timeout-input validation fence：`AtModem::Detect` 与 `WaitForNetworkReady` 现在明确拒绝 0 或非法负 timeout；无限等待仅接受 `-1`，避免负值意外转换为超大无符号预算。Fangtang-4G ESP-IDF 6.0.2 完整 configure/build/link 通过，app `0x35ab80`，最小 app 分区余 `0x45480`（7%）。真实 modem fault-domain、UART 分包及 COM3–COM6 HIL 仍未完成。

> 2026-08-30 B3/A9 UART baud-detection deadline fence：`AtUart::DetectBaudRate/SetBaudRate` 现在拒绝非法 timeout，以单调绝对父 deadline 约束每个波特率探测、退避和 `AT+IPR` 切换命令；不再因每轮 `AT` 探测或固定 1 秒 sleep 叠加超时。Fangtang-4G ESP-IDF 6.0.2 完整 configure/build/link 通过，app `0x35a740`，最小 app 分区余 `0x458c0`（7%）。真实 UART 分包、4G fault-domain 与 COM3–COM6 HIL 仍未完成。

> 2026-08-30 B3/A9 modem command/lifecycle deadline fence：ML307/EC801E 的 Reboot、SetSleepMode、flight-mode 与构造初始化命令现在统一使用有界 UART timeout，失败显式记录并 fail-closed；ML307 HTTP slot 清理逐项检查返回值；ML307 `WaitForNetworkReady` 的 PDP/IP 查询复用调用方原始父 deadline，避免 CEREG 成功后额外叠加独立 4.27 s 预算；负 delay/未初始化 UART 直接拒绝。Fangtang-4G ESP-IDF 6.0.2 完整 configure/build/link 通过，app `0x35a4d0`，最小 app 分区余 `0x45b30`（8%）。真实 UART 分包、HTTP abort、4G fault-domain 与 COM3–COM6 HIL 仍未完成。

> 2026-08-30 B3/A9 EC801E HTTP/UART lifecycle fence：通用 `HttpClient` 的 body 队列、chunked/trailer 终态、parser callback retirement 与读取锁序保持 fail-closed；`AtUart::Shutdown()` 以单调绝对 deadline 统一 join receive/event worker，迟到 DMA/overflow ISR 不再触碰已停用 queue/event，URC callback 先快照后调用以支持自注销。Fangtang-4G ESP-IDF 6.0.2 完整 configure/build/link 通过，app `0x359350`，最小 app 分区余 `0x46cb0`（8%）。真实 EC801E/ML307 UART 分包、HTTP abort、4G fault-domain 与 COM3–COM6 HIL 仍未完成。
...
| A8 | Gateway Dispatcher + Tool Registry：单 reader、ACK/cursor、tool/result 分发、durable outbox | 不存在；Gateway HTTP/长轮询在 main.c（直接 include `esp_http_client.h:17`） | mutation 前后、result POST 前后、ACK 前后断网均无重复副作用或消息丢失。**2026-08-19 第一增量完成：`gateway_dispatcher.c`。`gateway_transport.c` 已接管 request 车道/握手/voice pair/startup worker；2026-08-23 再接管 meeting chunk 的 Wi-Fi HTTP stream、active client cancel、可复用 keep-alive 及蜂窝 fallback，Meeting Service 仅调用值契约。2026-09-02 本轮 tool-result 投递编排（toolCall 分发与错误码/retryable 映射、delivered-id 去重、outbox flush 与持久化 fallback、factory_reset 送达门禁）已迁入 `services/gateway_tool_result_service.c`，main.c 仅保留 dispatcher host 薄壳转发。ACK/重投真机回归未做** |
...
| A12 | main.c 收尾：删除已迁移的全局状态与编排，只留 boot composition root | main.c 约 5.6k 行（2026-09-02 为 5,554 行；2026-08-18 基线为 12,894 行、约 376 个 static 函数） | main.c 不含 command/reply/meeting/alarm/portal/weather 全局状态；启动按依赖拓扑。**2026-08-20：main.c 约 6,930 非空行；weather/display-clock/portal HTTP+DNS 已迁出，Wi-Fi/scheduled/pet_state 发布已转 Ambient。2026-08-23 SNTP monitor/task registry/retry/System-Sleep owner 已抽为 `clock_sync_service`；本轮 pet-cache task/mutex/stop flag/registry callback 也已从 root 删除，转由 `pet_cache_service` 私有拥有。root→service 仅传 descriptor、lease 和取消 probe；service→root probe 均在 service lock 外调用，root 的 System Sleep ABORT 也在释放 `s_task_state_lock` 后才进入 service，避免锁序反转。pet cache worker 只有在 immutable Storage Registry identity 登记完成后才发布 handle，create→publish 中的 stop 失败闭合为 busy；这项保护也由 HAL gate 检查。2026-09-02：Wi-Fi/SoftAP/STA/DHCP/netif/httpd/cellular/SNTP 全局均已迁出（Wi-Fi 经 `wifi_startup_service` host glue），本轮 Gateway tool-result 投递编排亦迁入 `gateway_tool_result_service`；root 剩余为各 service 的 host glue 与 startup 拓扑编排，不再持有业务队列/去重状态** |
...
> 2026-08-30 B3/A9 EC801E TCP send/connect deadline fence：`Ec801ETcp::Send()` 现在以单调父 deadline 覆盖整次多分片发送，限制每片重试次数并在断连/错误事件时立即退出；不再对每次失败无限重试或为每个 chunk 重新开启完整 10 秒等待。`QISEND` URC 严格校验三参数类型，并要求确认字节数与请求 chunk 完全一致；`Connect()` 的配置、旧实例关闭、打开及连接等待共享同一 10 秒预算。Fangtang-4G ESP-IDF 6.0.2 完整构建通过，app `0x3597b0`，最小 app 分区余 `0x46850`（8%）。真实 UART 分包、HTTP abort、4G fault-domain 与 COM3–COM6 HIL 仍未完成。
> 2026-08-30 B3/A9 EC801E SSL deadline/ack fence：`Ec801ESsl` 的 Connect/Send 现在共享单调父 deadline；配置、旧实例关闭、打开与连接等待不再各自重新开始完整 10 秒预算，发送失败重试限制为 3 次并在断连/错误时立即结束。`QISEND` URC 严格校验完整三参数及确认长度，短确认不会被当作成功。Fangtang-4G ESP-IDF 6.0.2 完整构建通过，app `0x3599d0`，最小 app 分区余 `0x46630`（8%）。真实 SSL/UART 分包、HTTP abort、4G fault-domain 与 COM3–COM6 HIL 仍未完成。
> 2026-08-30 B3/A9 EC801E UDP deadline/ack fence：`Ec801EUdp` 的 Connect/Send 现在使用单调父 deadline；配置、旧实例关闭、打开与连接等待共享统一预算。发送失败重试限制为 3 次，断连/错误立即终止；`QISEND` 严格校验三参数类型并要求确认长度精确等于 payload。Fangtang-4G ESP-IDF 6.0.2 完整构建通过，app `0x359b80`，最小 app 分区余 `0x46480`（8%）。真实 UDP/UART 分包、4G fault-domain 与 COM3–COM6 HIL 仍未完成。
> 2026-08-30 B3/A9 EC801E MQTT deadline/URC fence：`Ec801EMqtt::Connect()` 现在以单调父 deadline 覆盖 SSL/版本/session/keepalive/dataformat 配置、QMTOPEN、冲突实例清理与 QMTCONN 等阶段，避免递归重连和每阶段重新开始完整 10 秒等待；QMTRECV/QMTCONN 进一步严格校验 arity 与字段类型。Fangtang-4G ESP-IDF 6.0.2 完整构建通过，app `0x359c30`，最小 app 分区余 `0x463d0`（8%）。真实 MQTT/UART 分包、4G fault-domain 与 COM3–COM6 HIL 仍未完成。
> 2026-08-30 B3/A9 ML307 TCP/SSL deadline/URC fence：`Ml307Tcp` 现在以单调父 deadline 覆盖 MIPSTATE、旧实例关闭、SSL/编码配置、MIPOPEN 及连接等待；发送阶段也使用同一有界预算，不再为每个 chunk 重新开启完整 10 秒等待。`MIPSEND` 失败码不再伪造成功，并在错误/断连事件中唤醒发送者；SSL 子类配置复用同一有界命令 seam。Fangtang-4G ESP-IDF 6.0.2 完整构建通过，app `0x359da0`，最小 app 分区余 `0x46260`（8%）。真实 ML307 TCP/SSL UART 分包、abort、4G fault-domain 与 COM3–COM6 HIL 仍未完成。
> 2026-08-30 B3/A9 ML307 UDP deadline/URC fence：`Ml307Udp` 的 MIPSTATE、旧实例关闭、编码/SSL 配置、MIPOPEN 与连接等待现在共享单调父 deadline；发送命令与确认等待复用同一预算，断连/错误立即 fail-closed。MIPSEND 非零返回码不再被误报为成功。Fangtang-4G ESP-IDF 6.0.2 完整构建通过，app `0x359f70`，最小 app 分区余 `0x46090`（8%）。真实 ML307 UDP/UART 分包、abort、4G fault-domain 与 COM3–COM6 HIL 仍未完成。
> 2026-08-30 B3/A9 ML307 MQTT deadline/URC fence：`Ml307Mqtt` 的 Connect、IsConnected、Disconnect、Publish、Subscribe 与 Unsubscribe 现在统一使用单调父 deadline；MQTT publish URC 严格校验 arity/type，分片 payload 在新连接成功时清理，避免旧代数据污染。Fangtang-4G ESP-IDF 6.0.2 完整构建通过，app `0x35a0b0`，最小 app 分区余 `0x45f50`（8%）。真实 MQTT/UART 分包、abort、4G fault-domain 与 COM3–COM6 HIL 仍未完成。
> 2026-08-30 B3/A9 ML307 HTTP Open deadline fence：`Ml307Http::Open()` 现在以单调父 deadline 覆盖 MHTTPCREATE、slot 初始化、SSL/chunked/encoding/timeout/header/content/request 以及 chunked IND 等阶段；配置和 payload command 不再在每个阶段重新启动完整 timeout。Fangtang-4G ESP-IDF 6.0.2 完整构建通过，app `0x35a150`，最小 app 分区余 `0x45eb0`（8%）。真实 HTTP abort、UART 分包、4G fault-domain 与 COM3–COM6 HIL 仍未完成。
> 2026-08-30 B3/A9 ESP native transport lifecycle fence：`EspTcp`/`EspUdp`/`EspSsl` 关闭流程现在在同一绝对 10 秒预算内先停止连接、唤醒阻塞接收，再等待 worker 退出；TLS `WANT_READ/WANT_WRITE` 忙等加入让步与 deadline，避免无限 spin。TCP/UDP socket 关闭前执行 shutdown，减少迟到 recv 访问已销毁资源。Fangtang-4G ESP-IDF 6.0.2 完整构建通过，app `0x35a150`，最小 app 分区余 `0x45eb0`（8%）。真实网络故障注入、TLS/Socket abort、4G fault-domain 与 COM3–COM6 HIL 仍未完成。
> 2026-08-30 B3/A9 ESP native receive-worker retirement fence：`EspTcp`/`EspUdp`/`EspSsl` 被动断连后仍保留 receive-task identity，析构和重连会继续执行 bounded drain；TCP/UDP socket 先 shutdown 唤醒阻塞 recv，TLS/Socket worker 退出后才允许 event group/transport owner 回收，避免“connected=false 但旧 worker 仍在运行”的重入窗口。Fangtang-4G ESP-IDF 6.0.2 完整构建通过，app `0x35a150`，最小 app 分区余 `0x45eb0`（8%）。真实网络故障注入、TLS/Socket abort、4G fault-domain 与 COM3–COM6 HIL 仍未完成。
> 2026-08-30 B3/C5 AtModem network-wait deadline fence：`AtModem::WaitForNetworkReady()` 现在以单调父 deadline 覆盖 SIM 查询重试、CEREG 配置/查询与注册事件等待；取消、命令阶段及 100 ms 轮询均消耗同一预算，不再在前置 CPIN 重试后重新开启完整 network timeout。Fangtang-4G ESP-IDF 6.0.2 完整构建通过，app `0x35a230`，最小 app 分区余 `0x45dd0`（8%）。真实 SIM/网络故障注入、4G fault-domain 与 COM3–COM6 HIL 仍未完成。
> 2026-08-31 B3/A9 UART transactional cleanup fence：`AtUart::Initialize()` 现在在 UHCI `Init()` 成功后立即记录 ownership，RI ISR 注册采用显式 registered 标志；部分初始化失败时先撤销 ISR、归还队列中的 DMA buffer，再销毁 UHCI，确保 GPIO/worker 创建失败后对象可安全重用。Fangtang-4G ESP-IDF 6.0.2 增量构建链接通过，app `0x35acf0`，最小 app 分区余 `0x45310`（7%）。真实 UART 分包、HTTP/socket abort、4G fault-domain 与 COM3–COM6 HIL 仍未完成。
> 2026-08-31 B3/A9 native transport callback self-disconnect fence：ESP TCP/UDP/TLS 的 Disconnect 现在识别 receive worker 自身调用，关闭底层 fd/socket 但不等待自身 exit bit，避免 stream/message callback 中自断连死锁；event-group 创建失败也显式返回 ENOMEM。Fangtang-4G ESP-IDF 6.0.2 增量构建链接通过，app `0x35acf0`，最小 app 分区余 `0x45310`（7%）。真实 socket/TLS abort、4G fault-domain 与 COM3–COM6 HIL 仍未完成。
> 2026-08-31 B3/A9 modem endpoint input fence：ML307 与 EC801E 的 TCP/UDP/TLS Connect 统一拒绝空主机、越界端口、控制字符、引号、逗号、反斜杠及超长 authority，避免 AT 命令注入和未定义 modem 解析；Fangtang-4G ESP-IDF 6.0.2 增量链接通过，app `0x35af70`，最小 app 分区余 `0x45090`（7%）。真实 4G fault-domain、UART 分包、HTTP/socket abort 与 COM3–COM6 HIL 仍未完成。
> 2026-08-31 B3/A9 modem endpoint input fence：ML307 与 EC801E 的 TCP/UDP/TLS Connect 统一拒绝空主机、越界端口、控制字符、引号、逗号、反斜杠及超长 authority，避免 AT 命令注入和未定义 modem 解析；Fangtang-4G ESP-IDF 6.0.2 增量链接通过，app `0x35af80`，最小 app 分区余 `0x45080`（7%）。真实 4G fault-domain、UART 分包、HTTP/socket abort 与 COM3–COM6 HIL 仍未完成。
> 2026-08-31 B3/A9 native UDP retirement fence：`EspUdp::Disconnect()` 即使接收 worker 先因 peer 错误清空 fd，也始终按 task identity 等待退出 bit；此前仅在 fd 仍有效时 join，可能留下未退役 worker。Fangtang-4G ESP-IDF 6.0.2 增量链接通过，app `0x35af80`，最小 app 分区余 `0x45080`（7%）。真实 socket/TLS abort、4G fault-domain 与 COM3–COM6 HIL 仍未完成。
> 2026-08-31 B3/A9 modem MQTT lifecycle/input fence：ML307 与 EC801E MQTT 构造函数对 EventGroup 分配失败 fail-closed，并仅撤销已成功注册的 URC callback；Connect/Publish/Subscribe/Unsubscribe 拒绝空/超长/控制字符/引号/反斜杠等 AT 文本注入输入，QoS 限定 0–2，且连接代际切换时清理分片 payload。ML307 lifecycle、HAL boundary 与 Fangtang-4G ESP-IDF 6.0.2 完整 Ninja 构建通过，app `0x35b2d0`，最小 app 分区余 `0x44d30`（7%）。真实 MQTT/UART 分包、4G fault-domain 与 COM3–COM6 HIL 仍未完成。
> 2026-08-31 B3/A9 WebSocket transport fence：WebSocket 现在在 EventGroup、URI scheme/authority/端口、握手状态行、控制帧、fragment 及 payload 指针边界上 fail-closed；析构时先断开并解绑 TCP callback，再释放握手 EventGroup，握手发送/超时失败主动断开，防止迟到回调和资源泄漏。ML307 lifecycle、HAL boundary 与 Fangtang-4G ESP-IDF 6.0.2 Ninja 构建通过，app `0x35b2d0`，最小 app 分区余 `0x44d30`（7%）。真实 WebSocket 分包/abort、4G fault-domain 与 COM3–COM6 HIL 仍未完成。
> 2026-08-31 B3/A9 WebSocket frame/URI hardening：补充 host header 与接收缓存上限、URI authority 端口解析、握手失败主动断开、服务端 frame 禁止 mask、fragment/control opcode 与消息累计长度校验；Close/重连复位 parser state，析构解绑 TCP callback 后再释放同步对象。ML307 lifecycle、HAL boundary 与 Fangtang Ninja 构建通过，app `0x35b2d0`，最小 app 分区余 `0x44d30`（7%）。真实 WebSocket 分包、abort、4G fault-domain 与 COM3–COM6 HIL 仍未完成。

> 2026-08-31 B3/A9 WebSocket handshake/close semantics fence：握手现校验 `Upgrade`、`Connection: Upgrade` 与基于随机 key 的 `Sec-WebSocket-Accept`（RFC 6455 SHA-1/base64）；文本消息及 Close reason 做 UTF-8 校验，收到 Close 会回发一次 Close 并复位连接/fragment 状态。Fangtang-4G ESP-IDF 6.0.2 增量 Ninja 构建通过，app `0x35b300`，最小 app 分区余 `0x44d00`（7%）。真实 WebSocket 分包、abort、4G fault-domain 与 COM3–COM6 HIL 仍未完成。

> 2026-08-31 B3/A9 native/modem endpoint lifecycle fence：ML307/EC801E TCP、UDP、SSL transport 对 EventGroup 创建及 UART owner 做 fail-closed 校验，析构时仅撤销已成功注册的 URC callback，并在释放后清空句柄；Connect 入口拒绝未初始化 transport。Fangtang-4G ESP-IDF 6.0.2 增量 Ninja 构建通过，app `0x35b460`，最小 app 分区余 `0x44ba0`（7%）。真实 UART 分包、socket/4G fault-domain 与 COM3–COM6 HIL 仍未完成。

> 2026-08-31 B3/A9 generic HTTP/native MQTT allocation fence：通用 `HttpClient` 与 ESP MQTT transport 现在显式记录 EventGroup 有效性；析构释放后清空句柄，Open/Connect 在依赖对象或同步资源无效时立即 fail-closed，避免空 EventGroup 上等待/发布事件。Fangtang-4G ESP-IDF 6.0.2 增量 Ninja 构建通过，app `0x35b490`，最小 app 分区余 `0x44b70`（7%）。真实网络 abort、UART 分包、4G fault-domain 与 COM3–COM6 HIL 仍未完成。
> 2026-08-31 B3/A9 generic HTTP reassembly fence：通用 `HttpClient::OnTcpData()` 对未解码 TCP 重组缓存设置 64 KiB 上限，超限立即终止 parser 并唤醒 reader；错误通知仅在 EventGroup owner 仍有效时发布，避免 slowloris/异常 transport callback 造成无界内存增长或迟到同步访问。Fangtang-4G ESP-IDF 6.0.2 增量 Ninja 构建通过，app `0x35bfa0`，最小 app 分区余 `0x44060`（7%）。真实 HTTP abort、UART 分包、4G fault-domain 与 COM3–COM6 HIL 仍未完成。
> 2026-08-31 B3/A9 generic HTTP suffix retirement fence：HTTP parser 在 TCP callback 中遇到超限重组后立即清空未解码数据并进入终态错误，禁止后续迟到 callback 复用旧 parser state；错误事件仅触达仍有效的 EventGroup owner。Fangtang-4G ESP-IDF 6.0.2 增量 Ninja 构建通过，app `0x35c030`，最小 app 分区余 `0x43fd0`（7%）。真实 HTTP abort、UART 分包、4G fault-domain 与 COM3–COM6 HIL 仍未完成。

> 2026-08-31 B3/A9 modem readiness and transport allocation fence：`AtModem` 构造及 `WaitForNetworkReady()` 现在校验 EventGroup、UART owner 与 UART 初始化状态；通用 HTTP 在创建 TCP/SSL transport 后检查空对象，ESP MQTT 对 client init、event registration、start 逐步检查并清理失败句柄。Fangtang-4G ESP-IDF 6.0.2 增量 Ninja 构建通过，app `0x35b570`，最小 app 分区余 `0x44a90`（7%）。真实网络 abort、UART 分包、4G fault-domain 与 COM3–COM6 HIL 仍未完成。
> 2026-08-31 B3/A9 native socket authority/retirement fence：原生 TCP/UDP/TLS Connect 现在拒绝控制字符、引号、逗号、反斜杠、路径/fragment 等非法 host authority；EventGroup 未创建时 Disconnect 仍安全关闭底层 owner，不等待无效同步对象。Fangtang-4G ESP-IDF 6.0.2 增量 Ninja 构建通过，app `0x35b950`，最小 app 分区余 `0x446b0`（7%）。真实网络 abort、4G fault-domain 与 COM3–COM6 HIL 仍未完成。

> 2026-08-31 B3/A9 native MQTT input/timeout fence：ESP MQTT 现在限制 broker/client/用户名/密码文本长度及控制字符，QoS 限定 0–2，keepalive 必须为正；等待连接超时或错误会主动销毁 client，事件回调在同步资源无效或 event data 为空时直接丢弃。Fangtang-4G ESP-IDF 6.0.2 增量 Ninja 构建通过，app `0x35b570`，最小 app 分区余 `0x44a90`（7%）。真实 MQTT 网络故障、socket abort、UART 分包、4G fault-domain 与 COM3–COM6 HIL 仍未完成。

> 2026-08-31 B3/A9 ML307 HTTP owner allocation fence：`Ml307Http` 现在对 EventGroup、UART owner 与 URC callback 注册过程做显式有效性记录；析构仅撤销已注册 callback，并在释放后清空同步句柄，避免内存分配失败路径仍接受 URC。Fangtang-4G ESP-IDF 6.0.2 增量 Ninja 构建通过，app `0x35b600`，最小 app 分区余 `0x44a00`（7%）。真实 HTTP abort、UART 分包、4G fault-domain 与 COM3–COM6 HIL 仍未完成。

> 2026-08-31 B3/A9 ML307 HTTP readiness fence：`Ml307Http::Open()` 现在在发起 MHTTPCREATE 前确认 EventGroup、UART owner 与 UART 初始化状态，避免未初始化串口上进入异步 HTTP 请求。Fangtang-4G ESP-IDF 6.0.2 增量 Ninja 构建通过，app `0x35b610`，最小 app 分区余 `0x449f0`（7%）。真实 HTTP abort、UART 分包、4G fault-domain 与 COM3–COM6 HIL 仍未完成。
> 2026-08-31 B3/A9 MQTT/SSL callback retirement fence：`Ml307Mqtt` 与 `Ec801EMqtt` 构造函数在 UART owner 为空或 EventGroup 分配失败时不注册 URC；所有 URC 入口、连接与发布操作均检查同步资源和 UART 生命周期，析构先退休回调并清空连接状态。`Ec801ESsl` 析构在回调解绑后释放 EventGroup，Disconnect/Send 对退休状态 fail-closed，避免迟到 URC 访问已释放对象。Fangtang-4G ESP-IDF 6.0.2 增量 Ninja 构建通过，app `0x35b950`，最小 app 分区余 `0x446b0`（7%）。真实 MQTT/SSL 分包、网络 abort、4G fault-domain 与 COM3–COM6 HIL 仍未完成。
> 2026-08-31 B3/A9 UART bounded reassembly fence：`AtUart` 对跨 DMA buffer 的响应重组增加 16 KiB 上限；超限立即清空残帧、锁存显式 command error 并唤醒等待者，避免缺失 CRLF 的恶意/故障输入无限增长或污染下一条命令。Receive/Event worker 与 SendCommand 同步资源无效时均 fail-closed。Fangtang-4G ESP-IDF 6.0.2 增量 Ninja 构建通过，app `0x35ba40`，最小 app 分区余 `0x445c0`（7%）。真实 UART 电气分包与 4G fault-domain/HIL 仍未完成。
> 2026-08-31 B3/A9 UART quoted-field reassembly fence：AT URC 参数解析改用有界 quoted-field tokenizer，保留引号字段内的逗号（MQTT topic/HTTP header 等），并拒绝未闭合引号或转义尾巴；畸形帧不再被错误拆成可执行 schema。Fangtang-4G ESP-IDF 6.0.2 增量 Ninja 构建通过，app `0x35bbb0`，最小 app 分区余 `0x44450`（7%）。真实 UART 电气分包、4G fault-domain 与 COM3–COM6 HIL 仍未完成。
> 2026-08-31 B3/A9 UART overlong-frame retirement fence：超长 AT 帧不再只丢弃当前 DMA buffer；`AtUart` 会持续丢弃跨 buffer 后续字节直到完整 `CRLF`，同一 buffer 中 terminator 后的下一帧仍被保留解析，避免 overlong/malformed 输入尾部伪装成新命令或 URC。Fangtang-4G ESP-IDF 6.0.2 增量 Ninja 构建通过，app `0x35bc40`，最小 app 分区余 `0x443c0`（7%）。真实 UART 电气分包、4G fault-domain 与 COM3–COM6 HIL 仍未完成。
> 2026-08-31 B3/A9 UART CSV resource fence：AT URC tokenizer 限制单字段 4 KiB、总参数 64 个，并在超过边界时整体拒绝，防止异常逗号/引号输入触发不可控 vector/string 分配；Fangtang-4G ESP-IDF 6.0.2 增量 Ninja 构建通过，app `0x35bcb0`，最小 app 分区余 `0x44350`（7%）。真实 UART 电气分包、4G fault-domain 与 COM3–COM6 HIL 仍未完成。
> 2026-08-31 B3/C5 AtModem callback retirement fence：`AtModem::Shutdown()` 现在先关闭 callback admission、撤销 URC，再停止并 join 借用的 UART worker；base/ML307/EC801E URC 入口均拒绝退休代际，EventGroup 释放延后至 UART 停止成功，避免迟到网络状态事件访问已释放同步对象。Fangtang-4G ESP-IDF 6.0.2 增量 Ninja 构建通过，app `0x35bd00`，最小 app 分区余 `0x44300`（7%）。真实 4G fault-domain 与 COM3–COM6 HIL 仍未完成。
> 2026-08-31 B3/C5 modem lifecycle atomic retirement fence：`AtModem::shutdown_requested_` 改为原子状态，URC dispatch 与 Shutdown 的可见性不再依赖未定义的数据竞争；析构/重复 Shutdown 可安全重入，UART 停止失败时保留 EventGroup owner 等待下一次有界收尾。Fangtang-4G ESP-IDF 6.0.2 增量 Ninja 构建通过，app `0x35bd20`，最小 app 分区余 `0x442e0`（7%）。真实 4G fault-domain 与 COM3–COM6 HIL 仍未完成。
> 2026-08-31 B3/C5 transport readiness atomic fence：ML307/EC801E TCP、UDP、SSL、MQTT 及 AtModem 的 EventGroup 有效标志改为原子状态，迟到 URC 与析构/Disconnect 并发读取获得明确 acquire/release 语义；避免 retirement flag 数据竞争导致事件发布到失效同步对象。Fangtang-4G ESP-IDF 6.0.2 增量 Ninja 构建通过，app `0x35bf70`，最小 app 分区余 `0x44090`（7%）。真实 4G fault-domain 与 COM3–COM6 HIL 仍未完成。
> 2026-08-31 B3/A9 ML307 HTTP callback/EventGroup retirement fence：`Ml307Http` 的 MHTTPURC 回调统一检查 callback admission、EventGroup 与 UART readiness；析构先撤销 callback admission，再注销 URC 并释放 EventGroup。FailClosed/信号路径在 EventGroup 无效时安全短路，Close 保留 owner 负责的 MHTTPDEL slot 回收。Fangtang-4G ESP-IDF 6.0.2 Ninja 构建通过，app `0x35c0b0`，最小 app 分区余 `0x43f50`（7%）；真实 HTTP abort、UART 分包、4G fault-domain 与 COM3–COM6 HIL 仍未完成。
> 2026-08-31 B3/A9 EC801E generic HTTP callback/EventGroup retirement fence：`HttpClient` 增加 callback admission 原子闸门，TCP stream/disconnect 回调在对象析构或 EventGroup 退休后直接丢弃；Open/Connect 失败会显式 Disconnect 已分配 transport，避免失败握手遗留 socket/worker。解析器事件统一经有效 EventGroup helper 发布。Fangtang-4G ESP-IDF 6.0.2 增量 Ninja 构建通过，app `0x35c170`，最小 app 分区余 `0x43e90`（7%）；真实 HTTP abort、UART 分包、4G fault-domain 与 COM3–COM6 HIL 仍未完成。
> 2026-08-31 B3/A9 EC801E generic HTTP transport-generation fence：`HttpClient` 为每个 TCP owner 分配单调 generation，Close/重连先推进 generation；stream/disconnect callback 在进入及取得 parser lock 后双重校验 generation 与 admission，迟到旧 socket 数据不会污染新请求。Fangtang-4G ESP-IDF 6.0.2 增量 Ninja 构建通过，app `0x35c240`，最小 app 分区余 `0x43dc0`（7%）；真实 HTTP abort、UART 分包、4G fault-domain 与 COM3–COM6 HIL 仍未完成。
> 2026-08-31 A9/C7 media transfer lane owner fence：`media_transfer_service` 现在记录成功获取共享传输 lane 的 FreeRTOS task owner；只有同一 task 才能释放 lane，非 owner 或生命周期已变化的迟到清理会 fail-closed，避免重复 finish/跨任务回收让两个物理传输重叠。该切片仍不替代真实 HTTP cancel、I2S/PSRAM 故障域与 COM3–COM6 HIL；A9 继续保持未完成声明。

> 2026-08-31 C1/A9 media System Sleep participant fence：媒体传输服务新增可逆 System Sleep PREPARE/ABORT；PREPARE 关闭新的音频/宠物 media admission，并等待既有 lane 与 server-audio wake lease 释放，超时后保持闭锁直到 Power/Connectivity 逆序 ABORT。媒体状态不再可能在网络 participant 已停后重新打开 HTTP/PSRAM/I2S 借用。该增量仍不启用真实 Light/Deep Sleep、HTTP abort、wake rail/RTC 或四板 HIL；C1/A9 继续保持未完成声明。

> 2026-08-31 A9 optional media lease admission fence：optional wake lease 的 begin/finish 现返回明确结果；runtime pet 与 startup pet 只有在成功取得本事务 lease 后才继续，拒绝 begin 时立即释放 lane 并返回 `BUSY`，避免失败路径误释放其他事务的全局 lease。该切片仍不替代真实 HTTP cancel、I2S/PSRAM 故障域及 COM3–COM6 HIL。

> 2026-08-31 C1/A9 media cancel-order fence：修正 System Sleep cancellation bridge 顺序，先通过 Gateway Transport 取消所有已登记 HTTP/蜂窝请求，再等待 Media lane/wake lease 排空；避免媒体 GET 自身持有 Gateway client 时被 Media PREPARE 先行等待而形成无效超时。所有操作继续共享同一父 deadline，失败保持闭锁并交由 Power 逆序回滚。
# 2026-08-31 B3/A9 transport boundary follow-up

`gateway_transport_init()` 的资源与 owner/epoch 状态现在在同一临界区完成发布，取消调用方不会观察到半初始化 generation；Fangtang ML307 的两个公开 HTTP 入口拒绝零/负 timeout，并拒绝非零 body 长度配套的空 reader。Fangtang-4G Ninja 全量构建与 Gateway/Media/HAL 静态门禁通过。该源码/build 证据仍不替代真实 ML307 abort、4G fault-domain、Light/Deep Sleep 或 COM3–COM6 HIL。

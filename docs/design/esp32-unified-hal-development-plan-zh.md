> 2026-08-12 圆屏 Display HAL profile contract 收口（EchoEar-2ST / COM3，Waveshare 1.75C / COM6）：继 audio profile 后，新增私有、纯标量 `round_display_profile_t`，由 selected profile 提供 viewport width/height、transfer stripe rows 与 pet animation cadence。共享 `board_port.c` 现在只通过 `round_profile_display()` 获取 geometry/cadence，不再引用 `ROUND_PROFILE_LCD_*` 或 `ROUND_PROFILE_DISPLAY_*` 宏；SPI/QSPI controller、DMA buffer、ESP-LCD types、pins、颜色交换和 fence 所有权仍保留在各 profile-private display adapter。该 contract 不属于 Device/Platform API，未向共享 public header 传递 SDK/RTOS/driver type。EchoEar 与 Waveshare 正式 images 均完成完整构建并已写入相应 COM3/COM6，esptool hash 均成功。COM3 42 秒串口确认 360x360 board、`LCD double buffer ready: 2 x 259200 bytes in DMA PSRAM, 150 ms cadence`、audio/input ready 与 `BOOT_STATUS.ready=true`；同窗口未观察到 SERVICE_STATUS，故不宣称在线闭环。COM6 42 秒确认 466x466 board、`LCD double buffer ready: 2 x 434312 bytes in DMA PSRAM, 150 ms cadence`、audio/input ready、`BOOT_STATUS.ready=true`、`SERVICE_STATUS.ready=true`，无 startup failed/panic/Guru/abort/transfer failed。两者都是正式镜像，无需回刷。此增量只完成 profile facts 的收敛；round `board_port.c` 仍同时含 scene composition、input scanner、wake/capture/PCM session，尚未达到独立 Display/Input/Audio source 的 Phase 5/6 完成条件。

> 2026-08-14 Fangtang-4G 动态显示 HIL 修复与收敛（COM5）：复核确认“无 animation 日志”不是双缓冲缺失，而是 Display Service 创建 profile-private 动画 task 时遗漏 wrapper entry 的 `kind` context，worker 在入口参数校验后立即退出；现由私有 Display Service 将 context 连同 callback 交给 profile adapter，Bread/Fangtang adapter 仅保留各自 task 资源规格，业务层与 Device/Platform API 不接触 FreeRTOS。Fangtang ambient visual profile 同时补齐 `begin_frame`/`fill_screen`/`finish_frame` 事务，8 帧缓存宠物进入共享双缓冲和差分 presenter。COM5 正式 app 已写入并 hash 校验；冷启动日志确认 `pet animation worker started: motion=1 frames=8`、`frames=8/8`、`BOOT_STATUS.ready=true`、`SERVICE_STATUS.ready=true`，无 startup failed/panic/Guru/assert/stack overflow。HIL 表明真实瓶颈是宠物逐帧合成与 NV3023 40 MHz 单线 SPI，而不是 framebuffer：176 px/50 ms 请求时 render 通常 53--70 ms、实际 60--80 ms。Fangtang visual profile 已将宠物安全区域收敛为 160 px，Display profile 将多帧节奏收敛为 80 ms；共享文字绘制在 composed frame 中直接写入 back buffer，避免临时位图回拷贝。最终日志 `iot-agentos/serial-com5-fangtang-animation-final80ms-20260814.log` 连续报告 `presented=13/13`、稳态约 84--89 ms、render 约 50--64 ms；少量 deadline miss 来自调度/TLS 并发，未发生传输失败。结论：已验证真实双缓冲差分路径的约 8--12 fps 宠物动画，不能宣称 20 fps/50 ms；后续仅可优先在 Fangtang visual/display profile 缩小变化区域或采用经 HIL 验证的资源包，禁止业务层板型分支，也禁止未经电气 HIL 提升 SPI 时钟或修改 32-row DMA stripe。

> 2026-08-14 Bread Compact 共享 Display Service 动画入口回归（COM4）：为确认上述 profile-private animation worker context 合约没有破坏业务行为基线，已重新构建并写入 Bread Compact 正式镜像，写入 hash 校验通过。冷启动日志 `iot-agentos/serial-com4-bread-displayservice-entryfix-20260814.log` 确认 `LCD double buffering ready: 307200 bytes PSRAM + 7680 bytes DMA`、缓存宠物先以 `frames=1/1` 恢复、`BOOT_STATUS.ready=true` 与 `SERVICE_STATUS.ready=true`；网络资源更新后出现 `pet animation worker started: motion=1 frames=8 frame_ms=450`。采样窗口除预期的 boot reset reason 外未见 startup failed、panic、Guru、assert、stack overflow、WDT、LCD transfer failure 或 pet-animation waiting。该证据证明共享 Display Service 向 Bread profile adapter 透传 worker entry context 后，矩形屏的双缓冲、正常启动及多帧 worker admission 仍成立；未进行人工实体键、DISPLAY_OFF/wake、录音播放或长时动态目视验收，不能将本条外推为这些交互级 HIL 已完成。

> 2026-08-14 DISPLAY_OFF 统一状态路径复核（COM3--COM6）：复核 Power → Display Service → Platform Display → profile-private adapter 与 Input → App Intent 的共同链路，确认四 profile 均以 `DISPLAY_OFF` 表示“仅关闭面板/背光、MCU/网络/闹钟/唤醒词继续运行”，没有把它误称为 light/deep sleep。Bread/Fangtang 的 ST7789/NV3023 adapter 分别在等待既有 DMA completion 后关 PWM 背光、best-effort DISP OFF，并在 wake 时先 DISP ON 再恢复缓存亮度；EchoEar 的 ST77916 同样先保证 PWM 黑屏；Waveshare AMOLED 则保留自身 DCS brightness/DISP OFF 次序。共同业务边界 `on_app_intent()` 在首个 primary contact 时调用 `app_ui_wake_from_idle()`，并按 input source 消费完整首手势，故触摸和实体键都只亮屏、不会落入命令录音；录音中 contact-down 仍先请求 stop 并消费对应完成手势，闹钟优先确认保持不变。COM6 55 秒被动串口采样 `iot-agentos/serial-com6-displayoff-audit-20260814.log` 记录 `sleeping=yes` 时 display heartbeat/ambient 继续推进但 rendered frame 保持冻结，证明实际 DISPLAY_OFF 不会终止系统业务 worker；同一窗口无 startup failed/panic/Guru/assert/transfer failure。COM4/COM5 同期观测也已处于 `sleeping=1`，与矩形 profile 的统一状态一致。随后 `tools/build-profile.cmd waveshare-amoled-1.75c build` 完整链接成功，image 为 `0x484430`，最小 app 分区余 `0x7bbd0`（10%），并由 build wrapper 再次通过 HAL boundary check。COM3 串口当前不输出，且本轮未主动改变 timeout、未人工点击/按键或逐台观察亮灭，故此条仅收口代码路径与现有被动 HIL；不得把它表述为四硬件的人工 wake 或指定时间休眠验收已经完成。

> 2026-08-14 Display Service 动画任务跨核发布收口（共享矩形 / 圆屏路径）：审计发现 `compact_display_service` 与 `round_display_service` 曾以 `volatile + taskYIELD()` 等待创建方完成 task handle/callback 发布；该模式不是明确的跨核发布屏障，可能令新建 worker 在 generation 完整发布前读取 slot。现改为每一代 service-private binary `started` gate：创建方在 lifecycle lock 下完成 callback/context、completion semaphore 与稳定 task handle 的发布后才 give；worker 必须 take gate 后才进入共享 renderer callback。stop 仍通过 task notification 唤醒，stop request 保留为 worker notification-wait 循环可见的 `volatile` 标志；completed 状态继续只在 lifecycle lock 下读写。该收口同时覆盖 Bread/Fangtang 的 shared compact Display Service 与 EchoEar/Waveshare 的 shared round Display Service，profile adapter 仍只决定 task stack/priority，Device/Platform/HAL public API、业务层与板型分支均未改变。`bread-compact`、`echoear-2st`、`fangtang-4g`、`waveshare-amoled-1.75c` 已各自完整链接，并逐次通过 `tools/check-hal-boundaries.ps1`；app 分别为 `0x33d8c0`（余 `0x62740` / 11%）、`0x3299e0`（余 `0x76620` / 13%）、`0x33a260`（余 `0x65da0` / 11%）、`0x484470`（余 `0x7bb90` / 10%）。随后已在四机完成正式镜像 HIL；更强的并发 stop/start 冲突尚未注入，因此不把启动证据扩大为完整 race 验收。

> 2026-08-14 Display Service 动画发布 gate 正式镜像 HIL（COM3--COM6）：上述 shared Display Service gate 变更已分别写入 EchoEar-2ST/COM3、Bread Compact/COM4、Fangtang-4G/COM5、Waveshare/COM6；四次 `app-flash` 均由 esptool 在 app 分区写后完成 hash 校验。四端并行 reset 后采样约 55 秒，均未出现 `startup failed`、panic、Guru、assert、stack overflow、WDT、abort 或 display transfer failure。随后发现 worker completion 中残存仅为防御目的的 `taskYIELD()` spin；由于 `started` gate 本身只会在创建方已取得 lifecycle mutex 后打开，已移除该残存 spin，worker 直接等待已发布 mutex。四 profile 再次完整构建，Bread 随后再次写入/采样，COM3/COM5/COM6 也再次写入并 hash 校验；最终 digest 分别为 COM3 `c629ac3ac`、COM4 `91a06a407`、COM5 `dcee4db7b`、COM6 `7ea692187`。COM4/COM5 在最终 digest 下分别报告 `LCD double buffering ready: 307200 bytes PSRAM + 7680 bytes DMA` 与 `230400 bytes PSRAM + 15360 bytes DMA`、`pet animation worker started: motion=1 frames=8`，并进入 `BOOT_STATUS.ready=true`、`SERVICE_STATUS.ready=true`；COM5 连续报告 80 ms target 的 `presented=12--13/ticks=12--13`，稳态平均约 84--92 ms，证明 worker 没有因发布 gate 停在入口。COM3 报告 360×360 双 buffer、`BOOT_STATUS.ready=true`，但 Hub token 被拒绝而进入 pairing recovery，故没有本次 `SERVICE_STATUS` 或动画 worker 闭环；COM6 报告 466×466 双 buffer、`BOOT_STATUS.ready=true`、`SERVICE_STATUS.ready=true`，display heartbeat 持续推进，无异常。原始日志为 `iot-agentos/serial-com3-animation-gate-nospin-20260814.log`、`serial-com4-animation-gate-nospin-20260814.log`、`serial-com5-animation-gate-nospin-20260814.log`、`serial-com6-animation-gate-nospin-20260814.log`。这验证正式固件启动和现有 animation worker admission；没有构造并发 stop/start 冲突、也未以无效 Hub 凭据替代 EchoEar 在线/宠物动画验收，因此这些更强的 race/交互结论仍未通过。

> 2026-08-14 紧凑 Display animation stop deadline 收口（Bread / Fangtang shared service）：按 3.26 复核发现 `compact_display_service_stop_animation()` 虽在等待 worker completion 前计算父事务剩余 tick，却在 completion 后重新以原始 `timeout_ms` 取得 lifecycle mutex；若 cleanup 与另一个 lifecycle caller 竞争，单次 stop 可超过调用方 deadline。现将 cleanup mutex 仅允许消费 `after_join_remaining`，预算耗尽或取锁失败时保留已关闭 generation、task identity 与同步对象，供后续 stop 在锁下安全清理，绝不删除并发 owner 仍可能观察的资源。此变更不改变 Device/Platform/HAL API 或 board profile；round service 原先已采用相同剩余预算规则。Bread 与 Fangtang 完整链接均通过，HAL boundary check 通过；Fangtang app `0x33a210`（最小 16 MiB app 分区余 `0x65df0` / 11%）已写入 COM5，写后 hash 校验成功。52 秒正式采样 `iot-agentos/serial-com5-animation-deadline-budget-20260814.log` 记录 `BOOT_STATUS.ready=true`、`SERVICE_STATUS.ready=true`、8 帧 pet worker 启动及连续 80 ms animation tick（12--14/12--14 presented/ticks），无 startup failed/panic/Guru/assert/stack overflow/WDT/transfer failure。该 HIL 证明正常 worker/显示回归；尚未构造 lifecycle mutex contention + stop deadline 到期的专门故障注入，因此 deadline 上界的压力场景仍待专用 test artifact 覆盖。

> 2026-08-14 紧凑 Display animation stop deadline 压力 HIL（COM5 Fangtang）：补齐上述缺口，新增独立、非发布的 `fangtang-4g-animation-deadline-fi` wrapper；它仅在 selected Fangtang profile/defaults 上启用 `TEST_BUILD=y` 与 `COMPACT_DISPLAY_ANIMATION_DEADLINE_TEST=y`，没有 Hub/HTTP/console/input runtime 开关，也不改变 Device/Platform/HAL 或任何发布 profile。私有 Display Service test worker 在 lifecycle mutex 下先延迟 70 ms 才发布 completion、再持锁延迟 70 ms；同一调用方的 100 ms stop 因此会在 completion 后遇到 mutex contention。测试断言该 stopper 只能使用父事务剩余预算：COM5 日志 `iot-agentos/serial-com5-animation-deadline-fi-final-20260814.log` 记录 `starting animation stop-deadline proof`、两段 70 ms delay，最终 `PASS animation stop deadline elapsed=11 ticks budget=10`；若恢复为旧的完整 timeout cleanup，stop 会在第二段锁竞争中错误等待一轮新 100 ms 而测试失败。随后测试通过正常 idempotent later-stop 清理保留 generation，无 panic/Guru/assert/WDT/reset loop。test app `0x374190`（16 MiB 最小 app 分区余 `0x2be70` / 5%，仅 test artifact）已写入且 hash 校验；完成后 COM5 已写回正式 Fangtang image，hash 校验及 `serial-com5-animation-deadline-release-final-20260814.log` 的 `BOOT_STATUS.ready=true`、`SERVICE_STATUS.ready=true`、8 帧 pet worker/连续 80 ms tick 均正常，且不存在 `compact_display: test:`/degraded/panic/WDT。四 profile 的 release 构建仍由 HAL boundary check 保护；本条证明 shared compact service 的定向 mutex-contention deadline contract，不外推为任意 renderer/panel hang、完整 board deinit 或 runtime restart。

> 2026-08-14 圆屏 Display animation stop deadline 压力 HIL（Waveshare 1.75C / COM6）：圆屏 `round_display_service_stop_animation()` 已使用父事务剩余预算，但此前没有故障注入证明该保证未在后续重构中退化。现新增与紧凑路径同构、仅测试的 `round_display` seam 和 `waveshare-amoled-1.75c-animation-deadline-fi` / `echoear-2st-animation-deadline-fi` wrappers：只在 selected profile 的 defaults 中启用 `TEST_BUILD=y`、`ROUND_DISPLAY_ANIMATION_DEADLINE_TEST=y`，不增加 Hub/HTTP/console/input runtime 控制面，也不泄漏 panel、FreeRTOS 或 board identity 到 Device/Platform API。测试 worker 在 service-private lifecycle mutex 下 completion 前延迟 70 ms、give completion 后继续持锁 70 ms；100 ms stopper 必须在原始 deadline 附近返回 timeout，随后由正常 idempotent later-stop 回收 retained generation。COM6 已写入 Waveshare test image 且 serial 记录 `starting animation stop-deadline proof`、两段 delay 和 `PASS animation stop deadline elapsed=13 ticks budget=10`；同一窗口完成 `BOOT_STATUS.ready=true`，未见 panic/Guru/assert/WDT/transfer failure。测试后 COM6 已刷回正式 Waveshare image，写入后 runtime digest 为 `fa20a097d`；`iot-agentos/serial-com6-animation-deadline-release-final-20260814.log` 记录双 framebuffer、`BOOT_STATUS.ready=true` 与 `SERVICE_STATUS.ready=true`，且没有 `round_display: test:`、degraded 或 reset loop。Waveshare test wrapper 已构建；EchoEar 新 wrapper 的初次全新 build 被 Xtensa GCC 15.2 在 ESP-IDF 自身 `esp_lcd_panel_rgb.c` 触发的 internal compiler error 中断，非本项目 source 编译诊断；随后 EchoEar release profile 已完整构建通过，故本条不把 COM6 HIL 外推为 COM3 实机验收。该证据只覆盖圆屏共享 Display Service 的 mutex-contention deadline/late cleanup 合约，不代表 panel hang、DMA teardown、运行时 restart 或 EchoEar online 动画闭环。
> 2026-08-12 圆屏 Audio HAL profile contract 收口（EchoEar-2ST / COM3，Waveshare 1.75C / COM6）：共享 `board_port.c` 和 `round_audio_codec_adapter.h` 先前依赖一组 `ROUND_PROFILE_AUDIO_*` 宏，虽实际 pins/controller 位于私有 adapter，却会把 profile 选择事实散落在共享圆屏 renderer。现新增私有、纯标量 `round_audio_profile_t`：profile 选择层提供 name、codec address/register、I2C/I2S pin、PA、sample rate、MCLK multiple 与 microphone slot；codec adapter 消费该 contract 后才构造 ESP-IDF I2C/I2S/GPIO private objects，shared renderer 只读取 sample rate/default volume/name，不再访问 audio pin/address/slot 宏。Device/Platform headers 未增加 SDK、RTOS 或 driver object。EchoEar 和 Waveshare 正式 profile 均完整构建成功；两镜像分别写入 COM3/COM6 并 hash 校验。COM3 36 秒验收记录 `EchoEar-2ST audio ready: 16kHz STD stereo 16-bit`、round display/input ready、`BOOT_STATUS.ready=true`（此窗口未等到 SERVICE_STATUS，不能扩大为完整 online 验收）；COM6 38 秒验收记录 `Waveshare 1.75C audio ready: 16kHz STD stereo 16-bit`、round display/input ready、`BOOT_STATUS.ready=true`、`SERVICE_STATUS.ready=true`，无 startup failed/panic/Guru/abort。两者均为正式 profile 镜像，无需回刷。该切片仅将物理 audio configuration 归并到 profile contract；圆屏 scene/input/wake session 尚未拆为独立 Display/Input/Audio source，Phase 5/6 仍未完成。
> 2026-08-12 Waveshare 1.75C / COM6 构建闭环：修复 `profile_components/waveshare_deps` 的 source-free dependency carrier。此前该 carrier 为空，虽 `main` 的 Waveshare 分支声明了 CO5300/CST9217 依赖，但预解析 component graph 未向 `main` 传递 `esp_lcd_touch.h` 公共头可见性，导致 `waveshare_peripheral_adapter.h` 编译失败。carrier 现在仅为被 wrapper 选中的 Waveshare profile 显式导出 `espressif__esp_lcd_co5300`、`espressif__esp_lcd_touch` 与 `waveshare__esp_lcd_touch_cst9217`；共享 HAL/Device API 未引入 SDK/private type，也没有让其他 profile 带入依赖。`tools/build-profile.cmd waveshare-amoled-1.75c build` 已完成，正式镜像已写入 COM6 并 hash 校验。40 秒串口验收：466x466 board profile，`LCD double buffer ready: 2 x 434312 bytes in DMA PSRAM, 150 ms cadence`、touch interaction monitor ready、fall detection service enabled，以及 `BOOT_STATUS.ready=true` 与 `SERVICE_STATUS.ready=true`；未见 startup failed/panic/Guru/transfer failure。该镜像为正式 Waveshare profile，无需测试镜像回刷。
> 2026-08-12 Fangtang-4G 显示吞吐候选验证：COM5 已确认正式镜像存在双软件帧缓冲（启动日志：`LCD double buffering ready: 230400 bytes PSRAM + 480 bytes DMA`）。慢动态效果的主瓶颈不是缺少后帧，而是 NV3023 正式适配器为电气稳定性按 1 行同步 DMA 提交。新增仅测试的 `fangtang-4g-display-chunk8-fi` profile：该 profile 通过 Fangtang 私有显示适配器将相邻 8 行作为一笔 DMA 事务，保留相同的 completion fence、超时所有权和共享 Display/HAL API；生产 profile 不变，仍强制 1 行。该测试镜像已在 COM5 写入且校验成功，38 秒启动验收得到 `BOOT_STATUS.ready=true`、`SERVICE_STATUS.ready=true`，未出现 panic/Guru、传输失败或重启；日志可见测试镜像的 DMA staging 为 3840 B。随即已刷回正式 `build-unified-fangtang` 镜像，34 秒复验再次得到 `BOOT_STATUS.ready=true` 与 `SERVICE_STATUS.ready=true`，并确认正式 staging 恢复 480 B。候选 8 行尚未获得“宠物长时动画 + 实机目视 + 熄屏/唤醒 + Wi-Fi TLS/4G 并发”的完整资格，不得提升为 release 默认；下一步按 8→16 的受控 profile 实测，只有全部通过才调整生产物理传输策略。
> 2026-08-12 Task Registry ???? HIL?Fangtang-4G / COM5?Phase 4 ????????? deadline ??????????????? `fangtang-4g-task-registry-fi`???? `CONFIG_MACLAW_TEST_BUILD` ????? profile driver ?????????? Registry ??????? FreeRTOS test worker ?? Registry bookkeeping mutex 120 ms?? `stop_all(40 ms)` ??? parent deadline ??? `ESP_ERR_TIMEOUT`????? AUDIO/POWER/BOARD ???? owner ? entry??????????? timeout?error?success ???????????? 100 ms transaction ??????????? stop ???? entry??? success entry?????? 2???? stop ?????????? entry?COM5 ????? `task_registry_test: PASS mutex contention + multi-owner mixed stop; elapsed_ticks=2/10 budgets=100/80/80`?????? `BOOT_STATUS.ready=true` ? `SERVICE_STATUS.ready=true`??? app?`0x371530`???/??????????? `build-unified-fangtang\maclaw_esp32s3_client.bin`????????? BOOT/SERVICE ready??????? Registry ?? mutex contention?multi-owner reverse traversal?success/error/timeout ??? retained-entry cleanup ? bounded contract????? Registry ??????? callback deadline ???????????? rollback?Wi-Fi/HTTP/portal?panel DMA ? runtime restart ??????????????? test worker/test storage?failure seam ?? Hub?HTTP?console?????? API ????
> 2026-08-12 Bread Compact profile-private display stage-5 HIL（COM4，Phase 4 增量）：新增独立非发布 `bread-compact-display-stage5-fi` wrapper，仅在 Bread 物理 profile/defaults 之上启用 `TEST_BUILD=y`、`DISPLAY_FAILURE_INITIALIZATION=y`、`DISPLAY_FAILURE_STAGE=5`。故障点严格位于 ST7789/SPI panel reset/init、inversion 和 display-on 已成功之后；panel、panel IO、SPI、LEDC backlight/timer 的逆序 partial rollback 始终留在 Bread profile-private adapter，shared compact renderer、Device/Platform API、业务状态和 Hub 均无新增入口。完整测试镜像（app `0x33ad40`）写入 COM4 后，串口精确命中 `display init fault injection after stage 5` 和 `compact display adapter init failed: ESP_FAIL`，随后 Display Task、command-cancel 与 volume worker 有序停止，进入 `startup degraded` 并从 `app_main()` 返回；38 秒窗口无 panic/assert/WDT/reset loop。测试完成后 COM4 已立即恢复 `build-unified-bread` 正式镜像并独立 `verify-flash`（digest matched）；冷启动确认 `LCD double buffering ready: 307200 bytes PSRAM + 7680 bytes DMA`、input/audio/power/deadline/schedule、`BOOT_STATUS.ready=true`、`SERVICE_STATUS.ready=true`，以及后续宠物下载与 offline wake 生命周期正常。此证据仅覆盖 ST7789/SPI/LEDC acquisition 完成后的 profile-private partial rollback，不代表 runtime restart、in-flight DMA teardown、physical scan-out、完整 board/audio teardown 或电源轨休眠。
> 2026-08-12 Waveshare 1.75C profile-private display stage-5 HIL（COM6，Phase 4 增量）：新增独立非发布 `waveshare-amoled-1.75c-display-stage5-fi` wrapper，仅在 Waveshare 物理 profile/defaults 之上启用 `TEST_BUILD=y`、`DISPLAY_FAILURE_INITIALIZATION=y`、`DISPLAY_FAILURE_STAGE=5`。故障点严格位于 CO5300/QSPI panel reset/init、display-on 与初始亮度已经成功之后；panel、panel IO、SPI 与 completion semaphore 的逆序 partial rollback 始终留在 profile-private adapter，shared round renderer、Device/Platform API、业务状态和 Hub 均无新增入口。完整测试镜像（app `0x482000`）写入 COM6 后，串口依序确认 CO5300 创建、初始化及 `set brightness to 100%`，随后精确命中 `display init fault injection after stage 5` 和 `round display adapter init failed: ESP_FAIL`，Display Task、command-cancel 与 volume worker 有序停止并进入 `startup degraded`、`Returned from app_main()`；32 秒窗口无 panic/assert/WDT/reset loop。测试完成后 COM6 已立即恢复 `build-unified-waveshare` 正式镜像并独立 `verify-flash`（digest matched）；冷启动取得双 framebuffer（`2 x 434312 bytes in DMA PSRAM`）、CST9217 touch、CO5300 brightness、audio、Motion HAL、fall detection、`BOOT_STATUS.ready=true`、`SERVICE_STATUS.ready=true` 和持续 display heartbeat。此证据仅覆盖 CO5300/QSPI acquisition 完成后的 profile-private partial rollback，不代表 runtime restart、in-flight DMA teardown、physical scan-out、触控/音频完整 teardown 或跌倒检测触发语义。
> 2026-08-12 EchoEar-2ST profile-private display stage-5 HIL（COM3，Phase 4 增量）：新增独立非发布 `echoear-2st-display-stage5-fi` wrapper，仅在 EchoEar 物理 profile/defaults 之上启用 `TEST_BUILD=y`、`DISPLAY_FAILURE_INITIALIZATION=y`、`DISPLAY_FAILURE_STAGE=5`。故障点严格位于 ST77916/QSPI panel reset/init、display-on 和 PWM 亮度均成功之后；panel、panel IO、LEDC/PWM、SPI 与 completion semaphore 的逆序 partial rollback 始终保留在 EchoEar profile-private adapter，shared renderer、Device/Platform API、业务状态和 Hub 均无新增入口。完整测试镜像（app `0x327490`，最小 app 分区余 `0x78b70` / 13%）写入 COM3 后，串口精确命中 `display init fault injection after stage 5` 和 `round display adapter init failed: ESP_FAIL`，随后 Display Task、command-cancel 与 volume worker 有序停止，进入 `startup degraded` 并从 `app_main()` 返回；采样窗口无 panic/assert/WDT/reset loop。测试完成后 COM3 已立即恢复 `build-unified-echoear` 正式镜像且执行独立 `verify-flash`（digest matched）；冷启动证实 `LCD double buffer ready`、`ambient overlay ready: 92160 bytes in DMA PSRAM`、EchoEar audio/input/power 初始化完成以及 `BOOT_STATUS.ready=true`。此证据仅覆盖 ST77916/QSPI/PWM acquisition 完成后的 profile-private partial rollback，不等同 runtime restart、in-flight DMA teardown、physical scan-out、完整 board/audio teardown 或 Hub 鉴权成功。
> 2026-08-12 Fangtang-4G profile-private display stage-5 HIL（COM5，Phase 4 增量）：新增独立非发布 `fangtang-4g-display-stage5-fi` wrapper，仅在 Fangtang 物理 profile/defaults 之上启用 `TEST_BUILD=y`、`DISPLAY_FAILURE_INITIALIZATION=y`、`DISPLAY_FAILURE_STAGE=5`。故障点位于 NV3023 panel reset/init、方向/反色、display-on 与 PWM 背光均成功之后，仍完全留在 profile-private adapter；shared renderer、Device/Platform API、业务状态和 Hub 均无新增入口。完整镜像（app `0x370c90`，最小 app 分区余 `0x2f370` / 5%）写入 COM5，写后 hash 与独立 `verify-flash` 均通过。HIL 日志依序确认 NV3023 panel 创建与实际初始化，再精确命中 `display init fault injection after stage 5`、`compact display adapter init failed: ESP_FAIL`，随后 Display Task、cancel 与 volume worker 有序停止并进入 `startup degraded`、`Returned from app_main()`；32 秒窗口无 panic/assert/WDT/reset loop。这证明 Fangtang adapter 的逆序 PWM/panel/IO/SPI partial rollback 在最后 acquisition boundary 后可稳定返回，不代表 runtime panel restart、in-flight DMA teardown、physical scan-out 或完整 board/ML307 deinit。测试后 COM5 已恢复 `build-unified-fangtang` 正式镜像并 `verify-flash`，冷启动达到 `BOOT_STATUS.ready=true`、`SERVICE_STATUS.ready=true`。

> 2026-08-12 Fangtang-4G compact-renderer stage-6 HIL（COM5，Phase 4 增量）：为共享 compact renderer 增加独立非发布 `fangtang-4g-renderer-stage6-fi` profile（仅 `TEST_BUILD=y` / `COMPACT_RENDERER_FAILURE_INITIALIZATION=y` / `STAGE=6`），故障点严格位于输入 scanner task 已成功发布之后。首次 HIL 暴露真实生命周期缺口：`input_service_start()` 的 platform adapter 返回失败分支会直接释放 Input Service 队列，而已发布 scanner 尚未被停止；现已改为先通过既有 Platform Input → `board_port_stop_input()` 合约 cooperative stop/join。若 stop/join 超时则保留该闭合 generation 的 task/queue 状态并返回失败，绝不释放 scanner 仍可能访问的对象；成功后才回收队列，且 adapter 仍维持 boot-lifetime、不可同 boot 重初始化。修复后完整 image（app `0x370c90`，最小 app 分区余 `0x2f370` / 5%）写入 COM5，写后 hash 和独立 `verify-flash` 均通过；实机日志按序命中 `test after input scanner task` → `board input scanner stopped` → Fangtang battery monitor/background → Display Task → cancel/volume worker 有序停止 → `startup degraded` → `Returned from app_main()`，35 秒窗口无 panic/assert/WDT/reset loop。随后已恢复 `build-unified-fangtang` 正式镜像并 `verify-flash`，冷启动同时取得 `BOOT_STATUS.ready=true` 和 `SERVICE_STATUS.ready=true`。此证据仅覆盖已发布 scanner 的启动失败 cooperative stop/join，不等同 runtime restart、stop timeout、完整 board/ML307 teardown 或 physical scan-out。

> 2026-08-12 Fangtang-4G compact-renderer stage-5 HIL（COM5，Phase 4 增量）：新增独立非发布 `fangtang-4g-renderer-stage5-fi` wrapper，固定 `TEST_BUILD=y` / `COMPACT_RENDERER_FAILURE_INITIALIZATION=y` / `STAGE=5`，只保留 Fangtang 的实际 profile/driver closure。首次构建在模型打包处受本机 stale ninja 进程阻塞；重新以同一 wrapper 构建后完整链接成功（app `0x370c20`，最小分区余 `0x2f3e0` / 5%），未将该工具问题误记为固件失败。COM5 写入与独立 `verify-flash` digest 均通过。HIL 日志确认 NV3023、input、direct-I2S audio、battery monitor、GPIO0 selector 均已完成，随后精确命中 `test before input scanner task`；接着 battery monitor/background、Display Task、command-cancel 与 volume worker 有序停止，进入 `startup degraded` 并从 `app_main()` 返回，期间没有 `board input scanner stopped`，证明 scanner task 从未发布，也没有 panic/assert/WDT/reset loop。测试后 COM5 已恢复 `build-unified-fangtang` 正式 image 并再次 `verify-flash`；冷启动为 `BOOT_STATUS.ready=true`、`SERVICE_STATUS.ready=true`。至此 Fangtang shared compact renderer 的 stage 1–5 受控启动边界均获得 HIL 覆盖；该结论不扩展为 runtime restart、scanner 已运行时 timeout、ML307 teardown 或 physical scan-out。\r\n\r\n> 2026-08-12 Fangtang-4G compact-renderer stage-4 HIL 与未发布同步资源修复（COM5，Phase 4 增量）：审计发现 stage 4 在 scanner completion semaphore 已创建、scanner task 尚未发布时，原 `compact_renderer_fail_after_hardware_init()` 会保留这个无 owner 的 semaphore；它既不是 boot-lifetime diagnostic hardware，也不可能被未创建的 task 归还。现仅在 `!s_button_task && s_button_task_stopped` 时于 fail-closed helper 回收该 semaphore；一旦 scanner 已发布，既有 `board_port_stop_input()` 仍独占其 completion ownership，故不会在 task 仍可访问时提前删除。新增非发布 `fangtang-4g-renderer-stage4-fi` profile（`TEST_BUILD=y` / renderer stage 4），全量 image（app `0x370c20`）写入 COM5 且 write hash/`verify-flash` 通过。HIL 在 NV3023、input、audio、battery monitor 和 Fangtang GPIO0 startup selector 均已运行后精确命中 `test input completion semaphore`；随后 battery monitor、background、Display Task、command-cancel/volume worker 有序停止，进入 `startup degraded` 并从 `app_main()` 返回，日志未出现 `board input scanner stopped`，符合 scanner 从未发布；窗口无 panic/assert/WDT/reset loop。测试后 COM5 已恢复正式 `build-unified-fangtang` image 并再次 `verify-flash`，冷启动有 `BOOT_STATUS.ready=true`、`SERVICE_STATUS.ready=true`。此项验证未发布 semaphore 的所有权，不代表 scanner 已运行时的 stop timeout、runtime restart 或 physical scan-out。\r\n\r\n> 2026-08-12 Fangtang-4G compact-renderer stage-3 HIL（COM5，Phase 4 增量）：新增独立非发布 `fangtang-4g-renderer-stage3-fi` wrapper，只在 Fangtang 物理 profile/private ML307-NV3023-UHCI 依赖闭包之上追加 `TEST_BUILD=y`、`COMPACT_RENDERER_FAILURE_INITIALIZATION=y`、`STAGE=3`。全量链接 image（app `0x370c00`，最小分区余 `0x2f400` / 5%）写入 COM5 后，写后 hash 与独立 `verify-flash` digest 均通过。HIL 精确确认 NV3023、双缓冲、input、direct-I2S audio 后 Fangtang 私有 battery ADC/charge monitor 已实际采样，继而命中 `test profile peripheral adapter`；rollback 先输出 `battery monitor stopped`，再停止 board background、Display Task、command-cancel 与 volume-persistence worker，最终进入 `startup degraded: phase=4 … reason=input service` 并从 `app_main()` 返回，无 panic/assert/WDT/reset loop。该证据证明 profile-private peripheral task/ADC ownership 被取得后，shared renderer 的 rollback 能按 profile 回收 background monitor，不让其越过 degraded generation。测试后 COM5 已恢复 `build-unified-fangtang` 正式 image，重新 `verify-flash` 并冷启动到 `BOOT_STATUS.ready=true`、`SERVICE_STATUS.ready=true`，无 test/degraded/panic/WDT。此项不覆盖 runtime renderer restart、实际 ADC/任务 stop timeout、ML307 teardown、Display STOP timeout 或 physical scan-out。\r\n\r\n> 2026-08-12 Fangtang-4G compact-renderer stage-2 HIL（COM5，Phase 4 增量）：为覆盖共享紧凑 renderer 在第二种矩形硬件 profile 的 audio acquisition 边界，新增独立非发布 `fangtang-4g-renderer-stage2-fi` wrapper；它保留 Fangtang 的 NV3023/ML307/UHCI 私有依赖闭包与物理 board identity，仅追加 `TEST_BUILD=y`、`COMPACT_RENDERER_FAILURE_INITIALIZATION=y`、`STAGE=2`。全量链接 image（app `0x370c00`）写入 COM5 后，esptool 写后 hash 与独立 `verify-flash` digest 均通过。HIL 串口依序确认 NV3023 panel、双缓冲、Fangtang input adapter、direct-I2S audio 已就绪，随后精确命中 `test audio adapter`；然后 board background、Display Task、command-cancel 与 volume-persistence worker 有序停止，进入 `startup degraded: phase=4 … reason=input service` 并从 `app_main()` 返回，窗口内没有 panic/assert/WDT/reset loop。此结果证明 input 与 audio 已获取后，shared renderer 的 boot-lifetime diagnostic hardware retention 与依赖顺序 rollback 在 Fangtang 私有硬件闭包内成立，不会继续进入 profile peripheral 或 scanner admission。测试后 COM5 已立即恢复 `build-unified-fangtang` 正式 image，再次 `verify-flash` 后冷启动取得 `BOOT_STATUS.ready=true`、`SERVICE_STATUS.ready=true`，无 test/degraded/panic/WDT。它不等价于运行期 renderer restart、真实 codec/外设故障、Display STOP timeout 或面板 physical scan-out HIL。\r\n\r\n> 2026-08-12 Bread Compact compact-renderer stage-3 HIL 收口（COM4，Phase 4 增量）：此前 stage 3 因本机构建/串口采集受阻仅有写入证据，现使用独立非发布 `bread-compact-renderer-stage3-fi` image 完成可引用运行验收。该 wrapper 只在 Bread 物理 profile/defaults 之上追加 `TEST_BUILD=y`、`COMPACT_RENDERER_FAILURE_INITIALIZATION=y`、`STAGE=3`，不改变业务状态、公共 HAL 或 release image。COM4 完整 app 写入和独立 `verify-flash` digest 通过；串口按顺序确认 LCD 双缓冲、Bread input adapter、direct-I2S audio 已就绪，随后精确命中 `test profile peripheral adapter`，继而 `board background tasks stopped`、`display task stopped`、command-cancel/volume worker stopped，最终 `startup degraded: phase=4 … reason=input service` 并从 `app_main()` 返回。采样窗口无 panic/assert/WDT/reset loop，证明 shared compact renderer 已获取 input/audio 与 profile-peripheral 后的 failure 路径仍保留 boot-lifetime 硬件诊断并按依赖关系回滚，而不会制造 scanner task。测试后 COM4 已立即恢复 `build-unified-bread` 正式 image，并再次 `verify-flash`；冷启动取得 `BOOT_STATUS.ready=true`、`SERVICE_STATUS.ready=true`，无 test/degraded/panic/WDT。此项不等价于已验证运行期 renderer restart、实际外围硬件故障、Display STOP timeout 或 panel physical scan-out。\r\n\r\n> 2026-08-12 Waveshare 1.75C 真实 CO5300/QSPI transfer-fence timeout HIL（COM6，Phase 4 增量）：新增独立非发布 `waveshare-amoled-1.75c-fence-fi` wrapper，保持 Waveshare 私有 component closure、实际 board identity 与 466×466 圆屏 layout，仅追加 `TEST_BUILD=y` / `DISPLAY_TRANSFER_FENCE_TIMEOUT_ONCE=y`。故障注入 seam 已位于 CO5300 profile-private adapter：首笔真实 color transaction 在 panel 提交后故意放弃一次 controller/QSPI transfer-completion callback wait；共享 renderer、Device/Platform HAL、Hub/HTTP/输入都没有 runtime 开关或 controller 类型泄漏。完整 image（app `0x481ff0`）已写入 COM6，esptool 写后 hash 与独立 `verify-flash` digest 均通过；启动日志命中 `waveshare_display: test: abandoning first transfer fence wait` 后，仍进入 `BOOT_STATUS.ready=true`、Wi-Fi/Gateway、offline wake 与 `SERVICE_STATUS.ready=true`，约 35 秒窗口无 startup degraded、panic/assert/WDT/reset loop，且显示 heartbeat 持续递增。此结果证明一次真实 CO5300/QSPI **传输完成**等待被放弃时，adapter 持有旧 source/fence ownership、迟到 callback 不会被下一笔提交错误消费；它不是面板物理 scan-out/GRAM fence，也不代表 renderer 无限阻塞、panel/audio teardown 或 runtime restart 已验收。测试后 COM6 已立即写回 `build-unified-waveshare` 正式 image，并再次 `verify-flash`；冷启动复查取得 `BOOT_STATUS.ready=true`、`SERVICE_STATUS.ready=true`，未命中测试日志或 degraded/panic/WDT。\r\n\r\n> 2026-08-12 EchoEar-2ST 真实 QSPI transfer-fence timeout HIL（COM3，Phase 4 增量）：四个 profile 早已各自持有 completion-fence 实现，但 EchoEar 的 ST77916/QSPI path 尚无独立可复现实机验收。现新增非发布 `echoear-2st-fence-fi` wrapper：保留 EchoEar 私有 component closure、物理 board identity 与圆屏 layout，仅追加 `TEST_BUILD=y` / `DISPLAY_TRANSFER_FENCE_TIMEOUT_ONCE=y`；test seam 仍完全位于 profile adapter，不向 Device/Platform/HAL、Hub、HTTP 或输入暴露控制接口。全量 image（app `0x327470`，factory 分区余 `0x78b90` / 13%）已写入 COM3，esptool 写后 hash 与独立 `verify-flash` digest 均通过。启动日志依序确认双 framebuffer、修复后的 `92160-byte DMA PSRAM` ambient overlay、EchoEar audio/input/power/deadline/schedule 就绪，然后命中 `echoear_display: test: abandoning first transfer fence wait`，系统仍稳定进入 `BOOT_STATUS.ready=true`；35 秒窗口无 startup degraded、panic/assert/WDT/reset loop。该结果证明一次真实 QSPI color transfer 的等待被故意放弃时，EchoEar profile-private adapter 保留已有 source/fence ownership，不把迟到 callback 误消费为下一笔传输；它不是物理 scan-out/GRAM fence，也不代表 renderer 无限阻塞、panel/audio teardown 或 runtime restart 已验收。测试后 COM3 已立即写回 `build-unified-echoear` 正式 image，并再次 `verify-flash`；冷启动复查仍有 `ambient overlay ready: 92160 bytes in DMA PSRAM` 与 `BOOT_STATUS.ready=true`，无 degraded/panic/WDT。
...
> 2026-08-12 圆屏 Audio/Input source owner 收口（EchoEar-2ST / COM3，Waveshare 1.75C / COM6，构建验收）：为避免共享 `board_port.c` 继续包含 ES7210/ES8311、I2C/I2S、DMA heap、音频任务与触控 controller 物理实现，新增私有 `round_audio_service.c/.h`。该 source 是 selected round profile 的唯一 codec transport 和 shared-I2C touch state owner：保留既有 codec 寄存器、mute/PA settle、I2S clock、PCM slot conversion、wake-task placement与 rollback 顺序；`board_port.c` 只依赖标准 PCM/播放阶段/timeout contract，`round_input_service.c` 只消费标准化 touch report，不再包含任何 driver/touch adapter static state。`round_profile_adapter.h` 现在仅选择纯标量 profile 与屏幕布局，不再把 codec implementation 注入 renderer translation unit。两圆屏均完整构建成功：EchoEar image `0x327780`，最小 app 分区余 `0x78880`（13%）；Waveshare image `0x4820c0`，余 `0x7df40`（10%）。`git diff --check` 与 `tools/check-hal-boundaries.ps1` 均通过。此项证明编译级的唯一所有权和公开边界；尚未刷机，仍需分别在 COM3/COM6 进行 cold boot、touch/input、录音上传、音频播放、睡眠/唤醒的实机回归后才可宣称运行时完整闭环。
> 2026-08-12 圆屏 Audio/Input source owner HIL（COM3 EchoEar-2ST、COM6 Waveshare 1.75C）：上述 Audio/Input source-owner 修改后，两台正式镜像均已重新写入 app 分区并独立 `verify-flash` digest 成功。COM6（32 MB）最终 image `0x482300`，剩余 `0x7dd00`（10%）；冷启动串口确认 Gateway handshake 200、`SERVICE_STATUS.ready=true`、runtime phase 5/first failure 0、input queue started、睡眠调度已初始化、display heartbeat 持续，且无 startup failed/panic/Guru/assert。修复 source-owner 拆分初次 HIL 中反复出现的 `motion HAL unavailable: 8`：根因是 QMI8658/AXP2101 private static handles 与 Audio source owner 不在同一 translation unit；现 power/motion facade 也改经 `round_audio_service` 的同一 owner。复刷 COM6 后 telemetry 为 battery 95%/charging=true，motion warning 消失，fall detection 继续 monitoring。COM3（16 MB）最终 image `0x3277c0`，剩余 `0x78840`（13%），写入及 verify-flash 均通过；通过 reset 后串口确认可进入 pairing recovery portal，未出现 restart loop/panic/assert。COM3 此窗口 Gateway 返回 401（missing/invalid bearer token），属外部认证状态，不是 Audio/HAL 启动失败；因此未把它标为 SERVICE online 完整验收。两设备都保留正式 image。仍需人工完成触控单/双击、命令录音上传、扬声器播放和 sleep/wake 的交互级回归。
> 2026-08-12 Display Service post-publication rollback HIL 修复闭环（Fangtang-4G / COM5，Phase 4）：此前该 test seam 暴露到 `startup_stop_local_workers()` 对“尚无 background worker/gate”的 inactive state 返回 timeout，导致 rollback 未到 Display Service STOP；随后 degraded path 在 `app_ui_init()` 之前碰 UI mutex，出现 assert/reset loop。修复后的 `board_port_stop_background_tasks()` 对未创建 gate 返回 idempotent OK，且 test injection 保持在 UI 初始化前，`startup_enter_degraded()` 只在 `s_startup_ui_initialized` 时尝试诊断画面。独立非发布 `fangtang-4g-display-service-fi` 已完整链接（app `0x370e70`，余 `0x2f190` / 5%），写入 COM5 并 `verify-flash` digest 成功；重启 HIL 精确记录 `forcing startup failure after Display Service publication` → `display task stopped` → cancel/volume worker stopped → `startup degraded ... display service test injection` → `Returned from app_main()`，35 秒窗口无 panic/Guru/assert/reset loop。随后完整构建 release（app `0x3374e0`，余 `0x68b20` / 11%）并恢复 COM5，写入与 verify-flash 均成功；cold boot 到 `SERVICE_STATUS.ready=true`、phase 5/first failure 0、NV3023 双缓冲/Input/direct-I2S/Power/Wake/Sleep Schedule/Gateway ready。此项只验收 Display Service 已发布、无 UI admission 时的启动失败 stop generation；不代表 busy render、STOP timeout、multiple stop、late exit、panel DMA fence 或 runtime restart 已验收。
> 2026-08-12 Display Service STOP timeout/late-exit HIL（Fangtang-4G / COM5，Phase 4）：独立非发布 `fangtang-4g-display-stop-timeout-fi`（`TEST_BUILD=y`、post-publication fail、terminal STOP delay 7000 ms、secondary stopper delay 10 ms/timeout 9000 ms）完整链接，app `0x370e60`、余 `0x2f1a0`（5%）；写入及 `verify-flash` digest 均成功。COM5 实机日志证明 primary rollback 在其 6 s parent deadline 正确到期：terminal STOP 已被 Display Task 唯一持有，约 6 s 后 `Display Service did not stop ... ESP_ERR_TIMEOUT`，rollback 保持下游 owner fail-closed 并进入 degraded/`Returned from app_main()`；约 1 s 后延迟 STOP 才释放，Display Task 退出，second stopper 以同一 completion/generation 得到 result=0。整个 19 s 窗口无 panic/Guru/assert/reset loop，未出现第二个 STOP。测试后已立即以 `build-unified-fangtang` 正式 app `0x3374e0` 恢复 COM5，写入与 verify-flash 成功，reset smoke 达到 `SERVICE_STATUS.ready=true`、phase 5/first failure 0，无 degraded/panic。此证据覆盖 timeout 后静态 STOP record 的留存、late exit 和第二 stopper join；仍不覆盖 busy renderer/DMA in-flight、Panel fence、三方以上 stopper、runtime restart 或跨 profile HIL。
## 2026-08-12：Fangtang-4G 显示传输块化与双缓冲 HIL

- 审计结论：Fangtang 先前已具备两张 240×240 RGB565 PSRAM 帧缓冲；动态效果迟缓的主因不在合成缓冲，而在 NV3023 生产路径将完整画面拆为 240 次“一行 + 设窗 + DMA fence”等待事务。双缓冲保证帧的后台合成/安全交换，不会消除该串行传输开销。
- 修复：仅在 `boards/fangtang_4g/fangtang_display_adapter.h` 的硬件适配器内，将生产传输粒度统一为 8 行/事务（最大 3840 B），`max_transfer_sz`、GRAM 行窗、DMA 源长度及 standby 条带粒度使用同一 profile-private 常量；业务层、Device/Platform API 与共享渲染器不感知 NV3023、SPI、DMA 或行数。
- 保持 fence 语义：每个 DMA 块仍等待本块 color-transfer 完成，超时后的旧源仍由适配器保持 controller-owned；没有通过取消 fence 或复用在途帧缓冲来换取速度。
- 为 COM5 HIL 增加仅适配器私有的每秒 transport telemetry（块数、行数、字节数、DMA active 时间、块均值）；不进入任何业务/HAL 公共接口。
- 实机验证：COM5 已写入并 `verify-flash` 新正式 Fangtang 镜像（app SHA-256 `b8fffef129515b34c855e642ea5b2bc2f213a7355ffd01cff6e5cbf087fc4e0d`）；冷启动到 `SERVICE_STATUS.ready=true`、runtime phase 5，无 panic/Guru/assert/reset。启动全屏刷新观测 `31 blocks / 244 rows / 116768 bytes / 286 ms`；稳态动态 patch 多为 3–8 块/秒，平均约 110–170 µs/块。该数据确认 8 行生产传输被启用且稳定。
- 验证门禁：`git diff --check` 与 `tools/check-hal-boundaries.ps1` 均通过；应用分区余量 `0x68840`（11%）。后续仍需在宠物包已成功安装的情况下补做长时宠物动画、录音波形、DISPLAY_OFF/wake、Wi-Fi/TLS 与 4G 并发 soak；不能把本次冷启动/稳态 HIL 替代为完整显示压力认证。
## 2026-08-12：COM5 Display Service busy-request STOP HIL

- 已构建专用 `fangtang-4g-display-busy-request-stop-fi` 镜像：`CONFIG_MACLAW_TEST_BUILD=y`、`CONFIG_MACLAW_DISPLAY_SERVICE_FAIL_AFTER_INIT=y`、`CONFIG_MACLAW_DISPLAY_SERVICE_REQUEST_DELAY_ONCE_MS=7000`。
- COM5 实测：私有普通 scene request 已入队且被 Display Task dequeue 后进入 7 s delay；启动回滚将 terminal STOP 排在该 in-flight request 之后。主 rollback 的 6 s deadline 正确返回 `ESP_ERR_TIMEOUT`，日志确认 `startup rollback stopped at Display Service; retaining downstream owners fail-closed` 与 degraded 返回 `app_main()`。
- 延迟 request 在约 7 s 后释放，随后同一 Display Task 消费此前排队的 STOP 并正常退出；无第二个 STOP、无 panic/Guru/assert/reset loop。该测试验证了 Service 的 queue ordering、静态 STOP record 的 timeout/late-exit 所有权，以及 downstream fail-closed 策略。
- 测试镜像已立即替换回 COM5 正式 Fangtang 镜像并 `verify-flash`；正式冷启动 `SERVICE_STATUS.ready=true`、phase 5、`local_ready=ok`。HIL 原始串口证据位于 `iot-agentos/build-test-fangtang-4g-display-busy-request-stop-fi/hil-com5-busy-request-stop.log`，正式恢复 smoke 位于 `iot-agentos/build-unified-fangtang/hil-com5-production-after-busy-stop.log`。
## 2026-08-12：COM5 Task Registry lifecycle HIL

- 已重新构建专用非发布 `fangtang-4g-task-registry-fi` 镜像，配置确认 `CONFIG_MACLAW_TEST_BUILD=y` 与 `CONFIG_MACLAW_TASK_REGISTRY_LIFECYCLE_TEST=y`；app SHA-256 为 `353977d299381141f92daa375f65f555e5f36932e5042c7388bc0fdd0d1b0b85`。
- COM5 写入后 `verify-flash` 成功。boot-only 测试在 profile driver 初始化之前完成并输出 `PASS mutex contention + multi-owner mixed stop`：验证 registry mutex contention 的 40 ms bounded deadline、reverse-order multi-owner stop、mixed timeout/error/success 时 residual deadline 单调递减、失败 entry 留存以及随后 retry 的安全清理。测试完成后正常继续全量启动至 `SERVICE_STATUS.ready=true`、phase 5，无 startup degraded、panic/Guru/assert/reset loop。
- 测试后立即恢复 `build-unified-fangtang` 正式镜像并 `verify-flash`；release cold-boot smoke 为 `SERVICE_STATUS.ready=true`、phase 5/first failure 0、`local_ready=ok`，且没有 test marker。HIL 原始串口证据：`iot-agentos/build-test-fangtang-4g-task-registry-fi/hil-com5-task-registry.log`；正式恢复：`iot-agentos/build-unified-fangtang/hil-com5-production-after-task-registry.log`。
- 该项仅验收 Registry 的通用 deadline/entry 所有权机制，不能替代各真实 owner 在网络阻塞、I2S/DMA、文件写入、portal、timer/ISR 或 runtime restart 中的 stop/join HIL。
## 2026-08-13：圆屏 Display source owner 收口（EchoEar-2ST / Waveshare 1.75C，构建验收）

- 将圆屏 renderer 对 profile display adapter 的直接调用收敛到私有 `main/round_display_service.c/.h`。`board_port.c` 继续负责场景组成、双帧切换、dirty-region 决策和显示休眠状态；但不再包含或调用 panel、SPI/QSPI、DMA completion fence、PSRAM 分配、亮度/Display-Off 事务、列对齐或启动图实现。
- 新 service 是所选圆屏 display adapter 的唯一 translation-unit owner，对 renderer 仅暴露普通 RGB565 buffer、同步提交、内存用途、亮度/睡眠动作与 task 创建语义；没有把 SDK panel/IO、FreeRTOS fence、controller、GPIO 或 capability flags 推入 Device/Platform 公共 HAL。
- EchoEar 的既有混合硬件头按 source owner 分隔 display 与 input/peripheral static implementation，防止同一 ST77916 static panel/transfer state 在 renderer 与 service 两个 translation unit 中重复实例化。Waveshare CO5300 adapter 同样仅由 service 编译；补齐其独立编译所需的 ESP-IDF log/check 依赖。
- 验证：`tools/build-profile.cmd echoear-2st build` 成功，镜像 `0x327a10`，16 MB app 分区余 `0x785f0`（13%）；`tools/build-profile.cmd waveshare-amoled-1.75c build` 成功，镜像 `0x482550`，32 MB app 分区余 `0x7dab0`（10%）。`git diff --check`、`tools/check-hal-boundaries.ps1` 均通过。
- 限制：本项是 source-owner 与双 profile 编译验证，未刷写 COM3/COM6；不得视为触控、录音、播放、DISPLAY_OFF/wake、panel fence 或圆屏显示 runtime restart 的 HIL 验收。下步须保留现有正式镜像约束，在 COM3/COM6 完成 cold boot 和上述交互回归后再更新运行时状态。
> 2026-08-13 圆屏 Display source owner HIL（COM3 EchoEar-2ST、COM6 Waveshare 1.75C）：上述 display service 收口后的正式镜像均已写入 app 分区并 `verify-flash` digest 成功。COM3 使用 `build-unified-echoear/maclaw_esp32s3_client.bin`（`0x327a10`）启动，日志确认 `LCD double buffer ready: 2 x 259200 bytes in DMA PSRAM` 以及 `EchoEar-2ST round display/input adapter ready`，无 startup failed/panic/Guru/assert；随后 Wi-Fi 成功关联，但 Gateway handshake 返回 401（缺失/无效 bearer token）并进入 pairing recovery，属于认证外部状态，故没有将其标为 SERVICE online 验收。COM6 使用 `build-unified-waveshare/maclaw_esp32s3_client.bin`（`0x482550`）启动，确认 `LCD double buffer ready: 2 x 434312 bytes in DMA PSRAM`、`Waveshare 1.75C round display/input adapter ready`、`SERVICE_STATUS.ready=true`、runtime phase 5/first failure 0、fall detection monitoring、battery telemetry，35 秒窗口无 startup failed/panic/Guru/assert/reset。两设备目前均保留此正式镜像；本次运行时证据覆盖 panel 初始化、双缓冲、显示 source owner 与正常启动，但仍未替代人工触控单/双击、命令录音上传、扬声器播放、DISPLAY_OFF/触屏或实体键唤醒及长时动画压力回归。
## 2026-08-13：圆屏 DMA completion-fence timeout HIL（EchoEar COM3 / Waveshare COM6）

- 复用并重新构建已存在的非发布 profile：`echoear-2st-fence-fi` 与 `waveshare-amoled-1.75c-fence-fi`。两份生成的 sdkconfig 均确认 `CONFIG_MACLAW_TEST_BUILD=y`、`CONFIG_MACLAW_DISPLAY_TRANSFER_FENCE_TIMEOUT_ONCE=y`；该 seam 只使第一笔真实 panel transfer 的 caller fence 返回超时，profile-private adapter 继续保留 `transfer_pending`，禁止新 transfer 或 DISPLAY_OFF/wake 将旧 DMA source 当作可复用资源。
- COM3：test app（`0x327a20`）写入和 `verify-flash` 成功。日志准确命中 `echoear_display: test: abandoning first transfer fence wait`，之后 local services 正常达到 BOOT phase 4、Wi-Fi 关联；约 28 s 窗口无 startup failed/panic/Guru/assert/reset。后续 Gateway 401 进入 pairing recovery，仍是外部凭据状态。
- COM6：test app（`0x482560`）写入和 `verify-flash` 成功。日志准确命中 `waveshare_display: test: abandoning first transfer fence wait`，后续 `SERVICE_STATUS.ready=true`、phase 5/first failure 0、fall monitoring 正常，33 s 窗口无 startup failed/panic/Guru/assert/reset。
- 两台测试后均立即恢复正式 source-owner 镜像并再次 `verify-flash`：COM3 为 `build-unified-echoear`（正常双缓冲启动，仍因既有 401 pairing recovery）；COM6 为 `build-unified-waveshare`（双缓冲启动并再次 `SERVICE_STATUS.ready=true`、phase 5）。串口原始证据位于 `iot-agentos/.tmp-round-com3-fence-fi.log`、`.tmp-round-com6-fence-fi.log`、`.tmp-round-com3-release-after-fence.log`、`.tmp-round-com6-release-after-fence-followup.log`。
- 该闭环证明“caller timeout 不会把在途 profile-private DMA source 提前重用”与测试后的正式镜像恢复；它不模拟真实 callback 永久丢失或 controller command 与晚到 callback 的重排序，仍不构成 panel runtime restart 或完整 DISPLAY_OFF/wake 压力验收。
## 2026-08-13：紧凑屏 Display source-owner 收口（Bread Compact / Fangtang-4G）

- 新增私有 `main/compact_display_service.c/.h`，成为所选紧凑屏 display adapter 的唯一 translation-unit owner。`compact_renderer.c` 继续负责业务场景、双帧选择、dirty-region 比较、显示休眠状态与动画准入，但只调用普通的 service contract；不再直接包含或调用 Bread ST7789 / Fangtang NV3023 的 controller、SPI、DMA fence、PSRAM/DMA allocation、背光或 panel power 实现。
- DMA completion semaphore 从 renderer 迁入 service。初始化失败时 service 在 adapter rollback 后释放未发布 fence；成功后仍遵循既有 boot-lifetime 硬件语义。这样 renderer 不再拥有或传递 FreeRTOS fence，且 late-DMA 的 profile-private ownership 语义不变。
- `compact_profile_adapter.h` 改为只选择音频、输入、布局、身份与连接性适配；不再间接包含 display adapter。Bread/Fangtang display adapter 增加 source-owner 编译保护，防止未来将 header-local panel/static state 意外注入其他 translation unit。
- 所有显示相关的缓冲分配、同步像素提交、亮度、DISPLAY_OFF/wake、profile thinking patch、启动图、宠物帧所有权和动画 task placement 均由 `compact_display_service` 代理；公共 Device/Platform HAL 未增加 SPI、GPIO、controller、DMA 或 FreeRTOS 类型。
- 验证：`tools/build-profile.cmd bread-compact build` 成功，app `0x33b610`（最小 app 分区余 `0x64a00` / 10%）；`tools/build-profile.cmd fangtang-4g build` 成功，app `0x338140`（余 `0x67ec0` / 11%）。`git diff --check` 与 `tools/check-hal-boundaries.ps1` 均通过。
- HIL：COM4 已写入并 hash 校验 Bread app；冷启动确认 `LCD double buffering ready: 307200 bytes PSRAM + 7680 bytes DMA`、`SERVICE_STATUS.ready=true`、phase 5/first failure 0，未见 startup failed/panic/Guru/assert。COM5 已写入并 hash 校验 Fangtang app；冷启动确认 `LCD double buffering ready: 230400 bytes PSRAM + 3840 bytes DMA`、`SERVICE_STATUS.ready=true`、phase 5/first failure 0，未见 startup failed/panic/Guru/assert。两台均保留本轮正式镜像。
- 限制：本轮验收覆盖 source owner、双缓冲启动与普通 service online；仍需在有人机操作时补做 Bread/Fangtang 的 DISPLAY_OFF/实体键唤醒、命令录音/播放、长期宠物动画及 Fangtang Wi-Fi/TLS/4G 并发压力回归，不能据此声称完整 runtime restart 或全部显示压力用例已验收。

## 2026-08-13：紧凑屏 Audio/Input source-owner 收口（Bread Compact / Fangtang-4G）

- 新增私有 `main/compact_audio_service.c/.h` 与 `main/compact_input_service.c/.h`。紧凑屏 renderer 的 I2S 读写、音量校准、录音/WAV 缓冲、播放边界、输入原始采样、按键定时和 worker 创建均只通过私有 service 契约访问；业务与 Device/Platform API 不暴露 GPIO、I2S/DMA、heap capability、FreeRTOS handle 或板型判断。
- Bread/Fangtang 的 audio/input adapter 均加入 source-owner 编译保护；`compact_profile_adapter.h` 不再 include input adapter。Fangtang 原有 GPIO0 启动双击 transport selector 的通用手势状态从 profile 私有 translation unit 收入 `compact_input_service`，adapter 仅声明其是否存在、时窗、触点极性与按键时序；cellular adapter 仅消费 service 给出的规范化结果，仍不读取 GPIO。
- 构建：`tools/build-profile.cmd bread-compact build` 成功，app `0x33b930`（16 MiB app 分区余 `0x646d0`，约 10%）；`tools/build-profile.cmd fangtang-4g build` 成功，app `0x338450`（余 `0x67bb0`，约 11%）。`git diff --check` 与 `tools/check-hal-boundaries.ps1` 均通过。
- HIL：正式 app 已分别写入 COM4 Bread 与 COM5 Fangtang，两个 `write-flash` 均完成 hash verify。冷启动均达到 `ready=true`、local_ready=ok、first failure 0，且无 startup failed/panic/Guru/assert；COM4 确认双缓冲、input adapter 与 direct-I2S audio ready，COM5 额外确认启动 selector `active → closed: unchanged`，说明新的统一 Input source owner 保持了 Fangtang 的启动窗口。
- 限制：以上是 source-owner、构建和冷启动证据，不替代实体按键单击/双击/长按、Bread 音量键、Fangtang 启动双击切换网络、命令录音上传、扬声器播放、音量调节与 sleep/wake 的人工交互回归；这些仍需随业务联调完成。

## 2026-08-13：紧凑屏 Connectivity source-owner 收口（Bread Compact / Fangtang-4G）

- 新增私有 `main/compact_connectivity_service.c/.h`，成为 Bread Wi-Fi-only no-op 与 Fangtang ML307 蜂窝适配器的唯一 translation-unit owner。`compact_renderer.c` 的 board-port facade 仅转发规范化 service 操作；`compact_profile_adapter.h` 不再 include connectivity adapter，renderer 不再间接包含 modem UART/GPIO、ML307 request、APN、旧版 transport preference migration 或网关兼容改写实现。
- Fangtang adapter 增加 source-owner 编译保护。其板级职责不变：modem prepare/start、HTTP/stream request、request cancel、NVS 旧 preference 迁移、启动双击选择写入与 ML307 TLS 兼容 URL 改写仍留在 adapter；Connectivity Service 继续拥有 uplink 选择和请求语义。Bread 在同一私有契约下明确返回不支持，不引入 4G 组件或硬件事实。
- 构建：`tools/build-profile.cmd bread-compact build` 成功，app `0x33ba00`（16 MiB app 分区余 `0x64600`，约 10%）；`tools/build-profile.cmd fangtang-4g build` 成功，app `0x3384e0`（余 `0x67b20`，约 11%）。`git diff --check` 与 `tools/check-hal-boundaries.ps1` 均通过。
- HIL：正式镜像已分别写入 COM4/COM5 并完成 hash verify。两台冷启动均已达到 `ready=true`、first failure 0，未见 startup failed/panic/Guru/assert；COM4 已实际完成 Wi-Fi + Gateway handshake HTTP 200，COM5 已完成 Wi-Fi 关联并保持 Fangtang GPIO0 startup selector `active → closed: unchanged` 后进入 Gateway startup。两台保留本轮正式镜像。
- 限制：该证据验证 source-owner 和 Wi-Fi cold-start 路径，不验证 COM5 实际蜂窝 attach、ML307 HTTP/stream/cancel、启动双击切换后网络持久化/恢复、Wi-Fi 与 4G 切换、TLS 兼容 URL、蜂窝并发上传或 runtime restart；这些仍需要独立 HIL 与故障注入回归。

## 2026-08-13：紧凑屏 Peripheral source-owner 收口（Bread Compact / Fangtang-4G）

- 新增私有 `main/compact_peripheral_service.c/.h`，作为所选 profile 的可选 battery/charge/motion adapter 唯一 source owner。`compact_renderer.c` 仅通过 value-only service 调用初始化、背景 worker 有界 stop、归一化电源快照与 motion sample；`compact_profile_adapter.h` 不再 include peripheral adapter。ADC、charge GPIO、worker handle、critical section 与无传感器 profile 的 no-op 语义均不进入 renderer 或 Device/Platform API。
- Fangtang 的既有 ADC 电量与充电监控继续由 profile 实现，且由新的 service translation unit 独占编译；Bread 保持无 fuel-gauge/no-motion 的规范化 unavailable 回退。两个 profile adapter 均增加 source-owner 编译保护，避免未来把 profile static/peripheral SDK state 注入其他 translation unit。
- 构建：`tools/build-profile.cmd bread-compact build` 成功，app `0x33ba80`（16 MiB app 分区余 `0x64580`，约 10%）；`tools/build-profile.cmd fangtang-4g build` 成功，app `0x338530`（余 `0x67ad0`，约 11%）。`git diff --check` 与 `tools/check-hal-boundaries.ps1` 均通过。
- HIL：正式镜像已写入 COM4/COM5 并完成 hash verify。两台 cold boot 都达到 `ready=true`、first failure 0，未见 startup failed/panic/Guru/assert；COM5 确认 Fangtang battery monitor 已启动并输出 ADC/charging 状态，COM4 按 profile 预期不暴露电池监控。两台保留正式镜像。
- 限制：本轮只验证 owner 边界与启动时 battery monitor；不替代 COM5 长时 ADC 稳定性、充电插拔、低电量策略、worker stop timeout/retry、sleep/wake 下电源监控或未来带 IMU 紧凑硬件的真实 motion HIL。

## 2026-08-13：紧凑屏 Visual Profile service（Bread Compact / Fangtang-4G）

- 审计结论：layout、启动图、产品 identity、网络 badge 属于产品视觉差异，不应伪装成硬件 HAL；但 shared renderer 也不应直接选择 Bread/Fangtang adapter。新增私有 `main/compact_visual_profile_service.c/.h`，将五类 layout、Fangtang sugar/identity composition、network badge state 与 renderer value bridge 收口为中性的 visual-profile contract。
- 硬件边界保持不变：visual profile 不包含 panel、GPIO、SPI、DMA、I2S、ADC、ML307 或 FreeRTOS hardware handle；Display/Audio/Input/Connectivity/Peripheral 五个 source-owner service 仍各自持有物理实现。这样新硬件可独立实现硬件 service，而新产品皮肤/屏幕内容可只实现 visual profile，不会将业务场景回退为板型条件分支。
- 构建：`tools/build-profile.cmd bread-compact build` 成功，app `0x33c020`（16 MiB app 分区余 `0x640e0`，约 10%）；`tools/build-profile.cmd fangtang-4g build` 成功，app `0x338ca0`（余 `0x67360`，约 11%）。`git diff --check` 与 `tools/check-hal-boundaries.ps1` 均通过。
- HIL：正式 app 已分别写入 COM4/COM5，写入 hash verify 成功。两台 cold boot 均出现双缓冲 ready 与 `ready=true`，未见 startup failed/panic/Guru/assert；COM4 完成 Gateway handshake，COM5 保持 Fangtang 网络 badge/identity 路径并进入 gateway startup。两台保留正式镜像。
- 限制：该轮验证的是 visual-profile 路由和启动稳定性，不是视觉像素级验收；仍需要对 Bread standby/天气/宠物、Fangtang sugar/网络 badge、录音/响应/闹钟页面、屏幕休眠唤醒及长时动画进行人工屏幕回归。

## 2026-08-13：Fangtang-4G 显示传输与交互策略复核

- 复核确认 Fangtang 并非未使用双缓冲：`compact_renderer` 保留两张 240×240 RGB565 PSRAM 场景帧，并以同一帧比较器仅提交变化矩形；NV3023 的 GRAM 行偏移、SPI/DMA fence、背光和实际传输全部仍由 `fangtang_display_adapter` 私有持有。双缓冲解决合成/撕裂和前后帧比较，不能让单线 SPI 面板硬件 page-flip。
- COM5 上的动态瓶颈是串行面板传输被拆成许多同步小 DMA 块，而不是缺失双缓冲。将发布默认相邻行块从 8 提为 16（小于 8 KiB 的 DMA 源），同步更新 standby 变化行条带；全屏更新的 command/fence 次数减半，且未将行数、SPI、DMA 或控制器事实暴露到业务层、Device API 或 Platform API。测试构建仍可通过 `CONFIG_MACLAW_FANGTANG_DISPLAY_TEST_TRANSFER_ROWS` 覆盖该硬件参数。
- 已有 profile 私有一秒滑窗串口指标 `fangtang_display: transport: blocks=… rows=… bytes=… active=… avg=…`，用于后续 COM5 动态画面 HIL 取证；不把 SPI 指标扩展为公共 HAL 合同。思考局部 patch 仍为 120 ms，远端多帧宠物仍为 50 ms 目标节拍；若实测吞吐不足，应先依据该指标缩小动态矩形或调整 profile 私有传输块，不以业务层板型分支规避。
- 同时审计了跨硬件输入业务策略：所有 profile 的触摸/实体键先归一化为 Device Input，再经 `app_intent_service` 进入共同意图队列；`main.c:on_app_intent` 已在业务边界统一保证 DISPLAY_OFF 的首个主交互“只亮屏、整次手势不进入录音/会议”，命令录音中 contact-down 请求停止并消费后续完成手势，双击在处理阶段取消，闹钟中的主交互优先确认。该结论适用于 EchoEar/Waveshare 的触摸与 Bread/Fangtang 的实体主键，未发现应再下沉到 HAL 的板型分歧。
- 验证：`tools/build-profile.cmd fangtang-4g build` 成功；`git diff --check` 与 `tools/check-hal-boundaries.ps1` 通过。正式 `build-unified-fangtang/maclaw_esp32s3_client.bin` 已写入 COM5 并 `Hash of data verified.`；冷启动确认 `LCD double buffering ready: 230400 bytes PSRAM + 7680 bytes DMA`、`input adapter ready`、`ready=true`，无 `startup failed`、panic、Guru 或 assert。
- 限制：本轮已验证构建、写入、启动和资源大小，尚未用真实多帧宠物/思考页面采集完整一秒 transport 指标并进行肉眼流畅度回归；16 行是已写入的保守发布提升，不应在没有 COM5 连续动画和 Wi-Fi/TLS/4G 并发压力证据前继续提高 SPI 时钟或 DMA 块大小。

## 2026-08-13：紧凑屏遗留 visual selector 删除与正式镜像回归

- 复核发现 `main/boards/compact_profile_adapter.h` 已不再被 renderer 或其他生产源引用，却仍保留 Bread/Fangtang 的 layout、startup、identity 与 network badge selector，属于已完成 Visual Profile service 收口后的死代码，容易在以后被误用并重新把产品视觉差异耦合进板级 selector。
- 已删除该遗留 header，并将 Fangtang visual profile 的内部导出重命名为 `fangtang_visual_profile_*`；`compact_visual_profile_service` 是唯一选择这类 product visual adapter 的 source owner。硬件 Display/Audio/Input/Connectivity/Peripheral service 的 source-owner 边界、公共 Device/Platform API 和业务状态机均未改变。
- 验证：`tools/build-profile.cmd bread-compact build` 与 `tools/build-profile.cmd fangtang-4g build` 均成功；app 分别为 `0x33c010`（余 `0x63ff0`，11%）和 `0x338be0`（余 `0x67420`，11%）。`git diff --check` 与 `tools/check-hal-boundaries.ps1` 通过。
- HIL：对应正式 app 已写入 COM4 Bread 和 COM5 Fangtang，两个 `write-flash` 均完成 `Hash of data verified.`。冷启动分别确认 `LCD double buffering ready: 307200 bytes PSRAM + 7680 bytes DMA` 与 `LCD double buffering ready: 230400 bytes PSRAM + 7680 bytes DMA`，均到达 `ready=true` / `local_ready=ok`，观察窗口内没有 startup failed、panic、Guru 或 assert。
- 限制：此项验证 source-owner 删除和正式启动，不能替代显示布局像素级、实体键/触摸、命令录音、扬声器、DISPLAY_OFF wake、或多帧宠物在 Wi-Fi/TLS/4G 并发下的长期 HIL；这些继续作为后续业务与硬件回归项。

## 2026-08-13：Fangtang visual profile 命名与构建所有权清理

- 删除 selector header 后继续复核发现 Fangtang 的视觉实现文件仍沿用 `board_port_fangtang_4g.c` 名称，虽不含硬件端口逻辑，却会误导后续维护者将产品绘制或业务代码继续塞入 board port。已重命名为 `boards/fangtang_4g/fangtang_visual_profile.c`，并同步 CMake source 集合。
- 该文件只持有 renderer 的 value-only bridge 和 Fangtang sugar/identity composition；它不包含 Display HAL、GPIO、SPI、DMA、I2S、网络请求或业务状态机。其私有导出保持 `fangtang_visual_profile_*` 命名，由 `compact_visual_profile_service` 唯一调用，进一步明确“硬件适配”和“产品视觉适配”是两条独立扩展路径。
- 验证：Fangtang 正式 profile 重新构建成功，app `0x338be0`，最小 app 分区余 `0x67420`（11%）；`git diff --check` 与 `tools/check-hal-boundaries.ps1` 通过。最新正式 app 已再次写入 COM5 并 `Hash of data verified.`，冷启动出现双缓冲 `230400 bytes PSRAM + 7680 bytes DMA` 和 `ready=true`，未见 startup failed、panic、Guru 或 assert。
- 限制：这是 source naming/owner clarity 的收口，不替代 visual identity 像素回归、远端宠物实际动画吞吐、蜂窝网络、录音/播放或 DISPLAY_OFF/wake 的 HIL；后续仍应基于 profile 私有 transport 指标在真实动态页采样后决定是否进一步调节 NV3023 传输参数。
## 2026-08-13：圆屏 Visual / Audio-Input Profile 分离收口（EchoEar-2ST / Waveshare 1.75C）

- 审计发现原 `round_profile_adapter.h` 同时选择圆屏几何、场景布局、字体、I2S/I2C/GPIO codec wiring、触摸/实体键手势参数；尽管它不属于 Device/Platform API，`board_port.c` 仍能经该聚合 header 间接接触音频 profile。这会让后续增加圆屏产品时容易把“产品视觉差异”和“硬件电气差异”重新耦合。
- 新增私有 `round_visual_profile_service.c/.h` 与 `boards/round_visual_profile_adapter.h`。公共 round scene renderer 现在只通过 value-only visual service 获得屏幕尺寸、stripe cadence、宠物节拍、圆形安全布局以及 24/32 点阵字体 raster；`board_port.c` 不再 include 旧 profile selector、font selector，也不读取 codec profile。
- 新增 `boards/round_audio_profile_adapter.h`，仅由 `round_audio_service.c` 和 `round_input_service.c` 的 private source owner 使用。codec、I2C/I2S、GPIO、touch/activate-key 与 gesture timing 仍留在 Audio/Input HAL 下；renderer 仅从 Audio service 查询采样率、默认音量和可诊断名称，启动 network-selector 入口也已转发为 Input service contract。
- 删除无引用的 `round_profile_adapter.h` 与 `round_display_font_adapter.h`。这一拆分不改变 EchoEar 的 360px 圆屏布局/24 点阵字体、Waveshare 的 466px 圆屏布局/32 点阵字体、ES7210/ES8311 初始化寄存器序列或既有触摸/按键语义；它只明确 source ownership 和扩展路径。
- 构建与边界验证：`tools/build-profile.cmd echoear-2st build`、`tools/build-profile.cmd waveshare-amoled-1.75c build`、`tools/build-profile.cmd bread-compact build`、`tools/build-profile.cmd fangtang-4g build` 均成功；`git diff --check` 与 `tools/check-hal-boundaries.ps1` 均通过。最新 app 分区余量：EchoEar `0x77a30`（13%），Waveshare `0x7cfa0`（10%），Bread/Fangtang 保持各自既有约 10%/11% 余量。
- HIL：正式 app 已写入并完成 `Hash of data verified.`：COM3 EchoEar（app SHA 前缀 `2b7edba22`）与 COM6 Waveshare（`4927fa851`）。两者冷启动都到达 `BOOT_STATUS.ready=true` / `local_ready=ok` / `first_failure_phase=0`，且观察窗口内无 `startup failed`、`panic`、`Guru` 或 `assert`；COM6 同时确认 QMI8658 fall-detection pipeline 正常启动。
- 限制：本轮证明 profile owner 分离、构建和冷启动，不替代 EchoEar/Waveshare 的像素级待机/宠物/曲线文字回归、触摸单击/双击/长按、录音/播放、DISPLAY_OFF 唤醒和长时动画/网络并发 HIL。Fangtang 多帧宠物或思考页的 COM5 transport 指标采样仍是显示性能的后续验收项；在取得该证据前不继续提高 SPI 时钟或传输块大小。

## 2026-08-13：Fangtang-4G 动态宠物传输取证增强（COM5）

- 复核结论：Fangtang 已使用双帧 RGB565 组合缓冲。COM5 冷启动日志确认 `LCD double buffering ready: 230400 bytes PSRAM + 7680 bytes DMA`；双帧用于让 CPU 在 PSRAM 中完成下一帧组合并以 dirty rectangle 提交，NV3023 仍是 40 MHz 单线 SPI，因而不存在硬件 page flip，也不能把双缓冲误解为屏幕扫描带宽翻倍。
- 为避免仅以 DMA 块数评估动态效果，Fangtang Display 私有 adapter 新增第二组只读诊断窗口：原有 `transport` 统计物理 DMA block；新增 `presentation` 统计每个 renderer rectangle 的 rows、bytes 和端到端提交耗时。统计完全保留在 board-private Display HAL 内，不向 Device/Platform API 暴露 SPI、DMA、controller handle 或传输行数。
- COM5 HIL：正式 app 构建成功（`0x338cd0`，最小 app 分区余 `0x67330` / 11%），仅 app 写入完成 `Hash of data verified.`；冷启动 `BOOT_STATUS.ready=true` / `local_ready=ok`，app SHA 前缀为 `5d7034253`，观察窗口未见 startup failed、panic、Guru 或 assert。首批全屏与状态重绘示例为 `presentation: rects=16 rows=252 bytes=120024 active=151 ms avg=9438 us`；常规待机小区域更新为每秒 1--4 个 rectangle、约 0.5--0.9 ms/rectangle。这说明双缓冲的 delta presenter 已实际工作，且普通待机并非被整屏 SPI 刷新拖慢。
- 远端 8 帧宠物包在同一 COM5 冷启动中完成逐帧 SHA 校验下载，并出现 `startup pet frames downloaded; installing first frame`、`startup pet first frame applied` 与 `pet asset cached`。但当前设备恢复的 60 秒 DISPLAY_OFF 超时早于慢链路下完整帧包安装完成；熄屏后 renderer 按业务规则不运行装饰动画。因此此次不能把下载完成或小区域统计当作“多帧 20 FPS 已完成”的证据。
- 后续 HIL 前置条件：在 GUI/Hub 将 COM5 的 screen sleep timeout 临时设为大于完整资源下载与安装时间，或先人工唤醒后再进入已缓存的 idle 多帧宠物；采集连续至少 1 秒的 `presentation` 和 `transport`，确认约 50 ms cadence、无全屏重复提交、无 DMA fence timeout，并在 Wi-Fi/TLS 与 4G 负载下重复。取得该证据前，不提高 40 MHz SPI 时钟，也不继续扩大 16 行 DMA block。

## 2026-08-13：命令取消手势迁移至 Input HAL

- 复核发现命令处理阶段的“允许取消”此前错误穿过 Display 链路：`app_ui -> device_display -> Display Service -> Platform Display -> board_port`，最终才到圆屏触摸手势识别器。这是输入策略被误建模为显示状态；新硬件若改为实体键、触屏或辅助无障碍控件，业务层会被迫继续依赖 Display HAL。
- 已改为统一调用链：`app_ui -> device_input_set_command_cancel_enabled -> Input Service -> Platform Input -> board_port -> profile-private Input adapter`。Display Service、Platform Display、Device Display 中的同名请求、枚举与函数已删除；输入策略不再占用 Display task，也不再让显示重放承担手势窗口传递。
- 行为保持：EchoEar-2ST/Waveshare 的 `round_input_service` 仍在窗口重新启用时重置触摸去重/双击状态，保留“处理阶段双击取消、控制器重复 contact 不取消”的规则；Bread Compact/Fangtang-4G 的实体主键 profile 收到同一归一化策略但当前无需改变扫描参数，明确实现为 profile-local no-op。业务层不读取 GPIO、触摸坐标、控制器事件、去抖或双击时间。
- UI model 仍保存该瞬态业务状态，理由是闹钟覆盖结束后的 UI 重放必须恢复与当前命令一致的 Input policy；但重放现在重投递至 Input HAL，而不是错误地重放为显示请求。闹钟激活时保持原有冻结规则，解除后由普通 replay 恢复。
- 验证：`tools/build-profile.cmd echoear-2st build`、`tools/build-profile.cmd waveshare-amoled-1.75c build`、`tools/build-profile.cmd bread-compact build`、`tools/build-profile.cmd fangtang-4g build` 均成功；`git diff --check` 与 `tools/check-hal-boundaries.ps1` 通过。最新 app 余量分别为 EchoEar `0x77a70`（13%）、Bread `0x64030`（11%）；Waveshare/Fangtang 亦完成链接和分区检查。构建中的第三方 Kconfig `default False` 提示为既有依赖警告，非本次改动引入。
- 限制与后续 HIL：本项是 source ownership 与全 profile 编译验证，尚未重新刷写设备。后续在 COM3/COM6 验证处理阶段触摸单击/双击取消窗口、COM4/COM5 验证实体主键不改变录音/唤醒语义；四机均需回归 DISPLAY_OFF 首次交互只亮屏、录音时 contact-down 停止、闹钟优先交互和取消后的网络/显示收敛。

## 2026-08-13：紧凑屏 Input scanner 生命周期迁入 Input source owner（Bread Compact / Fangtang-4G）

- 复核发现 Bread/Fangtang 虽然已有 `compact_input_service`，但实体键扫描 FreeRTOS task、completion semaphore、stop flag、去抖和单/双击/长按分类仍留在 `compact_renderer`。这会使 renderer 同时成为输入任务 owner，新增硬件仍需要复制 renderer 生命周期细节，违背“业务逻辑只依赖 HAL”的约束。
- 已将 scanner 的完整私有生命周期迁入 `compact_input_service`：该 service 独占 scanner task、completion semaphore、一次性 stop、20 ms 扫描时序、去抖、长按与双击分类；profile-private input adapter 仅保留 GPIO 极性/原始采样、实体键能力、启动 selector 能力和 scanner worker 的资源规格。renderer 只在 board 构造时提供规范化 `DEVICE_INPUT_*` publisher，并不持有 task/semaphore 或手势时序。
- 失败路径按发布状态区分：scanner 尚未发布时，`discard_unpublished_scanner_state()` 只释放 preparation semaphore；scanner 已发布后，任何 rollback 都不能提前释放 callback 仍可能使用的 state，必须经 Input Service 既有 `stop/join` 路径结束。stop timeout 保留 task/semaphore 所有权，供后续生命周期 retry 使用。
- 行为不变：Bread 继续发布 primary contact-down、单击、双击、长按和音量键规范化事件；Fangtang 的 GPIO0 启动双击网络选择仍在 scanner 启动前由 Input service 执行，运行期单/双/长按仍走同一规范化动作。公共 Device/Platform API 未新增 GPIO、FreeRTOS、去抖或板型分支。
- 验收门禁：需完成四 profile build、`git diff --check` 与 HAL boundary 检查；随后在 COM4/COM5 做实体键 HIL（包括录音中 contact-down 停止、DISPLAY_OFF 首次按键仅亮屏、Fangtang boot selector），才可把本项标记为交互级完成。

## 2026-08-13：启动网络选择跨 HAL 依赖收口

- 在 scanner 生命周期迁移后继续审计发现 Fangtang 的 Connectivity adapter 曾直接 include `compact_input_service.h`，由 modem/persistence adapter 主动读取 Input service 的启动双击结果；同时 Device/Platform/Connectivity 保留一条未被业务使用的 `take_startup_transport_toggle` pass-through。这会把“输入手势分类结果”反向泄漏到 Connectivity 的 profile-private source owner，增加新增硬件时的 service 耦合。
- 已改为单向契约：Input HAL 在启动窗口结束后只向 compact renderer/board facade 给出一个规范化 `toggle_requested` 布尔值；该值随同当前 uplink 进入 Connectivity Service，再由 Fangtang Connectivity adapter 仅负责持久化和应用 uplink 选择。adapter 不再 include Input service，也不读取扫描器或 GPIO 状态。
- 同步删除未使用的 `device/connectivity/platform/board_port take/wait startup selector` 透传，以及 EchoEar/Waveshare 对应的无效 round-input no-op hook。既有 `device_connectivity_apply_startup_transport_toggle()` 保持业务启动调用点不变，Bread/Waveshare/EchoEar 的 Wi-Fi-only 路径继续返回“不切换”。
- 验证：四个 profile 全量链接通过，当前 app 分区余量为 EchoEar `0x77a60`（13%）、Bread `0x640d0`（11%）、Fangtang `0x67510`（11%）、Waveshare `0x7cfd0`（10%）；`git diff --check` 与 `tools/check-hal-boundaries.ps1` 通过。第三方 ESP-SR Kconfig `default False` 仅为既有依赖提示。
- HIL 仍待执行：COM5 需要确认 GPIO0 启动双击仍正确在 Wi-Fi/4G 之间切换、重启后持久化恢复；COM3/COM4/COM6 需要确认无 selector 的启动路径不受影响。构建通过不替代上述实机语义验证。

## 2026-08-13：圆屏 Input scanner 调度迁入 profile-private adapter（EchoEar-2ST / Waveshare 1.75C）

- 继续对照紧凑屏 scanner 收口审计圆屏发现：`round_input_service` 虽已拥有 task、completion semaphore、stop/join 与触摸/实体键手势分类，但 3 KiB stack 和 priority 仍作为 `round_input_profile_t` 的共享标量，由 service 直接调用 `xTaskCreate`。这会让共享 Input 实现继续承担新硬件的 FreeRTOS 资源配置。
- 已将圆屏 scanner worker 的创建迁为 profile-private `round_input_adapter_start_scan_task()`：EchoEar 与 Waveshare adapter 各自保留相同的现有 3 KiB / priority 4 配置及独立 task 名；`round_input_service` 仅验证普通 gesture profile，管理 task 生命周期并调用 adapter 创建 worker。`round_input_profile_t` 现在只保留去抖、触摸过滤、单/双击/长按等可移植手势值。
- 同时删除没有行为的 round boot-window hook：圆屏从未支持启动网络选择，启动时不再经过 Input adapter 的空调用。触控 controller、GPIO、电气极性、native double-tap 编码与 scanner 调度均留在 adapter；业务侧仍只接收规范化 Input action。
- 构建：EchoEar `0x3285c0`（余 `0x77a40` / 13%）与 Waveshare `0x483060`（余 `0x7cfa0` / 10%）完整链接通过；`git diff --check` 和 `tools/check-hal-boundaries.ps1` 通过。编译中的 profile header `unused-function` 与第三方 Kconfig 提示是 source-owner header-local 既有告警，不改变链接结果。
- COM3 HIL：正式 EchoEar app 已写入并完成 `Hash of data verified.`。冷启动日志确认 `EchoEar-2ST round display/input adapter ready`、`interaction monitor ready: idle_pressed=no touch=yes`、input queue started 与 `BOOT_STATUS.ready=true/local_ready=ok`，35 秒窗口未见 `startup failed`、panic、Guru 或 assert。COM6 Waveshare 尚未刷写；仍需在 COM3/COM6 人工完成触摸/实体键单击、双击、长按、DISPLAY_OFF 首触只亮屏、录音 contact-down 停止与 command-cancel 回归。
## 2026-08-13：圆屏 Input profile source owner 与 Audio source owner 彻底分离（EchoEar-2ST / Waveshare 1.75C）

- 复核发现上一轮虽然已将圆屏 scanner 的调度参数迁入 profile-private adapter，但 `round_input_service.c` 仍经由 `round_audio_service` 临时 facade 获取 Input profile、GPIO 激活键、输入来源和 worker 创建函数。更深层的问题是 Audio profile selector 包含 EchoEar/Waveshare 的大体积硬件 adapter header，使 Input 相关编译单元可能被动看到面板、PMIC、触控或 I2C static state，不符合“Input 仅依赖 Input HAL、Audio 仅拥有共享触控总线生命周期”的 source-owner 约束。
- 新增独立的 `round_input_profile_service.c/.h` 与 `boards/round_input_profile_adapter.h`。`round_input_service` 现在只通过这个中性 private facade 获取 gesture policy、GPIO key 初始化/采样、输入来源归一化、boot gesture 消费和 profile-owned scanner task；EchoEar/Waveshare 分别以 `echoear_input_adapter.h`、`waveshare_input_adapter.h` 实现 input-only 合约。新硬件只需提供自身输入适配，不需要将 GPIO/task 配置塞入 Audio adapter 或 shared renderer。
- Audio 保留唯一合理的交叉桥接：触控控制器因共享 codec I2C bus，仍由 `round_audio_service` 创建、销毁并向 Input 提供 `touch_read/touch_ready/native_double_tap` 的已归一化观察；Audio profile 的 `touch_initialization_required` 已移动到 `round_audio_profile_t`，不再错误放在 Input gesture profile。PMIC/IMU 同样仍由 Audio-created bus 的 source owner 管理。
- 移除了 Audio 临时 Input facade，以及旧 EchoEar/Waveshare 大硬件 adapter 中的输入 GPIO、task 调度和 gesture 函数。CMake 仅在圆屏 profile 选择 `round_input_profile_service.c`；紧凑屏不携带该 source。Device/Platform 公共边界没有新增 GPIO、FreeRTOS、controller handle 或 board 分支。
- 验证：`tools/build-profile.cmd echoear-2st build` 与 `tools/build-profile.cmd waveshare-amoled-1.75c build` 均完成；`git diff --check`、`tools/check-hal-boundaries.ps1` 通过。Waveshare app 已写入 COM6 且 `Hash of data verified.`，运行日志达到 `ready=true`、`local_ready=ok`、`first_failure_phase=0`，fall-detection 仍为 enabled，未见 startup/panic/assert 失败。此时 COM3 EchoEar 未连接（当前系统无 COM3），因此该轮只能以 profile build 验证 EchoEar；恢复连接后需刷写并完成冷启动与手势 HIL。

## 2026-08-13：紧凑屏 Input HAL 正式构建与冷启动回归（Bread Compact / Fangtang-4G）

- 对照 Phase 6 的 source-owner 目标复核：实体键 scanner、completion semaphore、20 ms 扫描、去抖与单/双击/长按分类仍由 `compact_input_service` 独占；各 profile-private input adapter 仅提供 GPIO 原始采样、极性、实体键能力、启动 selector 能力与 worker 资源规格。共享业务层继续只接收规范化 `DEVICE_INPUT_*`/`APP_INTENT_*`，不读取 GPIO 或接触手势时序。
- 同时确认 DISPLAY_OFF 的首个主交互以及录音中的 contact-down 停止仍在 `main.c` 的共同业务边界处理：不论触摸还是实体按键，熄屏首个完整手势只恢复显示；录音中的 contact-down 请求停止并消费同一手势的完成事件。该策略没有被下沉到某个硬件 profile。
- 构建：`tools/build-profile.cmd bread-compact build` 和 `tools/build-profile.cmd fangtang-4g build` 已完整链接，且 build wrapper 中的 `tools/check-hal-boundaries.ps1` 通过。Bread app 大小为 `0x33cc80`；Fangtang app 为 `0x339750`，最小 app 分区余 `0x668b0`（11%）。第三方 Kconfig 的 `default False` 提示为既有依赖提示，不是本次 HAL 变更错误。
- HIL：上述正式镜像已分别写入 COM4 Bread 与 COM5 Fangtang，所有 bootloader/partition/model/app/storage 分段均完成 `Hash of data verified.`。COM4 冷启动日志确认 `LCD double buffering ready: 307200 bytes PSRAM + 7680 bytes DMA`、`bread-controls` input adapter、`BOOT_STATUS.ready=true` 与 `SERVICE_STATUS.ready=true`；COM5 确认 `LCD double buffering ready: 230400 bytes PSRAM + 15360 bytes DMA`、`fangtang-primary-control` input adapter、1.8 秒启动 selector 正常关闭、`BOOT_STATUS.ready=true` 与 `SERVICE_STATUS.ready=true`。两窗口均未出现 startup failed/panic/Guru/assert。
- 说明：COM5 本轮的 32 行 DMA staging 是既有 Display 私有 adapter 配置；日志仍显示差分 `presentation`，小区域通常为 1--14 ms，而大面积场景更新受 40 MHz 单线 SPI 物理吞吐限制。双缓冲已启用，不应将大场景的 45--98 ms 提交误诊断为未使用双缓冲。
- 未完成的交互级验收保持明确：仍需在 COM4/COM5 人工验证录音中实体键 contact-down 停止、DISPLAY_OFF 首按仅亮屏、Fangtang GPIO0 启动双击 Wi-Fi/4G 切换与持久化恢复；并需完成多帧宠物在 Wi-Fi/TLS/4G 并发下至少一秒的连续 transport/presentation 采样后，才能决定是否进一步调整 Fangtang SPI 传输参数。

## 2026-08-13：Fangtang-4G 动态宠物热路径复核与低风险优化（COM5）

- 复核结论：Fangtang 不是“未使用双缓冲”。`compact_renderer` 已分配两个 240×240 RGB565 PSRAM frame buffer（合计 230400 bytes）和一个 32-row internal-DMA staging buffer（15360 bytes）；每帧在 back buffer 合成后，以前缓冲比较并仅传输 changed rectangle。COM5 本轮合并后冷启动测得 `LCD double buffering ready: 230400 bytes PSRAM + 15360 bytes DMA`。
- 新证据说明问题在热路径而非缓冲缺失：旧 8 帧宠物 HIL 在 176 px 宠物绘制时出现约 76--126 ms 的 render 耗时、约 141--181 ms 实际 cadence；同时日志仍显示每帧是多个小 rectangle，而非持续全屏传输。原因是对两帧均为 opaque（alpha=255）的像素，仍走了包含三次 `/255` 的通用 premultiplied-alpha 插值路径。
- 已在共享 `compact_renderer` 中增加等价的 opaque-RGB565 快路径：只有两个 alpha 均为 255 时，按相同的 Q8 mix 对 5/6/5 三个通道做线性插值；半透明边缘仍保留原 premultiplied-alpha 路径。这是 scene 和 panel 无关的合成算法优化，不暴露 SPI/DMA 也不动任何 HAL 公共边界；对 opaque 像素的结果与原通用公式等价。
- 验证：`build-unified-fangtang` 完整重编译通过，app 为 `0x339790`，余 `0x66870` / 11%；只写入 COM5 app 分区并完成 `Hash of data verified.`。新镜像冷启动达到 `SERVICE_STATUS.ready=true`、`local_ready=ok`，日志未见 startup failed/panic/Guru/assert。默认与配置下宠物资源还在下载/安装中，因此实机只采集到待机小区域更新，尚未将引入新的 multi-frame cadence 数据写成验收结论。
- 后续 HIL 不变：临时提高 COM5 `screenSleepSeconds`，完整安装 8 帧宠物后采集至少 1 秒 `fangtang_display: animation/transport/presentation`，在 Wi-Fi/TLS 和 4G 负载下重复。如果仍未达到视觉目标，先以实测的 changed-pixel 面积和 CPU 占用率决定是否调整宠物尺寸/帧间插值策略；不在缺少完整 HIL 证据时提高 40 MHz SPI 时钟或更改已验证的 DMA stripe 大小。

## 2026-08-14：Fangtang-4G 双缓冲与完整宠物缓存复核（COM5）

- 结论：Fangtang 已使用双缓冲，不存在“未启用双缓冲”问题。`compact_renderer` 在 PSRAM 中保留两张 240×240 RGB565 framebuffer（共 230400 bytes），Fangtang Display adapter 保留 32 行 internal-DMA staging buffer（15360 bytes），并以前后帧比较后的 changed rectangle 进行 NV3023 40 MHz 单线 SPI 提交。双缓冲负责避免合成期间的撕裂和 DMA 源复用；它不能突破单线 SPI 的物理吞吐上限。
- 宠物缓存已改为完整包原子恢复：metadata 只有在所有帧写入并携带 SHA-256 后才提交；启动恢复放在 Input Service 就绪之后、Connectivity 启动之前，SPIFFS 工作由 internal-stack worker 执行。完整缓存失败或内存不足时只放弃持久化，已安装到内存的宠物继续显示。COM5 冷启动日志已确认 `frames=8/8`、`BOOT_STATUS.ready=true`、`SERVICE_STATUS.ready=true`，未见 startup failed/panic/Guru/assert。
- 当前 8 帧缓存的 frame_ms=450；本次冷启动采样在常规待机中只观测到小区域状态更新，尚未取得多帧宠物运动状态下连续一秒的 `fangtang_display: animation` 样本。因此不能将“约 50 ms cadence”或进一步 SPI 频率/stripe 调整写成验收结论。后续必须先在运动开启、idle/quiet 场景下，在 Wi-Fi/TLS 与 4G 负载下分别采集 animation/transport/presentation；若实测仍受限，再依据 changed-area、CPU render 与 SPI active time 决定减少动画变化面积、降低插帧目标或调整资源，而不是假定增加 framebuffer 会解决问题。

## 2026-08-14：圆屏 Wake Service runtime source owner 收口（EchoEar-2ST / Waveshare 1.75C）

- 复核发现 `round_wake_service` 先前只持有 recognizer/dispatcher 的 FreeRTOS 生命周期，而共享 `board_port.c` 仍直接持有 ESP-SR/MultiNet model、词表、阈值、识别循环、wake PCM capture 与 deferred dispatcher entry。这使新增圆屏硬件虽然已有 Wake Service，仍要修改 renderer 才能替换识别 runtime，且公开的 Wake private header 暴露 `TaskFunction_t`。
- 已将 ESP-SR/MultiNet 初始化、命令词表、阈值、模型清理、Audio HAL wake capture、pause acknowledgement、cooldown、diagnostic 和 deferred foreground callback 全部迁入 `round_wake_service.c`。`board_port.c` 现只在 Audio HAL 已准备后传入语义化的 `on_wake` 业务 callback，并负责 callback 触发后的前景命令流程；不再 include ESP-SR/model-path 头文件、不创建 recognizer/dispatcher task，也不接触 wake capture。
- `round_wake_service.h` 收敛为普通 callback/timeout contract，不再暴露 FreeRTOS task 或 ESP-SR 类型。新增 HAL boundary guard：禁止 `board_port.c` 出现 `esp_mn_*`、`esp_srmodel_*`、`model_path.h`、wake task entry 或 `round_audio_service_wake_capture_*`；并禁止 Wake header 将 RTOS/ESP-SR 实现类型重新泄漏给 renderer。
- 构建验证：`tools/check-hal-boundaries.ps1` 通过；`tools/build-profile.cmd waveshare-amoled-1.75c build` 与 `tools/build-profile.cmd echoear-2st build` 均完整链接成功，app 分别为 `0x484920`（余 `0x7b6e0` / 10%）和 `0x329e50`（余 `0x761b0` / 13%）。构建中第三方 ESP-SR Kconfig `default False` 提示为既有依赖警告；`board_port.c` 仅保留既有 `compose_text16_curve` 未使用 warning。
- COM6 HIL：正式 Waveshare app 已写入 COM6；启动日志确认 Wake Service 直接加载 `mn7_cn`、注册三条同 ID 词表、进入 `offline wake listening`、Audio HAL 采样为 16 kHz/512 samples，并由 `maclaw_board` 观察到 `offline wake task ready`。Gateway 在线、`SERVICE_STATUS.ready=true`、runtime phase 5/first failure 0，采样窗口无 startup failed/panic/Guru/assert；可选宠物媒体流程停止 wake 时确认 `offline wake stopped and model memory released`，说明 recognizer 正常释放模型与 wake capture。完整串口证据保存在 `iot-agentos/serial-com6-round-wake-service-hil-20260814.log`。
- 限制：本轮 COM6 只覆盖 startup/listening 与媒体流程的正常 stop/release，未人工触发语音词来证明“detect → recognizer 释放 → foreground callback”、录音 pause/resume、stop timeout 与恢复监听。COM3 必须在 Hub bearer token 有效时独立完成同类回归，不能用 COM6 结果外推。

## 2026-08-14：圆屏 Wake task placement 从 Audio HAL 迁回 Wake source owner

- 在前一轮 runtime 收口后继续审计发现：`round_audio_service.h` 仍公开 `TaskFunction_t` / `TaskHandle_t` 和两个 `start_wake_*_task` API，Audio adapter 也仍持有 recognizer/dispatcher 的 task 创建。这会使 Wake Service 虽拥有模型和 lifecycle state，却仍依赖 Audio HAL 暴露 FreeRTOS task 语义；新增 wake runtime 或改变 task footprint 时容易重新跨越 Audio/Wake 边界。
- 已将 recognizer 的 CPU1、10 KiB stack、priority 4 和 dispatcher 的 PSRAM、3 KiB stack、priority 5 placement 一并归入 `round_wake_service.c`；仅该 service 现在创建其 two task generations。`round_audio_codec_adapter.h` 只保留 ES7210/ES8311、I2S、PCM 线格式和 buffer allocator，`round_audio_service.h` 回到普通音频会话、PCM 和 WAV contract，不再泄漏 FreeRTOS task type 或 wake task creator。
- HAL boundary guard 已增加：renderer-facing Audio header 不能重新导出 wake task creation 或 `TaskFunction_t` / `TaskHandle_t`；Wake header 同样不暴露 RTOS/ESP-SR 类型。该规则不影响既有 Audio HAL 对 wake capture 的归一化数据服务，也不让业务层知晓 core、stack、PSRAM 或任务优先级。
- 构建：Waveshare `0x4848b0`（余 `0x7b750` / 10%）和 EchoEar `0x329df0`（余 `0x76210` / 13%）均完整链接成功，`tools/check-hal-boundaries.ps1` 与 `git diff --check` 通过。COM6 已写入正式 Waveshare app；串口确认 `offline wake listening`、`offline wake task ready`、Gateway `SERVICE_STATUS.ready=true`、可选媒体停用 wake 时 `offline wake stopped and model memory released`，且无 startup failed/panic/Guru/assert。证据：`iot-agentos/serial-com6-wake-placement-release-20260814.log`。
- 限制：COM6 仍未进行人工语音检测、foreground callback、pause/resume 和 timeout 重试；COM3 必须在有效 Hub token 下独立验证，不能由 COM6 启动/释放证据外推。

## 2026-08-14：圆屏 Peripheral readiness 收口至 Peripheral Service（EchoEar-2ST / Waveshare 1.75C）

- 审计发现圆屏启动阶段曾由 `board_port.c` 经 `round_audio_service_prepare_peripherals()` 请求触摸、PMIC 与 IMU 的 readiness。虽然共享 I2C bus 由 Audio source owner 创建是合理的，但该 API 名称与公开位置会让 renderer 看起来依赖 Audio HAL 来初始化非音频外设，新增圆屏板卡容易继续把外设生命周期塞入 Audio service。
- 已建立明确调用链：`board_port.c -> round_peripheral_service_prepare()`。Peripheral Service 现在是 renderer 唯一可见的 peripheral readiness requester，负责触摸、PMIC、IMU 及其可选能力的准备、失败分类和生命周期；Display renderer 不感知 I2C、PMIC 或 controller handle。
- 保留一个仅限 source-owner 的私有桥接 `round_audio_lifecycle_prepare_shared_bus()`（`round_audio_lifecycle.h`）：它只表达“Audio source owner 创建并维持圆屏共用 I2C bus”的内部 lifecycle 事实，不在 `round_audio_service.h`、Device API、Platform API 或 renderer-facing header 中导出。这样既不重复初始化共享总线，也不将总线 owner 错误转移给 Peripheral 或业务层。
- HAL boundary guard 新增禁止项：`board_port.c` 不得调用旧 `round_audio_service_prepare_peripherals`，`round_audio_service.h` 不得重新公开 peripheral-readiness API；任何新增圆屏外设必须通过 Peripheral Service 和 profile-private adapter 接入，而不是经 Audio/Display facade 横向穿透。
- 验证：`tools/check-hal-boundaries.ps1`、`tools/build-profile.cmd waveshare-amoled-1.75c build` 与 `tools/build-profile.cmd echoear-2st build` 通过。Waveshare app 为 `0x4848d0`（余 `0x7b730` / 10%），EchoEar app 为 `0x329e00`（余 `0x76200` / 13%）。COM6 已写入正式 Waveshare 镜像；日志 `iot-agentos/serial-com6-peripheral-ready-release-20260814.log` 记录 Gateway handshake 200、offline wake listener、`SERVICE_STATUS.ready=true`、fall-detection available/enabled、display heartbeat 以及媒体停用时 wake model 正常释放，观察窗口无 startup failed、panic、Guru 或 assert。
- 限制：该项证明 source ownership、构建与 COM6 冷启动/服务就绪；仍需分别进行 COM3 EchoEar 的有效 Hub token 回归，以及 COM3/COM6 的触摸、IMU、PMIC、电源策略和 DISPLAY_OFF 实际交互 HIL。COM6 的启动证据不能替代 EchoEar 的硬件行为验收。

## 2026-08-14：Fangtang-4G 动态宠物实机性能基线与传输资格修正（COM5）

- 在正式 COM5 镜像上连续采集 55 秒动态宠物而非只观察冷启动/待机小块刷新。日志 `iot-agentos/serial-com5-animation-audit-20260814.log` 的 1 秒滚动窗口稳定产生 12--14 次 logical tick，均实际完成 present；80 ms 目标下的平均完整帧间隔为约 82--97 ms，最大约 110--199 ms，每窗口出现 2--7 次 deadline miss。完整一帧的 `render_avg` 为约 50--65 ms，最大约 60--90 ms，说明当前卡顿来自“场景合成 + 已变化大区域的单线 SPI present”的综合预算，而非双 frame buffer 未启用或 DMA fence 丢失。
- 同一采样的 `presentation` 窗口每秒约提交 37--66 个 dirty rectangle、约 0.23--0.46 MiB 像素数据、active 时间约 0.17--0.35 秒。因为差分 presenter 按 32 行 band 计算每 band 的最小 x 范围，动态宠物的移动轮廓仍会在多条 band 形成宽矩形；双缓冲避免 CPU 合成和 DMA 源复用撕裂，但不能让这些变化像素免于穿过 NV3023 的 40 MHz 单线 SPI。
- 结论与策略：保留正式 profile 的 32-row、40 MHz、双 PSRAM frame buffer 和 delta present；在没有显示闪烁、重置、Wi-Fi/TLS/4G 并发 HIL 的情况下，不提高时钟也不扩大 transaction。若需进一步改善视觉，应先在 Fangtang visual profile 评估宠物有效变化面积、资源帧差异和插帧密度，并以相同 COM5 指标比较，而不是继续增加 framebuffer 或把 SPI 物理约束放入共享 scene。
- 同时修复测试资格描述：`MACLAW_FANGTANG_DISPLAY_TEST_TRANSFER_ROWS` 的测试范围现在覆盖生产值 32，Kconfig 说明也更正为“正式固定 32 行”。该 test-only knob 仍不导出 runtime setter、Hub 命令、Device/Platform API 或布局参数；它只能用于 COM5 的对照固件，不能改变发布镜像的传输策略。

## 2026-08-14：圆屏 Display profile selector 收口（EchoEar-2ST / Waveshare 1.75C）

- 审计发现 `round_display_service.c` 仍直接以 `CONFIG_MACLAW_BOARD_WAVESHARE_*` 在 EchoEar ST77916/QSPI adapter 与 Waveshare CO5300 adapter 之间选择。这一分支虽处于私有 service，但会让新增圆屏面板时必须修改共享 Display 生命周期源文件，而不是只新增板级适配与 selector 映射。
- 新增唯一的 `boards/round_display_adapter.h` selector。它只承担编译期 profile 映射，并保留 EchoEar adapter 所需的 implementation guard；`round_display_service.c` 现只引用 selector，继续独占显示亮度、DISPLAY_OFF、DMA fence、动画任务和错误处理的通用语义。公共 Device/Platform API、round renderer 与 visual-layout service 不接触 panel、SPI/QSPI、DMA、GPIO 或 controller handle。
- `tools/check-hal-boundaries.ps1` 新增 owner gate：该 selector 只能由 `round_display_service.c` include。新增圆屏 panel 若绕过 Display Service 直接包含 selector，或把 selector 拉入 renderer/visual profile，会在 CI 失败。
- 验证：`tools/check-hal-boundaries.ps1`、`tools/build-profile.cmd echoear-2st build` 与 `tools/build-profile.cmd waveshare-amoled-1.75c build` 通过；EchoEar app 为 `0x329e00`（余 `0x76200` / 13%），Waveshare app 为 `0x4848d0`（余 `0xbb730` / 14%）。本项不改变二者的显示驱动、屏幕形状、字体/布局、DISPLAY_OFF 或动画时序，故不需要刷写正式镜像；后续仍按各自 HIL 矩阵验证实际面板与交互。

## 2026-08-14：紧凑屏硬件 selector 收口（Bread Compact / Fangtang-4G）

- 继续同一 source-owner 审计后，发现 `compact_display_service.c`、`compact_audio_service.c`、`compact_input_service.c`、`compact_connectivity_service.c` 与 `compact_peripheral_service.c` 均在共享 Service 源文件内直接以 `CONFIG_MACLAW_BOARD_FANGTANG_4G` 选择 Bread/Fangtang 物理 adapter。虽然这些 Service 本身属于正确 owner，但新增紧凑硬件仍必须逐个修改共享生命周期实现，容易造成板型分支扩散。
- 新增五个 `boards/compact_*_adapter*.h` 私有 selector，分别对应 Display、Audio、Input、Connectivity 和 Peripheral。selector 仅持有编译期映射与 adapter implementation guard；共享 Service 仅 include 自己的 selector，仍独占统一会话、动画、扫描、传输、错误处理与生命周期语义。Fangtang peripheral 的 `.c` inclusion 仍被严格限制在 Peripheral selector 的唯一 source-owner translation unit。
- HAL boundary gate 同步改为两层 owner：Service 只能 include selector，具体 Bread/Fangtang adapter 只能由相应 selector include。这样新 profile 只需添加 adapter 和 selector 映射，renderer、Device/Platform API 与共享 Service 不应再产生新的硬件条件分支。
- 复查发现视觉 profile 仍是遗漏点：`compact_visual_profile_service.c` 曾在共享 Service 内按 `CONFIG_MACLAW_BOARD_FANGTANG_4G` 选择 Bread/Fangtang 的待机、录音、上传、回复、闹钟布局以及 Fangtang 身份艺术实现。新增 `boards/compact_visual_profile_adapter.h` 后，Visual Profile Service 只调用标准化 `compact_visual_profile_adapter_*` 契约；selector 独占产品差异和 Bread 的无额外艺术回退。该边界只承载布局、插画和网络徽标呈现，不承载控制器、DMA、GPIO 或电源事实。
- owner gate 扩展为：Visual Profile Service 只能 include visual selector，十个 Bread/Fangtang layout adapter 只能由该 selector include。这使新紧凑硬件只需实现并映射自己的视觉 profile，而共享 renderer、业务状态机、Display HAL 与公共 Device/Platform API 均无需新增板型条件分支。
- 验证（2026-08-14）：`tools/check-hal-boundaries.ps1` 与本次修改的 `git diff --check` 通过；`tools/build-profile.cmd bread-compact build` 和 `tools/build-profile.cmd fangtang-4g build` 均通过。Bread app 为 `0x33dcd0`（余 `0x62330` / 11%），Fangtang app 为 `0x33a660`（余 `0x659a0` / 11%）。本次仅移动编译期 profile 选择权，不改变面板驱动、SPI 频率、双缓冲、动画节奏或可见布局，因此无需刷写 COM4/COM5；后续实机验收继续复用既有输入、音频、联网和动态显示 HIL 矩阵。

## 2026-08-14：紧凑屏 Wake Service runtime source owner 收口（Bread Compact / Fangtang-4G）

- 继续审计发现紧凑屏虽然已经有 `compact_wake_service`，但共享 `compact_renderer.c` 仍直接持有 ESP-SR/MultiNet model、词表、阈值、识别循环、wake PCM capture 和 recognizer callback。这会迫使下一块 direct-I2S 硬件修改 renderer 才能替换 wake runtime，与圆屏已经完成的 Wake Service 边界不一致。
- 已把 ESP-SR/MultiNet 初始化、词表、阈值、模型清理、Audio HAL wake capture、pause acknowledgement、cooldown、诊断及 recognizer/dispatcher 生命周期全部迁入 `compact_wake_service.c`。`compact_renderer.c` 只在 Audio HAL prepare 成功后传入语义化业务 callback，并保留 command capture、UI 和业务流程；Wake private header 不再暴露 `TaskFunction_t`，业务与 renderer 不再接触 ESP-SR、model path、capture buffer 或 recognizer task entry。
- 检测命中后，Wake Service 先请求 recognizer 结束；独立 dispatcher 在确认 recognizer 已释放 ESP-SR model 与 direct-I2S wake capture 后再调用前景业务 callback。这把“检测 → 释放 → command capture”变成所有紧凑板共用的 lifecycle 不变量，避免 callback 在旧 wake I2S transaction 尚未退出时抢占麦克风。
- HAL boundary gate 已加强：共享 `compact_renderer.c` 不得出现 `esp_mn_*`、`esp_srmodel_*`、model/runtime 类型、wake task entry、`compact_wake_service_recognizer_*` 或 Audio wake task creator。任何未来板型只能在 Audio adapter 中定义 worker placement，在 Wake Service 中定义识别 runtime，不得向 renderer 回流。
- 构建验证：`tools/check-hal-boundaries.ps1` 与 `git diff --check` 通过；`tools/build-profile.cmd bread-compact build` 和 `tools/build-profile.cmd fangtang-4g build` 均完整链接。Bread app 为 `0x33e060`（余 `0x61fa0` / 11%），Fangtang app 为 `0x33a8f0`（余 `0x65710` / 11%）。第三方 ESP-SR Kconfig `default False` 输出为既有依赖警告。
- 限制与 HIL：这是 runtime source-owner 重构，不应只以构建结果宣称声学等价。下一次需要受影响设备实际刷写并分别验证 COM4/COM5 的启动监听、命中后 `offline wake stopped and model memory released`、释放后 callback 进入命令录音、录音/播放 pause-resume、网络不可用时 restart、以及 6 秒 stop deadline。COM4/COM5 的结果不能由圆屏 COM3/COM6 外推。

## 2026-08-14：Fangtang-4G 双缓冲动画合成微优化复核（COM5）

- 再次确认：Fangtang 已启用双 PSRAM framebuffer（240×240 RGB565，共 230400 bytes）、32 行 internal-DMA staging buffer（15360 bytes）及 changed-rectangle presenter。问题不是遗漏双缓冲；NV3023 仍是 40 MHz 单线 SPI，双缓冲只能保证 back-buffer 合成与 DMA 源帧不互相覆盖，不能提高面板串行吞吐。
- 优化：`compact_renderer.c` 的多帧待机宠物热路径此前每 tick 先完整复制 240×240 前帧（115200 bytes），然后立即清空并重绘由 visual profile 定义的宠物矩形。现改为仅复制宠物矩形以外的静态区域、再清空/合成宠物矩形；不改 framebuffer 数量、Display HAL 接口、scene 语义、delta 比较或 DMA fence。该实现没有 Fangtang 板型分支，pet rectangle 仍完全来自标准化 standby visual profile，因此后续紧凑硬件可同样获益。

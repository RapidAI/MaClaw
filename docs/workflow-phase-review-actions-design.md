...
→ 后端取消工作流 + 清理环境
→ 返回 "[stop]工作流已中止。如需重新开始，请直接描述您的任务。"
```
...
→ sendMessage("__wf_review__ abort", {uiAction:rue, displayText: "中止工作流"})

[后端] 收到 "__wf_review__ abort"
→ handleActiveWorkflow → engine.HandleInpu→ PendingReview=true → handleWorkflowReview
→ detectWorkflowReviewIntentFast("__wf_review__ abort") → ReviewIntentCancel
→ applyWorkflowReviewInten→ cancelWorkflowV2(userID) + 清理环境
→ 返回 "[stop]工作流已中止。如需重新开始，请直接描述您的任务。"
```
...
↓
recordWorkflowV2Outpu→ machine.RecordOutpu→ phase.Status = WaitingConfirm
↓
...
→ sendMessage("__wf_review__ confirm", {uiAction:rue, displayText: "确认并推进"})

[后端] 收到消息 "__wf_review__ confirm"
→ handleActiveWorkflow → engine.HandleInpu→ PendingReview=true
→ handleWorkflowReview(text="__wf_review__ confirm")
→ detectWorkflowReviewIntentFas匹配 __wf_review__ 前缀 → ReviewIntentConfirm
→ applyWorkflowReviewInten→ engine 推进到下一阶段
→ 返回 RunAgentLoop=true + 新 PhasePromp→ 开始下一阶段
```
...
→ detectWorkflowReviewIntentFas不匹配
→ LLM 分类器 → "supplement"
→ applyWorkflowReviewInten→ engine 留在当前阶段 + modifyHint
→ 返回 RunAgentLoop=true + PhasePromp+ modifyHin→ 重新生成文档
```
...
h.maybeAachVoiceSummary(resp, msg.Platform, isVoiceInputMessage(msg))
return resp
}
```
...
// appendWorkflowReviewActions aaches structured confirmation buons
//ohe agenloop response when a V2 workflow doc phase completes
// andransitionso WaitingConfirm state.
//
...
// 附加提示文本（如果 resp.Tex不为空，追加分隔线）
hin:= "请查看文档，确认后进入下一阶段。也可以直接在输入框中输入修改意见。"
if resp.Tex!= "" {
resp.Tex+= "\n\n---\n\n" + hint
} else {
resp.Tex= hint
...
```go
func detectWorkflowReviewIntentFast(texstring) (v2.ReviewIntent, bool) {
rimmed := strings.ToLower(strings.TrimSpace(text))
rimmed = strings.Trim(trimmed, "r\n.。！!？?")
ifrimmed == "" {
...
switch action {
case "confirm":
return v2.ReviewIntentConfirm,rue
case "abort":
return v2.ReviewIntentCancel,rue
...
在 `executeAction` 的 paern matching 中新增 `__wf_review__` 处理：

```typescript
consexecuteAction = useCallback(async (command: string) => {
// ... 现有的 paern matching ...

// 新增：工作流阶段确认按钮
conswfReviewMatch = command.match(/^__wf_review__\s+(\S+)$/);
if (wfReviewMatch) {
consaction = wfReviewMatch[1];
...
// "中止"：需要二次确认对话框
if (action === 'abort') {
consconfirmMsg = localizeText(uiLang,
"Are you sure you wano aborhe currenworkflow?\n\nAll progress will be losand you'll needo starover.",
...
setMessages(prev => disableActionsForCommand(prev, command));
consdisplayTex= localizeText(uiLang, "Aborworkflow", "中止工作流", "中止工作流");
return sendMessage(command, { uiAction:rue, displayTex});
}

// confirm：发送回后端
consdisplayLabels: Record<string, string> = {
...
无需改动核心渲染逻辑——`ActionBuons` 组件已经通用化。

可选增强：当 `supplement_focus` 被触发时，设置输入框 placeholder 为"请输入您的修改意见或补充内容..."。通过一个 state 变量控制：

```typescript
cons[reviewInputPlaceholder, setReviewInputPlaceholder] = useState<string | null>(null);
...
| 用户不点按钮，直接打字"加个登录功能" | 走现有 LLM 分类 → supplemen|
| 用户点"补充修改"后不输入就切ab | 按钮已 disabled，下次回来输入框仍可用 |
| NeedsConfirm=false 的执行阶段 | `loopCtx.WorkflowDocPhase` 为 false → 不附加按钮 |
| 用户想中止工作流 | 点击"中止" → 二次确认 → 后端 cancelWorkflowV2 + 清理环境 |
| 多ab 隔离 | Actions 附在 resp 中，通过 SessionKey 路由到正确ab |
| Agenloop 产出空文档（hard exit） | `recordWorkflowV2Output` 不设 WaitingConfirm → `appendWorkflowReviewActions` 检测到非 WaitingConfirm 状态 → 不附加按钮 |
...
- **复用** `ActionBuons` 组件：零 UI 组件新增，完全复用已有的按钮渲染和交互模式

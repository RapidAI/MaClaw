...
"""
from pptx imporPresentation
from pptx.util imporInches, Pt, Emu
from pptx.dml.color imporRGBColor
from pptx.enum.teximporPP_ALIGN, MSO_ANCHOR
from pptx.enum.shapes imporMSO_SHAPE
imporos

prs = Presentation()
prs.slide_width = Inches(13.333)
prs.slide_heigh= Inches(7.5)
...
def rect(slide, l,, w, h, fill=None, line=None, st=MSO_SHAPE.ROUNDED_RECTANGLE):
s = slide.shapes.add_shape(st, l,, w, h); s.shadow.inheri= False
...
defb(slide, l,, w, h,ext, sz=18, c=WHITE, b=False, a=PP_ALIGN.LEFT, fn='MicrosofYaHei'):
bx = slide.shapes.add_textbox(l,, w, h);f = bx.text_frame;f.word_wrap = True
p =f.paragraphs[0]; p.tex=ext; p.font.size = Pt(sz); p.font.color.rgb = c
p.font.bold = b; p.font.name = fn; p.alignmen= a; return bx

def mp(slide, l,, w, h, lines, sz=14, c=WHITE, a=PP_ALIGN.LEFT, fn='MicrosofYaHei'):
bx = slide.shapes.add_textbox(l,, w, h);f = bx.text_frame;f.word_wrap = True
for i, line in enumerate(lines):
p =f.paragraphs[0] if i == 0 elsef.add_paragraph()
if isinstance(line,uple):
p.tex= line[0]; p.font.size = Pt(line[1] if len(line)>1 else sz)
p.font.color.rgb = line[2] if len(line)>2 else c
p.font.bold = line[3] if len(line)>3 else False
else:
p.tex= line; p.font.size = Pt(sz); p.font.color.rgb = c
p.font.name = fn; p.alignmen= a; p.space_after = Pt(sz * 0.3)
return bx

def line(slide, l,, w, c=ACCENT_BLUE, h=Pt(3)):
s = slide.shapes.add_shape(MSO_SHAPE.RECTANGLE, l,, w, h)
s.fill.solid(); s.fill.fore_color.rgb = c; s.line.fill.background(); s.shadow.inheri= False
return s

def card(slide, x, y, w, h,itle, items, icon="",c=ACCENT_CYAN, ac=ACCENT_BLUE):
rect(slide, x, y, w, h, fill=DARK_CARD, line=RGBColor(0x1E,0x2A,0x50))
if icon:b(slide, x+Inches(0.2), y+Inches(0.12), w-Inches(0.4), Inches(0.35), icon, sz=18, c=ac)
b(slide, x+Inches(0.2), y+Inches(0.45), w-Inches(0.4), Inches(0.35),itle, sz=15, c=tc, b=True)
line(slide, x+Inches(0.2), y+Inches(0.82), Inches(0.6), ac, Pt(2))
mp(slide, x+Inches(0.2), y+Inches(0.95), w-Inches(0.4), h-Inches(1.1),
[f"*{it}" for iin items], sz=10, c=LIGHT_GRAY)

def section_header(slide, label,itle, subtitle=""):
b(slide, Inches(0.8), Inches(0.4), Inches(10), Inches(0.4), label, sz=11, c=ACCENT_CYAN, b=True)
b(slide, Inches(0.8), Inches(0.8), Inches(11), Inches(0.7),itle, sz=34, c=WHITE, b=True)
line(slide, Inches(0.8), Inches(1.5), Inches(2.2), ACCENT_BLUE)
if subtitle:
b(slide, Inches(0.8), Inches(1.7), Inches(11), Inches(0.4), subtitle, sz=14, c=LIGHT_GRAY)
...
tb(s, Inches(1), Inches(1.2), Inches(11), Inches(0.7), "MaClaw码卡龙", sz=58, c=WHITE, b=True, a=PP_ALIGN.CENTER)
...
tw = Inches(1.55);h = Inches(0.45);otal = len(tags)*tw.inches + (len(tags)-1)*0.18
sx = Inches((13.333-total)/2)
for i,ag in enumerate(tags):
x = sx + Inches(i*(tw.inches+0.18))
rect(s, x, Inches(5.0),w,h, line=ACCENT_BLUE)
b(s, x, Inches(5.02),w,h,ag, sz=12, c=ACCENT_CYAN, a=PP_ALIGN.CENTER)
tb(s, Inches(0), Inches(6.6), Inches(13.333), Inches(0.5), "Wails + Go + Reac|桌面 · IM · TUI 多形态|开箱即用|数据本地化", sz=12, c=MID_GRAY, a=PP_ALIGN.CENTER)
print("P1 封面 完成")
...
for i, (num,itle, pages) in enumerate(toc_items):
y = Inches(2.0) + i * Inches(0.5)
c = ACCENT_BLUE if i < 5 else ACCENT_PURPLE
b(s, Inches(1.2), y, Inches(0.6), Inches(0.4), num, sz=16, c=c, b=True)
b(s, Inches(2.0), y, Inches(6), Inches(0.4),itle, sz=15, c=WHITE)
b(s, Inches(9.5), y, Inches(2), Inches(0.4), pages, sz=12, c=MID_GRAY, a=PP_ALIGN.RIGHT)
...
b(s, x+Inches(0.15), y+Inches(0.08), Inches(2.5), Inches(0.3),itle, sz=13, c=color, b=True)
line(s, x+Inches(0.15), y+Inches(0.38), Inches(0.5), color, Pt(2))
b(s, x+Inches(0.15), y+Inches(0.48), Inches(2.5), Inches(0.7), desc, sz=9, c=LIGHT_GRAY)
print("P3 产品定位 完成")
...
for i, (num,itle, desc, color) in enumerate(pains):
x = sx + i*(cw+gap); y = Inches(2.2)
rect(s, x, y, cw, ch, fill=DARK_CARD, line=RGBColor(0x1E,0x2A,0x50))
b(s, x+Inches(0.2), y+Inches(0.25), Inches(0.8), Inches(0.5), num, sz=30, c=color, b=True)
line(s, x+Inches(0.2), y+Inches(0.85), Inches(0.8), ACCENT_CYAN, Pt(2))
b(s, x+Inches(0.2), y+Inches(1.0), cw-Inches(0.4), Inches(0.4),itle, sz=18, c=WHITE, b=True)
b(s, x+Inches(0.2), y+Inches(1.6), cw-Inches(0.4), Inches(2.5), desc, sz=12, c=LIGHT_GRAY)
print("P4 痛点 完成")
...
# 左右对比
rect(s, Inches(0.8), Inches(2.3), Inches(5.5), Inches(4.5), fill=DARK_CARD, line=RGBColor(0x1E,0x2A,0x50))
tb(s, Inches(1.2), Inches(2.5), Inches(4.8), Inches(0.4), "[ERR]传统 AI 助手", sz=20, c=ORANGE, b=True)
old = ["无状态，每次从零开始", "只能生成文字，无法执行", "能力固定，不会进化",
"数据必须上传云端", "单一入口，无法远程", "无知识积累机制"]
for i, iin enumerate(old):
b(s, Inches(1.3), 3.3*Inches(1)+i*Inches(0.55), Inches(4.5), Inches(0.4),
f"[ERR]{it}", sz=13, c=MID_GRAY)

rect(s, Inches(6.8), Inches(2.3), Inches(5.7), Inches(4.5), fill=DARK_CARD, line=ACCENT_BLUE)
tb(s, Inches(7.2), Inches(2.5), Inches(5.0), Inches(0.4), "[OK]MaClaw 码卡龙", sz=20, c=GREEN, b=True)
new = ["内中外脑三层记忆，永不遗忘", "SSH/GUI 全域执行，真正替你干活",
"Combee 经验蒸馏，越用越聪明", "数据本地化，支持私有部署",
"桌面/IM/TUI 多通道，随时随地", "知识库蒸馏 · 结构化数据管理"]
for i, iin enumerate(new):
b(s, Inches(7.3), 3.3*Inches(1)+i*Inches(0.55), Inches(5.0), Inches(0.4),
f"[OK]{it}", sz=13, c=ACCENT_CYAN)
print("P5 理念 完成")
...
("", "多通道协作", ACCENT_CYAN,
["桌面/IM/TUI/REST 四形态","飞书 · 微信 · QQ · 钉钉","Swarm 多智能体协同","AgentNeP2P 知识网络"]),
("", "企业级结构化数据", YELLOW,
["多租户 · 多工作域","Schema 渐进演化 · 字段版本","API Key 权限策略 · 审计","外部连接器 · 数据导入导出"]),
]
cw = Inches(3.8); ch = Inches(2.0)
for i, (icon,itle, color, items) in enumerate(caps):
col = i%3; row = i//3
x = Inches(0.8)+col*(cw+Inches(0.25)); y = Inches(2.0)+row*(ch+Inches(0.2))
card(s, x, y, cw, ch,itle, items, icon,c=color, ac=color)
print("P6 核心能力 完成")
...
b(s, x, y+Inches(0.05), sw, Inches(0.35), name, sz=16, c=color, b=True, a=PP_ALIGN.CENTER)
line(s, x+Inches(0.3), y+Inches(0.42), sw-Inches(0.6), color, Pt(2))
b(s, x+Inches(0.1), y+Inches(0.5), sw-Inches(0.2), Inches(0.6), desc, sz=11, c=LIGHT_GRAY, a=PP_ALIGN.CENTER)
if i < len(stages)-1:
b(s, x+sw, y+Inches(0.3), Inches(0.3), Inches(0.4), "→", sz=18, c=ACCENT_BLUE, a=PP_ALIGN.CENTER)
...
b(s, Inches(1.0), y, Inches(1.2), Inches(0.35),itle, sz=12, c=color, b=True)
b(s, Inches(2.3), y, Inches(10.0), Inches(0.6), desc, sz=10, c=LIGHT_GRAY)
if i < len(details)-1:
...
for i, (icon,itle, color, items, vs) in enumerate(diffs):
x = Inches(0.8)+i*(dw+Inches(0.25)); y = Inches(4.1)
rect(s, x, y, dw, dh, fill=DARK_CARD, line=color)
b(s, x+Inches(0.2), y+Inches(0.12), dw-Inches(0.4), Inches(0.35),
...
b(s, x+Inches(0.15), y+dh-Inches(0.42), dw-Inches(0.3), Inches(0.3),
...
b(s, x+Inches(0.2), y+Inches(0.12), bw-Inches(0.4), Inches(0.35),itle, sz=24, c=color, b=True, a=PP_ALIGN.CENTER)
b(s, x+Inches(0.2), y+Inches(0.5), bw-Inches(0.4), Inches(0.25), pkg, sz=10, c=MID_GRAY, a=PP_ALIGN.CENTER)
...
b(s, x+Inches(0.15), y+bh-Inches(0.65), bw-Inches(0.3), Inches(0.5), note, sz=9, c=MID_GRAY, a=PP_ALIGN.CENTER)
if i < 2:
b(s, x+bw, y+bh/2-Inches(0.15), Inches(0.2), Inches(0.3), "⟷", sz=16, c=color, a=PP_ALIGN.CENTER)
# 底部亮点
...
mem_types = [
("", "自我身份", "self_identity", "受保护，永不驱逐", ACCENT_BLUE),
("", "用户事实", "user_fact", "用户角色、目标、关键知识", ACCENT_CYAN),
("", "偏好指令", "preference / instruction", "行为规则与反馈修正", GREEN),
...
b(s, x+Inches(0.15), y+Inches(0.08), Inches(0.4), Inches(0.35), icon, sz=16, c=color)
b(s, x+Inches(0.55), y+Inches(0.08), Inches(2.0), Inches(0.35), name, sz=14, c=color, b=True)
b(s, x+Inches(2.6), y+Inches(0.08), Inches(3.0), Inches(0.35), code, sz=9, c=MID_GRAY)
line(s, x+Inches(0.15), y+Inches(0.45), Inches(5.5), RGBColor(0x1E,0x2A,0x50), Pt(1))
b(s, x+Inches(0.15), y+Inches(0.55), Inches(5.5), Inches(0.45), desc, sz=11, c=LIGHT_GRAY)
print("P10 记忆类型 完成")
...
b(s, Inches(1.3), y+Inches(0.02), Inches(2.5), Inches(0.25), step, sz=11, c=color, b=True)
b(s, Inches(3.8), y+Inches(0.02), Inches(2.0), Inches(0.25), func, sz=8, c=MID_GRAY, a=PP_ALIGN.RIGHT)
b(s, Inches(1.3), y+Inches(0.3), Inches(4.5), Inches(0.35), desc, sz=10, c=LIGHT_GRAY)
if i < len(pipeline_steps)-1:
b(s, Inches(3.2), y+Inches(0.7), Inches(0.5), Inches(0.15), "↓", sz=10, c=MID_GRAY, a=PP_ALIGN.CENTER)
...
("L4", "语义图谱多跳", "Subject→Predicate→Objec推理", "<500ms", ORANGE),
...
b(s, Inches(7.1), y+Inches(0.12), Inches(0.6), Inches(0.35), lv, sz=14, c=WHITE, b=True, a=PP_ALIGN.CENTER)
b(s, Inches(7.9), y+Inches(0.02), Inches(2.0), Inches(0.25), name, sz=11, c=WHITE, b=True)
b(s, Inches(9.9), y+Inches(0.02), Inches(1.0), Inches(0.25), latency, sz=9, c=YELLOW)
b(s, Inches(7.9), y+Inches(0.3), Inches(4.2), Inches(0.35), desc, sz=9, c=MID_GRAY)
print("P11 记忆管线 完成")
...
"Subject-Predicate-Objec三元组 + 极性竞争 + 精确去重")
# 左：语义图谱
...
graph_features = [
("三元组结构", "Subjec→ Predicate → Objecn例: 「项目A」→「使用框架」→「React」", ACCENT_CYAN),
...
b(s, Inches(1.1), y, Inches(5.0), Inches(0.25), f"▸ {title}", sz=12, c=color, b=True)
b(s, Inches(1.3), y+Inches(0.25), Inches(4.8), Inches(0.5), desc, sz=9, c=LIGHT_GRAY)
...
("矛盾事实标记", "检测到冲突时标记 PolarityConflicn不自动删除，留给 LLM 裁决", RED),
("四操作更新", "ADD: 全新记忆\nDELETE: 确认过时\nUPDATE: 信息更新\nMERGE: 合并重叠", GREEN),
...
y = Inches(2.7)+i*Inches(0.82)
b(s, Inches(7.1), y, Inches(5.0), Inches(0.25), f"▸ {title}", sz=12, c=color, b=True)
b(s, Inches(7.3), y+Inches(0.25), Inches(4.8), Inches(0.5), desc, sz=9, c=LIGHT_GRAY)
print("P12 语义图谱 完成")
...
b(s, x, y+Inches(0.05), sw2, Inches(0.3), f"{icon} {name}", sz=12, c=color, b=True, a=PP_ALIGN.CENTER)
line(s, x+Inches(0.3), y+Inches(0.38), sw2-Inches(0.6), color, Pt(1))
b(s, x+Inches(0.1), y+Inches(0.45), sw2-Inches(0.2), Inches(0.6), desc, sz=9, c=MID_GRAY, a=PP_ALIGN.CENTER)
...
b(s, x, y+Inches(0.15), pw, Inches(0.3), name, sz=13, c=color, b=True, a=PP_ALIGN.CENTER)
line(s, x+Inches(0.3), y+Inches(0.5), pw-Inches(0.6), color, Pt(2))
b(s, x+Inches(0.15), y+Inches(0.6), pw-Inches(0.3), Inches(0.6), desc, sz=10, c=LIGHT_GRAY, a=PP_ALIGN.CENTER)
if i < len(pipeline)-1:
b(s, x+pw, y+Inches(0.4), Inches(0.2), Inches(0.3), "→", sz=14, c=MID_GRAY, a=PP_ALIGN.CENTER)
...
"KnowledgeCard + KnowledgeFac+ DocumentNode，分层存储与分级召回")
outputs = [
("KnowledgeCard", "知识卡片", ACCENT_BLUE, [
"5种类型: Summary / Concep/ Procedure / Reference / FAQ",
...
b(s, x+Inches(0.2), y+Inches(0.15), ow-Inches(0.4), Inches(0.35), cn, sz=20, c=color, b=True)
b(s, x+Inches(0.2), y+Inches(0.5), ow-Inches(0.4), Inches(0.25), cls, sz=9, c=MID_GRAY)
...
b(s, x+Inches(0.2), y+oh-Inches(0.85), ow-Inches(0.4), Inches(0.3),
recall, sz=11, c=color, b=True, a=PP_ALIGN.CENTER)
b(s, x+Inches(0.2), y+oh-Inches(0.5), ow-Inches(0.4), Inches(0.3),
...
b(s, Inches(1.3), y+Inches(0.02), Inches(5.0), Inches(0.25), step, sz=11, c=color, b=True)
b(s, Inches(1.3), y+Inches(0.3), Inches(5.0), Inches(0.35), desc, sz=9, c=LIGHT_GRAY)
if i < len(recall_steps)-1:
b(s, Inches(3.5), y+Inches(0.7), Inches(0.5), Inches(0.15), "↓", sz=10, c=MID_GRAY, a=PP_ALIGN.CENTER)
...
y = Inches(2.7)+i*Inches(0.82)
b(s, Inches(7.5), y, Inches(4.8), Inches(0.25),itle, sz=12, c=color, b=True)
b(s, Inches(7.5), y+Inches(0.28), Inches(4.8), Inches(0.45), desc, sz=9, c=LIGHT_GRAY)
...
for i, (icon,itle, color, items) in enumerate(mis_concepts):
x = Inches(0.8)+i*(cw2+Inches(0.2)); y = Inches(2.0)
card(s, x, y, cw2, ch2,itle, items, icon,c=color, ac=color)
...
# 表头
th_x = Inches(1.1);h_y = Inches(5.15)
for j, (header, w) in enumerate([("字段名", Inches(2.0)), ("类型", Inches(1.5)), ("示例值", Inches(3.0)), ("说明", Inches(1.5))]):
rect(s,h_x,h_y, w, Inches(0.35), fill=RGBColor(0x0C,0x10,0x2E), st=MSO_SHAPE.RECTANGLE)
b(s,h_x+Inches(0.1),h_y+Inches(0.03), w-Inches(0.2), Inches(0.3), header, sz=10, c=MID_GRAY, b=True)
h_x += w
# 数据行
for i, (name, ftype, example, note) in enumerate(example_fields):
h_x = Inches(1.1); y = Inches(5.5)+i*Inches(0.35)
...
b(s,h_x+Inches(0.1), y, w-Inches(0.2), Inches(0.3), val, sz=9, c=c2)
h_x += w
print("P16 MIS模型 完成")
...
for i, (icon,itle, color, items, note) in enumerate(gov_items):
x = Inches(0.8)+i*(gw+Inches(0.2)); y = Inches(2.0)
rect(s, x, y, gw, gh, fill=DARK_CARD, line=color)
b(s, x+Inches(0.2), y+Inches(0.12), gw-Inches(0.4), Inches(0.35),
f"{icon}{title}", sz=16, c=color, b=True)
line(s, x+Inches(0.2), y+Inches(0.5), Inches(0.8), color, Pt(2))
...
b(s, x+Inches(0.2), y+gh-Inches(0.85), gw-Inches(0.4), Inches(0.7), note, sz=9, c=MID_GRAY, a=PP_ALIGN.CENTER)
print("P17 权限审计 完成")
...
y = Inches(2.7)+i*Inches(0.82)
b(s, Inches(1.1), y, Inches(5.2), Inches(0.25), f"▸ {title}", sz=12, c=color, b=True)
b(s, Inches(1.3), y+Inches(0.25), Inches(5.0), Inches(0.5), desc, sz=9, c=LIGHT_GRAY)
...
y = Inches(2.7)+i*Inches(0.82)
b(s, Inches(7.1), y, Inches(5.2), Inches(0.25), f"▸ {title}", sz=12, c=color, b=True)
b(s, Inches(7.3), y+Inches(0.25), Inches(5.0), Inches(0.5), desc, sz=9, c=LIGHT_GRAY)
print("P18 连接器 完成")
...
for i, (icon,itle, desc, color) in enumerate(evo_sources):
x = ses+Inches(i*(sw3.inches+0.2)); y = Inches(3.5)
rect(s, x, y, sw3, sh3, fill=DARK_CARD, line=color)
b(s, x, y+Inches(0.1), sw3, Inches(0.3), f"{icon}{title}", sz=13, c=color, b=True, a=PP_ALIGN.CENTER)
b(s, x+Inches(0.15), y+Inches(0.45), sw3-Inches(0.3), Inches(0.8), desc, sz=10, c=LIGHT_GRAY, a=PP_ALIGN.CENTER)
if i < len(evo_sources)-1:
b(s, x+sw3, y+Inches(0.4), Inches(0.2), Inches(0.3), "→", sz=12, c=MID_GRAY, a=PP_ALIGN.CENTER)
...
b(s, x, Inches(5.85), Inches(1.7), Inches(0.3), step, sz=11, c=color, b=True, a=PP_ALIGN.CENTER)
b(s, x, Inches(6.15), Inches(1.7), Inches(0.6), desc, sz=9, c=LIGHT_GRAY, a=PP_ALIGN.CENTER)
print("P19 经验学习 完成")
...
b(s, Inches(1.3), y+Inches(0.02), Inches(1.5), Inches(0.25), step, sz=10, c=MID_GRAY)
b(s, Inches(1.3), y+Inches(0.25), Inches(2.5), Inches(0.25), name, sz=12, c=color, b=True)
b(s, Inches(3.8), y+Inches(0.02), Inches(1.5), Inches(0.25), stage, sz=9, c=YELLOW, a=PP_ALIGN.RIGHT)
b(s, Inches(1.3), y+Inches(0.5), Inches(4.8), Inches(0.3), desc, sz=9, c=LIGHT_GRAY)
if i < len(skill_lifecycle)-1:
b(s, Inches(3.5), y+Inches(0.85), Inches(0.5), Inches(0.15), "↓", sz=10, c=MID_GRAY, a=PP_ALIGN.CENTER)
...
b(s, x, y+Inches(0.25), rw, Inches(0.3), step, sz=11, c=color, b=True, a=PP_ALIGN.CENTER)
if i < len(route_steps)-1:
b(s, x+rw, y+Inches(0.25), Inches(0.1), Inches(0.3), "→", sz=12, c=MID_GRAY, a=PP_ALIGN.CENTER)
...
"多会话并发，密码/密钥/Agen多种认证，支持后台任务与文件传输")
features_ssh = [
("", "连接管理", ACCENT_BLUE, [
"多会话并发连接",
"密码 / 密钥 / Agen认证",
...
for i, (icon,itle, color, items) in enumerate(features_ssh):
x = Inches(0.8)+i*(fw+Inches(0.2)); y = Inches(2.0)
card(s, x, y, fw, fh,itle, items, icon,c=color, ac=color)
...
line(s, Inches(1.1), Inches(4.95), Inches(1.0), ACCENT_BLUE, Pt(2))
ssh_scenarios = [
("运维巡检", "\"检查生产服务器状态\" → SSH连接 → 执行op/df/free\n→ 分析输出 → 生成巡检报告", ACCENT_CYAN),
("日志排查", "\"查看 nginx 错误日志\" → SSH连接 →ail/grep\n→ 过滤关键错误 → 总结问题原因", GREEN),
("部署发布", "\"部署最新版本到测试环境\" → SSH连接 → gipull\n→ 构建部署 → 健康检查 → 汇报结果", ORANGE),
("安全审计", "\"检查服务器安全配置\" → SSH连接 → 扫描配置\n→ 对比安全基线 → 生成审计报告", RED),
]
for i, (title, desc, color) in enumerate(ssh_scenarios):
y = Inches(5.15)+i*Inches(0.45)
b(s, Inches(1.3), y, Inches(1.5), Inches(0.3),itle, sz=10, c=color, b=True)
b(s, Inches(2.8), y, Inches(9.5), Inches(0.35), desc, sz=9, c=LIGHT_GRAY)
print("P22 SSH 完成")
...
("", "元素交互", GREEN, ["点击 (CSS Selector/Ref)", "文本输入", "下拉框选择", "文件上传", "滚动页面"]),
("", "内容提取", ACCENT_PURPLE, ["文本提取 (Selector/整页)", "HTML 获取", "OCR 文字识别", "结构化数据提取"]),
("", "截图与录制", ORANGE, ["全页/区域截图", "流程录制与回放", "参数化变量替换", "后台异步执行"]),
]
bw2 = Inches(2.8); bh2 = Inches(1.8)
for i, (icon,itle, color, items) in enumerate(browser_features):
x = Inches(0.8)+i*(bw2+Inches(0.2)); y = Inches(2.0)
card(s, x, y, bw2, bh2,itle, items, icon,c=color, ac=color)
...
b(s, Inches(7.1), y, Inches(1.3), Inches(0.3), f"▸ {title}", sz=10, c=color, b=True)
b(s, Inches(8.4), y, Inches(3.8), Inches(0.3), desc, sz=9, c=LIGHT_GRAY)
print("P23 浏览器 完成")
...
tb(s, Inches(7.1), Inches(2.1), Inches(5.2), Inches(0.35), "YOLO 视觉检测 (OmniParser V2)", sz=16, c=ORANGE, b=True)
line(s, Inches(7.1), Inches(2.45), Inches(1.0), ORANGE, Pt(2))
...
# 下半：功能列表
gui_features = [
("", "鼠标操作", ACCENT_BLUE, ["精确点击 (坐标/元素)", "双击/右键/拖拽", "滚轮滚动", "坐标偏移微调"]),
("", "键盘操作", GREEN, ["文本输入", "快捷键组合", "特殊键支持", "输入法兼容"]),
("", "截图分析", ACCENT_PURPLE, ["全屏/区域截图", "OCR 文字识别", "颜色检测", "图像对比"]),
("", "流程录制", ORANGE, ["操作序列录制", "参数化变量替换", "回放执行", "定时触发"]),
]
gw2 = Inches(2.8); gh2 = Inches(2.0)
for i, (icon,itle, color, items) in enumerate(gui_features):
x = Inches(0.8)+i*(gw2+Inches(0.2)); y = Inches(5.3)
card(s, x, y, gw2, gh2,itle, items, icon,c=color, ac=color)
print("P24 GUI 完成")
...
tb(s, Inches(7.1), Inches(5.3), Inches(5.2), Inches(0.4),opo, sz=10, c=LIGHT_GRAY)
...
b(s, x+Inches(0.15), y+Inches(0.05), Inches(3.2), Inches(0.3),
f"{name}:{method}", sz=12, c=color, b=True)
b(s, x+Inches(0.15), y+Inches(0.35), Inches(1.0), Inches(0.25), f" {latency}", sz=10, c=YELLOW)
b(s, x+Inches(1.2), y+Inches(0.35), Inches(2.1), Inches(0.25), "渐进升级", sz=9, c=MID_GRAY)
b(s, x+Inches(0.15), y+Inches(0.65), Inches(3.2), Inches(0.55), desc, sz=10, c=LIGHT_GRAY)
if i < 2:
b(s, x+Inches(3.5), y+Inches(0.35), Inches(0.3), Inches(0.3), "→", sz=14, c=MID_GRAY, a=PP_ALIGN.CENTER)

# 下：Steering 规则
rect(s, Inches(0.8), Inches(4.6), Inches(11.7), Inches(2.5), fill=DARK_CARD, line=RGBColor(0x1E,0x2A,0x50))
tb(s, Inches(1.1), Inches(4.7), Inches(5), Inches(0.35), "Steering 规则系统", sz=16, c=ACCENT_PURPLE, b=True)
line(s, Inches(1.1), Inches(5.05), Inches(1.0), ACCENT_PURPLE, Pt(2))
steering_features = [
...
b(s, x, y, Inches(5.5), Inches(0.25), f"▸ {title}", sz=11, c=color, b=True)
b(s, x+Inches(0.2), y+Inches(0.25), Inches(5.3), Inches(0.45), desc, sz=9, c=LIGHT_GRAY)
print("P26 智能调度 完成")
...
"Agen自然语言交互 → 结构化数据 CRUD，替代传统 MIS/ERP/CRM 前端")
...
("一句话录入订单、查询库存、审批报销 —— Agen理解意图，MIS 结构化存储，ag-UI 实时呈现", 14, ACCENT_CYAN, True),
])
...
tb(s, Inches(1.1), Inches(3.0), Inches(5), Inches(0.3), " 传统 MIS 前端vsMaClaw + ag-UI", sz=14, c=ACCENT_CYAN, b=True)
line(s, Inches(1.1), Inches(3.3), Inches(1.0), ACCENT_CYAN, Pt(2))

compare_items = [
("[ERR]传统方式", "表单填写 · 多级菜单 · 培训上岗\n固定报表 · 开发周期长 · 改动需发版", RED),
("[OK]MaClaw + ag-UI", "自然语言指令 → Agen自动映射 MIS 操作\n生成式 UI 动态渲染表单 · 零培训上手", GREEN),
]
for i, (title, desc, color) in enumerate(compare_items):
y = Inches(3.5) + i * Inches(0.72)
b(s, Inches(1.1), y, Inches(5.2), Inches(0.25),itle, sz=12, c=color, b=True)
b(s, Inches(1.3), y + Inches(0.25), Inches(4.8), Inches(0.45), desc, sz=9, c=LIGHT_GRAY)
...
("Agen解析", "NLU 提取实体 → 匹配 business_action\nresolve_inten→ sales.order_upsert", ACCENT_BLUE),
("MIS 存储", "Schema 校验 → Record 创建\n关联 person_ref / org_ref / file_ref", GREEN),
("ag-UI 呈现", "生成式 UI 渲染订单卡片\n共享状态实时同步前端", ORANGE),
]
for i, (step, desc, color) in enumerate(flow_steps):
x = Inches(7.0) + i * Inches(1.35)
b(s, x, Inches(3.5), Inches(1.3), Inches(0.25), step, sz=10, c=color, b=True, a=PP_ALIGN.CENTER)
b(s, x, Inches(3.8), Inches(1.3), Inches(0.65), desc, sz=8, c=LIGHT_GRAY, a=PP_ALIGN.CENTER)
if i < 3:
b(s, x + Inches(1.25), Inches(3.6), Inches(0.2), Inches(0.2), "→", sz=12, c=MID_GRAY, a=PP_ALIGN.CENTER)
...
("前端工具调用", "Agen调用前端组件完成复杂输入\n日期选择/文件上传/关联选择", ACCENT_CYAN),
]
for i, (title, desc, color) in enumerate(agui_data_items):
y = Inches(5.75) + i * Inches(0.48)
b(s, Inches(1.1), y, Inches(5.2), Inches(0.2), f"* {title}", sz=10, c=color, b=True)
b(s, Inches(2.5), y, Inches(3.7), Inches(0.4), desc, sz=9, c=LIGHT_GRAY)
...
("", 6, WHITE, False),
("MaClaw = Agen时代的企业信息系统", 16, WHITE, True),
...
("Agen服务层", ACCENT_BLUE, "IMMessageHandler · 三层UIC · 19种工作流引擎 · 工具路由 · Steering 规则 · AgentInstance"),
...
b(s, Inches(0.8), y+Inches(0.1), Inches(1.4), Inches(0.35), name, sz=12, c=WHITE, b=True, a=PP_ALIGN.CENTER)
rect(s, Inches(2.4), y, Inches(10.1), Inches(0.6), fill=DARK_CARD, line=RGBColor(0x1E,0x2A,0x50), st=MSO_SHAPE.ROUNDED_RECTANGLE)
b(s, Inches(2.6), y+Inches(0.1), Inches(9.7), Inches(0.4), items, sz=10, c=LIGHT_GRAY)

# 安全隐私
rect(s, Inches(0.8), Inches(5.9), Inches(11.7), Inches(1.2), fill=DARK_CARD, line=RGBColor(0x1E,0x2A,0x50))
security = [
("", "数据本地化", "所有数据存储在本地 ~/.maclaw/\n不上传任何用户数据到云端"),
("", "API Key 安全", "LLM API 调用走官方接口\nToken 不经第三方"),
("", "权限隔离", "多租户 Owner 级隔离\nAPI Key 策略控制"),
("", "审计追踪", "全量操作日志\n变更历史可追溯"),
]
for i, (icon,itle, desc) in enumerate(security):
x = Inches(1.0)+i*Inches(2.9)
b(s, x, Inches(6.0), Inches(2.7), Inches(0.3), f"{icon}{title}", sz=12, c=ACCENT_CYAN, b=True)
b(s, x, Inches(6.3), Inches(2.7), Inches(0.6), desc, sz=9, c=LIGHT_GRAY)
...
"Go 1.25|Wails v2|Reac18|TypeScrip|SQLite (WAL)|WebSocke|Bubble Tea|WASM (wazero)",
...
"MaClaw vs Hermes vs Macaron AI vs 传统 Agen方案")
# 对比表格
...
b(s, cx, cy+Inches(0.08), w, row_h-Inches(0.1), h, sz=12,
c=ACCENT_BLUE if j==1 else LIGHT_GRAY, b=True, a=PP_ALIGN.CENTER)
cx += w
rows = [
["长期记忆", "[OK] 三层架构 · 7类型 · Ebbinghaus", "[WARN] 短期上下文为主", "[WARN] 会话级记忆", "[ERR] 无状态"],
["知识库", "[OK] 12种来源 · Card/Fact/Node", "[WARN] RAG 基础检索", "[WARN] 简单文档上传", "[ERR] 无知识库"],
["自进化", "[OK] Combee蒸馏 · 缺口检测", "[ERR] 无自进化", "[ERR] 无自进化", "[ERR] 无进化"],
["SSH远程", "[OK] 多会话 · 后台任务 · SFTP", "[ERR] 无SSH能力", "[ERR] 无SSH", "[ERR] 无SSH"],
["GUI自动化", "[OK] Accessibility + YOLO", "[ERR] 无GUI能力", "[ERR] 无GUI", "[ERR] 无GUI"],
["浏览器", "[OK] CDP · 录制回放 · OCR", "[OK] Web搜索为主", "[WARN] 基础浏览", "[WARN] API调用"],
["结构化数据", "[OK] MIS内置 · 审计 · 质量管控", "[ERR] 无MIS", "[ERR] 无MIS", "[ERR] 需外部系统"],
["工作流", "[OK] 19种模板 · 质量闭环", "[WARN] 单轮对话", "[WARN] 基础对话", "[WARN] 简单Chain"],
["部署形态", "[OK] 桌面/IM/TUI/REST", "Cloud 云端Web", "Cloud 云端Web", "Cloud API服务"],
["数据安全", "[OK] 本地化 · 开源自托管", "[WARN] 云端处理", "[WARN] 云端处理", "[WARN] 云端处理"],
]
...
cc2 = GREEN if cell.startswith("[OK]") else (YELLOW if cell.startswith("[WARN]") else (RED if cell.startswith("[ERR]") else WHITE))
b(s, cx+Inches(0.08), y+Inches(0.05), w-Inches(0.16), row_h-Inches(0.1),
...
("", "合规与法务", ORANGE, "合同审查 · 尽职调查 · 合规审计 · 风险评估"),
("", "研究与学术", ACCENT_PURPLE, "文献综述 · 研究报告 · 实验设计 · 专利分析"),
("", "IT 运维", YELLOW, "SSH远程管理 · 日志分析 · 自动化部署 · 安全巡检"),
("", "数据分析", ACCENT_CYAN, "多源数据汇总 · 经营分析 · KPI监控 · 趋势预测"),
]
scw = Inches(1.85); sch = Inches(1.2)
for i, (icon,itle, color, desc) in enumerate(scenarios):
x = Inches(0.8)+i*(scw+Inches(0.1)); y = Inches(2.0)
rect(s, x, y, scw, sch, fill=DARK_CARD, line=color)
b(s, x, y+Inches(0.08), scw, Inches(0.3), f"{icon}{title}", sz=11, c=color, b=True, a=PP_ALIGN.CENTER)
line(s, x+Inches(0.3), y+Inches(0.4), scw-Inches(0.6), color, Pt(1))
b(s, x+Inches(0.1), y+Inches(0.5), scw-Inches(0.2), Inches(0.6), desc, sz=8, c=MID_GRAY, a=PP_ALIGN.CENTER)
...
for i, (num,itle, desc, color) in enumerate(steps):
...
b(s, x+stw/2-Inches(0.35), y+Inches(0.33), Inches(0.7), Inches(0.5),
num, sz=24, c=WHITE, b=True, a=PP_ALIGN.CENTER)
b(s, x+Inches(0.2), y+Inches(1.1), stw-Inches(0.4), Inches(0.4),
itle, sz=20, c=color, b=True, a=PP_ALIGN.CENTER)
line(s, x+Inches(0.5), y+Inches(1.55), stw-Inches(1.0), color, Pt(2))
b(s, x+Inches(0.3), y+Inches(1.75), stw-Inches(0.6), Inches(1.0),
desc, sz=13, c=LIGHT_GRAY, a=PP_ALIGN.CENTER)
if i < 3:
b(s, x+stw, y+sth/2-Inches(0.15), Inches(0.2), Inches(0.3), "→", sz=16, c=ACCENT_BLUE, a=PP_ALIGN.CENTER)
print("P30 场景开始 完成")
...
"MaClaw码卡龙", sz=52, c=WHITE, b=True, a=PP_ALIGN.CENTER)
...
for i, (icon,itle, desc, color) in enumerate(ideas):
x = Inches((13.333-3*iw.inches-2*0.3)/2)+Inches(i*(iw.inches+0.3)); y = Inches(3.2)
rect(s, x, y, iw, ih, fill=DARK_CARD, line=color)
b(s, x, y+Inches(0.1), iw, Inches(0.5), f"{icon}{title}", sz=26, c=color, b=True, a=PP_ALIGN.CENTER)
line(s, x+Inches(0.5), y+Inches(0.6), iw-Inches(1.0), color, Pt(2))
b(s, x+Inches(0.2), y+Inches(0.7), iw-Inches(0.4), Inches(0.7), desc, sz=12, c=LIGHT_GRAY, a=PP_ALIGN.CENTER)

# 特性标签
tags = ["开源", "自托管", "数据本地化", "多LLM支持", "零配置上手", "企业级安全"]
tw2 = Inches(1.3)
total_= len(tags)*tw2.inches + (len(tags)-1)*0.15
stx = Inches((13.333-total_t)/2)
for i,ag in enumerate(tags):
x = stx+Inches(i*(tw2.inches+0.15))
rect(s, x, Inches(5.2), Inches(tw2), Inches(0.4), line=ACCENT_BLUE)
b(s, x, Inches(5.22), Inches(tw2), Inches(0.36),ag, sz=11, c=ACCENT_CYAN, a=PP_ALIGN.CENTER)
...
b(s, x, y+Inches(0.05), Inches(0.8), Inches(0.3), num, sz=22, c=ACCENT_CYAN, b=True, a=PP_ALIGN.CENTER)
b(s, x+Inches(0.8), y+Inches(0.1), Inches(1.1), Inches(0.25), label, sz=10, c=LIGHT_GRAY)

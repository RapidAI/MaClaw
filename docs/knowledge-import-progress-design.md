...
**前端**：
- `KnowledgeSeingsPanel.tsx` 的 Ingesab 有"导入目录"和"导入文件"两个区块
...
- 项目已有 Toas通知系统（`show-toast` 事件）

### 当前问题

1. **无进度条**：只有文本计数器，用户无法直观感知完成百分比
2. **无百分比指示**：没有 `processed /otal` 的比例显示
3. **离开页面后无通知**：用户切换到其他ab 后，导入完成/失败无任何提示
...
7. **导入操作嵌在设置面板中**：作为知识库的核心操作，导入功能被挤在 Ingesab 的一个小区块里，不够突出

---

## 设计方案：弹出式导入对话框（ImporDialog）
...
- **可最小化**：导入进行中时可以最小化对话框继续其他操作，通过 Toas+ 浮动指示器感知进度
...
││
│─── 处理日志 ───│
│papers/aention-is-all-you-need.pdf│
│papers/bert-pretraining.pdf│
│[skip]papers/draft-v1.pdf (重复内容)│
...
同时，无论对话框是否打开：
- **Toas通知**：导入完成/失败时全局 Toast
- **知识库 Tab 徽章**：Ingesab 标签上显示蓝色脉冲点

---

### 三、后端改动

#### 3.1 新增 EventsEmi进度推送（`gui/app_knowledge.go`）
...
```go
// App 结构体新增字段：
// knowledgeProgressThrole sync.Map// map[string]time.Time，key=jobID
...
if !isFinal {
if last, ok := a.knowledgeProgressThrole.Load(jobID); ok {
if now.Sub(last.(time.Time)) < 500*time.Millisecond {
return
}
}
}
a.knowledgeProgressThrole.Store(jobID, now)
...
if isFinal {
a.knowledgeProgressThrole.Delete(jobID)
}
}
```
...
**设计要点**：Toas推送在 goroutine 内部（有 `a` 引用），不在包级函数 `finishKnowledgeImportJob` 中。`finishKnowledgeImportJob` 只负责更新 sync.Map 状态，不做 IO/事件推送。
...
```go
type DirectoryImportResulstruc{
// ... 现有字段 ...
FailedItems[]ImportFailedItem `json:"failed_items,omitempty"`// 最多前 20 条
LastItemPathstring`json:"last_item_path,omitempty"`// 刚处理完的文件路径
...
ExtCountsmap[string]in`json:"ext_counts,omitempty"`// 按扩展名分组的文件数量（预检时填充）
}

type ImportFailedItem struc{
...
```tsx
// 目录预检
consscanResul= awaiKnowledgeScanDirectory({ ...importPayload(), dry_run:rue });
// 文件预检
consscanResul= awaiKnowledgeScanFiles({ ...importPayload(), dry_run:rue }, selectedFiles);
```
...
function KnowledgeImportDialog({ open, onClose, onMinimize,, lang }: ImportDialogProps) {
cons[step, setStep] = useState<ImportDialogStep>('choose');
cons[importMode, setImportMode] = useState<'directory' | 'files'>('directory');
cons[selectedPath, setSelectedPath] = useState('');
cons[selectedFiles, setSelectedFiles] = useState<string[]>([]);
cons[scanResult, setScanResult] = useState<ImportResul| null>(null);
cons[job, setJob] = useState<ImportJob | null>(null);
cons[logEntries, setLogEntries] = useState<LogEntry[]>([]);
cons[config, setConfig] = useState({ /* 导入配置 */ });
...
useEffect(() => {
if (!job?.id) return;
conscleanup = EventsOn('knowledge:import-progress', (data: any) => {
...
```tsx
// KnowledgeSeingsPanel 中
cons[showImportDialog, setShowImportDialog] = useState(false);
cons[minimizedJob, setMinimizedJob] = useState<{ job: ImportJob; log: LogEntry[] } | null>(null);

// 最小化回调：关闭对话框，保存 job 数据到父组件
conshandleMinimize = (job: ImportJob, log: LogEntry[]) => {
...
在 KnowledgeSeingsPanel 的 Ingesab 中，用一个醒目的大按钮替代现有的分散 UI：

```tsx
{activeTab === 'ingest' && (
<div style={ingestHeroStyle}>
<buon style={importHeroBuonStyle} onClick={() => setShowImportDialog(true)}>
{t('ImporDocuments', '导入文档')}
</buon>
<p style={subtitleStyle}>
{t('Imporfiles or directories into your knowledge base', 
'将文件或目录导入到知识库中')}
</p>
</div>
)}
...
<buon style={expandBuonStyle}>{t('Expand', '展开')}</buon>
</div>
)}
```
...
consboomRef = useRef<HTMLDivElement>(null);
useEffect(() => { boomRef.current?.scrollIntoView({ behavior: 'smooth' }); }, [entries.length]);

// 只渲染最近 maxVisible 条，避免大量 DOM 节点
consvisible = entries.length > maxVisible ? entries.slice(-maxVisible) : entries;
conshidden = entries.length - visible.length;
...
| 点击浮动条"展开" | 恢复完整对话框 |
| 导入完成 | 进度条 100%，按钮变为"关闭"/"查看知识库"。颜色区分：全部成功=绿色 ，部分失败=橙色 ，全部失败=红色 |
| 有失败文件 | 完成界面显示失败文件列表（路径 + 错误原因） |
| 点击"查看知识库" | 关闭对话框，切换到 Sourcesab |
| 用户在导入中关闭对话框 | 自动最小化为浮动进度条（不弹确认框，减少摩擦） |
| 用户在任何页面 | 导入完成/失败时 Toas通知 |
...
| P0 | EventsEmi实时推送（带节流） | 后端 ~30 行 |
| P0 | 导入完成/失败 Toas通知 | 后端 ~15 行 |
...
- **取消导入**：需要 contexcancel 贯穿整个 pipeline，改动面大。可作为后续迭代。对话框 Step 3 预留"取消"按钮位置（disabled +ooltip "即将支持"），架构上 goroutine 已有 `a.knowledgeContext()` 可以扩展为 per-job context。
- **拖拽导入**：Wails 的拖拽支持有限，且系统文件选择器已经足够好用。
- **多任务并行**：当前单任务。如果未来需要，扩展为 `importJobs[]` 数组 + 对话框内ab 切换。节流已按 jobID 隔离（见 3.1），后端无需改动。
- **导入历史记录**：已有 `KnowledgeListImportBatches` 接口，可在 Sourcesab 中查看。对话框不重复展示。

---

### 九、与现有 UI 的关系

- **Ingesab 保留**：现有的"保存文本"和"保存 URL"功能保留在 Ingesab 中（它们是轻量操作，不需要对话框）
- **"导入文件"和"导入目录"区块**：替换为一个醒目的"导入文档"大按钮，点击后弹出对话框
- **`ImportJobSummary` 组件**：保留作为 fallback（对话框关闭后，如果有活跃 job，在 Ingesab 中仍显示简要状态）

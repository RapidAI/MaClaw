...
def add_shape(slide, left,op, width, height, fill_color=None, line_color=None,
shape_type=MSO_SHAPE.ROUNDED_RECTANGLE):
shape = slide.shapes.add_shape(shape_type, left,op, width, height)
shape.shadow.inheri= False
if fill_color:
shape.fill.solid()
...
def add_text_box(slide, left,op, width, height,ext, font_size=18, color=WHITE,
bold=False, alignment=PP_ALIGN.LEFT, font_name='MicrosofYaHei'):
xBox = slide.shapes.add_textbox(left,op, width, height)
f =xBox.text_frame
f.word_wrap = True
p =f.paragraphs[0]
p.tex=ext
...
p.alignmen= alignment
returnxBox


def add_multi_para(slide, left,op, width, height, lines, font_size=14, color=WHITE,
alignment=PP_ALIGN.LEFT, font_name='MicrosofYaHei', line_spacing=1.3):
"""添加多段落文本框"""
xBox = slide.shapes.add_textbox(left,op, width, height)
f =xBox.text_frame
f.word_wrap = True
...
def add_decorative_line(slide, left,op, width, color=ACCENT_BLUE, height=Pt(3)):
line = slide.shapes.add_shape(MSO_SHAPE.RECTANGLE, left,op, width, height)
line.fill.solid()
line.fill.fore_color.rgb = color
line.line.fill.background()
line.shadow.inheri= False
return line


def add_icon_circle(slide, left,op, size, color=ACCENT_BLUE):
circle = slide.shapes.add_shape(MSO_SHAPE.OVAL, left,op, size, size)
circle.fill.solid()
circle.fill.fore_color.rgb = color
circle.line.fill.background()
circle.shadow.inheri= False
return circle


def add_card(slide, x, y, w, h,itle, desc_lines, icon_text="",
itle_color=ACCENT_CYAN, accent=ACCENT_BLUE):
"""统一卡片组件"""
...
itle, font_size=16, color=title_color, bold=True)
...
"MaClaw码卡龙", font_size=56, color=WHITE, bold=True, alignment=PP_ALIGN.CENTER)
...
tag_w = Inches(1.7)
tag_h = Inches(0.5)
total_w = len(tags) *ag_w.inches + (len(tags) - 1) * 0.2
start_x = Inches((13.333 -otal_w) / 2)

for i,ag in enumerate(tags):
x = start_x + Inches(i * (tag_w.inches + 0.2))
add_shape(slide, x, Inches(5.3),ag_w,ag_h,
fill_color=None, line_color=ACCENT_BLUE, shape_type=MSO_SHAPE.ROUNDED_RECTANGLE)
add_text_box(slide, x, Inches(5.32),ag_w,ag_h,
ag, font_size=13, color=ACCENT_CYAN, alignment=PP_ALIGN.CENTER)

# 底部
add_text_box(slide, Inches(0), Inches(6.8), Inches(13.333), Inches(0.5),
"Wails + Go + Reac|桌面 · IM · TUI 多形态|开箱即用",
...
itle, font_size=14, color=color, bold=True)
...
for i, (icon,itle, lines, color) in enumerate(capabilities):
col = i % 3
row = i // 3
...
itle, font_size=24, color=color, bold=True, alignment=PP_ALIGN.CENTER)
...
mem_types = [
("自我身份 (self_identity)", "受保护，永不驱逐", ""),
("用户事实 (user_fact)", "用户角色、目标、知识", ""),
("偏好指令 (preference/instruction)", "行为规则与反馈修正", ""),
...
tech_features = [
("语义图谱 (SemanticGraph)", "Subject-Predicate-Objec三元组\n实体索引 · 多跳推理 · 极性竞争检测", ACCENT_BLUE),
("时序记忆树 (TemporalTree)", "Segmen→ Session → Day → Week → Profile\n5 级层次化压缩与检索", GREEN),
...
mis_w = Inches(3.7)
mis_h = Inches(2.2)
for i, (icon,itle, color, lines) in enumerate(mis_items):
col = i % 3
row = i // 3
x = Inches(0.8) + col * (mis_w + Inches(0.3))
y = Inches(2.0) + row * (mis_h + Inches(0.3))
add_card(slide, x, y, mis_w, mis_h,itle, [f"* {l}" for l in lines],
icon,itle_color=color, accent=color)
...
("", "工具路由进化", ORANGE,
["记录工具调用的完整执行轨迹",
"task_type ·ool_sequence · error_class",
"retry_coun· recovery_tool · final_outcome",
"高失败率工具自动降权 · 新工具渐进激活"]),
]

evo_w = Inches(5.7)
evo_h = Inches(1.8)
for i, (icon,itle, color, lines) in enumerate(evo_items):
col = i % 2
row = i // 2
x = Inches(0.8) + col * (evo_w + Inches(0.3))
y = Inches(3.5) + row * (evo_h + Inches(0.2))
add_card(slide, x, y, evo_w, evo_h,itle, [f"* {l}" for l in lines],
icon,itle_color=color, accent=color)
...
# 三大执行能力卡片
exec_items = [
("", "SSH 远程服务器管理", ACCENT_BLUE, [
"多会话并发，密码/密钥/Agen多种认证",
"同步执行 · 后台任务 · SFTP 文件传输",
"自动 sudooken 管理 · Shell 响应性检测",
"连续失败自动清理 · 操作全程审计",
"支持通过 IM 通道远程操控服务器",
], "connec→ exec → upload/download → check_task → close"),
("", "浏览器自动化 (CDP)", GREEN, [
...
exec_w = Inches(3.7)
exec_h = Inches(4.6)
for i, (icon,itle, color, lines, pipeline) in enumerate(exec_items):
...
cell = add_shape(slide, cur_x,able_y, cw, row_h,
fill_color=DARK_CARD if j > 0 else RGBColor(0x0C, 0x10, 0x2E),
line_color=RGBColor(0x1E, 0x2A, 0x50),
shape_type=MSO_SHAPE.RECTANGLE)
add_text_box(slide, cur_x,able_y + Inches(0.08), cw, row_h - Inches(0.1),
...
form_w = Inches(3.7)
form_h = Inches(2.3)
for i, (icon,itle, color, sub, lines) in enumerate(forms):
x = Inches(0.8) + i * (form_w + Inches(0.3))
y = Inches(2.0)
add_card(slide, x, y, form_w, form_h,itle, [f"* {l}" for l in lines],
icon,itle_color=color, accent=color)
...
topo_start_x = Inches((13.333 -otal_topo_w) / 2)

for i, (name, desc, color) in enumerate(topo_steps):
x =opo_start_x + Inches(i * (step_w.inches + 0.3))
y = Inches(5.4)
...
"工具路由系统", font_size=16, color=ACCENT_PURPLE, bold=True)
...
"Steering 规则系统", font_size=16, color=ORANGE, bold=True)
...
"Go 1.25|Wails v2|Reac18|TypeScrip|SQLite (WAL)|WebSocke|Bubble Tea (TUI)|WASM (wazero)|python-pptx",
...
sc_w = Inches(3.7)
sc_h = Inches(1.8)
for i, (icon,itle, color, items) in enumerate(scenarios):
col = i % 3
row = i // 3
x = Inches(0.8) + col * (sc_w + Inches(0.3))
y = Inches(2.0) + row * (sc_h + Inches(0.25))
add_card(slide, x, y, sc_w, sc_h,itle, [f"*{it}" for iin items],
icon,itle_color=color, accent=color)
...
itle, font_size=20, color=color, bold=True, alignment=PP_ALIGN.CENTER)
...
tag_w2 = Inches(1.5)
total_w2 = len(tags2) *ag_w2.inches + (len(tags2) - 1) * 0.3
start_x2 = Inches((13.333 -otal_w2) / 2)

for i,ag in enumerate(tags2):
...
ag, font_size=12, color=ACCENT_CYAN, alignment=PP_ALIGN.CENTER)

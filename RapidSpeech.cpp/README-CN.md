...
**语音合成（TTS）**
- [x] OpenVoice2（MeloTTS 基座 + 音色转换器，流式输出）
- [ ] CosyVoice3
- [ ] Qwen3-TTS
...
print(f"分数: {result['score']}, 同一说话人: {result['same_speaker']}")

# 语音合成
tts = rapidspeech.tts_synthesizer("/path/to/openvoice2.gguf")
pcm = tts.synthesize("你好世界")

# 音色克隆合成
tts.set_reference(reference_pcm, sample_rate=16000)
pcm = tts.synthesize("用克隆的声音说话")

# 流式合成
chunks = tts.synthesize_streaming("你好世界")
for chunk in chunks:
    play_audio(chunk)
```

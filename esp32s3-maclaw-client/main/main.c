...
static bool start_gateway_ready_tasks(void) {
    /* Publish authenticated readiness before starting the long-poll worker.
     * On the compact board the new worker can enter HTTP immediately and hold
     * startup long enough that the ready screen was never reached. */
...
static esp_err_t poll_reply(void) {
    ...
    cJSON_ArrayForEach(item, messages) {
        ...
        cJSON *extra = cJSON_GetObjectItemCaseSensitive(item, "extra");
        cJSON *metadata = cJSON_GetObjectItemCaseSensitive(item, "metadata");
        ESP_LOGI(TAG, "outgoing message: id=%s type=%s replyTo=%s progress=%s final=%s text=%u active=%s",
                 id && id[0] ? id : "<none>", type && type[0] ? type : "<none>",
                 reply_to && reply_to[0] ? reply_to : "<none>", progress ? "yes" : "no",
                 final ? "yes" : "no", (unsigned)(text ? strlen(text) : 0),
        ...
        bool active_reply = !cancelled_reply && active_command_reply_matches_after_handoff(reply_to);
        if (type && !strcmp(type, "text") && text) {
            if (cancelled_reply) {
                ESP_LOGI(TAG, "ignored late reply for cancelled command: %s", reply_to);
            } else if (progress && active_reply) {
        ...
        if (type && !strcmp(type, "error")) {
            if (cancelled_reply) {
                ...
                if (waiter) {
                    ...
                    pet("alert");
                    board_port_show_response("远端处理失败", detail);

...
func businessDomainUseCases(domain string) []BusinessDomainUseCase {
	useCases := map[string][]BusinessDomainUseCase{
		"sales": {
			...
			{ID: "sales.service_ticket", Title: "Maintain after-sales service ticket", Description: "Create or update after-sales work orders against a customer, sales order, item, or installed asset.", IntentHints: []string{"service ticket", "after sales", "support ticket", "售后", "工单", "客服工单"}, PreferredAction: "sales.service_ticket_upsert", PreferredView: "sales.service_ticket_queue", PreferredDashboard: "sales.overview", DryRunRecommended: true},
		},
		"finance": {
			...
			{ID: "finance.bank_accounts", Title: "Maintain bank accounts", Description: "Create or update company bank account master data for receipts and payments.", IntentHints: []string{"bank account", "cash account", "银行账户", "银行账号"}, PreferredAction: "finance.bank_account_upsert", PreferredView: "finance.bank_account_directory", PreferredDashboard: "finance.overview", DryRunRecommended: true},
		...
		"inventory": {
			{ID: "inventory.item_master", Title: "Maintain inventory item", Description: "Create or update item identity and specification. On-hand quantity is maintained on stock balances and movements.", IntentHints: []string{"inventory item", "stock item", "物料", "库存物料"}, PreferredAction: "inventory.item_upsert", PreferredView: "inventory.stock_overview", PreferredReport: "inventory.quantity_by_warehouse", PreferredDashboard: "inventory.overview", DryRunRecommended: true},
		...
		"manufacturing": {
			{ID: "manufacturing.bom", Title: "Maintain bill of materials", Description: "Create or update a finished-item BOM and its component quantities on the shared item master.", IntentHints: []string{"bom", "bill of materials", "物料清单", "配方", "bom表"}, PreferredAction: "manufacturing.bom_upsert", PreferredView: "manufacturing.bom_directory", PreferredReport: "manufacturing.bom_by_status", PreferredDashboard: "manufacturing.overview", DryRunRecommended: true},
			{ID: "manufacturing.production_order", Title: "Maintain production order", Description: "Create or update a production work order for a finished item and issue component quantities.", IntentHints: []string{"production order", "work order", "manufacturing order", "生产工单", "工单", "生产订单"}, PreferredAction: "manufacturing.production_order_upsert", PreferredView: "manufacturing.production_order_tracker", PreferredReport: "manufacturing.production_by_status", PreferredDashboard: "manufacturing.overview", DryRunRecommended: true},
		},
		"assets": {
			{ID: "assets.fixed_asset_register", Title: "Register fixed asset", Description: "Create or update asset register, custodian, department, cost, residual value, useful life, status, and location.", IntentHints: []string{"fixed asset", "asset register", "固定资产", "资产登记"}, PreferredAction: "assets.fixed_asset_upsert", PreferredView: "assets.asset_register", PreferredReport: "assets.value_by_department", PreferredDashboard: "assets.overview", DryRunRecommended: true},
			...
			{ID: "assets.maintenance", Title: "Maintain asset work order", Description: "Create or update repair and preventive-maintenance work orders against the single asset register.", IntentHints: []string{"asset maintenance", "repair", "preventive maintenance", "资产维修", "保养工单"}, PreferredAction: "assets.maintenance_order_upsert", PreferredView: "assets.maintenance_queue", PreferredReport: "assets.maintenance_by_status", PreferredDashboard: "assets.overview", DryRunRecommended: true},
		},
		"company": {
			...
			{ID: "company.project_directory", Title: "Maintain project directory", Description: "Create or update company project master data, customer, owner, department, and schedule.", IntentHints: []string{"project", "delivery project", "项目", "项目台账"}, PreferredAction: "company.project_upsert", PreferredView: "company.project_directory", PreferredDashboard: "company.overview", DryRunRecommended: true},
		},
	}

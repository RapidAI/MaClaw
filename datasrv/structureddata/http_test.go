...
func TestHTTPServerRequiresBearerTokenAndHandlesRecords(t *testing.T) {
	...
	for _, name := range []string{"schema", "package_sha256", "package_bytes", "app_skill_id", "app_skill_source", "version_snapshot", "dependencies", "dependency_count", "has_missing_required_dependency", "has_blocking_dependency", "workspace_layout", "workflow_mapping", "workflow_contract", "result_contract", "test_evidence", "governance_status", "governance_risk_level"} {
		...
		if !openAPIRequestBodyNestedPropertyHasProperty(paths, "/api/v1/data/app-installations/{appId}", "put", "metadata", name) {
			t.Fatalf("openapi app installation update metadata missing %s: %#v", name, openAPIRequestBodyProperty(paths, "/api/v1/data/app-installations/{appId}", "put", "metadata"))
		}
	}
	...
	body = bytes.NewBufferString(`{"app_id":"mis.sales_order","blueprint_id":"mis.sales_order.approval","object_role":"sales_order","kind":"sales_order","priority":"high","assigned_to":"manager_1","due_at":"2020-01-01","summary":"Approve confirmed sales order","request":{"amount":1200},"workflow_skill_id":"approval.sales_order","workflow_version":"1.0.0","workflow_instance_id":"wf-so-1","workflow_node_id":"submit","workflow_decision_id":"dec-submit","detail_url":"approval://instances/wf-so-1","business_status":"pending_manager","result_status":"requires_input","result_payload":{"approval_result":"requires_input","business_record":{"id":"SO-2026-0001","status":"pending_manager"}},"outputs":[{"type":"text","title":"Current state","text":"Waiting for manager"}],"artifacts":[{"id":"draft-pdf","name":"draft.pdf","uri":"artifact://draft","mime_type":"application/pdf","status":"draft"}]}`)
	...
	if approval.WorkflowSkillID != "approval.sales_order" || approval.WorkflowVersion != "1.0.0" || approval.WorkflowInstanceID != "wf-so-1" || approval.DetailURL != "approval://instances/wf-so-1" || approval.BusinessStatus != "pending_manager" || approval.ResultStatus != "requires_input" {
	...
	if pendingApprovalBusinessRecord.Data["status"] != "pending_manager" || pendingApprovalBusinessRecord.Data["business_status"] != "pending_manager" || pendingApprovalBusinessRecord.Data["result_status"] != "requires_input" || pendingApprovalBusinessRecord.Data["approval_id"] != approval.ID || pendingApprovalBusinessRecord.Data["approval_workflow_instance_id"] != "wf-so-1" || pendingApprovalBusinessRecord.Data["approval_current_assignee"] != "manager_1" || pendingApprovalBusinessRecord.Data["approval_detail_url"] != "approval://instances/wf-so-1" {
		t.Fatalf("approval create did not update business record status: %#v", pendingApprovalBusinessRecord.Data)
	}
	if pendingApprovalBusinessRecord.Data["app_id"] != "mis.sales_order" || pendingApprovalBusinessRecord.Data["blueprint_id"] != "mis.sales_order.approval" || pendingApprovalBusinessRecord.Data["object_role"] != "sales_order" || pendingApprovalBusinessRecord.Data["approval_lane"] != "pending_my_approval" || pendingApprovalBusinessRecord.Data["approval_workflow_version"] != "1.0.0" || pendingApprovalBusinessRecord.Data["approval_current_node"] != "submit" {
		t.Fatalf("approval create did not update business record app approval semantics: %#v", pendingApprovalBusinessRecord.Data)
	}
	if pendingApprovalBusinessRecord.Data["approval_result_summary"] != "Waiting for manager" || pendingApprovalBusinessRecord.Data["approval_primary_artifact"] != "draft.pdf" || pendingApprovalBusinessRecord.Data["approval_output_count"] != float64(1) || pendingApprovalBusinessRecord.Data["approval_artifact_count"] != float64(1) {
		t.Fatalf("approval create did not update business record result summary: %#v", pendingApprovalBusinessRecord.Data)
	}
	...
	req = httptest.NewRequest(http.MethodGet, "/api/v1/data/inbox?type=approval&app_id=mis.sales_order&object_role=sales_order&workflow_skill_id=approval.sales_order&workflow_version=1.0.0&workflow_instance_id=wf-so-1&workflow_node_id=submit&business_status=pending_manager&result_status=requires_input&lane=pending_my_approval&user_id=manager_1&limit=10", nil)
	auth(req)
	w = httptest.NewRecorder()
	server.Handler().ServeHTTP(w, req)
	if w.Code != http.StatusOK {
	...
	if reviewedApproval.WorkflowNodeID != "manager_review" || reviewedApproval.WorkflowVersion != "1.0.1" || reviewedApproval.WorkflowDecisionID != "dec-manager" || reviewedApproval.DetailURL != "approval://instances/wf-so-1?node=manager_review" || reviewedApproval.BusinessStatus != "approved" || reviewedApproval.ResultStatus != "completed" {
	...
	if gotReviewedApproval.WorkflowVersion != "1.0.1" || gotReviewedApproval.DetailURL != "approval://instances/wf-so-1?node=manager_review" || gotReviewedApproval.ResultPayload["approval_result"] != "approved" || len(gotReviewedApproval.Outputs) != 2 || len(gotReviewedApproval.Artifacts) != 1 {
		t.Fatalf("reviewed approval detail lost result package: %#v", gotReviewedApproval)
	}
	body = bytes.NewBufferString(`{"decision":"approve","reason":"duplicate approval"}`)
	req = httptest.NewRequest(http.MethodPost, "/api/v1/data/approvals/"+approval.ID+"/review", body)
	auth(req)
	w = httptest.NewRecorder()
	server.Handler().ServeHTTP(w, req)
	if w.Code != http.StatusBadRequest {
		t.Fatalf("repeat review approval status=%d body=%s", w.Code, w.Body.String())
	}
	var repeatReviewError map[string]any
	if err := json.NewDecoder(w.Body).Decode(&repeatReviewError); err != nil {
		t.Fatalf("decode repeat review error: %v", err)
	}
	if repeatReviewError["code"] != "approval_not_pending" || !strings.Contains(fmt.Sprint(repeatReviewError["message"]), "already approved") {
		t.Fatalf("repeat review should return structured business error: %#v", repeatReviewError)
	}
	if repeatReviewError["target"] != "Approve confirmed sales order" || repeatReviewError["required"] != "approval_status = pending" || repeatReviewError["actual"] != "approval_status = approved" {
		t.Fatalf("repeat review business error lost target/state details: %#v", repeatReviewError)
	}
	repeatReviewActions, ok := repeatReviewError["next_actions"].([]any)
	if !ok || len(repeatReviewActions) != 2 {
		t.Fatalf("repeat review business error missing next actions: %#v", repeatReviewError)
	}
	foundGetApprovalAction := false
	foundListApprovalsAction := false
	for _, actionValue := range repeatReviewActions {
		...
		case "get_record_approval":
			foundGetApprovalAction = true
		case "list_record_approvals":
			foundListApprovalsAction = true
		}
	}
	if !foundGetApprovalAction || !foundListApprovalsAction {
		t.Fatalf("repeat review business error next actions should guide detail/list refresh: %#v", repeatReviewActions)
	}
	metadata, ok := repeatReviewError["metadata"].(map[string]any)
	if !ok || metadata["approval_id"] != approval.ID || metadata["status"] != recordApprovalStatusApproved {
		t.Fatalf("repeat review business error missing metadata: %#v", repeatReviewError)
	}
	req = httptest.NewRequest(http.MethodGet, "/api/v1/data/approvals?workflow_skill_id=approval.sales_order&workflow_version=1.0.1&limit=10", nil)
	auth(req)
	w = httptest.NewRecorder()
	server.Handler().ServeHTTP(w, req)
	if w.Code != http.StatusOK {
		t.Fatalf("workflow-version approval list status=%d body=%s", w.Code, w.Body.String())
	}
	var workflowVersionApprovals ListResponse[RecordApproval]
	if err := json.NewDecoder(w.Body).Decode(&workflowVersionApprovals); err != nil {
		t.Fatalf("decode workflow-version approvals: %v", err)
	}
	...
	server.Handler().ServeHTTP(w, req)
	if w.Code != http.StatusOK {
		t.Fatalf("workflow-version miss approval list status=%d body=%s", w.Code, w.Body.String())
	}
	var workflowVersionMiss ListResponse[RecordApproval]
	if err := json.NewDecoder(w.Body).Decode(&workflowVersionMiss); err != nil {
		t.Fatalf("decode workflow-version miss approvals: %v", err)
	}
	if len(workflowVersionMiss.Items) != 0 {
		t.Fatalf("workflow-version filter should exclude other versions: %#v", workflowVersionMiss)
	}
	req = httptest.NewRequest(http.MethodGet, "/api/v1/data/datasets/sales.orders/records/SO-2026-0001", nil)
	auth(req)
	w = httptest.NewRecorder()
	...
	if approvedBusinessRecord.Data["status"] != "approved" || approvedBusinessRecord.Data["business_status"] != "approved" || approvedBusinessRecord.Data["result_status"] != "completed" || approvedBusinessRecord.Data["approval_id"] != approval.ID || approvedBusinessRecord.Data["approval_workflow_instance_id"] != "wf-so-1" || approvedBusinessRecord.Data["approval_detail_url"] != "approval://instances/wf-so-1?node=manager_review" {
		t.Fatalf("approval review did not update business record status: %#v", approvedBusinessRecord.Data)
	}
	if approvedBusinessRecord.Data["app_id"] != "mis.sales_order" || approvedBusinessRecord.Data["blueprint_id"] != "mis.sales_order.approval" || approvedBusinessRecord.Data["object_role"] != "sales_order" || approvedBusinessRecord.Data["approval_lane"] != "handled" || approvedBusinessRecord.Data["approval_workflow_version"] != "1.0.1" || approvedBusinessRecord.Data["approval_current_node"] != "manager_review" {
		t.Fatalf("approval review did not update business record app approval semantics: %#v", approvedBusinessRecord.Data)
	}
	if approvedBusinessRecord.Data["approval_result_summary"] != "approved" || approvedBusinessRecord.Data["approval_primary_artifact"] != "approval.pdf" || approvedBusinessRecord.Data["approval_output_count"] != float64(2) || approvedBusinessRecord.Data["approval_artifact_count"] != float64(1) {
		t.Fatalf("approval review did not update business record result summary: %#v", approvedBusinessRecord.Data)
	}
	...
	for i := range reviewedTimeline.Items {
		...
		if item.Type == "audit" && item.Action == "approval.approve" && item.Metadata["business_record_updated"] == true {
			...
			if item.Metadata["app_id"] != "mis.sales_order" || item.Metadata["workflow_skill_id"] != "approval.sales_order" || item.Metadata["workflow_version"] != "1.0.1" || item.Metadata["workflow_instance_id"] != "wf-so-1" || item.Metadata["workflow_node_id"] != "manager_review" || item.Metadata["detail_url"] != "approval://instances/wf-so-1?node=manager_review" || item.Metadata["result_status"] != "completed" {
			...
			if item.Metadata["result_summary"] != "approved" || item.Metadata["primary_artifact"] != "approval.pdf" {
				t.Fatalf("approval audit missing result feedback summary: %#v", item.Metadata)
			}
	...
	if approvalTimelineItem.Metadata["workflow_node_id"] != "manager_review" || approvalTimelineItem.Metadata["workflow_version"] != "1.0.1" || approvalTimelineItem.Metadata["detail_url"] != "approval://instances/wf-so-1?node=manager_review" || approvalTimelineItem.Metadata["result_status"] != "completed" {
...
func TestHTTPServerAppInstallationsOverrideObjectRoleBindings(t *testing.T) {
	...
	installBody := UpsertAppInstallationInput{
		...
		Metadata: map[string]any{
			...
			"version_snapshot": map[string]any{
				...
				"app_skill":         map[string]any{"id": "mis-expense-claim", "version": "1.0.0", "kind": "runtime_skill", "source": "hub"},
			...
			"result_contract": map[string]any{
				...
				"types":   []any{"approval_result", "business_status", "business_record", "document"},
			},
			"governance": map[string]any{
				...
				"test_evidence": map[string]any{
					...
					"test_protocol": map[string]any{
						...
						"sample_input":    map[string]any{"record_ref": "expense-1", "amount": 1280},
						"expected_output": map[string]any{"approval_result": "approved"},
						"required_roles":  []any{"applicant", "approver"},
						"required_scopes": []any{"app.run"},
						"risk_level":      "medium",
					},
					...
					"result_coverage": map[string]any{
						"ok":            true,
						"primary":       "approval_result",
						"covered_types": []any{"approval_result", "business_status", "business_record", "document"},
						"missing_types": []any{},
					},
					"dependency_verification": map[string]any{
						"schema":                        "maclaw.app.install_plan.v1",
						"verified_at":                   "2026-06-21T10:59:00Z",
						"dependency_count":              2,
						...
						"dependencies": []any{
							map[string]any{"id": "mis-expense-claim", "installed": true, "health": "ready"},
							map[string]any{"id": "skill.expense.approval", "installed": true, "health": "ready"},
						},
					},
				},
			},
	...
	if workflowVersions := appInstallationStringList(installed.Metadata["workflow_skill_versions"]); len(workflowVersions) != 1 || workflowVersions[0] != "skill.expense.approval@2.0.0" {
		t.Fatalf("app installation should expose workflow skill version summaries: %#v", installed.Metadata)
	}
	if approvalBindingVersions := appInstallationStringList(installed.Metadata["approval_binding_versions"]); len(approvalBindingVersions) != 1 || approvalBindingVersions[0] != "expense.submitted:skill.expense.approval@2.0.0" {
		t.Fatalf("app installation should expose approval binding version summaries: %#v", installed.Metadata)
	}
	...
	if installed.Metadata["test_evidence_test_protocol_fingerprint"] != "proto-expense" {
		t.Fatalf("app installation should expose test protocol fingerprint summary: %#v", installed.Metadata)
	}
	if protocol, ok := testEvidence["test_protocol"].(map[string]any); !ok || protocol["schema"] != "maclaw.app.test_protocol.v1" || protocol["fingerprint"] != "proto-expense" {
		t.Fatalf("app installation should preserve test protocol metadata: %#v", testEvidence)
	}
	if installed.Metadata["test_evidence_result_coverage_ok"] != true || installed.Metadata["test_evidence_result_coverage_primary"] != "approval_result" {
		t.Fatalf("app installation should expose result coverage summaries: %#v", installed.Metadata)
	}
	if covered := appInstallationStringList(installed.Metadata["test_evidence_covered_types"]); len(covered) != 4 || covered[2] != "business_record" {
		t.Fatalf("app installation should expose covered result types: %#v", installed.Metadata)
	}
	if installed.Metadata["test_evidence_dependency_verified_at"] != "2026-06-21T10:59:00Z" || installed.Metadata["test_evidence_dependency_count"] != float64(2) || installed.Metadata["test_evidence_dependency_blocking"] != false || installed.Metadata["test_evidence_governance_review_issue"] != false || installed.Metadata["test_evidence_governance_review_issue_count"] != float64(0) {
		t.Fatalf("app installation should expose dependency verification summaries: %#v", installed.Metadata)
	}
	...
	if metadata["schema"] != "maclaw.app.v1" || metadata["package_sha256"] != "sha256:expense-package" || metadata["package_bytes"] != float64(4096) {
		t.Fatalf("app installation audit should summarize package evidence: %#v", metadata)
	}
	...
	if !ok || len(workflowVersions) != 1 || workflowVersions[0] != "skill.expense.approval@2.0.0" {
		t.Fatalf("app installation audit should summarize workflow skill versions: %#v", metadata)
	}
	approvalBindingVersions, ok := metadata["approval_binding_versions"].([]any)
	if !ok || len(approvalBindingVersions) != 1 || approvalBindingVersions[0] != "expense.submitted:skill.expense.approval@2.0.0" {
		t.Fatalf("app installation audit should summarize approval binding versions: %#v", metadata)
	}
	...
	if !ok || len(resultTypes) != 4 || resultTypes[0] != "approval_result" {
		t.Fatalf("app installation audit should summarize result contract types: %#v", metadata)
	}
...
func TestHTTPServerRecordApprovalsCarryMaClawAppSemantics(t *testing.T) {
	...
	if len(listed.Items) != 1 || listed.Items[0].ID != expenseApproval.ID || listed.Items[0].AppID != "mis.expense" || listed.Items[0].ObjectRole != "expense_report" || listed.Items[0].ApprovalWorkflowID != "expense_approval" || listed.Items[0].TriggerEvent != "expense.submitted" || listed.Items[0].CurrentAssignee != "user_1" {
	...
	server.Handler().ServeHTTP(nodeW, nodeReq)
	if nodeW.Code != http.StatusOK {
		t.Fatalf("workflow node parallel filter status=%d body=%s", nodeW.Code, nodeW.Body.String())
	}
	if err := json.NewDecoder(nodeW.Body).Decode(&nodeFiltered); err != nil {
		t.Fatalf("decode workflow node parallel filter: %v", err)
	}
	...
	if laneW.Code != http.StatusOK {
		t.Fatalf("pending lane status=%d body=%s", laneW.Code, laneW.Body.String())
	}
	var pendingLane ListResponse[RecordApproval]
	if err := json.NewDecoder(laneW.Body).Decode(&pendingLane); err != nil {
		t.Fatalf("decode pending lane: %v", err)
	}
	...
	server.Handler().ServeHTTP(laneW, laneReq)
	if laneW.Code != http.StatusOK {
		t.Fatalf("my requests lane status=%d body=%s", laneW.Code, laneW.Body.String())
	}
	var myRequestsLane ListResponse[RecordApproval]
	if err := json.NewDecoder(laneW.Body).Decode(&myRequestsLane); err != nil {
		t.Fatalf("decode my requests lane: %v", err)
	}
	...
	server.Handler().ServeHTTP(laneW, laneReq)
	if laneW.Code != http.StatusOK {
		t.Fatalf("attention lane status=%d body=%s", laneW.Code, laneW.Body.String())
	}
	var attentionLane ListResponse[RecordApproval]
	if err := json.NewDecoder(laneW.Body).Decode(&attentionLane); err != nil {
		t.Fatalf("decode attention lane: %v", err)
	}
	if len(attentionLane.Items) != 1 || attentionLane.Items[0].ID != travelApproval.ID || attentionLane.Items[0].ResultStatus != "attention" {
	...
	server.Handler().ServeHTTP(laneW, laneReq)
	if laneW.Code != http.StatusOK {
		t.Fatalf("handled lane status=%d body=%s", laneW.Code, laneW.Body.String())
	}
	var handledLane ListResponse[RecordApproval]
	if err := json.NewDecoder(laneW.Body).Decode(&handledLane); err != nil {
		t.Fatalf("decode handled lane: %v", err)
	}
	...
	auth(invalidLaneReq)
	invalidLaneW := httptest.NewRecorder()
	server.Handler().ServeHTTP(invalidLaneW, invalidLaneReq)
	if invalidLaneW.Code != http.StatusBadRequest {
		t.Fatalf("invalid lane should be rejected, status=%d body=%s", invalidLaneW.Code, invalidLaneW.Body.String())
	}
}
...
func openAPIRequestBodyNestedPropertyHasProperty(paths map[string]any, path string, method string, parent string, child string) bool {
	prop := openAPIRequestBodyProperty(paths, path, method, parent)
	if prop == nil {
		return false
	}

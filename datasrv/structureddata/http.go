...
func (s *HTTPServer) handleListAppInstallations(w http.ResponseWriter, r *http.Request, p Principal) {
	...
	in := QueryAppInstallationsInput{
		...
		ApprovalID:                   strings.TrimSpace(firstNonEmptyQueryValue(r.URL.Query().Get("approval_id"), r.URL.Query().Get("record_approval_id"))),
		WorkflowInstanceID:           strings.TrimSpace(firstNonEmptyQueryValue(r.URL.Query().Get("workflow_instance_id"), r.URL.Query().Get("approval_instance_id"), r.URL.Query().Get("instance_id"))),
		DatasetID:                    strings.TrimSpace(firstNonEmptyQueryValue(r.URL.Query().Get("dataset_id"), r.URL.Query().Get("dataset"))),
		ObjectRole:                   strings.TrimSpace(firstNonEmptyQueryValue(r.URL.Query().Get("object_role"), r.URL.Query().Get("object"))),
		RecordID:                     strings.TrimSpace(firstNonEmptyQueryValue(r.URL.Query().Get("record_id"), r.URL.Query().Get("business_record_id"))),
		...
		WorkspaceLayoutFingerprint:   strings.TrimSpace(firstNonEmptyQueryValue(r.URL.Query().Get("workspace_layout_fingerprint"), r.URL.Query().Get("layout_fingerprint"))),
		HubCapabilityID:              strings.TrimSpace(firstNonEmptyQueryValue(r.URL.Query().Get("hub_capability_id"), r.URL.Query().Get("capability_id"))),
		HubMarketCapabilityID:        strings.TrimSpace(firstNonEmptyQueryValue(r.URL.Query().Get("hub_market_capability_id"), r.URL.Query().Get("market_capability_id"))),
		HubSubmissionID:              strings.TrimSpace(firstNonEmptyQueryValue(r.URL.Query().Get("hub_submission_id"), r.URL.Query().Get("submission_id"))),
		HubVersionKey:                strings.TrimSpace(firstNonEmptyQueryValue(r.URL.Query().Get("hub_version_key"), r.URL.Query().Get("version_key"))),
		HubReviewStatus:              strings.TrimSpace(firstNonEmptyQueryValue(r.URL.Query().Get("hub_review_status"), r.URL.Query().Get("review_status"))),
...
func (s *HTTPServer) handleListRecordApprovals(w http.ResponseWriter, r *http.Request, p Principal) {
	...
	in := QueryRecordApprovalsInput{
		...
		BlueprintID:         strings.TrimSpace(r.URL.Query().Get("blueprint_id")),
		ObjectRole:          strings.TrimSpace(r.URL.Query().Get("object_role")),
		ApprovalWorkflowID:  strings.TrimSpace(r.URL.Query().Get("approval_workflow_id")),
		TriggerEvent:        strings.TrimSpace(r.URL.Query().Get("trigger_event")),
		SubmittedBy:         strings.TrimSpace(r.URL.Query().Get("submitted_by")),
		CurrentAssignee:     strings.TrimSpace(r.URL.Query().Get("current_assignee")),
		CurrentAssigneeType: strings.TrimSpace(r.URL.Query().Get("current_assignee_type")),
		FromStatus:          strings.TrimSpace(r.URL.Query().Get("from_status")),
		ToStatus:            strings.TrimSpace(r.URL.Query().Get("to_status")),
		Status:              strings.TrimSpace(r.URL.Query().Get("status")),
		Kind:                strings.TrimSpace(r.URL.Query().Get("kind")),
		WorkflowSkillID:     strings.TrimSpace(r.URL.Query().Get("workflow_skill_id")),
		WorkflowVersion:     strings.TrimSpace(r.URL.Query().Get("workflow_version")),

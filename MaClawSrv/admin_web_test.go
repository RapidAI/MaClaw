...
func TestAdminWebAccessibilityContracts(t *testing.T) {
	...
	for _, needle := range []string{
		...
		`const f={overview,sandbox,logs,config,clientConfig,aiModels,tenants,accounts,knowledge,database,ops}[state.section] || overview;`,
		`id="dbProfileSave"`,
		`id="dbReceiptLookup"`,
		`id="dbSSHSession"`,
		`id="dbReplicaHost"`,
		...
		`/api/v1/admin/database/receipts/`,
		`/api/v1/admin/database/profiles/${encodeURIComponent(id)}/rotate-secret?${qs}`,

...
var openAPIRoutes = []openAPIRoute{
	...
	{Method: http.MethodGet, Path: "/metrics", Summary: "Prometheus metrics", Description: "Returns Prometheus text metrics for service and shared Agent Runtime counters, including credential lifecycle, persisted snapshot, run latency, tool, event, quota, and bounded tenant-hash gauges.", Tag: "system", ResponseContent: "text/plain"},
	...
	{Method: http.MethodPost, Path: "/api/v1/admin/scheduler/tasks", Summary: "Create scheduled task", Description: "Creates a scheduled task (owner admin). Supports delivery object or shorthand group_id/group_name/user_id/channel.", Tag: "admin", Security: adminSecurity(), AdminRole: "owner"},
	{Method: http.MethodPatch, Path: "/api/v1/admin/scheduler/tasks/{taskId}", Summary: "Update scheduled task", Description: "Partial update of a scheduled task (owner admin).", Tag: "admin", Security: adminSecurity(), AdminRole: "owner"},
	{Method: http.MethodDelete, Path: "/api/v1/admin/scheduler/tasks/{taskId}", Summary: "Delete scheduled task", Description: "Deletes a scheduled task (owner admin).", Tag: "admin", Security: adminSecurity(), AdminRole: "owner"},
	{Method: http.MethodPost, Path: "/api/v1/admin/scheduler/tasks/{taskId}/trigger", Summary: "Trigger scheduled task", Description: "Runs a task immediately (owner admin).", Tag: "admin", Security: adminSecurity(), AdminRole: "owner"},
	{Method: http.MethodPost, Path: "/api/v1/admin/scheduler/tasks/{taskId}/pause", Summary: "Pause scheduled task", Description: "Pauses a task (owner admin).", Tag: "admin", Security: adminSecurity(), AdminRole: "owner"},
	{Method: http.MethodPost, Path: "/api/v1/admin/scheduler/tasks/{taskId}/resume", Summary: "Resume scheduled task", Description: "Resumes a paused task (owner admin).", Tag: "admin", Security: adminSecurity(), AdminRole: "owner"},
	...
	{Method: http.MethodGet, Path: "/api/v1/admin/database/profiles", Summary: "List database profiles", Description: "Lists non-sensitive database profile summaries for a tenant user. secret_ref, DSN and file paths are never returned. replica_host/port, ssh_session_id and replica_ssh_session_id may be included without credentials.", Tag: "admin", Security: adminSecurity(), QueryParams: []string{"tenant_id", "user_id"}, AdminRole: "owner"},
	{Method: http.MethodPost, Path: "/api/v1/admin/database/profiles", Summary: "Create or update database profile", Description: "Creates or replaces a database profile for a tenant user after validation. Credentials must be bound via secret_ref; inline password/secret/token fields are rejected. Optional replica_host and replica_ssh_session_id route reads to a replica; writes always use the primary. Supports the Idempotency-Key header; the same key with a different body returns 409. Refreshes cached runtime connections.", Tag: "admin", Security: adminSecurity(), QueryParams: []string{"tenant_id", "user_id"}, AdminRole: "owner"},
	{Method: http.MethodPost, Path: "/api/v1/admin/database/profiles/{profileId}/test", Summary: "Test database profile", Description: "Probes a database profile with ping plus capability detection through a throwaway connection; never executes user SQL and never echoes secrets or local paths. Owner-only because the probe dials the configured host.", Tag: "admin", Security: adminSecurity(), QueryParams: []string{"tenant_id", "user_id"}, AdminRole: "owner"},
	{Method: http.MethodPost, Path: "/api/v1/admin/database/profiles/{profileId}/rotate-secret", Summary: "Rotate database profile secret", Description: "Rebinds a profile to a new secret_ref, bumps its schema version, persists the change and closes live connections of the old credential.", Tag: "admin", Security: adminSecurity(), QueryParams: []string{"tenant_id", "user_id"}, AdminRole: "owner"},
	{Method: http.MethodDelete, Path: "/api/v1/admin/database/profiles/{profileId}", Summary: "Delete database profile", Description: "Removes a database profile after confirm=true, refreshes cached runtime connections and keeps an audit reference.", Tag: "admin", Security: adminSecurity(), QueryParams: []string{"tenant_id", "user_id", "confirm"}, AdminRole: "owner"},
	{Method: http.MethodPost, Path: "/api/v1/admin/database/approvals", Summary: "Issue database write approval", Description: "Mints a one-time fingerprint-bound approval for a tenant user session after an explicit operator confirm. Returns only approval_id; the opaque token never leaves the host.", Tag: "admin", Security: adminSecurity(), QueryParams: []string{"tenant_id", "user_id"}, AdminRole: "owner"},
	{Method: http.MethodPost, Path: "/api/v1/admin/database/runtime", Summary: "Database tool kill switch", Description: "Enables or disables the database tool for a tenant user immediately, closing live connections when disabled.", Tag: "admin", Security: adminSecurity(), QueryParams: []string{"tenant_id", "user_id"}, AdminRole: "owner"},
	{Method: http.MethodGet, Path: "/api/v1/admin/database/metrics", Summary: "Database tool metrics", Description: "Returns process-local non-sensitive counters for a tenant user's cached database managers.", Tag: "admin", Security: adminSecurity(), QueryParams: []string{"tenant_id", "user_id"}, AdminRole: "owner"},
	{Method: http.MethodGet, Path: "/api/v1/admin/database/receipts/{receiptId}", Summary: "Lookup database mutation receipt", Description: "Returns a metadata-only mutation receipt by id from the tenant audit chain or file store. Tokens and SQL text are never included.", Tag: "admin", Security: adminSecurity(), QueryParams: []string{"tenant_id", "user_id"}, AdminRole: "owner"},
	...
	{Method: http.MethodPost, Path: "/api/v1/admin/dynamic-effects/{operationId}/resolve", Summary: "Resolve unknown dynamic effect", Description: "Owner-only reconciliation endpoint. Requires confirm=true, an explicit succeeded verdict, and evidence for an unknown dynamic operation outcome; the resolution is persisted and audited.", Tag: "admin", Security: adminSecurity(), AdminRole: "owner"},
	...
	{Method: http.MethodPost, Path: "/api/v1/admin/public-knowledge-libraries/{libraryId}/import/text", Summary: "Import public knowledge text", Description: "Imports text into a named public knowledge library. Without Idempotency-Key the legacy synchronous 201 response is preserved; with Idempotency-Key the operation is admitted as a durable Job with protected effect reconciliation.", Tag: "admin", Security: adminSecurity(), HeaderParams: []string{"Idempotency-Key"}},
	{Method: http.MethodPost, Path: "/api/v1/admin/public-knowledge-libraries/{libraryId}/import/file", Summary: "Import public knowledge files", Description: "Imports one or more documents or ZIP/RAR document archives into a named public knowledge library as an async job. Idempotency-Key deduplicates retries and the response reports idempotent_replay.", Tag: "admin", Security: adminSecurity(), HeaderParams: []string{"Idempotency-Key"}},
	{Method: http.MethodPost, Path: "/api/v1/admin/public-knowledge-libraries/{libraryId}/import/urls", Summary: "Import public knowledge URLs", Description: "Imports URLs into a named public knowledge library, optionally crawling to a configured depth. Idempotency-Key deduplicates retries and the response reports idempotent_replay.", Tag: "admin", Security: adminSecurity(), HeaderParams: []string{"Idempotency-Key"}},
	...
	{Method: http.MethodPost, Path: "/api/v1/knowledge/import/file", Summary: "Import knowledge files", Description: "Uploads one or more files and imports them into the current user's knowledge base as an async job. The protected effect ledger and optional Idempotency-Key make retries safe.", Tag: "knowledge", Security: bearerSecurity(), RequestContentTypes: []string{"multipart/form-data"}, HeaderParams: []string{"Idempotency-Key"}},
	...
	{Method: http.MethodPost, Path: "/api/v1/knowledge/import/urls", Summary: "Import knowledge URLs", Description: "Imports one or more URLs, optionally crawling discovered links up to max_depth. The async Job uses protected effects and an optional Idempotency-Key.", Tag: "knowledge", Security: bearerSecurity(), HeaderParams: []string{"Idempotency-Key"}},
	{Method: http.MethodPost, Path: "/api/v1/knowledge/import/directory", Summary: "Import knowledge directory", Description: "Imports a local directory into the current user's knowledge base as an async Job. Protected effects and optional Idempotency-Key prevent unsafe replay after uncertainty.", Tag: "knowledge", Security: bearerSecurity(), HeaderParams: []string{"Idempotency-Key"}},
	...
	{Method: http.MethodPost, Path: "/api/v1/knowledge/import/package", Summary: "Import knowledge package", Description: "Imports a MaClaw editable knowledge JSON package into the current user's own knowledge base as an async Job. URL entries are refetched; text entries require a content field. Optional Idempotency-Key deduplicates the admission.", Tag: "knowledge", Security: bearerSecurity(), HeaderParams: []string{"Idempotency-Key"}},
	{Method: http.MethodPost, Path: "/api/v1/knowledge/import/share", Summary: "Import shared knowledge", Description: "Imports shared knowledge by knowledge ID or by a human-readable, agent-importable share link after the host resolves the Hub and permissions, then starts an async Job. Private, tenant, or user-list shares can pass hub_token or authorization in the JSON body; optional Idempotency-Key deduplicates the local import.", Tag: "knowledge", Security: bearerSecurity(), HeaderParams: []string{"Idempotency-Key"}},
	...
	{Method: http.MethodPost, Path: "/api/v1/mcp/servers", Summary: "Create MCP server", Description: "Creates an MCP server synchronously or starts an async job when async=true. Idempotency-Key deduplicates retained async admissions.", Tag: "mcp", Security: bearerSecurity(), QueryParams: []string{"async"}, HeaderParams: []string{"Idempotency-Key"}},
	{Method: http.MethodGet, Path: "/api/v1/mcp/servers/{serverId}", Summary: "Get MCP server", Tag: "mcp", Security: bearerSecurity()},
	{Method: http.MethodPatch, Path: "/api/v1/mcp/servers/{serverId}", Summary: "Update MCP server", Description: "Updates an MCP server synchronously or starts an async job when async=true. Idempotency-Key deduplicates retained async admissions.", Tag: "mcp", Security: bearerSecurity(), QueryParams: []string{"async"}, HeaderParams: []string{"Idempotency-Key"}},
	{Method: http.MethodDelete, Path: "/api/v1/mcp/servers/{serverId}", Summary: "Delete MCP server", Tag: "mcp", Security: bearerSecurity()},
	{Method: http.MethodPost, Path: "/api/v1/mcp/servers/{serverId}/start", Summary: "Start MCP server", Description: "Starts an MCP server synchronously or starts an async job when async=true. Idempotency-Key deduplicates retained async admissions.", Tag: "mcp", Security: bearerSecurity(), QueryParams: []string{"async"}, HeaderParams: []string{"Idempotency-Key"}},
	{Method: http.MethodPost, Path: "/api/v1/mcp/servers/{serverId}/stop", Summary: "Stop MCP server", Description: "Stops an MCP server synchronously or starts an async job when async=true. Idempotency-Key deduplicates retained async admissions.", Tag: "mcp", Security: bearerSecurity(), QueryParams: []string{"async"}, HeaderParams: []string{"Idempotency-Key"}},
	{Method: http.MethodPost, Path: "/api/v1/mcp/servers/{serverId}/health-check", Summary: "Check MCP server", Description: "Checks MCP server health synchronously or starts an async job when async=true. Idempotency-Key deduplicates retained async admissions and rejects reuse with a different request. Async transient probe failures use the shared bounded retry policy; lookup and configuration failures are not retried.", Tag: "mcp", Security: bearerSecurity(), QueryParams: []string{"async"}, HeaderParams: []string{"Idempotency-Key"}},
	{Method: http.MethodGet, Path: "/api/v1/migration/status", Summary: "Get migration status", Description: "Returns migration configuration and current export state for the authenticated user.", Tag: "migration", Security: bearerSecurity()},
	{Method: http.MethodGet, Path: "/api/v1/migration/instances", Summary: "List migration instances", Description: "Lists migration-capable instances available through the configured Hub.", Tag: "migration", Security: bearerSecurity()},
	{Method: http.MethodPost, Path: "/api/v1/migration/export", Summary: "Start migration export", Description: "Starts a durable migration export Job. Idempotency-Key deduplicates retained admissions without persisting the password or raw key. Protected effect receipts allow an unknown Job to reconcile through a read-only Hub probe without replaying the export.", Tag: "migration", Security: bearerSecurity(), HeaderParams: []string{"Idempotency-Key"}},
	{Method: http.MethodPost, Path: "/api/v1/migration/import", Summary: "Start migration import", Description: "Starts a durable migration import Job. Idempotency-Key deduplicates retained admissions and rejects reuse with different export or password parameters. Protected local/Hub receipts reconcile unknown outcomes without replaying the import.", Tag: "migration", Security: bearerSecurity(), HeaderParams: []string{"Idempotency-Key"}},
	...
	{Method: http.MethodPost, Path: "/api/v1/skills/install", Summary: "Install skill", Description: "Installs a skill synchronously or starts an async Job when async=true. Async admissions use the protected effect ledger and an optional Idempotency-Key.", Tag: "skills", Security: bearerSecurity(), QueryParams: []string{"async"}, HeaderParams: []string{"Idempotency-Key"}},
	{Method: http.MethodPost, Path: "/api/v1/skills/import", Summary: "Import skill", Description: "Imports a skill archive synchronously or starts an async Job when async=true. Async admissions use the protected effect ledger and an optional Idempotency-Key.", Tag: "skills", Security: bearerSecurity(), QueryParams: []string{"async"}, HeaderParams: []string{"Idempotency-Key"}},
	...
	{Method: http.MethodGet, Path: "/api/v1/jobs/{jobId}", Summary: "Get async job", Description: "Returns the current state of an async user-scoped job started by async skill or MCP operations. Status uses the shared Runtime vocabulary (pending, running, succeeded, failed, canceled, unknown); version is the durable repository revision, retry_policy/attempt/next_attempt_at describe bounded in-process retries, and failures may include error_code such as job_persistence_failed or service_restarted.", Tag: "jobs", Security: bearerSecurity()},
	...
	{Method: http.MethodPost, Path: "/api/v1/skills/{skillName}/upload", Summary: "Upload skill", Description: "Uploads a skill synchronously or starts an async Job when async=true. The remote submission is recorded through the protected effect ledger and supports an optional Idempotency-Key.", Tag: "skills", Security: bearerSecurity(), QueryParams: []string{"async"}, HeaderParams: []string{"Idempotency-Key"}},
	...
	{Method: http.MethodGet, Path: "/api/v1/instances/{instanceId}/runtime-capabilities", Summary: "Shared Runtime capabilities", Description: "Returns the transport-neutral capability snapshot from the shared agentruntime facade, including host profile, module descriptors, effective tools, prompt digest, and canonical surface digest used by GUI and MaClawSrv.", Tag: "instances", Security: bearerSecurity()},
	...
	{Method: http.MethodPost, Path: "/api/v1/instances/{instanceId}/messages", Summary: "Send message", Description: "Creates or reuses a session and posts a message. Idempotency-Key may be supplied to deduplicate retries; Prefer: respond-async or async=true returns 202 after run admission.", Tag: "messages", Security: bearerSecurity(), QueryParams: []string{"async"}, HeaderParams: []string{"Idempotency-Key", "Prefer"}},
	...
	{Method: http.MethodPost, Path: "/api/v1/instances/{instanceId}/sessions/{sessionId}/messages", Summary: "Post message", Description: "Posts a message and returns the completed assistant response. Send Prefer: respond-async or async=true to receive 202 Accepted as soon as the run is durably admitted, then poll status_url or subscribe to run events.", Tag: "messages", Security: bearerSecurity(), QueryParams: []string{"async"}, HeaderParams: []string{"Prefer", "Idempotency-Key"}},
	...
	{Method: http.MethodGet, Path: "/api/v1/instances/{instanceId}/runs/{runId}/events", Summary: "Stream run events", Description: "Returns a server-sent events stream for run snapshots and terminal updates. Send Last-Event-ID to resume after a previously received durable event id.", Tag: "runs", Security: bearerSecurity(), HeaderParams: []string{"Last-Event-ID"}},
...
func buildOpenAPISpec() map[string]any {
	...
	for _, route := range openAPIRoutes {
		...
		if route.Method == http.MethodPost && (route.Path == "/api/v1/instances/{instanceId}/sessions/{sessionId}/messages" || route.Path == "/api/v1/instances/{instanceId}/messages") {
			responses["202"] = map[string]any{
				"description": "Run accepted for asynchronous execution (Prefer: respond-async)",
...
func openAPIQuerySchema(path, name string) map[string]any {
	...
	case "status":
		...
		case "/api/v1/jobs":
			return map[string]any{"type": "string", "enum": []string{"pending", "running", "succeeded", "failed", "canceled", "unknown"}}

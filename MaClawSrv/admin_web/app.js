...
const state = { locale: localStorage.getItem("maclaw.admin.locale") || "zh-CN", token: localStorage.getItem("maclaw.admin.token") || "", section: initialSection, me: null, pendingRiskFilter: null, pendingRequests: 0, locales: [{locale:"zh-CN",label:"zh-CN"},{locale:"en-US",label:"English"}] };
...
function toast(msg){ const el=$("toast"); el.textContent=msg; el.setAttribute("role","status"); el.setAttribute("aria-live","polite"); el.classList.remove("hidden"); clearTimeout(window.toastTimer); window.toastTimer=setTimeout(()=>el.classList.add("hidden"),3500); }
...
function setTitle(title,hint=""){ $("pageTitle").textContent=title; $("pageHint").textContent=hint; const main=$("main"); if(main) main.setAttribute("aria-labelledby","pageTitle"); const content=$("content"); if(content) content.setAttribute("aria-describedby", hint ? "pageHint" : ""); }
...
function setSection(id, updateHash=true){ if(!sections.includes(id)) id="overview"; state.section=id; localStorage.setItem("maclaw.admin.section",id); if(updateHash&&location.hash!==`#${id}`) history.replaceState(null,"",`#${id}`); }
...
function table(items, cols, action){ if(!items.length) return emptyState(t("empty")); return `<div class="table-wrap" tabindex="0" role="region" aria-label="${esc(cols.join(", "))}"><table><thead><tr>${cols.map(c=>`<th>${esc(c)}</th>`).join("")}<th>${t("action")}</th></tr></thead><tbody>${items.map(x=>`<tr>${cols.map(c=>`<td class="${cellClass(c,x[c])}">${cellValue(c,x[c])}</td>`).join("")}<td class="table-actions">${action?action(x):""}</td></tr>`).join("")}</tbody></table></div>`; }

...
function table(items, cols, action){ if(!items.length) return `<p class="muted">${t("empty")}</p>`; return `<div class="table-wrap"><table><thead><tr>${cols.map(c=>`<th>${esc(c)}</th>`).join("")}<th>${t("action")}</th></tr></thead><tbody>${items.map(x=>`<tr>${cols.map(c=>`<td class="${cellClass(c,x[c])}">${cellValue(c,x[c])}</td>`).join("")}<td class="table-actions">${action?action(x):""}</td></tr>`).join("")}</tbody></table></div>`; }

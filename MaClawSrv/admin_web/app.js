...
function hideMain(){ $("content").innerHTML=""; $("content").className="content"; $("loginPanel").classList.add("hidden"); $("bootstrapPanel").classList.add("hidden"); renderShell(); }
...
function bindTenantActions(){
  const out=()=>$("tenantOut");
  document.querySelectorAll("[data-tenant-summary]").forEach(b=>b.onclick=async()=>out().textContent=pretty(await api(`/api/v1/admin/tenants/${encodeURIComponent(b.dataset.tenantSummary)}/summary`)));
  document.querySelectorAll("[data-tenant-status]").forEach(b=>b.onclick=async()=>{const [tenant,status]=b.dataset.tenantStatus.split(":"); const action=status==="disabled"?"pause":"resume"; await api(`/api/v1/admin/tenants/${encodeURIComponent(tenant)}/${action}`,{method:"POST",body:"{}"}); render();});
  document.querySelectorAll("[data-tenant-audit]").forEach(b=>b.onclick=async()=>showTenantAudit(b.dataset.tenantAudit));
  document.querySelectorAll("[data-tenant-check]").forEach(b=>b.onclick=async()=>out().textContent=pretty(await api(`/api/v1/admin/tenants/${encodeURIComponent(b.dataset.tenantCheck)}/delete-check`)));
  document.querySelectorAll("[data-tenant-delete]").forEach(b=>b.onclick=async()=>{try{const id=b.dataset.tenantDelete; const check=await api(`/api/v1/admin/tenants/${encodeURIComponent(id)}/delete-check`); out().textContent=pretty(check); if(check.can_delete===false){if(protectedOnly(check)){const secret=await forceDeleteSecret(t("forceDeleteTenantPrompt",{users:check.users||0}),t("forceDeleteTenantConfirm")); if(!secret) return; out().textContent=pretty(await api(`/api/v1/admin/tenants/${encodeURIComponent(id)}?confirm=true&force=true`,{method:"DELETE",body:JSON.stringify({password:secret,admin_secret:secret})})); render(); return;} toast(t("deleteBlocked")); return;} if(await confirmDanger(t("deleteTenantConfirm"))){out().textContent=pretty(await api(`/api/v1/admin/tenants/${encodeURIComponent(id)}?confirm=true`,{method:"DELETE"})); render();}}catch(e){toast(`${t("failed")}: ${e.message}`);}});
  document.querySelectorAll("[data-user-status]").forEach(b=>b.onclick=async()=>{const [tenant,user,status]=b.dataset.userStatus.split(":"); const action=status==="disabled"?"pause":"resume"; await api(`/api/v1/admin/tenants/${encodeURIComponent(tenant)}/users/${encodeURIComponent(user)}/${action}`,{method:"POST",body:"{}"}); render();});
  document.querySelectorAll("[data-user-credentials]").forEach(b=>b.onclick=async()=>{const [tenant,user]=b.dataset.userCredentials.split(":"); await showCredentials(tenant,user);});
  document.querySelectorAll("[data-user-audit]").forEach(b=>b.onclick=async()=>{const [tenant,user]=b.dataset.userAudit.split(":"); await showTenantAudit(tenant,user);});
  document.querySelectorAll("[data-user-check]").forEach(b=>b.onclick=async()=>{const [tenant,user]=b.dataset.userCheck.split(":"); out().textContent=pretty(await api(`/api/v1/admin/tenants/${encodeURIComponent(tenant)}/users/${encodeURIComponent(user)}/delete-check`));});
  document.querySelectorAll("[data-user-plan]").forEach(b=>b.onclick=async()=>{const [tenant,user]=b.dataset.userPlan.split(":"); out().textContent=pretty(await api(`/api/v1/admin/tenants/${encodeURIComponent(tenant)}/users/${encodeURIComponent(user)}/retire-plan`));});
  document.querySelectorAll("[data-user-delete]").forEach(b=>b.onclick=async()=>{try{const [tenant,user]=b.dataset.userDelete.split(":"); const check=await api(`/api/v1/admin/tenants/${encodeURIComponent(tenant)}/users/${encodeURIComponent(user)}/delete-check`); out().textContent=pretty(check); if(check.can_delete===false){if(protectedOnly(check)){const secret=await forceDeleteSecret(t("forceDeleteUserPrompt"),t("forceDeleteUserConfirm")); if(!secret) return; out().textContent=pretty(await api(`/api/v1/admin/tenants/${encodeURIComponent(tenant)}/users/${encodeURIComponent(user)}?confirm=true&force=true`,{method:"DELETE",body:JSON.stringify({password:secret,admin_secret:secret})})); render(); return;} toast(t("deleteBlocked")); return;} if(await confirmDanger(t("deleteUserConfirm"))){out().textContent=pretty(await api(`/api/v1/admin/tenants/${encodeURIComponent(tenant)}/users/${encodeURIComponent(user)}?confirm=true`,{method:"DELETE"})); render();}}catch(e){toast(`${t("failed")}: ${e.message}`);}});
}
...
function bindKnowledgeActions(sources){
  ...
  $('loadSkillUser').onclick=run(async()=>{ const email=requireInput('skillUserEmail','userEmail'); if(!email) return; const cfg=await api(`/api/v1/admin/skill-sources/user/${encodeURIComponent(email)}`); applySkillSource('skillUserCfg',sources,cfg); show(cfg); });
  $('saveSkillUser').onclick=run(async()=>{ const email=requireInput('skillUserEmail','userEmail'); if(!email) return; saveOK(await api(`/api/v1/admin/skill-sources/user/${encodeURIComponent(email)}`,{method:'PUT',body:JSON.stringify(skillSourceBody('skillUserCfg',sources))})); });
  $('resolveSkillUser').onclick=run(async()=>{ const email=requireInput('skillUserEmail','userEmail'); if(!email) return; const tenant=$('skillTenant').value.trim(); const q=tenant?`?tenant_id=${encodeURIComponent(tenant)}`:''; show(await api(`/api/v1/admin/skill-sources/resolve/${encodeURIComponent(email)}${q}`)); });
  $('deleteSkillUser').onclick=run(async()=>{ const email=requireInput('skillUserEmail','userEmail'); if(!email) return; if(await confirmDanger(t("deleteSkillUserConfirm"))) saveOK(await api(`/api/v1/admin/skill-sources/user/${encodeURIComponent(email)}`,{method:'DELETE'})); });
}

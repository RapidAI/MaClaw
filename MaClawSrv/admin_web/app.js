...
function showAdminResult(title,value){ const prior=document.activeElement; const overlay=document.createElement("div"); overlay.className="modal-backdrop"; overlay.innerHTML=`<div class="modal admin-result-modal" role="dialog" aria-modal="true" aria-labelledby="adminResultTitle"><div class="modal-head"><h2 id="adminResultTitle">${esc(title)}</h2><button type="button" class="secondary w-auto" id="adminResultClose" aria-label="${esc(t("cancel"))}">&times;</button></div>${summaryBody(value)}</div>`; document.body.appendChild(overlay); const close=()=>{overlay.remove(); prior?.focus?.({preventScroll:true});}; overlay.querySelector("#adminResultClose").onclick=close; overlay.onkeydown=e=>{if(e.key==="Escape") close();}; overlay.addEventListener("click",e=>{if(e.target===overlay) close();}); overlay.querySelector("#adminResultClose")?.focus({preventScroll:true}); }
...
async function refreshAIModelStatus(silent=false){ const out=await api("/api/v1/admin/ai-models/status"); $("aiModelStatusPanel").outerHTML=renderAIModelStatusPanel(out.models||{}); bindAIModelStatusActions(); applyOwnerGuards(); if(!silent) toast(t("modelStatusRefreshed")); return out; }

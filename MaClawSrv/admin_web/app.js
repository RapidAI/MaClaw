...
function bindSchedulerTaskActions(){ document.querySelectorAll("[data-sch-trigger]").forEach(b=>b.onclick=async()=>withBusyButton(b,t("running"),async()=>{await api(`/api/v1/admin/scheduler/tasks/${encodeURIComponent(b.dataset.schTrigger)}/trigger`,{method:"POST"}); toast(t("loaded")); $("refreshSchedulerTasks")?.click();})); document.querySelectorAll("[data-sch-pause]").forEach(b=>b.onclick=async()=>withBusyButton(b,t("running"),async()=>{await api(`/api/v1/admin/scheduler/tasks/${encodeURIComponent(b.dataset.schPause)}/pause`,{method:"POST"}); toast(t("loaded")); $("refreshSchedulerTasks")?.click();})); document.querySelectorAll("[data-sch-resume]").forEach(b=>b.onclick=async()=>withBusyButton(b,t("running"),async()=>{await api(`/api/v1/admin/scheduler/tasks/${encodeURIComponent(b.dataset.schResume)}/resume`,{method:"POST"}); toast(t("loaded")); $("refreshSchedulerTasks")?.click();})); document.querySelectorAll("[data-sch-del]").forEach(b=>b.onclick=async()=>withBusyButton(b,t("running"),async()=>{if(await confirmDanger(t("delete")||"Delete?")){await api(`/api/v1/admin/scheduler/tasks/${encodeURIComponent(b.dataset.schDel)}`,{method:"DELETE"}); toast(t("loaded")); $("refreshSchedulerTasks")?.click();}})); }

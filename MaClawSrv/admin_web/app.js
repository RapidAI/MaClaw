...
const colLabels={env_key:"envKey",configured:"configuredState",default:"defaultValue",source:"source",restart_required:"restartRequired",mutable_at_runtime:"runtimeOnly",changed:"changed",current:"current",desired:"desired",sensitive:"sensitive",action:"action"};
...
async function render(){ hideMain(); if(!state.token){ renderLogin(); return; } renderShell(); $("content").setAttribute("aria-busy","true"); $("content").innerHTML=`<div class="card loading-card"><div class="skeleton skeleton-title"></div><div class="skeleton skeleton-line"></div><div class="skeleton skeleton-line short"></div></div>`; const f={overview,sandbox,logs,config,tenants,accounts,knowledge,ops}[state.section] || overview; try{ await f(); applyOwnerGuards(); enhanceA11y(); if(state.sectionChanged){ $("main")?.focus({preventScroll:true}); state.sectionChanged=false; } }catch(e){ if(e.status===401){state.token="";localStorage.removeItem("maclaw.admin.token");renderLogin();return;} $("content").innerHTML=`<div class="panel error-panel"><h2>${t("failed")}</h2><pre class="code">${esc(e.message)}\n${esc(pretty(e.body||{}))}</pre></div>`; enhanceA11y(); if(state.sectionChanged){ $("main")?.focus({preventScroll:true}); state.sectionChanged=false; } } }
...
async function config(){ setTitle(t("config"), t("configHint")); const [eff,draftResp,schemaResp,envResp,diffResp]=await Promise.all([api("/api/v1/admin/service-config/effective"),api("/api/v1/admin/service-config/draft"),api("/api/v1/admin/service-config/schema"),api("/api/v1/admin/service-config/environment"),api("/api/v1/admin/service-config/diff")]); const draft=draftResp.draft||{values:{}}; const schema=schemaResp.items||[]; const tabs=["draft","effective","environment","diff","secrets","raw"]; const saved=localStorage.getItem("maclaw.admin.configTab")||"draft"; const active=tabs.includes(saved)?saved:"draft"; const tabBtn=(id)=>`<button type="button" role="tab" class="${active===id?"active":""}" aria-selected="${active===id}" data-config-tab="${id}">${t(`cfgTab${id[0].toUpperCase()+id.slice(1)}`)}</button>`; const pane=(id,html)=>`<section class="config-tab-panel ${active===id?"":"hidden"}" data-config-pane="${id}" role="tabpanel">${html}</section>`; const effectiveRows=Object.values(eff.fields||{}).map((v,i)=>({key:Object.keys(eff.fields||{})[i],value:JSON.stringify(v.value),source:v.source,restart_required:v.restart_required,mutable_at_runtime:v.mutable_at_runtime})); $("content").innerHTML=`<div class="card stack config-card"><div class="segmented config-tabs" role="tablist" aria-label="${t("config")}">${tabs.map(tabBtn).join("")}</div>${pane("draft",`<div class="stack"><h2>${t("draft")}</h2><div class="panel"><h2>${t("howToApplyConfig")}</h2><p class="helper-text">${t("configApplyHint")}</p></div><div id="cfgFields" class="stack">${configFields(schema,draft.values||{},eff.fields||{})}</div><label>${t("reason")}</label><input id="cfgReason" value="${esc(draft.reason||"")}" placeholder="${t("reason")}"><div class="row"><button id="saveCfg">${t("save")}</button><button class="secondary" id="validateCfg">${t("validate")}</button><button class="secondary" id="exportCfg">${t("exportPlan")}</button><button class="danger" id="clearCfgDraft">${t("clearDraft")}</button></div><pre id="cfgOut" class="code">${esc(pretty(draftResp))}</pre></div>`)}${pane("effective",`<h2>${t("effective")}</h2>${table(effectiveRows,["key","value","source","restart_required","mutable_at_runtime"],null)}`)}${pane("environment",`<h2>${t("environment")}</h2>${table(envResp.items||[],["key","env_key","configured","value","default","source"],null)}`)}${pane("diff",`<h2>${t("draftDiff")}</h2>${table(diffResp.items||[],["key","env_key","configured","changed","current","desired","restart_required"],null)}`)}${pane("secrets",`<div class="panel stack"><h2>${t("revealAdminSecret")}</h2><p class="helper-text">${t("revealAdminSecretHint")}</p><div class="secret-field-row"><input id="revealSecretPassword" type="password" autocomplete="current-password" placeholder="${t("adminPassword")}"><button id="revealAdminSecret" class="warn">${t("revealAdminSecret")}</button></div><div id="adminSecretResult" class="code hidden"></div></div>`)}${pane("raw",`<h2>${t("raw")}</h2><pre id="cfgRawOut" class="code">${esc(pretty({draft:draftResp, schema:schemaResp, environment:envResp, diff:diffResp}))}</pre>`)}</div>`; document.querySelectorAll("[data-config-tab]").forEach(btn=>btn.onclick=()=>{const id=btn.dataset.configTab; localStorage.setItem("maclaw.admin.configTab",id); document.querySelectorAll("[data-config-tab]").forEach(b=>{const on=b.dataset.configTab===id; b.classList.toggle("active",on); b.setAttribute("aria-selected",String(on));}); document.querySelectorAll("[data-config-pane]").forEach(p=>p.classList.toggle("hidden",p.dataset.configPane!==id));}); const values=()=>buildConfigValues(schema); const out=()=>$("cfgOut")||$("cfgRawOut"); if($("saveCfg")) $("saveCfg").onclick=async()=>{try{const result=await api("/api/v1/admin/service-config/draft",{method:"PATCH",body:JSON.stringify({values:values(),reason:$("cfgReason").value})}); out().innerHTML=configValidationView(result); toast(t("saved"));}catch(e){toast(`${t("failed")}: ${e.message}`);}}; if($("validateCfg")) $("validateCfg").onclick=async()=>{try{out().innerHTML=configValidationView(await api("/api/v1/admin/service-config/validate",{method:"POST",body:JSON.stringify({values:values()})}));}catch(e){toast(`${t("failed")}: ${e.message}`);}}; if($("exportCfg")) $("exportCfg").onclick=async()=>{try{out().innerHTML=configExportView(await api("/api/v1/admin/service-config/export-plan",{method:"POST",body:JSON.stringify({values:values()})}));}catch(e){toast(`${t("failed")}: ${e.message}`);}}; if($("clearCfgDraft")) $("clearCfgDraft").onclick=async()=>{if(!confirm(t("clearDraftConfirm"))) return; try{out().textContent=pretty(await api("/api/v1/admin/service-config/draft?confirm=true",{method:"DELETE"})); toast(t("loaded")); render();}catch(e){toast(`${t("failed")}: ${e.message}`);}}; if($("revealAdminSecret")) $("revealAdminSecret").onclick=async()=>{try{const result=await api("/api/v1/admin/auth/reveal-admin-secret",{method:"POST",body:JSON.stringify({password:$("revealSecretPassword").value})}); const box=$("adminSecretResult"); box.classList.remove("hidden"); box.innerHTML=`<div class="row"><code>${esc(result.secret)}</code><button class="secondary" id="copyAdminSecret">${t("copy")}</button></div>`; $("copyAdminSecret").onclick=async()=>{await navigator.clipboard.writeText(result.secret); toast(t("copied"));};}catch(e){toast(`${t("failed")}: ${e.message}`)}}; }
async function config(){
  ...
  if($("clearCfgDraft")) $("clearCfgDraft").onclick=async()=>{if(!confirm(t("clearDraftConfirm"))) return; try{out().textContent=pretty(await api("/api/v1/admin/service-config/draft?confirm=true",{method:"DELETE"})); toast(t("loaded")); render();}catch(e){toast(`${t("failed")}: ${e.message}`);}};

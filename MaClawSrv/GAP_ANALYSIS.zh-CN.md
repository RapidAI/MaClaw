...
## 最近完成：tenant summary 凭证计数

- Tenant summary 已在租户级和用户级同时返回 credential 生命周期计数。
- 管理控制台无需逐个用户拉取 credential 列表，就能展示 total、active、suspended、revoked、expired、expiring 等计数。

## 最近完成：credential metrics

- `/metrics` 已输出 credential 总数、状态、已过期、即将过期等 Prometheus gauge。
- 运维侧可以直接基于 Prometheus 告警 credential 生命周期风险，不必额外轮询 admin JSON 接口。

## 最近完成：usage summary 凭证计数

- 已认证用户现在可以通过 `/api/v1/usage/summary` 查看自己的 credential 生命周期聚合计数。
- 该接口只返回计数，不返回任何 credential 密钥材料，保持默认安全。

## 最近完成：credential 创建时设置过期时间

- Credential 创建接口现在支持直接传入 `expires_at`。
- 管理端可以在一次请求中创建自动生成的凭证并同时设置过期时间，避免先创建无限制凭证再 PATCH 的短暂窗口。

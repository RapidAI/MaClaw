...
// RuntimeStatusValue projects the remote-session vocabulary onto the shared
// Runtime job lifecycle used by GUI and srv transports.
func (status SessionStatus) RuntimeStatusValue() agentruntime.JobStatus {
	switch normalizeSessionStatus(status.String()) {
	case SessionStarting, SessionRunning, SessionBusy:

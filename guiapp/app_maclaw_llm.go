...
func normalizeMaclawLLMProvider(provider corelib.MaclawLLMProvider) corelib.MaclawLLMProvider {
	...
	provider.CacheReadPricePerMTokensRMB = corelib.NormalizeLLMTokenPricePerMTokensRMB(provider.CacheReadPricePerMTokensRMB, provider.InputPricePerMTokensRMB/10)
	provider.CacheWritePricePerMTokensRMB = corelib.NormalizeLLMTokenPricePerMTokensRMB(provider.CacheWritePricePerMTokensRMB, provider.InputPricePerMTokensRMB)
...
func defaultMaclawLLMProviders() []corelib.MaclawLLMProvider {
	return []corelib.MaclawLLMProvider{
		...
		{Name: "Qwen", URL: "https://dashscope.aliyuncs.com/compatible-mode/v1", Model: "qwen3.8-flash", Protocol: "openai", ContextLength: 400000, TimeoutSec: corelib.DefaultLLMTimeoutSec},
...
func validateMaclawLLMProfiles(profiles corelib.MaclawLLMProfiles, providers []corelib.MaclawLLMProvider) error {
	...
	horizonOverrides := []struct {
		name    string
		profile corelib.MaclawLLMProfile
	}{
		{"horizon manager", profiles.Horizon.Manager},
		{"horizon auditor", profiles.Horizon.Auditor},
		{"horizon executor", profiles.Horizon.Executor},
	}
	for _, override := range horizonOverrides {
		if !maclawLLMProfileAssigned(override.profile) {
			continue
		}
		if strings.TrimSpace(override.profile.ProviderID) == "" || strings.TrimSpace(override.profile.Model) == "" {
			return fmt.Errorf("%s provider and model are required when a horizon role model is selected", override.name)
		}
		if _, ok := resolveMaclawLLMProviderByID(providers, override.profile.ProviderID); !ok {
			return fmt.Errorf("%s references an unavailable provider", override.name)
		}
	}
	return nil
}
...
func normalizeThinkingModeSetting(v string) string {
	return corelib.NormalizeGlobalThinkingMode(v)
}
...
func effectiveThinkingModeSetting(setting string) string {
	return corelib.EffectiveGlobalThinkingMode(setting)
}
...
func maclawLLMUsagePricesFromConfig(cfg *corelib.AppConfig, providerName string) (float64, float64) {
	inputPrice, outputPrice, _, _ := maclawLLMUsagePricesWithCacheFromConfig(cfg, providerName)
	return inputPrice, outputPrice
}

func maclawLLMUsagePricesWithCacheFromConfig(cfg *corelib.AppConfig, providerName string) (float64, float64, float64, float64) {
	...
	for _, provider := range normalizeMaclawLLMProviders(cfg.MaclawLLMProviders) {
		if strings.EqualFold(strings.TrimSpace(provider.Name), providerName) {
			...
			cacheReadPrice = corelib.NormalizeLLMTokenPricePerMTokensRMB(provider.CacheReadPricePerMTokensRMB, inputPrice/10)
			cacheWritePrice = corelib.NormalizeLLMTokenPricePerMTokensRMB(provider.CacheWritePricePerMTokensRMB, inputPrice)
			return inputPrice, outputPrice, cacheReadPrice, cacheWritePrice
		}
	}
	return inputPrice, outputPrice, cacheReadPrice, cacheWritePrice
	...
	if err := a.PatchConfig(func(cfg *corelib.AppConfig) {
		...
		for providerName, delta := range pending {
			...
			if delta.IsProfile {
				...
				inputPrice, outputPrice, cacheReadPrice, cacheWritePrice := maclawLLMUsagePricesWithCacheFromConfig(cfg, delta.ProviderDisplayName)
			...
			inputPrice, outputPrice, cacheReadPrice, cacheWritePrice := maclawLLMUsagePricesWithCacheFromConfig(cfg, providerName)
...
func (a *App) GetAllLLMTokenUsage() map[string]*corelib.TokenUsageStat {
	...
	for name, delta := range a.tokenUsagePending {
		...
		if stat.InputPricePerMTokensRMB == 0 && stat.OutputPricePerMTokensRMB == 0 {
			stat.InputPricePerMTokensRMB, stat.OutputPricePerMTokensRMB, stat.CacheReadPricePerMTokensRMB, stat.CacheWritePricePerMTokensRMB = maclawLLMUsagePricesWithCacheFromConfig(&cfg, name)
		} else {
			if stat.CacheReadPricePerMTokensRMB == 0 {
				stat.CacheReadPricePerMTokensRMB = stat.InputPricePerMTokensRMB / 10
			}
...
func (a *App) GetAllLLMProfileTokenUsage() map[string]*corelib.TokenUsageStat {
	...
	for key, delta := range a.tokenUsagePending {
		...
		if stat.InputPricePerMTokensRMB == 0 && stat.OutputPricePerMTokensRMB == 0 {
			stat.InputPricePerMTokensRMB, stat.OutputPricePerMTokensRMB, stat.CacheReadPricePerMTokensRMB, stat.CacheWritePricePerMTokensRMB = maclawLLMUsagePricesWithCacheFromConfig(&cfg, delta.ProviderDisplayName)
		} else {
			if stat.CacheReadPricePerMTokensRMB == 0 {
				stat.CacheReadPricePerMTokensRMB = stat.InputPricePerMTokensRMB / 10
			}
...
func recalculateLLMTokenUsageCosts(stat *corelib.TokenUsageStat) {
	...
	// Keep the displayed aggregate derived from the authoritative directional
	// token counters; older records may have omitted or stale total_tokens.
	stat.TotalTokens = stat.InputTokens + stat.OutputTokens
	stat.InputCostRMB, stat.OutputCostRMB, stat.CacheReadCostRMB, stat.CacheWriteCostRMB, stat.TotalCostRMB = corelib.CalculateLLMCostRMBWithCache(
		stat.InputTokens, stat.OutputTokens, stat.CachedInputTokens, stat.CacheWriteTokens,
		stat.InputPricePerMTokensRMB, stat.OutputPricePerMTokensRMB,
		stat.CacheReadPricePerMTokensRMB, stat.CacheWritePricePerMTokensRMB,
	)
}

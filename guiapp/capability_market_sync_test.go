...
func TestCommitMarketplaceMCPConfigPreservesConcurrentUnrelatedConfig(t *testing.T) {
	tmpHome := t.TempDir()
	...
	if err := app.SaveConfig(corelib.AppConfig{Language: "zh-CN"}); err != nil {
		t.Fatalf("SaveConfig: %v", err)
	}
	item := HubCapabilitySummary{ID: "mcp-cap", CapabilityID: "mcp-cap", DisplayName: "MCP"}
	installed, err := app.commitMarketplaceMCPConfig(context.Background(), item, "v1", "stdio", func(cfg *corelib.AppConfig) (bool, error) {
		cfg.LocalMCPServers = []corelib.LocalMCPServerEntry{{ID: "local-mcp", Name: "Local", Command: "node"}}
		return true, nil
	})
	if err != nil || !installed {
		t.Fatalf("commitMarketplaceMCPConfig installed=%v err=%v", installed, err)
	}
	...
	if got.Language != "zh-CN" {
		t.Fatalf("unrelated config was clobbered: language=%q", got.Language)
	}
	if len(got.LocalMCPServers) != 1 || got.LocalMCPServers[0].ID != "local-mcp" {
		t.Fatalf("MCP config was not committed: %#v", got.LocalMCPServers)
	}
	if records, err := cskill.ListEvolutionCompensationSummaries(); err != nil {
		t.Fatalf("ListEvolutionCompensationSummaries: %v", err)
	} else if len(records) != 0 {
		t.Fatalf("successful MCP commit left compensation records: %#v", records)
	}
}

func TestCommitMarketplaceMCPConfigCancelledBeforeWriteLeavesNoMutation(t *testing.T) {
	tmpHome := t.TempDir()
	t.Setenv("HOME", tmpHome)
	t.Setenv("USERPROFILE", tmpHome)
	oldBase := corelib.MaclawBaseDir()
	...
	app := &App{testHomeDir: tmpHome}
	original := corelib.AppConfig{Language: "zh-CN"}
	if err := app.SaveConfig(original); err != nil {
		t.Fatalf("SaveConfig: %v", err)
	}
	ctx, cancel := context.WithCancel(context.Background())
	cancel()
	called := false
	installed, err := app.commitMarketplaceMCPConfig(ctx, HubCapabilitySummary{ID: "cancelled-mcp"}, "v1", "stdio", func(cfg *corelib.AppConfig) (bool, error) {
		called = true
		cfg.Language = "should-not-write"
		return true, nil
	})
	if err == nil || installed {
		t.Fatalf("cancelled commit installed=%v err=%v, want cancellation", installed, err)
	}
	if called {
		t.Fatal("mutation callback ran for a pre-cancelled context")
	}
	got, err := app.LoadConfig()
	if err != nil {
		t.Fatalf("LoadConfig: %v", err)
	...
	if got.Language != original.Language {
		t.Fatalf("cancelled MCP transaction mutated config: language=%q", got.Language)
	}
	configPath := filepath.Join(tmpHome, ".maclaw", "config.json")
	if _, err := os.Stat(configPath); err != nil {
		t.Fatalf("config disappeared after cancellation: %v", err)
	}
}

func TestCommitMarketplaceMCPConfigAuditAndSummaryDoNotExposeSecrets(t *testing.T) {
	tmpHome := t.TempDir()
	t.Setenv("HOME", tmpHome)
	t.Setenv("USERPROFILE", tmpHome)
	oldBase := corelib.MaclawBaseDir()
	...
	app := &App{testHomeDir: tmpHome}
	item := HubCapabilitySummary{ID: "secret-mcp", CapabilityID: "secret-mcp"}
	const secret = "super-secret-token"
	const endpoint = "https://mcp.internal.example/private"
	installed, err := app.commitMarketplaceMCPConfig(context.Background(), item, "v1", "http", func(cfg *corelib.AppConfig) (bool, error) {
		cfg.MCPServers = []corelib.MCPServerEntry{{
			ID: "secret-mcp", Name: "Private MCP", EndpointURL: endpoint,
			AuthType: "bearer", AuthSecret: secret,
	...
	if strings.Contains(string(raw), secret) || strings.Contains(string(raw), endpoint) {
		t.Fatalf("MCP secret/endpoint leaked into audit: %s", raw)
	}
	...
	if strings.Contains(string(summaryJSON), secret) || strings.Contains(string(summaryJSON), endpoint) {
		t.Fatalf("MCP secret/endpoint leaked into operator summary: %s", summaryJSON)
	}
}

func TestCommitMarketplaceMCPConfigRecoveryAcrossRestartRestoresPreImage(t *testing.T) {
	tmpHome := t.TempDir()
	t.Setenv("HOME", tmpHome)
	t.Setenv("USERPROFILE", tmpHome)
	oldBase := corelib.MaclawBaseDir()
	...
	if err := app.SaveConfig(corelib.AppConfig{Language: "changed", MCPServers: []corelib.MCPServerEntry{{ID: "mcp"}}}); err != nil {
		t.Fatalf("SaveConfig changed: %v", err)
	}
	record := cskill.NewEvolutionCompensationRecord("restart-mcp", "restart-mcp", "capability_mcp_config", "", nil, false, nil, "mcp_config_rollback")
	record.SetFileSnapshots([]cskill.EvolutionFileSnapshot{{Path: configPath, Exists: true, BackupB64: base64.StdEncoding.EncodeToString(preImage)}})
	if err := cskill.PersistEvolutionCompensation(record); err != nil {
		t.Fatalf("PersistEvolutionCompensation: %v", err)
	}
	if _, pending, err := cskill.RecoverPendingEvolutionCompensationsForActionPrefixAndSkill("capability_mcp_config", "restart-mcp", nil, nil); err != nil || pending != 0 {
		t.Fatalf("recovery pending=%d err=%v", pending, err)
	}
	restarted := &App{testHomeDir: tmpHome}
	got, err := restarted.LoadConfig()
	if err != nil {
		t.Fatalf("LoadConfig after recovery: %v", err)
	}
	if got.Language != original.Language || len(got.MCPServers) != 0 {
		t.Fatalf("MCP pre-image was not restored: %#v", got)
	}
}

...
type registeredToolPendingApproval struct {
	...
	CreatedAt        time.Time
	// ExecutionToken is an opaque, one-time token retained only in memory. It
	// is injected into a request context after the user approves and is never
	// rendered in the AgentView payload or copied into tool arguments.
...
const databaseMutationApprovalWaitText = "Database write needs approval. An approval panel has been opened on the right. Wait for the user to approve; do not retry this write, and do not use bash, ssh, or passthrough."

func (h *IMMessageHandler) emitRegisteredToolApprovalAgentViewIfNeeded(name string, args map[string]interface{}, ctx *SecurityCallContext, policyOwnerID string, execCtx context.Context) bool {
	if h == nil || h.app == nil {
		return false
	}
	if strings.TrimSpace(name) == "database" && database.MutationNeedsHostApproval(execCtx, args) {
		return h.emitDatabaseMutationApproval(args, ctx, policyOwnerID)
	}
	if h.firewall == nil || h.firewall.analyzer == nil {
		return false
	}
...
func (h *IMMessageHandler) emitDatabaseMutationApprovalIfNeeded(ctx context.Context, args map[string]interface{}) (string, bool) {
	if h == nil || !database.MutationNeedsHostApproval(ctx, args) {
		return "", false
	}
	scope := database.RequestScopeFrom(ctx)
	securityCtx := &SecurityCallContext{SessionID: scope.SessionID, UserID: scope.OwnerID}
	if !h.emitDatabaseMutationApproval(args, securityCtx, scope.OwnerID) {
		return "", false
	}
	return databaseMutationApprovalWaitText, true
}

func (h *IMMessageHandler) emitDatabaseMutationApproval(args map[string]interface{}, ctx *SecurityCallContext, policyOwnerID string) bool {
	if h == nil || h.app == nil {
		return false
	}
	sessionID := ""
	...
	risk := security.RiskAssessment{Level: security.RiskHigh, Reason: "database mutation requires explicit host approval", Factors: []string{"database_write", "requires_explicit_user_approval"}}
	if h.firewall != nil && h.firewall.analyzer != nil {
		risk = h.firewall.analyzer.Assess("database", args, ctx)
	}
	principal := trustedAuditPrincipalFromSecurityContext(ctx, policyOwnerID)
	approval := storeRegisteredToolPendingApprovalForPrincipal("database", args, sessionID, policyOwnerID, principal, risk)
	if h.firewall != nil {
		h.firewall.recordAudit("database", args, risk, security.PolicyAsk, "agent_view_approval_pending", sessionID, principal)
	}
	...
	deleteRegisteredToolPendingApproval(approval.ID)
	return false
}

func registeredToolApprovalWaitText(name string, args map[string]interface{}, execCtx context.Context) string {
	if strings.TrimSpace(name) == "database" && database.MutationNeedsHostApproval(execCtx, args) {
		return databaseMutationApprovalWaitText
	}
...
func newRegisteredToolExecutionToken() string {
	bytes := make([]byte, 32)
	if _, err := rand.Read(bytes); err != nil {
		// crypto/rand failures are exceptionally rare; returning an empty token
		// is fail-closed because database mutations will then require approval
		// context and remain blocked.
		return ""
	}
...
func (h *IMMessageHandler) finishProposedDatabaseProfile(args map[string]interface{}) *IMAgentResponse {
	id := agentViewNonEmpty(args["profile_id"])
	if id == "" {
		id = agentViewNonEmpty(args["id"])
	}
	text := avTr("Data source saved to the OS keyring.", "数据源已保存到密钥环。")
	if id == "" || h == nil {
		return &IMAgentResponse{Text: text + " " + avTr("Continue with inspect/query; do not use ssh.", "请 inspect / query，不要用 ssh。"), ResponseSource: imResponseSourceAgentViewSubmit.String()}
	}
	result := h.toolDatabaseWithContext(context.Background(), map[string]interface{}{
		"action": "connect", "profile_id": id,
	})
	if strings.Contains(result, `"needs_secret":true`) {
		return &IMAgentResponse{
			Text:           avTr("Password was saved but did not authenticate. Update it in the task panel.", "密码已保存但仍无法认证。请在任务面板更新密码。"),
			Error:          "needs_secret",
			ResponseSource: imResponseSourceAgentViewSubmit.String(),
		}
	}
	if strings.Contains(result, `"ok":true`) {
		text += " " + avTr("Connected. Continue with inspect/query; do not use ssh.", "已连接。请 inspect / query，不要用 ssh。")
	} else {
		text += " " + avTr("Saved. Keep using the database tool; do not fall back to ssh.", "已保存。请继续使用 database 工具，不要改用 ssh。")
	}
	return &IMAgentResponse{Text: text + "\n" + result, ResponseSource: imResponseSourceAgentViewSubmit.String()}
}
...
func (h *IMMessageHandler) handleRegisteredToolAgentViewSubmit(toolName string, data map[string]interface{}) *IMAgentResponse {
	...
	if toolName == "database" && strings.EqualFold(stringVal(args, "action"), "propose_profile") {
		secret := agentViewNonEmpty(data["password"])
		delete(args, "password")
		if secret != "" && h.app != nil {
			if err := h.app.saveProposedDatabaseProfile(args, secret); err != nil {
				return &IMAgentResponse{Text: "未能保存数据源：" + err.Error(), Error: err.Error(), ResponseSource: imResponseSourceAgentViewSubmit.String()}
			}
			return h.finishProposedDatabaseProfile(args)
		}
		if h.app != nil {
			h.app.emitAgentView(buildDatabaseProposeProfileAgentView(args))
		}
		return &IMAgentResponse{Text: avTr("Enter the password in the task panel. It is stored in the OS keyring, not in chat.", "请在任务面板填写密码。密码写入系统密钥环，不会进入对话。"), Error: "needs_secret", ResponseSource: imResponseSourceAgentViewSubmit.String()}
	}
	if toolName == "database" && strings.EqualFold(fmt.Sprint(args["action"]), "export_excel") {
		if strings.TrimSpace(fmt.Sprint(args["file_path"])) == "" {
			if h.app != nil {
				h.app.emitAgentView(buildDatabaseExportAgentView(args))
			}
			return &IMAgentResponse{Text: "Choose an Excel output path in the task panel.", Error: "missing required parameters: file_path", ResponseSource: imResponseSourceAgentViewSubmit.String()}
		}
	}
	...
	if toolName == "database" || toolName == "database_query" {
		sessionID := "gui"
		if policyOwnerID != "" {
			sessionID = "gui:" + policyOwnerID
		}
		execCtx = database.WithRequestScope(execCtx, database.RequestScope{OwnerID: policyOwnerID, SessionID: sessionID})
	}
	if h.emitRegisteredToolApprovalAgentViewIfNeeded(toolName, args, securityCtx, policyOwnerID, execCtx) {
		return &IMAgentResponse{Text: registeredToolApprovalWaitText(toolName, args, execCtx), ResponseSource: imResponseSourceAgentViewSubmit.String()}
	}

	var result string
	if tool.HandlerCtx != nil {
		result = tool.HandlerCtx(execCtx, args, nil)
	} else if tool.HandlerProg != nil {
...
func (h *IMMessageHandler) handleRegisteredToolApprovalAgentViewSubmit(data map[string]interface{}) *IMAgentResponse {
	...
	execCtx := withTrustedAuditPrincipal(context.Background(), approval.trustedAuditPrincipal())
	if approval.ToolName == "database" && h != nil && h.databaseManager != nil {
		// Database mutations use strict, fingerprint-bound one-time approvals
		// minted by the manager (persisted pending mutation); the generic
		// in-memory ExecutionToken is only the fallback when the pending store
		// is unavailable. The request scope must match the one the direct
		// execution path uses, or the pending mutation's owner/session binding
		// will (correctly) reject the commit.
		scope := database.RequestScope{OwnerID: approval.PolicyOwnerID, SessionID: "gui"}
		if approval.PolicyOwnerID != "" {
			scope.SessionID = "gui:" + approval.PolicyOwnerID
		}
		execCtx = database.WithRequestScope(execCtx, scope)
		execCtx = issueDatabaseApprovalContext(execCtx, h.databaseManager, approval, scope)
	} else if strings.TrimSpace(approval.ExecutionToken) != "" {
		execCtx = database.WithApprovalContext(execCtx, database.ApprovalContext{
...
func registeredToolAgentViewFieldsForSchema(schema map[string]interface{}, requiredNames []string, args map[string]interface{}, missing []string, includeHiddenArgs bool) []map[string]interface{} {
	...
	if includeHiddenArgs {
		fields = append(fields, registeredToolHiddenArgsField(args))
	}
	return fields
}

func registeredToolHiddenArgsField(args map[string]interface{}) map[string]interface{} {
	return map[string]interface{}{
		"name":  registeredToolAgentViewArgsField,
		"label": registeredToolAgentViewArgsField,
		"type":  agentViewFieldTypeHidden.String(),
...
func buildRegisteredToolResultAgentView(tool RegisteredTool, result string) map[string]interface{} {
	...
	if strings.EqualFold(title, "database") || strings.EqualFold(title, "database_query") {
		if view := buildDatabaseResultAgentView(title, result); view != nil {
			return view
		}
	}
	return map[string]interface{}{
...
func buildDatabaseResultAgentView(toolName, result string) map[string]interface{} {
	trimmed := strings.TrimSpace(result)
	if strings.HasPrefix(trimmed, "[") {
		var favorites []map[string]interface{}
		if json.Unmarshal([]byte(trimmed), &favorites) != nil {
			return nil
		}
		items := make([]map[string]interface{}, 0, len(favorites))
		for i, fav := range favorites {
			id := strings.TrimSpace(fmt.Sprint(fav["id"]))
			name := strings.TrimSpace(fmt.Sprint(fav["name"]))
			if name == "" {
				name = id
			}
			item := map[string]interface{}{
				"id":       "fav-" + fmt.Sprint(i),
				"title":    name,
				"subtitle": strings.TrimSpace(fmt.Sprint(fav["sql"])),
				"status":   avTr("saved", "已收藏"),
				"data":     fav,
			}
			var actions []map[string]interface{}
			if id != "" && id != "<nil>" {
				runArgs := map[string]interface{}{"action": "query", "favorite_id": id}
				if profileID := strings.TrimSpace(fmt.Sprint(fav["profile_id"])); profileID != "" && profileID != "<nil>" {
					runArgs["profile_id"] = profileID
				}
				actions = append(actions, map[string]interface{}{
					"label":  avTr("Run", "运行"),
					"viewId": "tool:run:" + toolName,
					"data":   map[string]interface{}{registeredToolAgentViewArgsField: runArgs},
				})
				actions = append(actions, map[string]interface{}{
					"label":  avTr("Delete", "删除"),
					"viewId": "tool:run:" + toolName,
					"data":   map[string]interface{}{registeredToolAgentViewArgsField: map[string]interface{}{"action": "delete_favorite", "favorite_id": id}},
				})
			}
			if len(actions) > 0 {
		...
		if len(items) == 0 {
			items = append(items, map[string]interface{}{
				"id":     "empty",
				"title":  avTr("No saved queries", "没有查询收藏"),
				"status": avTr("done", "完成"),
			})
		}
		return map[string]interface{}{
			...
			"title":       avTr("Saved queries", "查询收藏"),
			"description": avTr("Owner-bound parameterized SELECT snippets.", "当前用户的参数化只读查询收藏。"),
			"results":     items,
		}
	}
	...
	if needsSecret, _ := parsed["needs_secret"].(bool); needsSecret {
		return buildDatabaseProposeProfileAgentView(mergeDatabaseProposeDraft(parsed))
	}
	if rawItems, ok := parsed["items"].([]interface{}); ok {
		return buildDatabaseProfileListAgentView(toolName, rawItems)
	}
	data := map[string]interface{}{}
	for _, key := range []string{"rows", "columns", "warnings", "receipt_id", "next_cursor", "result_handle", "profile_id", "connection_id", "row_count", "job_id", "status", "guidance", "sql", "prompt", "dialect", "tables"} {
		if parsed[key] != nil {
			data[key] = parsed[key]
		}
	}
	parts := make([]string, 0, 6)
	if id, _ := parsed["profile_id"].(string); strings.TrimSpace(id) != "" {
		parts = append(parts, avTr("source ", "来源 ")+strings.TrimSpace(id))
	}
	if parsed["row_count"] != nil {
		parts = append(parts, fmt.Sprintf("%v %s", parsed["row_count"], avTr("rows", "行")))
	}
	if rid, _ := parsed["receipt_id"].(string); strings.TrimSpace(rid) != "" {
		parts = append(parts, "receipt "+strings.TrimSpace(rid))
	}
	if truncated, _ := parsed["truncated"].(bool); truncated {
		parts = append(parts, avTr("truncated", "已截断"))
	}
	if jobID, _ := parsed["job_id"].(string); strings.TrimSpace(jobID) != "" {
		parts = append(parts, "job "+strings.TrimSpace(jobID))
	}
	if status, _ := parsed["status"].(string); strings.TrimSpace(status) != "" {
		parts = append(parts, strings.TrimSpace(status))
	}
	if guidance, _ := parsed["guidance"].(string); strings.TrimSpace(guidance) != "" && parsed["rows"] == nil {
		parts = append(parts, strings.TrimSpace(guidance))
	}
	item := map[string]interface{}{
		"id":     "result",
		"title":  avTr("Output", "输出"),
	...
	if subtitle := strings.Join(parts, " · "); subtitle != "" {
		item["subtitle"] = subtitle
	}
	if actions := databaseResultAgentViewActions(toolName, data); len(actions) > 0 {
		item["actions"] = actions
	}
	return map[string]interface{}{
		"type":        "result_browser",
		"id":          "tool:result:" + toolName,
		"title":       toolName + avTr(" result", " 结果"),
		"description": avTr("Tool execution completed.", "工具执行完成。"),
		"results":     []map[string]interface{}{item},
	}
}

func databaseResultAgentViewActions(toolName string, data map[string]interface{}) []map[string]interface{} {
	var actions []map[string]interface{}
	connectionID := agentViewNonEmpty(data["connection_id"])
	if cursor := agentViewNonEmpty(data["next_cursor"]); cursor != "" {
		pageArgs := map[string]interface{}{"action": "query", "cursor": cursor}
		if connectionID != "" {
			pageArgs["connection_id"] = connectionID
		}
		actions = append(actions, map[string]interface{}{
			"label":  avTr("Next page", "下一页"),
			"viewId": "tool:run:" + toolName,
			"data":   map[string]interface{}{registeredToolAgentViewArgsField: pageArgs},
		})
	}
	if jobID := agentViewNonEmpty(data["job_id"]); jobID != "" {
		status := strings.ToLower(agentViewNonEmpty(data["status"]))
		if status == "queued" || status == "running" {
			actions = append(actions, map[string]interface{}{
				"label":  avTr("Refresh", "刷新"),
				"viewId": "tool:run:" + toolName,
				"data":   map[string]interface{}{registeredToolAgentViewArgsField: map[string]interface{}{"action": "job_status", "job_id": jobID}},
			})
		}
	}
	exportArgs := map[string]interface{}{"action": "export_excel"}
	if handle := agentViewNonEmpty(data["result_handle"]); handle != "" {
		exportArgs["result_handle"] = handle
		if connectionID != "" {
			exportArgs["connection_id"] = connectionID
		}
		actions = append(actions, map[string]interface{}{
			"label":  avTr("Export Excel", "导出 Excel"),
			"viewId": "tool:run:database",
			"data":   map[string]interface{}{registeredToolAgentViewArgsField: exportArgs},
		})
	} else if rows, ok := data["rows"]; ok && rows != nil {
		exportArgs["rows"] = rows
		if connectionID != "" {
			exportArgs["connection_id"] = connectionID
		}
		actions = append(actions, map[string]interface{}{
...
func agentViewNonEmpty(value interface{}) string {
	text := strings.TrimSpace(fmt.Sprint(value))
	if text == "" || text == "<nil>" {
		return ""
	}
...
func buildDatabaseProfileListAgentView(toolName string, rawItems []interface{}) map[string]interface{} {
	...
	for i, raw := range rawItems {
		...
		id := agentViewNonEmpty(p["id"])
		title := agentViewNonEmpty(p["name"])
		if title == "" {
			title = id
		}
		subtitle := strings.TrimSpace(strings.Join([]string{
			agentViewNonEmpty(p["type"]),
			agentViewNonEmpty(p["host"]),
			agentViewNonEmpty(p["database"]),
		}, " "))
		item := map[string]interface{}{
			"id":       "profile-" + fmt.Sprint(i),
			"title":    title,
			"subtitle": subtitle,
			"status":   agentViewNonEmpty(p["status"]),
			"data":     p,
		}
		if id != "" {
			item["actions"] = []map[string]interface{}{{
				"label":  avTr("Connect", "连接"),
				"viewId": "tool:run:" + toolName,
				"data":   map[string]interface{}{registeredToolAgentViewArgsField: map[string]interface{}{"action": "connect", "profile_id": id}},
			}}
		}
		items = append(items, item)
	}
	if len(items) == 0 {
		items = append(items, map[string]interface{}{
			"id":     "empty",
			"title":  avTr("No data sources", "没有数据源"),
			"status": avTr("done", "完成"),
			"data":   map[string]interface{}{"guidance": avTr("Call propose_profile with host/username/database. Do not send the password as a tool argument.", "请调用 propose_profile，只传主机/用户名/库名，不要把密码当工具参数。")},
		})
	}
	return map[string]interface{}{
		"type":        "result_browser",
		"id":          "tool:result:" + toolName,
		"title":       avTr("Data sources", "数据源"),
		"description": avTr("Pick a profile, or add one from chat with propose_profile.", "选择一个 profile，或用 propose_profile 从对话添加。"),
		"results":     items,
	}
}

func parseDatabaseNeedsSecretDraft(result string) (map[string]interface{}, bool) {
	trimmed := strings.TrimSpace(result)
	...
	needsSecret, _ := parsed["needs_secret"].(bool)
	if !needsSecret {
		return nil, false
	}
	return mergeDatabaseProposeDraft(parsed), true
}

func databaseProposeDraftFromArgs(args map[string]interface{}) map[string]interface{} {
	normalized := cloneMISInterfaceMap(args)
	if normalized == nil {
		normalized = map[string]interface{}{}
	}
	if agentViewNonEmpty(normalized["profile_id"]) == "" {
		if id := agentViewNonEmpty(normalized["id"]); id != "" {
			normalized["profile_id"] = id
		}
	}
	p := database.DraftProposedProfile(normalized)
	typ := string(p.Type)
	if typ == "" {
		typ = "mysql"
	}
	...
	if strings.TrimSpace(catalog) == "" {
		catalog = databaseDefaultCatalog(typ)
	}
	port := p.Port
	if port <= 0 {
		port = databaseDefaultPort(typ)
	}
	...
	if _, ok := normalized["read_only"]; ok {
		draft["read_only"] = boolFromAny(normalized["read_only"])
	}
	if boolFromAny(normalized["update_secret"]) {
		draft["update_secret"] = true
	}
	if v := agentViewNonEmpty(normalized["file_path"]); v != "" {
		draft["file_path"] = v
	}
	return draft
}

func databaseDefaultCatalog(typ string) string {
...
func databaseDefaultPort(typ string) int {
	switch strings.ToLower(strings.TrimSpace(typ)) {
	case "postgres", "postgresql":
		return 5432
	case "sqlserver":
...
func databaseResultNeedsHostSecretForm(args map[string]interface{}, result string) bool {
	if args == nil {
		return false
	}
	action := strings.ToLower(strings.TrimSpace(stringVal(args, "action")))
	if action != "connect" && action != "list_connections" {
		return false
	}
	if agentViewNonEmpty(args["host"]) == "" && agentViewNonEmpty(args["file_path"]) == "" {
		return false
	}
	trimmed := strings.TrimSpace(result)
	if trimmed == "" || !strings.HasPrefix(trimmed, "{") {
	...
	if action == "connect" {
		err := strings.ToLower(agentViewNonEmpty(parsed["error"]))
		return err == "profile_not_found"
	}
	items, _ := parsed["items"].([]interface{})
	count, countOK := numberFromAny(parsed["count"])
	if !countOK {
		count = float64(len(items))
	}
	return len(items) == 0 && count == 0
}

func mergeDatabaseProposeDraft(parsed map[string]interface{}) map[string]interface{} {
	draft, _ := parsed["draft"].(map[string]interface{})
	if draft == nil {
		draft = map[string]interface{}{}
	}
	if profile, _ := parsed["profile"].(map[string]interface{}); profile != nil {
		for _, key := range []string{"id", "name", "type", "host", "port", "database", "username"} {
			if draft[key] == nil && profile[key] != nil {
...
func buildDatabaseProposeProfileAgentView(draft map[string]interface{}) map[string]interface{} {
	args := databaseProposeDraftFromArgs(draft)
	args["action"] = "propose_profile"
	typ := strings.ToLower(agentViewNonEmpty(args["type"]))
	if typ == "" {
		typ = "mysql"
	}
	args["type"] = typ
	strField := func(name, label, value string, required bool) map[string]interface{} {
		field := map[string]interface{}{
			"name":     name,
			"label":    label,
			"type":     agentViewFieldTypeText.String(),
			"required": required,
		}
		if value != "" {
			field["value"] = value
	...
	databaseField := strField("database", avTr("Database", "数据库名"), agentViewNonEmpty(args["database"]), true)
	databaseField["placeholder"] = "mysql"
	portField := map[string]interface{}{
		"name":     "port",
		"label":    avTr("Port", "端口"),
		"type":     agentViewFieldTypeNumber.String(),
		"required": false,
		"min":      1,
		"max":      65535,
	}
	if n, ok := numberFromAny(args["port"]); ok && n > 0 {
		portField["value"] = int(n)
	}
	title := avTr("Add data source", "添加数据源")
	description := avTr("Password is stored in the OS keyring and never sent back to the model.", "密码写入系统密钥环，不会回到模型上下文。")
	if boolFromAny(args["update_secret"]) {
		title = avTr("Update password", "更新数据源密码")
		description = avTr("The saved password could not connect. Enter a new password. It stays in the OS keyring.", "已保存的密码无法连接。请输入新密码，仍只写入系统密钥环。")
	}
	return attachAgentViewSchemaVersion(map[string]interface{}{
		"type":        "form",
		"id":          "tool:run:database",
		"title":       title,
		"description": description,
		"fields": []map[string]interface{}{
			strField("profile_id", avTr("ID", "ID"), agentViewNonEmpty(args["profile_id"]), true),
			strField("name", avTr("Name", "名称"), agentViewNonEmpty(args["name"]), false),
			{
				"name":     "type",
				"label":    avTr("Type", "类型"),
				"type":     agentViewFieldTypeSelect.String(),
				"required": true,
				"value":    typ,
				"options":  []string{"mysql", "postgres", "sqlserver"},
			},
			strField("host", avTr("Host", "主机"), agentViewNonEmpty(args["host"]), true),
			portField,
			databaseField,
			strField("username", avTr("Username", "用户名"), agentViewNonEmpty(args["username"]), false),
			{
				"name":      "password",
				"label":     avTr("Password", "密码"),
				"type":      agentViewFieldTypeText.String(),
				"required":  true,
				"sensitive": true,
				"format":    "password",
			},
			registeredToolHiddenArgsField(args),
		},
		"submitLabel": avTr("Save", "保存"),
		"hiddenData": map[string]interface{}{
			registeredToolAgentViewArgsField: args,
		},
		"meta": map[string]interface{}{"source": "tool.adapter", "tool": "database"},
	}, "tool.adapter", "database", nil)
}

func buildDatabaseExportAgentView(args map[string]interface{}) map[string]interface{} {
	return attachAgentViewSchemaVersion(map[string]interface{}{
		"type":        "form",
		"id":          "tool:run:database",
		"title":       avTr("Export Excel", "导出 Excel"),
		"description": avTr("Choose a workspace .xlsx path. Existing files are not overwritten without approval.", "选择工作区内的 .xlsx 路径。已有文件不会在未审批时覆盖。"),
		"fields": []map[string]interface{}{{
			"name":     "file_path",
			"label":    avTr("Output file", "输出文件"),
			"type":     agentViewFieldTypeText.String(),
			"required": true,
		}, registeredToolHiddenArgsField(args)},
		"submitLabel": avTr("Export", "导出"),
		"hiddenData": map[string]interface{}{
			registeredToolAgentViewArgsField: cloneMISInterfaceMap(args),
		},
		"meta": map[string]interface{}{"source": "tool.adapter", "tool": "database"},
...
func storeRegisteredToolPendingApprovalForPrincipal(toolName string, args map[string]interface{}, sessionID, policyOwnerID, auditPrincipalID string, risk security.RiskAssessment) registeredToolPendingApproval {
	...
	item := registeredToolPendingApproval{
		...
		Risk:             risk,
		CreatedAt:        time.Now(),
		ExecutionToken:   newRegisteredToolExecutionToken(),
	}
...
func buildRegisteredToolApprovalAgentView(approval registeredToolPendingApproval) map[string]interface{} {
	...
	if approval.ToolName == "database" || approval.ToolName == "database_query" {
		if action := strings.TrimSpace(fmt.Sprint(approval.Args["action"])); action != "" {
			reviewData["action"] = action
		}
		if sql := strings.TrimSpace(fmt.Sprint(approval.Args["sql"])); sql != "" {
			reviewData["sql"] = sql
		}
		if approval.Args["params"] != nil {
			reviewData["params"] = maskDatabasePreviewParams(approval.Args["params"])
		}
		if profile := strings.TrimSpace(fmt.Sprint(approval.Args["profile_id"])); profile != "" {
			reviewData["profile_id"] = profile
		}
	}
	return map[string]interface{}{
		"type":         "approval",
...
func redactAgentViewSubmitSecrets(data map[string]interface{}) map[string]interface{} {
	out := cloneMISInterfaceMap(data)
	if out == nil {
		return nil
	}
	for key, val := range out {
		...
		if strings.Contains(lower, "password") || strings.Contains(lower, "secret") || strings.Contains(lower, "token") || strings.Contains(lower, "passwd") {
			out[key] = "***"
			continue
		}
		if nested, ok := val.(map[string]interface{}); ok {
			out[key] = redactAgentViewSubmitSecrets(nested)
		}
	}
	return out
}

func maskDatabasePreviewParams(v interface{}) interface{} {
...
// nonEmptyStringFromAny delegates to the shared agentruntime helper.
func nonEmptyStringFromAny(value interface{}) string {
	return agentruntime.NonEmptyStringFromAny(value)
}

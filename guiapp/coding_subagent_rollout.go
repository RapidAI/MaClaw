...
type RolloutEntry struct {
	...
	RuntimeStatus agentruntime.JobStatus `json:"runtime_status,omitempty"`
}
...
func NewSubAgentRollout(dir, taskID string) (*SubAgentRollout, error) {
	...
	r.appendEntry(RolloutEntry{Type: "status", Status: "running", RuntimeStatus: agentruntime.JobStatusRunning})
	return r, nil
}
...
func (r *SubAgentRollout) Complete() {
	...
	r.appendEntry(RolloutEntry{Type: "status", Status: "completed", RuntimeStatus: agentruntime.JobStatusSucceeded})
	r.Close()
}
...
func (r *SubAgentRollout) Fail(errMsg string) {
	...
	r.appendEntry(RolloutEntry{Type: "status", Status: "failed", RuntimeStatus: agentruntime.JobStatusFailed, Summary: truncateRunesForRollout(errMsg, 200)})
	r.Close()
}

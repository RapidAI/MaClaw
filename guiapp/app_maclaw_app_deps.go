...
// updateInstalledMaclawAppDependency refreshes a mismatched Hub/SkillMarket
// dependency through the shared staged directory committer. The installed
// registry entry supplies the authoritative target directory; the plan's
// InstalledDir is only a legacy fallback when no registered path exists.
func (a *App) updateInstalledMaclawAppDependency(dep *maclawAppInstallPlanDependency) (bool, error) {
	...
	existing := a.installedMixedSkillForUpdate(downloadID, name)
	if existing == nil {
		return true, fmt.Errorf("installed dependency skill %q is not registered", name)
	}
	...
	if expected := filepath.Clean(cskill.PlannedSkillDir(targetRoot, name)); targetDir != expected {
		// Do not route an automatic update through a path the shared transaction
		// cannot prove it owns. The previous adapter tried to preserve arbitrary
		// layouts with a bespoke register/index/audit sequence; that left a second
		// production mutation protocol. Keep the installed package untouched and
		// require an explicit manual reinstall instead.
		return true, fmt.Errorf("dependency skill %q has a non-canonical installation directory; automatic update is blocked pending manual reinstall", name)
	}
	if err := os.MkdirAll(targetRoot, 0o755); err != nil {
		return true, fmt.Errorf("create dependency update staging root: %w", err)
	}
	stagingDir, err := os.MkdirTemp(targetRoot, ".maclaw-app-dep-update-*")
	if err != nil {
		return true, fmt.Errorf("create dependency update staging dir: %w", err)
	}
	defer cskill.CleanupStaging(stagingDir)
	updated, downloadTrace, err := downloadSkillJSONFromHubCenterLocatorToDirWithIntegrityTrace(
		context.Background(), a, dep.PackageDownloadURL,
		"/api/v1/skills/"+url.PathEscape(downloadID)+"/download", stagingDir,
		firstNonEmpty(dep.PackageSHA256, dep.PackageChecksum), dep.PackageSignature,
	)
	...
	if err := a.commitStagedSkillInstallWithExistingInRoot(
		context.Background(), updated, stagingDir, "maclaw_app_dependency", report,
		requestID, skillEvolutionConfigRevision(a), existing, targetRoot,
	); err != nil {
		return true, fmt.Errorf("dependency update not committed: %w", err)
	}
...
// updateInstalledMaclawAppDependencyLegacy is retained temporarily for source
// compatibility with historical in-package experiments. Production updates do
// not call it: non-canonical directories are rejected above rather than being
// sent through its legacy direct register/index/audit protocol.
func (a *App) updateInstalledMaclawAppDependencyLegacy(dep *maclawAppInstallPlanDependency) (bool, error) {
	if a == nil || dep == nil {
		return false, nil
	}
	if err := cskill.CheckEvolutionCompensationQueue(); err != nil {
	...
	// Never overwrite a stale backup left by an earlier interrupted update.
	// Treating it as this transaction's backup could destroy the only known
	// pre-image and make recovery ambiguous.
	previousDir := targetDir + ".prev"
	if _, prevErr := os.Stat(previousDir); prevErr == nil {
		return true, fmt.Errorf("dependency skill destination has a stale previous backup: %s", filepath.Base(previousDir))
	} else if !os.IsNotExist(prevErr) {
		return true, fmt.Errorf("inspect previous dependency skill backup: %w", prevErr)
	}
	...
	compensation.FinalAuditKind = cskill.KindFromEventName("skill:definition_installed")
	compensation.SetCreatedDirectories([]string{targetDir})
	// Cleanup artifacts are removed only after the final audit. Persisting them
	// makes a crash or cleanup failure an explicit committed/pending state.
	compensation.SetPostCommitCleanupPaths([]string{stagingDir, previousDir})
	...
	defer func() {
		...
		if rollbackErr := cskill.RestoreEvolutionCompensation(compensation, func(skills []corelib.NLSkillEntry) error { return a.skillExecutor.restoreSkillsSnapshot(skills) }, func() error { return a.refreshSkillIndexesAfterMutationChecked(name) }); rollbackErr != nil {
			...
			if markErr := cskill.MarkEvolutionCompensationRollbackFailure(&compensation, rollbackErr); markErr != nil {
				log.Printf("[maclaw.app] persist dependency rollback state failed for %s: %v", name, markErr)
			}
			return
		}
		// A successful pre-commit rollback must retire its durable record. If
		// this clear fails, retain the record and mark the rollback incomplete so
		// startup admission remains fail-closed instead of leaking a permanently
		// pending snapshot after a clean restore.
		if clearErr := cskill.ClearEvolutionCompensation(requestID, name, "install"); clearErr != nil {
			compensation.LastError = clearErr.Error()
			if markErr := cskill.MarkEvolutionCompensationRollbackFailure(&compensation, clearErr); markErr != nil {
				log.Printf("[maclaw.app] persist dependency rollback cleanup state failed for %s: %v", name, markErr)
			}
			return
		}
	...
	finalAudited = true
	// Cross the business boundary before attempting cleanup. Recovery must
	// never restore an audited dependency solely because cleanup is unavailable.
	...
	if err := cskill.ReplaceEvolutionCompensation(compensation); err != nil {
		return true, fmt.Errorf("persist committed dependency compensation: %w", err)
	}
	if backupDir != "" {
		if err := os.RemoveAll(backupDir); err != nil {
			cleanupErr := fmt.Errorf("dependency update backup cleanup pending: %w", err)
			if markErr := cskill.MarkEvolutionCompensationCleanupFailure(&compensation, cleanupErr); markErr != nil {
				return true, fmt.Errorf("%v; persist cleanup failure: %w", cleanupErr, markErr)
			}
	...
	if err := cskill.ClearEvolutionCompensation(requestID, name, "install"); err != nil {
		cleanupErr := fmt.Errorf("dependency update cleanup pending: %w", err)
		if markErr := cskill.MarkEvolutionCompensationCleanupFailure(&compensation, cleanupErr); markErr != nil {
			return true, fmt.Errorf("%v; persist cleanup failure: %w", cleanupErr, markErr)
		}
		return true, cleanupErr
...
func (a *App) updateRegisteredMaclawAppDependencySkill(updated corelib.NLSkillEntry) error {
	...
	// This helper is retained for the legacy maclaw.app replacement adapter,
	// but it still writes the authoritative registry. Keep it inside the same
	// process-wide mutation boundary as staged installs and re-check admission
	// after waiting for the lock so a queue transition cannot be bypassed.
...
func (a *App) installBundledMaclawAppSkill(bundle maclawAppBundledSkillEntry) error {
	...
	// Keep staging on the same managed data volume as the final Skill root so
	// the shared directory transaction can publish it atomically. os.TempDir()
	// may be another volume on Windows, where Rename would otherwise fail after
	// a successful scan and leave this path falling back to ad-hoc registration.
	stagingRoot, err := a.skillStagingDir()
	if err != nil {
		return fmt.Errorf("resolve bundled skill staging directory: %w", err)
	}
	if err := os.MkdirAll(stagingRoot, 0o755); err != nil {
		return fmt.Errorf("create bundled skill staging directory: %w", err)
	}
	tmpDir, err := os.MkdirTemp(stagingRoot, "maclaw-app-bundled-skill-*")
	if err != nil {
	...
	requestID := fmt.Sprintf("evo_maclaw_app_bundle_%d", time.Now().UnixNano())
	if err := a.commitStagedSkillInstall(
		context.Background(), entry, tmpDir, "maclaw_app_bundle", report,
		requestID, skillEvolutionConfigRevision(a),
	); err != nil {
		return fmt.Errorf("commit bundled dependency skill %q: %w", entry.Name, err)
	}
	return nil
}

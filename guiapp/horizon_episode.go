...
func (h *IMMessageHandler) runHorizonEpisode(ctx context.Context, sess *horizonSession, ep longhorizon.EpisodeContext) string {
	...
	cfg := h.applyCodingRoutePreference(ownerID, h.horizonModelForRole(ep.Role, h.getCodingLLMConfig()), false)
	if episodeTimeout := horizonEpisodeTimeout(ep); episodeTimeout > 0 {
		var timeoutCancel context.CancelFunc
		ctx, timeoutCancel = context.WithTimeout(ctx, episodeTimeout)
		defer timeoutCancel()
	}

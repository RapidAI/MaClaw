...
func (c *codingSubAgentCallbacks) codingDynamicAliasesMayMaterialize() bool {
	// Dynamic Skill/MCP aliases remain closed until the callback has a real
	// scope-bound planner/admission handoff. Transport/lifecycle qualification
	// and the hermetic relay override are evidence only; they must not let a
	// callback derive a surface from the process-wide dynamic catalog.
	return false
}

func (c *remoteCodingCallbacks) codingDynamicAliasesMayMaterialize() bool {
	// Remote Coding has the same scope/admission requirement as local Coding.
	return false
}
...
// codingDynamicFirstFamilyLegacyNames is the frozen E5 inventory boundary.
// These names are intentionally kept in one place so a future addition cannot
// silently widen the atomic cutover.  The semantic holder may expose the same
// capabilities through opaque, request-bound aliases; these model-visible
// names must never be treated as selectors once that holder is attached.
var codingDynamicFirstFamilyLegacyNames = map[string]struct{}{
	"Glob":           {},
	"ripgrep":        {},
	"read_file":      {},
	"list_directory": {},
...
var (
	codingDynamicFirstFamilyLegacyRemovalOnce sync.Once
	codingDynamicFirstFamilyLegacyRemovalOK   bool
)

func isCodingDynamicFirstFamilyLegacyName(name string) bool {
	_, ok := codingDynamicFirstFamilyLegacyNames[strings.TrimSpace(name)]
	return ok
}

// codingDynamicFirstFamilyLegacyRemoved is the E5 atomic-cutover marker.  It
// is deliberately a predicate over the actual semantic cutover seams rather
// than an unconditional release bool: the fixed read-only inventory, its
// project-bound executor and the durable surface publisher must all remain
// present before the production composition can claim that the old family has
// a replacement.  Model dispatch is additionally fenced below whenever a
// qualified holder is attached, so stale names cannot reach the compatibility
// dispatcher through a direct callback entry point.
func codingDynamicFirstFamilyLegacyRemoved() bool {
	codingDynamicFirstFamilyLegacyRemovalOnce.Do(func() {
		if len(codingDynamicFirstFamilyLegacyNames) != 5 {
			return
		}
		// The replacement must be the reviewed, project-bound semantic adapter;
		// merely having the old inventory or a generic dynamic catalog is
		// insufficient.  The compatibility inventory may remain as an audit
		// record for unqualified transports, but it is not evidence of cutover.
		identity := &trustedCodingInvocationIdentity{TenantID: "e5", PrincipalID: "e5", SessionID: "e5", RootTaskID: "e5", TurnID: "e5"}
		providers, err := codingStaticReadOnlyProviderSpecs(codingStaticExecutionEnvelope{
			Identity: identity, Workspace: codingStaticWorkspaceBinding{WorkspaceHandle: "e5", HostKind: codingStaticCompatibilityHostLocal},
		})
		if err != nil || len(providers) != 2 || providers[0].AdapterName != codingStaticReadAdapter || providers[1].AdapterName != codingStaticRepoAdapter {
			return
		}
		// Exercise the actual model-dispatch fence as part of the predicate.
		// An inert relay is sufficient: the call must terminate at the
		// catalog_incomplete rejection and never enter a name dispatcher.
		local := &codingSubAgentCallbacks{dynamicLifecycleRelay: &codingBoundDynamicRequestLifecycleRelay{}}
		remote := &remoteCodingCallbacks{dynamicLifecycleRelay: &codingBoundDynamicRequestLifecycleRelay{}}
		for name := range codingDynamicFirstFamilyLegacyNames {
			if !strings.Contains(local.ExecuteToolStructured(name, `{}`).Result, "catalog_incomplete") || !strings.Contains(remote.ExecuteToolStructured(name, `{}`).Result, "catalog_incomplete") {
				return
			}
		}
		codingDynamicFirstFamilyLegacyRemovalOK = true
	})
	return codingDynamicFirstFamilyLegacyRemovalOK
}

// isLegacyCodingDynamicGateway remains a deny-only compatibility query for
// older host code.  The E5 cutover removed executable selector cases; this
// guard preserves the stable catalog_incomplete rejection envelope for stale
// model payloads without dispatching either gateway.
...
func (c *codingSubAgentCallbacks) BuildToolsForModelRequest(userText string, iteration int) []map[string]interface{} {
	...
	// Once the qualified composition is attached, the dynamic holder is the
	// sole model-facing surface.  Never merge the legacy static definitions
	// into that request (the E5 first-family cutover removes the old bridge).

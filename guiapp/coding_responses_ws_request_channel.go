...
// codingResponsesWSAdapterVersion is the compile-time version of the
// repository's own Responses WebSocket single-use channel adapter: the frame
// builder, the streaming parser path (provider response ID and per-call
// provider tool-call ID extraction), and the cancellation/replay fences. Any
// behavioral change in those paths must bump this constant together with the
// loopback conformance suite version; their equality test is the E1 machine
// evidence behind the qualification AdapterVersion field.
const codingResponsesWSAdapterVersion = "responses-ws-adapter-v1"

// codingResponsesWSPolicyProjectionVersion identifies the fixed, host-owned
// mapping of tool_choice/parallel_tool_calls into the Responses envelope. The
// mapping is deliberately not inferred from provider, model, or conversation
// content; the replacement-semantics contract suite pins every row of it
// against this constant.
const codingResponsesWSPolicyProjectionVersion = "responses-ws-policy-projection-v1"

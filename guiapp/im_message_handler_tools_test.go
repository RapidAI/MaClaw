...
func TestBuildToolDefinitionsCapInlinePayloads(t *testing.T) {
	...
	// edit_file advertises the shared core maxLength so the model splits large
	// replacements; runtime still auto-passes oversized payloads.
	for _, propName := range []string{"old_string", "new_string"} {
		if _, exists := editProps[propName]; !exists {
			t.Fatalf("edit_file missing property %s", propName)
		}
		if got := toolSchemaMaxLength(t, editProps, propName); got != 1800 {
			t.Fatalf("edit_file %s maxLength = %v, want 1800", propName, got)
		}
	}
...
func TestBuiltinRegistryCapsInlinePayloads(t *testing.T) {
	...
	for _, propName := range []string{"old_string", "new_string"} {
		if got := registeredToolSchemaMaxLength(t, regEditProps, propName); got != 1800 {
			t.Fatalf("registry edit_file %s maxLength = %v, want 1800", propName, got)
		}
	}
...
func TestDiscoverToolActivatesDeferredToolDefinition(t *testing.T) {
	...
	if !strings.Contains(out, "gui_observe") || !strings.Contains(out, "granted for the next request") {
		t.Fatalf("discover output should grant gui_observe, got %q", out)
	}

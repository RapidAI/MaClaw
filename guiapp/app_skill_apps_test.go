...
func TestRecordMaclawAppRunEvidenceForSkillDoesNotRewriteUnchangedDefinition(t *testing.T) {
	tmpHome := t.TempDir()
	t.Setenv("USERPROFILE", tmpHome)
	t.Setenv("HOME", tmpHome)
	...
	first, err := app.RecordMaclawAppRunEvidenceForSkill("invoice-app", "skill-app-invoice-app-invoice-review", "feedbeef", "run-ok-1", filepath.Join(tmpHome, "out", "invoice.pdf"), "2026-06-17T10:00:00Z")
	if err != nil || first["manifest_updated"] != true {
		t.Fatalf("first evidence result = %#v, err=%v", first, err)
	}
	path := filepath.Join(skillDir, "maclaw.app.json")
	before, err := os.ReadFile(path)
	if err != nil {
		t.Fatalf("ReadFile before second run: %v", err)
	}

	second, err := app.RecordMaclawAppRunEvidenceForSkill("invoice-app", "skill-app-invoice-app-invoice-review", "feedbeef", "run-ok-2", filepath.Join(tmpHome, "out", "invoice-second.pdf"), "2026-06-18T10:00:00Z")
	if err != nil {
		t.Fatalf("Record second evidence: %v", err)
	}
	if second["manifest_updated"] != false {
		t.Fatalf("second evidence should not rewrite unchanged definition: %#v", second)
	}
	...
	if string(after) != string(before) {
		t.Fatal("unchanged definition's manifest was rewritten by second successful run")
	}
	history, err := app.ListMaclawAppRunHistory("skill-app-invoice-app-invoice-review", 10)
	if err != nil {
		t.Fatalf("ListMaclawAppRunHistory: %v", err)
	}
	if len(history) != 2 || history[0].RunID != "run-ok-2" || history[1].RunID != "run-ok-1" {
		t.Fatalf("durable run history = %#v, want both runs", history)
	}
}

func TestMaclawAppRunEvidenceAlreadyProvesDefinitionSupportsLegacyFields(t *testing.T) {
	legacyEvidence := map[string]any{
		"run_id":                 "run-previous",
		"verified_at":            "2026-06-17T10:00:00Z",
		"definition_fingerprint": "feedbeef",
		"artifact_present":       true,
	}
	incoming := map[string]any{
		"runId":           "run-current",
		"verifiedAt":      "2026-06-18T10:00:00Z",
		"definitionHash":  "feedbeef",
		"artifactPresent": true,
	}
	if !maclawAppRunEvidenceAlreadyProvesDefinition(legacyEvidence, incoming) {
		t.Fatalf("legacy run evidence should prove the unchanged definition: %#v", legacyEvidence)
	}
}

func TestRecordMaclawSingleAppRunEvidenceConcurrentFirstRunsWriteOnce(t *testing.T) {
	skillDir := t.TempDir()
	if err := os.WriteFile(filepath.Join(skillDir, "maclaw.app.json"), []byte(`{
  "app": {"id": "invoice-review", "governance": {}}
}`), 0o644); err != nil {
		t.Fatalf("WriteFile maclaw.app.json: %v", err)
	}
	...
	for _, runID := range []string{"run-first", "run-second"} {
		...
		go func() {
			defer wg.Done()
			<-start
			result, err := recordMaclawSingleAppRunEvidence("invoice-app", skillDir, "invoice-review", map[string]any{
				"runId":           runID,
				"verifiedAt":      "2026-06-18T10:00:00Z",
				"definitionHash":  "feedbeef",
				"artifactPresent": true,
	...
	close(errs)
	for err := range errs {
		t.Fatalf("record concurrent evidence: %v", err)
	}

	updated := 0
	for result := range results {
		if result["manifest_updated"] == true {
			updated++
		}
	}
	if updated != 1 {
		t.Fatalf("manifest updates = %d, want exactly one", updated)
	}
}
...
func TestRecordMaclawAppRunEvidenceForSkillUpdatesMaclawAppsManifest(t *testing.T) {
	...
	second, err := app.RecordMaclawAppRunEvidenceForSkill("doc-tools", "skill-app-doc-tools-redact", "c0ffee", "run-redact-2", filepath.Join(tmpHome, "out", "redacted-second.pdf"), "2026-06-19T10:00:00Z")
	if err != nil {
		t.Fatalf("Record second evidence: %v", err)
	}
	if second["manifest_updated"] != false {
		t.Fatalf("second evidence should not rewrite unchanged multi-app definition: %#v", second)
	}
	after, err := os.ReadFile(filepath.Join(skillDir, "maclaw.apps.json"))
	if err != nil {
		t.Fatalf("ReadFile maclaw.apps.json after second run: %v", err)
	}
	if string(after) != string(data) {
		t.Fatal("unchanged multi-app manifest was rewritten by second successful run")
	}
}

...
// RuntimeStatusValue projects the GUI's historical skill-run vocabulary onto
// the shared asynchronous Runtime job status contract.
func (s *SkillRunStatus) RuntimeStatusValue() agentruntime.JobStatus {
	if s == nil {
		return agentruntime.JobStatusUnknown
	}
	return agentruntime.ProjectLegacyJobStatus(s.LifecycleStatus().String())
}

...
func (h *IMMessageHandler) executeAgentLoopToolCall(opts agentLoopToolExecutionOptions) (result toolExecutionResult) {
	...
	if rewrittenName, rewrittenArgs, rewritten := agentruntime.CanonicalizeToolCallJSON(tc.Function.Name, tc.Function.Arguments); rewritten {
...
func normalizeAgentLoopToolArgumentsJSON(argsJSON string) string {
	return agentruntime.NormalizeToolArgumentsJSON(argsJSON)
}
...
func (h *IMMessageHandler) executeCodingWorkflowDelegateTask(opts agentLoopToolExecutionOptions) (toolExecutionResult, bool) {
	cleaned := agentruntime.NormalizeToolArgumentsJSON(opts.ToolCall.Function.Arguments)
...
func (h *IMMessageHandler) isWorkflowToolCallAllowedForOwner(policyUserID, name, argsJSON string) (bool, string) {
	...
	if strings.TrimSpace(argsJSON) != "" {
		if parsed, err := agentruntime.ParseToolArgumentsObject(argsJSON); err != nil {
...
func rewriteInternalBrowserToolCall(name string, args map[string]interface{}) (string, map[string]interface{}, bool) {
	return agentruntime.CanonicalizeBrowserToolCall(name, args)
}

func rewriteInternalBrowserToolCallJSON(name, argsJSON string) (string, string, bool) {
	return agentruntime.CanonicalizeBrowserToolCallJSON(name, argsJSON)
}
...
func rejectUnstableBrowserToolCallJSON(name, argsJSON string) string {
	...
	if unsupported := agentruntime.UnsupportedBrowserAction(name, map[string]any{"action": action}); unsupported != "" {
		return fmt.Sprintf("browser action %s is disabled in the stable browser mechanism; use session_start plus observe/click/type/wait/extract/task_run", unsupported)
	}
	return ""
}
...
func (h *IMMessageHandler) executeToolDetailedWithRuntimeContextAndContextTokensAndTaskAnchor(execCtx context.Context, policyUserID string, hasRuntimeOwner bool, runtimePlatform string, contextTokens int, taskAnchor *taskIdentityAnchor, name, argsJSON, userText string, onProgress coretool.ProgressCallback) (result toolExecutionResult) {
	...
	if taskText == "" && h != nil && strings.TrimSpace(policyUserID) != "" {
		// Only the current owner loop — never the leftover desktop snapshot
		// from another tab.
		taskText = strings.TrimSpace(h.runtimeTaskTextForOwner(policyUserID))
	}
	if reason := rejectSSHForDatabaseIntent(name, taskText); reason != "" {
		return toolExecutionResult{Text: "[system rejected] " + reason, ToolName: name, ToolKind: kind, Outcome: toolOutcomeFailed, FailureKind: toolFailurePolicyRejected}
	}
	defer func() {
		if r := recover(); r != nil {
	...
	var args map[string]interface{}
	if argsJSON != "" {
		cleaned := agentruntime.NormalizeToolArgumentsJSON(argsJSON)
		if cleaned != "" {
	...
	if reason := rejectBashForDatabaseIntent(name, args, taskText); reason != "" {
		return toolExecutionResult{Text: "[system rejected] " + reason, ToolName: name, ToolKind: kind, Outcome: toolOutcomeFailed, FailureKind: toolFailurePolicyRejected}
	}
	...
	if rewrittenName, rewrittenArgs, rewritten := canonicalizeLocalFileSearchToolCall(name, args); rewritten {
		name = rewrittenName
		args = rewrittenArgs
		kind = classifyAgentToolKind(name)
	}
	...
	if name == "database" || name == "database_query" {
		// The GUI registry is a legacy app-level adapter, so bind its database
		// manager through the authenticated execution context as well. Never use
		// model-provided session_id values for this binding; owner identity is
		// trusted metadata and the manager remains process/session scoped.
		ownerID := trustedAuditPrincipalFromContext(execCtx, policyUserID)
		sessionID := "gui"
		if ownerID != "" {
			sessionID = "gui:" + ownerID
		}
		execCtx = database.WithRequestScope(execCtx, database.RequestScope{OwnerID: ownerID, SessionID: sessionID})
	}
	...
	if groupPermissions != nil {
		...
		case "read_file", "list_directory", "search_files", "Glob", "glob", "send_file", "send_to_im", "archive":
	...
	if h.registry != nil {
		if tool, ok := h.registry.Get(name); ok {
			...
			if h.emitRegisteredToolApprovalAgentViewIfNeeded(name, args, securityCtx, policyUserID, execCtx) {
				return toolExecutionResult{Text: registeredToolApprovalWaitText(name, args, execCtx), Outcome: toolOutcomeUncertain, FailureKind: toolFailureApprovalRequired}
			}
			if h.firewall != nil {
...
var latinCatalogName库Re = regexp.MustCompile(`(?i)[a-z][a-z0-9_-]*库`)

func sqlDatabaseFollowUpUserText(userText string) bool {
	if databaseIntentUserText(userText) {
		return true
	}
	if strings.Contains(userText, "数据源") || strings.Contains(userText, "表结构") {
		return true
	}
	return latinCatalogName库Re.MatchString(userText)
}

func databaseIntentUserText(userText string) bool {
	text := strings.ToLower(userText)
	if strings.TrimSpace(text) == "" {
		return false
	}
	return strings.Contains(text, "mysql") ||
		strings.Contains(text, "mariadb") ||
		strings.Contains(text, "postgres") ||
		strings.Contains(text, "sqlserver") ||
		strings.Contains(text, "sql server") ||
		strings.Contains(text, "数据库")
}

var unsupportedExternalDatabaseRe = regexp.MustCompile(`(?i)(?:^|[^A-Za-z0-9_\p{L}])(oracle|mongodb|mongo|redis|sqlite|clickhouse|oceanbase|tidb|hive|cassandra|neo4j|elasticsearch|influxdb|couchdb|kingbase)(?:[^A-Za-z0-9_\p{L}]|$)`)

func unsupportedExternalDatabaseUserText(userText string) bool {
	text := strings.TrimSpace(userText)
	if text == "" {
		return false
	}
	lower := strings.ToLower(text)
	if strings.Contains(lower, "达梦") || strings.Contains(lower, "人大金仓") {
		return true
	}
	return unsupportedExternalDatabaseRe.MatchString(text)
}

func explicitRemoteShellUserText(userText string) bool {
	text := strings.ToLower(userText)
	return strings.Contains(text, "ssh") || strings.Contains(text, "登录服务器") || strings.Contains(text, "远程登录")
}

func rejectSSHForDatabaseIntent(toolName, userText string) string {
	if !strings.EqualFold(strings.TrimSpace(toolName), "ssh") {
		return ""
	}
	if explicitRemoteShellUserText(userText) {
		return ""
	}
	if unsupportedExternalDatabaseUserText(userText) {
		return ""
	}
	if databaseIntentUserText(userText) {
		return "this request is for a database connection; use database / database_query, not ssh"
	}
	return ""
}

var mysqlClientCommandRe = regexp.MustCompile(`(?i)(?:^|[^A-Za-z0-9_])(mysql|mysqldump|mariadb|mariadb-dump|psql|sqlcmd)(?:[^A-Za-z0-9_]|$)`)

func rejectBashForDatabaseIntent(toolName string, args map[string]interface{}, userText string) string {
	...
	cmd := strings.TrimSpace(stringVal(args, "command"))
	if cmd == "" {
		cmd = strings.TrimSpace(stringVal(args, "cmd"))
	}
	if cmd == "" {
		return ""
	}
	if mysqlClientCommandRe.MatchString(cmd) {
		return "this request is for a database connection; use database / database_query, not bash mysql/psql/sqlcmd"
	}
	return ""
}
...
// Delegates to the shared agentruntime admission gate.
func manageScheduleCreateBlockReason(name string, args map[string]interface{}, userText string) string {
	return agentruntime.ManageScheduleCreateBlockReason(name, args, userText)
}
...
func toolAcceptsRuntimePolicyOwnerArg(name string) bool {
	...
	case "bash",
		"read_file", "read_tool_result", "write_file", "edit_file", "edit_lines", "list_directory", "Glob", "search_files", "send_file", "send_to_im", "archive",
		"office",
...
func registeredToolExecutionResult(text string) toolExecutionResult {
	if isRegisteredToolApprovalWaitText(text) {
		return toolExecutionResult{Text: text, Outcome: toolOutcomeUncertain, FailureKind: toolFailureApprovalRequired}
	}
...
func isRegisteredToolApprovalWaitText(text string) bool {
	trimmed := strings.TrimSpace(text)
	if trimmed == "" {
		return false
	}
	return trimmed == databaseMutationApprovalWaitText || strings.Contains(trimmed, "An approval panel has been opened")
}
...
// inferRegisteredToolOutcome delegates to the shared agentruntime three-state
// classifier so GUI, srv and TUI apply the same MCP/PDF/handler failure
// marker vocabulary to legacy text-only registered tool responses.
func inferRegisteredToolOutcome(text string) toolOutcome {
	switch agentruntime.RegisteredToolTextOutcome(text) {
	case agentruntime.RegisteredToolOutcomeFailed:
		return toolOutcomeFailed
	case agentruntime.RegisteredToolOutcomeSucceeded:
...
func (h *IMMessageHandler) preCheckToolArgsForAgentLoop(name, argsJSON string, iteration int) *toolExecutionResult {
	...
	var args map[string]interface{}
	if cleaned := agentruntime.NormalizeToolArgumentsJSON(argsJSON); cleaned != "" {
		parsed, err := parseAgentToolArgumentsObject(cleaned)
		if err != nil {
...
// normalizeMCPToolCallArgsForAgentLoop delegates to the shared agentruntime
// MCP call-envelope normalizer.
func normalizeMCPToolCallArgsForAgentLoop(args map[string]interface{}) (map[string]interface{}, error) {
	return agentruntime.NormalizeMCPToolCallArgsForAgentLoop(args)
}

// promoteMCPRoutingFields delegates to the shared agentruntime routing-field
// promotion helper.
func promoteMCPRoutingFields(args map[string]interface{}, toolArgs map[string]interface{}) {
	agentruntime.PromoteMCPRoutingFields(args, toolArgs)
}

// promoteMCPNestedArgsFromCallEnvelope delegates to the shared agentruntime
// nested-argument promotion helper. Non-blank nested values win; blank nested
// strings do not.
func promoteMCPNestedArgsFromCallEnvelope(callArgs, toolArgs map[string]interface{}, inputSchema map[string]interface{}) map[string]interface{} {
	return agentruntime.PromoteMCPNestedArgsFromCallEnvelope(callArgs, toolArgs, inputSchema)
}
...
// mcpToolArgumentsFromAny delegates to the shared agentruntime MCP arguments
// normalizer.
func mcpToolArgumentsFromAny(raw interface{}) (map[string]interface{}, error) {
	return agentruntime.MCPToolArgumentsFromAny(raw)
}

	...
	if h.localBgTaskMgr != nil {
		for _, t := range h.localBgTaskMgr.List() {
			...
			rs.Tasks = append(rs.Tasks, RuntimeTaskInfo{
				...
				RuntimeStatus: normalizeRuntimeTaskStatus(t.Status).RuntimeStatusValue(),
				Command:       t.Command,
				TaskRole:      t.TaskRole,
	...
	if h.manager != nil {
		for _, s := range h.manager.List() {
			...
			rs.Sessions = append(rs.Sessions, RuntimeSessionInfo{
				...
				RuntimeStatus: normalizeRuntimeSessionStatus(s.Status).RuntimeStatusValue(),
			})
			s.mu.RUnlock()
		}
	}
	...
	if h.sshMgr != nil {
		for _, s := range h.sshMgr.List() {
			...
			rs.Sessions = append(rs.Sessions, RuntimeSessionInfo{
				...
				RuntimeStatus: normalizeRuntimeSessionStatus(summary.Status).RuntimeStatusValue(),

...
func (r *SkillRunner) UseDurableRuntimeStores(dataDir string) error {
	if r == nil {
		return fmt.Errorf("skill runner is unavailable")
	}
	...
	r.runtimeJobs = jobs
	r.runtimeEffects = effects
	return r.ReconcileRuntimeJobs(context.Background())
}

func (r *SkillRunner) ReconcileRuntimeJobs(ctx context.Context) error {
	if r == nil {
		return nil
	}
	return agentruntime.RecoverAndReconcileOpenJobs(ctx, r.runtimeJobs, r.runtimeEffects, "skill.run", r, nil)
}
...
func (r *SkillRunner) Close() error {
	...
	if r.runtimeJobs != nil {
		if err := r.runtimeJobs.Close(); err != nil && first == nil {
			first = err
		}
	}
	if r.runtimeEffects != nil {
		if err := r.runtimeEffects.Close(); err != nil && first == nil {
			first = err
		}
	}
	return first
...
func (r *SkillRunner) startRunForOwner(policyOwnerID, skillName string, preResolved *corelib.NLSkillEntry, runArgs map[string]interface{}) (runID string, retErr error) {
	...
	r.mu.Unlock()
	r.publishRuntimeJob(statusSnapshot)
...
func snapshotRunStatus(src *SkillRunStatus) SkillRunStatus {
	cp := *src
	cp.RuntimeStatus = cp.RuntimeStatusValue()
...
func (r *SkillRunner) publishRuntimeJob(status SkillRunStatus) {
	if r == nil || r.runtimeJobs == nil {
		return
	}
	now := time.Now().UTC()
	...
	runtimeStatus := status.RuntimeStatusValue()
	if parsed, err := time.Parse(time.RFC3339, status.EndedAt); err == nil {
		now = parsed
	}
	outcome := agentruntime.JobWorkerOutcome{ErrorCode: "skill_run_failed", ErrorText: status.Error}
	if runtimeStatus == agentruntime.JobStatusSucceeded {
		if payload, err := json.Marshal(map[string]string{"skill": status.Skill, "status": string(runtimeStatus)}); err == nil {
			outcome.Result = payload
		}
	}
	job = agentruntime.StampHostJobStatus(job, runtimeStatus, now, outcome)
	if err := agentruntime.UpsertJob(context.Background(), r.runtimeJobs, job); err != nil {
		log.Printf("[skill-runner] runtime job upsert failed run=%s err=%v", status.RunID, err)
	}
	r.syncRuntimeEffect(status)
}

func (r *SkillRunner) syncRuntimeEffect(status SkillRunStatus) {
	if r == nil || r.runtimeEffects == nil || strings.TrimSpace(status.RunID) == "" {
		return
	}
	ctx := context.Background()
	userID := strings.TrimSpace(status.OwnerID)
	if userID == "" {
		userID = "local"
	}
	now := time.Now().UTC()
	prepared, _, err := r.runtimeEffects.Prepare(ctx, agentruntime.JobEffect{
		JobID: status.RunID, Kind: "skill.run", JobKind: "skill.run",
		TenantID: "gui", UserID: userID, ResourceID: status.RunID,
		CreatedAt: now, UpdatedAt: now,
	})
	if err != nil {
		log.Printf("[skill-runner] runtime effect prepare failed run=%s err=%v", status.RunID, err)
		return
	}
	runtimeStatus := status.RuntimeStatusValue()
	next := prepared
	...
	case agentruntime.JobStatusSucceeded:
		...
		next.ReceiptDigest = agentruntime.JobEffectReceiptDigest(status.RunID + ":" + string(runtimeStatus))
		if payload, err := json.Marshal(map[string]string{"skill": status.Skill, "status": string(runtimeStatus)}); err == nil {
			next.Payload = payload
		}
	case agentruntime.JobStatusFailed:
		next.State = agentruntime.JobEffectFailed
		next.ReasonCode = "skill_run_failed"
	default:
		return
	}
	if _, err := r.runtimeEffects.Update(ctx, prepared.Version, next); err != nil {
		log.Printf("[skill-runner] runtime effect settle failed run=%s err=%v", status.RunID, err)
	}
}

func (r *SkillRunner) ReconcileJob(ctx context.Context, job agentruntime.Job, effects []agentruntime.JobEffect) (agentruntime.JobReconcileResult, error) {
	if r == nil || strings.TrimSpace(job.Kind) != "skill.run" {
		return agentruntime.JobReconcileResult{}, nil
	}
	if result := agentruntime.ReconcileFromProtectedEffects(effects, "skill.run", "skill run failed", nil); result.Resolved {
		return result, nil
	}
	status, err := r.GetRunStatus(job.ID)
	if err != nil || status == nil {
		return agentruntime.JobReconcileResult{}, nil
	}
	switch status.RuntimeStatusValue() {
	case agentruntime.JobStatusSucceeded:
		payload, _ := json.Marshal(map[string]string{"skill": status.Skill, "status": "succeeded"})
	...
	case agentruntime.JobStatusFailed:
		...
		if errText == "" {
			errText = "skill run failed"
		}
		return agentruntime.JobReconcileResult{Resolved: true, Status: agentruntime.JobStatusFailed, ErrorCode: code, Error: errText}, nil
	default:
		return agentruntime.JobReconcileResult{}, nil
	}
}

var _ agentruntime.JobReconciler = (*SkillRunner)(nil)
...
func (r *SkillRunner) executeAsync(ctx context.Context, run *skillRun, skill *corelib.NLSkillEntry) {
	...
	// ── OpenAI Proxy (default-on) ──
	// The local proxy starts by default so skills that call an
	// OpenAI-compatible API always find OPENAI_API_KEY/OPENAI_BASE_URL set.
	// It is skipped only when the caller/process already provides credentials
	// or the skill declares no_llm_api. See corelib.NeedsOpenAIProxyAuto.
...
// A missing or unreadable configuration is fail-closed: it cannot authorize
// automatic definition changes.
...
func (r *SkillRunner) updateUsageStats(skill *corelib.NLSkillEntry, execErr error) *corelib.NLSkillEntry {
	...
	successfulSkillName := ""
	app := r.executor.app
	// Usage counters are durable registry state. Serialize their read/modify/
	// write with installs, deletes and lifecycle transitions, and fail closed
	// when the compensation queue cannot prove that a prior mutation is safe.
	// The queue can change while this callback is waiting, so this admission is
	// intentionally repeated under the process-wide mutation lock.
	...
	if err := cskill.CheckEvolutionCompensationQueue(); err != nil {
		log.Printf("[skill-runner] skip usage persistence: evolution compensation queue unavailable: %v", err)
	...
	for i, s := range skills {
		if s.Name == skill.Name {
			if err := ensureSkillEvolutionMutationAdmission(app, s.Name); err != nil {
				log.Printf("[skill-runner] skip usage persistence for %s: %v", s.Name, err)
				break
			}
			...
			if saveErr != nil {
				log.Printf("[skill-runner] persist usage stats for %q failed: %v", skill.Name, saveErr)
				break
			}
...
func (r *SkillRunner) persistRepairResultWithContext(ctx context.Context, entry *corelib.NLSkillEntry) error {
	...
	// Drain load-time status overlay writers before taking installMutex. Taking
	// the locks in the opposite order can deadlock with an overlay goroutine
	// that is already waiting for installMutex.
	r.executor.suspendStatusOverlayPersistence()
	defer r.executor.resumeStatusOverlayPersistence()
	// Serialize the complete repair publication with every other App-owned
	// Skill mutation.  SkillCommitter protects its own durable snapshot, but it
	// intentionally does not own the adapter's process-wide lock; without this
	// boundary an install/delete/status writer could publish a newer registry
	// between SkillLoader and SkillSaver and have the repair commit overwrite
	// that change with a stale full-list snapshot.
	app.installMutex.Lock()
	defer app.installMutex.Unlock()
	if err := ensureSkillEvolutionMutationAdmission(app, entry.Name); err != nil {
		return err
	}
	...
	// Keep the durable compensation identity aligned with the audit request
	// correlation even when this legacy repair entry is invoked without a
	// pipeline-provided context.
	ctx = cskill.WithEvolutionRequestMetadata(ctx, requestID, attempt)
	...
	if result.State != "committed" || result.CleanupStatus != "clear" {
		reason := formatRepairCommitFailureReason(result.FailureReason)
		return fmt.Errorf("repair not committed: %s (%s), cleanup_status=%s", result.State, reason, result.CleanupStatus)
	}
	return nil
}

// formatRepairCommitFailureReason keeps the user-facing phase diagnosis even
// when the shared committer had to enter audit_pending because a rollback step
// failed. In that case FailureReason has the shape
// "<phase>:rollback_incomplete:<rollback details>"; matching only the complete
// string would hide the actionable phase behind a generic transaction error.
func formatRepairCommitFailureReason(failureReason string) string {
	reason := strings.TrimSpace(failureReason)
	phase := reason
	if marker := strings.Index(phase, ":rollback_incomplete"); marker >= 0 {
		phase = phase[:marker]
	}
	switch {
	case strings.HasPrefix(phase, "final_audit_failed"):
		return "final repair audit failed: " + reason
	case strings.HasPrefix(phase, "index_refresh_failed"):
		return "refresh skill index failed: " + reason
	case strings.HasPrefix(phase, "yaml_write_failed"):
		return "write repaired skill.yaml failed: " + reason
	case strings.HasPrefix(phase, "config_write_failed"):
		return "save repaired skill config failed: " + reason
	default:
		return reason
	}
}
...
func (r *SkillRunner) persistRepairResultWithContextLegacy(ctx context.Context, entry *corelib.NLSkillEntry) error {
	...
	refreshIndex := func() error {
		...
		if r.executor.app == nil {
			return fmt.Errorf("skill index refresh app not initialized")
		}
		return r.executor.app.refreshSkillIndexesAfterMutationChecked(entry.Name)
	}
...
func (r *SkillRunner) persistRepairMetadataWithContext(ctx context.Context, entry *corelib.NLSkillEntry) error {
	...
	if r.executor.app != nil {
		r.executor.suspendStatusOverlayPersistence()
		defer r.executor.resumeStatusOverlayPersistence()
		r.executor.app.installMutex.Lock()
		defer r.executor.app.installMutex.Unlock()
		if err := ensureSkillEvolutionMutationAdmission(r.executor.app, entry.Name); err != nil {
			return err
		}
	}
	r.executor.skillListMutateMu.Lock()
...
func (r *SkillRunner) RecordWorkaround(skillName, lastError string) {
	if r == nil || r.executor == nil || r.executor.app == nil || strings.TrimSpace(skillName) == "" {
		return
	}
	shouldEmit := false
	app := r.executor.app
	...
	app.installMutex.Lock()
	if err := cskill.CheckEvolutionCompensationQueue(); err != nil {
		log.Printf("[skill-runner] skip workaround persistence: evolution compensation queue unavailable: %v", err)
		app.installMutex.Unlock()
		return
	}
	...
	for i, s := range skills {
		if s.MatchesName(skillName) {
			if err := ensureSkillEvolutionMutationAdmission(app, s.Name); err != nil {
				log.Printf("[skill-runner] skip workaround persistence for %s: %v", s.Name, err)
				break
			}
			...
			if err := r.executor.saveSkills(skills); err != nil {
				log.Printf("[skill-runner] persist workaround stats for %q failed: %v", skillName, err)
				break
			}

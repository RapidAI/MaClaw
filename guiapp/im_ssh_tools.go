...
// sshExecChannelContext runs one command on its own non-PTY exec channel of
// the verified session's pooled connection. It is the transport for parallel
// isolated remote writers: concurrent children must never share the
// interactive PTY scrollback, so their commands, isolate merges and cleanups
// each get a dedicated channel with isolated stdout/stderr and exit status.
//

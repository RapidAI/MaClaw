...
func TestLoopContextNilReceiverIsCancelledDoesNotPanic(t *testing.T) {
	var ctx *LoopContext
	if ctx.IsCancelled() {
		t.Fatal("nil loop context is not cancelled")
	}
}

func TestLoopContextNilReceiverContextDoesNotPanic(t *testing.T) {
	var ctx *LoopContext
	requestCtx, cancel := ctx.Context()
	defer cancel()
	if requestCtx.Err() != nil {
		t.Fatalf("fresh nil-receiver context should be usable: %v", requestCtx.Err())
	}
	cancel()
	<-requestCtx.Done()
}

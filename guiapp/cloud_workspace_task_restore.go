...
func (a *App) loadDismissedCloudWorkspaceTasks() map[string]struct{} {
	...
	root := filepath.Join(a.GetDataDir(), "cloud-workspaces")
	if err := cloudWorkspaceCacheDirectoryExistsAndSafe(root, root); err != nil {
		return out
	}
...
func (a *App) saveDismissedCloudWorkspaceTasks(ids map[string]struct{}) {
	...
	if err := ensureCloudWorkspaceCacheDirectory(filepath.Join(a.GetDataDir(), "cloud-workspaces"), filepath.Join(a.GetDataDir(), "cloud-workspaces")); err != nil {
...
// cloudWorkspaceTranscriptFetch is a newly materialized cloud task whose
// session.json still has to be pulled after the restore loop releases
// cloudWorkspaceRestoreMu.
type cloudWorkspaceTranscriptFetch struct {
	workspaceID string
	projectPath string
}
...
func (a *App) restoreCloudWorkspaceTasks() []ProjectSearchResult {
	...
	transcriptFetches := make([]cloudWorkspaceTranscriptFetch, 0, len(ent.Workspaces))
	...
	for _, ws := range pending {
		...
		// Materializing the task must also materialize its durable transcript.
		// Restore runs without a lease, so no live resume path will otherwise
		// fetch session.json before the history panel asks for it.
		transcriptFetches = append(transcriptFetches, cloudWorkspaceTranscriptFetch{workspaceID: workspaceID, projectPath: created.ProjectPath})
		keepCloudWorkspacePath(keep, workspaceID, created.ProjectPath)
		restored = append(restored, created)
	}
	// Fetch transcripts outside cloudWorkspaceRestoreMu: a slow Hub must not
	// block hide/dismiss, and the session sidecar write below only touches the
	// pending-sidecar store and the tab-session persist. When a newer restore
	// run supersedes this one, stop early: rows materialized here are found by
	// that run as existing rows, and opening the task lazily fetches any
	// transcript that is still missing.
	for _, fetch := range transcriptFetches {
		if cloudWorkspaceRestoreGen.Load() != gen {
			break
		}
		a.refreshCloudWorkspaceSessionSidecar(fetch.workspaceID, fetch.projectPath)
	}
	extras := a.hideOtherLocalCloudWorkspaceTasksForKeep(keep)
	log.Printf("[cloud_workspace] restore tasks entitlement=%d pending=%d existing=%d restored=%d dismissed=%d invalid=%d hidden_unbound=%d extras=%d", len(ent.Workspaces), len(pending), existingCount, len(restored), skippedDismissed, skippedInvalid, unhid, extras)
	if unhid > 0 || extras > 0 {
		a.emitProjectIndexChanged("")
	}
	return restored

...
func TestKnowledgeImportRuntimeStatusUsesSharedVocabulary(t *testing.T) {
	cases := map[string]agentruntime.JobStatus{
		"queued":                        agentruntime.JobStatusPending,
		knowledge.ImportStatusRunning:   agentruntime.JobStatusRunning,
		knowledge.ImportStatusIndexing:  agentruntime.JobStatusRunning,
		knowledge.ImportStatusCompleted: agentruntime.JobStatusSucceeded,
		knowledge.ImportStatusFailed:    agentruntime.JobStatusFailed,
		"cancelled":                     agentruntime.JobStatusCanceled,
		"unexpected-legacy-status":      agentruntime.JobStatusUnknown,
	}
	for status, want := range cases {
		if got := knowledgeImportRuntimeStatus(status); got != want {

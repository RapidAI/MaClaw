...
func (m *BrowserAgentManager) syncFromCore() {
	...
	for _, sess := range coreSessions {
		...
		if !ok || view == nil {
			view = &BrowserSessionView{ID: state.ID, Tool: "browser", Status: SessionRunning, RuntimeStatus: agentruntime.JobStatusRunning, CreatedAt: state.CreatedAt}
	...
	for id, view := range m.mapViews {
		if !alive[id] && view != nil {
			...
			view.RuntimeStatus = agentruntime.JobStatusUnknown
...
func (m *BrowserAgentManager) applyStateLocked(view *BrowserSessionView, state browser.BrowserAgentState) {
	...
	if state.Alive {
		...
		view.RuntimeStatus = agentruntime.JobStatusRunning
	} else {
		view.Status = SessionExited
		view.RuntimeStatus = agentruntime.JobStatusUnknown
	}

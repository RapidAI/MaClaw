...
// RunQualityGate writes a draft directly into the primary skills directory.
// It is retained for explicit callers and tests. Background auto-summary must
// instead use runQualityGateInDirectory with a staging directory, then publish
// through commitStagedSkillInstall so config, directory, index, audit and
// rollback share one durable transaction.
func RunQualityGate(draft *skill.SkillYAMLFile, tagGen *TagGenerator) (*QualityGateResult, error) {
	...
	result, err := runQualityGateInDirectory(draft, tagGen, skillDir)
	if err != nil {
		return nil, err
	}
...
// runQualityGateInDirectory writes and evaluates a draft in an already-owned
// directory. The caller owns the directory lifecycle. It is deliberately
// separate from RunQualityGate so automatic producers can generate and scan a
// candidate while it is staged, never directly under the authoritative root.
func runQualityGateInDirectory(draft *skill.SkillYAMLFile, tagGen *TagGenerator, skillDir string) (*QualityGateResult, error) {
	if draft == nil {
		return nil, fmt.Errorf("quality gate: draft is required")
	}
	skillDir = strings.TrimSpace(skillDir)
	if skillDir == "" {
		return nil, fmt.Errorf("quality gate: skill directory is required")
	}
	if skillDefinitionExists(skillDir) {
		return nil, fmt.Errorf("quality gate: skill definition already exists at %s", skillDir)
	}

	// 1. Format the draft and write skill.yaml.
	...
	// 2-4. Generate tags (optional; failure is logged but not fatal).
	if tagGen != nil {
	...
	// 5. Simulate a successful execution result for scoring.
	result := SkillExecutionResult{
		Success:       true,
		OutputQuality: "basic",
	}

	// 6. Evaluate the skill execution to get a score.
	score := EvaluateSkillExecution(&result)

	// 7. Determine status based on score threshold.
...
// automaticMutationApp is the fail-closed policy boundary for trajectory
// summaries. Recording a trajectory is not permission to publish an
// executable definition: automatic definition writes require an explicit
// config opt-in and are always overridden by the process-wide kill switch.
func (p *SkillAutoSummaryPipeline) automaticMutationApp() (*App, error) {
	if p == nil || p.skillExec == nil || p.skillExec.app == nil {
		return nil, fmt.Errorf("skill auto-summary is not configured for durable mutation")
	}
	if skill.EvolutionEnvDisabled() {
		return nil, fmt.Errorf("automatic skill auto-summary mutation is disabled by environment")
	}
	cfg, err := p.skillExec.app.LoadConfig()
	if err != nil {
		return nil, fmt.Errorf("automatic skill auto-summary mutation blocked: load skill evolution config: %w", err)
	}
	if !cfg.IsSkillEvolutionEnabled() {
		return nil, fmt.Errorf("automatic skill auto-summary mutation is disabled")
	}
	return p.skillExec.app, nil
}
...
func (p *SkillAutoSummaryPipeline) RunPipeline(session *TrajectorySession) {
	...
	// All remaining stages can create or replace a durable definition. Do the
	// dynamic policy check immediately before that boundary rather than treating
	// trajectory logging as authorization. A disabled, missing, unreadable, or
	// kill-switched config produces no skill directory or config overlay.
	app, err := p.automaticMutationApp()
	if err != nil {
		log.Printf("[skill-auto-summary] session=%s durable mutation skipped: %v", sid, err)
		return
	}
	...
	if existing != nil {
		...
		if shouldUpdateSkill(draft, existing) {
			// The former Versioner path rewrote YAML and then independently wrote
			// the config overlay. It did not cover both operations with the
			// directory publication transaction, so an index/audit failure could
			// leave an unregistered or partially updated definition. Do not turn a
			// successful trajectory into an implicit replacement until that path is
			// staged and committed as one transaction.
			log.Printf("[skill-auto-summary] session=%s stage=VersionedUpdate deferred: automatic replacement requires a reviewed staged transaction", sid)
		} else {
	...
	log.Printf("[skill-auto-summary] session=%s stage=ValidateSkillDraft result=ok name=%s",
		sid, draft.Name)
	if err := ensureSkillEvolutionMutationAdmission(app, draft.Name); err != nil {
		log.Printf("[skill-auto-summary] session=%s stage=MutationAdmission blocked=%v", sid, err)
		return
	}

	// Stage 4: write and scan a staging-only candidate. The final directory is
	// not touched until commitStagedSkillInstall has prepared compensation and
	// its config/index/final-audit transaction.
	stagingName := fmt.Sprintf("auto-summary-%s-%d", draft.Name, time.Now().UnixNano())
	stagingRoot, err := app.skillStagingDir()
	if err != nil {
		log.Printf("[skill-auto-summary] session=%s stage=ResolveStagingRoot error=%v", sid, err)
		return
	}
	stagingDir, err := skill.PrepareStagingDirInRoot(stagingRoot, stagingName)
	if err != nil {
		log.Printf("[skill-auto-summary] session=%s stage=PrepareStaging error=%v", sid, err)
		return
	}
	stagingOwned := true
	defer func() {
	...
	gateResult, err := runQualityGateInDirectory(draft, p.tagGen, stagingDir)
	if err != nil {
	...
	// Stage 4.5: Parse the staged definition back into the exact runtime entry,
	// scan it with the non-interactive automatic policy, then publish it via the
	// same directory transaction used by marketplace installs.
	entry, err := loadImportedSkillEntry(stagingDir)
	if err != nil {
		log.Printf("[skill-auto-summary] session=%s stage=LoadStagedDefinition error=%v", sid, err)
		return
	}
	entry.Source = "learned"
	entry.TrustLevel = security.TrustLevelAgentCreated
	entry.SkillDir = stagingDir
	report, err := (experienceSkillStore{executor: p.skillExec}).scanBeforePersist(entry, "register")
	if err != nil {
		log.Printf("[skill-auto-summary] session=%s stage=SecurityAdmission error=%v", sid, err)
		return
	}
	// The setting may have changed while the candidate was staged. Recheck it
	// before the shared committer can publish the directory.
	if _, err := p.automaticMutationApp(); err != nil {
		log.Printf("[skill-auto-summary] session=%s stage=Commit blocked=%v", sid, err)
		return
	}
	if err := ensureSkillEvolutionMutationAdmission(app, draft.Name); err != nil {
		log.Printf("[skill-auto-summary] session=%s stage=Commit blocked=%v", sid, err)
	...
	requestID := fmt.Sprintf("evo_auto_summary_%d", time.Now().UnixNano())
	if err := app.commitAutoSummaryStagedSkillInstall(context.Background(), entry, stagingDir, report, requestID, skillEvolutionConfigRevision(app)); err != nil {
		log.Printf("[skill-auto-summary] session=%s stage=Commit error=%v", sid, err)
		return
	}
	stagingOwned = false
	log.Printf("[skill-auto-summary] session=%s stage=Commit result=ok name=%s", sid, draft.Name)

	// Stage 5: Generation never uploads. The skill is

...
// initializeContext performs the handshake while honoring the caller's
// cancellation. Startup callers use the manager lifetime context so a
// foreground config transaction can stop an in-flight handshake promptly.
func (c *LocalMCPClient) initializeContext(ctx context.Context) error {
	if ctx == nil {
		ctx = context.Background()
	}
	...
	_, err := c.sendRequestContext(ctx, "initialize", initParams)
	if err != nil {
		return err
	}
...
// sendRequestContext is the cancellation-aware JSON-RPC boundary. The
// historical sendRequest wrapper remains for ordinary tool calls, while
// startup/discovery transactions pass their request context through here.
func (c *LocalMCPClient) sendRequestContext(ctx context.Context, method string, params interface{}) (json.RawMessage, error) {
	if ctx == nil {
		ctx = context.Background()
	}
	c.stateMu.RLock()
...
func (c *LocalMCPClient) DiscoverTools() ([]MCPToolView, error) {
	return c.discoverToolsContext(context.Background())
}

// DiscoverToolsContext is the cancellation-aware tools/list boundary used by
// managed runtime synchronization.
func (c *LocalMCPClient) DiscoverToolsContext(ctx context.Context) ([]MCPToolView, error) {
	return c.discoverToolsContext(ctx)
}

func (c *LocalMCPClient) discoverToolsContext(ctx context.Context) ([]MCPToolView, error) {
	result, err := c.sendRequestContext(ctx, "tools/list", map[string]interface{}{})
	if err != nil {
		return nil, err
	}

...
func (a *App) handleAgentViewSubmitPayload(payload AgentViewSubmitPayload) (resp *IMAgentResponse) {
	...
	a.emitAgentViewLifecycle(agentViewLifecycleSubmit, map[string]interface{}{
		"view_id":        payload.ViewID,
		"data":           redactAgentViewSubmitSecrets(payload.Data),
		"view_revision":  agentViewSubmitRevision(payload),
		"schema_version": agentViewSubmitSchemaVersion(payload),
	})
...
func (a *App) executeMISDataTool(args map[string]interface{}) string {
	...
	case "update_record_approval_progress", "mis.approval.progress":
		approvalID := strings.TrimSpace(firstNonEmptyMISAgentView(stringArg(args, "approval_id"), stringArg(args, "id")))
		if approvalID == "" {
		...
		body := map[string]interface{}{
			...
			"outputs":               args["outputs"],
			"artifacts":             args["artifacts"],
			"progress":              firstNonEmptyMISAgentView(stringArg(args, "progress"), stringArg(args, "message"), stringArg(args, "reason")),
		}
		return a.callMISDataAPI(cfg, http.MethodPost, "/api/v1/data/approvals/"+pathEscape(approvalID)+"/progress", compactPayload(body))

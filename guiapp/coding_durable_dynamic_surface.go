...
type codingDurableDynamicSurface struct {
	...
	// parentRevision is the durable lineage parent this surface's revision was
	// published against. It is nil for a lineage root and never carries grants,
	// aliases, or any predecessor in-memory state.
	parentRevision *tool.RouteRevisionRef
}
...
func (a *App) publishCodingDurableDynamicSurfaceForEpoch(identity *trustedCodingInvocationIdentity, prepared codingDynamicPlanPreparation, dynamic codingDynamicCatalogSnapshot, protocol, connectionID, epoch string, now time.Time) (*codingDurableDynamicSurface, error) {
	return a.publishCodingDurableDynamicSurfaceRevision(identity, prepared, dynamic, protocol, connectionID, epoch, nil, now)
}

// publishCodingDurableDynamicSurfaceChildForEpoch publishes a successor
// reservation's surface as the next revision of the durable lineage the
// predecessor committed. parent must be the lineage-current revision read from
// the durable coordinator by the lifecycle owner; task text, runtime IDs,
// project paths, and provider configuration are never accepted as a lineage
// source. The retired predecessor revision is not revived and no grant or
// alias crosses the revision boundary: the child re-identifies the same
// immutable plan content and receives freshly issued grants.
func (a *App) publishCodingDurableDynamicSurfaceChildForEpoch(identity *trustedCodingInvocationIdentity, prepared codingDynamicPlanPreparation, dynamic codingDynamicCatalogSnapshot, protocol, connectionID, epoch string, parent tool.RouteRevisionRef, now time.Time) (*codingDurableDynamicSurface, error) {
	if identity == nil || parent.RootTaskID != identity.RootTaskID || parent.SessionID != identity.SessionID || parent.PrincipalID != identity.PrincipalID {
		return nil, fmt.Errorf("coding dynamic child revision parent does not match the verified identity")
	}
	return a.publishCodingDurableDynamicSurfaceRevision(identity, prepared, dynamic, protocol, connectionID, epoch, &parent, now)
}

// codingDynamicChildRevisionPlanID re-identifies the immutable plan content of
// a successor reservation as the next lineage revision. Its only inputs are
// the durable parent revision record; nothing model-, task-, or
// transport-controlled enters lineage identity.
func codingDynamicChildRevisionPlanID(parent tool.RouteRevisionRef) string {
	return fmt.Sprintf("%s:r%d", parent.PlanID, parent.Revision+1)
}

func (a *App) publishCodingDurableDynamicSurfaceRevision(identity *trustedCodingInvocationIdentity, prepared codingDynamicPlanPreparation, dynamic codingDynamicCatalogSnapshot, protocol, connectionID, epoch string, parent *tool.RouteRevisionRef, now time.Time) (*codingDurableDynamicSurface, error) {
	...
	// A successor reservation publishes its surface as a child revision of the
	// durable lineage record. The plan content stays immutable; only its
	// revision identity advances, so a terminated predecessor revision is never
	// revived or rematerialized.
	plan := prepared.Plan
	var expectedParent *tool.RouteRevisionRef
	if parent != nil {
		parentCopy := *parent
		expectedParent = &parentCopy
		plan.ID = codingDynamicChildRevisionPlanID(parentCopy)
	}
	...
	// ToolSnapshotID is the exact route/tool-surface identity. CatalogDigest
	// remains the provider inventory identity and must not be substituted when
	// a full planner snapshot is available.
	toolSnapshotID := strings.TrimSpace(plan.SnapshotDigest)
	if toolSnapshotID == "" {
		toolSnapshotID = strings.TrimSpace(plan.CatalogDigest)
	}
	scope := tool.InvocationScope{
		RootTaskID: identity.RootTaskID, PlanID: plan.ID, SessionID: identity.SessionID,
		TurnID: identity.TurnID, PrincipalID: identity.PrincipalID,
		ToolSnapshotID: toolSnapshotID,
	}
	published, grants, err := coordinator.PublishSurface(tool.NewSurfacePublishRequest(
		tool.RouteRevisionPublishRequest{Scope: scope, Plan: plan, ExpectedParent: expectedParent, SnapshotDigest: plan.SnapshotDigest},
		identity.TenantID, issuer, tool.DefaultInvocationGrantTTL, now,
	))
	if err != nil {
	...
	// The render closure must equal the grant closure: PublishSurface issues
	// grants only for the current repeat-exposure subset of ready selections,
	// so a sibling-rich plan (the production Coding needs declare invocation
	// budgets) must present exactly the selections that received grants.
	// Presenting more would demand grants that were deliberately not issued.
	grantedSelections := make(map[string]bool, len(grants))
	for _, grant := range grants {
		grantedSelections[grant.SelectionID] = true
	}
	renderPlan := plan
	renderPlan.Selections = make([]tool.PlannedSelection, 0, len(grants))
	for _, selection := range plan.Selections {
		if grantedSelections[selection.ID] {
			renderPlan.Selections = append(renderPlan.Selections, selection)
		}
	}
	rendered, err := tool.NewCatalogRenderer(newIMSemanticCapabilityRegistry()).Render(renderPlan, grants, dynamic.Catalog.Definitions)
	if err != nil {
	...
	return &codingDurableDynamicSurface{
		coordinator: coordinator, plan: plan, scope: scope, tenantID: identity.TenantID, protocol: protocol, connection: connectionID,
		epoch: epoch, aliases: aliases, definitions: definitions, parentRevision: published.ParentRevision,
	}, nil
}
...
func (a *App) recoverCodingDurableDynamicSurface(identity *trustedCodingInvocationIdentity, protocol, connectionID, epoch string) (*codingDurableDynamicSurface, error) {
	...
	// A durable surface may carry an explicit host snapshot identity that is
	// intentionally different from the planner's provider CatalogDigest. Keep
	// that identity authoritative on recovery; only legacy rows with no value
	// fall back to the immutable plan snapshot.
...
func codingDynamicHostCallResult(coordinator *tool.SQLiteSemanticExecutionCoordinator, scope tool.InvocationScope, selectionID string, record tool.HostCallRecord, action tool.HostCallAcquireAction, requestDigest string) (tool.SelectionExecutionResult, bool) {
	acquired := action
	resolved := tool.ResolveHostCallAcquireAction(action, record, requestDigest)
	var store tool.PlanExecutionStore
	if coordinator != nil {
		store = coordinator.Executions
	}
	return tool.HostCallAcquireTerminal(acquired, resolved, record, store, scope, selectionID)
}

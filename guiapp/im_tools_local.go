...
func (h *IMMessageHandler) toolBash(execCtx context.Context, args map[string]interface{}, onProgress coretool.ProgressCallback) string {
	...
	if rejection, rejected := coretool.RejectShellDatabaseCLI(command); rejected {
		return rejection
	}
...
func canonicalizeLocalFileSearchToolName(name string) string {
	canonical, _, _ := agentruntime.CanonicalizeLocalFileSearchToolCall(name, nil)
	return canonical
}

func canonicalizeLocalFileSearchToolCall(name string, args map[string]interface{}) (string, map[string]interface{}, bool) {
	return agentruntime.CanonicalizeLocalFileSearchToolCall(name, args)
}

func canonicalizeLocalFileSearchToolCallJSON(name, argsJSON string) (string, string, bool) {
	return agentruntime.CanonicalizeLocalFileSearchToolCallJSON(name, argsJSON)
}

func (h *IMMessageHandler) toolGlobFiles(ctx context.Context, args map[string]interface{}) string {
	if ctx == nil {
	...
	ownerID, hasRuntimeOwner := consumeRuntimePolicyOwnerIDFromToolArgsWithPresence(args)
	if hasRuntimeOwner && ownerID == "" {
		return "Glob failed: runtime owner is missing; isolated runtime will not fall back to desktop working directory"
	}
	_, rewrittenArgs, rewritten := canonicalizeLocalFileSearchToolCall("Glob", args)
...
func (h *IMMessageHandler) toolSendFile(args map[string]interface{}) string {
	...
	if class, ok := database.ClassifiedExport(absPath); ok && database.BlocksExternalDelivery(class) {
		if applySendFileForwardArgs(args) {
			return "permission: classified database export cannot be uploaded to IM or the public network"
		}
	}
...
func (h *IMMessageHandler) toolSendToIM(args map[string]interface{}) string {
	...
	p, _ := args["path"].(string)
	if class, ok := database.ClassifiedExport(p); ok && database.BlocksExternalDelivery(class) {
		return "permission: classified database export cannot be uploaded to IM or the public network"
	}

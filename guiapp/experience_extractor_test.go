...
func enableAutomaticExperienceSkillMutation(t *testing.T, app *App) {
	t.Helper()
	oldBase := corelib.MaclawBaseDir()
	corelib.SetMaclawBaseDir(filepath.Join(t.TempDir(), "maclaw-data"))
	...
	if err := app.PatchConfig(func(cfg *corelib.AppConfig) {
		...
		t.Fatalf("enable automatic experience skill mutation: %v", err)
	}
}

func TestExperienceSkillStoreDefaultsToNoAutomaticWrite(t *testing.T) {
	app := &App{testHomeDir: t.TempDir()}
	executor := NewSkillExecutor(app, nil, nil)
	app.skillExecutor = executor
	store := experienceSkillStore{executor: executor}

	err := store.Register(corelib.NLSkillEntry{
		Name: "run-project-tests", Description: "Run the project test suite after making changes.",
		Triggers: []string{"run tests"}, Source: "learned",
		Steps: []corelib.NLSkillStep{{Action: "bash", Params: map[string]interface{}{"command": "go test ./..."}}},
	})
	if err == nil || !strings.Contains(err.Error(), "automatic experience skill mutation is disabled") {
		t.Fatalf("Register() error = %v, want disabled automatic mutation", err)
	}
	...
	if len(cfg.NLSkills) != 0 {
		t.Fatalf("disabled automatic extraction persisted a skill: %#v", cfg.NLSkills)
	}
}

func TestExperienceSkillStoreDefaultsToNoAutomaticUpdate(t *testing.T) {
	app := &App{testHomeDir: t.TempDir()}
	executor := NewSkillExecutor(app, nil, nil)
	app.skillExecutor = executor
	if err := executor.Register(corelib.NLSkillEntry{
		Name: "run-project-tests", Description: "Run the existing project test suite.", Status: "active", Source: "manual",
		Triggers: []string{"run tests"}, Steps: []corelib.NLSkillStep{{Action: "bash", Params: map[string]interface{}{"command": "go test ./..."}}},
	}); err != nil {
		t.Fatalf("seed skill Register() error = %v", err)
	}
	store := experienceSkillStore{executor: executor}

	err := store.Update(corelib.NLSkillEntry{
		Name: "run-project-tests", Description: "Replace the command.", Status: "active", Source: "learned",
		Triggers: []string{"replace tests"}, Steps: []corelib.NLSkillStep{{Action: "bash", Params: map[string]interface{}{"command": "go test ./... -race"}}},
	})
	if err == nil || !strings.Contains(err.Error(), "automatic experience skill mutation is disabled") {
		t.Fatalf("Update() error = %v, want disabled automatic mutation", err)
	}
	cfg, loadErr := app.LoadConfig()
	if loadErr != nil {
		t.Fatalf("LoadConfig() error = %v", loadErr)
	}
	if len(cfg.NLSkills) != 1 || cfg.NLSkills[0].Description != "Run the existing project test suite." || cfg.NLSkills[0].Steps[0].Params["command"] != "go test ./..." {
		t.Fatalf("disabled automatic extraction changed existing skill: %#v", cfg.NLSkills)
	}
}

func TestExperienceSkillStoreHonorsEvolutionKillSwitch(t *testing.T) {
	app := &App{testHomeDir: t.TempDir()}
	executor := NewSkillExecutor(app, nil, nil)
	app.skillExecutor = executor
	enableAutomaticExperienceSkillMutation(t, app)
	...
	if err == nil || !strings.Contains(err.Error(), "disabled by environment") {
		t.Fatalf("Register() error = %v, want environment kill switch rejection", err)
	}
	cfg, loadErr := app.LoadConfig()
	if loadErr != nil {
		t.Fatalf("LoadConfig() error = %v", loadErr)
	...
	if len(cfg.NLSkills) != 0 {
		t.Fatalf("environment-disabled automatic extraction persisted a skill: %#v", cfg.NLSkills)
	}
}
...
func TestExperienceSkillStoreAllowsSafeAgentCreatedSkill(t *testing.T) {
	...
	if cfg.NLSkills[0].TrustLevel != security.TrustLevelAgentCreated {
		t.Fatalf("learned skill trust level = %q, want %q", cfg.NLSkills[0].TrustLevel, security.TrustLevelAgentCreated)
	}
	audit, err := os.ReadFile(cskill.DefaultEvolutionAuditPath())
	if err != nil {
		t.Fatalf("read automatic experience audit: %v", err)
	}
	for _, want := range []string{"definition_experience_register_started", "definition_experience_registered", "automatic_experience"} {
		if !strings.Contains(string(audit), want) {
			t.Fatalf("automatic experience audit missing %q: %s", want, audit)
		}
	}
}

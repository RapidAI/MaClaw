...
func (h *IMMessageHandler) toolListMCPTools(args map[string]interface{}) string {
	...
	if registry := h.getMCPRegistry(); registry != nil {
		for _, s := range registry.ListServers() {
			// Runtime readiness is an admission boundary, not merely a health
			// display hint. Do not let tool inventory perform a best-effort
			// network fetch for a managed server whose checked runtime sync is
			// pending or requires review.
			if h.app != nil && h.app.mcpRuntimeSyncPending(s.ID) {
				continue
			}
...
func (h *IMMessageHandler) toolInstallSkillHub(args map[string]interface{}) string {
	...
	if existingInstall == nil && h.app.skillNameAlreadyRegistered(entry.Name) {
		cskill.CleanupStaging(stagingDir)
		return fmt.Sprintf("Skill 安装被拒绝：名称 %q 已被其它身份占用", entry.Name)
	}
	...
	// Developer mode records scan findings but never blocks installation.
	{
		...
		if h.app.skillInstallScanShouldBlockForSource(scanReport, effectiveSource) {
			cskill.CleanupStaging(stagingDir)
		...
		if h.app.skillInstallReviewNeedsConfirmationForSource(scanReport, effectiveSource) {
	...
	// All file-backed and config-only installs use the App-owned shared
	// transaction. Normalization runs while staged; if it changes package
	// files, regenerate the integrity manifest before publication.
	if stagingDir != "" {
		var stagedManifest *cskill.PackageManifest
		stagedManifest, err = cskill.ReadPackageManifest(stagingDir)
		if err != nil {
			cskill.CleanupStaging(stagingDir)
			return fmt.Sprintf("安装失败（包完整性清单不可读）: %s", err.Error())
		}
		if stagedManifest != nil {
			if verifyErr := cskill.VerifyPackageIntegrity(stagingDir, stagedManifest); verifyErr != nil {
				cskill.CleanupStaging(stagingDir)
				return fmt.Sprintf("安装失败（包完整性校验不通过）: %s", verifyErr.Error())
			}
		}
		entry = h.app.normalizeInstalledSkill(entry)
		if stagedManifest != nil {
			manifestID := firstNonEmpty(entry.SkillID, entry.HubSkillID, entry.Name)
			manifestVersion := firstNonEmpty(entry.Version, entry.HubVersion)
			regenerated, genErr := cskill.GeneratePackageManifest(stagingDir, manifestID, manifestVersion)
			if genErr != nil {
				cskill.CleanupStaging(stagingDir)
				return fmt.Sprintf("安装失败（重新生成完整性清单）: %s", genErr.Error())
			}
			if writeErr := cskill.WritePackageManifest(stagingDir, regenerated); writeErr != nil {
				cskill.CleanupStaging(stagingDir)
				return fmt.Sprintf("安装失败（写入完整性清单）: %s", writeErr.Error())
			}
		}
		...
		if existingInstall == nil {
			commitErr = h.app.commitStagedSkillInstall(ctx, entry, stagingDir, effectiveSource, installScanReport, requestID, skillEvolutionConfigRevision(h.app))
		} else {
			commitErr = h.app.commitStagedSkillInstallWithExisting(ctx, entry, stagingDir, effectiveSource, installScanReport, requestID, skillEvolutionConfigRevision(h.app), existingInstall)
		}
		if commitErr != nil {
			return "Skill 安装失败: " + commitErr.Error()
		}
		go h.app.installSkillDepsIfMissing(entry.SkillDir, entry.Name)
	} else {
		if existingInstall != nil {
			entry.Name = existingInstall.Name
		}
		if err := h.app.commitNLSkillDefinitionAfterAdmission(ctx, *entry, "install", "skill:definition_installed", true, "im_tool_"+effectiveSource, installScanReport); err != nil {
...
// missing or not a string. Delegates to the shared agentruntime helper.
func stringVal(m map[string]interface{}, key string) string {
	return agentruntime.StringMapValue(m, key)
}
...
// commitIMSkillDefinitionPatch is the single durable write boundary for IM
// patch operations. The definition bytes, authoritative config entry,
// routing index, patch history and strict audit all participate in one
// compensating transaction; callers must not write any of them directly.
func (h *IMMessageHandler) commitIMSkillDefinitionPatch(ctx context.Context, target *corelib.NLSkillEntry, defPath, defFormat string, original, modified []byte, patch patchRecord) error {
	if h == nil || h.app == nil || h.app.skillExecutor == nil {
		return fmt.Errorf("Skill Executor is not initialized")
	}
	if target == nil || strings.TrimSpace(target.Name) == "" {
		return fmt.Errorf("skill target is required")
	}
	if strings.TrimSpace(defPath) == "" || !filepath.IsAbs(defPath) {
		return fmt.Errorf("skill definition path must be absolute")
	}
	if strings.TrimSpace(defFormat) == "" {
		return fmt.Errorf("skill definition format is required")
	}
	// Parse the post-image before entering the transaction so malformed or
	// identity-changing patches cannot alter config or create a queue record.
	sf, err := cskill.ParseSkillDefinitionFile(modified, defFormat)
	if err != nil {
		return fmt.Errorf("patched skill definition is invalid; refused to save: %w", err)
	}
	...
	if !strings.EqualFold(name, target.Name) {
		return fmt.Errorf("patched skill definition changes skill name from %q to %q", target.Name, name)
	}
	...
	for _, step := range sf.Steps {
		steps = append(steps, skillYAMLStepToEntryStep(step))
	}
	candidate := importedSkillEntryFromDefinition(sf, target.Name, status, target.SkillDir, steps, defPath)
	if candidate == nil {
		return fmt.Errorf("failed to construct patched skill entry")
	}
	cskill.NormalizeSkillForRunner(candidate)

	patchPath := filepath.Join(target.SkillDir, ".patches.json")
	if !filepath.IsAbs(patchPath) {
		return fmt.Errorf("patch history path must be absolute")
	}
	// Serialize the final definition/patch-history/index/audit transaction with
	// every other Skill writer. All parsing and policy checks above stay outside
	// this boundary; from the pre-image snapshot onward the lock prevents a
	// stale registry/sidecar state from being committed.
	...
	patchBefore, patchExists, err := readPatchHistorySnapshot(patchPath)
	if err != nil {
		return fmt.Errorf("read patch history before commit: %w", err)
	}
	...
	if normalizeSkillEntryStatus(target.Status) == skillEntryStatusActive {
		if err := ensureSkillEvolutionMutationAdmission(h.app, target.Name); err != nil {
			return err
		}
	}
	h.app.ensureRemoteInfra()
	requestID := fmt.Sprintf("evo_patch_%d", time.Now().UnixNano())
	ctx = cskill.WithEvolutionRequestMetadata(ctx, requestID, 1)
	configRevision := skillEvolutionConfigRevision(h.app)
	startData := map[string]string{
		"skill": target.Name, "action": "patch", "decision": "pending", "via": "im_patch",
		"request_id": requestID, "attempt": "1", "config_revision": configRevision,
		"schema_version": "2", "evidence_mode": "none",
	}
	if err := cskill.RecordEvolutionEventStrict("skill:definition_patch_started", startData, "desktop"); err != nil {
		return fmt.Errorf("patch audit preflight failed: %w", err)
	}

	committer := &cskill.SkillCommitter{
		SkillLoader: func() []corelib.NLSkillEntry { return h.app.skillExecutor.loadSkills() },
		SkillSaver: func(skills []corelib.NLSkillEntry) error {
			return h.app.skillExecutor.withSkillListMutate(func() error { return h.app.skillExecutor.saveSkills(skills) })
		},
		RollbackSkillSaver: func(skills []corelib.NLSkillEntry) error {
			return h.app.skillExecutor.withSkillListMutate(func() error { return h.app.skillExecutor.restoreSkillsSnapshot(skills) })
		},
		EntryMerger: mergeIMPatchedSkillDefinitionEntry,
		DefinitionWriterWithCompensation: func(_ *corelib.NLSkillEntry, record *cskill.EvolutionCompensationRecord) error {
			currentDefinition, err := os.ReadFile(defPath)
			if err != nil {
				return err
			}
			if !bytes.Equal(currentDefinition, original) {
			...
			currentPatch, currentPatchExists, err := readPatchHistorySnapshot(patchPath)
			if err != nil {
				return err
			}
			if currentPatchExists != patchExists || !bytes.Equal(currentPatch, patchBefore) {
				return fmt.Errorf("patch history changed concurrently; patch was not applied")
			}
			patchData, err := buildPatchHistoryBytes(target.SkillDir, patch)
			if err != nil {
				return err
			}
			if err := fileutil.AtomicWriteFile(patchPath, patchData, 0644); err != nil {
				return err
			}
			return record.SetFileSnapshotPostImage(patchPath, patchData, true)
		},
		CompensationMutator: func(record *cskill.EvolutionCompensationRecord) {
			record.SetFileSnapshots([]cskill.EvolutionFileSnapshot{{
				Path: patchPath, Exists: patchExists,
				BackupB64: base64.StdEncoding.EncodeToString(patchBefore),
			}})
		},
		IndexRefresher: func() error { return h.app.refreshSkillIndexesAfterMutationChecked(target.Name) },
		RollbackIndexRefresher: func() error {
			return h.app.refreshSkillIndexesAfterRollbackChecked(target.Name, h.app.skillExecutor.loadSkills())
		},
	...
	result := committer.Commit(ctx, target.Name, candidate, "skill:definition_patched", map[string]string{
		"skill": target.Name, "action": "patch", "via": "im_patch", "find": patch.Find,
		"request_id": requestID, "attempt": "1", "config_revision": configRevision,
		"schema_version": "2", "evidence_mode": "none",
	})
	if result.State != "committed" || result.CleanupStatus != "clear" {
		return fmt.Errorf("skill patch not committed: %s", result.FailureReason)
	}
	return nil
}

// mergeIMPatchedSkillDefinitionEntry applies every definition-owned field
// parsed from skill.yaml while preserving stable identity and runtime usage
// counters maintained by the executor.
func mergeIMPatchedSkillDefinitionEntry(dst, src *corelib.NLSkillEntry) {
	if dst == nil || src == nil {
		return
	}
	identity := cskill.CloneNLSkillEntry(dst)
	updated := cskill.CloneNLSkillEntry(src)
	if identity != nil {
		updated.SkillID = identity.SkillID
		updated.Name = identity.Name
		updated.DirName = identity.DirName
		updated.CreatedAt = identity.CreatedAt
		updated.Source = identity.Source
		updated.SourceProject = identity.SourceProject
		updated.ExperienceDomain = identity.ExperienceDomain
		updated.HubSkillID = identity.HubSkillID
		updated.HubVersion = identity.HubVersion
		updated.TrustLevel = identity.TrustLevel
		updated.Publisher = identity.Publisher
		updated.SkillDir = identity.SkillDir
		updated.UsageCount = identity.UsageCount
		updated.SuccessCount = identity.SuccessCount
		updated.FailureCount = identity.FailureCount
		updated.WorkaroundCount = identity.WorkaroundCount
		updated.LastUsedAt = identity.LastUsedAt
		updated.LastError = identity.LastError
		updated.RepairAttemptCount = identity.RepairAttemptCount
		updated.LastRepairAt = identity.LastRepairAt
		updated.RepairHistory = identity.RepairHistory
		updated.OptimizationCount = identity.OptimizationCount
		updated.LastOptimizedAt = identity.LastOptimizedAt
		updated.DiscoveredFrom = identity.DiscoveredFrom
		updated.SolidificationCandidates = identity.SolidificationCandidates
		updated.TotalTokensCost = identity.TotalTokensCost
	}
	*dst = *updated
}

func readPatchHistorySnapshot(path string) ([]byte, bool, error) {
...
func buildPatchHistoryBytes(skillDir string, record patchRecord) ([]byte, error) {
	patchesPath := filepath.Join(skillDir, ".patches.json")
	data, exists, err := readPatchHistorySnapshot(patchesPath)
	if err != nil {
		return nil, err
	}
	...
	if exists {
		if jsonErr := json.Unmarshal(data, &records); jsonErr != nil {
			return nil, fmt.Errorf("parse patch history: %w", jsonErr)
		}
	}
...
// definition file. It validates the result before entering the shared
// definition/config/index/audit transaction and records patch history as a
// compensatable sidecar.
...
func (h *IMMessageHandler) toolPatchSkillText(skillName string, args map[string]interface{}) string {
	...
	if err := h.commitIMSkillDefinitionPatch(context.Background(), target, defPath, defFormat, content, []byte(modified), patchRecord{
...
func (h *IMMessageHandler) toolPatchSkillStructured(skillName string, args map[string]interface{}) string {
	...
	if err := h.commitIMSkillDefinitionPatch(context.Background(), target, defPath, defFormat, content, modified, patchRecord{
		Timestamp: time.Now().Format(time.RFC3339),
...
// toolUploadSkillSuite packages several installed Skills as one distributable
// Suite. The model may pass names as a JSON array (or a comma-separated string
// for compatibility); the App performs the same outbound quality/security
// gates used by single-Skill uploads.
func (h *IMMessageHandler) toolUploadSkillSuite(args map[string]interface{}) string {
	var names []string
	...
	if len(names) == 0 {
		if legacy := strings.TrimSpace(stringVal(args, "name")); legacy != "" {
	...
	if len(names) < 2 {
		return "至少需要 names 中的两个本地 Skill 才能创建 Suite"
	}
	force := false
	if v, ok := args["force"].(bool); ok {
		force = v
	} else if v, ok := args["force"].(string); ok {
		force = strings.EqualFold(strings.TrimSpace(v), "true")
	}
	suiteName := stringVal(args, "suite_name")
	if !force {
		for _, name := range names {
			if gate := h.runUploadPortabilityGate(name); gate != "" {
				return fmt.Sprintf("Suite 上传前 Skill「%s」检查失败: %s", name, gate)
			}
			if idErr := h.ensureSkillIDForUpload(name); idErr != "" {
				return idErr
			}
	...
	id, err := h.app.UploadSkillSuite(names, suiteName, force)
	if err != nil {
		return fmt.Sprintf("Suite 上传失败: %s", err.Error())
	}
	return fmt.Sprintf("Skill Suite「%s」已上传到 SkillMarket，提交 ID: %s（成员 %d 个）", firstNonEmpty(suiteName, strings.Join(names, " + ")), id, len(names))
}
...
func (h *IMMessageHandler) runUploadPortabilityGate(name string) string {
	...
	if err != nil {
		if restoreErr := restoreSkillDirFromSnapshot(skillDir, snapshotDir); restoreErr != nil {
			return fmt.Sprintf("上传前可移植性检查失败: %s；回滚失败: %v", err.Error(), restoreErr)
		}
	...
	if len(result.AutoFixed) > 0 {
		...
		if h.app == nil {
			if restoreErr := restoreSkillDirFromSnapshot(skillDir, snapshotDir); restoreErr != nil {
				return fmt.Sprintf("可移植性自动修复缺少事务上下文；回滚失败: %v", restoreErr)
			}
			return "可移植性自动修复缺少事务上下文，已回滚改动"
		}
		if indexErr := h.app.refreshSkillIndexesAfterMutationChecked(name); indexErr != nil {
			if restoreErr := h.restoreManagedSkillDirAndIndex(skillDir, snapshotDir, name); restoreErr != nil {
				return fmt.Sprintf("可移植性自动修复后索引刷新失败: %s；回滚失败: %v", indexErr.Error(), restoreErr)
			}
			return fmt.Sprintf("可移植性自动修复后索引刷新失败，已回滚改动: %s", indexErr.Error())
		}
	}
...
func (h *IMMessageHandler) ensureSkillIDForUpload(name string) string {
	...
	// Skill ID generation writes the authoritative definition. Serialize that
	// write with installs/patches and keep a pre-image so a checked index
	// publication failure cannot leave a durable YAML change that the runtime
	// cannot route.
	...
	if strings.TrimSpace(target.SkillDir) != "" {
		var snapshotErr error
		snapshotDir, cleanupSnapshot, snapshotErr = snapshotSkillDirForRollback(target.SkillDir)
		if snapshotErr != nil {
			return fmt.Sprintf("Skill ID 生成失败：无法创建回滚快照: %s", snapshotErr.Error())
		}
		defer cleanupSnapshot()
	}
	...
	if err != nil {
		if snapshotDir != "" {
			if restoreErr := restoreSkillDirFromSnapshot(target.SkillDir, snapshotDir); restoreErr != nil {
				return fmt.Sprintf("Skill ID 生成失败: %s；回滚失败: %v", err.Error(), restoreErr)
			}
		}
	...
	// Publish the new identity through the checked index boundary. If that
	// fails, restore both the definition and derived indexes before returning.
	if h.app != nil {
		if indexErr := h.app.refreshSkillIndexesAfterMutationChecked(name); indexErr != nil {
			if snapshotDir == "" {
				return fmt.Sprintf("Skill ID 已生成但索引刷新失败: %s", indexErr.Error())
			}
			if restoreErr := h.restoreManagedSkillDirAndIndex(target.SkillDir, snapshotDir, name); restoreErr != nil {
				return fmt.Sprintf("Skill ID 索引刷新失败: %s；回滚失败: %v", indexErr.Error(), restoreErr)
			}
			return fmt.Sprintf("Skill ID 索引刷新失败，已回滚改动: %s", indexErr.Error())
		}
	}
...
func (h *IMMessageHandler) toolValidateSkill(args map[string]interface{}) string {
	...
	if fixErr != nil {
		if restoreErr := restoreSkillDirFromSnapshot(skillDir, snapshotDir); restoreErr != nil {
			return fmt.Sprintf("automatic repair failed: %s; rollback failed: %v\n\n%s", fixErr.Error(), restoreErr, cskill.FormatPortabilityReport(report))
		}
		return fmt.Sprintf("automatic repair failed and changes were rolled back: %s\n\n%s", fixErr.Error(), cskill.FormatPortabilityReport(report))
	}
	...
	if h.app == nil {
		if restoreErr := restoreSkillDirFromSnapshot(skillDir, snapshotDir); restoreErr != nil {
			return fmt.Sprintf("repair lacks transaction context: rollback failed: %v\n\n%s", restoreErr, cskill.FormatPortabilityChanges(changes))
		}
		return "repair lacks transaction context; changes were rolled back\n\n" + cskill.FormatPortabilityChanges(changes)
	}
	if indexErr := h.app.refreshSkillIndexesAfterMutationChecked(name); indexErr != nil {
		if restoreErr := h.restoreManagedSkillDirAndIndex(skillDir, snapshotDir, name); restoreErr != nil {
			return fmt.Sprintf("repair index refresh failed: %s; rollback failed: %v\n\n%s", indexErr.Error(), restoreErr, cskill.FormatPortabilityChanges(changes))
		}
		return fmt.Sprintf("repair index refresh failed and changes were rolled back: %s\n\n%s", indexErr.Error(), cskill.FormatPortabilityChanges(changes))
	}
	var b strings.Builder
...
// restoreManagedSkillDirAndIndex restores a portability auto-fix snapshot and
// then rebuilds the derived routing index from the authoritative pre-image.
// A successful directory restore without a successful index restore is still
// an incomplete transaction and must remain visible to the caller.
func (h *IMMessageHandler) restoreManagedSkillDirAndIndex(skillDir, snapshotDir, name string) error {
	if err := restoreSkillDirFromSnapshot(skillDir, snapshotDir); err != nil {
		return err
	}
	if h == nil || h.app == nil {
		return nil
	...
	if exec := h.getSkillExecutor(); exec != nil {
		restored = exec.loadSkills()
	}
	return h.app.refreshSkillIndexesAfterRollbackChecked(name, restored)
}

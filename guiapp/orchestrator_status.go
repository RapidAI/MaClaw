...
// RuntimeStatusValue projects orchestration-specific states onto the shared
// asynchronous Runtime vocabulary. Partial failures remain unknown because
// the shared contract intentionally has no misleading partial-success state.
func (s orchestratorTaskStatus) RuntimeStatusValue() agentruntime.JobStatus {
	return agentruntime.ProjectLegacyJobStatus(s.Normalized().String())
}

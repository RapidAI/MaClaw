...
func (a *App) installMixedSkillWithIntegrityAndLocatorTrace(source, id, installRef, downloadLocator, expectedPackageSHA256, expectedPackageSignature string) (trace skillHubDownloadTrace, retErr error) {
	...
	defer func() {
		...
		if rollbackErr != nil {
			...
			if markErr := cskill.MarkEvolutionCompensationRollbackFailure(&installCompensation, rollbackErr); markErr != nil {
				installCompensation.LastError += "; persist rollback state: " + markErr.Error()
			}
		...
		if clearErr := cskill.ClearEvolutionCompensation(requestID, installCompensation.Skill, "install"); clearErr != nil {
			...
			if stateErr := cskill.MarkEvolutionCompensationRollbackFailure(&installCompensation, clearErr); stateErr != nil {
				retErr = fmt.Errorf("%v; install rolled back; compensation cleanup failed: %v; persist compensation: %w", retErr, clearErr, stateErr)
			} else {
	...
	commitSimpleInstall := func(entry *corelib.NLSkillEntry, report *cskill.ScanReport) error {
		...
		// Config-only mixed installs use the same App adapter as every other
		// config mutation. This keeps locking, no-op handling, compensation,
		// checked-index refresh and final-audit semantics in one place.
		return a.commitNLSkillDefinitionAfterAdmission(
			ctx, *entry, "install", "skill:definition_installed", true,
			"mixed_"+strings.ToLower(strings.TrimSpace(string(kind))), report,
		)
	}
	switch kind {
	case skillSearchSourceEnterpriseHub:
	...
	case skillSearchSourceSkillMarket, skillSearchSourceSkillHub:
		...
		// Directory-backed mixed installs and replacements share exactly one
		// publication transaction.  The former legacy replacement adapter had
		// its own backup, register, index and audit sequence; it could therefore
		// diverge from the create path or leave nested compensation state.  The
		// shared committer preserves the installed runtime counters while keeping
		// the old package in .prev until config/index/final-audit have committed.
		if strings.TrimSpace(skill.SkillDir) != "" {
			if existing == nil && a.skillNameAlreadyRegistered(skill.Name) {
			...
			var commitErr error
			if existing == nil {
				commitErr = a.commitStagedSkillInstall(ctx, skill, stagingDir, string(kind), report, requestID, configRevision)
			} else {
				commitErr = a.commitStagedSkillInstallWithExisting(ctx, skill, stagingDir, string(kind), report, requestID, configRevision, existing)
			}
			if commitErr != nil {
				return trace, commitErr
			}
			return trace, nil
		}
		// Some legacy HubCenter payloads describe only a config entry. They still
		// need a single durable config/index/audit commit; never fall through to
		// the former hand-written Register sequence below.
		if existing != nil {
		...
		if existing == nil {
			...
			return trace, fmt.Errorf("mixed install reached retired direct registration path")
		}
		...
		if installCompensation.DirMoved && strings.TrimSpace(installCompensation.DirBackupPath) != "" {
			if err := os.RemoveAll(installCompensation.DirBackupPath); err != nil {
				if markErr := cskill.MarkEvolutionCompensationCleanupFailure(&installCompensation, err); markErr != nil {
					return trace, fmt.Errorf("install committed but backup cleanup state could not persist: %w", markErr)
				}
		...
		if err := cskill.ClearEvolutionCompensation(requestID, skill.Name, "install"); err != nil {
			if markErr := cskill.MarkEvolutionCompensationCleanupFailure(&installCompensation, err); markErr != nil {
				return trace, fmt.Errorf("install committed but compensation cleanup state could not persist: %w", markErr)
			}
...
func (a *App) installedSkillForInstall(candidate *corelib.NLSkillEntry) *corelib.NLSkillEntry {
	...
	wantSkillID := strings.TrimSpace(candidate.SkillID)
	wantName := strings.TrimSpace(candidate.Name)
	wantDirName := strings.TrimSpace(candidate.DirName)
	// Read one authoritative snapshot. Two independent loadSkills calls could
	// straddle another registry write and return an entry that never existed as
	// a complete state; the caller already holds installMutex at commit time.
	installed := a.skillExecutor.loadSkills()
	// When the candidate carries a stable identity, a same-name legacy entry is
	// not a valid replacement target. HubSkillID is the package authority;
	// SkillID is checked when the installed entry has one (older installs may
	// legitimately lack it). Otherwise return nil so the caller can fail closed
	// on a name collision instead of overwriting another publisher's Skill.
	if wantID != "" || wantSkillID != "" {
		for _, existing := range installed {
			if wantID != "" && !strings.EqualFold(strings.TrimSpace(existing.HubSkillID), wantID) {
				continue
			}
			if wantSkillID != "" && strings.TrimSpace(existing.SkillID) != "" && !strings.EqualFold(strings.TrimSpace(existing.SkillID), wantSkillID) {
				continue
			}
			copy := existing
			return &copy
	...
	for _, existing := range installed {
		if (wantName != "" && (strings.EqualFold(strings.TrimSpace(existing.Name), wantName) || existing.MatchesName(wantName))) ||
			(wantDirName != "" && existing.MatchesName(wantDirName)) {
			copy := existing
			return &copy
		}
	}
...
// commitAutoSummaryStagedSkillInstall labels the otherwise shared directory
// transaction with its non-interactive origin. A staged auto-summary has the
// same rollback mechanics as an operator install, but auditing it as an
// operator action would obscure the policy boundary that admitted it.
func (a *App) commitAutoSummaryStagedSkillInstall(ctx context.Context, entry *corelib.NLSkillEntry, stagingDir string, report *cskill.ScanReport, requestID, configRevision string) error {
	return a.commitStagedSkillInstallWithExistingInRootAndVia(
		ctx, entry, stagingDir, "auto_summary", report, requestID, configRevision, nil, "", "automatic_auto_summary",
	)
}
...
func (a *App) commitStagedSkillInstallWithExisting(ctx context.Context, entry *corelib.NLSkillEntry, stagingDir, source string, report *cskill.ScanReport, requestID, configRevision string, existing *corelib.NLSkillEntry) error {
	return a.commitStagedSkillInstallWithExistingInRoot(ctx, entry, stagingDir, source, report, requestID, configRevision, existing, "")
}

// commitStagedSkillInstallWithExistingInRoot is the directory transaction for
// an explicitly selected, trusted installation root. Most callers must use
// commitStagedSkillInstallWithExisting so the primary skills root remains the
// default. A caller may provide a root only after it has validated the exact
// installed target against its authoritative registry entry; this keeps a
// stale UI/plan path from redirecting a package publication.
func (a *App) commitStagedSkillInstallWithExistingInRoot(ctx context.Context, entry *corelib.NLSkillEntry, stagingDir, source string, report *cskill.ScanReport, requestID, configRevision string, existing *corelib.NLSkillEntry, finalRoot string) error {
	return a.commitStagedSkillInstallWithExistingInRootAndVia(ctx, entry, stagingDir, source, report, requestID, configRevision, existing, finalRoot, "operator")
}

// commitStagedSkillInstallWithExistingInRootAndVia is the implementation
// shared by explicit installs and narrow automatic producers. The caller must
// provide a stable audit origin; it never changes package admission or the
// durable directory/config transaction semantics.
func (a *App) commitStagedSkillInstallWithExistingInRootAndVia(ctx context.Context, entry *corelib.NLSkillEntry, stagingDir, source string, report *cskill.ScanReport, requestID, configRevision string, existing *corelib.NLSkillEntry, finalRoot, via string) error {
	if a == nil || a.skillExecutor == nil || entry == nil {
		cskill.CleanupStaging(stagingDir)
		return fmt.Errorf("skill install transaction is not configured")
	}
	// Bind the durable compensation identity to the request ID already used by
	// pre/final audit rows. Without this context metadata a crash between final
	// audit and committed-state persistence could not be recognized safely.
	ctx = cskill.WithEvolutionRequestMetadata(ctx, requestID, 1)
	// Directory publication and the accompanying registry/index/audit commit
	// form one process-local critical section. Callers deliberately do not own
	// installMutex: this boundary is shared by Wails, IM, capability-market and
	// background update paths, so serializing here prevents a stale pre-image or
	// competing .prev rename from crossing another install/delete transaction.
	a.skillExecutor.suspendStatusOverlayPersistence()
	defer a.skillExecutor.resumeStatusOverlayPersistence()
	a.installMutex.Lock()
	...
	// Admission must be checked again inside the publication lock. Callers do
	// the network download and scan outside this critical section, so the
	// compensation queue may have become unreadable (or acquired a pending
	// record for this identity) while they were waiting. Do this before scan
	// cache writes, path rewrites, or any directory/config side effect.
	...
	// Resolve the existing entry again while the install lock is held. Callers
	// commonly discover it before downloading/scanning, so the pointer they
	// pass can be stale by the time publication starts. Never use that stale
	// pre-image: a concurrent delete/update must either be observed here or
	// fail closed rather than have its newer registry state overwritten.
	providedExisting := existing != nil
	resolvedExisting := a.installedSkillForInstall(entry)
	if resolvedExisting == nil && providedExisting {
		cskill.CleanupStaging(stagingDir)
		return fmt.Errorf("existing skill installation is no longer authoritative; retry")
	}
	existing = resolvedExisting
	// Keep idempotency at the shared transaction boundary as well as at
	// individual callers. This protects newly added install entry points from
	// publishing a package that is already authoritative locally.
	...
	// Verify upload-time integrity evidence while the package is still staged,
	// before publishing it into the authoritative Skill root. A malformed or
	// unreadable manifest is not equivalent to an absent legacy manifest and
	// must fail closed without creating a durable install side effect.
	if stagingDir != "" {
		manifest, manifestErr := cskill.ReadPackageManifest(stagingDir)
		if manifestErr != nil {
			cskill.CleanupStaging(stagingDir)
			return fmt.Errorf("read package integrity manifest: %w", manifestErr)
		}
		if manifest != nil {
			if verifyErr := cskill.VerifyPackageIntegrity(stagingDir, manifest); verifyErr != nil {
				cskill.CleanupStaging(stagingDir)
				return fmt.Errorf("package integrity verification failed: %w", verifyErr)
			}
		}
	}
	...
	finalRoot = strings.TrimSpace(finalRoot)
	if finalRoot == "" {
		var err error
	...
	if err := cskill.RecordEvolutionEventStrict("skill:definition_install_started", map[string]string{
		"skill": entry.Name, "action": "install", "decision": "pending", "via": via, "source": source,
		"request_id": requestID, "attempt": "1", "config_revision": configRevision,
		"schema_version": "2", "evidence_mode": "none",
	}, "desktop"); err != nil {
		cskill.CleanupStaging(stagingDir)
	...
	committer := &cskill.SkillCommitter{
		...
		ExternalRollback: func() error {
			...
			removeOwned := func(path string) error {
				var last error
			...
			if published || existing == nil {
				if err := removeOwned(finalDir); err != nil {
					return err
				}
			}
			if existing != nil {
				backup := finalDir + ".prev"
				// A scanner/file watcher can briefly recreate the destination after
				// the first remove.  Treat rename success as provisional and verify
				// the post-condition (destination exists, backup is gone); otherwise
				// retry while removing only this transaction-owned destination.
				var lastErr error
				for attempt := 0; attempt < 40; attempt++ {
					if _, err := os.Stat(backup); os.IsNotExist(err) {
						if _, dstErr := os.Stat(finalDir); dstErr == nil {
							return nil
						...
						if err := cskill.RetryDirectoryRename(backup, finalDir); err != nil {
							lastErr = err
						}
					}
					if _, srcErr := os.Stat(backup); os.IsNotExist(srcErr) {
						if _, dstErr := os.Stat(finalDir); dstErr == nil {
							return nil
						}
					}
				...
				return fmt.Errorf("restore previous skill backup did not reach post-condition")
			}
			return nil
		},
	...
	auditData := map[string]string{
		"skill": entry.Name, "action": "install", "decision": "applied", "via": via, "source": source,
		"request_id": requestID, "attempt": "1", "config_revision": configRevision,
		"schema_version": "2", "evidence_mode": "none",
	}
	...
	if result.State != "committed" || result.CleanupStatus != "clear" {
		if cleanupErr := cskill.CleanupStagingChecked(stagingDir); cleanupErr != nil {
			return fmt.Errorf("install not committed: state=%s cleanup_status=%s reason=%s; staging cleanup failed: %w", result.State, result.CleanupStatus, result.FailureReason, cleanupErr)
		}
...
// ResolveFrontendConfirm is called by CustomDialog after the user answers a
// backend-emitted show-confirm prompt (cloud workspace conflict, steal, …).
func (a *App) ResolveFrontendConfirm(id string, confirmed bool) error {
	return resolveFrontendConfirm(id, confirmed)
}
...
func (a *App) ImportAgentSkillDir(skillDir string) (string, error) {
	...
	if err := a.commitNLSkillDefinitionAfterAdmission(
		context.Background(), *entry,
		"agent_skill_import", "skill:definition_agent_skill_imported", true,
		"operator", report,
	); err != nil {
		return "", err
	}

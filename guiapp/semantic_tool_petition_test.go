...
func TestGrantLegacySQLDatabasePetitionOverlaysLiveAdapter(t *testing.T) {
	h := &IMMessageHandler{registry: NewToolRegistry()}
	registerBuiltinTools(h.registry, h)
	cb := &sharedAgentLoopCallbacks{handler: h, userText: "查看 rapidbi库"}
	granted, message := cb.grantLegacySQLDatabasePetition("database")
	if !granted || !strings.Contains(message, "database") {
		t.Fatalf("live SQL adapter overlay: granted=%v message=%q", granted, message)
	}
	if !cb.legacyPetitionAllows("database") {
		t.Fatal("legacy SQL database petition was not recorded")
	}
	found := false
	for _, def := range cb.appendLegacyPetitionedDatabaseTools(nil) {
		if extractToolName(def) == "database" {
			found = true
			break
		}
	}
	if !found {
		t.Fatal("overlay did not add the live database definition")
	}
	if granted, _ := cb.grantLegacySQLDatabasePetition("database"); !granted {
		t.Fatal("repeat overlay must stay granted")
	}
	cb.setVisibleToolDefinitions(nil)
	foundAfterSync := false
	...
	if !foundAfterSync {
		t.Fatal("setVisibleToolDefinitions dropped the SQL database overlay")
	}
	if semanticToolsSearchStatus(cb, "database") != "[已在当前工具面]" {
		t.Fatalf("tools_search status after overlay = %q", semanticToolsSearchStatus(cb, "database"))
	}
}

func TestBuildToolsForModelRequestOverlaysSQLDatabaseOnCatalogFollowUp(t *testing.T) {
	cb := petitionTestOfficeCallbacks(t, &intent.ClassificationResult{Primary: intent.LabelSearch, Confidence: .98})
	found := false
	for _, def := range cb.BuildToolsForModelRequest("查看 rapidbi库", 0) {
		if extractToolName(def) == "database" {
			found = true
			break
		}
	...
	if !found {
		t.Fatal("first managed request for a catalog follow-up must list database")
	}
	if !cb.legacyPetitionAllows("database") || !cb.IsToolAllowed("database") {
		t.Fatal("auto overlay must make database callable without a petition round")
	}
}

func TestSemanticToolCallPetitionGrantsLiveSQLDatabaseWhenMISExpansionFails(t *testing.T) {
	cb := petitionTestOfficeCallbacks(t, &intent.ClassificationResult{Primary: intent.LabelSearch, Confidence: .98})
	cb.userText = "查看 rapidbi库"
	granted, message := cb.PetitionToolCall("database")
	if !granted || !strings.Contains(message, "database") {
		t.Fatalf("database petition on a search turn must be admitted: granted=%v message=%q", granted, message)
	}
	if !cb.legacyPetitionAllows("database") && !cb.IsToolAllowed("database") {
		t.Fatal("petitioned database must be callable via overlay or semantic grant")
	}
	found := false
	for _, def := range cb.BuildToolsForModelRequest(cb.userText, 1) {
		switch extractToolName(def) {
		case "database", "database_query", "mis_data":
			found = true
		}
	}
	if !found {
		t.Fatal("widened request surface lacks a database-family tool")
	}
}

func TestSemanticToolCallPetitionDoesNotGrantSQLDatabaseForUnsupportedEngine(t *testing.T) {
	cb := petitionTestOfficeCallbacks(t, &intent.ClassificationResult{Primary: intent.LabelSearch, Confidence: .98})
	cb.userText = "查看 oracle 数据库"
	granted, message := cb.PetitionToolCall("database")
	if granted || message != "" {
		t.Fatalf("unsupported engine must not overlay the SQL adapter: granted=%v message=%q", granted, message)
	}
	if cb.legacyPetitionAllows("database") {
		t.Fatal("oracle request recorded a SQL database overlay")
	}
}

...
func configureCloudWorkspaceEntitlementTestApp(t *testing.T, hubURL string) *App {
	...
	seedCloudWorkspaceInstanceSession(hubURL, "machine-token", "machine-test")
	return app
}

func seedCloudWorkspaceInstanceSession(hubURL, machineToken, machineID string) {
	key := cloudWorkspaceInstanceSessionCacheKey(hubURL, machineID, machineToken)
	cloudWorkspaceSessions.Lock()
	cloudWorkspaceSessions.items[key] = &cloudWorkspaceInstanceSessionEntry{session: cloudWorkspaceInstanceSession{
		SessionID:        "cwses_test",
		SessionToken:     "cwst_test",
		ClientInstanceID: "cwi_server_issued_test",
		Protocol:         cloudWorkspaceProtocolVersion,
...
func TestCloudWorkspaceEntitlementEnabledCachesWorkspaces(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		_, _ = w.Write([]byte(`{
			...
			"logical_bytes":10,
			"retained_bytes":18,
			"snapshot_retained_bytes":8,
			"staging_bytes":0,
			"tenant_max_total_bytes":53687091200,
			"workspaces":[{"id":"cws_0123456789abcdef0123456789abcdef","name":"标书项目","used_bytes":10,"logical_bytes":10,"retained_bytes":18,"snapshot_retained_bytes":8,"updated_at":"2026-08-28T10:00:00Z"}],
	...
	if ent.LogicalBytes != 10 || ent.RetainedBytes != 18 || ent.SnapshotBytes != 8 || ent.TenantMaxTotalBytes != 53687091200 {
		t.Fatalf("retained usage=%+v", ent)
	}
	if ent.Workspaces[0].LogicalBytes != 10 || ent.Workspaces[0].RetainedBytes != 18 || ent.Workspaces[0].SnapshotBytes != 8 {
		t.Fatalf("workspace retained usage=%+v", ent.Workspaces[0])
	}
...
func TestCloudWorkspaceEntitlementDeletedPurgesAllLocalCopies(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		_, _ = w.Write([]byte(`{"enabled":true,"workspaces":[],"deleted":[{"id":"cws_revoked"}]}`))
	}))
	defer server.Close()
	app := configureCloudWorkspaceEntitlementTestApp(t, server.URL)
	writer := app.cloudWorkspaceCachePath("tenant_default", "cws_revoked")
	readonly := filepath.Join(app.GetDataDir(), "cloud-workspaces-readonly", "tenant_default", "cws_revoked", "instance-1")
	...
	if err := os.WriteFile(filepath.Join(writer, "secret.txt"), []byte("x"), 0o600); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(readonly, "secret.txt"), []byte("x"), 0o600); err != nil {
		t.Fatal(err)
	}
	_ = app.CloudWorkspaceEntitlement()
	if _, err := os.Stat(writer); !os.IsNotExist(err) {
		t.Fatalf("writer cache remains after entitlement revocation: err=%v", err)
	}
	if _, err := os.Stat(readonly); !os.IsNotExist(err) {
		t.Fatalf("read-only cache remains after entitlement revocation: err=%v", err)
	}
}

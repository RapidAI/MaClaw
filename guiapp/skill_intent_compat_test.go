...
func TestExtractUserIntentCategory(t *testing.T) {
	tests := []struct {
		...
		{"将 PDF 复制到当前工作区", intentCatModify},
		{"拷贝这份文档到桌面", intentCatModify},
		{"复制品.pdf 生成一份报告", intentCatGenerate},
		...
		{"copy the pdf into the workspace", intentCatModify},
...
func TestShouldPreferSkillForTask_QueryIntentBlocked(t *testing.T) {
	...
	if shouldPreferSkillForTask("将 C:\\output\\人工智能数学基础-v2.2.3.pdf 复制到当前工作区") {
		t.Error("copying an existing PDF is operate-on-existing, not a generate-skill turn")
	}
	if shouldPreferSkillForTask("copy the handbook pdf into the workspace") {
		t.Error("copying an existing PDF is operate-on-existing, not a generate-skill turn")
	}
}

...
func newLegacyToolSurface(definitions []map[string]interface{}) legacyToolSurface {
	return newLegacyToolSurfaceWithClientTools(definitions, nil)
}

// newLegacyToolSurfaceWithClientTools snapshots the host replacement
// definitions plus the client names that clientToolDefinitionsForAgent
// actually appended. Do not pass every LoopContext.ClientTools name: a hidden
// or colliding declaration must not become an executable binding.
func newLegacyToolSurfaceWithClientTools(definitions []map[string]interface{}, clientToolNames []string) legacyToolSurface {
	...
	return legacyToolSurface{names: names, contracts: contracts, clientToolNames: clients, epochs: &legacyToolSurfaceEpoch{}}
}
...
func (s legacyToolSurface) Clone() legacyToolSurface {
	...
	return legacyToolSurface{names: clone, contracts: contracts, clientToolNames: clientToolNames, epochs: &legacyToolSurfaceEpoch{}}
}
...
func (s legacyToolSurface) AllowsLiveProvision(name string) bool {
	name = strings.TrimSpace(name)
	// Request-local client bindings are outside the static host catalog.
	if s.IsClientTool(name) {
		return true
	}
	if s.names != nil {
		if _, exposed := s.names[name]; !exposed {
			return false
		}
	}

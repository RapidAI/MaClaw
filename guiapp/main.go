...
// version defaults to "dev"; cmd/maclaw-gui injects its ldflags-set value
// through Main at startup.
var version = "dev"

// Main is the entry point of the desktop application. The actual main
// package lives in cmd/maclaw-gui. If v is non-empty it overrides the
// package-level version (normally injected via -ldflags "-X main.version=..."
// on cmd/maclaw-gui).
func Main(v string) {
	if v != "" {
		version = v
	...
	// Native window background, visible before the WebView first-paints.
	// Match the HTML boot splash (#f3f5f7), not the dark assistant chrome;
	// a navy fill reads as a black flash on the compact env-check window.
	bgColour := &options.RGBA{R: 243, G: 245, B: 247, A: 255} // #f3f5f7
	frameless := true
	...
	appOptions := &options.App{
		...
		Windows: &windows.Options{
			...
			// Blank compact window with no asset requests is a known WebView2
			// GPU-compositor failure mode on some NVIDIA/high-DPI setups.
	...
	if raw, err := assets.ReadFile("frontend/dist/index.html"); err != nil {
		bootLog("embed index.html missing err=%v", err)
	} else {
		bootLog("embed index.html bytes=%d", len(raw))
	}
	...
	bootLog("wails.Run returned err=%v", err)
...
func embeddedFrontendFingerprint() (string, error) {
	...
	_, _ = io.WriteString(h, "\nwebview-profile-reset=20260903b")

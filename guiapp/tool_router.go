...
// Route delegates to the legacy text router. Task-scoped callers should use
// RouteForScope so user wording cannot change the tool surface.
...
// RouteWithOptions delegates to the legacy text router. It remains for
// compatibility while callers migrate to RouteForScope.
...
// RouteForScope returns the host-admitted tool surface without consulting
// user text or the legacy retrieval stack.
...
// RouteForScopeWithPolicy applies task-scoped policy gates before exposing a
// host-admitted surface. Denied policies return an empty surface and never
// fall back to text routing.
func (r *ToolRouter) RouteForScopeWithPolicy(cfg corelib.AppConfig, allTools []map[string]interface{}, allowedNames []string, coverageState string, selections, schemaTokens, iterations int) []map[string]interface{} {
	policy := cfg.EffectiveRoutingPolicy()
	if !policy.AllowsExecutableScope() || !policy.AllowsCatalogCoverage(coverageState) || !policy.WithinBudget(selections, schemaTokens, iterations) {
		return nil
	}
	return r.RouteForScope(allTools, allowedNames)
}

// RouteForScopePlan renders an immutable host-admitted scope plan and returns
// diagnostics for missing definitions or dependencies. It never consults the
// user message or the legacy retrieval stack.
...
// LegacyRouteCalls exposes migration telemetry for the deprecated text path.
func (r *ToolRouter) LegacyRouteCalls() uint64 {
	if r == nil || r.inner == nil {
		return 0
	}
	return r.inner.LegacyRouteCalls()
}
...
// SetSearchTextExtra delegates host-owned retrieval tokens to the inner router.
func (r *ToolRouter) SetSearchTextExtra(name string, queries []string) {
	if r == nil || r.inner == nil {
		return
	}
	r.inner.SetSearchTextExtra(name, queries)
}

// SearchTextExtra returns host-owned retrieval tokens for tests and diagnostics.
func (r *ToolRouter) SearchTextExtra(name string) []string {
	if r == nil || r.inner == nil {
		return nil
	}
	return r.inner.SearchTextExtra(name)
}

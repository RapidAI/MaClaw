...
func TestNewRemoteExperimentOrchestratorInitializesKnownBaselineAsBest(t *testing.T) {
	...
	if state.RuntimeStatus != agentruntime.JobStatusRunning {
		t.Fatalf("initial runtime status=%q, want running", state.RuntimeStatus)
	}
}

func TestExperimentRoundRuntimeStatusUsesSharedVocabulary(t *testing.T) {
	cases := map[string]agentruntime.JobStatus{
		"completed": agentruntime.JobStatusSucceeded,
		"failed":    agentruntime.JobStatusFailed,
		"timeout":   agentruntime.JobStatusFailed,
		"cancelled": agentruntime.JobStatusCanceled,
		"running":   agentruntime.JobStatusRunning,
		"partial":   agentruntime.JobStatusUnknown,
	}
	for input, want := range cases {
		if got := experimentRoundRuntimeStatus(input); got != want {
			t.Errorf("experimentRoundRuntimeStatus(%q)=%q, want %q", input, got, want)
		}
	}
}

...
func TestInstallBundledMaclawAppSkillRollsBackIndexFailure(t *testing.T) {
	tmpHome := t.TempDir()
	t.Setenv("USERPROFILE", tmpHome)
	t.Setenv("HOME", tmpHome)
	...
	sourceDir := filepath.Join(t.TempDir(), "bundled-rollback")
	if err := os.MkdirAll(sourceDir, 0o755); err != nil {
		t.Fatalf("MkdirAll sourceDir: %v", err)
	}
	if err := os.WriteFile(filepath.Join(sourceDir, "skill.yaml"), []byte("name: bundled-rollback\ndescription: rollback test\nsteps:\n  - action: bash\n    params:\n      command: echo bundled\n"), 0o644); err != nil {
		t.Fatalf("WriteFile skill.yaml: %v", err)
	}
	if err := os.WriteFile(filepath.Join(sourceDir, "SKILL.md"), []byte("# bundled rollback\n"), 0o644); err != nil {
		t.Fatalf("WriteFile SKILL.md: %v", err)
	}
	bundle, err := maclawAppBundleInstalledSkill(NLSkillDefinition{
		Name: "bundled-rollback", SkillDir: sourceDir, Source: "local", HubSkillID: "bundled-rollback", HubVersion: "1.0.0",
	}, maclawAppInstallPlanDependency{ID: "bundled-rollback"})
	if err != nil {
		t.Fatalf("maclawAppBundleInstalledSkill() error = %v", err)
	}
	...
	err = app.installBundledMaclawAppSkill(bundle)
	if err == nil || !strings.Contains(err.Error(), "not committed") {
		t.Fatalf("installBundledMaclawAppSkill() error = %v, want commit failure", err)
	}
	cfg, err := app.LoadConfig()
	if err != nil {
		t.Fatalf("LoadConfig() error = %v", err)
	...
	if len(cfg.NLSkills) != 0 {
		t.Fatalf("failed bundled install remained registered: %#v", cfg.NLSkills)
	}
	if _, statErr := os.Stat(filepath.Join(tmpHome, ".maclaw", "data", "skills", "bundled-rollback")); !os.IsNotExist(statErr) {
		t.Fatalf("failed bundled install left published directory: %v", statErr)
	}
	if refreshCalls < 2 {

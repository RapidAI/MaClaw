...
func TestRestoreCloudWorkspaceTasksCreatesLocalTaskWithoutLease(t *testing.T) {
	...
	if projectRecordHasTagLike(restored[0].Tags, taskCodingDevTag) || projectRecordHasTagLike(restored[0].Tags, taskRemoteCodingDevTag) {
		t.Fatalf("cloud identity must not carry coding/remote tags: %v", restored[0].Tags)
	}
...
func TestRestoreCloudWorkspaceTasksRemapsRemoteCodingMode(t *testing.T) {
	resetCloudWorkspaceEntitlementCache()
	t.Cleanup(resetCloudWorkspaceEntitlementCache)
	hub := newRestoreCloudWorkspaceTestHub("cws_remote", "人工智能数学基础", taskRemoteCodingDevTag, true)
	app := newCloudWorkspaceMountTestApp(t, hub)
	restored := app.RestoreCloudWorkspaceTasks()
	if len(restored) != 1 {
		t.Fatalf("restored=%+v", restored)
	...
	if projectRecordHasTagLike(restored[0].Tags, taskCodingDevTag) || projectRecordHasTagLike(restored[0].Tags, taskRemoteCodingDevTag) {
		t.Fatalf("cloud restore must not carry coding/remote identity tags: %v", restored[0].Tags)
	}
}
...
func TestCreateTaskWithCloudWorkspaceFlushesTaskSidecarImmediately(t *testing.T) {
	...
	if task.Name != "标书项目" || task.Tag != cloudWorkspaceTag("cws_flush") {
...
func TestRestoreCloudWorkspaceTasksSkipsSidecarGetWhenNamePresent(t *testing.T) {
	...
	hub.mu.Lock()
	gets := hub.sidecarGetsByName[cloudWorkspaceSidecarTask]
	hub.mu.Unlock()
	if gets != 0 {
		t.Fatalf("entitlement task_name should skip task.json GET, gets=%d", gets)
	}
}

// Materializing a cloud task on a new machine must also pull its durable
// transcript, otherwise reopening the task renders an empty conversation.
func TestRestoreCloudWorkspaceTasksMaterializesTranscript(t *testing.T) {
	resetCloudWorkspaceEntitlementCache()
	t.Cleanup(resetCloudWorkspaceEntitlementCache)
	hub := newRestoreCloudWorkspaceTestHub("cws_history", "历史任务", "", true)
	hub.sidecars[cloudWorkspaceSidecarSession] = marshalCloudWorkspaceSessionSidecar(&TabSessionData{
		Conversation: []interface{}{
			map[string]interface{}{"role": "user", "content": "云端问题"},
			map[string]interface{}{"role": "assistant", "content": "云端回答"},
		},
	})
	app := newCloudWorkspaceMountTestApp(t, hub)
	...
	hub.mu.Lock()
	sessionGets := hub.sidecarGetsByName[cloudWorkspaceSidecarSession]
	hub.mu.Unlock()
	if sessionGets == 0 {
		t.Fatal("restore must fetch session.json for newly materialized tasks")
	}
	history := app.LoadProjectConversationHistory(restored[0].ProjectPath)
	if len(history) != 2 || history[0].Role != "user" || history[0].Content != "云端问题" || history[1].Content != "云端回答" {

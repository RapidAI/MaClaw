...
func TestRestoreSkillsBlocksOnUnreadableCompensationQueue(t *testing.T) {
	tempHome := t.TempDir()
	t.Setenv("HOME", tempHome)
	t.Setenv("USERPROFILE", tempHome)
	...
	zipPath := filepath.Join(t.TempDir(), "blocked-restore.zip")
	writeSkillBackupZip(t, zipPath, corelib.NLSkillEntry{
		Name: "blocked-restore", Source: "manual", Status: "active",
		Steps: []corelib.NLSkillStep{{Action: "noop"}},
	})
	if _, err := executor.RestoreSkills(zipPath); err == nil || !strings.Contains(err.Error(), "compensation queue unavailable") {
		t.Fatalf("RestoreSkills() error = %v, want unreadable queue block", err)
	}
	if got := executor.List(); len(got) != 0 {
		t.Fatalf("blocked restore changed registry: %#v", got)
	}
}

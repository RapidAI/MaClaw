...
func (e *SkillExecutor) RestoreSkills(zipPath string) (*RestoreReport, error) {
	...
	// Fail closed before doing any ZIP work when the durable queue cannot be
	// read. Parsing/scanning is intentionally performed outside installMutex so
	// large backups do not block normal Skill execution or installation.
	if e.app != nil {
		if err := cskill.CheckEvolutionCompensationQueue(); err != nil {
			return nil, fmt.Errorf("skill restore blocked: evolution compensation queue unavailable: %w", err)
		}
	}
	...
	var candidates []corelib.NLSkillEntry
	...
	// All duplicate checks and the final read/modify/write happen against the
	// latest authoritative registry while holding the same lock as other Skill
	// mutations. This closes the parse-time TOCTOU window without serializing
	// ZIP I/O and security scans.
	if e.app != nil {
		...
		if err := cskill.CheckEvolutionCompensationQueue(); err != nil {
			return nil, fmt.Errorf("skill restore blocked before commit: evolution compensation queue unavailable: %w", err)
		}
	}
	if len(candidates) == 0 {
		report.State = "skipped"
		report.CleanupStatus = "clear"
		return report, nil
	}
	existingSkills := e.loadSkills()
	existingNames := make(map[string]bool, len(existingSkills))
	for _, s := range existingSkills {
		existingNames[strings.ToLower(strings.TrimSpace(s.Name))] = true
	}
	accepted := make([]corelib.NLSkillEntry, 0, len(candidates))
	for _, candidate := range candidates {
		key := strings.ToLower(strings.TrimSpace(candidate.Name))
		if key == "" || existingNames[key] {
			report.Skipped++
			report.Details = append(report.Details, fmt.Sprintf("%s: skipped (duplicate)", candidate.Name))
			continue
		}
		if e.app != nil {
			if err := ensureSkillEvolutionMutationAdmission(e.app, candidate.Name); err != nil {
				return nil, err
			}
		}
	...
	report.Restored = len(accepted)
	for _, candidate := range accepted {
		report.Details = append(report.Details, fmt.Sprintf("%s: restored", candidate.Name))
	}
	if len(accepted) == 0 {
		report.State = "skipped"
		report.CleanupStatus = "clear"
	...
	// Detached executors retain the historical config-only behavior. App-owned
	// executors use SkillCommitter so the restored batch has a durable snapshot,
	// checked index refresh, strict final audit, and explicit cleanup state.
	if e.app == nil {
		merged := append(append([]corelib.NLSkillEntry(nil), existingSkills...), accepted...)
		if err := e.withSkillListMutate(func() error { return e.saveSkills(merged) }); err != nil {
			return report, fmt.Errorf("failed to persist restored skills: %w", err)
		}
		report.State = "committed"
		report.CleanupStatus = "clear"
		return report, nil
	}
	requestID := fmt.Sprintf("evo_backup_restore_%d", time.Now().UnixNano())
	report.RequestID = requestID
	configRevision := skillEvolutionConfigRevision(e.app)
	auditData := map[string]string{
		"skill": accepted[0].Name, "action": "backup_restore", "decision": "applied", "via": "operator",
		"request_id": requestID, "attempt": "1", "config_revision": configRevision,
		"schema_version": "2", "evidence_mode": "none", "restored_count": fmt.Sprintf("%d", len(accepted)),
	}
	if err := cskill.RecordEvolutionEventStrict("skill:backup_restore_started", auditData, "desktop"); err != nil {
		report.State, report.CleanupStatus, report.FailureReason = "rolled_back", "clear", "audit_unavailable"
		return report, fmt.Errorf("backup restore audit preflight failed: %w", err)
	}
	committer := &cskill.SkillCommitter{
		SkillLoader: func() []corelib.NLSkillEntry { return e.loadSkills() },
		SkillSaver: func(entries []corelib.NLSkillEntry) error {
			return e.withSkillListMutate(func() error { return e.saveSkills(entries) })
		},
		RollbackSkillSaver: func(entries []corelib.NLSkillEntry) error {
			return e.withSkillListMutate(func() error { return e.restoreSkillsSnapshot(entries) })
		},
		EntriesMutator: func(entries []corelib.NLSkillEntry) ([]corelib.NLSkillEntry, error) {
			seen := make(map[string]bool, len(entries)+len(accepted))
		...
		IndexRefresher:   func() error { return e.app.refreshSkillIndexesAfterMutationChecked("backup_restore") },
	...
	commitEntry := accepted[0]
	result := committer.Commit(cskill.WithEvolutionRequestMetadata(context.Background(), requestID, 1), commitEntry.Name, &commitEntry, "skill:backup_restored", auditData)
	report.State, report.CleanupStatus, report.FailureReason = result.State, result.CleanupStatus, result.FailureReason
	if result.State != "committed" || result.CleanupStatus != "clear" {
		return report, fmt.Errorf("backup restore not committed: %s (%s)", result.State, result.FailureReason)
	}
	return report, nil
}

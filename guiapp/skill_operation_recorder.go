...
// Stop ends recording and generates a skill directly in the primary skills
// directory. It remains for compatibility with non-App callers. GUI bindings
// must use StopToDirectory with a transaction-owned staging directory before
// publishing through the shared directory committer.
func (r *SkillOperationRecorder) Stop(skillName, description string) (string, []string, error) {
	return r.stopToDestination(skillName, description, func(resolvedName string) (string, error) {
		skillsDir, err := skill.PrimarySkillsDir()
		if err != nil {
			return "", fmt.Errorf("resolve skills dir: %w", err)
		}
		dirName := sanitizeSkillDirName(resolvedName)
		skillDir := filepath.Join(skillsDir, dirName)
		if _, err := os.Stat(skillDir); err == nil {
			// Avoid overwriting existing skill directories — append a numeric suffix.
			for suffix := 2; suffix <= 99; suffix++ {
				candidate := filepath.Join(skillsDir, fmt.Sprintf("%s-%d", dirName, suffix))
				if _, err := os.Stat(candidate); os.IsNotExist(err) {
					return candidate, nil
				} else if err != nil {
					return "", fmt.Errorf("inspect skill directory candidate: %w", err)
				}
			}
			return "", fmt.Errorf("no available skill directory for %q", resolvedName)
...
// StopToDirectory ends recording and writes the generated package only into an
// empty caller-owned directory. It deliberately never chooses or creates a
// primary skill destination: the caller must scan and transactionally publish
// this staging directory before treating the recording as saved.
func (r *SkillOperationRecorder) StopToDirectory(skillName, description, skillDir string) (string, []string, error) {
	return r.stopToDestination(skillName, description, func(string) (string, error) {
		candidate := filepath.Clean(strings.TrimSpace(skillDir))
		if candidate == "" || candidate == "." {
			return "", fmt.Errorf("staging skill directory is required")
		}
		if info, err := os.Stat(candidate); err == nil {
			if !info.IsDir() {
				return "", fmt.Errorf("staging skill path is not a directory")
			}
			entries, err := os.ReadDir(candidate)
			if err != nil {
				return "", fmt.Errorf("read staging skill directory: %w", err)
			}
			if len(entries) != 0 {
				return "", fmt.Errorf("staging skill directory is not empty")
			}
		} else if !os.IsNotExist(err) {
			return "", fmt.Errorf("inspect staging skill directory: %w", err)
		}
		return candidate, nil
	})
}

// stopToDestination drains the recording once, generates the package, and
// writes it to a destination selected by the caller. The destination callback
// is intentionally invoked only after a stable generated name is available.
func (r *SkillOperationRecorder) stopToDestination(skillName, description string, destination func(string) (string, error)) (string, []string, error) {
	if r == nil || destination == nil {
		return "", nil, fmt.Errorf("skill recorder destination is required")
	}
	r.mu.Lock()
	entries := r.entries
	workDir := r.workDir
	...
	skillDir, err := destination(skillName)
	if err != nil {
		return "", nil, err
	}

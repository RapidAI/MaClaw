...
var safeFilenameRe = agentruntime.SafeFilenameRe
...
func (r *TrajectoryRecorder) SetOutcome(status, errMsg string, iterations, toolCalls, inputTokens, outputTokens int) {
	...
	if s := strings.TrimSpace(status); s != "" {
		r.session.Status = s
		r.session.RuntimeStatus = trajectoryRuntimeStatus(s)
	}
...
func trajectoryRuntimeStatus(status string) agentruntime.JobStatus {
	switch strings.ToLower(strings.TrimSpace(status)) {
	case "success", "succeeded", "completed", "complete":
		return agentruntime.JobStatusSucceeded
	case "error", "failed", "failure", "hard_exit", "timeout":

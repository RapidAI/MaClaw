...
// skillUploadLocalStateError means every configured remote target accepted the
// package, but the local receipt could not be persisted. SubmissionID remains
// available to callers for support/reconciliation; it must not be reported as
// a fully completed local upload.
type skillUploadLocalStateError struct {
	SubmissionID string
	Err          error
}

func (e *skillUploadLocalStateError) Error() string {
	if e == nil {
		return "skill submitted, but local upload state was not persisted"
	}
	return fmt.Sprintf("skill submitted as %q, but local upload state was not persisted: %v", e.SubmissionID, e.Err)
}

func (e *skillUploadLocalStateError) Unwrap() error {
	if e == nil {
		return nil
	}
	return e.Err
...
func (m *SkillLifecycleManager) HasRunnableUploadItems(now time.Time) (bool, error) {
	...
	for _, item := range q.Items {
		...
		case skillUploadStatusRemoteSubmitted:
			if uploadQueueItemDue(item, now) {
				return true, nil
			}
...
func (m *SkillLifecycleManager) EnqueueUpload(ctx context.Context, skillName, skillDir, reason string, requireRuntimeProof bool, processNow bool) (*SkillUploadQueueItem, error) {
	...
	if strings.TrimSpace(skillDir) != "" {
		...
		// Portability preparation may have taken long enough for another
		// transaction to enqueue a compensation. Do not persist quality evidence
		// from a stale admission snapshot.
		if err := m.ensureEvolutionUploadAdmission(skillName); err != nil {
			return nil, err
		}
...
func (m *SkillLifecycleManager) UploadNowWithCompletedTargets(ctx context.Context, skillName, reason string, requireRuntimeProof bool, completedTargets map[string]string) (string, error) {
	...
	if target := m.findRegisteredSkill(skillName); target != nil && strings.TrimSpace(target.SkillDir) != "" {
		if prepErr := m.prepareUploadSourceWithCheckedIndex(canonicalSkillName, target.SkillDir); prepErr != nil {
			return "", prepErr
		}
	}
	...
	if target != nil && strings.TrimSpace(target.SkillDir) != "" {
		// The quality status is a persisted upload decision, so it is also
		// fenced by the latest compensation admission rather than relying only
		// on the check performed before source portability preparation.
		if err := m.ensureEvolutionUploadAdmission(canonicalSkillName); err != nil {
			return "", err
		}
	...
	// Preparation, quality evaluation and packaging happen outside a durable
	// transaction lock. Re-check the compensation queue immediately before the
	// remote submission so a newly pending recovery cannot be bypassed by a
	...
	if err := m.persistUploadedSkillReceipt(canonicalSkillName, "", submissionID); err != nil {
		return submissionID, &skillUploadLocalStateError{SubmissionID: submissionID, Err: err}
	}
	return submissionID, nil
}

// prepareUploadSourceWithCheckedIndex applies upload portability fixes to the
// authoritative source while serializing the mutation with other Skill writes.
// Any checked index failure restores both the source directory and derived
// indexes before returning, so callers never upload an unroutable revision.
func (m *SkillLifecycleManager) prepareUploadSourceWithCheckedIndex(skillName, skillDir string) error {
	if m == nil || m.app == nil {
		return fmt.Errorf("skill lifecycle manager not initialized")
	}
	if m.app.skillExecutor != nil {
		m.app.skillExecutor.suspendStatusOverlayPersistence()
		defer m.app.skillExecutor.resumeStatusOverlayPersistence()
	}
	m.app.installMutex.Lock()
	defer m.app.installMutex.Unlock()
	// Admission was checked by UploadNow before entering this helper, but the
	// call may have waited behind another transaction.  Re-check under the lock
	// before snapshotting or rewriting the authoritative source directory.
	if err := m.ensureEvolutionUploadAdmission(skillName); err != nil {
		return err
	}
	snapshotDir, cleanupSnapshot, snapshotErr := snapshotSkillDirForRollback(skillDir)
	if snapshotErr != nil {
		return fmt.Errorf("snapshot source skill before upload preflight: %w", snapshotErr)
	}
	defer cleanupSnapshot()
	preflight, fixErr := skill.PrepareSkillForUpload(skillDir)
	if fixErr != nil {
		restoreErr := restoreSkillDirFromSnapshot(skillDir, snapshotDir)
		if restoreErr != nil {
			return fmt.Errorf("source upload preflight failed: %w; rollback failed: %v", fixErr, restoreErr)
		}
		return fmt.Errorf("source upload preflight failed; changes rolled back: %w", fixErr)
	}
	if preflight == nil || len(preflight.AutoFixed) == 0 {
		return nil
	}
	if indexErr := m.app.refreshSkillIndexesAfterMutationChecked(skillName); indexErr != nil {
		restoreErr := restoreSkillDirFromSnapshot(skillDir, snapshotDir)
		if restoreErr == nil {
			var restored []corelib.NLSkillEntry
			if m.app.skillExecutor != nil {
				restored = m.app.skillExecutor.loadSkills()
			}
			restoreErr = m.app.refreshSkillIndexesAfterRollbackChecked(skillName, restored)
		}
		if restoreErr != nil {
			return fmt.Errorf("source upload preflight index refresh failed: %w; rollback failed: %v", indexErr, restoreErr)
		}
		return fmt.Errorf("source upload preflight index refresh failed; changes rolled back: %w", indexErr)
	}
	return nil
}

// persistUploadedSkillReceipt records a completed remote submission locally.
// Registered skills must use SkillExecutor so config overlays and scanner
// state stay aligned. An ad-hoc directory has no registry entry to update, so
// it persists the same atomic receipt directly beside skill.yaml.
func (m *SkillLifecycleManager) persistUploadedSkillReceipt(skillName, skillDir, submissionID string) error {
	if m == nil || m.app == nil {
		return fmt.Errorf("skill lifecycle manager not initialized")
	}
	if m.app.skillExecutor != nil {
		if registered := m.findRegisteredSkill(skillName); registered != nil {
			return m.app.skillExecutor.MarkUploaded(registered.Name, submissionID)
		}
	}
	if strings.TrimSpace(skillDir) == "" {
		return fmt.Errorf("submitted skill %q is no longer registered and has no source directory for its upload receipt", skillName)
	}
	// Ad-hoc directory uploads do not pass through SkillExecutor.MarkUploaded,
	// so enforce the same durable compensation admission and process-wide
	// serialization before writing upload_status.json.  Re-check after waiting
	// for the lock to close the race with a transaction that becomes pending
	// while this remote-submission callback is queued.
	if err := ensureSkillEvolutionMutationAdmission(m.app, skillName); err != nil {
		return err
	}
	if m.app.skillExecutor != nil {
		m.app.skillExecutor.suspendStatusOverlayPersistence()
	...
	return writeSkillUploadStatusAtomic(skillDir, submissionID)
}
...
func (m *SkillLifecycleManager) UploadDirNowWithCompletedTargets(ctx context.Context, skillName, skillDir, reason string, requireRuntimeProof bool, completedTargets map[string]string) (string, error) {
	...
	// Resolve a name from the package definition only when the caller omitted
	// one; this read-only probe lets the queue gate run before any portability
	// auto-fix or runtime/client setup can write state. UploadDirNow is also used
	// by queue retries, so an unreadable compensation log must block before
	// prepareSkillDirForMarket mutates skill.yaml in place.
	skillName = strings.TrimSpace(skillName)
	if skillName == "" {
		candidate, loadErr := loadMarketPackageSkillEntry(skillDir, nil)
		if loadErr != nil {
			return "", fmt.Errorf("load skill entry for upload admission: %w", loadErr)
		}
		if candidate != nil {
			skillName = strings.TrimSpace(candidate.Name)
		}
	}
	if skillName == "" {
		return "", fmt.Errorf("skill name is empty")
	}
	// This must precede PrepareSkillDirForMarket: that helper may persist
	// portability fixes to the authoritative directory. A pending or unreadable
	// evolution queue is a global integrity failure and therefore forbids even
	// that pre-upload mutation.
	if err := m.ensureEvolutionUploadAdmission(skillName); err != nil {
		return "", err
	}
	m.app.ensureSkillMarketClient()
	...
	// Re-check immediately before crossing the remote side-effect boundary.
	// Preparation and packaging can take long enough for another transaction to
	// enqueue a compensation; never submit a package using an admission snapshot
	...
	if err := m.persistUploadedSkillReceipt(entry.Name, skillDir, submissionID); err != nil {
		return submissionID, &skillUploadLocalStateError{SubmissionID: submissionID, Err: err}
	}
	return submissionID, nil
}
...
func (m *SkillLifecycleManager) processPendingUploads(ctx context.Context, limit int, skillName string, forceReady bool) error {
	...
	for {
		...
		processed++
		if item.Status == skillUploadStatusRemoteSubmitted {
			if err := m.persistUploadedSkillReceipt(item.SkillName, item.SkillDir, item.SubmissionID); err != nil {
				m.markUploadFailed(item.ID, &skillUploadLocalStateError{SubmissionID: item.SubmissionID, Err: err})
				continue
			}
			m.markUploaded(item.ID, item.SubmissionID)
			continue
		}
...
func (m *SkillLifecycleManager) EvaluateInstalledSkills(requireRuntimeProof bool) ([]SkillQualityStatus, error) {
	...
	// Quality auditing persists quality_status.json and may apply portability
	// fixes through prepareSkillDirForMarket. Treat it as a governed mutation:
	// an unreadable queue or a pending compensation must stop the entire audit
	// before the first authoritative Skill directory is touched.
	if err := skill.CheckEvolutionCompensationQueue(); err != nil {
		return nil, fmt.Errorf("skill quality audit blocked: evolution compensation queue unavailable: %w", err)
	}
	m.app.ensureInteractionInfra()
	if m.app.skillExecutor == nil {
...
func (m *SkillLifecycleManager) RetryBlocked(skillName string) error {
	...
	for i := range q.Items {
		...
		// Retry-all must apply the same per-Skill admission as a named retry.
		// Otherwise a pending compensation for one blocked item could be hidden
		// simply by omitting the optional skill name and the re-evaluation below
		// would rewrite its authoritative directory/quality status.
		if m.app != nil {
			if admissionErr := m.ensureEvolutionUploadAdmission(q.Items[i].SkillName); admissionErr != nil {
				q.Items[i].LastError = admissionErr.Error()
...
func (m *SkillLifecycleManager) reevaluateBlockedUploadItem(item SkillUploadQueueItem, now string) (SkillUploadQueueItem, bool) {
	// This helper mutates the source directory (portability auto-fix) and writes
	// quality_status.json. Keep a second admission fence here so future callers
	// cannot accidentally invoke it without the RetryBlocked loop's guard.
	if err := skill.CheckEvolutionCompensationQueue(); err != nil {
		item.LastError = "upload blocked: evolution compensation queue unavailable: " + err.Error()
		item.UpdatedAt = now
		return item, false
	}
	if m.app != nil {
	...
	// The preparation/read phase can be long enough for a new compensation to
	// appear. Do not persist quality evidence or move the queue back to pending
	// using that stale admission snapshot.
	if err := skill.CheckEvolutionCompensationQueue(); err != nil {
		item.LastError = "upload blocked: evolution compensation queue unavailable: " + err.Error()
		item.UpdatedAt = now
		return item, false
...
func (m *SkillLifecycleManager) upsertPending(skillName, skillDir, localHash, reason string, requireRuntimeProof bool, score int) (*SkillUploadQueueItem, error) {
	...
	for i := range q.Items {
		if q.Items[i].ID == id {
			if q.Items[i].Status == skillUploadStatusUploaded || q.Items[i].Status == skillUploadStatusRemoteSubmitted {
...
func (m *SkillLifecycleManager) nextPendingItemMatching(now time.Time, skillName string, forceReady bool) (SkillUploadQueueItem, bool, error) {
	...
	for i := range q.Items {
		...
		if status == skillUploadStatusRemoteSubmitted {
...
func (m *SkillLifecycleManager) markUploadFailed(id string, uploadErr error) {
	...
	var localState *skillUploadLocalStateError
	for i := range q.Items {
		if q.Items[i].ID != id {
			continue
		}
		...
		if errors.As(uploadErr, &localState) && strings.TrimSpace(localState.SubmissionID) != "" {
			q.Items[i].Attempts++
			q.Items[i].Status = skillUploadStatusRemoteSubmitted
			q.Items[i].SubmissionID = strings.TrimSpace(localState.SubmissionID)
			q.Items[i].UploadedTargets = m.parseUploadedTargetResults(localState.SubmissionID)
			q.Items[i].LastError = localState.Error()
			q.Items[i].UpdatedAt = now.Format(time.RFC3339)

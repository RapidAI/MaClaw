...
func (o *RemoteExperimentOrchestrator) Run() (StopReason, string) {
	defer func() {
		...
		if o.state.Status == "running" {
			...
			o.state.RuntimeStatus = agentruntime.JobStatusSucceeded
		}
		o.state.mu.Unlock()
	}()
	...
	for round := 1; round <= params.MaxRounds; round++ {
		...
		if elapsed >= params.MaxRuntime {
			o.state.mu.Lock()
			o.state.Status = "paused"
			o.state.RuntimeStatus = agentruntime.JobStatusUnknown
			o.state.mu.Unlock()
		...
		result.RuntimeStatus = experimentRoundRuntimeStatus(result.Status)
		...
		if consecutiveFailures >= params.FailureTolerance {
			...
			o.state.RuntimeStatus = agentruntime.JobStatusFailed
			o.state.mu.Unlock()
...
func (o *RemoteExperimentOrchestrator) Stop() {
	o.stopOnce.Do(func() {
		...
		o.state.RuntimeStatus = agentruntime.JobStatusCanceled
		o.state.mu.Unlock()
	})
}
...
func (o *RemoteExperimentOrchestrator) GetState() ExperimentOrchestratorState {
	...
	return ExperimentOrchestratorState{
		...
		RuntimeStatus:            o.state.RuntimeStatus,
		consecutiveNoImprovement: o.state.consecutiveNoImprovement,
		consecutiveFailures:      o.state.consecutiveFailures,
	}
}
...
func experimentRoundRuntimeStatus(status string) agentruntime.JobStatus {

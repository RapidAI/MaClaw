...
type App struct {
	...
	// legacyDetailedMu serializes the clear-call-read sequence used by the
	// *Detailed compatibility wrappers.  The underlying legacy methods already
	// serialize filesystem mutations with installMutex, but without this second
	// lock two concurrent detailed calls for the same Skill could overwrite the
	// process-local result map and return the other request's outcome.
	legacyDetailedMu sync.Mutex
	...
	// legacySkillClearCompensation is a narrow test seam for the post-commit
	// cleanup boundary. Production uses the durable queue implementation; tests
	// can force cleanup failure and verify committed/pending recovery semantics
	// without relying on platform-specific file permission behavior.
	legacySkillClearCompensation func(string, string, string) error
	// skillDeleteClearCompensation is the equivalent narrow seam for the
	// shared DeleteNLSkill adapter. Production leaves it nil; tests can force
	// the queue-clear boundary to fail and verify that an audited deletion
	// remains committed while cleanup stays pending across restart.
	skillDeleteClearCompensation       func(string, string, string) error
	...
	// evolutionAuditWriter is a narrow dependency-injection seam for lifecycle
	// transaction tests. Production leaves it nil so strict audit events use the
	// durable core sink; tests can deterministically fail the final-audit phase
	// without relying on platform-specific filesystem permissions.
	evolutionAuditWriter func(string, map[string]string) error
	...
	frontendDOMReady         atomic.Bool                     // set when Wails OnDomReady fires (may be about:blank)
	frontendHTMLReady        atomic.Bool                     // set when index.html boot-ping arrives
	frontendReactReady       atomic.Bool                     // set when main.tsx boot-ping arrives
	frontendEnvCheckDone     atomic.Bool                     // set when env-check-done has been emitted
	frontendEnvCheckPromoted atomic.Bool                     // set when compact splash has been dismissed from Go
...
func (a *App) ensureSkillRunner() {
	...
	if err := a.skillRunner.UseDurableRuntimeStores(filepath.Join(a.GetDataDir(), "runtime")); err != nil {
		log.Printf("[skill-runner] durable runtime stores unavailable: %v", err)
	}
...
func (a *App) ensureEvolutionPipeline() {
	...
	// Automatic definition changes are opt-in. Keep the worker alive for
	// read-only observation and diagnostics, but do not allow its mutating
	// stages until both config and the environment permit them.
	...
	if cfg, err := a.LoadConfig(); err == nil {
		...
		automaticEvolutionEnabled = cfg.IsSkillEvolutionEnabled() && !skill.EvolutionEnvDisabled()
	}
	pipeline.EnableRepair = automaticEvolutionEnabled
	pipeline.EnableOptimizer = automaticEvolutionEnabled
	// NudgePromoter currently writes its generated YAML directly to the final
	// SkillsDir before asking a registrar to update config.  That bypasses the
	// GUI's staged SkillCommitter transaction (compensation, checked index,
	// final audit, and post-commit cleanup), so it is deliberately unavailable
	// here even when the broader evolution opt-in is enabled.  Do not wire a
	// registrar as an alternate persistence path; re-enable only once the core
	// promoter can submit a fully staged candidate to the shared committer.
	pipeline.EnablePromoter = false
	...
	// The pipeline's shared committer must hold the App-wide mutation boundary
	// across config, YAML, checked-index and final-audit phases. SkillSaver only
	// protects its individual config write; the transaction-wide guard prevents
	// a concurrent install/delete from publishing a stale full-list snapshot.
	pipeline.MutationMutex = &a.installMutex
	pipeline.MutationAdmission = func(skillName string) error {
		return ensureSkillEvolutionMutationAdmission(a, skillName)
	}
	...
	// Wire LLM for optimization (lazy config picks up provider changes).
...
func (a *App) autoStartLocalMCPServers(entries []corelib.LocalMCPServerEntry) {
	for _, server := range entries {
		if !server.Disabled && server.AutoStart && a.mcpRuntimeSyncAllowsAutomaticStart(server) {
...
func (a *App) ensureTaskOrchestrator2() {
	...
	if a.taskOrchestrator2 == nil && a.remoteSessions != nil && a.toolSelector != nil {
		...
		if err := a.taskOrchestrator2.UseDurableRuntimeStores(filepath.Join(a.GetDataDir(), "runtime")); err != nil {
			log.Printf("[task-orchestrator] durable runtime stores unavailable: %v", err)
		}
	}
}
...
func (a *App) domReady(ctx context.Context) {
	...
	if !a.frontendHTMLReady.Load() {
		// about:blank completes navigation before index.html is requested.
		return
	}
...
func (a *App) markFrontendHTMLReady() {
	...
	a.startBackgroundUpdateChecks()
	a.scheduleClampMaximizedWindowToWorkArea(120 * time.Millisecond)
	a.watchEnvCheckPromote()
}

func (a *App) markFrontendReactReady() {
	if a == nil || !a.frontendReactReady.CompareAndSwap(false, true) {
		return
	}
	bootLog("react main.tsx loaded; envCheckDone=%v", a.frontendEnvCheckDone.Load())
...
func preserveBackendOwnedFields(incoming *corelib.AppConfig, ondisk *corelib.AppConfig) {
	...
	// Once the backend has persisted a HubCenter candidate list, preserve it
	// from stale full-config snapshots. During first-time setup the on-disk
	// field is nil, so allow SaveConfig to write the caller's initial list.
	if ondisk.RemoteHubCenterURLs != nil {
...
type SkillEvolutionStatus struct {
	...
	// ConfigEnabled is the persisted skill_evolution_enabled setting (opt-in).
...
func (a *App) ListSkillEvolutionCompensations() map[string]interface{} {
	...
	if err != nil {
		out["ok"] = false
		out["error"] = skill.SummarizeEvolutionCompensationError(err)
...
// RetrySkillEvolutionCompensation retries one explicitly selected durable
// compensation. The request ID, Skill and action must all match exactly;
// confirm is required so this endpoint cannot be triggered accidentally by a
// stale read-only queue refresh.
func (a *App) RetrySkillEvolutionCompensation(requestID, skillName, action string, confirm bool) map[string]interface{} {
	out := map[string]interface{}{"ok": false, "request_id": strings.TrimSpace(requestID), "skill": strings.TrimSpace(skillName), "action": strings.TrimSpace(action), "recovered": false}
	if !confirm {
	...
	a.ensureEvolutionPipeline()
	summary, recovered, err := skill.RetryEvolutionCompensation(requestID, skillName, action, a.evolutionPipeline)
	out["summary"] = summary
...
// ClearSkillEvolutionCompensation removes one committed compensation after
// verifying its declared post-commit artifacts are already gone. It never
// discards an uncommitted rollback snapshot and always requires confirmation.
func (a *App) ClearSkillEvolutionCompensation(requestID, skillName, action string, confirm bool) map[string]interface{} {
	out := map[string]interface{}{"ok": false, "request_id": strings.TrimSpace(requestID), "skill": strings.TrimSpace(skillName), "action": strings.TrimSpace(action)}
	if !confirm {
		out["error"] = "confirm=true is required"
		out["fail_closed"] = true
		return out
	...
	summary, err := skill.ClearEvolutionCompensationManually(requestID, skillName, action)
	out["summary"] = summary
	if err != nil {
		out["error"] = skill.SummarizeEvolutionCompensationError(err)
		out["fail_closed"] = true
...
func (a *App) RestoreSkillYAMLBackup(skillName string, version int, confirm bool) map[string]interface{} {
	...
	// Version restore changes the authoritative YAML and registry/index view.
	// Serialize the complete commit with installs/deletes/status writes, then
	// repeat admission after waiting so a newly-pending compensation cannot be
	// bypassed by a stale pre-lock check.
	a.installMutex.Lock()
...
func (a *App) ApplySkillMaintenanceAction(kind, skillName, relatedSkill string, confirm, allowDuplicateRetire bool) map[string]interface{} {
	...
	// Drain asynchronous status-overlay writers before taking installMutex;
	// otherwise a writer waiting on the same mutex could deadlock the drain.
	a.skillExecutor.suspendStatusOverlayPersistence()
	defer a.skillExecutor.resumeStatusOverlayPersistence()
	// Maintenance computes a full-list mutation before entering SkillCommitter.
	// Serialize that planning snapshot with legacy/install/delete writers so a
	// concurrent mutation cannot be overwritten by the captured `updated` list.
	a.installMutex.Lock()
	defer a.installMutex.Unlock()
	...
	// Emit the started audit only after planning proves that this confirmed
	// action changes something. A no-op must remain genuinely side-effect free:
	// no compensation, index refresh, final audit, or misleading "started"
	...
	committer := &skill.SkillCommitter{
		...
		RollbackSkillSaver: func(entries []corelib.NLSkillEntry) error {
			// Rollback receives the authoritative pre-image. Bypass the normal
			// anti-wipe/overlay projection so a failed maintenance mutation cannot
			// retain newly-created entries or silently drop fields from the snapshot.
			return a.skillExecutor.withSkillListMutate(func() error { return a.skillExecutor.restoreSkillsSnapshot(entries) })
		},
		...
		DefinitionWriterWithCompensation: func(entry *corelib.NLSkillEntry, record *skill.EvolutionCompensationRecord) error {
			...
			if err == nil && record != nil && strings.TrimSpace(path) != "" {
				// Preserve rollback artifacts registered by the maintenance planner;
				// the versioned contract file is an additional transaction-owned
				// artifact, not a replacement for the existing cleanup plan.
				rollbackPaths := append([]string(nil), record.RollbackCleanupPaths...)
				rollbackPaths = append(rollbackPaths, path)
				record.SetRollbackCleanupPaths(rollbackPaths)
			}
			return err
		},
	...
	// A merge/delete mutator may remove the requested entry from the published
	// list.  Keep the committer identity stable by falling back to the original
	// pre-image instead of attributing the transaction to an unrelated
	// updated[0] entry (or failing because the target disappeared).
	commitEntry := maintenanceEntryByName(updated, skillName)
	if commitEntry == nil {
		commitEntry = maintenanceEntryByName(original, skillName)
	}
	if commitEntry == nil {
		out["state"] = "rolled_back"
		out["cleanup_status"] = "clear"
		out["error"] = "maintenance primary entry disappeared"
		return out
	}
	commitCtx := skill.WithEvolutionRequestMetadata(context.Background(), requestID, 1)
	commitResult := committer.Commit(commitCtx, skillName, commitEntry, "skill:maintenance_apply", auditData)
...
func (a *App) GetSkillEvolutionStatus() SkillEvolutionStatus {
	...
	// A read failure is not permission to mutate Skill definitions. Keep the
	// diagnostic fail-closed as well, so the UI cannot claim automatic evolution
	// is active without an authoritative configuration.
	out.ConfigEnabled = false
	if cfg, err := a.LoadConfig(); err == nil {
...
func (a *App) PatchConfigFields(patch map[string]interface{}) (corelib.AppConfig, error) {
	...
	semanticRoutingField := func(key string, value interface{}, base corelib.SemanticToolScopeRoutingConfig) (corelib.SemanticToolScopeRoutingConfig, error) {
		...
		cfg := base
		// Old/partially materialized configs may have a zero nested struct. Seed
		// only fields omitted by this patch; an explicit zero budget remains an
		// invalid fail-closed configuration.
	...
	if _, ok := patch["skill_evolution_enabled"]; ok && a.evolutionPipeline != nil {
		enabled := cfg.IsSkillEvolutionEnabled() && !skill.EvolutionEnvDisabled()
		a.evolutionPipeline.EnableRepair = enabled
		a.evolutionPipeline.EnableOptimizer = enabled
		// See ensureEvolutionPipeline: the GUI intentionally has no automatic
		// Nudge promotion until core accepts staged shared-committer publication.
		a.evolutionPipeline.EnablePromoter = false
	}
...
func (a *App) emitEvent(name string, data ...interface{}) {
	if a != nil && name == "env-check-done" {
		a.frontendEnvCheckDone.Store(true)
		bootLog("emit env-check-done reactReady=%v", a.frontendReactReady.Load())
		go a.promotePastEnvCheckSplash("emit")
	}
...
// legacySkillNoChangeError marks an idempotent legacy request. Compatibility
// APIs translate it back to nil while the detailed result preserves skipped.
type legacySkillNoChangeError struct{}

func (e *legacySkillNoChangeError) Error() string { return "no changes required" }
...
type LegacySkillInstallResult struct {
	OK               bool   `json:"ok"`
	State            string `json:"state"`          // skipped | committed | rolled_back | audit_pending
...
func (a *App) clearLegacySkillCompensation(requestID, skillName, action string) error {
	if a != nil && a.legacySkillClearCompensation != nil {
		return a.legacySkillClearCompensation(requestID, skillName, action)
	}
	return skill.ClearEvolutionCompensation(requestID, skillName, action)
}
...
func (a *App) InstallSkillDetailed(name, description, skillType, value, location, projectPath, toolName string) LegacySkillInstallResult {
	...
	a.legacyDetailedMu.Lock()
	defer a.legacyDetailedMu.Unlock()
...
func legacySkillResultFromError(operation, skillName string, err error) LegacySkillInstallResult {
	...
	if err != nil {
		...
		if errors.As(err, &committedErr) && committedErr.committed {
			...
			result.RequestID = strings.TrimSpace(committedErr.requestID)
			result.CleanupStatus = strings.TrimSpace(committedErr.cleanupStatus)
			if result.CleanupStatus == "" || result.CleanupStatus == "clear" {
...
func (a *App) legacyAddressInstallAlreadyCurrent(name, description, skillType, value, toolName string) bool {
	if a == nil || !normalizeRemoteToolNameKind(toolName).IsClaude() {
		return false
	}
	metadataPath := filepath.Join(a.GetSkillsDir(toolName), "metadata.json")
	data, err := os.ReadFile(metadataPath)
	if err != nil {
		return false
	}
	...
	found := false
	for _, entry := range entries {
		if entry.Name == name && entry.Description == description && entry.Type == skillType && entry.Value == value {
			found = true
			break
		}
	}
	...
	settingsPath := filepath.Join(a.GetUserHomeDir(), ".claude", "settings.json")
	settingsData, err := os.ReadFile(settingsPath)
	if err != nil {
		return false
	}
	var settings map[string]interface{}
	if json.Unmarshal(settingsData, &settings) != nil || settings == nil {
		return false
	}
	...
	enabledValue, ok := enabled[value].(bool)
	return ok && enabledValue
}
...
func (a *App) AddSkill(name, description, skillType, value, toolName string) (retErr error) {
	...
	// Legacy metadata writes share installMutex with asynchronous status-overlay
	// persistence. Drain first so a queued overlay cannot deadlock behind a lock
	// that this method is about to hold, or overwrite the new metadata afterward.
	if a.skillExecutor != nil {
		a.skillExecutor.suspendStatusOverlayPersistence()
		defer a.skillExecutor.resumeStatusOverlayPersistence()
	}
	...
	defer func() {
		if noChange {
			a.recordLegacySkillResult("add", name, LegacySkillInstallResult{OK: true, State: "skipped", CleanupStatus: "clear", RollbackComplete: true, FailureReason: "no_change"})
			return
		}
	...
	var noChangeErr *legacySkillNoChangeError
...
func (a *App) addSkillLockedWithRecord(name, description, skillType, value, toolName string, sharedRecord *skill.EvolutionCompensationRecord) error {
	...
	if sharedRecord == nil {
		...
		// Recovery of legacy records must run before admission so corrupt
		// rollback payloads can accumulate bounded attempts and transition to
		// needs_review. The admission check still executes before the first
		// metadata/package write below.
	...
	found := false
	// ZIP metadata stores only the package basename. Normalize the incoming
	// value before the no-op comparison so retrying AddSkill with the same
	// absolute path does not manufacture a second package transaction.
	comparisonValue := value
	if skillKind.IsZip() {
		comparisonValue = filepath.Base(resolveLegacySkillPackagePath(value, skillsDir))
	}
	...
	for i, s := range skills {
		if s.Name == name {
			...
			storedValue := s.Value
			if skillKind.IsZip() {
				// Older metadata rows may have retained an absolute package path;
				// compare their basename as well so the migration remains idempotent.
				storedValue = filepath.Base(storedValue)
			}
			if sharedRecord == nil && s.Description == description && s.Type == skillType && storedValue == comparisonValue {
				return &legacySkillNoChangeError{}
			}
			finalValue := value
			if skillKind.IsZip() {
	...
	if stagedPackage != "" {
		rollbackCleanup := append([]string(nil), legacyRecord.RollbackCleanupPaths...)
		rollbackCleanup = append(rollbackCleanup, stagedPackage)
		legacyRecord.SetRollbackCleanupPaths(rollbackCleanup)
	...
	clearLegacyRecord := func(committed bool, cleanupErr error) error {
		...
		if committed {
			...
			legacyRecord.CleanupStatus = "pending"
			if cleanupErr != nil {
				// Keep committed business state separate from post-commit cleanup.
				// The shared helper owns bounded retry and needs_review escalation
				// so legacy Add/Install paths cannot drift from newer installers.
				if err := skill.MarkEvolutionCompensationCleanupFailure(&legacyRecord, cleanupErr); err != nil {
					return fmt.Errorf("persist cleanup failure: %w", err)
				}
				return cleanupErr
			}
		...
		if cleanupErr != nil {
			...
			if err := skill.MarkEvolutionCompensationRollbackFailure(&legacyRecord, cleanupErr); err != nil {
				return fmt.Errorf("persist rollback compensation: %w", err)
			}
			return nil
		}
		if err := a.clearLegacySkillCompensation(legacyRecord.RequestID, legacyRecord.Skill, legacyRecord.Action); err != nil {
			legacyRecord.TransactionState = "rolled_back"
			legacyRecord.CleanupStatus = "pending"
			legacyRecord.LastError = err.Error()
			if persistErr := skill.MarkEvolutionCompensationRollbackFailure(&legacyRecord, err); persistErr != nil {
				return fmt.Errorf("persist compensation cleanup failure: %w", persistErr)
			}
			return err
		}
	...
	if err := publishPackage(); err != nil {
		...
		if rollbackErr != nil {
			if stateErr := clearLegacyRecord(false, rollbackErr); stateErr != nil {
				return fmt.Errorf("publish skill package: %w; rollback package: %v; persist compensation: %v", err, rollbackErr, stateErr)
			}
			return fmt.Errorf("publish skill package: %w; rollback package: %v", err, rollbackErr)
		}
		if stateErr := clearLegacyRecord(false, err); stateErr != nil {
			return fmt.Errorf("publish skill package: %w; persist compensation: %v", err, stateErr)
		}
		return err
	}
	if stagedPackage == "" && packagePublished {
		if packageHadPrevious && packageBackup != "" {
			...
			// InstallSkill may already have registered input staging artifacts
			// (for example the temporary ZIP snapshot) on the outer record. Keep
			// those durable cleanup targets when adding the package backup; using
			// the setter with a fresh slice would silently drop the earlier target
			// and leak it across a crash after the final audit.
			postCommitCleanup := append([]string(nil), legacyRecord.PostCommitCleanupPaths...)
			postCommitCleanup = append(postCommitCleanup, packageBackup)
			legacyRecord.SetPostCommitCleanupPaths(postCommitCleanup)
		} else {
		...
		if err := persistLegacyRecord(); err != nil {
			...
			if rollbackErr != nil {
				if stateErr := clearLegacyRecord(false, rollbackErr); stateErr != nil {
					return fmt.Errorf("persist legacy package compensation: %w; rollback package: %v; persist compensation: %v", err, rollbackErr, stateErr)
				}
				return fmt.Errorf("persist legacy package compensation: %w; rollback package: %v", err, rollbackErr)
			}
			if stateErr := clearLegacyRecord(false, err); stateErr != nil {
				return fmt.Errorf("persist legacy package compensation: %w; persist compensation: %v", err, stateErr)
			}
	...
	if err := atomicWriteFile(metadataPath, data); err != nil {
		...
		if rollbackErr != nil {
			if stateErr := clearLegacyRecord(false, rollbackErr); stateErr != nil {
				return fmt.Errorf("write skill metadata: %w; rollback package: %v; persist compensation: %v", err, rollbackErr, stateErr)
			}
		...
		if metadataExists {
			if restoreErr := atomicWriteFile(metadataPath, originalMetadata); restoreErr != nil {
				if stateErr := clearLegacyRecord(false, restoreErr); stateErr != nil {
					return fmt.Errorf("write skill metadata: %w; restore metadata: %v; persist compensation: %v", err, restoreErr, stateErr)
				}
			...
			if removeErr := os.Remove(metadataPath); removeErr != nil && !os.IsNotExist(removeErr) {
				if stateErr := clearLegacyRecord(false, removeErr); stateErr != nil {
					return fmt.Errorf("write skill metadata: %w; remove metadata: %v; persist compensation: %v", err, removeErr, stateErr)
				}
				return fmt.Errorf("write skill metadata: %w; remove metadata: %v", err, removeErr)
			}
		}
		if stateErr := clearLegacyRecord(false, err); stateErr != nil {
			return fmt.Errorf("write skill metadata: %w; persist compensation: %v", err, stateErr)
		}
	...
	if sharedRecord == nil {
		if err := a.refreshSkillIndexesAfterMutationChecked(name); err != nil {
			...
			if rollbackErr != nil {
				if stateErr := clearLegacyRecord(false, rollbackErr); stateErr != nil {
					return fmt.Errorf("refresh Skill index: %w; rollback: %v; persist compensation: %v", err, rollbackErr, stateErr)
				}
	...
	if err := skill.RecordEvolutionEventStrict("skill:legacy_installed", map[string]string{
		...
		"attempt": "1", "config_revision": skillEvolutionConfigRevision(a), "schema_version": "2", "evidence_mode": "none",
	}, "desktop"); err != nil {
		rollbackErr := rollbackPackage()
		if metadataExists {
			if restoreErr := atomicWriteFile(metadataPath, originalMetadata); restoreErr != nil {
		...
		if rollbackErr != nil {
			if stateErr := clearLegacyRecord(false, rollbackErr); stateErr != nil {
				return fmt.Errorf("legacy Skill final audit: %w; rollback: %v; persist compensation: %v", err, rollbackErr, stateErr)
			}
	...
	if err := clearLegacyRecord(true, nil); err != nil {
		return &legacySkillCommitError{err: fmt.Errorf("legacy Skill commit state persist failed: %w", err), committed: true, requestID: legacyRecord.RequestID}
	}
	if packageBackup != "" {
		if err := os.Remove(packageBackup); err != nil && !os.IsNotExist(err) {
			...
			if stateErr := clearLegacyRecord(true, cleanupErr); stateErr != nil && !errors.Is(stateErr, cleanupErr) {
				cleanupErr = fmt.Errorf("%v; persist cleanup state: %w", cleanupErr, stateErr)
			}
			return &legacySkillCommitError{err: cleanupErr, committed: true, cleanupStatus: legacyRecord.CleanupStatus, requestID: legacyRecord.RequestID}
		}
	}
	if err := a.clearLegacySkillCompensation(legacyRecord.RequestID, legacyRecord.Skill, legacyRecord.Action); err != nil {
		if markErr := skill.MarkEvolutionCompensationCleanupFailure(&legacyRecord, err); markErr != nil {
			return &legacySkillCommitError{err: fmt.Errorf("legacy Skill cleanup state could not persist: %w", markErr), committed: true, cleanupStatus: legacyRecord.CleanupStatus, requestID: legacyRecord.RequestID}
		}
		return &legacySkillCommitError{err: fmt.Errorf("legacy Skill compensation cleanup failed: %w", err), committed: true, cleanupStatus: legacyRecord.CleanupStatus, requestID: legacyRecord.RequestID}
	}
	if a.skillExecutor != nil {
...
const (
	legacyMarketplaceAction = "legacy_gui_marketplace"
	legacyMarketplaceSkill  = "default_marketplace"
)

// recoverLegacyMarketplaceCompensations replays only the durable settings
// transactions owned by the legacy GUI marketplace adapter. A pending record
// blocks new writes until the exact settings pre-image is restored (or the
// record reaches needs_review), and the recovery scope prevents another home
// directory from claiming the record.
func (a *App) recoverLegacyMarketplaceCompensations() error {
	if a == nil {
		return fmt.Errorf("app not initialized")
	}
	home := strings.TrimSpace(a.GetUserHomeDir())
	if home == "" {
		return fmt.Errorf("user home directory is unavailable")
	}
	if err := skill.CheckEvolutionCompensationQueue(); err != nil {
		return fmt.Errorf("legacy marketplace mutation blocked: compensation queue unavailable: %w", err)
	}
	pipeline := skill.NewEvolutionPipeline()
	if _, pending, err := pipeline.RecoverPendingCompensationsForActionPrefixAndScopeWithExternalRecovery(legacyMarketplaceAction, home, nil); err != nil {
		return fmt.Errorf("legacy marketplace compensation recovery failed: %w", err)
	} else if pending > 0 {
		return fmt.Errorf("legacy marketplace mutation blocked: pending compensation requires review")
	}
	return nil
}

// prepareDefaultMarketplaceSettings builds the exact settings post-image
// without writing anything. Keeping this pure lets the public API persist a
// durable pre-image before creating ~/.claude or replacing settings.json.
func (a *App) prepareDefaultMarketplaceSettings() (settingsFile string, before []byte, existed bool, after []byte, changed bool, err error) {
	home := strings.TrimSpace(a.GetUserHomeDir())
	if home == "" {
		return "", nil, false, nil, false, fmt.Errorf("user home directory is unavailable")
	}
	settingsFile = filepath.Join(home, ".claude", "settings.json")
	var settings map[string]interface{}
	if data, readErr := os.ReadFile(settingsFile); readErr == nil {
		before = append([]byte(nil), data...)
		existed = true
		if err := json.Unmarshal(data, &settings); err != nil {
			return "", nil, false, nil, false, fmt.Errorf("decode marketplace settings: %w", err)
		}
		if settings == nil {
			return "", nil, false, nil, false, fmt.Errorf("decode marketplace settings: top-level JSON must be an object")
		}
	} else if !os.IsNotExist(readErr) {
		return "", nil, false, nil, false, fmt.Errorf("read marketplace settings: %w", readErr)
	} else {
		settings = make(map[string]interface{})
	}
	...
	if !ok {
		if _, exists := settings["extraKnownMarketplaces"]; exists {
			return "", nil, false, nil, false, fmt.Errorf("decode marketplace settings: extraKnownMarketplaces must be an object")
		}
		marketplaces = make(map[string]interface{})
	}
	marketplaceMatches := func(key, repo string) bool {
		entry, ok := marketplaces[key].(map[string]interface{})
		if !ok {
			return false
		}
		source, ok := entry["source"].(map[string]interface{})
		return ok && source["source"] == "github" && source["repo"] == repo
	}
	if existing, exists := marketplaces["anthropic-agent-skills"]; exists && !marketplaceMatches("anthropic-agent-skills", "anthropics/skills") {
		return "", nil, false, nil, false, fmt.Errorf("decode marketplace settings: anthropic-agent-skills has a conflicting source (%T)", existing)
	}
	if existing, exists := marketplaces["superpowers-marketplace"]; exists && !marketplaceMatches("superpowers-marketplace", "obra/superpowers-marketplace") {
		return "", nil, false, nil, false, fmt.Errorf("decode marketplace settings: superpowers-marketplace has a conflicting source (%T)", existing)
	}
	if _, exists := marketplaces["anthropic-agent-skills"]; !exists {
		marketplaces["anthropic-agent-skills"] = map[string]interface{}{"source": map[string]interface{}{"source": "github", "repo": "anthropics/skills"}}
		changed = true
	}
	if _, exists := marketplaces["superpowers-marketplace"]; !exists {
		marketplaces["superpowers-marketplace"] = map[string]interface{}{"source": map[string]interface{}{"source": "github", "repo": "obra/superpowers-marketplace"}}
		changed = true
	}
	if !changed {
		return settingsFile, before, existed, nil, false, nil
	}
	settings["extraKnownMarketplaces"] = marketplaces
	after, err = json.MarshalIndent(settings, "", "  ")
	if err != nil {
		return "", nil, false, nil, false, fmt.Errorf("marshal marketplace settings: %w", err)
	}
	return settingsFile, before, existed, after, true, nil
}

// installDefaultMarketplaceMutationLocked applies only the settings mutation
// while the caller owns installMutex. InstallSkill uses this helper inside its
// outer durable record; it must not create a nested compensation or audit.
func (a *App) installDefaultMarketplaceMutationLocked() error {
	...
	settingsFile, _, _, data, changed, err := a.prepareDefaultMarketplaceSettings()
	if err != nil {
		return err
	}
	if !changed {
	...
	if err := a.applyDefaultMarketplaceSettingsLocked(settingsFile, data); err != nil {
		return fmt.Errorf("failed to write settings: %w", err)
	}
	return nil
}

// applyDefaultMarketplaceSettingsLocked performs only the prepared settings
// write. Callers that already own a durable outer transaction use this helper
// so the write can be fenced and rolled back together with the rest of the
// install; the standalone InstallDefaultMarketplace path keeps its own
// transaction around the same mutation.
func (a *App) applyDefaultMarketplaceSettingsLocked(settingsFile string, data []byte) error {
...
// InstallDefaultMarketplace is a standalone legacy settings transaction. It
// persists the pre-image before the first write, uses a strict audit as the
// business boundary, and never restores an already-audited settings image.
func (a *App) InstallDefaultMarketplace() error {
	...
	a.installMutex.Lock()
	defer a.installMutex.Unlock()
	if err := a.recoverLegacyMarketplaceCompensations(); err != nil {
		return err
	}
	if err := a.ensureWorkflowAllowsRemoteToolCall("manage_skill", map[string]interface{}{"action": "install", "source": "default_marketplace"}); err != nil {
		return err
	}
	settingsFile, before, existed, after, changed, err := a.prepareDefaultMarketplaceSettings()
	if err != nil || !changed {
		return err
	}
	requestID := fmt.Sprintf("%s_%d", legacyMarketplaceAction, time.Now().UnixNano())
	record := skill.NewEvolutionCompensationRecord(requestID, legacyMarketplaceSkill, legacyMarketplaceAction, "", nil, false, nil, "legacy_marketplace_mutation")
	record.FinalAuditKind = skill.KindFromEventName("skill:legacy_marketplace_committed")
	record.TransactionState = "prepared"
	record.CleanupStatus = "pending"
	record.SetSkipIndexRefresh(true)
	record.SetRecoveryScope(a.GetUserHomeDir())
	record.SetFileSnapshots([]skill.EvolutionFileSnapshot{{Path: settingsFile, BackupB64: base64.StdEncoding.EncodeToString(before), Exists: existed}})
	if err := skill.PersistEvolutionCompensation(record); err != nil {
		return fmt.Errorf("persist legacy marketplace compensation: %w", err)
	}
	persisted := true
	rollback := func(cause error) error {
		...
		if restoreErr := skill.RestoreEvolutionCompensation(record, nil, nil); restoreErr != nil {
			...
			record.CleanupStatus = "pending"
			record.LastError = restoreErr.Error()
			if stateErr := skill.MarkEvolutionCompensationRollbackFailure(&record, restoreErr); stateErr != nil {
				return fmt.Errorf("%v; restore marketplace settings: %v; persist compensation: %w", cause, restoreErr, stateErr)
			}
			return fmt.Errorf("%v; restore marketplace settings: %w", cause, restoreErr)
		}
		if clearErr := a.clearLegacySkillCompensation(record.RequestID, record.Skill, record.Action); clearErr != nil {
			record.TransactionState = "rolled_back"
			record.CleanupStatus = "pending"
			record.LastError = clearErr.Error()
			if stateErr := skill.MarkEvolutionCompensationRollbackFailure(&record, clearErr); stateErr != nil {
				return fmt.Errorf("%v; clear marketplace compensation: %v; persist compensation: %w", cause, clearErr, stateErr)
			}
			return fmt.Errorf("%v; clear marketplace compensation: %w", cause, clearErr)
		}
		persisted = false
		return cause
	}
	if err := a.writeLegacySkillFile(settingsFile, after); err != nil {
		return rollback(fmt.Errorf("write marketplace settings: %w", err))
	}
	a.log("Default marketplaces added to ~/.claude/settings.json")
	postImage, err := os.ReadFile(settingsFile)
	if err != nil {
		return rollback(fmt.Errorf("snapshot published marketplace settings: %w", err))
	}
	if err := record.SetFileSnapshotPostImage(settingsFile, postImage, true); err != nil {
		return rollback(fmt.Errorf("record published marketplace settings: %w", err))
	}
	if err := skill.ReplaceEvolutionCompensation(record); err != nil {
		return rollback(fmt.Errorf("persist published marketplace settings fence: %w", err))
	}
	if err := skill.RecordEvolutionEventStrict("skill:legacy_marketplace_committed", map[string]string{
		"skill": legacyMarketplaceSkill, "action": legacyMarketplaceAction, "decision": "applied", "request_id": requestID,
		"attempt": "1", "config_revision": skillEvolutionConfigRevision(a), "schema_version": "2", "evidence_mode": "none",
	}, "desktop"); err != nil {
		return rollback(fmt.Errorf("legacy marketplace final audit: %w", err))
	}
	record.TransactionState = "committed"
	record.CleanupStatus = "pending"
	record.FailureReason = "post_commit_cleanup_pending"
	if err := skill.ReplaceEvolutionCompensation(record); err != nil {
		return &legacySkillCommitError{err: fmt.Errorf("legacy marketplace commit state persist failed: %w", err), committed: true, requestID: requestID, cleanupStatus: record.CleanupStatus}
	}
	if err := a.clearLegacySkillCompensation(record.RequestID, record.Skill, record.Action); err != nil {
		if markErr := skill.MarkEvolutionCompensationCleanupFailure(&record, err); markErr != nil {
			return &legacySkillCommitError{err: fmt.Errorf("legacy marketplace cleanup state persist failed: %w", markErr), committed: true, requestID: requestID, cleanupStatus: record.CleanupStatus}
		}
		return &legacySkillCommitError{err: fmt.Errorf("legacy marketplace compensation cleanup failed: %w", err), committed: true, requestID: requestID, cleanupStatus: record.CleanupStatus}
	}
	return nil
}
...
func ensureSkillZipDoesNotOverwriteExisting(archivePath, destinationRoot string) error {
	...
	if rootHasFiles {
		...
		// Root-level files are merged by the legacy extractor. An empty
		// destination is safe because every incoming file is a new path and is
		// recorded in the durable CreatedDirs rollback plan below. A non-empty
		// destination could contain files that would be overwritten without a
		// bounded pre-image, so retain the fail-closed guard there.
		if len(entries) == 0 {
			return nil
		}
...
// legacySkillInstallCreatedPaths predicts the top-level paths (and root-level
// files) an archive publication may create. It is persisted before extraction
// so a crash during unzip still leaves a durable, bounded removal plan.
// Existing top-level paths are rejected by ensureSkillZipDoesNotOverwriteExisting.
func legacySkillInstallCreatedPaths(archivePath, destinationRoot string) ([]string, error) {
	...
	for _, entry := range archive.File {
		...
		if len(parts) == 1 {
			if !entry.FileInfo().IsDir() {
				rootHasFiles = true
				rootFiles[parts[0]] = struct{}{}
			}
			continue
		}
		tops[parts[0]] = struct{}{}
	}
	paths := make([]string, 0, len(tops)+len(rootFiles)+1)
	if rootHasFiles {
		if _, statErr := os.Stat(destinationRoot); os.IsNotExist(statErr) {
			...
			// The overwrite guard allows an existing destination only when it is
			// empty. Record each root-level file explicitly so rollback removes
			// only files created by this transaction rather than the destination
			// directory (which may be shared by other skills).
			for file := range rootFiles {
				paths = append(paths, filepath.Join(destinationRoot, file))
			}
		}
	}
	for top := range tops {
...
// legacySkillInstallScanCacheDirs identifies only the Skill directories that
// this ZIP can publish. The legacy installer may add a package beside existing
// Skills in one shared root; caching every discovered directory there would
// overwrite unrelated Skills' scan evidence with the current package report.
func legacySkillInstallScanCacheDirs(archivePath, destinationRoot string) ([]string, error) {
	archive, err := zip.OpenReader(archivePath)
	if err != nil {
		return nil, fmt.Errorf("inspect skill ZIP scan-cache paths: %w", err)
	}
	defer archive.Close()

	rootHasSkillDoc := false
	topLevels := make(map[string]struct{})
	for _, entry := range archive.File {
		rawName := strings.ReplaceAll(entry.Name, "\\", "/")
		if strings.HasPrefix(rawName, "/") {
			return nil, fmt.Errorf("unsafe absolute skill ZIP path %q", entry.Name)
		}
		...
		for _, part := range parts {
			if part == "" || part == "." || part == ".." || filepath.VolumeName(part) != "" {
				return nil, fmt.Errorf("unsafe skill ZIP path %q", entry.Name)
			}
		}
		if len(parts) == 1 {
			if !entry.FileInfo().IsDir() && isSkillMarkdownDocFileName(parts[0]) {
				rootHasSkillDoc = true
			}
			continue
		}
		topLevels[parts[0]] = struct{}{}
	...
	dirs := make([]string, 0, len(topLevels)+1)
	if rootHasSkillDoc && directoryHasSkillDoc(destinationRoot) {
		dirs = append(dirs, destinationRoot)
	}
	tops := make([]string, 0, len(topLevels))
	for top := range topLevels {
		tops = append(tops, top)
	}
	sort.Strings(tops)
	for _, top := range tops {
		dir := filepath.Join(destinationRoot, top)
...
func (a *App) InstallSkill(name, description, skillType, value, location, projectPath, toolName string) (retErr error) {
	...
	// Legacy recovery must precede admission so a corrupt rollback record gets
	// its bounded retry accounting and can become needs_review. Both recovery
	// helpers are fail-closed on an unreadable queue, and this check remains
	// before any new package, metadata, or settings write.
	if err := ensureSkillEvolutionMutationAdmission(a, name); err != nil {
		return err
	}
	locationKind := normalizeSkillInstallLocationKind(location)
	if !locationKind.IsUser() && !locationKind.IsProject() {
		return fmt.Errorf("skill install location must be user or project")
	}
	projectPath = normalizeProjectSessionPath(projectPath)
	...
	skillKind := normalizeSkillTypeKind(skillType)
	// Address/plugin installs are idempotent when both the legacy metadata
	// entry and the enabledPlugins setting already match. Return a structured
	// skipped result before touching marketplace/settings or creating durable
	// compensation state.
	if locationKind.IsUser() && skillKind.IsAddress() && a.legacyAddressInstallAlreadyCurrent(name, description, skillType, value, toolName) {
		a.recordLegacySkillResult("install", name, LegacySkillInstallResult{
			OK: true, State: "skipped", CleanupStatus: "clear", RollbackComplete: true, FailureReason: "already_current",
		})
		return nil
	}
	// 1. Validate
	...
	var marketplaceSettingsPath string
	var marketplaceSettingsData []byte
	marketplaceSettingsChanged := false
	if locationKind.IsUser() && skillKind.IsAddress() {
		home := a.GetUserHomeDir()
		if strings.TrimSpace(home) == "" {
			return fmt.Errorf("user home directory is unavailable")
		}
		// Prepare the marketplace mutation before the outer compensation is
		// persisted, but defer the actual write until after that durable boundary.
		// This keeps marketplace registration and plugin enablement in the same
		// rollback scope instead of committing a nested settings transaction that
		// could survive a later metadata/index failure.
		var prepErr error
		var preparedBefore []byte
		var preparedExisted bool
		marketplaceSettingsPath, preparedBefore, preparedExisted, marketplaceSettingsData, marketplaceSettingsChanged, prepErr = a.prepareDefaultMarketplaceSettings()
		if prepErr != nil {
			return prepErr
		}
		// Reuse the exact pre-image read by prepareDefaultMarketplaceSettings for
		// the outer snapshot. A second independent read would widen the race
		// window and could restore an image that never preceded the prepared write.
		settingsPath = marketplaceSettingsPath
		settingsBefore = append([]byte(nil), preparedBefore...)
		settingsExisted = preparedExisted
	...
	if skillKind.IsZip() && zipSnapshotPath != "" {
		// The archive snapshot is an input staging artifact. Record it in both
		// cleanup directions so a crash before extraction or after commit cannot
		// leak a temporary ZIP outside the process lifetime. The normal defer is
		// only an in-process fast path; durable recovery owns the crash boundary.
		installRecord.SetRollbackCleanupPaths([]string{zipSnapshotPath})
		installRecord.SetPostCommitCleanupPaths([]string{zipSnapshotPath})
	...
	defer func() {
		...
		if retErr != nil {
			...
			if errors.As(retErr, &committedErr) && committedErr.committed {
				result.OK = false
				result.State = "committed"
				// A legacy cleanup hook may exhaust its bounded retry budget in
				// process (for example when a locked file prevents queue removal).
				// Preserve the durable terminal state instead of flattening it back
				// to pending in the compatibility result.
				result.CleanupStatus = strings.TrimSpace(installRecord.CleanupStatus)
				if result.CleanupStatus == "" || result.CleanupStatus == "clear" {
					result.CleanupStatus = "pending"
				}
				result.RollbackComplete = true
	...
	installCleanup := func() error {
		...
		if installCommitted {
			// Remove only explicitly registered post-commit artifacts before
			// clearing the durable record. Clearing first would lose the recovery
			// plan if the process crashed between queue deletion and artifact
			// cleanup (notably the temporary ZIP snapshot used by legacy installs).
			if err := skill.CleanupCommittedEvolutionCompensation(installRecord); err != nil {
				if markErr := skill.MarkEvolutionCompensationCleanupFailure(&installRecord, err); markErr != nil {
					return fmt.Errorf("persist install cleanup state: %w", markErr)
				}
				return err
			}
			if err := a.clearLegacySkillCompensation(installRecord.RequestID, installRecord.Skill, installRecord.Action); err != nil {
				if markErr := skill.MarkEvolutionCompensationCleanupFailure(&installRecord, err); markErr != nil {
					return fmt.Errorf("persist install cleanup state: %w", markErr)
				}
				return err
		...
		if err := skill.RestoreEvolutionCompensation(installRecord, nil, nil); err != nil {
			...
			installRecord.CleanupStatus = "pending"
			if stateErr := skill.MarkEvolutionCompensationRollbackFailure(&installRecord, err); stateErr != nil {
				return fmt.Errorf("install rollback failed: %v; persist compensation: %w", err, stateErr)
			}
			return err
		}
		...
		if err := a.refreshSkillIndexesAfterMutationChecked(name); err != nil {
			...
			installRecord.CleanupStatus = "pending"
			if stateErr := skill.MarkEvolutionCompensationRollbackFailure(&installRecord, err); stateErr != nil {
				return fmt.Errorf("restore install index failed: %v; persist compensation: %w", err, stateErr)
			}
			return err
		}
		if err := a.clearLegacySkillCompensation(installRecord.RequestID, installRecord.Skill, installRecord.Action); err != nil {
			...
			installRecord.CleanupStatus = "pending"
			if stateErr := skill.MarkEvolutionCompensationRollbackFailure(&installRecord, err); stateErr != nil {
				return fmt.Errorf("clear install compensation failed: %v; persist compensation: %w", err, stateErr)
			}
			return err
		}
		installRecordPersisted = false
	...
	// 2. Install to Tool
	// Settings restoration is owned by the durable outer compensation record
	// (FileSnapshots). Do not perform a second best-effort process-local restore
	// here: an ignored write/remove error could leave settings divergent while
	// the queue is incorrectly cleared. installCleanup restores the snapshot and
	// retains the record whenever that checked restore cannot be proven.
	if locationKind.IsUser() {
		if skillKind.IsAddress() {
			...
			if marketplaceSettingsChanged {
				if err := a.applyDefaultMarketplaceSettingsLocked(marketplaceSettingsPath, marketplaceSettingsData); err != nil {
			...
			// Capture the published image before any later metadata/index/audit
			// step can fail. Recovery compares this digest before restoring so a
			// concurrent settings writer is never overwritten by a stale pre-image.
			if installRecordPersisted {
				postSettings, readErr := os.ReadFile(settingsFile)
				if readErr != nil {
					return fmt.Errorf("snapshot published plugin settings: %w", readErr)
				}
				if err := installRecord.SetFileSnapshotPostImage(settingsFile, postSettings, true); err != nil {
					return fmt.Errorf("record published plugin settings: %w", err)
				}
				if err := skill.ReplaceEvolutionCompensation(installRecord); err != nil {
					return fmt.Errorf("persist published plugin settings fence: %w", err)
				}
			}
			...
			// Re-check the destination after the durable record has been
			// prepared. This closes the common race where another writer populated
			// an initially empty skills directory during validation.
			if err := ensureSkillZipDoesNotOverwriteExisting(fullPath, destDir); err != nil {
				return fmt.Errorf("recheck skill ZIP destination: %w", err)
			}
			_, destinationStatErr := os.Stat(destDir)
			...
			for _, dir := range newlyInstalledSkillDirs(destDir, beforeDirs) {
				// A pre-existing empty destination can legitimately receive
				// root-level package files. It is not transaction-owned as a whole;
				// the durable CreatedDirs plan tracks those individual files.
				if destinationExisted && filepath.Clean(dir) == filepath.Clean(destDir) {
					continue
				}
				installedSkillDirs = append(installedSkillDirs, dir)
			}
			...
			cacheDirs, cacheDirsErr := legacySkillInstallScanCacheDirs(fullPath, destDir)
			if cacheDirsErr != nil {
				return cacheDirsErr
			}
			if err := writeSkillScanCacheForInstalledZipDirs(name, description, cacheDirs, installScanReport, installScanReports); err != nil {
		...
		if errors.As(err, &committedErr) && committedErr.committed {
			...
			if commitErr := markInstallCommitted(); commitErr != nil {
				...
				if installCommitted {
					...
					return &legacySkillCommitError{err: fmt.Errorf("%v; outer install cleanup: %w", err, commitErr), committed: true, requestID: installRecord.RequestID, cleanupStatus: installRecord.CleanupStatus}
				}
			...
			if cleanupErr := cleanupImportedSkillDirs(installedSkillDirs); cleanupErr != nil {
				return fmt.Errorf("%v; %w", err, cleanupErr)
			}
		}
		return err
	}
	// addSkillLockedWithRecord has now published the metadata registry and
	// package copy. Persist a post-image digest before the checked index and
	// final audit; rollback will refuse to overwrite a concurrent metadata
	// mutation that occurred after this transaction's write.
	if installRecordPersisted {
		postMetadata, readErr := os.ReadFile(installMetadataPath)
		if readErr != nil {
			return fmt.Errorf("snapshot published install metadata: %w", readErr)
		}
		if err := installRecord.SetFileSnapshotPostImage(installMetadataPath, postMetadata, true); err != nil {
			return fmt.Errorf("record published install metadata: %w", err)
		}
		if err := skill.ReplaceEvolutionCompensation(installRecord); err != nil {
			return fmt.Errorf("persist published install metadata fence: %w", err)
		}
	}
	...
	if err := markInstallCommitted(); err != nil {
		if installCommitted {
			return &legacySkillCommitError{err: fmt.Errorf("legacy Skill install committed but cleanup failed: %w", err), committed: true, requestID: installRecord.RequestID, cleanupStatus: installRecord.CleanupStatus}
		}
...
func snapshotSkillZipForInstall(src string) (string, func(), error) {
	...
	// Keep the snapshot isolated while preserving the caller's basename. The
	// legacy metadata registry stores filepath.Base(value); a random CreateTemp
	// basename would make an identical retry look like a different package.
	tmpDir, err := os.MkdirTemp("", "maclaw-skill-install-*")
	if err != nil {
		return "", func() {}, fmt.Errorf("create skill zip snapshot directory: %w", err)
	}
	base := filepath.Base(src)
	if base == "." || base == string(filepath.Separator) || strings.TrimSpace(base) == "" {
		_ = os.RemoveAll(tmpDir)
		return "", func() {}, fmt.Errorf("invalid skill zip filename")
	}
	...
	if err := tmp.Sync(); err != nil {
		_ = tmp.Close()
		cleanup()
		return "", func() {}, fmt.Errorf("sync skill zip snapshot: %w", err)
	}
	if err := tmp.Close(); err != nil {
		cleanup()
...
func (a *App) DeleteSkill(name, toolName string) (retErr error) {
	...
	defer func() {
		if noChange {
			a.recordLegacySkillResult("delete", name, LegacySkillInstallResult{OK: true, State: "skipped", CleanupStatus: "clear", RollbackComplete: true, FailureReason: "no_change"})
			return
		}
	...
	restoreOrQueue := func(cause error) error {
		if restoreErr := skill.RestoreEvolutionCompensation(record, nil, nil); restoreErr != nil {
			...
			if stateErr := skill.MarkEvolutionCompensationRollbackFailure(&record, restoreErr); stateErr != nil {
				return fmt.Errorf("%v; legacy Skill delete rollback state persist failed: %w", cause, stateErr)
			}
		...
		if indexErr := a.refreshSkillIndexesAfterMutationChecked(name); indexErr != nil {
			...
			if stateErr := skill.MarkEvolutionCompensationRollbackFailure(&record, indexErr); stateErr != nil {
				return fmt.Errorf("%v; legacy Skill delete index rollback state persist failed: %w", cause, stateErr)
			}
		...
		if clearErr := a.clearLegacySkillCompensation(record.RequestID, record.Skill, record.Action); clearErr != nil {
			...
			record.LastError = clearErr.Error()
			if stateErr := skill.MarkEvolutionCompensationRollbackFailure(&record, clearErr); stateErr != nil {
				return fmt.Errorf("%v; clear compensation state persist failed: %w", cause, stateErr)
			}
	...
	if err := skill.ReplaceEvolutionCompensation(record); err != nil {
		return &legacySkillCommitError{err: fmt.Errorf("legacy Skill delete committed but compensation update failed: %w", err), committed: true, requestID: record.RequestID, cleanupStatus: record.CleanupStatus}
	}
	for _, item := range deleted {
		if err := os.Remove(item.staged); err != nil && !os.IsNotExist(err) {
			if markErr := skill.MarkEvolutionCompensationCleanupFailure(&record, err); markErr != nil {
				return &legacySkillCommitError{err: fmt.Errorf("legacy Skill metadata committed but cleanup state could not persist: %w", markErr), committed: true, cleanupStatus: record.CleanupStatus, requestID: record.RequestID}
			}
			return &legacySkillCommitError{err: fmt.Errorf("legacy Skill metadata committed but package cleanup failed: %w", err), committed: true, cleanupStatus: record.CleanupStatus}
		}
	}
	if err := a.clearLegacySkillCompensation(record.RequestID, record.Skill, record.Action); err != nil {
		if markErr := skill.MarkEvolutionCompensationCleanupFailure(&record, err); markErr != nil {
			return &legacySkillCommitError{err: fmt.Errorf("legacy Skill delete cleanup state could not persist: %w", markErr), committed: true, cleanupStatus: record.CleanupStatus, requestID: record.RequestID}
		}
		return &legacySkillCommitError{err: fmt.Errorf("legacy Skill delete compensation cleanup failed: %w", err), committed: true, cleanupStatus: record.CleanupStatus}
	}
	if a.skillExecutor != nil {
		a.skillExecutor.invalidateSkillCache()
	} else if a.cachedSkillScanner != nil {

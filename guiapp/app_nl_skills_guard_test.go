...
func TestSetNLSkillStatusSameValueIsSideEffectFree(t *testing.T) {
	tempHome := t.TempDir()
	...
	t.Cleanup(func() { app.shutdown(context.Background()) })
	if err := app.skillExecutor.Register(corelib.NLSkillEntry{
		Name: "already-active", Status: "active", Source: "manual",
		Steps: []corelib.NLSkillStep{{Action: "noop", Params: map[string]interface{}{}}},
	}); err != nil {
		t.Fatalf("Register() error = %v", err)
	}

	if err := app.SetNLSkillStatus("already-active", "active"); err != nil {
		t.Fatalf("SetNLSkillStatus() error = %v", err)
	}
	if _, err := os.Stat(skill.DefaultEvolutionCompensationPath()); !os.IsNotExist(err) {
		t.Fatalf("same-value status change created compensation queue: err=%v", err)
	}
	if events, err := skill.ListEvolutionAudit(skill.DefaultEvolutionAuditPath(), 20); err != nil {
		t.Fatalf("ListEvolutionAudit() error = %v", err)
	} else if len(events) != 0 {
		t.Fatalf("same-value status change emitted audit events: %#v", events)
	}
	if err := os.MkdirAll(filepath.Dir(skill.DefaultEvolutionCompensationPath()), 0o755); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(skill.DefaultEvolutionCompensationPath(), []byte("{malformed\n"), 0o600); err != nil {
		t.Fatal(err)
	}
	if err := app.SetNLSkillStatus("already-active", "active"); err == nil || !strings.Contains(err.Error(), "compensation queue unavailable") {
		t.Fatalf("same-value status change hid malformed queue: %v", err)
	}
}
...
func TestDefinitionMutationBlocksWhenCompensationQueueUnreadableEvenIfDisabled(t *testing.T) {
	...
	def := corelib.NLSkillEntry{Name: "queue-guard-disabled", Status: "disabled", Source: "manual"}
	if err := app.commitNLSkillDefinitionAfterAdmission(context.Background(), def, "create", "skill:definition_created", true, "operator", nil); err == nil || !strings.Contains(err.Error(), "compensation queue unavailable") {
		t.Fatalf("disabled definition mutation error = %v, want unreadable queue block", err)
	}
	if entries := app.skillExecutor.loadSkills(); len(entries) != 0 {
		t.Fatalf("queue-blocked mutation changed registry: %#v", entries)
	}
}

func TestSkillExecutorCompatibilityMutationsBlockOnUnreadableCompensationQueue(t *testing.T) {
	tempHome := t.TempDir()
	t.Setenv("HOME", tempHome)
	t.Setenv("USERPROFILE", tempHome)
	t.Setenv("AppData", filepath.Join(tempHome, "AppData", "Roaming"))
	...
	app.skillExecutor = NewSkillExecutor(app, nil, nil)
	t.Cleanup(func() { app.shutdown(context.Background()) })
	seed := corelib.NLSkillEntry{
		Name:   "compatibility-guard",
		Status: "active",
		Source: "manual",
		Steps:  []corelib.NLSkillStep{{Action: "noop", Params: map[string]interface{}{}}},
	}
	if err := app.skillExecutor.Register(seed); err != nil {
	...
	if err := app.skillExecutor.Register(corelib.NLSkillEntry{Name: "compatibility-new", Source: "manual"}); err == nil || !strings.Contains(err.Error(), "compensation queue unavailable") {
		t.Fatalf("Register() error = %v, want unreadable queue block", err)
	}
	if err := app.skillExecutor.UpdateStatus("compatibility-guard", "needs_review"); err == nil || !strings.Contains(err.Error(), "compensation queue unavailable") {
		t.Fatalf("UpdateStatus() error = %v, want unreadable queue block", err)
	}
	if err := app.skillExecutor.Update(corelib.NLSkillEntry{Name: "compatibility-guard", Status: "disabled"}); err == nil || !strings.Contains(err.Error(), "compensation queue unavailable") {
		t.Fatalf("Update() error = %v, want unreadable queue block", err)
	}
	entries := app.skillExecutor.loadSkills()
	if len(entries) != 1 || entries[0].Name != "compatibility-guard" || entries[0].Status != "active" {
		t.Fatalf("compatibility mutations changed registry: %#v", entries)
	}
}

func TestSkillExecutorMarkUploadedBlocksOnUnreadableCompensationQueue(t *testing.T) {
	tempHome := t.TempDir()
	t.Setenv("HOME", tempHome)
	t.Setenv("USERPROFILE", tempHome)
	t.Setenv("AppData", filepath.Join(tempHome, "AppData", "Roaming"))
	...
	app.skillExecutor = NewSkillExecutor(app, nil, nil)
	t.Cleanup(func() { app.shutdown(context.Background()) })
	skillDir := filepath.Join(tempHome, "skill-upload-guard")
	if err := os.MkdirAll(skillDir, 0o755); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(skillDir, "SKILL.md"), []byte("# guarded\n"), 0o600); err != nil {
		t.Fatal(err)
	}
	if err := app.skillExecutor.Register(corelib.NLSkillEntry{
		Name: "upload-guard", Source: "file", SkillDir: skillDir,
		Status: "active", Steps: []corelib.NLSkillStep{{Action: "noop"}},
	}); err != nil {
		t.Fatalf("seed Register() error = %v", err)
	}
	queuePath := skill.DefaultEvolutionCompensationPath()
	...
	if err := app.skillExecutor.MarkUploaded("upload-guard", "pkg-123"); err == nil || !strings.Contains(err.Error(), "compensation queue unavailable") {
		t.Fatalf("MarkUploaded() error = %v, want unreadable queue block", err)
	}
	if _, err := os.Stat(filepath.Join(skillDir, "upload_status.json")); !os.IsNotExist(err) {
		t.Fatalf("blocked MarkUploaded created upload receipt: %v", err)
	}
}

func TestSkillExecutorUsagePersistenceBlocksOnUnreadableCompensationQueue(t *testing.T) {
	app := setupNLSkillTransactionApp(t)
	seed := corelib.NLSkillEntry{
		Name: "usage-guard", Status: "active", Source: "manual",
		Steps: []corelib.NLSkillStep{{Action: "noop"}},
	}
	if err := app.skillExecutor.Register(seed); err != nil {
		t.Fatalf("seed Register() error = %v", err)
	}
	...
	app.skillExecutor.recordSkillExecution(func(entry corelib.NLSkillEntry) bool {
		return entry.Name == "usage-guard"
	}, "output", nil)
	got := findNLSkillDefinitionForTest(app.ListNLSkills(), "usage-guard")
	if got == nil || got.UsageCount != 0 || got.SuccessCount != 0 {
		t.Fatalf("blocked usage persistence changed counters: %#v", got)
	}
}

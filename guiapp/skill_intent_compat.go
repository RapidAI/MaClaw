...
const (
	...
	intentCatModify   intentCategory = "modify"   // 修改/编辑/更新/替换/复制
...
var intentVerbsCJK = map[string]intentCategory{
	...
	// Modify / operate-on-existing — 2-char verbs
	"修改": intentCatModify, "编辑": intentCatModify, "更新": intentCatModify,
	"替换": intentCatModify, "复制": intentCatModify, "拷贝": intentCatModify,
	// Send — 2-char verbs
...
var intentVerbsEN = map[string]intentCategory{
	...
	// Modify / operate-on-existing
	"edit": intentCatModify, "modify": intentCatModify, "update": intentCatModify,
	"copy": intentCatModify, "copies": intentCatModify, "copying": intentCatModify,
	// Send
...
// path. Generate/send are the skill-preference jobs. Modify (including copy)
// can still be capability-compatible with a generate skill for "修改后导出",
// but it must not enter skill preference: the user is operating on a file
// that already exists.
...
func extractUserIntentCategory(text string) intentCategory {
	...
	// Pass 1: CJK 2-char verb scan. Skip hits that sit inside a path or
	// filename token so "复制品.pdf 生成报告" is generate, not modify.
	runes := []rune(lower)
	for i := 0; i+2 <= len(runes); i++ {
		twoChar := string(runes[i : i+2])
		cat, ok := intentVerbsCJK[twoChar]
		if !ok {
			cat, ok = supplementalIntentVerbsCJK[twoChar]
		}
		if !ok || skillDocMatchLooksLikePathOperand(runes, i, i+2) {
			continue
		}
		return cat
	}

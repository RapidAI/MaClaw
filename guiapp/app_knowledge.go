...
// knowledgeImportRuntimeStatus projects the legacy UI status strings onto the
// shared asynchronous-job vocabulary without breaking existing Wails clients.
func knowledgeImportRuntimeStatus(status string) agentruntime.JobStatus {
...
func updateKnowledgeImportJobProgress(a *App, id string, result knowledge.DirectoryImportResult) {
	...
	if a != nil && a.ctx != nil {
		...
		eventData := map[string]interface{}{
			...
			"runtime_status":   string(knowledgeImportRuntimeStatus(result.Status)),

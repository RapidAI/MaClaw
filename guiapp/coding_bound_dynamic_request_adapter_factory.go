...
func (r *codingBoundDynamicRequestLifecycleRelay) ReserveToolSurfaceRequestChannel(ctx context.Context, cfg corelib.MaclawLLMConfig) (agent.ToolSurfaceRequestChannel, error) {
	...
	// The kill switch makes an attached composition inert for new requests: a
	// nil, nil answer keeps RunLoop on the S0.5 compatibility path, and no
	// reservation, catalog read, plan publication, or alias materialization
	// happens. In-flight reservations keep their existing lifecycle contract.
	...
	// A successor reservation must publish its surface as a child revision of
	// the durable lineage, never as an independent root. The lineage parent is
	// read only from the durable coordinator record of this relay's verified
	// identity; the retired holder's memory, task text, runtime IDs, and
	// configuration are never consulted. By this point the predecessor's
	// terminal write has completed (the fences above), so the record is stable.
	parent, parentErr := r.successorParentRevision()
	if parentErr != nil {
		adapter.Close(fmt.Errorf("coding dynamic successor lineage is unavailable"))
		return nil, fmt.Errorf("coding dynamic successor lineage is unavailable: %w", parentErr)
	}
	if parent != nil {
		adapter.setSuccessorParentRevision(*parent)
	}
	r.active = adapter
	return adapter, nil
}

// ActiveAdapterForTesting returns the currently reserved adapter for hermetic
// lifecycle tests. Production code must go through the relay methods above.
func (r *codingBoundDynamicRequestLifecycleRelay) ActiveAdapterForTesting() *codingBoundDynamicRequestAdapter {
	if r == nil {
		return nil
	}
	r.mu.Lock()
...
// successorParentRevision returns the lineage-current durable revision for
// this relay's verified identity, or nil when this reservation opens the
// lineage as its root. It is the only lineage source a successor reservation
// may use.
func (r *codingBoundDynamicRequestLifecycleRelay) successorParentRevision() (*tool.RouteRevisionRef, error) {
	coordinator, err := r.handler.app.semanticExecutionCoordinatorForApp()
	if err != nil {
		return nil, err
	}
	scope := tool.InvocationScope{RootTaskID: r.identity.RootTaskID, SessionID: r.identity.SessionID, PrincipalID: r.identity.PrincipalID}
	ref, err := coordinator.Routes.CurrentRevision(scope)
	if err != nil {
		if strings.Contains(err.Error(), "route_revision_not_found") {
			return nil, nil
		}
		return nil, err
	}
...
func newQualifiedCodingBoundDynamicRequestLifecycleRelay(handler *IMMessageHandler, identity *trustedCodingInvocationIdentity, cfg corelib.MaclawLLMConfig) *codingBoundDynamicRequestLifecycleRelay {
	...
	// A compile-time qualification does not make an empty runtime endpoint
	// usable. Keep such callbacks on the compatibility path instead of
	// installing a relay that can only fail reservation.
	if strings.TrimSpace(cfg.URL) == "" {
		return nil
	}
	qualification := codingDynamicProductionAdapterQualificationForAssembly(cfg)
...
func reserveCodingBoundDynamicRequestAdapter(ctx context.Context, handler *IMMessageHandler, identity *trustedCodingInvocationIdentity, cfg corelib.MaclawLLMConfig) (*codingBoundDynamicRequestAdapter, error) {
	...
	// scope-plan/admission handoff. Hermetic E3/E4 callers install the explicit
	// package-internal override before exercising this factory.
	if codingDynamicProductionAdapterQualificationOverride == nil {
		return nil, nil
	}
	qualification := codingDynamicProductionAdapterQualificationForAssembly(cfg)

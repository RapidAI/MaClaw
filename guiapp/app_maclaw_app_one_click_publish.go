...
func (a *App) uploadInstalledNLSkillToSkillMarkets(ctx context.Context, skillName string) (string, error) {
	...
	// One-click publishing is still an upload entry point. Reuse the same
	// queue/compensation admission and authoritative-source portability fence as
	// manage_skill(action="upload") before any network submission.
	a.ensureSkillLifecycleManager()
	if a.skillLifecycle == nil {
		return "", fmt.Errorf("skill lifecycle manager not initialized")
	}
	if err := a.skillLifecycle.ensureEvolutionUploadAdmission(skillName); err != nil {
		return "", err
	}
	...
	if target := a.skillLifecycle.findRegisteredSkill(skillName); target != nil && strings.TrimSpace(target.SkillDir) != "" {
		if err := a.skillLifecycle.prepareUploadSourceWithCheckedIndex(target.Name, target.SkillDir); err != nil {
			return "", err
		}
	}
...
func (a *App) uploadMaclawAppPackAsSkillMarketZip(ctx context.Context, packageJSON string) (string, error) {
	...
	// Even generated App packs cross the remote upload boundary. A malformed
	// or unreadable evolution compensation queue must therefore block the
	// submission instead of letting this special path bypass global recovery
	// admission.
	if err := cskill.CheckEvolutionCompensationQueue(); err != nil {
		return "", fmt.Errorf("upload blocked: evolution compensation queue unavailable: %w", err)
	}
	a.ensureSkillMarketClient()
	if a.skillMarketClient == nil {
		return "", fmt.Errorf("skill market client not initialized")

...
const (
	...
	userDataMigrationJobRetention         = agentruntime.DefaultJobRetention
...
// userDataMigrationRuntimeStatus projects the legacy migration status onto
// the shared Runtime job vocabulary. The legacy Status field remains in the
// response for older Wails clients, while RuntimeStatus is the canonical
// cross-host value consumed by new clients.
func userDataMigrationRuntimeStatus(status string) agentruntime.JobStatus {
	return agentruntime.ProjectLegacyJobStatus(status)
}

func normalizeUserDataMigrationJob(job *userDataMigrationJob) {
	if job == nil {
		return
	}
	job.RuntimeStatus = userDataMigrationRuntimeStatus(job.Status)
}

var (
	userDataMigrationJobs sync.Map
	// userDataMigrationRuntimeJobs is a transitional shared-repository mirror.
	// The Wails DTO remains map-backed for compatibility, while every lifecycle
	// mutation is also published through the transport-neutral JobRepository.
	userDataMigrationRuntimeJobs           = agentruntime.NewMemoryJobRepository()
	userDataMigrationRuntimeJobsMu         sync.Mutex
...
func userDataMigrationRuntimeJob(job userDataMigrationJob) agentruntime.Job {
	runtimeJob := agentruntime.Job{
		ID: job.ID, Kind: job.Kind, Status: job.RuntimeStatus,
		RecoveryPolicy: agentruntime.JobRecoveryPolicyReconcile,
		Progress:       job.Progress, ProgressText: job.ProgressText,
		Error: job.Error, CreatedAt: job.CreatedAt, CompletedAt: job.CompletedAt,
	}
	if job.Result != nil {
		if payload, err := json.Marshal(job.Result); err == nil {
			runtimeJob.Result = payload
		}
	}
	return runtimeJob
}

func publishUserDataMigrationRuntimeJob(job userDataMigrationJob) {
	if userDataMigrationRuntimeJobs == nil || strings.TrimSpace(job.ID) == "" {
		return
	}
	userDataMigrationRuntimeJobsMu.Lock()
	defer userDataMigrationRuntimeJobsMu.Unlock()
	if err := agentruntime.UpsertJob(context.Background(), userDataMigrationRuntimeJobs, userDataMigrationRuntimeJob(job)); err != nil {
		log.Printf("[onboarding-migration] runtime job upsert failed job_id=%s err=%v", job.ID, err)
	}
}

// userDataMigrationJobReporter adapts the GUI's transitional in-memory job
// registry to the shared Runtime progress contract. The worker only sees the
// agentruntime.JobReporter interface; storage can later be replaced by a
// durable repository without changing migration code.
type userDataMigrationJobReporter struct {
	jobID string
}

var _ agentruntime.JobReporter = userDataMigrationJobReporter{}

func (r userDataMigrationJobReporter) ReportJobUpdate(ctx context.Context, update agentruntime.JobUpdate) error {
	...
	if update.Progress != nil {
		...
		updateUserDataMigrationJob(r.jobID, func(job *userDataMigrationJob) {
	...
	// Migration currently has no resumable domain checkpoint. Rejecting a
	// checkpoint here keeps that future data out of the GUI's private registry
	// until a durable repository is available.
	if update.Checkpoint != nil {
		return fmt.Errorf("%w: GUI migration checkpoints are not supported", agentruntime.ErrInvalidJobUpdate)
	}
	return nil
}
...
func (a *App) GetUserDataMigrationJob(jobID string) (userDataMigrationJob, error) {
	...
	normalizeUserDataMigrationJob(&job)
	return job, nil
}
...
func (a *App) startUserDataMigrationJob(kind string, run func(context.Context, func(float64, string)) (map[string]interface{}, error)) (userDataMigrationJob, error) {
	...
	job := userDataMigrationJob{
		...
		Kind:          kind,
		Status:        "running",
		RuntimeStatus: agentruntime.JobStatusRunning,
		CreatedAt:     now,
		UpdatedAt:     now,
	}
	userDataMigrationJobs.Store(job.ID, job)
	publishUserDataMigrationRuntimeJob(job)
	log.Printf("[onboarding-migration] job_started job_id=%s kind=%s", job.ID, kind)
	go func(jobID string) {
		jobCtx := agentruntime.WithJobReporter(a.userDataMigrationContext(), userDataMigrationJobReporter{jobID: jobID})
		progress := func(v float64, text string) {
			...
			if err := agentruntime.ReportJobProgress(jobCtx, v, text); err != nil && !errors.Is(err, context.Canceled) {
				log.Printf("[onboarding-migration] job_progress_report_failed job_id=%s err=%v", jobID, err)
			}
		...
		func() {
			...
			result, err = run(jobCtx, progress)
		}()
...
func userDataMigrationCompleteConfigMap(cfg corelib.AppConfig) (map[string]interface{}, error) {
	...
	for i := 0; i < typeOfConfig.NumField(); i++ {
		...
		name := coreconfig.JSONFieldName(field)
		if name == "" {
			continue
		}
		if name == "-" {

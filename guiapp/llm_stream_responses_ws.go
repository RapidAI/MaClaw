...
func (h *IMMessageHandler) streamResponsesWSRequestChannelVerified(
	...
	for {
		...
		if err != nil {
			// A cancelled request context closed this socket: the read error is
			// the cancellation, not a transport failure. Route it through the
			// post-loop fence so the outcome carries the context error and no
			// late frame can be mistaken for this reservation's response.
			if reqCtx.Err() != nil {
				goto postLoop
			}
		...
		case responsesEventOutputItemDone:
			// A compatible provider may deliver the complete function_call item
			// only at done time; the added event then carried an incomplete
			// shell without the provider-issued call ID. Extract the same
			// identity fields here so every function call reaching the response
			// owns its provider call ID. Fields already accumulated from earlier
			// events always win.
			var done struct {
				OutputIndex int                    `json:"output_index"`
				Item        map[string]interface{} `json:"item"`
			...
			if itemType, _ := done.Item["type"].(string); normalizeResponsesOutputItemKind(itemType) == responsesOutputItemFunctionCall {
				acc := itemAccums[done.OutputIndex]
				if acc == nil {
					acc = &responsesItemAccum{}
					itemAccums[done.OutputIndex] = acc
				}
				if acc.itemType == "" {
					acc.itemType = itemType
				}
				if acc.callID == "" {
					if cid, _ := done.Item["call_id"].(string); cid != "" {
						acc.callID = cid
					}
				}
				if acc.name == "" {
					if n, _ := done.Item["name"].(string); n != "" {
						acc.name = n
					}
				}
				if acc.args.Len() == 0 {
					if args, _ := done.Item["arguments"].(string); args != "" {
...
postLoop:
	// A cancelled request context makes the transport outcome ambiguous: the
	// watchdog closed the socket and any frame arriving afterwards belongs to
	// no reservation. Return the accumulated partial response only as
	// diagnostic payload alongside the context error, so the request owner can
	// never bind it as this reservation's response.
	if reqCtx.Err() != nil {
		return agent.VerifiedToolSurfaceDispatch{Response: &llm.Response{
			ResponseID: responseID,
			Choices:    []llm.Choice{{Message: llm.Message{Role: "assistant"}, FinishReason: "stop"}},
			Usage:      usage,
		}, Receipt: receipt}, reqCtx.Err()
	}

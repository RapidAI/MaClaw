...
func TestCapabilityGapDetectorResolveDoesNotAutoInstall(t *testing.T) {
	d := &CapabilityGapDetector{}
	var statuses []string
	name, result, err := d.Resolve(context.Background(), "install a missing capability", nil, func(status string) {
		statuses = append(statuses, status)
	})
	if err == nil || !strings.Contains(err.Error(), "automatic capability-gap installation is disabled") {
		t.Fatalf("Resolve() error = %v, want automatic-install rejection", err)
	}
	if name != "" || result != "" {
		t.Fatalf("Resolve() returned mutation result without desktop app: name=%q result=%q", name, result)
	}
	if len(statuses) != 1 || !strings.Contains(statuses[0], "人工审核并安装") {
		t.Fatalf("Resolve() statuses = %#v, want manual-install guidance", statuses)
	}
}

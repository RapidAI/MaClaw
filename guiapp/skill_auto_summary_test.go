...
func autoSummaryDurableMutationSession(id string) *TrajectorySession {
	return &TrajectorySession{
		SessionID: id,
		Entries: []TrajectoryEntry{
			{Role: "user", Content: "summarize the repeatable remote workspace inspection"},
			{Role: "assistant", ToolCalls: makeToolCallsWithID("auto-summary-1", "ssh_read_file", `{"path":"/workspace/config.json"}`)},
			{Role: "tool_result", ToolCallID: "auto-summary-1", Content: "{}"},
			{Role: "assistant", ToolCalls: makeToolCallsWithID("auto-summary-2", "ssh_list_dir", `{"path":"/workspace"}`)},
			{Role: "tool_result", ToolCallID: "auto-summary-2", Content: "config.json"},
			{Role: "assistant", ToolCalls: makeToolCallsWithID("auto-summary-3", "ssh_write_file", `{"path":"/workspace/report.txt","content":"checked"}`)},
			{Role: "tool_result", ToolCallID: "auto-summary-3", Content: "ok"},
		},
	}
}

func TestSkillAutoSummaryPipelineDefaultsToNoDurableMutation(t *testing.T) {
	app := setupNLSkillTransactionApp(t)
	if err := app.SaveConfig(corelib.AppConfig{}); err != nil {
		t.Fatal(err)
	}
	p := NewSkillAutoSummaryPipeline(nil, allowAllChecker(), app.skillExecutor, nil)
	p.RunPipeline(autoSummaryDurableMutationSession("auto-summary-disabled"))
	...
	if len(cfg.NLSkills) != 0 {
		t.Fatalf("disabled auto-summary registered skills: %#v", cfg.NLSkills)
	}
	...
	if entries, err := os.ReadDir(root); err == nil && len(entries) != 0 {
		t.Fatalf("disabled auto-summary created durable skill entries: %#v", entries)
	} else if err != nil && !os.IsNotExist(err) {
		t.Fatalf("read primary skills root: %v", err)
	}
}

func TestSkillAutoSummaryPipelineKillSwitchBlocksDurableMutation(t *testing.T) {
	app := setupNLSkillTransactionApp(t)
	...
	p := NewSkillAutoSummaryPipeline(nil, allowAllChecker(), app.skillExecutor, nil)
	p.RunPipeline(autoSummaryDurableMutationSession("auto-summary-kill-switch"))
	...
	if len(loaded.NLSkills) != 0 {
		t.Fatalf("kill-switched auto-summary registered skills: %#v", loaded.NLSkills)
	}
}

func TestSkillAutoSummaryPipelineCommitsStagedDefinition(t *testing.T) {
	app := setupNLSkillTransactionApp(t)
	cfg := corelib.AppConfig{}
	cfg.SetSkillEvolutionEnabled(true)
	if err := app.SaveConfig(cfg); err != nil {
	...
	p := NewSkillAutoSummaryPipeline(nil, allowAllChecker(), app.skillExecutor, nil)
	p.RunPipeline(autoSummaryDurableMutationSession("auto-summary-staged-commit"))

	loaded := app.skillExecutor.loadSkills()
	if len(loaded) != 1 {
		t.Fatalf("installed skills=%#v, want one staged auto-summary", loaded)
	}
	entry := loaded[0]
	if entry.Source != "learned" {
		t.Fatalf("auto-summary provenance=%#v, want learned source", entry)
	}
	root, err := app.primarySkillsDir()
	if err != nil {
		t.Fatal(err)
	...
	if filepath.Dir(filepath.Clean(entry.SkillDir)) != filepath.Clean(root) {
		t.Fatalf("auto-summary skill dir=%q, want direct child of %q", entry.SkillDir, root)
	}
	if _, err := os.Stat(filepath.Join(entry.SkillDir, "skill.yaml")); err != nil {
		t.Fatalf("committed auto-summary definition is missing: %v", err)
	}
	auditData, err := os.ReadFile(skill.DefaultEvolutionAuditPath())
	if err != nil {
		t.Fatalf("read auto-summary audit: %v", err)
	}
	if !strings.Contains(string(auditData), `"via":"automatic_auto_summary"`) {
		t.Fatalf("auto-summary audit lost automatic origin: %s", auditData)
	}
	...
	if entries, err := os.ReadDir(stagingRoot); err == nil && len(entries) != 0 {
		t.Fatalf("committed auto-summary left staging entries: %#v", entries)
	} else if err != nil && !os.IsNotExist(err) {
		t.Fatalf("read staging root: %v", err)
	}
}

func TestSkillAutoSummaryPipelineRollsBackStagedDefinitionOnIndexFailure(t *testing.T) {
	app := setupNLSkillTransactionApp(t)
	cfg := corelib.AppConfig{}
	cfg.SetSkillEvolutionEnabled(true)
	if err := app.SaveConfig(cfg); err != nil {
	...
	p := NewSkillAutoSummaryPipeline(nil, allowAllChecker(), app.skillExecutor, nil)
	p.RunPipeline(autoSummaryDurableMutationSession("auto-summary-index-rollback"))
	...
	if len(loaded.NLSkills) != 0 {
		t.Fatalf("index-failed auto-summary remained registered: %#v", loaded.NLSkills)
	}
	root, err := app.primarySkillsDir()
	if err != nil {
		t.Fatal(err)
	...
	if entries, err := os.ReadDir(root); err == nil && len(entries) != 0 {
		t.Fatalf("index-failed auto-summary left durable skill entries: %#v", entries)
	} else if err != nil && !os.IsNotExist(err) {
		t.Fatalf("read primary skills root: %v", err)
	}
}

func TestSkillAutoSummaryPipelineDefersAutomaticReplacement(t *testing.T) {
	app := setupNLSkillTransactionApp(t)
	cfg := corelib.AppConfig{}
	cfg.SetSkillEvolutionEnabled(true)
	if err := app.SaveConfig(cfg); err != nil {
	...
	session := autoSummaryDurableMutationSession("auto-summary-existing")
	draft, err := DraftSkill(session)
	if err != nil {
		t.Fatal(err)
	}
	...
	for i := range draft.Steps {
		// The incoming successful trajectory removes these recoverable-error
		// markers, so shouldUpdateSkill would approve it if replacement were
		// still permitted to write directly.
		draft.Steps[i].OnError = "skip"
	}
	draft.Name = "auto-summary-existing"
	first, err := RunQualityGate(draft, nil)
	if err != nil {
		t.Fatal(err)
	}
	t.Cleanup(func() { _ = os.RemoveAll(first.SkillDir) })
	before, err := os.ReadFile(filepath.Join(first.SkillDir, "skill.yaml"))
	if err != nil {
		t.Fatal(err)
	}
	...
	after, err := os.ReadFile(filepath.Join(first.SkillDir, "skill.yaml"))
	if err != nil {
		t.Fatal(err)
	}
	if string(after) != string(before) {
		t.Fatalf("automatic replacement rewrote existing definition:\nbefore=%s\nafter=%s", before, after)
	}
	if backups, err := filepath.Glob(filepath.Join(first.SkillDir, "skill.yaml.v*")); err != nil || len(backups) != 0 {
		t.Fatalf("automatic replacement created version artifacts=%v err=%v", backups, err)
	}
	loaded, err := app.LoadConfig()
	if err != nil {
		t.Fatal(err)
	...
	if len(loaded.NLSkills) != 0 {
		t.Fatalf("automatic replacement wrote a config overlay: %#v", loaded.NLSkills)
	}
}

...
//
// The sibling expansion uses ExpandNeedTemplates so an iterative coding
// budget is represented by individually planned selections, never by a
// runtime counter or a provider-selected repeat policy. Keeping the
...
func codingDynamicCapabilityNeedsFromTemplates(templates []agentservice.IntentCapabilityNeedTemplate) []tool.CapabilityNeed {
	normalized := make([]agentservice.IntentCapabilityNeedTemplate, 0, len(templates))
	...
	return agentservice.ExpandNeedTemplates(normalized, agentservice.ExpandNeedTemplateOptions{
		IDPrefix:    "need:coding:",
		Confidence:  1,
...
func prepareCodingDynamicSemanticPlan(identity *trustedCodingInvocationIdentity, dynamic codingDynamicCatalogSnapshot, needs []tool.CapabilityNeed, facts []tool.RoutingFact, constraints []tool.RoutingConstraint, budget tool.PlanningBudget, now time.Time) (codingDynamicPlanPreparation, error) {
	...
	plan, err := tool.NewToolPlanner(registry).Plan(tool.RouteRequest{
		...
		ChannelScope: "coding",
		Snapshot:     snapshot,
		Needs:        tool.CloneCapabilityNeeds(needs),
		Facts:        tool.CloneRoutingFacts(facts),
		Constraints:  tool.CloneRoutingConstraints(constraints),
		Budget:       budget,
		Now:          now,
	})
	if err != nil {

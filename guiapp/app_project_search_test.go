...
func TestLocalCloudWorkspaceIDIgnoresRemoteCodingStrayTag(t *testing.T) {
	remote := memory.ProjectRecord{
		ProjectPath: "D:/tasks/remote-fix",
		Tags: []string{
			taskManagementTag, taskRemoteCodingDevTag, cloudWorkspaceTag("cws_math"),
			recentTaskWorkingDirTag("/var/www/app"),
		},
	}
	if got := localCloudWorkspaceID(remote); got != "" {
		t.Fatalf("remote SSH row id=%q, want empty", got)
	}
	cloud := memory.ProjectRecord{
		ProjectPath: "C:/data/cloud-workspaces/tenant/cws_math",
		Tags: []string{
			taskManagementTag, cloudWorkspaceTag("cws_math"),
			recentTaskWorkingDirTag("C:/data/cloud-workspaces/tenant/cws_math"),
		},
	}
	if got := localCloudWorkspaceID(cloud); got != "cws_math" {
		t.Fatalf("cloud row id=%q", got)
	}
}

func TestNormalizeCloudWorkspaceTaskModeIsExclusive(t *testing.T) {
	if got := normalizeCloudWorkspaceTaskMode("remote_coding_dev"); got != "" {
		t.Fatalf("mode=%q, want exclusive cloud identity", got)
	}
	if got := normalizeCloudWorkspaceTaskMode("coding_dev"); got != "" {
		t.Fatalf("coding mode=%q, want exclusive cloud identity", got)
	}
	if got := normalizeCloudWorkspaceTaskMode(""); got != "" {
		t.Fatalf("chat mode=%q", got)
	}
	if got := cloudWorkspaceModeFromTags([]string{taskManagementTag, taskRemoteCodingDevTag, cloudWorkspaceTag("cws_x")}); got != "" {
		t.Fatalf("sidecar mode=%q, want empty identity mode", got)
	}
}

func TestScrubCloudWorkspaceIdentityTagsDropsCodingAndRemote(t *testing.T) {
	got := scrubCloudWorkspaceIdentityTags([]string{
		taskManagementTag, taskCodingDevTag, taskRemoteCodingDevTag, cloudWorkspaceTag("cws_scholar"),
		"remote_host:www.example.test", recentTaskWorkingDirTag("C:/data/cloud-workspaces/tenant/cws_scholar"),
	})
	if projectRecordHasTagLike(got, taskCodingDevTag) || projectRecordHasTagLike(got, taskRemoteCodingDevTag) || projectRecordHasTagLike(got, "remote_host:www.example.test") {
		t.Fatalf("overlap tags survived: %v", got)
	}
	if !projectRecordHasTagLike(got, cloudWorkspaceTag("cws_scholar")) || !projectRecordHasTagLike(got, taskManagementTag) {
		t.Fatalf("cloud identity dropped: %v", got)
	}
}

func TestListTasksScrubsCodingTagsFromCloudWorkspace(t *testing.T) {
	app := newProjectSearchTestApp(t)
	app.ensureMemoryStore()
	created := app.createTaskRecordWithWorkingDir(
		"长江学者申请",
		"# 长江学者申请\n",
		[]string{taskManagementTag, taskUserCreatedTag, cloudWorkspaceTag("cws_scholar"), taskCodingDevTag},
		"C:/data/cloud-workspaces/tenant/cws_scholar",
		true,
	)
	if created.ProjectPath == "" {
		t.Fatal("cloud task path empty")
	}
	for _, result := range app.ListTasks(20) {
		if result.ProjectPath != created.ProjectPath {
			continue
		}
		if projectRecordHasTagLike(result.Tags, taskCodingDevTag) || projectRecordHasTagLike(result.Tags, taskRemoteCodingDevTag) {
			t.Fatalf("list leaked coding identity tags: %v", result.Tags)
		}
		if !projectRecordHasTagLike(result.Tags, cloudWorkspaceTag("cws_scholar")) {
			t.Fatalf("cloud tag missing: %v", result.Tags)
		}
		return
	}
	t.Fatal("cloud workspace row missing from ListTasks")
}

func TestCodingModeForProjectRecordRemapsCloudRemoteOverlap(t *testing.T) {
	cloud := memory.ProjectRecord{
		ProjectPath: "C:/data/cloud-workspaces/tenant/cws_math",
		Tags: []string{
			taskManagementTag, taskRemoteCodingDevTag, cloudWorkspaceTag("cws_math"),
	...
	if got := codingModeForProjectRecord(cloud); got != "" {
		t.Fatalf("cloud+remote mode=%q, want exclusive cloud identity", got)
	}
	remote := memory.ProjectRecord{
		ProjectPath: "D:/tasks/remote-fix",
		Tags: []string{
	...
	if got := codingModeForProjectRecord(remote); got != taskRemoteCodingDevTag {
		t.Fatalf("ssh mode=%q, want remote coding", got)
	}
	chat := memory.ProjectRecord{
		ProjectPath: "C:/data/cloud-workspaces/tenant/cws_chat",
		Tags:        []string{taskManagementTag, cloudWorkspaceTag("cws_chat")},
	}
	if got := codingModeForProjectRecord(chat); got != "" {
		t.Fatalf("cloud chat mode=%q, want empty", got)
	}
}

func TestCollapseDuplicateCloudWorkspaceRecordsDoesNotAbsorbRemoteCoding(t *testing.T) {
	now := time.Now()
	records := []memory.ProjectRecord{
		{ProjectPath: "remote", Name: "远程修复", Tags: []string{
			taskManagementTag, taskRemoteCodingDevTag, cloudWorkspaceTag("cws_math"),
			recentTaskWorkingDirTag("/var/www/app"),
		}, LastActivity: now.Add(time.Hour)},
		{ProjectPath: "cloud", Name: "人工智能数学基础", Tags: []string{
			taskManagementTag, cloudWorkspaceTag("cws_math"),
			recentTaskWorkingDirTag("C:/data/cloud-workspaces/tenant/cws_math"),
		}, LastActivity: now},
	}
	got := collapseDuplicateCloudWorkspaceRecords(records)
	if len(got) != 2 {
		t.Fatalf("records=%+v, want remote SSH and cloud workspace kept apart", got)
	}
}

func TestCapTaskSearchResultsKeepsOlderCloudWorkspace(t *testing.T) {
	results := []ProjectSearchResult{
		{ProjectPath: "local-new", Name: "newer chat", Tags: []string{taskManagementTag}},
		{ProjectPath: "local-mid", Name: "mid chat", Tags: []string{taskManagementTag}},
		{ProjectPath: "cloud-old", Name: "人工智能数学基础", Tags: []string{taskManagementTag, cloudWorkspaceTag("cws_math")}, WorkingDir: "C:/data/cloud-workspaces/tenant/cws_math"},
	}
	got := capTaskSearchResultsKeepingCloudWorkspaces(results, 2)
	if len(got) != 2 {
		t.Fatalf("got=%+v, want 2 rows", got)
	}
	foundCloud := false
	for _, result := range got {
		if result.ProjectPath == "cloud-old" {
			foundCloud = true
		}
	}
	if !foundCloud {
		t.Fatalf("older cloud workspace dropped: %+v", got)
	}
}
...
func TestSearchTasksDoesNotInjectUnrelatedCloudWorkspaces(t *testing.T) {
	app := newProjectSearchTestApp(t)
	app.ensureMemoryStore()
	cloud := app.createTaskRecordWithWorkingDir(
		"人工智能数学基础",
		"# 人工智能数学基础\n",
		[]string{taskManagementTag, taskUserCreatedTag, cloudWorkspaceTag("cws_math")},
		"C:/data/cloud-workspaces/tenant/cws_math",
		true,
	)
	if cloud.ProjectPath == "" {
		t.Fatal("cloud task path empty")
	}
	if created := app.CreateTask("unrelated local chat", ""); created.ProjectPath == "" {
		t.Fatal("local task empty")
	}
	results := app.SearchTasks("zzz-no-such-task-xyz", 20)
	for _, result := range results {
		if result.ProjectPath == cloud.ProjectPath {
			t.Fatalf("search injected unrelated cloud workspace: %+v", results)
		}
	}
}

func TestListTasksDoesNotDropVisibleCloudWorkspaceForHiddenDuplicate(t *testing.T) {
	app := newProjectSearchTestApp(t)
	app.ensureMemoryStore()
	named := app.createTaskRecordWithWorkingDir(
		"人工智能数学基础",
		"# named\n",
		[]string{taskManagementTag, taskUserCreatedTag, cloudWorkspaceTag("cws_dup")},
		"C:/data/cloud-workspaces/tenant/cws_dup",
		true,
	)
	generic := app.createTaskRecordWithWorkingDir(
		"新建云端工作区任务",
		"# generic\n",
		[]string{taskManagementTag, taskUserCreatedTag, cloudWorkspaceTag("cws_dup")},
		"C:/data/cloud-workspaces/tenant/cws_dup",
	...
	if named.ProjectPath == "" || generic.ProjectPath == "" {
		t.Fatal("setup paths empty")
	}
	app.memoryStore.ProjectIndex().SetHidden(named.ProjectPath, true)
	results := app.ListTasks(20)
	foundGeneric, foundNamed := false, false
	for _, result := range results {
		if result.ProjectPath == generic.ProjectPath {
			foundGeneric = true
		}
		if result.ProjectPath == named.ProjectPath {
			foundNamed = true
		}
	}
	if foundNamed {
		t.Fatalf("hidden named row must stay off the list: %+v", results)
	}
	if !foundGeneric {
		t.Fatalf("visible cloud row dropped after hidden duplicate merged: %+v", results)
	}
}

func TestListTasksKeepsOlderCloudWorkspaceWhenLimitIsFull(t *testing.T) {
	app := newProjectSearchTestApp(t)
	app.ensureMemoryStore()
	older := app.createTaskRecordWithWorkingDir(
		"人工智能数学基础",
		"# 人工智能数学基础\n",
		[]string{taskManagementTag, taskUserCreatedTag, cloudWorkspaceTag("cws_math")},
		"C:/data/cloud-workspaces/tenant/cws_math",
	...
	if older.ProjectPath == "" {
		t.Fatal("cloud task path empty")
	}
	for i := 0; i < 4; i++ {
		if created := app.CreateTask(fmt.Sprintf("newer local %d", i), ""); created.ProjectPath == "" {
			t.Fatalf("local task %d empty", i)
		}
	}
	results := app.ListTasks(2)
	foundCloud := false
	for _, result := range results {
		if result.ProjectPath == older.ProjectPath {
			foundCloud = true
		}
	}
	if !foundCloud {
		t.Fatalf("ListTasks(2)=%+v, want older cloud workspace kept", results)
	}
}

...
func registerBuiltinTools(registry *ToolRegistry, h *IMMessageHandler) {
	...
	regCtxP := func(name, desc string, cat ToolCategory, tags []string, schema map[string]interface{}, required []string, handler ToolHandlerWithContext) {
		...
		registry.Register(RegisteredTool{
			...
			Source:                "builtin",
			ExecutionContract:     defaultExplicitExecutionContractMetadata(name),
			RuntimePolicyOwnerArg: name == "database" || name == "database_query",
			HandlerCtx:            handler,
		})
	}
	...
	screenshotSchema, screenshotRequired := overlayCoreToolSchema("screenshot", map[string]interface{}{
		"session_id": map[string]string{"type": "string", "description": "Session ID."},
	})
	reg("screenshot", "Capture a screenshot and send it to the user.",
		ToolCategoryBuiltin, []string{"session", "screenshot", "capture"},
		screenshotSchema, screenshotRequired,
	...
	// --- MCP tools ---
	listMCPSchema, listMCPRequired := overlayCoreToolSchema("list_mcp_tools", nil)
	reg("list_mcp_tools", "列出已注册的 MCP Server 及其工具",
		ToolCategoryBuiltin, []string{"mcp", "list", "tools"},
		listMCPSchema, listMCPRequired,
		func(args map[string]interface{}) string { return h.toolListMCPTools(args) })

	importMCPSchema, importMCPRequired := overlayCoreToolSchema("import_mcp_servers", map[string]interface{}{
	...
	reg("import_mcp_servers", "通过 JSON 配置导入 MCP Server。支持标准 {\"mcpServers\": {\"name\": {...}}}，也兼容 mcpServers/mcp_servers/mcpservers 缺外层大括号的片段；本地 stdio 使用 command/args/env，远程 HTTP 使用 url 或 endpoint_url/headers。",
		...
		importMCPSchema, importMCPRequired,
	...
	manageSkillSchema, manageSkillRequired := overlayCoreToolSchema("manage_skill", map[string]interface{}{
	...
	regCtxP("manage_skill", skill.ManageSkillDescription(),
		...
		manageSkillSchema, manageSkillRequired,
	...
	askUserSchema, askUserRequired := overlayCoreToolSchema("ask_user", nil)
	...
	recordAudioSchema, recordAudioRequired := overlayCoreToolSchema("record_audio", nil)
	reg("record_audio", "【仅桌面】打开交互式长时录音界面（波形+暂停/停止）并等待用户结束录音。IM 通道不支持会议/长时录音（平台语音过短）。用户当前消息已明确要求开始录音/会议录音时立即调用，不要二次确认、不要搜已有音频文件。仅当意图含糊时再澄清。用户停止后，下一条消息会带上音频路径与时长等摘要，再继续转写/纪要或投递音频文件。若生成会议纪要，必须同时 send_file 投递原始音频（可点击路径，便于用户备份）。",
		...
		recordAudioSchema, recordAudioRequired,
	...
	taskSchema, taskRequired := overlayCoreToolSchema("task", nil)
	...
	goalSchema, goalRequired := overlayCoreToolSchema("goal", map[string]interface{}{
	...
	delegateTaskSchema, delegateTaskRequired := overlayCoreToolSchema("delegate_task", map[string]interface{}{
	...
	reg("delegate_task", "将任务委派给专业子 Agent 处理。coding_workflow 会同步运行内部 CodingSubAgent 完成编码任务，不返回占位激活文本；help 用于使用帮助。",
		...
		delegateTaskSchema, delegateTaskRequired,
	...
	// --- Local machine tools ---
	bashSchema, bashRequired := overlayCoreToolSchema("bash", map[string]interface{}{
		"command":    map[string]interface{}{"type": "string", "description": "要执行的 shell 命令", "maxLength": maxAgentLoopInlineBashCommandRunes},
		"background": map[string]string{"type": "boolean", "description": "后台执行（可选，默认 false）。设为 true 时命令在后台运行，立即返回 task_id，用 async_wait 查询进度。"},
	})
	regCtxP("bash", "在本机直接执行 shell 命令（如创建目录、移动文件、运行脚本等）。不要用它连接数据库或把密码写进命令，数据库请用 database 工具。命令在 MaClaw 所在设备上执行，不需要会话。",
		ToolCategoryBuiltin, []string{"shell", "bash", "command", "execute"},
		bashSchema, bashRequired,
		func(ctx context.Context, args map[string]interface{}, onProgress tool.ProgressCallback) string {
	...
	readToolResultSchema, readToolResultRequired := overlayCoreToolSchema("read_tool_result", nil)
	reg("read_tool_result", "分段回读被截断的工具完整输出（使用 [tool_result_handle] 的 id）。大日志/网页/bash 输出被投影为预览后，用此工具按 offset/limit 读取细节。",
		...
		readToolResultSchema, readToolResultRequired,
	...
	writeFileSchema, writeFileRequired := overlayCoreToolSchema("write_file", map[string]interface{}{
	...
	editFileSchema, editFileRequired := overlayCoreToolSchema("edit_file", nil)
	reg("edit_file", "修改已有文件的首选工具（搜索替换模式，token 开销极小）。old_string 必须精确匹配文件原文（含缩进），建议包含修改点前后 1-2 行确保唯一匹配。修改已有文件时优先用此工具，不要用 write_file 重写整个文件。",
		...
		editFileSchema, editFileRequired,
	...
	editLinesSchema, editLinesRequired := overlayCoreToolSchema("edit_lines", map[string]interface{}{
	...
	reg("edit_lines", "按行号精确编辑文件（替换/插入/删除指定行）。比 edit_file 更精确——用行号定位，不怕重复内容。先用 read_file 查看行号，再用此工具编辑。",
		...
		editLinesSchema, editLinesRequired,
	...
	listDirectorySchema, listDirectoryRequired := overlayCoreToolSchema("list_directory", map[string]interface{}{
	...
	reg("list_directory", "列出本机目录内容",
		...
		listDirectorySchema, listDirectoryRequired,
	...
	globSchema, globRequired := overlayCoreToolSchema("Glob", nil)
	regCtxP("Glob", "按 glob 通配符查找本机文件路径，支持 ** 递归和 {html,md} 花括号。先发现相关文件，再用 read_file 查看内容。",
		ToolCategoryBuiltin, []string{"file", "glob", "search"},
		globSchema, globRequired,
	...
	sendFileSchema, sendFileRequired := overlayCoreToolSchema("send_file", map[string]interface{}{
	...
	reg("send_file", "读取本机文件并交付：桌面端默认展示在当前对话；已在微信/飞书通道中则发回本对话即等于发到该通道",
		...
		sendFileSchema, sendFileRequired,
	...
	sendToIMSchema, sendToIMRequired := overlayCoreToolSchema("send_to_im", map[string]interface{}{
		"phase_id": map[string]string{"type": "string", "description": workflowDocDeliveryPhaseIDSchemaDescription()},
		"doc_type": map[string]string{"type": "string", "description": workflowDocDeliveryTypeSchemaDescription()},
	})
	reg("send_to_im", "桌面端把文件发到绑定 IM；已在微信/飞书通道中时用 send_file 发回本对话即可，勿报发送器未配置",
		...
		sendToIMSchema, sendToIMRequired,
	...
	openSchema, openRequired := overlayCoreToolSchema("open", nil)
	...
	ttsRenderSchema, ttsRenderRequired := overlayCoreToolSchema("tts_render", nil)
	reg("tts_render", "将文本合成为语音产物。不播放、不发送，也不能指定通道或投递目标。",
		ToolCategoryBuiltin, []string{"tts", "speech", "语音产物"},
		ttsRenderSchema, ttsRenderRequired,
	...
	asrSchema, asrRequired := overlayCoreToolSchema("asr", map[string]interface{}{
	...
	webSearchSchema, webSearchRequired := overlayCoreToolSchema("web_search", nil)
	reg("web_search", "搜索互联网内容，查询天气、新闻、汇率、股价等实时信息。返回搜索结果列表（标题、URL、摘要）。适用于查找资料、技术文档、最新信息等。搜索 API 失效、超时、被拦，或结果不含查询中的目标站点（如 OpenReview）时，自动继续用真实浏览器兜底搜索 Google/Bing。",
		...
		webSearchSchema, webSearchRequired,
	...
	webFetchSchema, webFetchRequired := overlayCoreToolSchema("web_fetch", nil)
	reg("web_fetch", "抓取指定 URL 的网页内容并提取正文文本。支持自动编码检测（GBK/UTF-8 等）、HTML 正文提取。可选 JS 渲染（需本机安装 Chrome）。下载文件时务必传 save_path（相对路径落在当前工作目录）。通用 HTTP/PDF 下载优先用 download_file 或本工具+save_path，不要安装 ClawHub 的 wget/curl skill。抓取与下载都内置反爬升级：遇 Cloudflare/403 拦截自动逐级升级（浏览器请求头 → 模拟 Chrome TLS 指纹 → 复用浏览器会话 cookie，后者需先用 browser 工具打开过该网站）。下载过程日志见 ~/.maclaw/logs/download.log。长页面支持续读：当返回 has_more=true 时，请使用 offset=next_offset 继续读取后续内容。",
		...
		webFetchSchema, webFetchRequired,
	...
	downloadFileSchema, downloadFileRequired := overlayCoreToolSchema("download_file", nil)
	reg("download_file", "将 HTTP/HTTPS URL 下载到当前工作目录（顶栏 working_directory / 项目工作区）。通用文件与 PDF 下载的首选工具；返回绝对路径。不要为简单下载去 Hub 安装 wget/curl/Paper Fetch。内置反爬升级链：被 Cloudflare/403 拦截时自动逐级升级（浏览器请求头 → 模拟 Chrome TLS 指纹 → 复用浏览器会话 cookie）；网络错误与 429/5xx 自动退避重试。终极手段：via_browser=true 让浏览器亲自下载（真实内核 cookie/指纹/JS 环境，可过强反爬与内联 PDF）。下载过程日志见 ~/.maclaw/logs/download.log。",
		...
		downloadFileSchema, downloadFileRequired,
	...
	regCtxP("database", database.ToolDescription(),
		ToolCategoryBuiltin, []string{"database", "mysql", "postgres", "postgresql", "sqlserver", "access", "excel", "query", "data", "库", "schema", "表", "表结构", "查看"},
		database.ToolProperties(), []string{"action"}, func(ctx context.Context, args map[string]interface{}, _ tool.ProgressCallback) string {
			return h.toolDatabaseWithContext(ctx, args)
		})
	annotateSemanticTool(registry, "database", []tool.CapabilityProvision{{
		Capability: tool.CapabilityBusinessDataMIS, Quality: 1,
	}}, []tool.EffectClass{tool.EffectSensitive})

	// Read-only projection of the same handler. Restricted workflow states
	// deny business.data.mis, so a lookup must not have to be granted the
	// mutating capability. semanticManagedInvocationRefusal holds this
	// adapter to inspect/query/read_table.
	regCtxP("database_query", database.ToolDescriptionReadOnly(),
		ToolCategoryBuiltin, []string{"database", "sql", "query", "inspect", "readonly", "库", "schema", "表", "表结构", "查看"},
		database.ToolProperties(), []string{"action"}, func(ctx context.Context, args map[string]interface{}, _ tool.ProgressCallback) string {
	...
	annotateSemanticTool(registry, "database_query", []tool.CapabilityProvision{{
		Capability: tool.CapabilityBusinessDataRead, Quality: 1,
	}}, []tool.EffectClass{tool.EffectReadOnly})

	officeSchema, officeRequired := overlayCoreToolSchema("office", map[string]interface{}{
	...
	generatePDFSchema, generatePDFRequired := overlayCoreToolSchema("generate_pdf", map[string]interface{}{
		"phase_id": map[string]string{"type": "string", "description": workflowDocGeneratePDFPhaseIDSchemaDescription()},
	...
	sshSchema, sshRequired := overlayCoreToolSchema("ssh", map[string]interface{}{
...
func annotateSemanticTool(registry *ToolRegistry, name string, provisions []tool.CapabilityProvision, effects []tool.EffectClass) {
	...
	if tool.IsLegacyModelDynamicGateway(name) {
		return
	}
	if _, ok := tool.LegacyAdapterProvisionForTool(name, time.Now().UTC()); !ok {
		log.Printf("[legacy-adapter] registered %q without a reviewed provision; unmanaged turns will drop it", name)
	}
}

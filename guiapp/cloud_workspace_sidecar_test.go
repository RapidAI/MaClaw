...
func TestCloudWorkspaceSidecarFlushOnRelease(t *testing.T) {
	...
	if task.Name != "标书项目" || task.Tag != cloudWorkspaceTag("cws_sidecar") {
		t.Fatalf("task sidecar=%+v", task)
	}
	if task.Mode == taskRemoteCodingDevTag {
		t.Fatalf("sidecar must not store SSH remote mode: %+v", task)
	}
...
func TestMergeCloudWorkspaceTabSessionKeepsClearFence(t *testing.T) {
	dst := &TabSessionData{Conversation: []interface{}{map[string]any{"id": "new"}}, ConversationClearedAt: 200}
	src := &TabSessionData{Conversation: []interface{}{map[string]any{"id": "old"}}, ConversationClearedAt: 100}
	mergeCloudWorkspaceTabSession(dst, src, "tab-1", "/tmp/task")
	if dst.ConversationClearedAt != 200 {
		t.Fatalf("cleared_at=%d", dst.ConversationClearedAt)
	}
	if len(dst.Conversation) != 1 || dst.Conversation[0].(map[string]any)["id"] != "new" {
		t.Fatalf("conversation=%+v", dst.Conversation)
	}
}
...
func TestCloudWorkspaceSidecarRestoreOnCreateTask(t *testing.T) {
	...
	if projectRecordHasTagLike(created.Tags, taskCodingDevTag) || projectRecordHasTagLike(created.Tags, taskRemoteCodingDevTag) {
		t.Fatalf("cloud identity must not carry coding/remote tags: %v", created.Tags)
	}
	...
	history := app.LoadProjectConversationHistory(created.ProjectPath)
	if len(history) == 0 {
		t.Fatal("sidecar conversation must be visible before CreateProjectTabSession")
	}
	if history[0].Role != "assistant" || history[0].Content != "restored chat" {
		t.Fatalf("history before tab=%+v", history)
	}
	...
	if leftover, err := app.ensureProjectTabSessionPersist().LoadSession(cloudWorkspaceRestoreSessionTabID(created.ProjectPath)); err != nil {
		t.Fatal(err)
	} else if leftover != nil {
		t.Fatalf("pre-tab restore snapshot should be dropped after CreateProjectTabSession: %+v", leftover)
	}
}

func TestCloudWorkspaceSidecarRestoreSurvivesPendingReset(t *testing.T) {
	sessionRaw, _ := json.Marshal(cloudWorkspaceSessionSidecar{
		Conversation: []interface{}{map[string]any{"role": "user", "content": "open this on another machine"}},
	})
	hub := &fakeCloudWorkspaceHub{
		acquired: cloudWorkspaceAcquiredGranted,
		sidecars: map[string][]byte{cloudWorkspaceSidecarSession: sessionRaw},
	}
	app := newCloudWorkspaceMountTestApp(t, hub)
	created := mustCreateCloudWorkspaceTask(t, app, "跨设备任务", "", "", "cws_pending_reset")
	resetCloudWorkspaceSidecarState()

	notice := app.CreateProjectTabSession("tab-after-restart", created.ProjectPath)
	if notice == "" {
		t.Fatal("expected tab session")
	}
	session, err := app.ensureProjectTabSessionPersist().LoadSession("tab-after-restart")
	if err != nil || session == nil {
		t.Fatalf("load session err=%v session=%v", err, session)
	}
	raw, _ := json.Marshal(session.Conversation)
	if !strings.Contains(string(raw), "open this on another machine") {
		t.Fatalf("disk restore snapshot was not adopted after pending reset: %s", raw)
	}
}

func TestCreateProjectTabSessionAdoptsRestoreSnapshotOverEmptyTab(t *testing.T) {
	sessionRaw, _ := json.Marshal(cloudWorkspaceSessionSidecar{
		Conversation: []interface{}{map[string]any{"role": "assistant", "content": "sidecar turns"}},
	})
	hub := &fakeCloudWorkspaceHub{
		acquired: cloudWorkspaceAcquiredGranted,
		sidecars: map[string][]byte{cloudWorkspaceSidecarSession: sessionRaw},
	...
	app := newCloudWorkspaceMountTestApp(t, hub)
	created := mustCreateCloudWorkspaceTask(t, app, "跨设备任务", "", "", "cws_empty_tab")
	...
	resetCloudWorkspaceSidecarState()
	if history := app.LoadProjectConversationHistory(created.ProjectPath); len(history) == 0 || history[0].Content != "sidecar turns" {
		t.Fatalf("history behind empty tab=%+v", history)
	}

	if notice := app.CreateProjectTabSession("tab-empty", created.ProjectPath); notice == "" {
		t.Fatal("expected tab session")
	}
	session, err := persist.LoadSession("tab-empty")
	if err != nil || session == nil {
		t.Fatalf("load session err=%v session=%v", err, session)
	}
	raw, _ := json.Marshal(session.Conversation)
	if !strings.Contains(string(raw), "sidecar turns") {
		t.Fatalf("empty newer tab hid restore snapshot: %s", raw)
	}
}

func TestResumeCloudWorkspaceTaskRestoresConversationWithoutTab(t *testing.T) {
	hub := &fakeCloudWorkspaceHub{acquired: cloudWorkspaceAcquiredGranted}
	app := newCloudWorkspaceMountTestApp(t, hub)
	created := mustCreateCloudWorkspaceTask(t, app, "新建云端工作区任务", "", "", "cws_resume_chat")
	sessionRaw, _ := json.Marshal(cloudWorkspaceSessionSidecar{
		Conversation: []interface{}{
			map[string]any{"role": "user", "content": "北京天气怎么样"},
			map[string]any{"role": "assistant", "content": "今天晴，26度"},
		},
	})
	hub.mu.Lock()
	if hub.sidecars == nil {
		hub.sidecars = map[string][]byte{}
	}
	hub.sidecars[cloudWorkspaceSidecarSession] = sessionRaw
	hub.mu.Unlock()

	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	if err := app.releaseCloudWorkspace(ctx, "cws_resume_chat", false); err != nil {
		t.Fatalf("release: %v", err)
	}

	resumed := mustResumeCloudWorkspaceTask(t, app, "cws_resume_chat", created.ProjectPath)
	history := app.LoadProjectConversationHistory(resumed.ProjectPath)
	if len(history) != 2 {
		t.Fatalf("history len=%d want 2: %+v", len(history), history)
	}
	if history[0].Content != "北京天气怎么样" || history[1].Content != "今天晴，26度" {

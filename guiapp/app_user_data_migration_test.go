...
func TestUserDataMigrationRuntimeStatusVocabulary(t *testing.T) {
	cases := map[string]agentruntime.JobStatus{
		"queued":     agentruntime.JobStatusPending,
		"pending":    agentruntime.JobStatusPending,
		"running":    agentruntime.JobStatusRunning,
		"indexing":   agentruntime.JobStatusRunning,
		"completed":  agentruntime.JobStatusSucceeded,
		"succeeded":  agentruntime.JobStatusSucceeded,
		"failed":     agentruntime.JobStatusFailed,
		"cancelled":  agentruntime.JobStatusCanceled,
		"canceled":   agentruntime.JobStatusCanceled,
		"unexpected": agentruntime.JobStatusUnknown,
	}
	for input, want := range cases {
		if got := userDataMigrationRuntimeStatus(input); got != want {
			t.Errorf("userDataMigrationRuntimeStatus(%q)=%q, want %q", input, got, want)
		}
	}
}

func TestUserDataMigrationJobPublishesRuntimeStatus(t *testing.T) {
	clearUserDataMigrationJobsForTest()
	t.Cleanup(clearUserDataMigrationJobsForTest)
	...
	if job.RuntimeStatus != agentruntime.JobStatusRunning {
		t.Fatalf("initial runtime status=%q, want running", job.RuntimeStatus)
	}
	deadline := time.After(2 * time.Second)
	for {
		got, err := app.GetUserDataMigrationJob(job.ID)
		...
		if got.Status == "succeeded" {
			if got.RuntimeStatus != agentruntime.JobStatusSucceeded {
				t.Fatalf("terminal runtime status=%q, want succeeded", got.RuntimeStatus)
			}
			return
		}
		select {
		case <-deadline:
			t.Fatalf("job did not finish, status=%s runtime_status=%s", got.Status, got.RuntimeStatus)
		case <-time.After(10 * time.Millisecond):
		}
	}
}

func TestUserDataMigrationJobReporterUsesSharedProgressContract(t *testing.T) {
	clearUserDataMigrationJobsForTest()
	t.Cleanup(clearUserDataMigrationJobsForTest)

	now := time.Now().UTC()
	userDataMigrationJobs.Store("mig-reporter", userDataMigrationJob{ID: "mig-reporter", Status: "running", RuntimeStatus: agentruntime.JobStatusRunning, CreatedAt: now, UpdatedAt: now})
	ctx := agentruntime.WithJobReporter(context.Background(), userDataMigrationJobReporter{jobID: "mig-reporter"})
	if err := agentruntime.ReportJobProgress(ctx, 0.4, "uploading"); err != nil {
		t.Fatalf("ReportJobProgress: %v", err)
	}
	job, err := (&App{}).GetUserDataMigrationJob("mig-reporter")
	if err != nil {
		t.Fatalf("GetUserDataMigrationJob: %v", err)
	}
	if job.Progress != 0.4 || job.ProgressText != "uploading" || job.RuntimeStatus != agentruntime.JobStatusRunning {
		t.Fatalf("job progress projection = %#v", job)
	}
	runtimeJob, err := userDataMigrationRuntimeJobs.Get(context.Background(), "mig-reporter")
	if err != nil {
		t.Fatalf("runtime repository job: %v", err)
	}
	if runtimeJob.Status != agentruntime.JobStatusRunning || runtimeJob.Progress != 0.4 {
		t.Fatalf("runtime repository projection = %#v", runtimeJob)
	}
}

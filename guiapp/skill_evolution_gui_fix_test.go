...
func TestExecutePipelineAsyncFailureNotifiesEvolutionPipeline(t *testing.T) {
	...
	app := setupEvolutionTestApp(t, []corelib.NLSkillEntry{pipeEntry}, func(cfg *corelib.AppConfig) {
		// Pipeline notification is an automatic-evolution path and therefore
		// requires the same explicit opt-in as production.
...
func TestUpdateUsageStatsRespectsEvolutionSwitch(t *testing.T) {
	...
	t.Run("explicit enable permits the automatic path", func(t *testing.T) {
		...
		if !runner.skillEvolutionEnabled() {
			t.Fatal("explicit skill_evolution_enabled=true must permit automatic evolution")
		}
	})
...
func TestSkillRunnerUsageStatsBlocksOnUnreadableCompensationQueue(t *testing.T) {
	app := setupEvolutionTestApp(t, []corelib.NLSkillEntry{{
		Name: "runner-usage-guard", Status: "active", Source: "manual",
		Steps: []corelib.NLSkillStep{{Action: "noop"}},
	}}, nil)
	runner := NewSkillRunner(app.skillExecutor)
	queuePath := cskill.DefaultEvolutionCompensationPath()
	original, readErr := os.ReadFile(queuePath)
	originalExists := readErr == nil
	t.Cleanup(func() {
		if originalExists {
			_ = os.WriteFile(queuePath, original, 0o600)
		} else {
	...
	updated := runner.updateUsageStats(&corelib.NLSkillEntry{Name: "runner-usage-guard"}, nil)
	if updated != nil {
		t.Fatalf("updateUsageStats() = %#v, want nil when queue is unreadable", updated)
	}
	got := findNLSkillDefinitionForTest(app.ListNLSkills(), "runner-usage-guard")

...
// TestApplySkillRepairDraftCleanupFailurePersistsAndRecovers verifies the
// post-commit boundary: a draft unlink failure must return committed/pending,
// keep the applied YAML and durable queue row, and allow a later startup-style
// cleanup pass to remove only the draft before clearing the queue.
func TestApplySkillRepairDraftCleanupFailurePersistsAndRecovers(t *testing.T) {
	entry, draftName := writeFileBackedSkillWithDraft(t, t.TempDir())
	app := setupEvolutionTestApp(t, []corelib.NLSkillEntry{entry}, nil)

	origRemove := removeReviewedRepairDraft
	removeReviewedRepairDraft = func(string) error { return errors.New("injected draft cleanup failure") }
	t.Cleanup(func() { removeReviewedRepairDraft = origRemove })

	parsed := applyDraftResult(t, app, "draft-skill", draftName)
	if ok, _ := parsed["ok"].(bool); ok {
		t.Fatalf("apply unexpectedly reported success despite cleanup failure: %v", parsed)
	}
	if state, _ := parsed["state"].(string); state != "committed" {
		t.Fatalf("state = %q, want committed: %v", state, parsed)
	}
	if cleanup, _ := parsed["cleanup_status"].(string); cleanup != "pending" {
		t.Fatalf("cleanup_status = %q, want pending: %v", cleanup, parsed)
	}
	yamlData, err := os.ReadFile(filepath.Join(entry.SkillDir, "skill.yaml"))
	if err != nil {
		t.Fatal(err)
	...
	if !strings.Contains(string(yamlData), "echo new") {
		t.Fatalf("committed YAML was unexpectedly rolled back: %s", yamlData)
	}
	draftPath := filepath.Join(entry.SkillDir, cskill.RepairDraftsDirName, draftName)
	if _, err := os.Stat(draftPath); err != nil {
		t.Fatalf("draft should remain for durable cleanup recovery: %v", err)
	}
	summaries, err := cskill.ListEvolutionCompensationSummaries()
	if err != nil {
		t.Fatal(err)
	}
	var found bool
	for _, summary := range summaries {
		if summary.Skill == entry.Name && summary.Action == "repair" {
			found = true
			if summary.TransactionState != "committed" || summary.CleanupStatus != "pending" {
				t.Fatalf("unexpected committed cleanup summary: %+v", summary)
			}
		}
	}
	if !found {
		t.Fatalf("missing durable repair cleanup record: %+v", summaries)
	}

	removeReviewedRepairDraft = origRemove
	recovered, pending, err := cskill.RecoverPendingEvolutionCompensationsForActionPrefixAndSkill("repair", entry.Name, nil, nil)
	if err != nil {
		t.Fatalf("recover committed repair cleanup: %v", err)
	}
	if recovered != 1 || pending != 0 {
		t.Fatalf("recovery result recovered=%d pending=%d, want 1/0", recovered, pending)
	}
	if _, err := os.Stat(draftPath); !os.IsNotExist(err) {
		t.Fatalf("draft remains after durable cleanup recovery: %v", err)
	}
}
...
// TestApplySkillRepairDraftDisableCleanupFailurePersistsAndRecovers verifies
// that the config-only disable variant crosses the committed boundary before
// draft cleanup. A failed unlink must keep the disabled overlay and durable
// cleanup blocker, then become recoverable after a restart-style pass.
func TestApplySkillRepairDraftDisableCleanupFailurePersistsAndRecovers(t *testing.T) {
	entry, draftName := writeDisableDraftSkill(t, t.TempDir(), "disable-cleanup", "cleanup later")
	app := setupEvolutionTestApp(t, []corelib.NLSkillEntry{entry}, nil)

	origRemove := removeReviewedRepairDraft
	removeReviewedRepairDraft = func(string) error { return errors.New("injected disable draft cleanup failure") }
	t.Cleanup(func() { removeReviewedRepairDraft = origRemove })

	parsed := applyDraftResult(t, app, entry.Name, draftName)
	if ok, _ := parsed["ok"].(bool); ok {
		t.Fatalf("disable apply unexpectedly reported success despite cleanup failure: %v", parsed)
	}
	if state, _ := parsed["state"].(string); state != "committed" {
		t.Fatalf("state = %q, want committed: %v", state, parsed)
	}
	...
	if overlay := findOverlay(t, app, entry.Name); overlay == nil || overlay.Status != "disabled" {
		t.Fatalf("disabled overlay was not committed: %+v", overlay)
	}
	draftPath := filepath.Join(entry.SkillDir, cskill.RepairDraftsDirName, draftName)
	if _, err := os.Stat(draftPath); err != nil {
		t.Fatalf("draft should remain for cleanup recovery: %v", err)
	}

	removeReviewedRepairDraft = origRemove
	recovered, pending, err := cskill.RecoverPendingEvolutionCompensationsForActionPrefixAndSkill("disable", entry.Name, nil, nil)
	if err != nil {
		t.Fatalf("recover committed disable cleanup: %v", err)
	}
	if recovered != 1 || pending != 0 {
		t.Fatalf("recovery result recovered=%d pending=%d, want 1/0", recovered, pending)
	}
	if _, err := os.Stat(draftPath); !os.IsNotExist(err) {
		t.Fatalf("draft remains after disable cleanup recovery: %v", err)
	}
}

// TestRejectSkillRepairDraftCleanupFailurePersistsAndRecovers covers the
// rejection path independently from apply/disable. Rejection still commits
// its attempt/history overlay before cleanup, so a failed unlink must not
// roll that accounting back or lose the draft needed by recovery.
func TestRejectSkillRepairDraftCleanupFailurePersistsAndRecovers(t *testing.T) {
	entry, draftName := writeDisableDraftSkill(t, t.TempDir(), "reject-cleanup", "cleanup later")
	app := setupEvolutionTestApp(t, []corelib.NLSkillEntry{entry}, nil)

	origRemove := removeReviewedRepairDraft
	removeReviewedRepairDraft = func(string) error { return errors.New("injected reject draft cleanup failure") }
	t.Cleanup(func() { removeReviewedRepairDraft = origRemove })

	var parsed map[string]interface{}
	if err := json.Unmarshal([]byte(app.RejectSkillRepairDraft(entry.Name, draftName)), &parsed); err != nil {
		t.Fatalf("invalid JSON response: %v", err)
	}
	if ok, _ := parsed["ok"].(bool); ok {
		t.Fatalf("reject unexpectedly reported success despite cleanup failure: %v", parsed)
	}
	if state, _ := parsed["state"].(string); state != "committed" {
		t.Fatalf("state = %q, want committed: %v", state, parsed)
	}
	...
	overlay := findOverlay(t, app, entry.Name)
	if overlay == nil || overlay.RepairAttemptCount != 1 || len(overlay.RepairHistory) != 1 {
		t.Fatalf("rejection accounting was not committed: %+v", overlay)
	}
	draftPath := filepath.Join(entry.SkillDir, cskill.RepairDraftsDirName, draftName)
	if _, err := os.Stat(draftPath); err != nil {
		t.Fatalf("draft should remain for rejection cleanup recovery: %v", err)
	}

	removeReviewedRepairDraft = origRemove
	recovered, pending, err := cskill.RecoverPendingEvolutionCompensationsForActionPrefixAndSkill("reject", entry.Name, nil, nil)
	if err != nil {
		t.Fatalf("recover committed reject cleanup: %v", err)
	}
	if recovered != 1 || pending != 0 {
		t.Fatalf("recovery result recovered=%d pending=%d, want 1/0", recovered, pending)
	}
	if _, err := os.Stat(draftPath); !os.IsNotExist(err) {
		t.Fatalf("draft remains after reject cleanup recovery: %v", err)
	}
}

...
func TestAutomaticCraftSkillMutationDefaultsToNoWrite(t *testing.T) {
	tempHome := t.TempDir()
	t.Setenv("HOME", tempHome)
	t.Setenv("USERPROFILE", tempHome)
	t.Setenv("AppData", filepath.Join(tempHome, "AppData", "Roaming"))
	...
	if err := automaticCraftSkillMutationAllowed(app); err == nil || !strings.Contains(err.Error(), "automatic crafted skill mutation is disabled") {
		t.Fatalf("automaticCraftSkillMutationAllowed() error = %v, want disabled automatic mutation", err)
	}

	scriptPath := filepath.Join(tempHome, "crafted.sh")
	if err := os.WriteFile(scriptPath, []byte("echo hi"), 0o644); err != nil {
		t.Fatalf("write script: %v", err)
	}
	result := buildCraftSuccessResult(app, craftToolRequest{
		SaveAsSkill:        true,
		ShouldAutoRegister: true,
		OriginalTask:       "collect local logs",
	}, craftAttemptResult{
		ScriptPath:          scriptPath,
		Language:            "bash",
		Attempts:            1,
		VerificationStatus:  craftVerificationPassed,
		VerificationMessage: "verified",
	}, func(string) {})
	if !strings.Contains(result, "未自动保存为 Skill") {
		t.Fatalf("success result = %q, want automatic-registration refusal", result)
	}
	if skills := app.skillExecutor.loadSkills(); len(skills) != 0 {
		t.Fatalf("disabled craft mutation registered skills: %#v", skills)
	}
}

func TestAutomaticCraftSkillMutationRequiresConfigAndHonorsKillSwitch(t *testing.T) {
	tempHome := t.TempDir()
	t.Setenv("HOME", tempHome)
	t.Setenv("USERPROFILE", tempHome)
	t.Setenv("AppData", filepath.Join(tempHome, "AppData", "Roaming"))
	...
	if err := app.PatchConfig(func(cfg *corelib.AppConfig) {
		...
		t.Fatalf("enable automatic craft mutation: %v", err)
	}
	if err := automaticCraftSkillMutationAllowed(app); err != nil {
		t.Fatalf("enabled automaticCraftSkillMutationAllowed() error = %v", err)
	}

	t.Setenv("MACLAW_DISABLE_SKILL_EVOLUTION", "1")
	if err := automaticCraftSkillMutationAllowed(app); err == nil || !strings.Contains(err.Error(), "disabled by environment") {
		t.Fatalf("kill-switched automaticCraftSkillMutationAllowed() error = %v, want environment rejection", err)
	}
}

func TestBuildCraftSuccessResultRegistersOneDurableSkillWhenEnabled(t *testing.T) {
	tempHome := t.TempDir()
	t.Setenv("HOME", tempHome)
	t.Setenv("USERPROFILE", tempHome)
	t.Setenv("AppData", filepath.Join(tempHome, "AppData", "Roaming"))
	...
	result := buildCraftSuccessResult(app, craftToolRequest{
		SaveAsSkill:        true,
		ShouldAutoRegister: true,
		OriginalTask:       "collect reusable local logs",
		SkillName:          "crafted-logs",
	}, craftAttemptResult{
		ScriptPath:         scriptPath,
		Language:           "bash",
		Attempts:           1,
	...
	if !strings.Contains(result, "已注册为 Skill") {
		t.Fatalf("success result = %q, want durable registration", result)
	}
	cfg, err := app.LoadConfig()
	if err != nil {
		t.Fatalf("load config: %v", err)
	}
	if len(cfg.NLSkills) != 1 || cfg.NLSkills[0].Name != "crafted-logs" || cfg.NLSkills[0].Source != "crafted" {
		t.Fatalf("crafted registration should write one durable definition: %#v", cfg.NLSkills)
	}
	if _, err := os.Stat(filepath.Join(app.GetDataDir(), "skills", cskill.CraftedSkillsSubdir)); !os.IsNotExist(err) {
		t.Fatalf("automatic registration should not create a second uncommitted crafted directory: %v", err)
	}
}
...
func TestRegisterCraftedSkillEntryRollsBackIndexFailure(t *testing.T) {
	tempHome := t.TempDir()
	t.Setenv("HOME", tempHome)
	t.Setenv("USERPROFILE", tempHome)
	t.Setenv("AppData", filepath.Join(tempHome, "AppData", "Roaming"))
	...
	app.toolRouter.refreshSkillIndexOverride = func() error {
		refreshCalls++
		if refreshCalls == 1 {
			return fmt.Errorf("injected index provider failure")
		}
		return nil
	}
	scriptPath := filepath.Join(tempHome, "rollback.sh")
	if err := os.WriteFile(scriptPath, []byte("echo hi"), 0o644); err != nil {
		t.Fatalf("write script: %v", err)
	}

	result := registerCraftedSkillEntry(app, "collect rollback logs", "crafted-rollback", scriptPath, "bash")
	if !strings.Contains(result, "注册失败") || !strings.Contains(result, "not committed") {
		t.Fatalf("register result = %q, want durable commit failure", result)
	}
	cfg, err := app.LoadConfig()
	if err != nil {
	...
	if len(cfg.NLSkills) != 0 {
		t.Fatalf("failed crafted registration remained in config: %#v", cfg.NLSkills)
	}
	if refreshCalls < 2 {

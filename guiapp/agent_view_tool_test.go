...
func TestBuildRegisteredToolApprovalAgentViewCarriesApprovalID(t *testing.T) {
	view := buildRegisteredToolApprovalAgentView(registeredToolPendingApproval{
		ID:             "approval-1",
		ToolName:       "bash",
		ExecutionToken: "opaque-token-must-not-render",
	...
	if strings.Contains(fmt.Sprintf("%#v", view), "opaque-token-must-not-render") {
		t.Fatal("approval execution token leaked into AgentView payload")
	}
}

func TestRegisteredToolApprovalGetsOpaqueExecutionToken(t *testing.T) {
	first := storeRegisteredToolPendingApproval("database", map[string]interface{}{"action": "execute"}, "sess", "owner", security.RiskAssessment{Level: security.RiskHigh})
	second := storeRegisteredToolPendingApproval("database", map[string]interface{}{"action": "execute"}, "sess", "owner", security.RiskAssessment{Level: security.RiskHigh})
	defer deleteRegisteredToolPendingApproval(first.ID)
	defer deleteRegisteredToolPendingApproval(second.ID)
	if first.ExecutionToken == "" || second.ExecutionToken == "" || first.ExecutionToken == second.ExecutionToken {
		t.Fatalf("approval execution tokens must be unique and non-empty: first=%q second=%q", first.ExecutionToken, second.ExecutionToken)
	}
	view := buildRegisteredToolApprovalAgentView(first)
	if strings.Contains(fmt.Sprintf("%#v", view), first.ExecutionToken) {
		t.Fatal("opaque execution token leaked into approval view")
	}
}
...
func TestBuildDatabaseResultAgentViewExportUsesHandle(t *testing.T) {
	tool := RegisteredTool{Name: "database"}
	view := buildRegisteredToolResultAgentView(tool, `{"columns":[{"name":"n"}],"rows":[["a"]],"row_count":1,"result_handle":"db-result-1","connection_id":"conn-1","truncated":true,"next_cursor":"db-result-1"}`)
	results, _ := view["results"].([]map[string]interface{})
	if len(results) != 1 {
		t.Fatalf("results = %#v", view["results"])
	}
	actions, _ := results[0]["actions"].([]map[string]interface{})
	var export map[string]interface{}
	for _, action := range actions {
		if action["label"] == "Export Excel" || action["label"] == "导出 Excel" {
			export = action
		}
	}
	if export == nil {
		t.Fatalf("missing export action: %#v", actions)
	}
	data, _ := export["data"].(map[string]interface{})
	args, _ := data[registeredToolAgentViewArgsField].(map[string]interface{})
	if fmt.Sprint(args["result_handle"]) != "db-result-1" {
		t.Fatalf("export args = %#v", args)
	}
	if _, ok := args["rows"]; ok {
		t.Fatalf("export with handle must not embed rows: %#v", args)
	}
}

func TestBuildDatabaseResultAgentViewFavorites(t *testing.T) {
	tool := RegisteredTool{Name: "database_query"}
	view := buildRegisteredToolResultAgentView(tool, `[{"id":"db-fav-1","name":"orders","sql":"SELECT 1","profile_id":"crm"}]`)
	if fmt.Sprint(view["title"]) != "Saved queries" && fmt.Sprint(view["title"]) != "查询收藏" {
	...
	actions, _ := results[0]["actions"].([]map[string]interface{})
	if len(actions) < 2 {
		t.Fatalf("expected run+delete, got %#v", actions)
	}
}

func TestBuildDatabaseResultAgentViewListsProfiles(t *testing.T) {
	tool := RegisteredTool{Name: "database"}
	view := buildRegisteredToolResultAgentView(tool, `{"count":1,"items":[{"id":"crm","name":"CRM","type":"mysql","host":"192.168.1.242","database":"crm","status":"configured"}]}`)
	if fmt.Sprint(view["title"]) != "Data sources" && fmt.Sprint(view["title"]) != "数据源" {
		t.Fatalf("title = %#v", view["title"])
	}
	results, _ := view["results"].([]map[string]interface{})
	if len(results) != 1 {
...
func TestBuildDatabaseResultAgentViewProposeProfileForm(t *testing.T) {
	tool := RegisteredTool{Name: "database"}
	view := buildRegisteredToolResultAgentView(tool, `{"needs_secret":true,"draft":{"id":"mysql-192-168-1-242","host":"192.168.1.242","username":"root","type":"mysql","port":0},"guidance":"password stays in the keyring"}`)
	if fmt.Sprint(view["type"]) != "form" {
		t.Fatalf("type = %#v", view["type"])
	}
	...
	if password == nil || password["sensitive"] != true || fmt.Sprint(password["format"]) != "password" {
		t.Fatalf("password field must be sensitive with format=password, got %#v", password)
	}
	if fmt.Sprint(password["type"]) != agentViewFieldTypeText.String() {
		t.Fatalf("password type = %#v, want text so the task panel can mask it", password["type"])
	}
	if fmt.Sprint(byName["port"]["value"]) != "3306" {
		t.Fatalf("zero port should default to 3306: %#v", byName["port"])
	}
	if fmt.Sprint(byName["database"]["value"]) != "mysql" {
		t.Fatalf("empty database should default to mysql: %#v", byName["database"])
	}
	if fmt.Sprint(view["title"]) != "Add data source" && fmt.Sprint(view["title"]) != "添加数据源" {
		t.Fatalf("new-profile title = %#v", view["title"])
	}
	if fmt.Sprint(byName["type"]["type"]) != agentViewFieldTypeSelect.String() {
		t.Fatalf("type field = %#v", byName["type"])
	}
	if opts := fmt.Sprint(byName["type"]["options"]); strings.Contains(opts, "excel") || strings.Contains(opts, "access") {
		t.Fatalf("server form must not offer file sources without file_path: %s", opts)
	}
	hidden := byName[registeredToolAgentViewArgsField]
	if hidden == nil || fmt.Sprint(hidden["type"]) != agentViewFieldTypeHidden.String() {
		t.Fatalf("missing hidden tool args: %#v", fields)
	}
	hiddenArgs, _ := hidden["value"].(map[string]interface{})
	if fmt.Sprint(hiddenArgs["action"]) != "propose_profile" {
		t.Fatalf("hidden args action = %#v", hiddenArgs)
	}
	if _, ok := hiddenArgs["password"]; ok {
		t.Fatalf("hidden args must not carry password: %#v", hiddenArgs)
	}
}

func TestDatabaseResultNeedsHostSecretForm(t *testing.T) {
	tests := []struct {
		name   string
		...
		{"connect missing profile", map[string]interface{}{"action": "connect", "host": "192.168.1.242"}, `{"ok":false,"error":"profile_not_found","items":[]}`, true},
		{"connect success", map[string]interface{}{"action": "connect", "host": "192.168.1.242"}, `{"ok":true,"profile_id":"crm","connection_id":"c1"}`, false},
		{"connect without host", map[string]interface{}{"action": "connect"}, `{"ok":false,"error":"profile_not_found"}`, false},
		{"ambiguous host", map[string]interface{}{"action": "connect", "host": "192.168.1.242"}, `{"ok":false,"error":"ambiguous_profile","items":[{"id":"a"},{"id":"b"}]}`, false},
		{"list empty with host", map[string]interface{}{"action": "list_connections", "host": "192.168.1.242"}, `{"items":[],"count":0}`, true},
		{"list empty without host", map[string]interface{}{"action": "list_connections"}, `{"items":[],"count":0}`, false},
		{"list hits", map[string]interface{}{"action": "list_connections", "host": "192.168.1.242"}, `{"items":[{"id":"crm"}],"count":1}`, false},
		{"query must not pop form", map[string]interface{}{"action": "query", "host": "192.168.1.242"}, `{"error":"profile_not_found"}`, false},
	}
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			got := databaseResultNeedsHostSecretForm(tc.args, tc.result)
			if got != tc.want {
...
func TestDatabaseProposeDraftFromConnectArgsPrefillsID(t *testing.T) {
	draft := databaseProposeDraftFromArgs(map[string]interface{}{
		"action": "connect", "host": "192.168.1.242", "username": "root", "port": 3306,
	})
	if fmt.Sprint(draft["profile_id"]) != "mysql-192-168-1-242" {
		t.Fatalf("profile_id = %#v", draft["profile_id"])
	}
	if fmt.Sprint(draft["name"]) != "192.168.1.242" {
		t.Fatalf("name = %#v", draft["name"])
	}
	view := buildDatabaseProposeProfileAgentView(draft)
	fields, _ := view["fields"].([]map[string]interface{})
	byName := map[string]map[string]interface{}{}
	for _, field := range fields {
		byName[fmt.Sprint(field["name"])] = field
	}
	if fmt.Sprint(byName["profile_id"]["value"]) != "mysql-192-168-1-242" {
		t.Fatalf("form profile_id = %#v", byName["profile_id"])
	}
	if fmt.Sprint(byName["port"]["value"]) != "3306" {
		t.Fatalf("form port = %#v", byName["port"])
	}
	if fmt.Sprint(byName["database"]["value"]) != "mysql" {
		t.Fatalf("form database = %#v", byName["database"])
	}
}

func TestBuildDatabaseProposeProfileAgentViewDefaultsCatalogAndPort(t *testing.T) {
	view := buildDatabaseProposeProfileAgentView(map[string]interface{}{
		"host": "192.168.1.242", "username": "root",
	})
	fields, _ := view["fields"].([]map[string]interface{})
	byName := map[string]map[string]interface{}{}
	for _, field := range fields {
	...
	if fmt.Sprint(byName["profile_id"]["value"]) != "mysql-192-168-1-242" {
		t.Fatalf("profile_id = %#v", byName["profile_id"])
	}
	if fmt.Sprint(byName["database"]["value"]) != "mysql" {
		t.Fatalf("default database = %#v", byName["database"])
	}
	if fmt.Sprint(byName["port"]["value"]) != "3306" {
		t.Fatalf("default port = %#v", byName["port"])
	}
	pg := databaseProposeDraftFromArgs(map[string]interface{}{"host": "db.internal", "type": "postgres"})
	if fmt.Sprint(pg["database"]) != "postgres" || fmt.Sprint(pg["port"]) != "5432" {
		t.Fatalf("postgres defaults = %#v", pg)
	}
}

func TestBuildDatabaseProposeProfileAgentViewUpdateSecretTitle(t *testing.T) {
	view := buildDatabaseProposeProfileAgentView(map[string]interface{}{
		"id": "mysql-192-168-1-242", "host": "192.168.1.242", "username": "root", "update_secret": true,
	})
	title := fmt.Sprint(view["title"])
	if title != "Update password" && title != "更新数据源密码" {
		t.Fatalf("retry title = %#v", view["title"])
	}
}

func TestRedactAgentViewSubmitSecrets(t *testing.T) {
	got := redactAgentViewSubmitSecrets(map[string]interface{}{
		"password":   "form-secret",
		"host":       "192.168.1.242",
		"secret_ref": "keyring://maclaw/database/x",
		"_tool_args": map[string]interface{}{"action": "propose_profile", "password": "nested"},
	})
	if got["password"] != "***" {
		t.Fatalf("password = %#v", got["password"])
	}
	if got["host"] != "192.168.1.242" {
		t.Fatalf("host was redacted: %#v", got["host"])
	}
	if got["secret_ref"] != "keyring://maclaw/database/x" {
		t.Fatalf("secret_ref should stay: %#v", got["secret_ref"])
	}
	nested, _ := got["_tool_args"].(map[string]interface{})
	if nested["password"] != "***" || nested["action"] != "propose_profile" {
		t.Fatalf("nested = %#v", nested)
	}
}

func TestBuildDatabaseResultAgentViewExplainGuidance(t *testing.T) {
	tool := RegisteredTool{Name: "database"}
	view := buildRegisteredToolResultAgentView(tool, `{"guidance":"use :name placeholders","prompt":"查订单","dialect":"postgres"}`)
	results, _ := view["results"].([]map[string]interface{})
	if len(results) != 1 {
		t.Fatalf("results = %#v", view["results"])
	}
	subtitle, _ := results[0]["subtitle"].(string)
	if !strings.Contains(subtitle, ":name") {

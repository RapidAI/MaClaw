...
func TestResponsesAPIStreamConvertsLineOrientedGlobAndSuppressesTokens(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Path != "/v1/responses" {
			t.Fatalf("path = %q, want /v1/responses", r.URL.Path)
		}
		...
		w.WriteHeader(http.StatusOK)
		_, _ = fmt.Fprint(w, "event: response.output_text.delta\n")
		_, _ = fmt.Fprint(w, `data: {"delta":"正在定位第16页源文件并修复。\n terc3 glob_"}`+"\n")
		_, _ = fmt.Fprint(w, "event: response.output_text.delta\n")
		_, _ = fmt.Fprint(w, `data: {"delta":"file_search_tool\npath\nC:\\tmp\\book\nglob_pattern\n**/*.{html,md,json}<|eos|>"}`+"\n")
		_, _ = fmt.Fprint(w, "event: response.completed\n")
		_, _ = fmt.Fprint(w, `data: {"response":{"status":"completed"}}`+"\n")
	}))
	defer srv.Close()
	...
	resp, err := (&IMMessageHandler{}).doResponsesAPILLMRequestStream(
		context.Background(),
		cfg,
		[]interface{}{map[string]interface{}{"role": "user", "content": "公式渲染不对"}},
		nil,
		srv.Client(),
		func(delta string) { streamed.WriteString(delta) },
		nil,
	...
	if got := streamed.String(); strings.Contains(got, "glob_file_search_tool") || strings.Contains(got, "terc3") || strings.Contains(got, "<|eos|>") {
		t.Fatalf("stream leaked line-oriented glob call: %q", got)
	}
	if !strings.Contains(streamed.String(), "正在定位第16页源文件并修复。") {
		t.Fatalf("prose before leaked tool was dropped: %q", streamed.String())
	}
	if len(resp.Choices) != 1 {
		t.Fatalf("choices len = %d, want 1", len(resp.Choices))
	}
	...
	if len(choice.Message.ToolCalls) != 1 {
		t.Fatalf("tool_calls len = %d, want 1; content=%q", len(choice.Message.ToolCalls), choice.Message.Content)
	}
	call := choice.Message.ToolCalls[0]
	if call.Function.Name != "glob_file_search_tool" {
		t.Fatalf("tool name = %q", call.Function.Name)
	}
	if !strings.Contains(call.Function.Arguments, `"path":"C:\\tmp\\book"`) {
		t.Fatalf("path missing: %s", call.Function.Arguments)
	}
	if !strings.Contains(call.Function.Arguments, `"glob_pattern":"**/*.{html,md,json}"`) {
		t.Fatalf("glob_pattern missing: %s", call.Function.Arguments)
	}
}

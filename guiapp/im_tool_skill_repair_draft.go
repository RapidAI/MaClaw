...
// removeReviewedRepairDraft is indirected so cleanup failures can be tested
// without changing the durable transaction code or relying on platform-specific
// file-lock behavior. Production uses the ordinary unlink operation.
var removeReviewedRepairDraft = func(path string) error {
...
func (h *IMMessageHandler) commitReviewedDraftRejection(entry *corelib.NLSkillEntry, draft cskill.RepairDraft, draftErr error, draftPath string, draftBytes []byte, auditData map[string]string) string {
	...
	committer := &cskill.SkillCommitter{
		...
		PostCommitCleanup: func() error {
			err := removeReviewedRepairDraft(draftPath)
	...
	commitCtx := cskill.WithEvolutionRequestMetadata(context.Background(), auditData["request_id"], 1)
	// Serialize the final config/YAML/index publication with every other App
	// owned Skill mutation. Draft parsing and validation stay outside the lock;
	// only the durable commit boundary is serialized so a stale full-list
	// snapshot cannot overwrite a concurrent install/delete/status update.
	result := func() cskill.EvolutionCommitResult {
		h.app.installMutex.Lock()
		defer h.app.installMutex.Unlock()
		return committer.Commit(commitCtx, entry.Name, &candidate, cskill.EventSkillRepairDraftReady, auditData)
	}()
...
func (h *IMMessageHandler) toolApplySkillRepairDraft(args map[string]interface{}) string {
	...
	if draft.Disable {
		...
		result := func() cskill.EvolutionCommitResult {
			h.app.installMutex.Lock()
			defer h.app.installMutex.Unlock()
			return committer.Commit(commitCtx, entry.Name, &candidate, cskill.EventSkillRepaired, auditData)
		}()
		if result.State != "committed" || result.CleanupStatus != "clear" {
	...
	result := func() cskill.EvolutionCommitResult {
		h.app.installMutex.Lock()
		defer h.app.installMutex.Unlock()
		return committer.Commit(commitCtx, entry.Name, entry, cskill.EventSkillRepaired, auditData)
	}()
	if result.State != "committed" || result.CleanupStatus != "clear" {
		base["state"] = result.State
		base["cleanup_status"] = result.CleanupStatus
		...
		if strings.Contains(result.FailureReason, "persistence_failed") ||
			strings.Contains(result.FailureReason, "config_write_failed") {

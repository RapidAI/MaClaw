...
func (b *semanticArtifactBroker) consumeTrustedInput(consumer tool.PlannedSelection, contract tool.ArtifactContract) (tool.ArtifactPayload, error) {
	...
	dependency, err := tool.UniqueBoundArtifactDependency(consumer.ArtifactDependencies, contract)
	if err != nil {
		return tool.ArtifactPayload{}, err
	}
	if err := tool.ValidateBoundArtifactDependency(dependency, contract); err != nil {
		return tool.ArtifactPayload{}, err
	}
...
// Unique matching, bound validation, and family-newest producer selection
// live in corelib/tool so a host cannot pick a different artifact than the
// planner. The consumer still receives exactly one opaque grant.
func (b *semanticArtifactBroker) consumePlannedArtifact(consumer tool.PlannedSelection, contract tool.ArtifactContract) (tool.ArtifactPayload, error) {
	...
	dependency, err := tool.UniqueMatchingArtifactDependency(consumer.ArtifactDependencies, contract)
	if err != nil {
		return tool.ArtifactPayload{}, err
	}
	...
	source, ok := tool.NewestFamilyProducerArtifact(refs, dependency.ProducerSelection, contract)
	if !ok {

...
export interface HubCreditGrantLike {
    ...
    period_limits?: { five_hour?: unknown; daily?: unknown; weekly?: unknown; monthly?: unknown };
    PeriodLimits?: { FiveHour?: unknown; Daily?: unknown; Weekly?: unknown; Monthly?: unknown };
}

/**
 * Merge grant projections returned by older and newer Hub endpoints.
 * A grant can appear in either `credit_grants` or `active_grants`; choosing
 * one array based on length hides welcome benefits when a paid card exists.
 * De-duplicate by stable ID, with a field-based fallback for legacy payloads.
 */
export function mergeHubCreditGrants<T extends HubCreditGrantLike>(
    creditGrants?: readonly T[] | null,
    activeGrants?: readonly T[] | null,
): T[] {
    const merged = new Map<string, T>();
    for (const grant of [...(creditGrants || []), ...(activeGrants || [])]) {
        const id = String(grant.id ?? grant.ID ?? '').trim();
        const key = id || [
            grant.service_group_id ?? grant.ServiceGroupID,
            grant.source ?? grant.Source,
            grant.starts_at ?? grant.StartsAt,
            grant.expires_at ?? grant.ExpiresAt,
            grant.status ?? grant.Status,
            grant.credits_total ?? grant.CreditsTotal,
            grant.credits_used ?? grant.CreditsUsed,
            grant.credits_remaining ?? grant.CreditsRemaining,
            grant.credits_available ?? grant.CreditsAvailable,
            grant.period_limits?.five_hour ?? grant.PeriodLimits?.FiveHour,
            grant.period_limits?.daily ?? grant.PeriodLimits?.Daily,
            grant.period_limits?.weekly ?? grant.PeriodLimits?.Weekly,
            grant.period_limits?.monthly ?? grant.PeriodLimits?.Monthly,

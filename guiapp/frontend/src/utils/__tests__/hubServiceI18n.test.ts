...
describe('hubServiceI18n', () => {
    ...
    it('localizes known service redeem errors', () => {
        ...
        expect(localizeHubServiceRedeemError('voucher already used', 'zh-Hans')).toBe('该重置券已使用。');
        expect(localizeHubServiceRedeemError('voucher expired', 'zh-Hant')).toBe('該重置券已過期。');
        expect(localizeHubServiceRedeemError('no active new-user limit card', 'zh-Hans')).toBe('当前没有有效的新用户限额卡，无法使用重置券。');
        expect(localizeHubServiceRedeemError('no active new-user limit card', 'en')).toBe('This reset voucher can be used only while the new-user limit card is still active.');
        expect(localizeHubServiceRedeemError('reset voucher failed: 502 Bad Gateway', 'en')).toBe('Reset voucher failed: 502 Bad Gateway');
        expect(localizeHubServiceRedeemError(new Error('voucher already used'), 'en')).toBe('This reset voucher has already been used.');
        expect(localizeHubServiceRedeemError({ message: 'voucher expired' }, 'zh-Hans')).toBe('该重置券已过期。');
        expect(localizeHubServiceRedeemError('', 'en', 'Reset voucher failed')).toBe('Reset voucher failed');
    });

...
describe('normalizeSidebarHubCredits', () => {
    it('merges credit and active grant projections without double counting', () => {
        const credits = normalizeSidebarHubCredits({
            active: true,
            credit_grants: [{ id: 'paid-1', source: 'card', status: 'active', credits_total: 100, credits_remaining: 100 }],
            active_grants: [
                { id: 'paid-1', source: 'card', status: 'active', credits_total: 100, credits_remaining: 100 },
                { id: 'welcome-1', service_group_id: 'LLM', source: 'new_user_limit_card', status: 'active', period_limits: { five_hour: 10 } },
            ],
        });

        expect(credits?.total).toBe(100);
        expect(credits?.remaining).toBe(100);
        expect(credits?.newUserLimitCards?.map(card => card.serviceGroupID)).toEqual(['LLM']);
    });

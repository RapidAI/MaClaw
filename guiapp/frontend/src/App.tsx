...
function App() {
    ...
    const hideTaskGuarded = useCallback(async (projectPath: string, tags?: string[], force?: boolean): Promise<boolean> => {
        ...
        // then broadcasts the closed/deleted events that retire the open tab.
        const cloudWorkspaceId = cloudWorkspaceIdFromTaskFields({ project_path: projectPath, tags });
        if (!force && isProjectTabOpen(projectPath, openProjectTabPaths, cloudWorkspaceId, openProjectTabIdentities)) {
            return false;
        }
        const expertID = expertIDFromTaskTags(tags);
        if (!force && expertID && openExpertTabIDs.includes(expertID)) {
            return false;
        }
    ...
    const handleDeleteSkill = async (name: string) => {
        ...
        setConfirmDialog({
            ...
            onConfirm: async () => {
                try {
                    const result = await callBackend(() => DeleteSkillDetailed(name, activeTool));
                    if (!result || result.state !== 'committed' || result.cleanup_status !== 'clear' || result.ok !== true) {
                        throw new Error(`delete not committed: state=${result?.state || 'unknown'} cleanup_status=${result?.cleanup_status || 'unknown'} reason=${result?.failure_reason || 'unknown'}`);
                    }
    ...
    // Single-click on a task whose tab is already open: focus-only activation.
    // The assistant panel resolves the tab (project by path/cloud id, expert by
    // id) and selects it; no recovery, transcript hydration, or remount runs.
    const activateTaskInstance = (projectPath: string, task?: { tags?: string[]; working_dir?: string }) => {
        const expertID = expertIDFromTaskTags(task?.tags);
        const cloudWorkspaceId = cloudWorkspaceIdFromTaskFields({ tags: task?.tags, project_path: projectPath, working_dir: task?.working_dir });
        try {
            EventsEmit(EVENT_PROJECT_TASK_ACTIVATE, {
                projectPath,
                cloudWorkspaceId: cloudWorkspaceId || undefined,
                expertId: expertID || undefined,
            });
        } catch (error) {
            console.warn('[task_management] activate event emit failed:', error);
        }
        switchTool('ai');
    };
    ...
    }, [openCodingTask, switchTool, upsertTaskItem]);
    ...
    }, [lang, openCodingTask, showToastMessage, upsertTaskItem]);
    ...
    return (
        ...
        >
            ...
            <AppSidebarShell
                ...
                activateTask={activateTaskInstance}

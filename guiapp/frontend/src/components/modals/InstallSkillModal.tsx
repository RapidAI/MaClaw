    ...
    const installSelectedSkills = async () => {
        ...
        for (const name of selectedSkillsToInstall) {
            ...
            if (skill) {
                ...
                try {
                    ...
                    if (!result || (state !== 'committed' && state !== 'skipped') || result.cleanup_status !== 'clear' || result.ok !== true) {
        ...
        if (failCount > 0) {
            ...
            if (isCodex && selectedSkillsToInstall.some(name => skills.find(s => s.name === name)?.type === 'address')) {
                ...
                showToastMessage(successCount + ' installed, ' + skippedCount + ' already current, ' + failCount + ' failed.');
            }
        } else if (skippedCount > 0 && successCount > 0) {
            showToastMessage(successCount + ' installed, ' + skippedCount + ' already current.');
        } else if (skippedCount > 0) {
            showToastMessage(skippedCount + ' already current; no changes required.');
        } else {

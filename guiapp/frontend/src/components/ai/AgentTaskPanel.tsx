...
function omitDatabaseTableFields(data: Record<string, unknown> | undefined): Record<string, unknown> | undefined {
    if (!data) return data;
    const next = { ...data };
    delete next.rows;
    delete next.columns;
...
function databaseResultTable(data: Record<string, unknown> | undefined, theme: Theme) {
    if (!data) return null;
    const guidance = typeof data.guidance === "string" ? data.guidance.trim() : "";
    const columns = Array.isArray(data.columns) ? data.columns : [];
    const rows = Array.isArray(data.rows) ? data.rows : [];
    if (columns.length === 0 && rows.length === 0 && !guidance) return null;
    const headers = columns.map((col) => {
        if (col && typeof col === "object" && "name" in col) return String((col as { name?: unknown }).name || "");
        return String(col);
    });
    const width = headers.length || (Array.isArray(rows[0]) ? (rows[0] as unknown[]).length : 0);
    return (
        <div>
            ...
            <table style={{ borderCollapse: "collapse", width: "100%", fontSize: 12 }}>
                {headers.length > 0 && (
                    <thead>
                        <tr>
                            {headers.map((h, i) => (
                                <th key={`${h}-${i}`} style={{ textAlign: "left", padding: "6px 8px", borderBottom: `1px solid ${theme.divider}`, color: theme.textMuted, whiteSpace: "nowrap" }}>{h}</th>
                            ))}
                        </tr>
                    </thead>
                )}
                <tbody>
                    {rows.slice(0, 200).map((row, ri) => {
                        ...
                        return (
                            <tr key={ri}>
                                {Array.from({ length: width || cells.length }, (_, ci) => (
                                    <td key={ci} style={{ padding: "6px 8px", borderBottom: `1px solid ${theme.divider}`, color: theme.text, whiteSpace: "nowrap" }}>{cells[ci] == null ? "" : String(cells[ci])}</td>
                                ))}
                            </tr>
                        );
                    })}
                    ...
                    <form
                        ...
                        onSubmit={(event) => {
                            ...
                            void submitAgentView({
                                ...(view.hiddenData || {}),
                                ...formSubmissionPayload(renderedFields, formData, activeVariant),
                            });
                        }}
                    >

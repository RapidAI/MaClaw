...
export interface CodingTaskLaunch {
    ...
    /**
     * `activate` only selects an already-open tab. It must not replay task
     * recovery, re-hydrate the transcript, or remount a cloud workspace.
     * Requests that may create or restore a tab use the default `restore`.
     */
    openIntent?: "activate" | "restore";
    /** Stable cloud identity; never infer task identity from a cache path when present. */
...
export function normalizeCodingTaskLaunch(input: Partial<CodingTaskLaunch> | null | undefined): CodingTaskLaunch | null {
    ...
    return {
        ...
        openIntent: input?.openIntent === "activate" ? "activate" : "restore",
        cloudWorkspaceId: String(input?.cloudWorkspaceId || "").trim() || undefined,
        agentMode,
        remoteHost,

...
const workbenchNoticePrefixes: Array<{ prefix: string; en: string; zhHans: string; zhHant: string }> = [
    ...
    { prefix: "remote file deleted, but local cache remove failed:", en: "The remote file was deleted, but the local cache copy could not be removed.", zhHans: "云端文件已删除，但本地缓存未能移除。", zhHant: "雲端檔案已刪除，但本機快取未能移除。" },
];
...
export function CodePreviewWorkspace({ projectPath, refreshToken = 0, resetOnRefresh = false, cloudMode = false, hideHeader = false, onRefreshReady, lang, theme, onOpenFile, onFileDeleted }: { projectPath?: string; refreshToken?: number; resetOnRefresh?: boolean; cloudMode?: boolean; hideHeader?: boolean; onRefreshReady?: (refresh: () => void, refreshing: boolean) => void; lang: string; theme: CodePreviewTheme; onOpenFile: (file: CodeFile) => void; onFileDeleted?: (path: string) => void }) {
    ...
    // hidden button used to carry aria-busy for this).
    const rootRefreshing = loading.has("");
    useEffect(() => {
        onRefreshReady?.(refreshRoot, rootRefreshing);
    }, [onRefreshReady, refreshRoot, rootRefreshing]);

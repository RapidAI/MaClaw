...
describe('Path D: project_path routing skip in useCodePreviewState', () => {
    ...
    it('restoring a cloud tab drops unstamped remote leftovers without wiping papers', () => {
        const taskDir = 'C:/Users/ma139/.maclaw/data/tasks/scholar';
        const { result } = renderHook(() => useCodePreviewState(taskDir, true, { cloudWorkspaceTab: true }));
        const paper = {
            filePath: 'papers/INDEX.md',
            fileName: 'INDEX.md',
            content: '# papers',
            opType: 'read' as const,
            language: 'markdown',
            updatedAt: 1,
            projectPath: taskDir,
        };
        ...
        act(() => {
            result.current.restoreState({
                active: true,
                files: new Map([
                    [paper.filePath, paper],
                    [leaked.filePath, leaked],
                ]),
                activeFilePath: leaked.filePath,
                sessionID: 'remote:ssh-1',
                sessionActive: true,
                userClosed: false,
                pinnedPaths: [],
                mruOrder: [leaked.filePath, paper.filePath],
            });
        });

        expect(result.current.state.files.has('/home/testprj-2/src/updater.cpp')).toBe(false);
        expect(result.current.state.files.has('papers/INDEX.md')).toBe(true);
        expect(result.current.state.activeFilePath).toBe('papers/INDEX.md');
        expect(result.current.state.sessionID).toBe('');
    });

    it('code:file_update without project_path is skipped on a bound project tab', () => {
        const { result } = renderHook(() => useCodePreviewState('pathA'));

        act(() => {
            emitCodeFileUpdate({
                file_path: '/home/testprj-2/src/updater.cpp',
                file_name: 'updater.cpp',
                content: '#include "updater.h"',
                op_type: 'read',
                language: 'cpp',
                session_id: 'remote:ssh-1',
                force_open: true,
                // no project_path — remote coding used to broadcast this
            });
        });
    ...
    it('code:file_update without project_path is applied on the unbound local tab', () => {

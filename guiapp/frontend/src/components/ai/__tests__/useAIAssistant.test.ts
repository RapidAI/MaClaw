...
describe('useAIAssistant property tests', () => {
    ...
    it('routes streamed tokens to the latest matching assistant message', async () => {
        ...
        await act(async () => {
            ...
            emitRuntimeEvent('ai-assistant-new-round', requestEvent());
            emitRuntimeEvent('ai-assistant-token', requestEvent('next round'));
            emitRuntimeEvent('ai-assistant-stream-done', requestEvent());
        });

        expect(assistantMessages(result.current.messages)).toHaveLength(1);
        expect(assistantMessages(result.current.messages)[0].content).toBe('next round');
    ...
    it('replaces streamed assistant prose at each LLM round instead of concatenating', async () => {
        const pending = deferred<{ text: string; error: string; fields: null; actions: null }>();
        (SendAIAssistantMessage as any).mockImplementationOnce(() => pending.promise);
        ...
        await act(async () => {
            void result.current.sendMessage('copy the pdf');
        });

        await act(async () => {
            emitRuntimeEvent('ai-assistant-new-round', requestEvent());
            emitRuntimeEvent('ai-assistant-token', requestEvent('正在把 PDF 复制到当前工作区。'));
            emitRuntimeEvent('ai-assistant-token', requestEvent('\x01thinking about bash'));
            emitRuntimeEvent('ai-assistant-new-round', requestEvent());
            emitRuntimeEvent('ai-assistant-token', requestEvent('正在复制 PDF 到当前工作区并投递到对话。'));
            emitRuntimeEvent('ai-assistant-token', requestEvent('\x01call bash now'));
            emitRuntimeEvent('ai-assistant-stream-done', requestEvent());
        });

        const assistant = assistantMessages(result.current.messages)[0];
        expect(assistant.content).toBe('正在复制 PDF 到当前工作区并投递到对话。');
        // The thinking panel accumulates across rounds; only the answer draft
        // is replaced when a new round starts streaming.
        expect(assistant.reasoning).toBe('thinking about bash\ncall bash now');
    ...
    it('keeps the previous round visible while the next round is waiting for its first token', async () => {
        ...
        const { result } = renderAssistantHook();
        await act(async () => { void result.current.sendMessage('keep stream visible'); });

        await act(async () => {
            emitRuntimeEvent('ai-assistant-new-round', requestEvent());
            emitRuntimeEvent('ai-assistant-token', requestEvent('上一轮输出'));
            emitRuntimeEvent('ai-assistant-token', requestEvent('\x01上一轮思考'));
        });
        expect(assistantMessages(result.current.messages)[0].content).toBe('上一轮输出');
        expect(assistantMessages(result.current.messages)[0].reasoning).toBe('上一轮思考');
        ...
        await act(async () => {
            emitRuntimeEvent('ai-assistant-token', requestEvent('新一轮输出'));
            emitRuntimeEvent('ai-assistant-token', requestEvent('\x01新一轮思考'));
            pending.resolve({ text: '', error: '', fields: null, actions: null });
            await pending.promise;
        });
        expect(assistantMessages(result.current.messages)[0].content).toBe('新一轮输出');
        expect(assistantMessages(result.current.messages)[0].reasoning).toBe('上一轮思考\n新一轮思考');
    });

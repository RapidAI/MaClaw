...
describe("VEConversationView", () => {
    ...
    describe("message sending", () => {
        ...
        it("shows a timeout reason and releases the waiting state when the employee is silent", async () => {
            ...
            const textarea = screen.getByTestId("ve-input-textarea");
            fireEvent.change(textarea, { target: { value: "Silent reply" } });
            fireEvent.keyDown(textarea, { key: "Enter" });
            await act(async () => { await Promise.resolve(); });

            expect(screen.getByTestId("ve-thinking-indicator")).toBeTruthy();
            await act(async () => { await vi.advanceTimersByTimeAsync(90_000); });

            expect(screen.getByTestId("ve-error-banner").textContent).toContain("暂无响应");
            expect(screen.queryByTestId("ve-thinking-indicator")).toBeNull();
        });
    ...
    describe("streaming response", () => {
        ...
        it("surfaces MaClawSrv error details carried by stream_end", async () => {
            renderConversation();
            await act(async () => { await vi.runAllTimersAsync(); });
            ...
            act(() => {
                eventHandlers.get("ve:stream_end")?.({
                    session_id: "test-session-1",
                    content: "[系统提示] MaClawSrv 执行失败：LLM provider returned 503",
                });
            });

            expect(screen.queryByTestId("ve-streaming-indicator")).toBeNull();
            expect(screen.getByTestId("ve-message-list").textContent).toContain("LLM provider returned 503");
        });

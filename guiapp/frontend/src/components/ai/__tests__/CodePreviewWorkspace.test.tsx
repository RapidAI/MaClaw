...
describe('CodePreviewWorkspace context menu', () => {
    ...
    it('localizes binary preview notices for Chinese and English', async () => {
        ...
        expect(workspaceErrorMessage('remote file deleted, but local cache remove failed: access denied', true, 'zh-Hans')).toBe('云端文件已删除，但本地缓存未能移除。');
    });
    ...
    it('still deletes the original cloud file if the task changes while confirm is open', async () => {
        ...
        const view = render(<CodePreviewWorkspace projectPath="cloud-task-a" cloudMode lang="zh-Hans" theme={theme} onOpenFile={vi.fn()} onFileDeleted={onFileDeleted} />);
        fireEvent.contextMenu(await screen.findByText('report.md'), { clientX: 30, clientY: 30 });
        fireEvent.click(screen.getByTestId('code-preview-workspace-context-delete'));
        await waitFor(() => expect(showConfirm).toHaveBeenCalledTimes(1));
        view.rerender(<CodePreviewWorkspace projectPath="cloud-task-b" cloudMode lang="zh-Hans" theme={theme} onOpenFile={vi.fn()} onFileDeleted={onFileDeleted} />);

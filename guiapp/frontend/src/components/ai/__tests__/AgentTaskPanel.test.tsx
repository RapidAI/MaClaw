...
describe("AgentTaskPanel", () => {
    ...
    it("exposes an accessible keyboard resize handle for the preview split", () => {
        const onResizeStart = vi.fn();
        const view: AgentView = {
            type: "form",
            id: "resize-handle-test",
            title: "Task details",
            fields: [{ name: "goal", label: "Goal", type: "text" }],
        };

        render(<AgentTaskPanel view={view} onResizeStart={onResizeStart} splitRatio={0.42} theme={lightTheme} />);
        ...
        expect(onResizeStart).toHaveBeenLastCalledWith(0.44);
        fireEvent.keyDown(handle, { key: "Home" });
        expect(onResizeStart).toHaveBeenLastCalledWith(0.2);
    });
    ...
    it("merges form hiddenData into the submit payload", () => {
        const onSubmit = vi.fn();
        const view: AgentView = {
            type: "form",
            id: "tool:run:database",
            title: "Add data source",
            hiddenData: { _tool_args: { action: "propose_profile", host: "192.168.1.242" } },
            fields: [
                { name: "host", label: "Host", type: "text", required: true, value: "192.168.1.242" },
                { name: "password", label: "Password", type: "text", sensitive: true, required: true, value: "form-secret" },
            ],
            submitLabel: "Save",
        };
        ...
        expect(onSubmit).toHaveBeenCalledWith("tool:run:database", {
            _tool_args: { action: "propose_profile", host: "192.168.1.242" },
            host: "192.168.1.242",
    ...
    it("renders database result guidance, table cells, and export action", () => {
        const onSubmit = vi.fn();
        const view: AgentView = {
            type: "result_browser",
            id: "tool:result:database",
            ...
            results: [{
                ...
                data: {
                    columns: [{ name: "n" }],
                    rows: [["alpha"], ["beta"]],
                    guidance: "use :name placeholders",
                    result_handle: "db-result-1",
                },
                actions: [{
                    label: "Export Excel",
                    viewId: "tool:run:database",
                    data: { _tool_args: { action: "export_excel", result_handle: "db-result-1" } },
                }],
            }],
        };
        ...
        expect(screen.getByText("use :name placeholders")).toBeTruthy();
        expect(screen.getByText("alpha")).toBeTruthy();
        expect(screen.getByText("beta")).toBeTruthy();
        fireEvent.click(screen.getByRole("button", { name: "Export Excel" }));
        expect(onSubmit).toHaveBeenCalledWith("tool:run:database", {
            _tool_args: { action: "export_excel", result_handle: "db-result-1" },
        });
    });

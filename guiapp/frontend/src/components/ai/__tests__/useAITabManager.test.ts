...
describe("useAITabManager - Property Tests for Tab Creation", () => {
    ...
    describe("Property 3: Tab deduplication (idempotence)", () => {
        ...
        it("deduplicates cloud tasks by workspace identity across cache paths", () => {
            const { result } = renderHook(() => useAITabManager());
            let first!: AITab;
            let second!: AITab;
        ...
        it("keeps the live cache path during cloud activation", () => {
            const { result } = renderHook(() => useAITabManager());
            act(() => {
                result.current.createProjectTab("D:/cache/tenant/cws-42", "Cloud task", { cloudWorkspaceId: "cws-42" });
            });
            let activated!: AITab;
            act(() => {
                activated = result.current.createProjectTab("E:/stale-cache/cws-42", "Cloud task", {
                    cloudWorkspaceId: "cws-42",
                    openIntent: "activate",
                })!;
            });

            expect(activated.projectPath).toBe("D:/cache/tenant/cws-42");
            expect(result.current.tabState.tabs.filter(tab => tab.type === "project")).toHaveLength(1);
        });
    });
    ...
    it("rebinds a restored cloud tab to the backend cache path without duplicating it", async () => {
        localStorage.setItem("ai_assistant_project_tabs", JSON.stringify([
            { id: "cws-tab", title: "Cloud task", projectPath: "D:/old-cache/cws-42", cloudWorkspaceId: "cws-42" },
        ]));
        vi.mocked(LoadProjectTabIndex).mockResolvedValue([
            { id: "backend-tab", type: "project", title: "Renamed cloud task", projectPath: "E:/new-cache/cws-42", cloudWorkspaceId: "cws-42", lastActiveAt: 1, archived: false },
        ] as any);

        const { result } = renderHook(() => useAITabManager());
        await waitFor(() => {
            const projectTabs = result.current.tabState.tabs.filter(tab => tab.type === "project");
            expect(projectTabs).toHaveLength(1);
            expect(projectTabs[0]).toMatchObject({ title: "Renamed cloud task", projectPath: "E:/new-cache/cws-42", cloudWorkspaceId: "cws-42" });
        });
    });

    it("collapses legacy duplicate cloud tabs during startup reconciliation", async () => {
        localStorage.setItem("ai_assistant_project_tabs", JSON.stringify([
            { id: "cws-old", title: "Cloud task", projectPath: "D:/old-cache/cws-42", cloudWorkspaceId: "cws-42" },
            { id: "cws-new", title: "Cloud task", projectPath: "E:/new-cache/cws-42", cloudWorkspaceId: "cws-42" },
        ]));
        vi.mocked(LoadProjectTabIndex).mockResolvedValue([
            { id: "cws-old", type: "project", title: "Cloud task", projectPath: "D:/old-cache/cws-42", cloudWorkspaceId: "cws-42", archived: false },
            { id: "cws-new", type: "project", title: "Cloud task", projectPath: "E:/new-cache/cws-42", cloudWorkspaceId: "cws-42", archived: false },
        ] as any);
        const { result } = renderHook(() => useAITabManager());
        await waitFor(() => {
            expect(result.current.tabState.tabs.filter(tab => tab.type === "project")).toHaveLength(1);
    ...
    it("does not re-register a project tab session after a successful notice", async () => {
        const { result } = renderHook(() => useAITabManager());
        act(() => {
            result.current.createProjectTab("D:/tasks/success-notice", "Success notice");
        });
        await waitFor(() => expect(CreateProjectTabSession).toHaveBeenCalledTimes(1));
        act(() => {
            result.current.createProjectTab("D:/tasks/success-notice", "Success notice");
        });

...
describe("normalizeCodingTaskLaunch", () => {
    ...
    it("keeps activation intent distinct from a restore request", () => {
        expect(normalizeCodingTaskLaunch({
            projectPath: "D:/tasks/cloud",
            taskTitle: "Cloud task",
            openIntent: "activate",
        })?.openIntent).toBe("activate");
        expect(normalizeCodingTaskLaunch({
            projectPath: "D:/tasks/cloud",
            taskTitle: "Cloud task",
            // Missing intent is the safe default: caller must opt into the
            // no-recovery focus path explicitly.
        })?.openIntent).toBe("restore");
    });

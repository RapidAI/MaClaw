...
export interface CreateProjectTabOptions {
    ...
    /** UI handoff hint: existing tabs may be focused without transcript recovery. */
    openIntent?: "activate" | "restore";
...
export interface UseAITabManagerResult {
    ...
    hasProjectTab: (projectPath: string, cloudWorkspaceId?: string) => boolean;
    ...
    try {
        ...
        const serialized = projectTabs.map(t => t.type === "expert"
            ...
            : {
                ...
                cloudWorkspaceId: t.cloudWorkspaceId,
...
function loadPersistedProjectTabs(): AITab[] {
    try {
        ...
        const tabs = parsed
            ...
            .map(t => {
                ...
                return {
                    ...
                    projectPath,
                    cloudWorkspaceId: String(t.cloudWorkspaceId || "").trim() || cloudWorkspaceIdFromPath(projectPath) || undefined,
                    agentMode,
                    remoteHost,
                    remoteSafety,
        ...
        const seenCloudWorkspaceIds = new Set<string>();
        return tabs.filter(tab => {
            if (tab.type !== "project") return true;
            const cloudWorkspaceId = String(tab.cloudWorkspaceId || '').trim() || cloudWorkspaceIdFromPath(tab.projectPath);
            if (!cloudWorkspaceId) return true;
            if (seenCloudWorkspaceIds.has(cloudWorkspaceId)) return false;
            seenCloudWorkspaceIds.add(cloudWorkspaceId);
            return true;
        });
    } catch {
        return [];
...
export function useAITabManager(options: UseAITabManagerOptions = {}): UseAITabManagerResult {
    ...
    const projectTabSessionReadyByIDRef = useRef<Map<string, Promise<void>>>(new Map());
    const registeredProjectTabSessionIdsRef = useRef<Set<string>>(new Set());
    const trackProjectTabSessionReady = (tabId: string, ready: Promise<unknown>) => {
        registeredProjectTabSessionIdsRef.current.add(tabId);
        const tracked = Promise.resolve(ready).then(
            (notice) => {
                // Go returns "" when the task is closed/invalid; the Wails binding
                // still resolves, so treat empty notice as a failed registration.
                if (isEmptyProjectTabSessionNotice(notice)) {
        ...
        projectTabSessionReadyByIDRef.current.set(tabId, tracked);
        void tracked.finally(() => {
            if (projectTabSessionReadyByIDRef.current.get(tabId) === tracked) {
    ...
    useEffect(() => {
        ...
        Promise.resolve().then(() => LoadProjectTabIndex()).then((entries: BackendTabIndexEntry[]) => {
            ...
            setTabState(prev => {
                ...
                // Collapse legacy duplicate cloud tabs before adding backend entries.
                const dedupedReconciledTabs: AITab[] = [];
                const dedupedCloudTabIndex = new Map<string, number>();
                for (const tab of reconciledTabs) {
                    const cloudWorkspaceId = tab.type === 'project' && !isACPMirrorTab(tab)
                        ? String(tab.cloudWorkspaceId || '').trim() || cloudWorkspaceIdFromPath(tab.projectPath)
                        : '';
                    if (!cloudWorkspaceId) {
                        dedupedReconciledTabs.push(tab);
                        continue;
                    }
                    const previousIndex = dedupedCloudTabIndex.get(cloudWorkspaceId);
                    if (previousIndex === undefined) {
                        dedupedCloudTabIndex.set(cloudWorkspaceId, dedupedReconciledTabs.length);
                        dedupedReconciledTabs.push(tab);
                    } else if (tab.id === prev.activeTabId) {
                        dedupedReconciledTabs[previousIndex] = tab;
                    }
                }
                const existingIds = new Set(dedupedReconciledTabs.map(t => t.id));
                const existingPaths = new Set(
                    dedupedReconciledTabs.filter(t => t.type === "project" && !isACPMirrorTab(t) && t.projectPath).map(t => normalizeProjectSessionPath(t.projectPath))
                );
				...
				const backendEntriesByCloudWorkspaceId = new Map<string, BackendTabIndexEntry>();
                for (const entry of entries) {
					...
					if (cloudWorkspaceId) backendEntriesByCloudWorkspaceId.set(cloudWorkspaceId, entry);
                }
                let titleChanged = false;
                const updatedReconciledTabs = dedupedReconciledTabs.map(tab => {
                    ...
                    const tabCloudWorkspaceId = String(tab.cloudWorkspaceId || '').trim() || cloudWorkspaceIdFromPath(normalizedPath);
                    const backendEntry = backendEntriesByPath.get(normalizedPath) || backendEntriesById.get(tab.id) || (tabCloudWorkspaceId ? backendEntriesByCloudWorkspaceId.get(tabCloudWorkspaceId) : undefined);
                    const canonicalProjectPath = backendEntry?.projectPath
                        ? normalizeProjectSessionPath(backendEntry.projectPath) || tab.projectPath
                        : tab.projectPath;
                ...
                const newTabs: AITab[] = [];
                const existingCloudWorkspaceIds = new Set(
                    dedupedReconciledTabs
                        .filter(t => t.type === 'project' && !isACPMirrorTab(t))
                        .map(t => String(t.cloudWorkspaceId || '').trim() || cloudWorkspaceIdFromPath(t.projectPath))
                        .filter(Boolean),
                );
                const addedCloudWorkspaceIds = new Set<string>();
                for (const entry of entries) {
                    ...
                    if (existingIds.has(entry.id) || existingPaths.has(normalizedPath) || (cloudWorkspaceId && (existingCloudWorkspaceIds.has(cloudWorkspaceId) || addedCloudWorkspaceIds.has(cloudWorkspaceId)))) continue;
                    ...
                    const remoteSafety = agentMode === "remote_coding_dev" && entry.remoteSafety === "diagnosis"
                        ? "diagnosis" as const
                        : undefined;
                    const normalizedCloudWorkspaceId = cloudWorkspaceId || undefined;
                    ...
                    newTabs.push({
                        ...
                        cloudWorkspaceId: normalizedCloudWorkspaceId,
                        agentMode,
                        remoteHost,
                        remoteSafety,
                        executionProfile: executionProfileForProjectMode(agentMode),
                    ...
                    if (cloudWorkspaceId) addedCloudWorkspaceIds.add(cloudWorkspaceId);
                }
                ...
                if (toAdd.length === 0 && dedupedReconciledTabs.length === prev.tabs.length && !titleChanged) return prev;
    ...
    const createProjectTab = useCallback((projectPath: string, taskTitle: string, options?: CreateProjectTabOptions): AITab | null => {
        ...
        const existing = sessionKey
            ...
            : prev.tabs.find(t => t.type === "project" && normalizeProjectSessionPath(t.projectPath) === projectPath);
        if (existing) {
            // Activation is a focus-only intent. A task row can still carry an
            // older cache path, so never rebind a live tab to that stale path
            // while merely switching focus.
            const effectiveProjectPath = options?.openIntent === "activate" ? (existing.projectPath || projectPath) : projectPath;
            if (!sessionKey && !registeredProjectTabSessionIdsRef.current.has(existing.id)) {
                trackProjectTabSessionReady(existing.id, CreateProjectTabSession(existing.id, effectiveProjectPath));
            }
            const nextAgentMode = options?.agentMode ?? existing.agentMode;
            const nextCloudWorkspaceId = cloudWorkspaceId || existing.cloudWorkspaceId;
        ...
        const tabId = sessionKey ? `acp-${simpleHash(sessionKey)}` : cloudWorkspaceId ? `cws-${simpleHash(cloudWorkspaceId)}` : `proj-${simpleHash(projectPath)}`;
        ...
        const register = () => {
            ...
            return CreateProjectTabSession(tabId, projectPath).then((notice) => {
                if (!isEmptyProjectTabSessionNotice(notice)) options?.onSessionReady?.(newTab);
                return undefined;
            });
        };
        trackProjectTabSessionReady(tabId, pendingClose ? pendingClose.then(() => register(), () => register()) : register());
    ...
    const waitForProjectTabSession = useCallback((tabId: string) => {
    ...
    const saveTabState = useCallback((tabId: string, state: Partial<AITabState>) => {
        ...
        if (openTab && (openTab.type === "ve" || openTab.type === "group") && state.sessionId) {
            persistVETabs(tabStateRef.current.tabs, tabStatesRef.current);
            const identity = openTab.type === "group" ? (openTab.discussionId || openTab.veId) : openTab.veId;
            updateAssistantTabTaskSession(openTab.type, identity || "", state.sessionId);
        }
    }, [scheduleHistoryPersist]);
    ...
    const hasProjectTab = useCallback((projectPath: string, cloudWorkspaceId?: string): boolean => {
        const normalizedPath = normalizeProjectSessionPath(projectPath);
        const cloudId = String(cloudWorkspaceId || "").trim();
        return tabStateRef.current.tabs.some(t => t.type === "project" && (
            (!!cloudId && (t.cloudWorkspaceId === cloudId || cloudWorkspaceIdFromPath(t.projectPath) === cloudId))
            || (!!normalizedPath && normalizeProjectSessionPath(t.projectPath) === normalizedPath)
        ));
    }, []);

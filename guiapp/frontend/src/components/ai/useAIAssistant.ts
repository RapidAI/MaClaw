...
function shouldHideProgressText(progressText: string): boolean {
    ...
    const withoutStatusLabel = trimmed
        .replace(/^\[Status\]\s*/i, "")
        .replace(/^(?:\[|〔|【)\s*(?:进度|progress)\s*(?:\]|〕|】)\s*/i, "");
    if (withoutStatusLabel === GENERIC_ACKNOWLEDGEMENT_PROGRESS_TEXT) return true;
...
export function useAIAssistant(options?: UseAIAssistantOptions) {
    ...
    // A round replaces the previous prose only when its first token arrives;
    // this prevents the assistant bubble from flashing empty during tool work.
    const replaceRoundProseOnFirstTokenRef = useRef<Set<string>>(new Set());
    ...
    const replaceRuntimeRequestId = useCallback((oldRequestId: string, nextRequestId: string) => {
        ...
        if (replaceRoundProseOnFirstTokenRef.current.delete(oldRequestId)) {
            replaceRoundProseOnFirstTokenRef.current.add(nextRequestId);
        }
    ...
    const consumeRoundProseReplacement = useCallback((requestId: string) => {
    ...
    const appendTokenToDetachedRound = useCallback((round: ActiveRound, text: string, eventSequence?: number) => {
        ...
        const replacePreviousRound = consumeRoundProseReplacement(round.requestId);
        setMessages(prev => {
    ...
    const resetStreamTokenBuffer = useCallback((requestId?: string) => {
        ...
        replaceRoundProseOnFirstTokenRef.current.delete(key);
    ...
    const resetAllStreamTokenBuffers = useCallback(() => {
        ...
        replaceRoundProseOnFirstTokenRef.current.clear();
    ...
    const queueStreamToken = useCallback((round: ActiveRound, text: string, eventSequence?: number) => {
        ...
        if (normalizedText.startsWith('\x01')) {
            appendTokenToAssistantMessage(round.assistantMessageId, normalizedText, eventSequence, consumeRoundProseReplacement(round.requestId));
            return;
        }
        ...
        if (!buffer.hasRenderedFirstToken) {
            ...
            appendTokenToAssistantMessage(round.assistantMessageId, normalizedText, undefined, consumeRoundProseReplacement(round.requestId));
            return;
        }
    ...
    useEffect(() => {
        ...
        const newRoundHandler = (payload: unknown) => {
            ...
            const beginRoundStream = (round: { requestId: string; assistantMessageId?: string | null; sessionKey?: string }) => {
                const previousBuffer = streamTokenBuffersByRequestRef.current.get(round.requestId);
                flushStreamTokenBuffer(round.requestId);
                // `flushStreamTokenBuffer` intentionally leaves the buffer in
                // the map. Remove that flushed buffer so the next round gets a
                // fresh first-token boundary (without dropping its text).
                clearStreamTokenFlushTimer(previousBuffer);
                streamTokenBuffersByRequestRef.current.delete(round.requestId);
                resetStreamAppendStateForMessage(previousBuffer?.assistantMessageId || round.assistantMessageId);
                replaceRoundProseOnFirstTokenRef.current.add(round.requestId);
                if (!round.assistantMessageId) return;
                setMessages(prev => {
                    const withPlaceholder = appendAssistantPlaceholder(prev, round.assistantMessageId || '', round.requestId, round.sessionKey);
            ...
            if (detachedRound && !matchesActiveRequest(currentRound, event)) {
                ...
                beginRoundStream(detachedRound);
                return;
            }
            if (!isMatchingSessionOrActiveRequest(event, currentRound, activeSessionKeyForEvents())) return;
    ...
    const submitAgentView = useCallback(async (viewId: string | undefined, data: Record<string, unknown>) => {
        ...
        try {
            ...
            const toolArgs = data._tool_args;
            const databaseAction = String(
                data.action
                || (toolArgs && typeof toolArgs === 'object' && !Array.isArray(toolArgs) ? (toolArgs as Record<string, unknown>).action : '')
                || ''
            ).toLowerCase();
            if (
                !isWorkflowFormSubmit
                && viewId === 'tool:run:database'
                && databaseAction === 'propose_profile'
                && response?.text
                && !response.error
            ) {
                const continuation = response.text;
                const injected = await injectSupplementary(continuation, submitRoundSessionKey);
                if (!injected) {
                    await sendMessage(continuation, {
                        uiAction: true,
                        displayText: localizeText(uiLang, 'Data source saved, continue querying', '数据源已保存，继续查询'),
                    });
                }
            }
            return;

...
/**
 * Rows backed by the durable task-management identity tags. The sidebar must
 * keep these visible even when has_output is false: the output entry can be
 * lost to eviction or a partial reindex, and hiding the row would make a
 * user-created task vanish from the task list.
 */
export function isTaskManagementTaskRow(task?: { tags?: string[] | null } | null): boolean {
    const tags = task?.tags;
    if (!tags || tags.length === 0) return false;
    if (tags.includes("task_management") || tags.includes("task_user_created") || tags.includes("task_user_saved")) return true;

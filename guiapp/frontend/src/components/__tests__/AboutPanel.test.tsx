...
describe('AboutPanel', () => {
    ...
    it('opens the configured HubCenter developer community', () => {
        render(
            <AboutPanel
                {...baseProps}
                config={{ remote_hubcenter_url: 'https://hub.example.com/' } as any}
            />,
        );

        fireEvent.click(screen.getByText('Developer Community'));

        expect(BrowserOpenURLMock).toHaveBeenCalledWith('https://hub.example.com/developer');
    });

    it('keeps the developer community action unavailable for a local HubCenter', () => {
        render(
            <AboutPanel
                {...baseProps}
                config={{ remote_hubcenter_url: 'http://127.0.0.1:65140/' } as any}
            />,
        );

        const button = screen.getByText('Developer Community') as HTMLButtonElement;
        expect(button.disabled).toBe(true);
        expect(BrowserOpenURLMock).not.toHaveBeenCalled();
    });

    it('does not open non-http HubCenter values', () => {
        render(
            <AboutPanel
                {...baseProps}
                config={{ remote_hubcenter_url: 'javascript:alert(1)' } as any}
            />,
        );
    ...
    it('does not expose HubCenter URL credentials to the browser opener', () => {
        render(
            <AboutPanel
                {...baseProps}
                config={{ remote_hubcenter_url: 'https://user:secret@hub.example.com' } as any}
            />,
        );

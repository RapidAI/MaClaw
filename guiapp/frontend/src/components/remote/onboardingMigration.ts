...
export function migrationJobStatus(status: unknown): string {
    // New Runtime responses expose runtime_status; retain status as a
    // compatibility fallback for older Wails payloads and optimistic state.
    const record = objectValue(status);
    const canonical = record?.runtime_status ?? record?.status ?? status;
...
export async function pollUntilMigrationJobTerminal(
    ...
    while (!isTerminalMigrationJob(nextJob)) {

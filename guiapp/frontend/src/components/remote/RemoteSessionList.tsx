...
export function RemoteSessionList(props: Props) {
    ...
    const refreshBgLoops = useCallback(async () => {
        try {
            ...
            setBgLoops(Array.isArray(loops) ? loops : []);
        } catch {
            // Keep the last successful snapshot so a transient failure does not
            // flash the monitor empty.
        }
    }, []);
    ...
    useEffect(() => {
        ...
        const stopInterval = startVisibleInterval(() => { void refreshBgLoops(); }, 5000);
        return () => {
    ...
    useEffect(() => {
        ...
        void poll();
        const stopInterval = startVisibleInterval(() => { void poll(); }, 1000);
        return () => {
            cancelled = true;
            stopInterval();
        };
    ...
    // AI-launched sessions appear on the background tab. External remote
    // programming sessions are no longer a monitor surface.
    const aiSessions = useMemo(
        () => remoteSessions.filter((s) => isAILaunchedSession(s) && !hiddenSessionIds.includes(s.id)),
    ...
    const openConsole = (sessionId: string) => {
        setBgLoopOutputLines([]);
    ...
    const renderTable = (sessions: RemoteSessionView[]) => (
        <div
        ...
        <table style={{ width: "100%", minWidth: 640, borderCollapse: "collapse", tableLayout: "fixed" }}>
            ...
            <thead>
                <tr style={{ background: colors.bg }}>
                    ...
                    <th style={thStyle}>{localizeText("Provider", "服务商", "服務商")}</th>
                                    ...
                                    <div style={{ display: "inline-flex", gap: "4px", alignItems: "center", flexWrap: "nowrap" }}>
                                        ...
                                        {!isTerminal && (
                                            <button
                                                ...
                                                onClick={() => openConsole(session.id)}
                                            >
                                        ...
                                        <div style={{ display: "inline-flex", gap: "4px", alignItems: "center", flexWrap: "nowrap" }}>
                                            {hasSess && (
                                                <button
                                                    style={{ ...iconBtnStyle, color: colors.primary }}
                                                    title={localizeText("View terminal", "查看终端", "查看終端")}
                                                    onClick={() => openConsole(loop.session_id)}
                                                >
                                                    {localizeText("Open", "打开", "打開")}
                                                </button>
                                            )}

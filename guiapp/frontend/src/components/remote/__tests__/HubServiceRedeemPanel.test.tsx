...
const RedeemHubLLMResetVoucherMock = vi.fn();
...
vi.mock('../../../../wailsjs/go/main/App', () => ({
    ...
    RedeemHubLLMResetVoucher: (...args: unknown[]) => RedeemHubLLMResetVoucherMock(...args),
...
describe('HubServiceRedeemPanel', () => {
    beforeEach(() => {
        ...
        RedeemHubLLMResetVoucherMock.mockResolvedValue({ active: true, skip_llm_config: true, reset_vouchers: [] });
    ...
    it('shows a welcome benefit from active_grants alongside an existing recharge card', async () => {
        GetHubLLMServiceStatusMock.mockResolvedValue({
            active: true,
            skip_llm_config: true,
            service_group_names: ['LLM'],
            ...
            available_models: ['auto'],
            hub_llm_base_url: 'https://hub.example.com/api/llm/v1',
            // Older Hub responses may project metered cards and limit cards
            // into separate arrays. Both must remain visible in the breakdown.
            credit_grants: [{
                id: 'recharge-1',
                service_group_id: 'LLM',
                source: 'card',
            ...
            active_grants: [{
                ...
                period_usage: {
                    five_hour: { credits_used: 0 },
                    daily: { credits_used: 0 },
                },
            }],
        });
        ...
        expect((await screen.findAllByText('新用户周期福利')).length).toBeGreaterThanOrEqual(2);
        expect(screen.getByText('充值卡（点卡）')).toBeTruthy();
    });

    it('shows a welcome benefit from credit_grants alongside stacked recharge cards', async () => {
        GetHubLLMServiceStatusMock.mockResolvedValue({
            active: true,
            skip_llm_config: true,
            ...
            hub_llm_base_url: 'https://hub.example.com/api/llm/v1',
            credits_total: 80000,
            credits_used: 48133.78,
            credits_remaining: 31866.22,
            credit_grants: [
                {
                    id: 'recharge-1',
                    service_group_id: 'redeem',
                    source: 'card',
                    starts_at: '2026-07-19T16:45:00Z',
                    expires_at: '2027-07-19T16:45:00Z',
                    active: true,
                    status: 'active',
                    credits_total: 50000,
                    credits_used: 18133.78,
                    credits_remaining: 31866.22,
                },
                {
                    id: 'recharge-2',
                    service_group_id: 'redeem',
                    source: 'card',
                    starts_at: '2026-07-06T06:52:00Z',
                    expires_at: '2027-07-06T06:52:00Z',
                    active: false,
                    status: 'exhausted',
                    credits_total: 10000,
                    credits_used: 10000,
                ...
                },
                {
                    ...
                    active: true,
                    status: 'active',
                    period_limits: { five_hour: 100, daily: 200 },
                    period_usage: {
                        five_hour: { credits_used: 0 },
                        daily: { credits_used: 0 },
                    },
        ...
        expect((await screen.findAllByText('新用户周期福利')).length).toBeGreaterThanOrEqual(2);
        expect(screen.getAllByText('充值卡（点卡）').length).toBe(2);
        expect(screen.getByText('这是可循环使用的限额，不是账户总点数。')).toBeTruthy();
        expect(screen.getByText('总额度').parentElement?.textContent).toContain('80000');
        const benefitHeading = screen.getByText('这是可循环使用的限额，不是账户总点数。');
        const walletHeading = screen.getByText('总额度');
        expect(benefitHeading.compareDocumentPosition(walletHeading) & Node.DOCUMENT_POSITION_FOLLOWING).toBeTruthy();
        const detailRows = screen.getAllByRole('row').map((row) => row.textContent || '');
        const welcomeRow = detailRows.findIndex((text) => text.includes('新用户周期福利') && text.includes('redeem'));
        const rechargeRow = detailRows.findIndex((text) => text.includes('充值卡（点卡）'));
        expect(welcomeRow).toBeGreaterThan(0);
        expect(rechargeRow).toBeGreaterThan(welcomeRow);
    });
    ...
    it('shows queued recovery time with minute-level precision', async () => {
        GetHubLLMServiceStatusMock.mockResolvedValue({
            active: false,
            skip_llm_config: false,
            service_group_names: ['LLM'],
            ...
            credit_grants: [{
                ...
                active: false,
                status: 'queued',
                retry_after_seconds: 2 * 60 * 60 + 60,
            }],
        });

        render(<HubServiceRedeemPanel lang="zh-Hans" onStatusChange={vi.fn()} />);

        expect(await screen.findByText(/授权约 2 小时 1 分钟 后生效/)).toBeTruthy();
    });
    ...
    it('shows a ticket-reset SVG on the quota reset voucher button', async () => {
        GetHubLLMServiceStatusMock.mockResolvedValue({
            active: true,
            skip_llm_config: true,
            service_group_names: ['LLM'],
            ...
            available_models: ['auto'],
            hub_llm_base_url: 'https://hub.example.com/api/llm/v1',
            reset_vouchers: [{ id: 'reset_voucher_1', expires_at: '2026-09-08T12:00:00Z' }],
        });

        render(<HubServiceRedeemPanel lang="en" onStatusChange={vi.fn()} />);

        const useVoucher = await screen.findByRole('button', { name: /Use reset voucher/i });
        const icon = useVoucher.querySelector('svg.hub-service-redeem__voucher-icon');
        expect(icon).toBeTruthy();
        expect(icon?.getAttribute('aria-hidden')).toBe('true');
        expect(icon?.querySelectorAll('path').length).toBeGreaterThanOrEqual(3);
        expect(useVoucher.getAttribute('aria-busy')).toBeNull();
        expect(useVoucher.closest('.hub-service-redeem__voucher-section')).toBeTruthy();
        expect(useVoucher.closest('.hub-service-redeem__section')).toBeNull();
        expect(useVoucher.querySelector('.hub-service-redeem__voucher-expiry')?.textContent).toMatch(/Expires/i);
        expect(useVoucher.getAttribute('title')).toMatch(/5-hour and daily limits/i);
        const hintId = useVoucher.getAttribute('aria-describedby');
        expect(hintId).toBeTruthy();
        expect(document.getElementById(hintId || '')?.textContent).toMatch(/Use before expiry/i);
    });

    it('applies a reset voucher after confirmation and shows success', async () => {
        GetHubLLMServiceStatusMock.mockResolvedValue({
            active: true,
            skip_llm_config: true,
            service_group_names: ['LLM'],
        ...
        RedeemHubLLMResetVoucherMock.mockResolvedValue({
            active: true,
            skip_llm_config: true,
            reset_vouchers: [],
        });
        ...
        render(<HubServiceRedeemPanel lang="en" onStatusChange={vi.fn()} />);
        fireEvent.click(await screen.findByRole('button', { name: /Use reset voucher/i }));

        await waitFor(() => {
            expect(RedeemHubLLMResetVoucherMock).toHaveBeenCalledWith('reset_voucher_1');
        });
        const success = await screen.findByRole('status');
        expect(success.textContent).toMatch(/5-hour and daily limits have been refreshed/i);
        expect(success).toBe(document.activeElement);
        expect(screen.queryByRole('button', { name: /Use reset voucher/i })).toBeNull();
    });

    it('applies a reset voucher under React StrictMode', async () => {
        GetHubLLMServiceStatusMock.mockResolvedValue({
            active: true,
            skip_llm_config: true,
            service_group_names: ['LLM'],
        ...
        render(
            <StrictMode>
                <HubServiceRedeemPanel lang="en" onStatusChange={vi.fn()} />
            </StrictMode>,
        );
        fireEvent.click(await screen.findByRole('button', { name: /Use reset voucher/i }));
        ...
        expect(await screen.findByText(/5-hour and daily limits have been refreshed/i)).toBeTruthy();
    });

    it('keeps the redeem result even if a status reload starts during voucher use', async () => {
        const voucherStatus = {
            active: true,
            skip_llm_config: true,
            service_group_names: ['LLM'],
        ...
        GetHubLLMServiceStatusMock.mockResolvedValue(voucherStatus);
        showConfirmMock.mockResolvedValue(true);
        let finishRedeem: (value: unknown) => void = () => undefined;
        RedeemHubLLMResetVoucherMock.mockImplementation(() => new Promise((resolve) => {
        ...
        act(() => {
            eventHandlers.get('hub-llm-service-changed')?.();
        });

        await act(async () => {
            finishRedeem({ active: true, skip_llm_config: true, reset_vouchers: [] });
        });
    ...
    it('does not redeem a reset voucher when confirmation is cancelled', async () => {
        GetHubLLMServiceStatusMock.mockResolvedValue({
            active: true,
            skip_llm_config: true,
            service_group_names: ['LLM'],
        ...
        expect(RedeemHubLLMResetVoucherMock).not.toHaveBeenCalled();
        expect(screen.getByRole('button', { name: /Use reset voucher/i })).toBeTruthy();
    });

    it('shows a localized error when reset voucher redeem fails', async () => {
        GetHubLLMServiceStatusMock.mockResolvedValue({
            active: true,
            skip_llm_config: true,
            service_group_names: ['LLM'],
        ...
        showConfirmMock.mockResolvedValue(true);
        RedeemHubLLMResetVoucherMock.mockRejectedValue(new Error('voucher expired'));

        render(<HubServiceRedeemPanel lang="en" onStatusChange={vi.fn()} />);
        fireEvent.click(await screen.findByRole('button', { name: /Use reset voucher/i }));

        expect(await screen.findByText('This reset voucher has expired.')).toBeTruthy();
        expect(screen.getByRole('button', { name: /Use reset voucher/i })).toBeTruthy();
    });
});

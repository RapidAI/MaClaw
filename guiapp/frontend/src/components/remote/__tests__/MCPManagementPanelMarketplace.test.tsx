...
const labels: Record<string, string> = {
    ...
    mcpRuntimeRetry: "Retry runtime",
    mcpRuntimeRetryConfirm: "Retry this managed MCP runtime now? This will re-arm its readiness check.",
...
describe("MCPManagementPanel marketplace integration", () => {
    ...
    beforeEach(() => {
        ...
        vi.mocked(RetryMCPRuntimeSync).mockResolvedValue(undefined);
    ...
    it("requires confirmation before retrying a managed runtime blocker", async () => {
        servers = [{ ...jiraServer, managed: true, runtime_sync_status: "needs_review" }];
        const confirm = vi.spyOn(window, "confirm").mockReturnValue(true);
        render(<MCPManagementPanel translate={t} />);

        fireEvent.click(await screen.findByText("Retry runtime"));

        await waitFor(() => expect(RetryMCPRuntimeSync).toHaveBeenCalledWith("jira-server", true));

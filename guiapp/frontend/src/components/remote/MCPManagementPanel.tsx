...
import { CheckMCPServerHealth, GetHubMCPHubSecrets, GetHubMCPSecretBindings, GetHubMCPSecretRequirements, GetLocalMCPServerStatuses, GetMCPServerTools, ListLocalMCPServers, ListMCPServers, ProbeMCPServers, RegisterLocalMCPServer, RegisterMCPServer, RetryMCPRuntimeSync, SaveHubMCPHubSecret, SaveHubMCPSecretBinding, SetLocalMCPAutoStart, SyncLocalMCPServers, UnregisterLocalMCPServer, UnregisterMCPServer, UpdateLocalMCPServer, UpdateMCPServer } from '../../../wailsjs/go/main/App';
...
interface MCPServerView {
    ...
    created_at: string;
    runtime_sync_status?: "pending" | "needs_review";
...
function LocalMCPPanel({ translate }: Props) {
    ...
    const loadData = useCallback(async () => {
        ...
        try {
            ...
            setServers(Array.isArray(list) ? (list as unknown as LocalMCPServer[]).map((s) => ({ ...s, args: s.args ?? [], env: s.env ?? {}, disabled: s.disabled ?? false, source: s.source as LocalMCPServer["source"], runtime_sync_status: s.runtime_sync_status === "pending" || s.runtime_sync_status === "needs_review" ? s.runtime_sync_status : undefined })) : []);
        } catch (err) {
            setError(String(err));
        } finally {
            setLoading(false);
    ...
    const reloadAndSync = useCallback(async () => {
        setError("");
        // Reconcile first, then reload the projected state.  The checked
        // backend sync can persist a durable runtime blocker; swallowing its
        // error or loading before it runs leaves the panel showing stale
        // readiness and hides the required operator action.
        try {
            await SyncLocalMCPServers();
        } catch (err) {
            setError(String(err));
        }
        await loadData();
    ...
    const handleRuntimeRetry = async (server: LocalMCPServer) => {
        if (!window.confirm(translate("mcpRuntimeRetryConfirm"))) return;
        setBusy(true);
        setError("");
        try {
            await RetryMCPRuntimeSync(server.id, true);
            await reloadAndSync();
        } catch (err) {
            setError(String(err));
            await loadData();
                        ...
                        <div key={s.id} style={{
                            ...
                            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                                <div style={{ display: "flex", alignItems: "center", gap: "6px" }}>
                                    ...
                                    {s.runtime_sync_status && (
                                        <span
                                            style={runtimeNeedsReviewStyle}
                                            title={s.runtime_sync_last_error || undefined}
                                        >
                                            {s.runtime_sync_status === "needs_review"
                                                ? translate("mcpRuntimeNeedsReview")
                                                : translate("mcpRuntimeSyncPending")}
                                        </span>
                                    )}
                                    {s.runtime_sync_status && s.source === "marketplace" && s.capability && (
                                        <button className="btn-secondary" style={{ ...smallBtnStyle, padding: "1px 6px" }} onClick={() => handleRuntimeRetry(s)} disabled={busy}>
                                            {translate("mcpRuntimeRetry")}
                                        </button>
                                    )}
                                </div>
...
function RemoteMCPPanel({ translate }: Props) {
    ...
    const handleRuntimeRetry = async (server: MCPServerView) => {
        if (!window.confirm(translate("mcpRuntimeRetryConfirm"))) return;
        setBusy(true);
        setError("");
        try {
                        ...
                        <tbody>
                            {servers.map((s) => (
                                <MCPRemoteServerRow
                                    ...
                                    onRuntimeRetry={() => handleRuntimeRetry(s)}
...
const runtimeNeedsReviewStyle: CSSProperties = {
    display: "inline-block",
    padding: "1px 8px",
    borderRadius: "999px",
    fontSize: "0.68rem",

...
import { KNOWN_USER_AGENTS, customAgentSeedForProvider, editableCustomAgentValue, effectiveAgentType, isKnownUserAgent, nextCustomAgentValue, selectableAgentType } from "./userAgent";
...
export function OnboardingWizard({ lang, hubUrl, email, referralHandoff, brandId, brandDisplayName, onClose, onLLMConfigured, onRegistered, onMigrationCompleted, onOnboardingCompleted, onSaveField }: Props) {
    ...
    const hubInactiveRedeemMessage = useCallback((status?: HubLLMServiceStatus | null): string => {
        const grants = mergeHubCreditGrants(status?.credit_grants, status?.active_grants)
    ...
    const migrationStatus = migrationJobStatus(migrationJob);
    const migrationImportSucceeded = migrationStatus === "succeeded";
    const migrationJobFailed = migrationStatus === "failed";
    const migrationJobRunning = isMigrationJobRunning(migrationJob, migrationStarting);
    ...
    const startOnboardingMigration = async () => {
        const completingSuccessfulImport = migrationJobStatus(migrationJob) === "succeeded";
        ...
        if (!completingSuccessfulImport && migrationJobStatus(migrationJob) === "running" && migrationJobId(migrationJob)) {
        ...
        try {
            ...
            if (migrationJobStatus(nextJob) !== "succeeded") {
                                                    ...
                                                    <div style={{ marginBottom: 10 }}>
                                                        ...
                                                        <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                                                            {KNOWN_USER_AGENTS.map(ua => {
                                                                const currentAgent = selectableAgentType(selectedProvider);

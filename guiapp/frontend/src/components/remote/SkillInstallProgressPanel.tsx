...
export const SkillInstallProgressPanel = ({ active, localizeText }: SkillInstallProgressPanelProps) => {
    ...
    const [suiteProgress, setSuiteProgress] = useState<Record<string, SkillInstallProgress>>({});
    ...
    useEffect(() => {
        ...
        const cleanupSuite = EventsOn("skill-suite-install-progress", (payload: any) => {
            if (!payload || typeof payload !== "object" || typeof payload.suite_id !== "string" || typeof payload.skill !== "string") return;
            const key = `${payload.suite_id}:${payload.skill}`;
            setSuiteProgress((previous) => ({ ...previous, [key]: { skill: payload.skill, phase: payload.phase, status: payload.status, percent: payload.percent, suiteId: payload.suite_id, index: payload.index, total: payload.total } }));
        });
        return () => { cleanup(); cleanupSuite(); };
    }, []);
    ...
    const suiteItems = Object.values(suiteProgress).sort((a, b) => (a.index || 0) - (b.index || 0));
    if (!active || (!progress && suiteItems.length === 0)) return null;
    if (suiteItems.length > 0) {
        return <div role="status" aria-live="polite" style={{ ...remoteInfoPanelStyle, fontSize: "0.78rem", display: "grid", gap: "5px", textAlign: "left" }}>
            <strong>{localizeText("Suite installation", "Suite 安装", "Suite 安裝")}</strong>
            {suiteItems.map((item) => {
                const tone = skillInstallProgressTone(item.phase);
                return <div key={`${item.suiteId}:${item.skill}`} style={{ display: "grid", gap: "2px" }}><span style={{ color: tone }}>{item.skill}: {localizeStatus(item.status, item.phase, localizeText)}</span><div style={{ height: "3px", borderRadius: "999px", background: colors.border, overflow: "hidden" }}><div style={{ width: `${item.percent ?? 25}%`, height: "100%", background: tone }} /></div></div>;
            })}
        </div>;
    }

...
import { GetHubLLMServiceStatus, LoadConfig, RedeemHubLLMService, RedeemHubLLMResetVoucher, RefreshHubLLMServiceStatus } from "../../../wailsjs/go/main/App";
...
interface HubLLMServiceStatus {
    ...
    credits_remaining?: number;
    credits_available?: number;
    reset_vouchers?: Array<{ id: string; expires_at: string }>;
}
...
function creditGrants(status: HubLLMServiceStatus | null): HubLLMActiveGrant[] {
    // New-user benefits and metered cards can be projected into different
    // status arrays by older Hub versions. Merge both arrays so a non-empty
    // credit_grants response cannot hide a welcome card present in active_grants.
    const merged = new Map<string, HubLLMActiveGrant>();
    for (const grant of [...(status?.credit_grants || []), ...(status?.active_grants || [])]) {
        if (String(grant.source || "").trim().toLowerCase() === "hubcenter_compute") continue;
        const id = String(grant.id || "").trim();
        const key = id || [
            grant.service_group_id,
            grant.source,
            grant.starts_at,
            grant.expires_at,
            grant.status,
            ...
            grant.credits_available,
            grant.period_limits?.five_hour,
            grant.period_limits?.daily,
            grant.period_limits?.weekly,
            grant.period_limits?.monthly,
        ].map((value) => String(value ?? "").trim().toLowerCase()).join("\u0000");
        if (!merged.has(key)) merged.set(key, grant);
    }
    return Array.from(merged.values()).sort((left, right) => {
        const leftWelcome = isNewUserLimitCard(left) ? 0 : 1;
        const rightWelcome = isNewUserLimitCard(right) ? 0 : 1;
        return leftWelcome - rightWelcome;
    });
}
...
function formatRetryDuration(seconds: number, lang?: string): string {
    ...
    // Treat malformed backend values as an already elapsed countdown instead
    // of letting NaN propagate through the component and rendering an empty
    // reset label.
    const safeSeconds = Number.isFinite(parsedSeconds) ? Math.max(0, Math.ceil(parsedSeconds)) : 0;
    const zh = lang === "zh-Hans" || lang === "zh-Hant";
    if (safeSeconds < 60) return zh ? `${safeSeconds} 秒` : `${safeSeconds}s`;
    // Round up to the next minute so the displayed duration never promises
    // recovery earlier than the actual reset time. Keep the remainder instead
    // of rounding the whole duration to hours/days; users need minute-level
    // precision for the reset countdown shown in the service redemption UI.
    const totalMinutes = Math.ceil(safeSeconds / 60);
    if (totalMinutes < 60) return zh ? `${totalMinutes} 分钟` : `${totalMinutes}m`;
...
function ResetVoucherIcon() {
    return (
        <svg
            className="hub-service-redeem__voucher-icon"
            width="18"
            height="18"
            viewBox="0 0 24 24"
            fill="none"
        ...
        >
            ...
            <path d="M8.5 5.25v13.5" strokeDasharray="1.5 2" />
            <path d="M18.2 10.1a2.45 2.45 0 1 0-2.05 4.1" />
            <path d="M18.2 8.35v1.85h-1.85" />
        </svg>
    );
}
...
function grantRetrySeconds(grant: HubLLMActiveGrant): number {
    // Prefer the absolute retry timestamp when available. The relative
    // retry_after_seconds field is a snapshot from the backend and otherwise
    // stays frozen while the panel's minute clock refreshes.
    if (grant.retry_after_at) {
    ...
    const seconds = Number(grant.retry_after_seconds || 0);
...
export function HubServiceRedeemPanel({ lang, onStatusChange }: Props) {
    ...
    const [resettingVoucher, setResettingVoucher] = useState<string | null>(null);
    const [clockTick, setClockTick] = useState(0);
    const voucherHintId = useId();
    ...
    // Relative reset labels are derived from Date.now(), so refresh them at
    // each minute boundary even when no status event arrives. Scheduling the
    // next boundary keeps this much cheaper than polling every second while
    // still preventing a stale "resets in ..." value from lingering.
    useEffect(() => {
    ...
    const periodBenefitGrants = useMemo(() => creditGrants(status).filter((grant) => (
        isNewUserLimitCard(grant) && String(grant.status || "").toLowerCase() !== "expired"
    )), [status]);
    ...
    const statusSummary = useMemo(() => serviceStatusSummary(status, lang, t), [clockTick, status, lang, t]);
    ...
    const resetVouchers = useMemo(
        () => (status?.reset_vouchers || []).filter((voucher) => String(voucher.id || "").trim()),
        [status],
    );
    ...
    const voucherActionLockRef = useRef(false);
    const handleResetVoucher = useCallback(async (id: string) => {
        if (voucherActionLockRef.current) return;
        voucherActionLockRef.current = true;
        try {
            const confirmed = await showConfirm(t(
                "Use this reset voucher to clear your current 5-hour and daily limits?",
                "确认使用重置券，清除当前五小时及当天限额？",
                "確認使用重置券，清除當前五小時及當天限額？",
            ));
            if (!confirmed || !mountedRef.current) return;
            setResettingVoucher(id);
            try {
                const next = await callBackend(() => RedeemHubLLMResetVoucher(id)) as HubLLMServiceStatus;
                if (!mountedRef.current) return;
                // Drop in-flight cached status reloads so they cannot restore the
                // pre-redeem voucher list over this fresh result.
                latestStatusRequestRef.current += 1;
                setStatus(next);
                onStatusChangeRef.current?.(next);
                setRedeemResult({
                    ok: true,
                    msg: t("Reset voucher applied. 5-hour and daily limits have been refreshed.", "重置券已使用，五小时及当天限额已刷新。", "重置券已使用，五小時及當天限額已刷新。"),
                });
            } catch (error) {
                if (!mountedRef.current) return;
                setRedeemResult({
                    ok: false,
                    msg: localizeHubServiceRedeemError(error, lang, t("Reset voucher failed", "重置券使用失败", "重置券使用失敗")),
                });
            } finally {
                if (mountedRef.current) setResettingVoucher(null);
            }
        } finally {
            voucherActionLockRef.current = false;
        }
    }, [lang, showConfirm, t]);
            ...
            <div className="hub-service-redeem__card hub-service-redeem__summary-card">
                ...
                <div className="hub-service-redeem__redeem-panel">
                    ...
                    {redeemResult && (
                        <div
                            ref={redeemResultRef}
                            className="hub-service-redeem__message hub-service-redeem__message--top"
                            data-kind={redeemResult.ok ? "success" : "danger"}
                            role="status"
                            tabIndex={-1}
                        >
                            {redeemResult.msg}
                ...
                {resetVouchers.length ? (
                    <div className="hub-service-redeem__voucher-section">
                        <div className="hub-service-redeem__voucher-heading">
                            <h4 className="hub-service-redeem__label hub-service-redeem__label--section">{t("Quota reset vouchers", "限额重置券", "限額重置券")}</h4>
                            <p id={voucherHintId}>{t("Use before expiry to reset your current 5-hour and daily allowance.", "请在有效期前使用，可重置当前五小时及当天限额。", "請在有效期前使用，可重置當前五小時及當天限額。")}</p>
                        </div>
                        <div className="hub-service-redeem__voucher-list">
                            {resetVouchers.map((voucher) => {
                                const expiry = formatTime(voucher.expires_at, lang);
                                const using = resettingVoucher === voucher.id;
                                return (
                                    <button
                                        key={voucher.id}
                                        type="button"
                                        className="hub-service-redeem__button hub-service-redeem__button--voucher"
                                        disabled={!!resettingVoucher}
                                        aria-busy={using || undefined}
                                        aria-describedby={voucherHintId}
                                        onClick={() => void handleResetVoucher(voucher.id)}
                                        title={t("Use this reset voucher to clear current 5-hour and daily limits", "使用此重置券，清除当前五小时及当天限额", "使用此重置券，清除當前五小時及當天限額")}
                                    >
                                        <ResetVoucherIcon />
                                        <span className="hub-service-redeem__voucher-copy">
                                            <span className="hub-service-redeem__voucher-action">
                                                {using ? t("Using...", "使用中...", "使用中...") : t("Use reset voucher", "使用重置券", "使用重置券")}
                                            </span>
                                            <span className="hub-service-redeem__voucher-expiry">
                                                {t(`Expires ${expiry}`, `有效期至 ${expiry}`, `有效期至 ${expiry}`)}
                                            </span>
                                        </span>
                                    </button>
                                );

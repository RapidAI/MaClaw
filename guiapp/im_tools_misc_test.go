...
func TestToolListMCPToolsSkipsManagedRuntimeBlocker(t *testing.T) {
	base := t.TempDir()
	...
	entry := corelib.MCPServerEntry{
		ID: "blocked-managed", Name: "Blocked managed", EndpointURL: "https://example.invalid/mcp",
	...
	result := h.toolListMCPTools(map[string]interface{}{})
	if !strings.Contains(result, "没有已注册的 MCP Server") {
		t.Fatalf("blocked managed MCP should not be listed as ready inventory: %s", result)
	}
}

...
func prepareSkillDirForMarket(skillDir string, autoFix bool, appOpt ...*App) ([]cskill.PortabilityChange, *cskill.PortabilityReport, error) {
	...
	managedSource := false
	managedSkillName := ""
	if autoFix && app != nil && app.skillExecutor != nil {
		cleanDir := filepath.Clean(skillDir)
		for _, existing := range app.skillExecutor.loadSkills() {
			if strings.TrimSpace(existing.SkillDir) != "" && filepath.Clean(existing.SkillDir) == cleanDir {
				managedSource = true
				managedSkillName = strings.TrimSpace(existing.Name)
				break
			}
		}
	}
	if autoFix && app != nil {
		// Portability auto-fix mutates the authoritative directory even for an
		// ad-hoc upload that is not registered yet.  Serialize every such write
		// with installs/deletes, not only managed sources, and re-check the queue
		// after waiting so a newly pending compensation cannot be bypassed.
		...
		if managedSource && managedSkillName != "" {
			if err := ensureSkillEvolutionMutationAdmission(app, managedSkillName); err != nil {
				return nil, nil, err
			}
		} else if err := cskill.CheckEvolutionCompensationQueue(); err != nil {
			return nil, nil, fmt.Errorf("skill portability fix blocked: evolution compensation queue unavailable: %w", err)
		}
	}
	...
	if autoFix {
		...
		if snapshotErr != nil {
			return nil, nil, fmt.Errorf("snapshot skill before portability preflight: %w", snapshotErr)
		}
		defer cleanupSnapshot()
	}
	restoreOnError := func(cause error) error {
		if !autoFix || snapshotDir == "" {
			return cause
		}
		...
		if managedSource && app != nil {
			var restored []corelib.NLSkillEntry
			if app.skillExecutor != nil {
				restored = app.skillExecutor.loadSkills()
			}
			if indexErr := app.refreshSkillIndexesAfterRollbackChecked("", restored); indexErr != nil {
				return fmt.Errorf("%w; changes rolled back; index rollback failed: %v", cause, indexErr)
			}
		}
	...
	if err != nil {
		return nil, nil, restoreOnError(err)
	}
	changes := append([]cskill.PortabilityChange(nil), preflight.AutoFixed...)
	report := preflight.Report
	publishManagedSource := func() error {
		if !managedSource || len(changes) == 0 || report == nil || app == nil {
			return nil
		}
		if indexErr := app.refreshSkillIndexesAfterMutationChecked(report.SkillName); indexErr != nil {
			return restoreOnError(fmt.Errorf("refresh skill index after portability fix: %w", indexErr))
		}
		return nil
	}
	if !preflight.Portable() {
		if preflightHasOnlyQualityGatePathRefs(preflight) {
			if err := publishManagedSource(); err != nil {
	...
	if scanReport, scanErr := scanSkillDirForWriteback(skillDir); scanErr != nil {
		...
		return changes, nil, restoreOnError(scanErr)
	} else if app != nil && app.skillInstallScanShouldBlock(scanReport) {
		return changes, nil, restoreOnError(fmt.Errorf("skill package blocked by security scan: level=%s summary=%s", scanReport.FinalLevel, scanReport.Summary))
	}
	if err := publishManagedSource(); err != nil {
		return changes, nil, err
	}
...
func normalizeInstalledSkillEntry(entry *corelib.NLSkillEntry, appOpt ...*App) *corelib.NLSkillEntry {
	...
	if app := firstSkillLifecycleApp(appOpt...); app != nil {
		// Normalization is not read-only: portability fixes rewrite the
		// authoritative skill.yaml and quality_status.json records the resulting
		// decision.  Apply the same durable queue admission as other lifecycle
		// mutations before touching the directory.  This helper has no error
		// return, so preserve the original entry and surface the block in logs.
		if err := ensureSkillEvolutionMutationAdmission(app, entry.Name); err != nil {
			log.Printf("[skill-lifecycle] normalization blocked for %s: %v", entry.Name, err)
			return entry
		}
	}
	...
	if app := firstSkillLifecycleApp(appOpt...); app != nil {
		// The preflight helper releases its lock before returning. Reacquire it
		// around the quality-status write so a compensation that appears between
		// preflight and evidence persistence cannot leave a stale decision file.
		if app.skillExecutor != nil {
			app.skillExecutor.suspendStatusOverlayPersistence()
			defer app.skillExecutor.resumeStatusOverlayPersistence()
		}
		...
		defer app.installMutex.Unlock()
		if err := ensureSkillEvolutionMutationAdmission(app, entry.Name); err != nil {
			log.Printf("[skill-lifecycle] quality status blocked for %s: %v", entry.Name, err)
			return entry
		}
	}

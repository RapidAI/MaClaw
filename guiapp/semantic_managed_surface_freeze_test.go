...
func TestVisionFallthroughExecutionToolsExcludesSearch(t *testing.T) {
	...
	if attached := handler.attachVisionFallthroughExecutionTools(ctx, nil, nil, "user-1", "文字堆在一起", nil); len(attached) == 0 {
		...
		for _, want := range []string{"write_file", "bash", "read_file", "Glob"} {
			if !names[want] {
	...
	globHistory := []agent.ConversationEntry{{Role: "tool", ToolName: "glob_file_search_tool"}}
	if attached := handler.attachVisionFallthroughExecutionTools(ctx, nil, nil, "user-1", "再看一下这张图", globHistory); len(attached) == 0 {
		t.Fatal("leaked glob_file_search_tool history must still unlock the vision execution surface")
	}
	...
	if attached := handler.attachVisionFallthroughExecutionTools(ctx, nil, nil, "user-1", "如图，第16页的数学公式渲染不对：", nil); len(attached) == 0 {
		t.Fatal("formula rendering critique must attach the no-search execution surface")
	}

...
func (s experienceSkillStore) Register(entry corelib.NLSkillEntry) error {
	app, err := s.automaticMutationApp()
	if err != nil {
		return err
	}
	stampCodingExperienceDomain(&entry)
	entry.TrustLevel = security.TrustLevelAgentCreated
	if app.skillNameAlreadyRegistered(entry.Name) {
		return fmt.Errorf("skill %q already exists", strings.TrimSpace(entry.Name))
	}
	report, err := s.scanBeforePersist(&entry, "register")
	if err != nil {
		return err
	}
	return app.commitNLSkillDefinitionAfterAdmission(
		context.Background(), entry,
		"experience_register", "skill:definition_experience_registered", true,
		"automatic_experience", report,
	)
}
...
func (s experienceSkillStore) Update(entry corelib.NLSkillEntry) error {
	...
	entry.TrustLevel = security.TrustLevelAgentCreated
	report, err := s.scanBeforePersist(&entry, "update")
	if err != nil {
		return err
	}
	return app.commitNLSkillDefinitionAfterAdmission(
		context.Background(), entry,
		"experience_update", "skill:definition_experience_updated", false,
		"automatic_experience", report,
	)
}

// automaticMutationApp is the fail-closed admission point for experience
// extraction. Successful remote sessions trigger this path asynchronously, so
// it must never infer permission from a missing config, a config read failure,
// or a process-wide kill switch that is set after the extractor was created.
func (s experienceSkillStore) automaticMutationApp() (*App, error) {
	if s.executor == nil || s.executor.app == nil {
		return nil, errors.New("experience skill store is not configured")
	}
	if cskill.EvolutionEnvDisabled() {
		return nil, errors.New("automatic experience skill mutation is disabled by environment")
	}
	cfg, err := s.executor.app.LoadConfig()
	if err != nil {
		return nil, fmt.Errorf("automatic experience skill mutation blocked: load skill evolution config: %w", err)
	}
	if !cfg.IsSkillEvolutionEnabled() {
		return nil, errors.New("automatic experience skill mutation is disabled")
	}
	return s.executor.app, nil
}

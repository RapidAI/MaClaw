...
func validateMaclawAppKindContract(entry parsedMaclawAppEntry, path string) error {
	...
	case "automation_app":
		// binding.automation is optional (an unconfigured automation app stays
		// installable), but a present block must parse into a valid schedule.
		if _, err := parseMaclawAppAutomationBindingForEntry(entry, path); err != nil {
			return err
		}
		return nil
	case "":

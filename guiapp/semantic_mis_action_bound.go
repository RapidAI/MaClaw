...
func semanticManagedInvocationRefusal(selection tool.PlannedSelection, canonicalArgs tool.CanonicalRequest) (string, bool) {
	if reason, refused := databaseManagedInvocationRefusal(selection, canonicalArgs); refused {
		return reason, true
	}
...
func semanticManagedMISRefusalText(adapterName, reason string) string {
	if strings.HasPrefix(reason, "database_") {
		return semanticManagedDatabaseRefusalText(adapterName, reason)
	}

...
func TestSkillRunnerStartRunDoesNotWaitForExecutorMutationLock(t *testing.T) {
	...
	// Warm the one-time memory-store initialization before taking the mutation
	// lock. This test measures lock independence, not cold-start embedding/SQLite
	// setup latency; without the warm-up a healthy lock-free start can exceed
	// the assertion window on Windows CI.
...
func TestSkillRunnerTryAutoUploadUsesQualityGateAfterSingleSuccess(t *testing.T) {
	...
	// The auto-upload switch is intentionally opt-in. Enable it explicitly so
	// this test exercises the hash-deduplication path rather than the disabled

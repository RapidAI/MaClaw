...
func (d *CapabilityGapDetector) Resolve(
	...
	// This resolver is invoked after the model detects a capability gap, not by
	// an explicit install command. The review policy keeps such remote package
	// installation disabled until every legacy and non-desktop mutation path has
	// the same durable evidence. Surface a deterministic blocked result so the
	// caller can direct the operator to the Marketplace/Skill management flow.
	if sendStatus != nil {
		sendStatus("自动安装 Skill 当前已关闭；请在 Skills/Marketplace 中人工审核并安装。")
	}
	return "", "", fmt.Errorf("automatic capability-gap installation is disabled; use the reviewed manual install flow")
	defer func() {
			...
			if rollbackErr != nil {
				...
				if clearErr := cskill.ClearEvolutionCompensation(installCompensation.RequestID, installCompensation.Skill, "install"); clearErr != nil {
					...
					if stateErr := cskill.MarkEvolutionCompensationRollbackFailure(&installCompensation, clearErr); stateErr != nil {
						err = fmt.Errorf("%v; install rolled back; compensation cleanup failed: %v; persist compensation: %w", err, clearErr, stateErr)
					} else {
	...
	// Resolve is an automatic installer. Without the desktop App there is no
	// authoritative config/index/audit/compensation owner, so the historical
	// standalone Register fallback must fail closed rather than creating a
	// partially durable Skill.
	if d.app == nil {
		return "", "", fmt.Errorf("capability-gap automatic installation requires the desktop durable transaction owner")
	}
	if err = ensureSkillEvolutionMutationAdmission(d.app, "capability-gap"); err != nil {
		return "", "", err
	}
	if d.skillExecutor == nil {
		return "", "", fmt.Errorf("skill executor not initialized")
	...
	prepareInstallCompensation := func(entry *corelib.NLSkillEntry, finalDir string) error {
		...
		if err := cskill.RecordEvolutionEventStrict("skill:definition_install_started", map[string]string{
			...
			if clearErr := cskill.ClearEvolutionCompensation(requestID, entry.Name, "install"); clearErr != nil {
				installCompensation.LastError = clearErr.Error()
				installCompensation.TransactionState = "audit_pending"
				installCompensation.CleanupStatus = "pending"
				if markErr := cskill.MarkEvolutionCompensationRollbackFailure(&installCompensation, clearErr); markErr != nil {
					return fmt.Errorf("capability-gap install audit preflight failed: %w; clear compensation: %v; persist state: %w", err, clearErr, markErr)
				}
				return fmt.Errorf("capability-gap install audit preflight failed: %w; clear compensation: %v", err, clearErr)
			}
	...
	if len(candidates) == 0 {
		...
		if d.app != nil {
			// GitHub definitions are usually config-only. Route them through the
			// common App adapter so this automatic path inherits the same lock,
			// idempotent short circuit, compensation and checked-index/audit boundary
			// as operator and IM registrations.
			if err := d.app.commitNLSkillDefinitionAfterAdmission(
				ctx, *imported, "install", "skill:definition_installed", true,
				"capability_gap_github", nil,
			); err != nil {
				return "", "", fmt.Errorf("github install not committed: %w", err)
			}
		} else {
			return "", "", fmt.Errorf("github capability-gap install requires the desktop durable transaction owner")
		}
	...
	if strings.TrimSpace(skill.SkillDir) == "" {
		cskill.CleanupStaging(stagingDir)
		return "", "", fmt.Errorf("capability-gap package has no staged skill directory")
	}
	if strings.TrimSpace(skill.SkillDir) != "" {
	...
	return "", "", fmt.Errorf("capability-gap install reached retired direct registration path")
	if d.app != nil {
	...
	if d.app != nil {
		...
		if err := cskill.ClearEvolutionCompensation(requestID, skill.Name, "install"); err != nil {
			if markErr := cskill.MarkEvolutionCompensationCleanupFailure(&installCompensation, err); markErr != nil {
				return "", "", fmt.Errorf("capability-gap install cleanup state could not persist: %w", markErr)
			}

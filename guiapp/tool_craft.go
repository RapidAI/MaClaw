...
func buildCraftSuccessResult(app *App, request craftToolRequest, attempt craftAttemptResult, sendProgress func(string)) string {
	...
	if request.ShouldAutoRegister && app != nil && app.skillExecutor != nil {
		if err := automaticCraftSkillMutationAllowed(app); err != nil {
			...
			result.WriteString("\n")
			// registerCraftedSkillEntry uses the shared definition committer. The
			// generated script already has a durable crafted_tools path, so a second
			// asynchronous package write would create another definition outside the
			// same compensation/audit transaction.
...
func registerCraftedSkillEntry(app *App, task, skillName, scriptPath, language string) string {
	...
	register := func(candidate corelib.NLSkillEntry) (*cskill.ScanReport, error) {
		...
		if app.skillNameAlreadyRegistered(candidate.Name) {
			return report, fmt.Errorf("skill %q already exists", candidate.Name)
		}
		if err := app.commitNLSkillDefinitionAfterAdmission(
			context.Background(), candidate,
			"craft_register", "skill:definition_crafted", true,
			"tool_craft", report,
		); err != nil {
			return report, err
		}

...
func TestSkillLifecycleUploadDirChecksAdmissionBeforePortabilityAutofix(t *testing.T) {
	base := t.TempDir()
	oldBase := corelib.MaclawBaseDir()
	corelib.SetMaclawBaseDir(base)
	t.Cleanup(func() { corelib.SetMaclawBaseDir(oldBase) })
	...
	if err := os.MkdirAll(filepath.Dir(absScript), 0o755); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(absScript, []byte("print('ok')\n"), 0o644); err != nil {
		t.Fatal(err)
	}
	yamlPath := filepath.Join(skillDir, "skill.yaml")
	originalYAML := []byte("name: upload-dir-admission\ndescription: queue gate\nsteps:\n  - action: bash\n    params:\n      command: python " + absScript + "\n")
	if err := os.WriteFile(yamlPath, originalYAML, 0o644); err != nil {
		t.Fatal(err)
	}

	_, err := m.UploadDirNowWithCompletedTargets(context.Background(), "upload-dir-admission", skillDir, "test", false, nil)
	if err == nil || !strings.Contains(err.Error(), "pending evolution compensation") {
		t.Fatalf("UploadDirNow() error = %v, want unreadable queue admission block", err)
	}
	gotYAML, readErr := os.ReadFile(yamlPath)
	if readErr != nil {
		t.Fatal(readErr)
	}
	if string(gotYAML) != string(originalYAML) {
		t.Fatalf("upload admission rewrote skill.yaml before queue check:\n got %s\nwant %s", gotYAML, originalYAML)
	}
}

func TestSkillLifecycleAdHocUploadReceiptChecksCompensationAdmission(t *testing.T) {
	base := t.TempDir()
	oldBase := corelib.MaclawBaseDir()
	corelib.SetMaclawBaseDir(base)
	t.Cleanup(func() { corelib.SetMaclawBaseDir(oldBase) })
	...
	if err := m.persistUploadedSkillReceipt("adhoc-upload", skillDir, "submission-1"); err == nil || !strings.Contains(err.Error(), "compensation queue unavailable") {
		t.Fatalf("persistUploadedSkillReceipt() error = %v, want unreadable queue block", err)
	}
	if _, err := os.Stat(filepath.Join(skillDir, "upload_status.json")); !os.IsNotExist(err) {
		t.Fatalf("blocked ad-hoc receipt created upload_status.json: %v", err)
	}
	if err := writeSkillUploadStatusAtomic(skillDir, " "); err == nil || !strings.Contains(err.Error(), "submission ID is required") {
		t.Fatalf("empty submission ID error = %v, want validation failure", err)
	}
}

func TestPrepareSkillDirForMarketBlocksAdHocAutofixOnUnreadableQueue(t *testing.T) {
	base := t.TempDir()
	oldBase := corelib.MaclawBaseDir()
	corelib.SetMaclawBaseDir(base)
	t.Cleanup(func() { corelib.SetMaclawBaseDir(oldBase) })
	...
	skillDir := t.TempDir()
	yamlPath := filepath.Join(skillDir, "skill.yaml")
	original := []byte("name: adhoc-preflight\nsteps:\n  - action: bash\n    params:\n      command: echo ok\n")
	...
	if _, _, err := prepareSkillDirForMarket(skillDir, true, app); err == nil || !strings.Contains(err.Error(), "compensation queue unavailable") {
		t.Fatalf("prepareSkillDirForMarket() error = %v, want unreadable queue block", err)
	}
	...
	if string(got) != string(original) {
		t.Fatalf("blocked ad-hoc preflight rewrote skill.yaml\n got: %q\nwant: %q", got, original)
	}
}
...
func TestNormalizeInstalledSkillEntryBlocksOnUnreadableCompensationQueue(t *testing.T) {
	tempHome := t.TempDir()
	...
	dir := filepath.Join(tempHome, "skills", "normalize-queue-guard")
	...
	yamlData := []byte("name: normalize-queue-guard\ndescription: queue guard\nsteps:\n  - action: bash\n    params:\n      command: python " + absScript + "\n")
	if err := os.WriteFile(filepath.Join(dir, "skill.yaml"), yamlData, 0o644); err != nil {
		t.Fatal(err)
	}
	...
	normalizeInstalledSkillEntry(entry, app)
	gotYAML, err := os.ReadFile(filepath.Join(dir, "skill.yaml"))
	if err != nil {
		t.Fatal(err)
	}
	if string(gotYAML) != string(yamlData) {
		t.Fatalf("normalization changed skill.yaml while queue unreadable:\\n got: %q\\nwant: %q", string(gotYAML), string(yamlData))
	}
	if _, err := os.Stat(filepath.Join(dir, "quality_status.json")); !os.IsNotExist(err) {
		t.Fatalf("normalization wrote quality status while queue unreadable: %v", err)
	}
}
...
func TestPrepareUploadSourceWithCheckedIndexRollsBack(t *testing.T) {
	home := t.TempDir()
	app := &App{testHomeDir: home}
	skillDir := filepath.Join(home, "skills", "source-index-rollback")
	...
	absScript := filepath.Join(skillDir, "scripts", "run.py")
	yaml := "name: source-index-rollback\nsteps:\n  - action: bash\n    params:\n      command: python " + absScript + "\n"
	yamlPath := filepath.Join(skillDir, "skill.yaml")
	if err := os.WriteFile(yamlPath, []byte(yaml), 0o644); err != nil {
		t.Fatal(err)
	}
	if err := app.SaveConfig(corelib.AppConfig{NLSkills: []corelib.NLSkillEntry{{Name: "source-index-rollback", SkillDir: skillDir}}}); err != nil {
		t.Fatal(err)
	}
	app.skillExecutor = NewSkillExecutor(app, nil, nil)
	...
	m := NewSkillLifecycleManager(app)
	if err := m.prepareUploadSourceWithCheckedIndex("source-index-rollback", skillDir); err == nil || !strings.Contains(err.Error(), "已回滚") && !strings.Contains(err.Error(), "rolled back") {
		t.Fatalf("prepareUploadSourceWithCheckedIndex() = %v, want rollback error", err)
	}
	restored, err := os.ReadFile(yamlPath)
	if err != nil {
		t.Fatal(err)
	}
	if string(restored) != yaml {
		t.Fatalf("source skill.yaml was not restored\n got: %q\nwant: %q", restored, yaml)
	}
	// The lower-level market preflight is also used by directory/queue uploads;
	// it must apply the same checked publication rule for a managed source.
	refreshCalls = 0
	if _, _, err := prepareSkillDirForMarket(skillDir, true, app); err == nil || !strings.Contains(err.Error(), "rolled back") {
		t.Fatalf("prepareSkillDirForMarket() = %v, want checked-index rollback error", err)
	}
	restored, err = os.ReadFile(yamlPath)
	if err != nil {
		t.Fatal(err)
	...
	if string(restored) != yaml {
		t.Fatalf("market preflight did not restore source skill.yaml\n got: %q\nwant: %q", restored, yaml)
	}
}
...
func TestSkillLifecycleUploadDirReturnsSubmissionIDWhenLocalReceiptFails(t *testing.T) {
	originalDefaultCenter := defaultRemoteHubCenterURL
	originalDefaultCenters := remote.DefaultRemoteHubCenterURLs
	defaultRemoteHubCenterURL = ""
	remote.DefaultRemoteHubCenterURLs = nil
	...
	app := &App{testHomeDir: t.TempDir()}
	skillDir := filepath.Join(t.TempDir(), "receipt-failure-skill")
	writeLifecycleTestSkill(t, skillDir, "receipt-failure-skill")
	if err := os.WriteFile(filepath.Join(skillDir, "SKILL.md"), []byte("# Receipt failure skill\n\nPortable fixture for upload receipt recovery.\n"), 0o644); err != nil {
		t.Fatalf("WriteFile(SKILL.md) error = %v", err)
	}
	...
	server = httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "/api/client/hubcenters":
			_ = json.NewEncoder(w).Encode(map[string]any{"ok": true, "urls": []string{server.URL}})
		case "/api/v1/skills/submit":
			// Simulate a post-submit local filesystem failure. The remote receipt
			// is already accepted, but its local status file can no longer be made.
			if err := os.RemoveAll(skillDir); err != nil {
				t.Errorf("RemoveAll(skillDir) error = %v", err)
				http.Error(w, err.Error(), http.StatusInternalServerError)
				return
			}
			if err := os.WriteFile(skillDir, []byte("not a directory"), 0o644); err != nil {
				t.Errorf("WriteFile(skillDir) error = %v", err)
				http.Error(w, err.Error(), http.StatusInternalServerError)
				return
			}
			_ = json.NewEncoder(w).Encode(map[string]any{"submission_id": "sub-receipt-failure"})
		default:
			http.NotFound(w, r)
		}
	}))
	...
	submissionID, err := m.UploadDirNow(context.Background(), "receipt-failure-skill", skillDir, "test", false)
	if submissionID != "sub-receipt-failure" {
		t.Fatalf("submissionID = %q, want remote submission id", submissionID)
	}
	var localState *skillUploadLocalStateError
	if !errors.As(err, &localState) {
		t.Fatalf("UploadDirNow() error = %v, want skillUploadLocalStateError", err)
	}
	if localState.SubmissionID != submissionID || !strings.Contains(err.Error(), "local upload state was not persisted") {
		t.Fatalf("local state error = %+v (%v)", localState, err)
	}
}

func TestSkillLifecycleRetriesRemoteSubmittedReceiptWithoutResubmitting(t *testing.T) {
	app := &App{testHomeDir: t.TempDir()}
	skillDir := filepath.Join(t.TempDir(), "remote-submitted-receipt")
	writeLifecycleTestSkill(t, skillDir, "remote-submitted-receipt")
	m := NewSkillLifecycleManager(app)
	now := time.Now().Add(-time.Minute).Format(time.RFC3339)
	item := SkillUploadQueueItem{
		ID:            "remote-submitted-receipt",
		SkillName:     "remote-submitted-receipt",
		SkillDir:      skillDir,
		Status:        skillUploadStatusRemoteSubmitted,
		Attempts:      1,
		SubmissionID:  "sub-remote-already-accepted",
		LastError:     "prior local receipt write failed",
		NextAttemptAt: now,
		CreatedAt:     now,
		UpdatedAt:     now,
	}
	...
	if items[0].Status != skillUploadStatusUploaded || items[0].SubmissionID != item.SubmissionID {
		t.Fatalf("queue item = %+v, want locally-finalized submitted item", items[0])
	}
	statusData, err := os.ReadFile(filepath.Join(skillDir, "upload_status.json"))
	if err != nil {
		t.Fatalf("ReadFile(upload_status.json) error = %v", err)
	}
	var status uploadStatusFile
	if err := json.Unmarshal(statusData, &status); err != nil {
		t.Fatalf("Unmarshal(upload_status.json) error = %v", err)
	}
	if status.SubmissionID != item.SubmissionID || strings.TrimSpace(status.UploadedAt) == "" {
...
func TestCleanupImportedSkillDirsRemovesInstalledDirs(t *testing.T) {
	...
	if err := cleanupImportedSkillDirs([]string{first, second}); err != nil {
		t.Fatalf("cleanupImportedSkillDirs: %v", err)
	}
...
func TestSkillLifecycleRetryBlockedAllKeepsPendingSkillBlocked(t *testing.T) {
	base := t.TempDir()
	oldBase := corelib.MaclawBaseDir()
	corelib.SetMaclawBaseDir(base)
	t.Cleanup(func() { corelib.SetMaclawBaseDir(oldBase) })
	...
	m := NewSkillLifecycleManager(app)
	m.queuePath = filepath.Join(t.TempDir(), "queue.json")
	m.recordBlocked("blocked-with-recovery", "", "test", "hash-a", false, "needs verification", 42)
	m.recordBlocked("blocked-ready", "", "test", "hash-b", false, "needs verification", 42)
	if err := skill.PersistEvolutionCompensation(skill.NewEvolutionCompensationRecord(
		"req-retry-all", "blocked-with-recovery", "repair", "", nil, false, nil, "rollback_incomplete",
	)); err != nil {
		t.Fatal(err)
	}
	if err := m.RetryBlocked(""); err != nil {
		t.Fatalf("RetryBlocked(all) error = %v", err)
	}
	items, err := m.ListUploadQueue()
	if err != nil {
		t.Fatal(err)
	...
	if len(items) != 2 {
		t.Fatalf("queue after retry-all = %+v", items)
	}
	byName := make(map[string]SkillUploadQueueItem, len(items))
	for _, item := range items {
		byName[item.SkillName] = item
	}
	if item := byName["blocked-with-recovery"]; item.Status != skillUploadStatusBlocked || !strings.Contains(item.LastError, "pending evolution compensation") {
		t.Fatalf("pending-recovery item after retry-all = %+v", item)
	}
	if item := byName["blocked-ready"]; item.Status != skillUploadStatusPending || item.LastError != "" {
		t.Fatalf("ready item after retry-all = %+v", item)
	}
}
...
func TestAuditInstalledSkillQualityBlocksBeforePortabilityAutofixOnUnreadableQueue(t *testing.T) {
	base := t.TempDir()
	oldBase := corelib.MaclawBaseDir()
	corelib.SetMaclawBaseDir(base)
	t.Cleanup(func() { corelib.SetMaclawBaseDir(oldBase) })
	...
	skillDir := filepath.Join(app.testHomeDir, "skills", "audit-admission")
	if err := os.MkdirAll(filepath.Join(skillDir, "scripts"), 0o755); err != nil {
		t.Fatal(err)
	}
	absScript := filepath.Join(skillDir, "scripts", "run.py")
	...
	yamlPath := filepath.Join(skillDir, "skill.yaml")
	originalYAML := []byte("name: audit-admission\ndescription: queue gate\nsteps:\n  - action: bash\n    params:\n      command: python " + absScript + "\n")
	if err := os.WriteFile(yamlPath, originalYAML, 0o644); err != nil {
		t.Fatal(err)
	}
	cfg, err := app.LoadConfig()
	...
	cfg.NLSkills = []corelib.NLSkillEntry{{Name: "audit-admission", SkillDir: skillDir, Source: "file", UsageCount: 1, SuccessCount: 1}}
	if err := app.SaveConfig(cfg); err != nil {
		t.Fatal(err)
	}
	queuePath := skill.DefaultEvolutionCompensationPath()
	...
	if _, err := app.AuditInstalledSkillQuality(false); err == nil || !strings.Contains(err.Error(), "compensation queue unavailable") {
		t.Fatalf("AuditInstalledSkillQuality() error = %v, want queue admission block", err)
	}
	gotYAML, readErr := os.ReadFile(yamlPath)
	if readErr != nil {
		t.Fatal(readErr)
	...
	if string(gotYAML) != string(originalYAML) {
		t.Fatalf("quality audit rewrote skill.yaml before queue check:\n got %s\nwant %s", gotYAML, originalYAML)
	}
	if _, statErr := os.Stat(filepath.Join(skillDir, "quality_status.json")); !os.IsNotExist(statErr) {
		t.Fatalf("quality status was written despite queue block: %v", statErr)
	}
}

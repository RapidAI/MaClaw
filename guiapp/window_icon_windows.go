...
const (
	...
	iconSmall      = 0
	iconBig        = 1
	// Wails/WebView2 can create the top-level HWND several seconds after
	// startup while the environment check is running. Keep retrying long
	// enough to cover that window-creation race instead of leaving the
	// taskbar entry with the default blank icon.
	windowIconTries = 100
)

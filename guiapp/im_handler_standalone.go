...
type StandaloneConfig struct {
	// SharedAgentRuntime optionally supplies the transport-neutral Runtime from
	// a composition root. When nil, the handler installs its compatibility
	// adapter lazily, preserving existing standalone callers.
	SharedAgentRuntime agentruntime.Runtime
...
func NewIMMessageHandlerStandalone(cfg StandaloneConfig) *IMMessageHandler {
	...
	h := &IMMessageHandler{
		...
		sharedRuntime:     cfg.SharedAgentRuntime,
	}

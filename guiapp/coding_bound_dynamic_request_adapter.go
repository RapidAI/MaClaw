...
type codingBoundDynamicRequestAdapter struct {
	...
	terminal   bool
	// successorParent, when set, is the durable lineage-current revision this
	// reservation succeeds. The lifecycle relay installs it from the
	// coordinator record before the first render; the holder then publishes its
	// surface as a child revision of that lineage instead of opening an
	// independent root. It never carries predecessor grants, aliases, or
	// in-memory state.
	successorParent *tool.RouteRevisionRef
...
// SurfaceAliasesForTesting returns a copy of the published surface aliases for
// hermetic lifecycle tests. Production dispatch must go through
// beginBoundToolCall so admission stays reservation-bound.
func (a *codingBoundDynamicRequestAdapter) SurfaceAliasesForTesting() map[string]tool.InvocationGrant {
	if a == nil {
		return nil
	}
	a.mu.Lock()
	...
	out := make(map[string]tool.InvocationGrant, len(a.surface.aliases))
	for name, grant := range a.surface.aliases {
		out[name] = grant
	}
	return out
}
...
// setSuccessorParentRevision installs the durable lineage parent for a
// successor reservation. It is accepted only before the first render: a
// terminal or already-published holder ignores it, so a late lifecycle write
// can never re-scope an existing surface.
func (a *codingBoundDynamicRequestAdapter) setSuccessorParentRevision(parent tool.RouteRevisionRef) {
	if a == nil {
		return
	}
	a.mu.Lock()
...
func (a *codingBoundDynamicRequestAdapter) RenderPublishedBoundToolSurface(_ string, _ int, execution agent.ToolCallExecutionContext) agent.BoundToolSurfaceRender {
	...
	var surface *codingDurableDynamicSurface
	var err error
	if a.successorParent != nil {
		surface, err = a.handler.app.publishCodingDurableDynamicSurfaceChildForEpoch(a.identity, a.prepared, a.dynamic, a.protocol, a.connection, a.epoch, *a.successorParent, time.Now().UTC())
	} else {

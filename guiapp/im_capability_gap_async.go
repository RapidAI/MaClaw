...
// Automatic capability-gap installation remains disabled until every remote
// source and runtime has the same durable install, recovery, and final-audit
// evidence. Keep this as an explicit rollout fence so a confirmation dialog
// cannot accidentally re-enable the background mutation path.
const automaticCapabilityGapInstallEnabled = false
...
func (h *IMMessageHandler) maybeStartAsyncCapabilityGapSearch(ctx *LoopContext, iteration int, visibleContent, msgContent string, truncatedToolCount int, userText, userID string, totalToolCallsInLoop int, phase *agentLoopPhase) {
	if !automaticCapabilityGapInstallEnabled ||
		!shouldStartAsyncCapabilityGapSearch(iteration, visibleContent, truncatedToolCount, totalToolCallsInLoop, phase) ||
...
func (h *IMMessageHandler) runAsyncCapabilityGapSearch(userText, gapText, recoverReason, platform, userID, policyOwnerID string) {
	if !automaticCapabilityGapInstallEnabled {
		log.Printf("[skill-auto-async] capability-gap installation disabled by rollout fence")
		return
	}

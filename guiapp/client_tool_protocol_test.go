...
func TestBuildClientToolPromptRequiresESPAlarmToolCall(t *testing.T) {
	...
	wantShared := agentruntime.BuildClientToolPrompt(agentruntime.ClientToolPromptRequest{ToolNames: []string{"alarm_create", "alarm_clear"}, ClientID: "bread-compact"})
	if prompt != wantShared {
		t.Fatalf("GUI adapter drifted from shared runtime prompt:\ngot=%s\nwant=%s", prompt, wantShared)
	}

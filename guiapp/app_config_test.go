...
func TestPreserveBackendOwnedFieldsAllowsInitialHubCenterCandidates(t *testing.T) {
	// A nil on-disk slice means the backend has not persisted a candidate list
	// yet; the first full-config save must be allowed to establish it.
	ondisk := corelib.AppConfig{RemoteHubCenterURLs: nil}
	incoming := corelib.AppConfig{RemoteHubCenterURLs: []string{"http://127.0.0.1:1", "http://127.0.0.1:2"}}

	preserveBackendOwnedFields(&incoming, &ondisk)

	if !reflect.DeepEqual(incoming.RemoteHubCenterURLs, []string{"http://127.0.0.1:1", "http://127.0.0.1:2"}) {
		t.Fatalf("initial HubCenter candidates were unexpectedly discarded: %#v", incoming.RemoteHubCenterURLs)
	}
}

func TestPreserveBackendOwnedFieldsKeepsPersistedHubCenterCandidates(t *testing.T) {
	// Once the backend has a persisted list, stale frontend snapshots must not
	// overwrite failover state maintained by discovery/selection workers.
	ondisk := corelib.AppConfig{RemoteHubCenterURLs: []string{"http://127.0.0.1:9"}}
	incoming := corelib.AppConfig{RemoteHubCenterURLs: []string{"http://127.0.0.1:1"}}

	preserveBackendOwnedFields(&incoming, &ondisk)

	if !reflect.DeepEqual(incoming.RemoteHubCenterURLs, ondisk.RemoteHubCenterURLs) {
		t.Fatalf("persisted HubCenter candidates were not protected: got %#v want %#v", incoming.RemoteHubCenterURLs, ondisk.RemoteHubCenterURLs)
	}
}
...
func TestPatchConfigFieldsSkillEvolutionEnabled(t *testing.T) {
	...
	if st.EnableRepair || st.EnableOptimizer || st.EnablePromoter {
		t.Fatalf("automatic pipeline stages unexpectedly enabled by default: %+v", st)
	}
	...
	st = app.GetSkillEvolutionStatus()
	if st.ConfigDisabled || !st.ConfigEnabled || st.Disabled || !st.EnableRepair || !st.EnableOptimizer {
		t.Fatalf("status after re-enable: %+v", st)
	}
	if st.EnablePromoter || st.HasPromoter {
		t.Fatalf("unsafe automatic nudge promotion was wired after opt-in: %+v", st)
	}
	...
	if err != nil {
		t.Fatalf("disable PatchConfigFields() error = %v", err)
	}
	if patched.IsSkillEvolutionEnabled() {
		t.Fatal("expected skill evolution disabled after patch")
	}
	st = app.GetSkillEvolutionStatus()
	if !st.ConfigDisabled || st.ConfigEnabled || !st.Disabled || st.EnableRepair || st.EnableOptimizer || st.EnablePromoter {
...
func TestGetSkillEvolutionStatus(t *testing.T) {
	...
	if !st.ConfigDisabled || st.ConfigEnabled || !st.Disabled {
		t.Fatalf("expected opt-in evolution disabled by default: %+v", st)
	}
	if st.EnableRepair || st.EnableOptimizer || st.EnablePromoter || st.HasPromoter {
		t.Fatalf("automatic stages unexpectedly enabled by default: %+v", st)
	}
	if st.RepairCooldown == "" {
		t.Fatal("expected non-empty repair_cooldown string")
	}
	if st.EnvDisabled {
		t.Fatal("expected empty kill switch")
	}

	if _, err := app.PatchConfigFields(map[string]interface{}{"skill_evolution_enabled": true}); err != nil {
		t.Fatalf("PatchConfigFields(enable) error = %v", err)
	}
	st = app.GetSkillEvolutionStatus()
	if st.Disabled || !st.EnableRepair || !st.EnableOptimizer {
		t.Fatalf("expected enabled repair and optimizer after explicit opt-in: %+v", st)
	}
	if st.EnablePromoter || st.HasPromoter {
		t.Fatalf("automatic nudge promotion must stay unavailable: %+v", st)
	}
...
func TestPatchConfigFieldsSemanticRoutingPartialObject(t *testing.T) {
	tmpHome := t.TempDir()
	t.Setenv("USERPROFILE", tmpHome)
	t.Setenv("HOME", tmpHome)
	app := &App{testHomeDir: tmpHome}
	patched, err := app.PatchConfigFields(map[string]interface{}{
		"semantic_tool_scope_routing": map[string]interface{}{"enabled": true, "mode": "scope_only"},
	})
	if err != nil {
		t.Fatal(err)
	}
	policy := patched.EffectiveRoutingPolicy()
	if !policy.Valid || !policy.AllowsExecutableScope() {
		t.Fatalf("partial semantic policy patch should retain defaults: %#v", policy)
	}
}

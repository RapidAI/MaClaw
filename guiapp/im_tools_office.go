...
func (h *IMMessageHandler) toolOffice(args map[string]interface{}) string {
	...
	case "preview_pptx":
		return agent.ToolPreviewPPTX(args)
	default:
		return fmt.Sprintf("未知的 office action: %q。支持的 action: generate_pdf, read_document, read_doc, read_docx, read_pdf, read_excel, write_excel, read_pptx, write_pptx, preview_pptx", action)
	}
}

...
func TestDeriveLLMTokenUsage_DoesNotEstimateWhenTotalOnlyReported(t *testing.T) {
	var usage llm.Usage
	...
	resp := &llm.Response{
		Choices: []llm.Choice{{
			Message: llm.Message{Content: "this must not inflate the reported output"},
		}},
		Usage: &usage,
	}
	input, output := deriveLLMTokenUsage(resp, []interface{}{
		map[string]interface{}{"role": "user", "content": "please summarize this request in detail"},
	})
	if input != 18 || output != 0 {
		t.Fatalf("usage = input:%d output:%d, want 18/0 without local estimate", input, output)
	}
}

func TestDeriveLLMTokenUsage_DoesNotEstimateExplicitZeros(t *testing.T) {
	var usage llm.Usage
	if err := json.Unmarshal([]byte(`{"prompt_tokens":0,"completion_tokens":0,"total_tokens":0}`), &usage); err != nil {
		t.Fatalf("unmarshal: %v", err)
	}
	resp := &llm.Response{
		Choices: []llm.Choice{{
			Message: llm.Message{Content: "this must not be estimated as output"},
		}},
		Usage: &usage,
	}
	input, output := deriveLLMTokenUsage(resp, []interface{}{
	...
	if input != 0 || output != 0 {
		t.Fatalf("usage = input:%d output:%d, want 0/0 without local estimate", input, output)
	}
}

...
// UnmarshalJSON keeps the legacy Hub review status while exposing the shared
// Runtime lifecycle vocabulary to both hosts and frontend clients.
func (v *DigitalAssetSubmissionView) UnmarshalJSON(data []byte) error {
	type alias DigitalAssetSubmissionView
	var decoded struct {
		alias
		RuntimeStatus agentruntime.JobStatus `json:"runtime_status"`
	}
	...
	*v = DigitalAssetSubmissionView(decoded.alias)
	if decoded.RuntimeStatus != "" {
		v.RuntimeStatus = agentruntime.NormalizeJobStatus(decoded.RuntimeStatus)
	} else {
		v.RuntimeStatus = agentruntime.ProjectLegacyJobStatus(v.Status)
	}
	return nil
}

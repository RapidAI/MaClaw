...
// Legacy adapters that own recovery (for example App.AddSkill/InstallSkill)
// must replay their scoped records before calling this admission-only helper;
// otherwise a corrupt rollback would be short-circuited before its bounded
// retry budget can advance to needs_review.
func ensureSkillEvolutionMutationAdmission(a *App, skillName string) error {
	...
	if a != nil {
		...
		// Mutation APIs accept the same stable aliases as run_skill (SkillID,
		// HubSkillID, publisher-qualified IDs and directory names). Resolve the
		// alias to the canonical registry entry before checking the durable queue;
		// otherwise a pending record keyed by the canonical Name could be bypassed
		// simply by submitting an alias.
		if a.skillExecutor != nil {
			for _, entry := range a.skillExecutor.loadSkills() {
				if !entry.MatchesName(skillName) {
					continue
				}
				aliases := []string{entry.Name, entry.SkillID, entry.HubSkillID, entry.DirName}
				if strings.TrimSpace(entry.Publisher) != "" {
					aliases = append(aliases, entry.Publisher+":"+entry.Name)
				}
				for _, alias := range aliases {
					if strings.TrimSpace(alias) != "" && pipeline.HasPendingCompensation(alias) {
						return fmt.Errorf("skill activation blocked: pending evolution compensation for skill %q", alias)
					}
				}
				break
			}
...
func (a *App) recordSkillEvolutionEventStrict(event string, data map[string]string) error {
	if a != nil && a.evolutionAuditWriter != nil {
		return a.evolutionAuditWriter(event, data)
	}
...
func (e *SkillExecutor) loadSkills() []corelib.NLSkillEntry {
	...
	// Keep a successfully loaded empty registry non-nil so durable committers
	// can distinguish it from a loader failure and capture an authoritative
	// empty config pre-image for crash recovery.
	skills := make([]corelib.NLSkillEntry, 0, len(nlSkills))
	skills = append(skills, nlSkills...)
...
func (e *SkillExecutor) persistSkillStatusOverlays(skills []corelib.NLSkillEntry) error {
	...
	// Load-time reconciliation runs asynchronously, but it still mutates the
	// authoritative registry.  Serialize it with installs/deletes and re-check
	// the queue while holding the same process-wide lock; otherwise an overlay
	// captured before a transaction could be written after that transaction has
	// published a pending compensation record.
	e.app.installMutex.Lock()
	defer e.app.installMutex.Unlock()
	// loadSkills may discover a broken/retired definition and schedule this
	// overlay write asynchronously.  That write is still an authoritative
	// registry mutation; never let it proceed while the durable evolution queue
	// is unreadable or while any of the captured Skills has pending recovery.
	if err := skill.CheckEvolutionCompensationQueue(); err != nil {
		return fmt.Errorf("skill status overlay blocked: evolution compensation queue unavailable: %w", err)
	}
	pipeline := e.app.evolutionPipeline
	if pipeline == nil {
		pipeline = skill.NewEvolutionPipeline()
	}
	for _, s := range skills {
		if name := strings.TrimSpace(s.Name); name != "" && pipeline.HasPendingCompensation(name) {
			return fmt.Errorf("skill status overlay blocked: pending evolution compensation for skill %q", name)
		}
	}
...
func (e *SkillExecutor) Register(entry corelib.NLSkillEntry) error {
	...
	// Register is retained for compatibility, but it is still an authoritative
	// registry mutation.  Do the same durable compensation admission check as
	// the App-owned lifecycle APIs before taking any write lock or reading a
	// potentially stale list.  Detached executors are test/legacy read-only
	// shims and have no durable queue context, so they keep their in-memory
	// behavior.
	if e.app != nil {
		// Drain asynchronous load-time overlay writers before taking the
		// process-wide mutation lock.  Overlay persistence takes this lock
		// itself; reversing the order can deadlock a writer that is already
		// queued behind installMutex.
		e.suspendStatusOverlayPersistence()
		defer e.resumeStatusOverlayPersistence()
		if err := ensureSkillEvolutionMutationAdmission(e.app, name); err != nil {
			return err
		}
	}
	// Legacy executor callers still mutate the authoritative registry directly.
	// Fence them with the App-wide install mutex so they cannot race a package
	// publication/delete/update transaction.  The shared experience store and
	// Wails definition APIs do not call this method, so there is no nested lock.
	if e.app != nil {
		e.app.installMutex.Lock()
		defer e.app.installMutex.Unlock()
		// Re-check after waiting for the process-wide mutation lock.  A
		// concurrent installer may have persisted a compensation record while
		// this compatibility call was queued; the pre-lock admission result is
		// then stale and must not authorize a registry write.
		if err := ensureSkillEvolutionMutationAdmission(e.app, name); err != nil {
			return err
		}
	}
...
func (e *SkillExecutor) UpdateLearnedSource(entry corelib.NLSkillEntry) error {
	...
	e.app.installMutex.Lock()
	defer e.app.installMutex.Unlock()
	// The admission check above happens before lock acquisition to fail fast;
	// repeat it after the lock so a queue transition while waiting cannot be
	// bypassed by this legacy overlay writer.
	if err := ensureSkillEvolutionMutationAdmission(e.app, name); err != nil {
		return err
	}
	e.skillListMutateMu.Lock()
...
// Delete is retained for legacy in-process callers. It must not perform a
// standalone config-plus-filesystem delete: route through App.DeleteNLSkill so
// compensation, checked index publication, and final audit remain mandatory.
func (e *SkillExecutor) Delete(name string) error {
	if e == nil || e.app == nil || e.app.skillExecutor != e {
		return fmt.Errorf("durable skill deletion is not configured")
	}
	return e.app.DeleteNLSkill(name)
}
...
// Rename changes the name of a skill through the App-owned durable transaction.
// Detached executors are rejected so this compatibility surface cannot bypass
// the shared directory/config/index/audit compensation boundary.
func (e *SkillExecutor) Rename(oldName, newName string) error {
	// This method is retained for compatibility, but it must never perform the
	// historical best-effort YAML/directory/config writes itself. Only the
	// App-owned executor has enough context to publish checked indexes, strict
	// audit events, and durable compensation. Refuse detached or stale executor
	// references instead of silently bypassing that transaction boundary.
	if e == nil || e.app == nil {
		return fmt.Errorf("skill executor app not initialized")
	}
	if e.app.skillExecutor != e {
		return fmt.Errorf("skill executor is not the app-owned executor")
	}
	return e.app.RenameNLSkill(oldName, newName)
}
...
// For file-based skills, it atomically writes upload_status.json next to skill.yaml
...
func (e *SkillExecutor) MarkUploaded(name, submissionID string) error {
	...
	if e.app != nil {
		e.app.installMutex.Lock()
		defer e.app.installMutex.Unlock()
		// Upload receipts are authoritative local mutations.  Re-check after
		// lock acquisition so a newly pending compensation cannot be bypassed
		// by a stale remote submission callback.
		if err := ensureSkillEvolutionMutationAdmission(e.app, name); err != nil {
			return err
		}
	}
	...
	for i, s := range skills {
		...
		if normalizeSkillEntrySource(s.Source) == skillEntrySourceFile && s.SkillDir != "" {
			// File-based skill: atomically persist upload metadata before updating
			// the in-memory scanner. A partially-written status file could otherwise
			// make a successful remote submission disappear after restart.
			if err := writeSkillUploadStatusAtomic(s.SkillDir, submissionID); err != nil {
				return err
			}
...
// writeSkillUploadStatusAtomic persists the file-backed upload receipt as one
// replace operation. It is shared by registered and ad-hoc directory uploads
// so both paths have the same restart-safe on-disk representation.
func writeSkillUploadStatusAtomic(skillDir, submissionID string) error {
	...
	if err != nil {
		return fmt.Errorf("marshal upload status: %w", err)
	}
	if err := atomicWriteFile(filepath.Join(skillDir, "upload_status.json"), data); err != nil {
		return fmt.Errorf("write upload status: %w", err)
	}
	return nil
}
...
func (e *SkillExecutor) executeSkillStepsDetailed(entry *corelib.NLSkillEntry, runArgs map[string]interface{}) skillExecutionResult {
	...
	needsProxy := corelib.NeedsOpenAIProxyAuto(proxyRequiredEnv, extraEnv, proxyProbeSteps, preparedEntry.SkillDir, preparedEntry.NoLLMAPI)
...
func (e *SkillExecutor) recordSkillExecution(matches func(corelib.NLSkillEntry) bool, output string, execErr error) {
	// Usage counters are persisted config state, not an in-memory-only metric.
	// Serialize them with installs/deletes and fail closed when the durable
	// compensation queue is unavailable; otherwise a post-run callback could
	// rewrite config while a failed transaction is still awaiting recovery.
	if e.app != nil {
		e.app.installMutex.Lock()
		defer e.app.installMutex.Unlock()
		if err := skill.CheckEvolutionCompensationQueue(); err != nil {
			log.Printf("[skill-execution] skip usage persistence: evolution compensation queue unavailable: %v", err)
			return
		}
	}
	...
	for i, s := range skills {
		if matches(s) {
			if e.app != nil {
				if err := ensureSkillEvolutionMutationAdmission(e.app, s.Name); err != nil {
					log.Printf("[skill-execution] skip usage persistence for %s: %v", s.Name, err)
					// The predicate may match aliases. Once the selected canonical
					// entry is blocked, do not fall through to another alias match
					// and accidentally credit a different registry row.
					break
				}
			}
			skills[i].UsageCount++
...
func (e *SkillExecutor) executeBoundSkill(stableID, name, version, contentDigest string, arguments map[string]interface{}) (string, error) {
	...
	if e.app != nil {
		e.app.installMutex.Lock()
		if err := ensureSkillEvolutionMutationAdmission(e.app, name); err != nil {
			e.app.installMutex.Unlock()
			log.Printf("[skill-execution] skip bound usage persistence for %s: %v", name, err)
			return output, execErr
		}
	}
	e.skillListMutateMu.Lock()
...
// Serializes the read/compare/write cycle for app manifests. Without this, two
// simultaneous successful runs of a newly created app can both observe missing
// evidence and each rewrite the manifest, creating duplicate package versions.
var maclawAppRunEvidenceManifestMu sync.Mutex
...
func recordMaclawSingleAppRunEvidence(skillName string, skillDir string, appID string, evidence map[string]any, artifactName string) (map[string]any, error) {
	maclawAppRunEvidenceManifestMu.Lock()
	defer maclawAppRunEvidenceManifestMu.Unlock()
	...
	// Runtime evidence contains a unique run ID and timestamp. Rewriting the
	// app definition for every successful execution changes the packaged app
	// checksum, which makes Hub/HubCenter create a needless new version. One
	// successful proof is sufficient for an unchanged definition; detailed
	// per-run evidence is stored separately in app_run_history.json.
	if maclawAppRunEvidenceAlreadyProvesDefinition(maclawAppExistingRunEvidence(governance), evidence) {
		result := maclawAppRunEvidenceResult(skillName, "maclaw.app.json", data, evidence, artifactName)
	...
	result := maclawAppRunEvidenceResult(skillName, "maclaw.app.json", out, evidence, artifactName)
...
func recordMaclawAppsRunEvidence(skillName string, skillDir string, appID string, evidence map[string]any, artifactName string) (map[string]any, error) {
	...
	// See recordMaclawSingleAppRunEvidence: test evidence is mutable runtime
	// state and must not continuously invalidate an otherwise unchanged app
	// package. The durable run-history store keeps the later run details.
	if maclawAppRunEvidenceAlreadyProvesDefinition(maclawAppExistingRunEvidence(manifest.Apps[targetIndex].Governance), evidence) {
		result := maclawAppRunEvidenceResult(skillName, "maclaw.apps.json", data, evidence, artifactName)
		result["manifest_updated"] = false
		return result, nil
	}
	...
	result := maclawAppRunEvidenceResult(skillName, "maclaw.apps.json", out, evidence, artifactName)
	result["manifest_updated"] = true
	return result, nil
}

// maclawAppRunEvidenceAlreadyProvesDefinition returns true when the manifest
// already has a complete successful proof for the same immutable definition.
// Only definition changes need a new manifest-level proof; later executions
// are recorded in the durable run history instead.
func maclawAppRunEvidenceAlreadyProvesDefinition(existing any, incoming map[string]any) bool {
	current, ok := existing.(map[string]any)
	if !ok || len(current) == 0 {
		return false
	}
	definitionHash := strings.TrimSpace(fmt.Sprint(incoming["definitionHash"]))
	currentDefinitionHash := firstNonEmptyMaclawAppString(
		stringMapValue(current, "definitionHash"),
		stringMapValue(current, "definition_hash"),
		stringMapValue(current, "definitionFingerprint"),
		stringMapValue(current, "definition_fingerprint"),
	)
	if definitionHash == "" || !strings.EqualFold(definitionHash, strings.TrimSpace(currentDefinitionHash)) {
		return false
	}
	if firstNonEmptyMaclawAppString(stringMapValue(current, "runId"), stringMapValue(current, "run_id")) == "" ||
		firstNonEmptyMaclawAppString(stringMapValue(current, "verifiedAt"), stringMapValue(current, "verified_at")) == "" {
		return false
	}
	// Do not suppress the first artifact-backed proof when the stored evidence
	// only shows a non-artifact result.
	if wantArtifact, _ := incoming["artifactPresent"].(bool); wantArtifact {
		if !maclawAppBoolValue(current, "artifactPresent", "artifact_present") {
			return false
		}
	}
	return true
...
// maclawAppExistingRunEvidence retains compatibility with packages created by
// older desktop builds, which used the snake_case key. Avoiding a migration
// write here is important: it would otherwise create one avoidable Hub
// version merely to rename runtime-only evidence.
func maclawAppExistingRunEvidence(governance map[string]any) any {
	if governance == nil {
		return nil
	}
	if evidence := governance["testEvidence"]; evidence != nil {
		return evidence
	}
	return governance["test_evidence"]
}
...
func (a *App) commitNLSkillDefinition(ctx context.Context, def corelib.NLSkillEntry, action, event string, allowCreate bool) error {
	...
	return a.commitNLSkillDefinitionAfterAdmission(ctx, def, action, event, allowCreate, "operator", report)
}

// commitNLSkillDefinitionAfterAdmission commits a definition whose source-
// specific security scan and admission decision have already completed. This
// lets automatic producers retain their stricter, non-interactive admission
// policy while sharing the same durable config/YAML/index/audit transaction as
// operator-created definitions.
func (a *App) commitNLSkillDefinitionAfterAdmission(ctx context.Context, def corelib.NLSkillEntry, action, event string, allowCreate bool, via string, report *skill.ScanReport) error {
	if a == nil || a.skillExecutor == nil {
		return fmt.Errorf("skill executor not initialized")
	}
	if strings.TrimSpace(def.Name) == "" {
	...
	// Drain asynchronous load-time overlay writers before taking the
	// process-wide mutation lock. Overlay persistence takes this lock itself;
	// taking installMutex first can deadlock while waiting for that writer.
	a.skillExecutor.suspendStatusOverlayPersistence()
	defer a.skillExecutor.resumeStatusOverlayPersistence()
	// Config-only definitions share the same install/delete critical section as
	// directory-backed commits. Keeping the lock at this common boundary avoids
	// races between mixed-source/IM/background registrations and legacy GUI
	// metadata writers without holding it during network or scan work.
	a.installMutex.Lock()
	defer a.installMutex.Unlock()
	a.ensureRemoteInfra()
	// Check the durable queue for every definition mutation, not only active
	// transitions. A disabled/staged write must not proceed while recovery is
	// unreadable or a pending record for this Skill could be bypassed.
	...
	if err := skill.RecordEvolutionEventStrict("skill:definition_"+action+"_started", map[string]string{
		"skill": def.Name, "action": action, "decision": "pending", "via": via,
	...
	committer := &skill.SkillCommitter{
		...
		EntryMerger: mergeNLSkillDefinitionEntry,
	...
	result := committer.Commit(ctx, def.Name, &def, event, map[string]string{
		"skill": def.Name, "action": action, "via": via, "request_id": requestID,
...
// mergeNLSkillDefinitionEntry preserves live execution state while applying
// the definition fields supported by the legacy Register/Update API. The
// generic evolution merger intentionally ignores triggers and treats most
// fields as runtime state, which is correct for repair but would silently
// discard a normal definition update.
func mergeNLSkillDefinitionEntry(dst, src *corelib.NLSkillEntry) {
	if dst == nil || src == nil {
		return
	}
	dst.Description = src.Description
	dst.Triggers = append([]string(nil), src.Triggers...)
	...
	// LastError can carry an explicit lifecycle explanation even before the
	// Skill has any usage, so preserve the established Update semantics here.
	if strings.TrimSpace(src.LastError) != "" {
...
func (a *App) SetNLSkillStatus(name, status string) error {
	...
	// Status-only governance writes still replace the authoritative registry
	// snapshot and refresh routing indexes. Serialize the complete admission →
	// commit window with installs, deletes, and legacy writers.
	a.skillExecutor.suspendStatusOverlayPersistence()
	defer a.skillExecutor.resumeStatusOverlayPersistence()
	a.installMutex.Lock()
	defer a.installMutex.Unlock()
	...
	// Even a no-op must prove that the durable compensation queue is readable;
	// otherwise callers could mistake a successful return for a healthy
	// lifecycle state while an unrelated pending recovery is being hidden.
	if err := skill.CheckEvolutionCompensationQueue(); err != nil {
		return fmt.Errorf("skill status blocked: evolution compensation queue unavailable: %w", err)
	}
	// Setting the lifecycle field to its current value is a genuine no-op. Do
	// not run the activation scan, emit a "requested" audit row, refresh the
	// checked index, or create a durable compensation record for a change that
	// cannot alter the authoritative state. Workflow/policy checks above still
	// run, and this no-op does not clear or bypass any existing blocker.
	if normalizeSkillEntryStatus(current.Status) == normalizeSkillEntryStatus(status) {
		return nil
	}
	...
	committer := &skill.SkillCommitter{
		...
		CompensationMutator: func(record *skill.EvolutionCompensationRecord) {
			// Status-only changes have no directory artifact. Keep the callback
			// explicit so future fields cannot accidentally introduce filesystem
			// cleanup into this governance-only transaction.
...
func (a *App) verifyAndActivateNLSkillWithArgs(name string, runArgs map[string]interface{}) error {
	...
	// The replay and gate run outside the mutation lock so slow sandbox work
	// does not block installs. Reacquire the lock before writing any evidence,
	// re-check admission, and make sure the exact staged definition that was
	// verified is still authoritative. A concurrent replacement must never be
	// promoted using proof for an older step set.
	a.installMutex.Lock()
	defer a.installMutex.Unlock()
	if err := ensureSkillEvolutionMutationAdmission(a, name); err != nil {
		return err
	}
	var latest *corelib.NLSkillEntry
	for _, item := range a.skillExecutor.loadSkills() {
		if item.Name != name {
			continue
		}
		if normalizeSkillEntryStatus(item.Status) != skillEntryStatusStaged || !strings.EqualFold(strings.TrimSpace(item.Source), "auto_discovered") {
			return fmt.Errorf("skill %q changed during verification; retry", name)
		}
		copy := item
		latest = &copy
		break
	...
	if _, latestDigest := stagedVerificationIdentity(name, latest, args); latestDigest != digest {
		return fmt.Errorf("skill %q changed during verification; retry", name)
	}
	staged = latest
	...
	commitCtx := skill.WithEvolutionRequestMetadata(context.Background(), requestID, 1)
	commitResult := committer.Commit(commitCtx, name, &verified, "skill:staged_verified", map[string]string{
...
func (a *App) DeleteNLSkill(name string) error {
	...
	// Drain asynchronous load-time overlay writers before taking installMutex;
	// otherwise a writer waiting on the same lock can deadlock this transaction.
	a.skillExecutor.suspendStatusOverlayPersistence()
	defer a.skillExecutor.resumeStatusOverlayPersistence()
	// Keep the complete quarantine → config → index → audit sequence inside the
	// same process-wide critical section as installs and legacy writers.
	a.installMutex.Lock()
	defer a.installMutex.Unlock()
	if err := ensureSkillEvolutionMutationAdmission(a, name); err != nil {
		return err
	...
	// External directories may contain a valid definition that has not yet
	// reached the asynchronous scanner cache (or has no runnable steps and is
	// therefore omitted by the normal loader).  Deletion still needs a durable
	// quarantine boundary for that directory; discover it with the scanner's
	// unfiltered API rather than falling back to the old direct RemoveAll path.
	if target == nil {
		if cfg, cfgErr := a.LoadConfig(); cfgErr == nil {
			for _, root := range skill.SkillScanRootsWithExternal(cfg.ExternalSkillDirs) {
				for _, discovered := range skill.ScanSkillDirAll(root) {
					if !discovered.MatchesName(name) || strings.TrimSpace(discovered.SkillDir) == "" {
						continue
					}
					cp := discovered
					target = &cp
	...
	oldDir := filepath.Clean(strings.TrimSpace(target.SkillDir))
	...
	commitCtx := skill.WithEvolutionRequestMetadata(context.Background(), requestID, 1)
	result := committer.Commit(commitCtx, name, &candidate, "skill:definition_deleted", auditData)
	// The index refresh can publish a scanner snapshot containing the quarantine
	// directory before post-commit cleanup runs. Invalidate the executor/scanner
	// cache on every terminal result; otherwise a committed delete whose queue
	// cleanup is merely pending can keep serving the deleted Skill from memory.
	clearDeleteCaches := func(committed bool) {
		if a.cachedSkillScanner != nil {
			if quarantine != "" {
				a.cachedSkillScanner.RemoveByDir(quarantine)
			}
			if committed {
				a.cachedSkillScanner.RemoveByName(name)
				// The request may have used a stable alias while the scanner
	...
	if result.State != "committed" || result.CleanupStatus != "clear" {
		clearDeleteCaches(result.State == "committed")
...
func (a *App) renameNLSkillLegacy(oldName, newName string) error {
	...
	// Reject path-like names at the API boundary (user input). In particular,
	// "." and ".." would make filepath.Join escape the Skill parent directory
	// even though they contain no separator.
	if err := validateSkillRenameName(newName); err != nil {
		return err
	}
	a.ensureRemoteInfra()
	if a.skillExecutor == nil {
...
func (a *App) RenameNLSkill(oldName, newName string) error {
	...
	a.skillExecutor.suspendStatusOverlayPersistence()
	defer a.skillExecutor.resumeStatusOverlayPersistence()
	// Directory/config identity changes must not overlap an install or another
	// registry writer; the shared committer does not own an App-level mutex.
	a.installMutex.Lock()
	defer a.installMutex.Unlock()
	if err := ensureSkillEvolutionMutationAdmission(a, oldName); err != nil {
		return err
	...
	// The lookup above intentionally accepts stable aliases (SkillID,
	// HubSkillID, publisher-qualified IDs and directory names).  SkillCommitter
	// resolves the authoritative registry entry by its canonical Name, so pass
	// that identity into the transaction instead of the caller-provided alias;
	// otherwise an alias match would be found here but rejected as
	// skill_not_found inside the committer.
	transactionName := strings.TrimSpace(target.Name)
	if transactionName == "" {
		return fmt.Errorf("skill %q has no canonical name", oldName)
	}
	requestID := fmt.Sprintf("evo_rename_%d", time.Now().UnixNano())
	configRevision := skillEvolutionConfigRevision(a)
	oldDir := filepath.Clean(strings.TrimSpace(target.SkillDir))
	...
	if oldDir != "" {
		...
		if _, err := os.Stat(newDir); err == nil {
			...
			// A permission or I/O error while checking the destination must not
			// be treated as proof that it is absent; otherwise a later rename
			// could overwrite an undiscoverable directory.
	...
	if oldDir != "" {
		...
		if definitionPath != "" {
			var readErr error
			definitionBackup, readErr = os.ReadFile(definitionPath)
			if readErr != nil {
				return fmt.Errorf("read existing skill definition: %w", readErr)
			}
		}
	}
	// Keep the audit identity aligned with the compensation/committer key. If
	// an alias were stored here, a crash after the strict final-audit append but
	// before `transaction_state=committed` would leave recovery unable to match
	// its commit marker and could incorrectly roll back an already-audited
	// rename. The caller's alias is still accepted for lookup; canonical Name is
	// the durable transaction identity.
	auditData := map[string]string{"skill": transactionName, "new_skill": newName, "action": "rename", "decision": "applied", "via": "operator", "request_id": requestID, "attempt": "1", "config_revision": configRevision, "schema_version": "2", "evidence_mode": "none"}
	if err := a.recordSkillEvolutionEventStrict("skill:definition_rename_started", auditData); err != nil {
		return fmt.Errorf("rename audit preflight failed: %w", err)
	}
	candidate := *target
	...
	committer := &skill.SkillCommitter{
		...
		ExternalCommitWithCompensation: func(record *skill.EvolutionCompensationRecord) error {
			...
			// Mark the destination as published only after the directory move has
			// crossed its filesystem boundary. The committer persists this enriched
			// record before touching config/index/audit, so a crash in the gap can
			// restore newDir back to oldDir without deleting the final directory on
			...
			if definitionPath != "" {
				...
				// Replace the canonical YAML identity, not the user-provided alias.
				// Alias-based rename requests are valid, but the top-level name in
				// skill.yaml remains canonical and may not contain the alias text.
				data := replaceYAMLNameField(string(definitionBackup), transactionName, newName)
				if data == "" {
					return fmt.Errorf("skill definition has no matching top-level name")
				}
		...
		ExternalRollback: func() error {
			...
			if definitionPath != "" {
				// An empty but existing definition file is still part of the
				// authoritative pre-image and must be restored after a failed
				// rename. The durable record carries the same empty payload for
		...
		CompensationMutator: func(record *skill.EvolutionCompensationRecord) {
			if oldDir != "" {
				// Rename's destination is the committed directory, not a
				// post-commit cleanup target. Keep an intent-only singular move
				// (oldDir -> newDir) and mark it published from the external
				// callback; recovery can restore it, while committed cleanup will
				// never remove the final newDir.
				record.SetDirectoryBackupIntent(oldDir, newDir)
			}
			if definitionPath != "" {
				record.YAMLPath = filepath.Join(oldDir, filepath.Base(definitionPath))
				record.YAMLExists = true
				record.YAMLBackup = base64.StdEncoding.EncodeToString(definitionBackup)
			}
		},
		ConfigRevision: configRevision,
	}
	commitCtx := skill.WithEvolutionRequestMetadata(context.Background(), requestID, 1)
	result := committer.Commit(commitCtx, transactionName, &candidate, "skill:definition_renamed", auditData)
	// A failed rename may have already published the destination in the checked
	// scanner before rollback, while a committed transaction with pending queue
	// cleanup must not leave the old identity cached. Reconcile both caches on
	// every terminal result instead of returning with a stale in-memory alias.
	clearRenameCaches := func(committed bool) {
		if a.cachedSkillScanner != nil {
			if committed {
				a.cachedSkillScanner.RemoveByName(oldName)
				a.cachedSkillScanner.RemoveByName(transactionName)
				a.cachedSkillScanner.RemoveByDir(oldDir)
			} else {
				if newDir != "" && newDir != oldDir {
					a.cachedSkillScanner.RemoveByDir(newDir)
				}
				// Rollback restores oldDir; leave that identity available while
				// forcing a fresh scan to discard any partial destination snapshot.
				a.cachedSkillScanner.Invalidate()
			}
		}
		if a.skillExecutor != nil {
	...
	if result.State != "committed" || result.CleanupStatus != "clear" {
		clearRenameCaches(result.State == "committed")
		if result.State == "rolled_back" {
	...
	clearRenameCaches(true)
	return nil
}

// validateSkillRenameName constrains a user-provided destination to one safe
// directory component. filepath.Join cleans dot components, so checking only
// for slash characters is insufficient to keep a rename inside the Skill root.
func validateSkillRenameName(name string) error {
	name = strings.TrimSpace(name)
	if name == "" {
		return fmt.Errorf("skill name is required")
	}
	if strings.ContainsRune(name, '\x00') {
		return fmt.Errorf("skill name contains NUL byte")
	}
	if filepath.IsAbs(name) || filepath.VolumeName(name) != "" || name == "." || name == ".." ||
		filepath.Base(name) != name || filepath.Clean(name) != name || strings.ContainsAny(name, "/\\") {
		return fmt.Errorf("skill name must be a single directory component")
	}
	return nil
}

// validateSkillMutationDir rejects relative paths and filesystem roots before
// a rename/delete can move an entire parent tree or create an unrecoverable
// relative compensation target.
func validateSkillMutationDir(path string) error {
	path = strings.TrimSpace(path)
	if path == "" {
		return nil
	}
	if !filepath.IsAbs(path) {
		return fmt.Errorf("skill directory must be an absolute path")
	}
	clean := filepath.Clean(path)
	if filepath.Dir(clean) == clean {
		return fmt.Errorf("skill directory cannot be a filesystem root")
	}
	...
	if info.Mode()&os.ModeSymlink != 0 {
		return fmt.Errorf("skill directory cannot be a symbolic link")
	}
	if !info.IsDir() {
		return fmt.Errorf("skill directory is not a directory")
	}
...
func (a *App) importFileBackedSkillZipPath(selection string) (string, error) {
	...
	a.skillExecutor.suspendStatusOverlayPersistence()
	defer a.skillExecutor.resumeStatusOverlayPersistence()
	// ImportNLSkillZip is a legacy directory publisher and therefore must not
	// race AddSkill/InstallSkill/DeleteSkill or another import in the same App.
	// The filesystem transaction itself remains durable; this mutex only
	// serializes in-process writers while the recovery gate below handles
	// process crashes. Acquire it after the overlay drain to preserve lock order.
	a.installMutex.Lock()
	defer a.installMutex.Unlock()
	// A prior process may have crashed after publishing one or more package
	// directories. Recover all GUI ZIP-import records before accepting another
	// mutation. The queue is global, but this action prefix is owned by the
	// desktop Skill registry, so recovering it here cannot claim AgentService
	// records (which use the distinct agentservice_install prefix).
	if err := skill.CheckEvolutionCompensationQueue(); err != nil {
		return "", fmt.Errorf("Skill ZIP import blocked: compensation queue unavailable: %w", err)
	}
	if _, pending, err := skill.RecoverPendingEvolutionCompensationsForActionPrefixAndScope(
		"import", primaryDir,
		func(skills []corelib.NLSkillEntry) error {
			return a.skillExecutor.withSkillListMutate(func() error {
				return a.skillExecutor.restoreSkillsSnapshot(skills)
			})
		},
		func() error { return a.refreshSkillIndexesAfterMutationChecked("import") },
	); err != nil {
		return "", fmt.Errorf("Skill ZIP import compensation recovery failed: %w", err)
	} else if pending > 0 {
		return "", fmt.Errorf("Skill ZIP import blocked: pending compensation requires review")
	}
	...
	compensation.SetRecoveryScope(primaryDir)
	compensation.TransactionState = "prepared"
	compensation.CleanupStatus = "pending"
	compensation.FinalAuditKind = skill.KindFromEventName("skill:definition_imported")
	affectedSkills := make([]string, 0, len(entries))
	for _, entry := range entries {
		if entry != nil {
			affectedSkills = append(affectedSkills, entry.Name)
		}
	}
	compensation.SetAffectedSkills(affectedSkills)
	...
	rollbackImport := func(cause error) error {
		if cleanupErr := cleanupImportedSkillDirs(plannedDirs); cleanupErr != nil {
			compensation.LastError = fmt.Sprintf("%v (rollback directory cleanup: %v)", cause, cleanupErr)
			compensation.TransactionState = "audit_pending"
			compensation.CleanupStatus = "pending"
			if replaceErr := skill.MarkEvolutionCompensationRollbackFailure(&compensation, cleanupErr); replaceErr != nil {
				return fmt.Errorf("%v; import rollback cleanup failed: %v; persist compensation: %w", cause, cleanupErr, replaceErr)
			}
			return fmt.Errorf("%v; import rollback cleanup incomplete; compensation queued: %v", cause, cleanupErr)
		}
		if a.cachedSkillScanner != nil {
			for _, dir := range plannedDirs {
		...
		if err := a.refreshSkillIndexesAfterMutationChecked(entries[0].Name); err != nil {
			...
			compensation.TransactionState = "audit_pending"
			compensation.CleanupStatus = "pending"
			if replaceErr := skill.MarkEvolutionCompensationRollbackFailure(&compensation, err); replaceErr != nil {
				return fmt.Errorf("%v; import rollback index refresh failed: %v; persist compensation: %w", cause, err, replaceErr)
			}
		...
		if err := skill.ClearEvolutionCompensation(requestID, entries[0].Name, "import"); err != nil {
			...
			compensation.CleanupStatus = "pending"
			if replaceErr := skill.MarkEvolutionCompensationRollbackFailure(&compensation, err); replaceErr != nil {
				return fmt.Errorf("%v; import rollback compensation cleanup failed: %v; persist compensation: %w", cause, err, replaceErr)
			}
			return fmt.Errorf("%v; import rollback incomplete; compensation queued", cause)
		}
	...
	if err := skill.ClearEvolutionCompensation(requestID, entries[0].Name, "import"); err != nil {
		if markErr := skill.MarkEvolutionCompensationCleanupFailure(&compensation, err); markErr != nil {
			return entries[0].Name, fmt.Errorf("import cleanup failed (%v); persist cleanup state: %w", err, markErr)
		}
...
func cleanupImportedSkillDirs(paths []string) error {
	var failures []string
	for i := len(paths) - 1; i >= 0; i-- {
		if strings.TrimSpace(paths[i]) == "" {
			continue
		}
		if err := os.RemoveAll(paths[i]); err != nil && !os.IsNotExist(err) {
			failures = append(failures, fmt.Sprintf("%s: %v", filepath.Base(paths[i]), err))
		}
	}
	if len(failures) > 0 {
		return fmt.Errorf("remove imported skill directories: %s", strings.Join(failures, "; "))
	}
	return nil
}
...
// CleanupStaleSkills is retained only for compatibility with the older
// automatic-maintenance hook. The hook has no request identity, confirmation,
// durable compensation, checked index refresh, or final audit boundary, so it
// must never write the registry directly. Approved maintenance plans own the
// corresponding mutation through SkillCommitter instead.
func (e *SkillExecutor) CleanupStaleSkills() []string {
	if e != nil {
		log.Printf("[skill-maintenance] automatic stale cleanup is disabled; use an approved maintenance plan")
	}
	return nil
}

// CleanupStaleNLSkills remains an error-free Wails compatibility binding. It
// deliberately performs no mutation because its legacy signature cannot carry
// an explicit confirmation or report a durable transaction result. Callers
// must use the approved maintenance-plan flow for stale-Skill retirement.

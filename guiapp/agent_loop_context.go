...
type LoopContext struct {
	...
	// Host is the request-level capability surface supplied through Runtime.Execute.
	// When nil, the loop uses the process GUI host (desktop capture/open).
	Host agentruntime.HostCapabilities
	// RuntimeEventSink is an optional transport-neutral event sink. GUI hosts
	// may install it to receive the same assistant/tool/run event envelope as
	// MaClawSrv; nil preserves the existing Wails/IM callback behavior.
	RuntimeEventSink agentruntime.EventSink
	// Loop-scoped tools unlocked by discover_tool. Not a shared pin.
	discoveredConditionalTools map[string]bool
	// exposedToolNames is the last closed surface rendered for this loop.
	// Discovery status is relative to this set, not the global core catalog.
	exposedToolNames []string
	// discoverToolCalls counts discover_tool invocations on this loop.
...
func (c *LoopContext) setExposedToolNames(names []string) {
	if c == nil {
		return
	}
	copied := append([]string(nil), names...)
	c.mu.Lock()
	c.exposedToolNames = copied
	c.mu.Unlock()
}

func (c *LoopContext) exposedToolNameSet() map[string]bool {
	if c == nil {
		return nil
	}
	c.mu.RLock()
	defer c.mu.RUnlock()
	if len(c.exposedToolNames) == 0 {
		return nil
	}
	out := make(map[string]bool, len(c.exposedToolNames))
	for _, name := range c.exposedToolNames {
		if name != "" {
			out[name] = true
		}
	}
...
func (c *LoopContext) noteDiscoverToolCall() int {
	if c == nil {
		return 0
	}
	c.mu.Lock()

...
func (b *BtwSubAgent) Execute(query string) *BtwResult {
	...
	result := agentruntime.RunAgentTurn(cb, query, nil, b.httpClient)
...
func buildBtwSystemPrompt(h *IMMessageHandler, _ string) string {
	...
	roleName, roleDesc := agentruntime.ResolveRole("", "")
	if cfg, err := h.loadConfig(); err == nil {
		roleName, roleDesc = agentruntime.ResolveRole(cfg.MaclawRoleName, cfg.MaclawRoleDescription)
	}

...
func TestToolUploadSkill_PortabilityGateRollsBackOnCheckedIndexFailure(t *testing.T) {
	tempHome := t.TempDir()
	app := &App{testHomeDir: tempHome}
	cfg, err := app.LoadConfig()
	if err != nil {
	...
	skillDir := filepath.Join(tempHome, "skills", "upload-index-failure")
	if err := os.MkdirAll(filepath.Join(skillDir, "scripts"), 0o755); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(skillDir, "scripts", "run.py"), []byte("print('ok')\n"), 0o644); err != nil {
	...
	absScript := filepath.Join(skillDir, "scripts", "run.py")
	yaml := "name: upload-index-failure\nsteps:\n  - action: bash\n    params:\n      command: python " + absScript + "\n"
	yamlPath := filepath.Join(skillDir, "skill.yaml")
	if err := os.WriteFile(yamlPath, []byte(yaml), 0o644); err != nil {
		t.Fatal(err)
	}
	cfg.NLSkills = append(cfg.NLSkills, corelib.NLSkillEntry{Name: "upload-index-failure", SkillDir: skillDir, Source: "test"})
	if err := app.SaveConfig(cfg); err != nil {
		t.Fatalf("SaveConfig() error = %v", err)
	}
	app.skillExecutor = NewSkillExecutor(app, nil, nil)
	...
	h := &IMMessageHandler{app: app}
	got := h.runUploadPortabilityGate("upload-index-failure")
	if !strings.Contains(got, "索引刷新失败") || !strings.Contains(got, "已回滚") {
		t.Fatalf("runUploadPortabilityGate() = %q", got)
	}
	...
	if string(restored) != yaml {
		t.Fatalf("skill.yaml was not restored after index failure\n got: %q\nwant: %q", restored, yaml)
	}
}

func TestEnsureSkillIDForUploadRollsBackOnCheckedIndexFailure(t *testing.T) {
	tempHome := t.TempDir()
	app := &App{testHomeDir: tempHome}
	cfg, err := app.LoadConfig()
	if err != nil {
	...
	cfg.RemoteEmail = "alice@example.com"
	skillDir := filepath.Join(tempHome, "skills", "ensure-id-rollback")
	if err := os.MkdirAll(skillDir, 0o755); err != nil {
		t.Fatal(err)
	}
	yaml := "name: ensure-id-rollback\ndescription: test\nsteps:\n  - action: bash\n    params:\n      command: echo ok\n"
	yamlPath := filepath.Join(skillDir, "skill.yaml")
	if err := os.WriteFile(yamlPath, []byte(yaml), 0o644); err != nil {
		t.Fatal(err)
	}
	cfg.NLSkills = append(cfg.NLSkills, corelib.NLSkillEntry{Name: "ensure-id-rollback", SkillDir: skillDir, Source: "test"})
	if err := app.SaveConfig(cfg); err != nil {
		t.Fatalf("SaveConfig() error = %v", err)
	}
	app.skillExecutor = NewSkillExecutor(app, nil, nil)
	...
	h := &IMMessageHandler{app: app}
	if got := h.ensureSkillIDForUpload("ensure-id-rollback"); !strings.Contains(got, "已回滚") {
		t.Fatalf("ensureSkillIDForUpload() = %q, want rollback error", got)
	}
	restored, err := os.ReadFile(yamlPath)
	if err != nil {
		t.Fatal(err)

...
func TestCopyExistingPDFDoesNotForceSkillPreferenceEvenWhenBasenameLooksLikeASkill(t *testing.T) {
	cjkSkill := bookPDFTestSkill()
	cjkSkill.Name = "人工智能数学基础"
	app, _ := newSkillExecutorWithEntries(t, bookPDFTestSkill(), cjkSkill, weatherPDFTestSkill())
	h := &IMMessageHandler{app: app}

	for _, text := range []string{
		`将 C:\Users\me\output\book-pdf-v2.2.3.pdf 复制到当前工作区`,
		`将 C:\Users\ma139\.maclaw\workspace\ai-math-handbook\output\人工智能数学基础-v2.2.3.pdf 复制到当前工作区`,
		`copy book-pdf-v2.2.3.pdf into the workspace`,
	} {
		phase := h.initialAgentLoopPhase(text, nil)
		if phase.ForceSkillPreference {
			t.Fatalf("copying an existing PDF must not force skill preference; text=%q phase=%+v", text, phase)
		}
	}

	named := h.initialAgentLoopPhase("使用 book-pdf skill 写手册", nil)
	if !named.ForceSkillPreference || named.PreferredSkillName != bookPDFTestSkill().Name {
		t.Fatalf("naming the skill in prose must still prefer it, got %+v", named)
	}
}

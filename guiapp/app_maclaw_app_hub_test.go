...
func TestInstallMixedSkillReplacementRollsBackIndexFailure(t *testing.T) {
	tmpHome := t.TempDir()
	t.Setenv("USERPROFILE", tmpHome)
	t.Setenv("HOME", tmpHome)
	...
	const skillID = "mixed-replacement-id"
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		switch r.URL.Path {
		case "/api/client/hubcenters":
			_ = json.NewEncoder(w).Encode(map[string]any{"urls": []string{}, "nodes": []any{}})
		case "/api/v1/skills/" + skillID + "/download":
			w.Header().Set("Content-Type", "application/json")
			_, _ = w.Write([]byte(`{"id":"mixed-replacement","name":"mixed-replacement","description":"replacement package","version":"2.0.0","trust_level":"trusted","steps":[{"action":"bash","params":{"command":"echo replacement"},"on_error":"stop"}]}`))
		default:
			http.NotFound(w, r)
		}
	}))
	...
	skillDir := filepath.Join(tmpHome, ".maclaw", "data", "skills", "mixed-replacement")
	if err := os.MkdirAll(skillDir, 0o755); err != nil {
		t.Fatalf("create original skill dir: %v", err)
	}
	if err := os.WriteFile(filepath.Join(skillDir, "SKILL.md"), []byte("old-package"), 0o644); err != nil {
		t.Fatalf("write original skill file: %v", err)
	}
	app := &App{testHomeDir: tmpHome}
	if err := app.SaveConfig(corelib.AppConfig{
		...
		NLSkills: []corelib.NLSkillEntry{{
			Name: "mixed-replacement", SkillDir: skillDir, Status: "active",
			Source: "skillmarket", HubSkillID: skillID, HubVersion: "1.0.0",
		}},
	}); err != nil {
	...
	err := app.installMixedSkillWithIntegrity("skillmarket", skillID, skillID, "", "")
	if err == nil || !strings.Contains(err.Error(), "install not committed") {
		t.Fatalf("replacement install error = %v, want checked-index commit failure", err)
	}
	if refreshCalls < 2 {
		t.Fatalf("index refresh calls = %d, want mutation and rollback refresh", refreshCalls)
	}
	cfg, err := app.LoadConfig()
	if err != nil {
	...
	if len(cfg.NLSkills) != 1 || cfg.NLSkills[0].HubVersion != "1.0.0" {
		t.Fatalf("failed replacement changed config: %#v", cfg.NLSkills)
	}
	data, err := os.ReadFile(filepath.Join(skillDir, "SKILL.md"))
	if err != nil || string(data) != "old-package" {
		t.Fatalf("failed replacement did not restore original package: data=%q err=%v", data, err)
	}
	if summaries, err := cskill.ListEvolutionCompensationSummaries(); err != nil {
		t.Fatalf("list compensation summaries: %v", err)
	} else if len(summaries) != 0 {
		t.Fatalf("rolled back replacement left compensation records: %#v", summaries)
	}
}

...
type codingSubAgentCallbacks struct {
	...
	// It deliberately bypasses lexical/BM25 recall; the host-owned semantic
	// scope and capability catalog decide what is visible for this task.
...
func (c *codingSubAgentCallbacks) BuildSystemPrompt(userText string, isFirstTurn bool) string {
	...
	if !inspectionRole && !operational && (c == nil || c.subagent == nil || !c.subagent.horizonPosture) {
		if c.codingDynamicCapabilityPromptAvailable() {
			...
			if snapshotID := c.codingToolSnapshotID(); snapshotID != "" {
				prompt += fmt.Sprintf("\n## Tool scope snapshot\n- snapshot_id: %s\n- admitted_skills: %d\n- admitted_mcp_tools: %d\n- The tool set is fixed for this task scope; do not infer missing tools from request wording.\n", snapshotID, len(c.matchedSkills), len(c.matchedMCPTools))
			}
...
func (c *codingSubAgentCallbacks) ensureMatchedSkillsSelected() {
	...
	if selected {
		// The other side may have completed after this side was marked selected;
		// always give the finalizer a chance to publish the complete snapshot.
	...
	c.toolScopeMu.Unlock()
	c.finalizeCodingToolScopeSnapshot()
	log.Printf("[coding-subagent] tool scope snapshot=%s skills=%d mcp=%d", c.codingToolSnapshotID(), len(c.matchedSkills), len(c.matchedMCPTools))
}
...
func (c *codingSubAgentCallbacks) BuildTools(userText string) []map[string]interface{} {
	...
	if c.dynamicLifecycleRelaySnapshot() != nil && !codingDynamicKillSwitchClosed() {
		// E5: the first family is rendered exclusively by the bound semantic
		// holder.  Returning the legacy belt here would create a second model
		// surface even though RunLoop normally selects the bound renderer.
		return nil
	}
...
func (c *codingSubAgentCallbacks) ExecuteToolStructured(name, argsJSON string) agent.ToolExecutionResult {
	...
	// Once the semantic holder is attached, the first-family compatibility
	// names are stale model payloads.  Keep the epoch-less host maintenance
	// path available when no holder exists, but never let a bound request fall
	// through to the legacy name dispatcher.

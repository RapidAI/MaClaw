...
func TestBuildRemoteCodingCodeFileEvent_ForceOpensRightHandPanel(t *testing.T) {
	...
	for _, op := range []string{"read", "modify", "create"} {
		...
		if evt.ProjectPath != tabPath {
			t.Fatalf("ProjectPath = %q, want local tab path %q (not the remote SSH path)", evt.ProjectPath, tabPath)
		}
		if evt.FilePath != "/home/user/proj/src/main.go" {
			t.Fatalf("FilePath = %q, want remote display path", evt.FilePath)
		}
		if evt.Language != "go" {
			t.Fatalf("language = %q, want go", evt.Language)
		}
	}
	empty := buildRemoteCodingCodeFileEvent("remote:ssh-1:1:1", "", "/home/user/proj/src/main.go", "package main\n", "", "read", false, false)
	if empty.ProjectPath != "" {
		t.Fatalf("empty route path must stay empty, got %q", empty.ProjectPath)
	}
}

func TestEmitRemoteCodePreview_SkipsWhenLocalTabPathMissing(t *testing.T) {
	app := &App{}
	app.codeEventEmitter = NewCodeEventEmitter(app)
	agent := &RemoteCodingSubAgent{
		sessionID:  "ssh-1",
		projectDir: "/home/testprj-2",
		handler:    &IMMessageHandler{app: app},
	}
	agent.SetSourcePreviewEnabled(true)
	cb := &remoteCodingCallbacks{agent: agent}
	cb.emitRemoteCodePreview("/home/testprj-2/src/updater.cpp", "#include \"updater.h\"\n", "", "read", true, false)
}

func TestRemoteCodingPreviewRouteProjectPath_UsesLocalTabOwner(t *testing.T) {
	tabPath := `C:\Users\ma139\.maclaw\data\tasks\linux-sysinfo`
	agent := &RemoteCodingSubAgent{
		loopCtx:    &LoopContext{UserID: projectSessionOwnerID(tabPath)},
		projectDir: "/home/testprj-2",
		workDir:    "/home/testprj-2",
	}
	got := remoteCodingPreviewRouteProjectPath(agent)
	if got != normalizeProjectSessionPath(tabPath) {
		t.Fatalf("route path = %q, want local tab %q (not remote %q)", got, tabPath, agent.projectDir)
	}
	if remoteCodingPreviewRouteProjectPath(&RemoteCodingSubAgent{projectDir: "/home/testprj-2"}) != "" {
		t.Fatal("missing session owner must not invent a remote project path")
	}
	cb := &remoteCodingCallbacks{agent: agent}
	if cb.previewRouteProjectPath() != got {
		t.Fatalf("callback route path = %q, want %q", cb.previewRouteProjectPath(), got)
	}
}

...
func semanticDocumentReadNeedPresent(needs []tool.CapabilityNeed) bool {
	for _, need := range needs {
		if tool.IsDocumentRead(need.Capability) {
			return true
		}
	}
	return false
...
func (h *IMMessageHandler) semanticActiveLocalDocumentInputsForTurn(rootTaskID, turnID, userID, channel, destination string) ([]semanticTrustedArtifactInput, bool, error) {
	...
	for _, context := range contexts {
		...
		inputs = append(inputs, semanticTrustedArtifactInput{Payload: payload, Format: context.Format, Suffix: agent.DocumentAttachmentTempSuffix(context.CanonicalPath, context.Format)})
	}

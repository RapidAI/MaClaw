...
func TestUnmanagedLegacyHostCatalogOnlyContainsLiveProvisions(t *testing.T) {
	handler := NewIMMessageHandler(&App{testHomeDir: t.TempDir()}, nil)
	live, raw := handler.unmanagedLegacyHostDefinitions()
	if len(live) == 0 {
		t.Fatal("expected a provisioned unmanaged host catalog")
	}
	now := time.Now().UTC()
	for _, def := range live {
		name := extractToolName(def)
		if _, ok := tool.LegacyAdapterProvisionForTool(name, now); !ok {
			t.Errorf("unmanaged catalog leaked unprovisioned host tool %q", name)
		}
	}
	if len(raw) < len(live) {
		t.Fatalf("raw=%d live=%d, raw must not be smaller than the filtered catalog", len(raw), len(live))
	}
}

func TestFilterLegacyDefinitionsToLiveProvisionsDropsDuplicatesAndUnnamed(t *testing.T) {
	live, missing := filterLegacyDefinitionsToLiveProvisions([]map[string]interface{}{
		legacyReviewedDefinition("read_file"),
	...
	if len(live) != 1 || extractToolName(live[0]) != "read_file" {
		t.Fatalf("live = %#v, want only read_file", live)
	}
	if len(missing) != 2 || missing[0] != "(unnamed)" || missing[1] != "unreviewed_host_tool" {
		t.Fatalf("missing = %v, want [(unnamed) unreviewed_host_tool]", missing)
	}
}

func TestClosedLegacyReplacementDropsUnprovisionedHostDefinitionKeepsReviewed(t *testing.T) {
	...
	if err != nil || !planBacked {
		t.Fatalf("mixed surface must keep reviewed tools: rendered=%#v clients=%v planBacked=%v err=%v", rendered, clientNames, planBacked, err)
	}
	if len(clientNames) != 0 {
		t.Fatalf("client names = %v, want none", clientNames)
	}
	if len(rendered) != 1 || extractToolName(rendered[0]) != "read_file" {
		t.Fatalf("replacement definitions = %#v, want only read_file", rendered)
	}
}

func TestClosedLegacyReplacementRejectsWhenNoProvisionedHostDefinition(t *testing.T) {
	host := []map[string]interface{}{
		legacyReviewedDefinition("unreviewed_host_tool"),
	}
	rendered, clientNames, planBacked, err := (&IMMessageHandler{}).renderClosedLegacyReplacementSurface("read file", nil, host, nil)
	if err == nil || planBacked || len(rendered) != 0 || len(clientNames) != 0 {
		t.Fatalf("empty provisioned remainder must close the replacement surface: rendered=%#v clients=%v planBacked=%v err=%v", rendered, clientNames, planBacked, err)
	}
...
func TestAllowsLiveProvisionDeniesUnprovisionedHostToolOnCompatibilitySurface(t *testing.T) {
	surface := newLegacyToolSurface([]map[string]interface{}{
		legacyReviewedDefinition("read_file"),
		legacyReviewedDefinition("unreviewed_host_tool"),
	})
	if !surface.AllowsLiveProvision("read_file") {
		t.Fatal("reviewed host tool must keep live-provision admission")
	}
	if surface.AllowsLiveProvision("unreviewed_host_tool") {
		t.Fatal("unprovisioned host tool must not gain execution admission by being absent from the catalog")
	}
	if surface.AllowsLiveProvision("bash") {
		t.Fatal("a provisioned host tool that was not on this request surface must not execute")
	}
}
